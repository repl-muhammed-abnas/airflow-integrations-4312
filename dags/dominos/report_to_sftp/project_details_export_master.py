...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"dominos_project_details_export_master_dag_{config.instance}",
        description=f"Dominos Project Details Export Master Dag {config.instance}",
        ...
        get_report_details = rail.RepliconReportDetailsOperator(
            ...
            report_name=config.project_details_report_name,
        )
        ...
        upload_csv_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.output_file_path +
                "repliconprojects_details_{{current_time_in_specified_tz('Canada/Eastern', '%Y%m%d')}}.csv",
        )
