...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"dominos_user_details_export_master_dag_{config.instance}",
        description=f"Dominos User Details Export Master Dag {config.instance}",
        ...
        upload_csv_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.output_file_path +
                "repliconusers_details_{{current_time_in_specified_tz('Canada/Eastern', '%Y%m%d')}}.csv",
        )
