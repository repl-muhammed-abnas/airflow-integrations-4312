...
from cohnreznick.user_sync_v1.utils import response_filter,request_payload

def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_users,
        description='Cohnreznick User Sync - Process Users',
        ...
        create_user_log >> get_user_data >> has_valid_data >> rail.Label('No') >> log_invalid_data >> catch_and_log_errors
        has_valid_data >> rail.Label('Yes') >> is_user_available
        is_user_available >> rail.Label('No') >> process_new_user >> wait_for_process_new_user >> catch_and_log_errors
        is_user_available >> rail.Label('Yes') >> process_update_user >> wait_for_process_update_user >> catch_and_log_errors >> log_to_sumo
