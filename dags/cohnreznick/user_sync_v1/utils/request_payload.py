...
def concat_cost_center_values(cost_center_code, cost_center_name):
    return f"""{cost_center_code}{DELIMITER}{cost_center_name}"""

def cost_center_mapper_concatenated_values(cost_center_mapper):
    return  [concat_cost_center_values(item['cost_center_code'], item['cost_center_name']) for item in cost_center_mapper]

def get_costcenter_mapping_for_mapper(cost_center, cost_center_mapper):
    cost_center_mapper_value_concatenated = cost_center_mapper_concatenated_values(cost_center_mapper)
    if cost_center in cost_center_mapper_value_concatenated:
        return "Available"
    return "Not Available"

# pylint: disable=too-many-arguments
def get_timesheet_template_punch_policy_name(locationname, costcentercode, costcentername, employeetype, cost_center_mapper,  timesheet_punch_policy_mapper):

    # for simplifying comparision concatinating both costcenter name and costcenter code
    costcenter_value_for_comparison_in_mapper = concat_cost_center_values(costcentercode, costcentername)

    cost_center_value = get_costcenter_mapping_for_mapper(costcenter_value_for_comparison_in_mapper, cost_center_mapper)

    if "california" in locationname.lower():
        result = list(filter(lambda x: x['location']=='California'
            and employeetype in x['employee_type'] and x['cost_center']== cost_center_value, timesheet_punch_policy_mapper))

        return {
            "timesheet_template_name": result[0]['timesheet_template_name'],
            "punch_policy_name": result[0]['punch_policy_name']
        }

    result = list(filter(lambda x: x['location']=='others'
        and employeetype in x['employee_type'] and x['cost_center']== cost_center_value, timesheet_punch_policy_mapper))
...
def get_payrulescript_name(locationname, employeetype, payrule_mapper):
    if employeetype in ["Exempt Hourly", "Exempt Salary"]:
        return list(filter(lambda x: x['location']=='all'
            and x['employeetype']==employeetype, payrule_mapper))[0]['payrule']

    if "california" in locationname.lower():
        return list(filter(lambda x: x['location']=='California'
            and x['employeetype']==employeetype, payrule_mapper))[0]['payrule']
    return list(filter(lambda x: x['location']=="other"
            and x['employeetype']==employeetype, payrule_mapper))[0]['payrule']
...
def get_process_users_conf(item, config):
    get_timesheet_template_name = get_timesheet_template_punch_policy_name(item['locationname'],item['costcentercode']
        ,item['costcentername'], item['employeetype'],config.COSTCENTER_MAPPER,
        config.TIMESHEET_TEMPLATE_PUNCH_POLICY_MAPPER)['timesheet_template_name']

    get_punch_policy_name = get_timesheet_template_punch_policy_name(item['locationname'],item['costcentercode']
        ,item['costcentername'], item['employeetype'],config.COSTCENTER_MAPPER,
        config.TIMESHEET_TEMPLATE_PUNCH_POLICY_MAPPER)['punch_policy_name']

    get_payrule_name = get_payrulescript_name(item['locationname'], item['employeetype'], config.PAYRULE_MAPPER)

    return {
        **dict(item.items()),
        **{
            'employeenumberdefinitionuri': rail.result('get_user_oefs')['employeenumberdefinitionuri'],
            'timesheettemplatename': get_timesheet_template_name,
            'timesheettemplateuri': rail.find_first_by_attr_and_get_attr(rail.result("get_all_policy_sets"),'displayText',get_timesheet_template_name,"uri"),
            ...
            'payrulescriptname': get_payrule_name,
            'payrulescripturi': rail.find_first_by_attr_and_get_attr(rail.result("get_all_payrule_scripts"),'displayText',get_payrule_name,"uri"),
            ...
            "punch_policy_name":get_punch_policy_name,
            "punch_policy_uri":rail.find_first_by_attr_and_get_attr(rail.result("get_all_policy_sets"),'displayText',get_punch_policy_name,"uri")
                 if get_punch_policy_name else null
        },
        **{
...
def get_put_user_payload(dag_run,workweek):
    ...
    return {
        "user": {
            ...
            ]if not dag_run.conf['punch_policy_name'] else
            [
                {
                    "uri": dag_run.conf['timesheettemplateuri'],
                    "name": null
...
def update_policy_set(dag_run):
    ...
    assigned_punch_policy = rail.result("get_assigned_punch_policy_to_user")

    if not assigned_timesheet_template or ((dag_run.conf['timesheettemplatename'] and dag_run.conf['timesheettemplateuri']) \
        and (dag_run.conf['timesheettemplateuri'] != assigned_timesheet_template['uri'])):
        policy_set_to_assign.append(dag_run.conf['timesheettemplateuri'])
    ...
    if dag_run.conf['punch_policy_name']:
        ...
        and (dag_run.conf['punch_policy_uri'] != assigned_punch_policy['policySet']['uri']))):
