...
MANDATORY_FIELDS_MAPPING = {'entry_id': 'EntryID', 'employee_id': 'Employee ID', 'entry_date': 'Entry Date',
                            'project_code': 'Project Code', 'hours': 'Hours',
                            'task_lvl1_code': 'Task Level 1 Code',
...
def is_timeentry_task_found(record, task_lvl1_details):
    if record['task_lvl1_code'] and task_lvl1_details:
        return True
    return False
...
def map_user_details_with_feed_callable():
    ...
    replicon_billing_rate_data = rail.result('get_all_billing_rates')
    final_data = []

    def find_max_min_date_for_user_in_feed(emp_id):
        user_records_in_feed = sorted(list((map(get_entry_date, filter(
            lambda item: item['employee_id'] == emp_id, feed_data)))), key=lambda x: x[1])
        min_date = null
        max_date = null
    ...
    for record in feed_data:
        user_details = rail.find_first_by_attr_and_get_attr(
            report_user_data, "Employee_ID", record['employee_id'])
        ...
        project_data = list(filter(lambda x: len(x['Task_Name__Full_Path_'].split(' / ')) == 1,list(filter(
            lambda project_data: project_data['Project_Code'] == record['project_code'], project_report_data))))
        task_lvl1_details_via_code = rail.find_first_by_attr_and_get_attr(
            project_data, "Task_Code", record['task_lvl1_code'], default={})
        final_data.append({
            **record,
            **{
                "user_login_name": user_details['Login_Name'],
                "user_uri": user_details['user_uri'],
                ...
                "is_valid_dates": test_timeenrty_daterange(user_details['User_Start_Date'],
                                                           user_details['User_End_Date'], record['entry_date']),
                ...
                "task_lvl_1_details": task_lvl1_details_via_code,
                "task_lvl_2_details": null,
                "task_lvl_1_uri": task_lvl1_details_via_code.get('Task_uri'),
                "task_lvl_2_uri": null,
                "task_to_use_uri": task_lvl1_details_via_code.get('Task_uri') if record['task_lvl1_code'] else null,
                "time_entry_task_found" : is_timeentry_task_found(record, task_lvl1_details_via_code),
                "is_billable": task_lvl1_details_via_code.get('Task_Time___Expense_Entry_Type') != "Non-Billable"
            },
            **{
                "billing_rates": rail.find_first_by_attr_and_get_attr(replicon_billing_rate_data, 'description', record['billing_rate_code'], default={}),
                "is_billing_rate_found": bool(rail.find_first_by_attr_and_get_attr(
                                                replicon_billing_rate_data, 'description', record['billing_rate_code'], default={}))
            }
        })
...
def map_timesheet_with_user_data(user_data_task_id, ts_data_task_id):
    ...
    unique_entry_ids = list(set(map(lambda record: record['entry_id'], user_data)))

    def get_all_data_for_entry_id(item_entry_id):
        data_for_item_entry_id = list(filter(lambda rec: rec['entry_id']==item_entry_id,user_data))
        actual_hours = sum(list(map(lambda entry_id_record: float(entry_id_record['hours']), data_for_item_entry_id)))
        return {
    ...
    def get_map_data(item):
        timesheet_data = get_timesheet_data(item['entry_date'], ts_data)
        return {
            **{
                "billing_rate": json.loads(item['billing_rates'])
            },
            **item,
            **timesheet_data
        }
