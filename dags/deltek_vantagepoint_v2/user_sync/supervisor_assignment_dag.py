...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'{config.company_key} Handles Supervisor Assignment for Users',
        ...
        def get_supervisor_not_assigned_error():
            return {
                'error': 'Supervisor not assigned because it is either not present or is disabled in Replicon'
            }

        add_log_supervisor_not_assigned = rail.PythonOperator(
            task_id='add_log_supervisor_not_assigned',
            python_callable=get_supervisor_not_assigned_error
        )

        def get_downstreamtasks_error(error_message):
            return {
                'error': f'Error in supervisor assignment - {error_message}'
            }
        ...
        can_run_batch_task >> rail.Label('No') >> search_supervisor >> if_supervisor_user_present_and_enabled
        batch_task >> search_supervisor
        if_supervisor_user_present_and_enabled >> rail.Label('Yes') >> search_supervisor_permission >> if_supervisor_permission_not_present
        if_supervisor_permission_not_present >> rail.Label('Yes') >> assign_supervisor_permission >> assign_supervisor >> catch_error
        if_supervisor_permission_not_present >> rail.Label('No') >> assign_supervisor >> catch_error
        if_supervisor_user_present_and_enabled >> rail.Label('No') >> add_log_supervisor_not_assigned >> catch_error
        return dag
