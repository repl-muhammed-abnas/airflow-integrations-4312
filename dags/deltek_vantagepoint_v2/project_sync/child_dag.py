...
from deltek_vantagepoint_v2.project_sync.utils import request_payload, python_callable_method


def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.project_sync_child_dag_id,
        description=f'{config.company_key} Deltek Vantagepoint project and task sync child DAG',
        ...
        def get_downstreamtasks_error(error_message):
            return {
                'error': f'Error in project sync - {error_message}'
            }
        ...
        if_principal_not_found >> rail.Label('No') >> catch_error
