...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.user_webhook_event_dag_id,
        description=f'{config.company_key} Vantagepoint Webhook Event Handler for User Events',
        ...
        is_new_vantagepoint_user_event = rail.IfOperator(
            task_id='is_new_vantagepoint_user_event',
            test=lambda dag_run: (
        ...
        def prepare_user_sync_conf(dag_run):
            ...
            user_sync_clients = clientids.get('user_sync', [])
            company_config = next(
                (client for client in user_sync_clients if client.get('company_key') == company_key),
                None
            )
        ...
        parse_vantagepoint_clientids >> is_new_vantagepoint_user_event

        is_new_vantagepoint_user_event >> rail.Label('Yes') >> trigger_user_sync
        is_new_vantagepoint_user_event >> rail.Label('No') >> should_delete_dagrun

        trigger_user_sync >> should_delete_dagrun
