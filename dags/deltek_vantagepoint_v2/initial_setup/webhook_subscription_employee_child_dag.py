...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.webhook_employee_child_dag_id,
        description=f'{config.company_key} Vantagepoint Employee Webhook Creation',
        ...
        create_workflow_event = rail.VantagepointAPIOperator(
            task_id='create_employee_workflow_event',
        ...
        def extract_webhook_event_id(dag_run):
            ...
            result = rail.result('create_employee_workflow_event')
        ...
        is_action_created = rail.IfOperator(
            ...
            yes_task='link_employee_workflow_action',
            no_task='create_employee_webhook_action'
        )
        ...
        def get_webhook_action_payload():
            ...
            return {
                'ActionID': event_id,
                'WebhookURL': config.user_webhook_url,
        ...
        create_webhook_action = rail.VantagepointAPIOperator(
            task_id='create_employee_webhook_action',
            vp_conn_id='{{ dag_run.conf.vantagepoint_conn_id }}',
        ...
        link_workflow_action = rail.VantagepointAPIOperator(
            task_id='link_employee_workflow_action',
            vp_conn_id='{{ dag_run.conf.vantagepoint_conn_id }}',
            endpoint='/Workflow',
            request_method='PUT',
        ...
        manage_webhook_args = rail.ForEachOperator(
            ...
            items=utils.get_missing_args_for_child,
        ...
        get_existing_conditions_for_event = rail.VantagepointAPIOperator(
            task_id='get_existing_conditions_for_event',
            vp_conn_id='{{ dag_run.conf.vantagepoint_conn_id }}',
            endpoint='/Workflow/dlgWorkflowConditions',
            request_method='GET',
            filters='?ApplicationName=EmployeeICBO',
            trigger_rule='one_success'
        )

        needs_filter_lookups = rail.IfOperator(
            task_id='needs_filter_lookups',
            test=lambda dag_run: utils.filters_need_lookups(dag_run.conf.get('customSettings', {})),
            yes_task='get_all_companies_for_filter',
            no_task='manage_webhook_conditions'
        )
        ...
        manage_webhook_conditions = rail.ForEachOperator(
            task_id='manage_webhook_conditions',
            items=utils.get_conditions_to_update_or_create_for_child,
            start_task='update_or_create_single_condition',
            end_task='foreach_conditions_end',
            trigger_rule='one_success'
        )
        ...
        update_or_create_single_condition = rail.VantagepointAPIOperator(
            ...
            vp_conn_id='{{ dag_run.conf.vantagepoint_conn_id }}',
            endpoint="{{ result('manage_webhook_conditions')['_endpoint'] }}",
            request_method="{{ result('manage_webhook_conditions')['_method'] }}",
            request_body=lambda: {
                k: v for k, v in rail.result('manage_webhook_conditions').items()
                if not k.startswith('_')
            }
        )

        foreach_conditions_end = rail.EmptyOperator(
            task_id='foreach_conditions_end'
        )

        def get_downstreamtasks_error(error_message):
            return {
                'error': f'Error in employee webhook creation - {error_message}'
            }
        ...
        manage_webhook_args >> foreach_args_end >> get_existing_conditions_for_event

        get_existing_conditions_for_event >> needs_filter_lookups
        needs_filter_lookups >> rail.Label('Yes') >> get_all_companies_for_filter >> get_all_orgs_for_filter >> get_all_countries_for_filter >> get_all_states_for_filter >> get_all_labor_codes_for_filter >> manage_webhook_conditions
        needs_filter_lookups >> rail.Label('No') >> manage_webhook_conditions
        manage_webhook_conditions >> update_or_create_single_condition >> foreach_conditions_end
        manage_webhook_conditions >> foreach_conditions_end >> catch_error
