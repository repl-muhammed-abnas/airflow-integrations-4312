...
from alvarezandmarsalholdings.time_export.time_export_s4hc_v2.utils import custom_methods
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        query_valid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM raw_timeexport_data rtd WHERE NULLIF(rtd.employee_id, '') IS NOT NULL AND NULLIF(rtd.break_type_name, '') IS NULL"""
        )

        # Switches off posting to the S4HC API endpoint at runtime; defaults to enabled.
        ...
        has_any_blank_emp_id >> rail.Label("No") >> query_valid_records >> if_can_post_to_api_endpoint

        if_can_post_to_api_endpoint >> rail.Label("Yes") >> trigger_post_to_api >> \
            wait_for_trigger_post_to_api >> catch_error

        if_can_post_to_api_endpoint >> rail.Label("No") >> skip_posting >> catch_error
