...
import json
import rail
from alvarezandmarsalholdings.time_export.time_export_s4hc_v2.utils import custom_methods, response_filters
from alvarezandmarsalholdings.time_export.time_export_s4hc_v2.custom_http_operator.CustomSimpleHttpOperator import CustomSimpleHttpOperator
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        upload_s4hc_payload_data_to_sftp = rail.SFTPUploadFileOperator(
            ...
            '/s4hc_{{dag_run.conf.time_export_name.replace(":", "_")}}_{{ dag_run.conf.index }}' + '.json'
        )

        generate_download_link = rail.GeneratePresignedDownloadUrlOperator(
            task_id='generate_download_link',
            artifact_name="{{result('create_s4hc_json_payload')}}",
            output_file_name='/s4hc_{{dag_run.conf.time_export_name.replace(":", "_")}}_{{ dag_run.conf.index }}' + '.json',
        ...
        send_s4hc_data_to_sap_endpoint = CustomSimpleHttpOperator(
            ...
            data=lambda : json.dumps(rail.load_json_artifact(rail.result("create_s4hc_json_payload"))).encode("utf-8"),
            extra_options={
                'verify': False
            },
        ...
        check_error_message = rail.PythonOperator(
            task_id = "check_error_message",
            python_callable=custom_methods.check_error
        )
        ...
        can_fail_dag = rail.IfOperator(
            ...
            test="{{ get_error_message() | is_truthy and result('check_error_message') | is_falsy }}",
        ...
        final_export_data >> create_s4hc_json_payload >> upload_s4hc_payload_data_to_sftp >> \
        ...
        check_failure_is_post_to_endpoint_failed >> rail.Label("Yes") >> check_error_message >> send_posting_failed_email >> can_fail_dag
