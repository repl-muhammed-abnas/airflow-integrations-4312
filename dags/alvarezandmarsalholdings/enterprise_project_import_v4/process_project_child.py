...
import itertools
import rail
from alvarezandmarsalholdings.enterprise_project_import_v4.utils import request_payload, response_filter, python_callable
from alvarezandmarsalholdings.enterprise_project_import_v4.task.process_project_manager import process_project_manager_task_group
from alvarezandmarsalholdings.enterprise_project_import_v4.task.add_update_tasks import get_task_added_or_updated
...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        def get_project_uri():
            if request_payload.does_project_code_exist() and python_callable.get_task_state('update_project') == 'success':
                return rail.result('update_project')['uri']
            if python_callable.get_task_state('create_project') == 'success':
                return rail.result('create_project')['uri']
            return ''

        has_all_users_staffing_rule = rail.IfOperator(
            task_id='has_all_users_staffing_rule',
            test=request_payload.should_assign_all_users,
            yes_task="assign_all_users_to_project",
        ...
        assign_all_users_to_project = rail.RepliconServiceOperator(
            ...
            data=lambda: request_payload.get_all_users_assignment_payload(get_project_uri())
        )
        ...
        log_orphan_tasks = rail.WriteLogOperator(
            ...
            log='{{ result("create_project_log") }}',
            items=lambda: python_callable.get_all_data_from_json_artifact(rail.result('format_payload_tasks'))[0]['orphan_tasks'],
        ...
        log_unsupported_hierarchy_tasks = rail.WriteLogOperator(
            task_id='log_unsupported_hierarchy_tasks',
            log='{{ result("create_project_log") }}',
            items=lambda: python_callable.get_all_data_from_json_artifact(
                rail.result('format_payload_tasks'))[0]['unsupported_hierarchy'],
            message="Task is below the supported two-level hierarchy",
            severity='Exception',
            properties=lambda item, dag_run: {
                'projectcode': dag_run.conf['Project'],
                'projectname': dag_run.conf['ProjectDescription'],
        ...
        has_billing_resp_users = rail.IfOperator(
            ...
            test=lambda: bool(python_callable.get_all_data_from_json_artifact(rail.result('format_payload_tasks'))[0]['billing_responsibles']),
        ...
        has_task_mandatory_fields = rail.IfOperator(
            ...
            test=lambda: not bool(python_callable.get_all_data_from_json_artifact(rail.result('format_payload_tasks'))[0]['missing_mandatory_fields']),
        ...
        log_mandatory_task_fields_not_present = rail.WriteLogOperator(
            ...
            log='{{ result("create_project_log") }}',
            items=lambda: python_callable.get_all_data_from_json_artifact(rail.result('format_payload_tasks'))[0]['missing_mandatory_fields'],
        ...
        get_resources_add_remove = rail.PythonOperator(
            ...
            python_callable=lambda dag_run:python_callable.get_add_remove_resource(dag_run, config.instance)
        )

        process_remove_resource = rail.TriggerDagRunForEachItemOperator(
            task_id='process_remove_resource',
            items=lambda: rail.result("get_resources_add_remove")['resource_to_remove'],
        ...
        end_batch = rail.EmptyOperator(
            task_id='end_batch'
        )

        has_resources_to_add = rail.IfOperator(
            task_id='has_resources_to_add',
            test=lambda: python_callable.get_task_state('log_mandatory_project_fields_not_present') != 'success'
                and bool((rail.result('get_resources_add_remove') or {}).get('resource_to_add')),
            yes_task='start_add_resource',
            no_task='finish'
        )

        start_add_resource = rail.EmptyOperator(
            task_id='start_add_resource'
        )

        def get_process_add_resource_trigger_id(item):
            modulo = int(item.get('record_id', 0)) % config.ADD_RESOURCE_BATCH_COUNT
            if modulo == 0:
                return config.process_add_resource
            return f"{config.process_add_resource}_batch_{str(modulo)}"

        process_add_resource = rail.trigger_parallel_dagrun(
            task_id='process_add_resource',
            items=lambda: rail.result('get_resources_add_remove')['resource_to_add'],
            parallel_count=config.trigger_parallel_dagrun_count_process_resource,
            trigger_dag_id=get_process_add_resource_trigger_id,
            conf=lambda item: {"task_uri": item['task_uri'], "uris_artifact": item['uris_artifact']},
            execution_timeout=timedelta(days=config.execution_timeout_days),
        )

        get_process_add_resource_dag_ids = rail.PythonOperator(
            task_id='get_process_add_resource_dag_ids',
            python_callable=lambda: list(itertools.chain(
                *list(map(lambda x: rail.result(f'process_add_resource_{x+1}'),
                          range(config.trigger_parallel_dagrun_count_process_resource))))),
        ...
        has_mandatory_fields >> rail.Label(
            "No") >> log_mandatory_project_fields_not_present >> end_batch
        ...
        log_project_success >> has_all_users_staffing_rule
        has_all_users_staffing_rule >> rail.Label(
            'Yes') >> assign_all_users_to_project >> get_all_tasks_for_project
        has_all_users_staffing_rule >> rail.Label('No') >> get_all_tasks_for_project
        get_all_tasks_for_project >> format_payload_tasks >> log_orphan_tasks >> \
        log_unsupported_hierarchy_tasks >> has_billing_resp_users
        ...
        process_tasks_with_parents_exit >> project_team_member_uris
        ...
        has_invalid_team_member_uris >> rail.Label("No") >> process_tasks_resource >> get_final_descendant_task_details
        get_final_descendant_task_details >> get_resources_add_remove >> \
        process_remove_resource >> wait_process_remove_resource >> end_batch
        end_batch >> has_resources_to_add >> rail.Label('Yes') >> start_add_resource >> process_add_resource
        has_resources_to_add >> rail.Label('No') >> finish
        process_add_resource >> get_process_add_resource_dag_ids >> get_valid_hierarchy_tasks >> \
