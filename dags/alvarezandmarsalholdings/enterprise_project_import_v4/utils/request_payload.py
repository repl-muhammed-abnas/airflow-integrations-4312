...
# StaffingRule is a project/header level field. 'A' = All Users Access, which grants
# every user access to the project and all of its tasks. Any other value is not an
# error: the StaffingCostCenter / StaffedEmployee lists drive the assignments instead.
# The two are independent - the staffing lists are always processed, including for 'A'.
STAFFING_RULE_ALL_USERS = 'A'
TEAM_MEMBER_ASSIGNMENT_OPTION_ASSIGN = 'urn:replicon:project-team-member-assignment-option:assign'
...
def get_payload_item_with_artifacts(item):
    ...
    return {
        ...
        "StaffingRule": item.get("StaffingRule", ''),
...
def should_assign_all_users(dag_run):
    """StaffingRule 'A' grants All Users access."""
    return (dag_run.conf.get('StaffingRule') or '').strip().upper() == STAFFING_RULE_ALL_USERS

def get_all_users_assignment_payload(project_uri):
    return {
        "projectUri": project_uri,
        "resourceUri": [f"urn:replicon-tenant:{rail.get_tenant_slug()}:department:1"],
        "projectTeamMemberAssignmentOptionUri": TEAM_MEMBER_ASSIGNMENT_OPTION_ASSIGN
    }
...
def get_updated_task_details(level):
    ...
    def get_updated_tasks(task_id):
        ...
        return null
    # Reverse order of execution (parent_in_system -> level1): each add_task stage rebuilds
    # the map cumulatively, so the latest stage that ran holds the fullest one.
    current_task_in_project = get_updated_tasks('add_task_1parent_in_system') \
        or get_updated_tasks('add_task_1') or current_task_in_project
