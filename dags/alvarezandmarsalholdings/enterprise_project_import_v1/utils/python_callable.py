...
import uuid
import rail
from alvarezandmarsalholdings.enterprise_project_import_v1.utils.request_payload import MANDATORY_FIELDS, get_updated_task_details

null = None
EXISTING_DATE_FORMAT = "%m/%d/%Y"
PAYLOAD_DATE_FORMAT = "%Y-%m-%d"
TASK_ADD_UPDATE_BATCH_SIZE = 400
RESOURCE_BATCH_SIZE = 400

def can_update_task(replicon_task_details, payload_task_details):

    payload_allow_time_entry = replicon_task_details.get('allow_time_entry')
    replicon_allow_time_entry = payload_task_details.get('allow_time_entry')
    ...
    if (payload_allow_time_entry == 'X' and replicon_allow_time_entry == 'Yes') or \
       (payload_allow_time_entry in [False, null, ''] and replicon_allow_time_entry == 'No'):
        return True
...
# This function gets all tasks from level 1, level 2, and level 3 at once.
# We don’t need to create a new collection for these tasks because we already have their details here.
# Creating a new collection would duplicate the data and slow down the performance.
# This way, we keep things faster and avoid storing the same data twice.

def get_tasks_by_level(payload_tasks, current_project_tasks, projectcode):
    """
    Classifies incoming tasks into levels, orphans, and tasks whose parents exist in the system.
    """
    ...
    for task in payload_tasks:
        ...
        parent_code = task.get('parent_task').strip()
        ...
        if parent_code and parent_code not in all_known_codes:
            ...
            grandparent_code = incoming_lookup[parent_code].get('parent_task')
            if grandparent_code and grandparent_code == projectcode:
                # Parent is Level 1 → current task is Level 2
                level2.append(task)
            elif grandparent_code in incoming_lookup:
                # Parent is Level 2 → current task is Level 3
                level3.append(task)
            else:
                # Parent relationship unclear → fallback to orphan
                orphan_tasks.append(task)
        else:
            orphan_tasks.append(task)

    # Re-evaluate orphan_tasks: check if their parent is in parent_in_system
    parent_in_system_codes = {task['taskcode'] for task in parent_in_system}
    remaining_orphans = []

    for orphan in orphan_tasks:
        parent_code = orphan.get('parent_task')
        if not parent_code:
            continue
        if parent_code in parent_in_system_codes:
            parent_task = next((p for p in parent_in_system if p['taskcode'] == parent_code), None)
            if parent_task:
                grandparent_code = parent_task.get('parent_task')
                if grandparent_code and grandparent_code == projectcode:
                    level3.append(orphan)  # parent is level 2 → child is level 3
                else:
                    level3.append(orphan)  # default to level 3 (deep nesting)
            else:
                level2.append(orphan)  # fallback
        else:
...
def get_formatted_payload_tasks(dag_run):
    ...
    task_details = dag_run.conf['A_EnterpriseProjectElementType']
    ...
    for item in task_details:
        ...
        if item.get('StaffingCostCenter') and item['StaffingCostCenter'].get('CostCenter'):
            cost_center_resource[task_code].append(item['StaffingCostCenter']['CostCenter'])

        for staff in item.get('StaffedEmployee', []):
            if staff and staff.get('StaffedEmployee'):
                resources[task_code].append(staff['StaffedEmployee'])
    ...
    valid_tasks = (
        ...
        task_levels['task_level2'] +
        task_levels['task_level3'] + 
...
def get_task_to_add_update_skip(level, task_to_process, project_code):
    ...
    for task in task_to_process:
        ...
        # For the task level1, level2 or level3 first check with task_code
...
def map_task_success_error(all_task_name, task_id, action, _type):
    ...
    for idx, task_res in enumerate(task_add_update_result):
        ...
        if task_res.get("error"):
            ...
            if msg == "The specified Task already exists.":
                status = "Exception"
                msg = "The specified taskname already exists."
            else:
                status = "Error"
...
def batch_resources(data_dict, batch_size=400):
    """Compact version using list comprehension"""
    batched_list = []
    
    for task_uri, uris in data_dict.items():
        batched_list.extend([
            {'task_uri': task_uri, 'uris': uris[i:i + batch_size]}
...
def get_add_remove_resource(dag_run, instance):
    ...
    resources_to_remove = defaultdict(list)
    for task_code, task in assigned_lookup.items():
        ...
        for cc in task.get("costcenters", []):
            if cc["code"] not in received_ccs:
                uris.append(cc['uri'])
        if uris:
            resources_to_remove[task['taskuri']] = uris
    ...
    response['resource_to_add'] = batch_resources(resources_to_add, batch_size=RESOURCE_BATCH_SIZE)
    response['resource_to_remove'] = batch_resources(resources_to_remove, batch_size=RESOURCE_BATCH_SIZE)
...
def get_tasks_for_hierarchy_update():
    ...
    update_hierarchy = rail.result('format_payload_tasks')['task_code_and_name']
