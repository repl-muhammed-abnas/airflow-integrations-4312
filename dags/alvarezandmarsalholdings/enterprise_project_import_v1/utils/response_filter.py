...
from alvarezandmarsalholdings.enterprise_project_import_v1.utils.request_payload import get_updated_task_details
...
def get_resource_assigned(assignment, dag_run):
    resp = {
        "costcenters": [],
        "users": [],
    }
    for item in assignment.get('assignments', []):
        if item['resource']['resourceType']['displayText'] == 'User':
            item['resource']['user']['employeeid'] = find_first_by_attr_and_get_attr(
            load_all_records(dag_run.conf['get_all_users_data']), 'uri', item['resource']['user']['uri'], 'employeeid', ''
        )
            resp['users'].append(item['resource']['user'])
        elif item['resource']['resourceType']['displayText'] == 'Cost Center':
            item['resource']['costCenter']['code'] = find_first_by_attr_and_get_attr(
                load_all_records(dag_run.conf['get_all_costcenters']), 'uri', item['resource']['costCenter']['uri'], 'code'
            )
            resp['costcenters'].append(item['resource']['costCenter'])
...
def get_task_resource(response, dag_run):
    ...
    return list(map(lambda assignment: {
        ...
        'taskcode': get_taskcode_from_taskuri(assignment['taskUri'], get_all_task_details),
        'costcenters': get_resource_assigned(assignment, dag_run)['costcenters'],
        'users': get_resource_assigned(assignment, dag_run)['users']
    }, response))
