...
import rail
from alvarezandmarsalholdings.customer_project_import_v1.utils.python_callable import do_format_logs

def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_log_generation,
        description=f'{config.company_key} Customer Project Import - Process Log Generation',
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            {%- else -%}\
                "+config.alert_email+"\
            {%- endif -%}",
            subject='{{ get_company_key() + " | Customer Project Import is " }} \
