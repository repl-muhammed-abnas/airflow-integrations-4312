...
import rail
from adtalem.user_import.utils.python_callable_method import get_pto_policy_assignments_rehire
...
def create_pto_policy_assignment_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'adtalem_userimport_ptopolicyassignmentrehireuser_2021_{config.instance}',
        description=f'PTO policy assignment rehire user_2021 {config.instance}',
        ...
        is_userdisabled = rail.IfOperator(
            task_id='is_userdisabled',
            test="{{ dag_run.conf.status == 'Disabled' }}",
            yes_task='is_rft_rpt_policyname',
        ...
        get_rft_rpt_policies = rail.PythonOperator(
            ...
            python_callable=get_pto_policy_assignments_rehire,
            op_args=['rft_rpt']
        )
        ...
        def get_policySetScheduleEntries(previous_policy):
            new_policy_sets = rail.result('get_rft_rpt_policies') or rail.result(
        ...
        get_pto_policyset >> is_userdisabled

        is_userdisabled >> rail.Label(
            'Yes') >> is_rft_rpt_policyname
