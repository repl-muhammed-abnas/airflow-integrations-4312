...
from adtalem.user_import.utils.request_payload import get_batchid, get_createuser_payload, get_datetime_obj, get_putdropdownoption_jobcode, get_today_date
...
def create_adduser_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'adtalem_userimport_child_add_user_active2020_crv4.0_{config.instance}',
        description=f'Adtalem_Child_Add User_Production CRV4.0 {config.instance}',
        ...
        batch_task = rail.BatchTaskRunOperator(
            ...
            start_task='process_adduser',
            end_task='catch_and_log_errors',
        )

        process_adduser = rail.EmptyOperator(
            task_id='process_adduser'
        )
        ...
        if_regulartemp_not_equals_t = rail.IfOperator(
            ...
            yes_task="trigger_timeoff_adduser_cr40",
            no_task="is_homestate_equals_to_selected",
        )

        trigger_timeoff_adduser_cr40 = rail.TriggerDagRunOperator(
            task_id='trigger_timeoff_adduser_cr40',
            retries=0,
            trigger_dag_id=f'adtalem_userimport_child_timeoff_add_new_user_crv4.0_{config.instance}',
        ...
        can_run_batch_task >> rail.Label(
            'No') >> process_adduser >> is_loginname_not_present

        is_loginname_not_present >> rail.Label(
            'Yes') >> write_loginname_exception >> catch_and_log_errors

        is_loginname_not_present >> rail.Label(
            'No') >> get_ususer_mappercombination >> get_mapper_entries >> get_employeetype_from_mapper >> is_mapper_present
        ...
        if_regulartemp_not_equals_t >> rail.Label(
            'Yes') >> trigger_timeoff_adduser_cr40 >> get_required_payrule_from_mapper
        if_regulartemp_not_equals_t >> rail.Label(
            'No') >> is_homestate_equals_to_selected
        is_homestate_equals_to_selected >> rail.Label(
            'Yes') >> trigger_timeoff_updateuser_cr20 >> get_required_payrule_from_mapper
        is_homestate_equals_to_selected >> rail.Label(
