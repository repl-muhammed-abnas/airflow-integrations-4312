...
from adtalem.user_import.utils.request_payload import get_batchid, get_datetime_obj, get_putdropdownoption_jobcode, get_today_date
...
def create_updateuser_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'adtalem_userimport_child_update_user_active2020_crv4.0_{config.instance}',
        description=f'Adtalem_Child_Update_User_Production CRV4.0 {config.instance}',
        ...
        batch_task = rail.BatchTaskRunOperator(
            ...
            start_task='process_updateuser',
            end_task='catch_and_log_errors',
        )

        process_updateuser = rail.EmptyOperator(
            task_id='process_updateuser'
        )
        ...
        put_jobcode_dropdowns = rail.RepliconServiceOperator(
            ...
            data=get_putdropdownoption_jobcode
        )
        ...
        is_regulartemp_equals_t = rail.IfOperator(
            ...
            test="{{ dag_run.conf.regulartemp == 'T' and \
                result('parse_csv_user_data')['Regular/Temp'] == 'R' and \
                    dag_run.conf.homestate == 'CA' or dag_run.conf.homestate == 'MA' or \
                        dag_run.conf.homestate == 'OR' or dag_run.conf.homestate == 'NJ' }}",
            yes_task="trigger_timeoff_updateuser_cr20",
            no_task="timeoff_trigger_var",
        )

        trigger_timeoff_updateuser_cr20 = rail.TriggerDagRunOperator(
            task_id='trigger_timeoff_updateuser_cr20',
            retries=0,
            trigger_dag_id=f'adtalem_userimport_child_timeoff_update_user_crv2.0_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
                'lastname': '{{ dag_run.conf.lastname }}',
                'firstname': '{{ dag_run.conf.firstname }}',
                ...
                'useruri': "{{ dag_run.conf.useruri }}",
                'newmapperlookup': "{{ result('get_ususer_mappercombination') if dag_run.conf.ususer == 'yes' else dag_run.conf.paygroup }}"
            }
        )
        ...
        is_timeoff_trigger_yes = rail.IfOperator(
            ...
            yes_task="trigger_timeoff_updateuser_cr40",
            no_task="if_regular_temp_equals_t_2",
        )

        trigger_timeoff_updateuser_cr40 = rail.TriggerDagRunOperator(
            task_id='trigger_timeoff_updateuser_cr40',
            retries=0,
            trigger_dag_id=f'adtalem_userimport_child_timeoff_update_user_crv4.0_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
                "lastname": "{{ dag_run.conf.lastname }}",
                "firstname": "{{ dag_run.conf.firstname }}",
        ...
        if_regular_temp_equals_t_2 = rail.IfOperator(
            ...
            test="{{ dag_run.conf.regulartemp == 'T' and result('parse_csv_user_data')['Regular/Temp'] == 'R' and \
                dag_run.conf.homestate != 'CA' and dag_run.conf.homestate != 'MA' and dag_run.conf.homestate != 'OR' \
                    and dag_run.conf.homestate != 'NJ'}}",
            yes_task="trigger_child_disableuser_timeoff_cr20",
            no_task="log_user_updated",
        )

        trigger_child_disableuser_timeoff_cr20 = rail.TriggerDagRunOperator(
            task_id='trigger_child_disableuser_timeoff_cr20',
            retries=0,
            trigger_dag_id=f'adtalem_userimport_disable_user_timeoff_crv2.0_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
                "lastname": "{{ dag_run.conf.lastname }}",
                "firstname": "{{ dag_run.conf.firstname }}",
        ...
        is_activeleavestatus_contains_t = rail.IfOperator(
            ...
            yes_task="trigger_child_disableuser_timeoff_cr20_2",
            no_task="is_activeleave_status_contains_t_3",
        )

        trigger_child_disableuser_timeoff_cr20_2 = rail.TriggerDagRunOperator(
            task_id='trigger_child_disableuser_timeoff_cr20_2',
            retries=0,
            trigger_dag_id=f'adtalem_userimport_disable_user_timeoff_crv2.0_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        can_run_batch_task >> rail.Label(
            'No') >> process_updateuser >> get_ususer_mappercombination >> get_user_customfieldgroupuri
        ...
        is_activeleavestatus_contains_t >> rail.Label(
            'Yes') >> trigger_child_disableuser_timeoff_cr20_2 >> is_activeleavestatus_not_same
