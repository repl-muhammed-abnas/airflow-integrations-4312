...
from adtalem.user_import.utils.request_payload import get_put_timeoffpolicywithinitialbalance_terminationdate

# config :
# https://github.com/replicon/airflow-integrations/blob/main/dags/adtalem/user_import/config.py


def create_child_put_0_balance_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'adtalem_userimport_put_0_balance_for_update_users_cr14.0_{config.instance}',
        description=f'Adtalem Put 0 balance update users CRV14.0 {config.instance}',
        ...
        put_timeoffpolicy_with_initial_balance_as_0 = rail.RepliconServiceOperator(
            ...
            data=get_put_timeoffpolicywithinitialbalance_terminationdate
        )
...
rail.for_each_instance(create_child_put_0_balance_dag)
