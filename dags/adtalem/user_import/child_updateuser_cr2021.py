...
from adtalem.user_import.utils.request_payload import get_batchid, get_datetime_obj, get_today_date
from adtalem.user_import.utils.response_filter import get_permissions_to_assign_updateuser, get_putlocationschedule_user
from adtalem.user_import.utils.response_filter import get_putpayrulescheduleentries_user, get_studentworker_dropdown_uri
...
def create_updateuser_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'adtalem_userimport_child_update_user_v2_2021_{config.instance}',
        description=f'Adtalem Userimport Child_Update_User_CR2021_V1 {config.instance}',
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task='batch_task',
            no_task='process_updateuser_cr2021'
        )
        ...
        batch_task = rail.BatchTaskRunOperator(
            ...
            start_task='process_updateuser_cr2021',
            end_task='catch_and_log_errors',
        )

        process_updateuser_cr2021 = rail.EmptyOperator(
            task_id='process_updateuser_cr2021'
        )
        ...
        parse_csv = rail.LoadCSVFileOperator(
            ...
            headers=['User First Name', 'User Last Name', 'User Email', 'User Status', 'User Start Date',
                     ...
                     'Standard Hours', 'Department Number', 'Work Location', 'Salary Grade Code']
        )
        ...
        is_activeleavestatus_not_contains_t = rail.IfOperator(
            ...
            yes_task="get_replicon_mappercombination",
            no_task="is_activeleavestatus_contains_t",
        )

        get_replicon_mappercombination = rail.PythonOperator(
            task_id='get_replicon_mappercombination',
            python_callable=python_callable_method.get_replicon_mappercombination_from_report
        )
        ...
        get_mapper_entries = rail.PythonOperator(
            ...
            op_args=[
                "{{ result('get_ususer_mappercombination') if dag_run.conf.ususer == 'yes' else dag_run.conf.paygroup }}",
                'new']
        )

        get_replicon_mapper_entries = rail.PythonOperator(
            task_id='get_replicon_mapper_entries',
            python_callable=python_callable_method.get_replicon_mapper_entries_from_adtalem_mapperfile
        )
        ...
        if_userstatus_is_disabled = rail.IfOperator(
            ...
            no_task="get_employeetype_from_mapper",
        )
        ...
        update_enddate = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "dateRange": {
                    "startDate": get_datetime_obj(dag_run.conf['startdate']),
                    "endDate": get_datetime_obj(dag_run.conf['enddate'])
                }
            }
        )
        ...
        is_paygroup_present = rail.IfOperator(
            ...
            no_task="get_authtype_from_mapper",
        )
        ...
        get_authtype_from_mapper = rail.PythonOperator(
            task_id='get_authtype_from_mapper',
            python_callable=python_callable_method.get_mapper_entry_value,
            op_args=['Authentication Type']
        )

        is_authtype_mapper_present = rail.IfOperator(
            task_id='is_authtype_mapper_present',
            test="{{ result('get_authtype_from_mapper') | is_truthy \
                and result('get_authtype_from_mapper') == 'SSO' }}",
            yes_task="set_sso_auth_user",
            no_task="get_required_policysets_to_assign",
        )

        set_sso_auth_user = rail.RepliconServiceOperator(
            task_id='set_sso_auth_user',
        ...
        should_update_timeoff_approvalpath = rail.IfOperator(
            ...
            no_task='is_employeetype_from_mapper_present'
        )
        ...
        is_employeetype_from_mapper_present = rail.IfOperator(
            task_id='is_employeetype_from_mapper_present',
            test="{{ result('get_employeetype_from_mapper') | is_truthy and \
                result('get_employeetype_from_mapper') != result('parse_csv_user_data')['Employee Type'] }}",
        ...
        is_timesheetperiod_employeetype = rail.IfOperator(
            ...
            no_task='get_required_departmenturi'
        )
        ...
        is_timesheetperiod_department = rail.IfOperator(
            ...
            test="{{ result('get_required_timesheet_period') | \
                is_truthy and result('get_required_timesheet_period') == 'Department' and result('get_required_departmenturi') | \
                    is_truthy and result('get_required_timesheet_period') != result('parse_csv_user_data')['Timesheet Period Type'] }}",
        ...
        is_repliconuser_salaryhourly_present = rail.IfOperator(
            ...
            test="{{ result('parse_csv_user_data')['Salary/Hourly'] | is_truthy \
                and result('parse_csv_user_data')['Salary/Hourly'] | lower == 'h' }}",
        ...
        is_jobcode_present = rail.IfOperator(
            ...
            no_task="search_chamberlain_jobcode_mapper_newjobcode",
        )
        ...
        search_chamberlain_jobcode_mapper_newjobcode = rail.PythonOperator(
            task_id='search_chamberlain_jobcode_mapper_newjobcode',
            python_callable=python_callable_method.search_chamberlain_jobcode_mapper,
            op_args=['{{ dag_run.conf.jobcode }}']
        )

        search_chamberlain_jobcode_mapper_oldjobcode = rail.PythonOperator(
            task_id='search_chamberlain_jobcode_mapper_oldjobcode',
            python_callable=python_callable_method.search_chamberlain_jobcode_mapper,
            op_args=["{{ result('parse_csv_user_data')['Job Code'] }}"]
        )

        get_trigger_pto_policyupdate = rail.PythonOperator(
            task_id='get_trigger_pto_policyupdate',
            python_callable=python_callable_method.get_trigger_pto_policyupdate
        )
        ...
        is_paygrade_present = rail.IfOperator(
            ...
            test="{{ dag_run.conf.paygrade | is_truthy and \
                    dag_run.conf.paygrade != result('parse_csv_user_data')['Salary Grade Code'] }}",
            yes_task="get_us_timeoff_policy_mapper_fto_s_existing",
            no_task="is_departmentnumber_present",
        )

        get_us_timeoff_policy_mapper_fto_s_existing = rail.PythonOperator(
            task_id='get_us_timeoff_policy_mapper_fto_s_existing',
            python_callable=python_callable_method.get_us_timeoff_policy_mapper,
            op_args=[
                "{{ result('parse_csv_user_data')['Salary Grade Code'] }}", 'S']
        )

        get_us_timeoff_policy_mapper_fto_any_existing = rail.PythonOperator(
            task_id='get_us_timeoff_policy_mapper_fto_any_existing',
            python_callable=python_callable_method.get_us_timeoff_policy_mapper,
            op_args=[
                "{{ result('parse_csv_user_data')['Salary Grade Code'] }}", 'Any']
        )

        get_us_timeoff_policy_mapper_fto_s_new = rail.PythonOperator(
            task_id='get_us_timeoff_policy_mapper_fto_s_new',
            python_callable=python_callable_method.get_us_timeoff_policy_mapper,
            op_args=['{{ dag_run.conf.paygrade }}', 'S']
        )

        get_us_timeoff_policy_mapper_fto_any_new = rail.PythonOperator(
            task_id='get_us_timeoff_policy_mapper_fto_any_new',
            python_callable=python_callable_method.get_us_timeoff_policy_mapper,
            op_args=['{{ dag_run.conf.paygrade }}', 'Any']
        )

        get_flex_existing_new_vars = rail.PythonOperator(
            task_id='get_flex_existing_new_vars',
            python_callable=python_callable_method.get_flex_existing_new_vars,
            op_args=["{{ result('parse_csv_user_data')['Salary/Hourly'] }}",
                     '{{ dag_run.conf.paygroup }}', "{{ result('parse_csv_user_data')['Salary Grade Code'] }}"]
        )

        update_salarygradecode_udf = rail.RepliconServiceOperator(
            task_id='update_salarygradecode_udf',
            endpoint="/services/CustomFieldService1.svc/UpdateTextValue",
            data={
                "objectUri": "{{ dag_run.conf.useruri }}",
        ...
        (should_update_supervisor,
         finish_supervisor_assignment) = process_supervisor_assignment_task_group(is_update_user=True)
        ...
        get_locationschedule_user = rail.RepliconServiceOperator(
            ...
            data_handler=get_putlocationschedule_user
        )
        ...
        get_payrule_schedule_entries = rail.RepliconServiceOperator(
            ...
            data_handler=get_putpayrulescheduleentries_user
        )
        ...
        get_permissions_to_assign = rail.RepliconServiceOperator(
            ...
            data_handler=get_permissions_to_assign_updateuser
        )
        ...
        is_jobcode_present2 = rail.IfOperator(
            ...
            no_task="is_regulartemp_equals_t",
        )
        ...
        is_regulartemp_equals_t = rail.IfOperator(
            task_id='is_regulartemp_equals_t',
            test="{{ dag_run.conf.regulartemp == 'T' and \
                result('parse_csv_user_data')['Regular/Temp'] == 'R' }}",
            yes_task="trigger_child_disableuser_timeoff_cr14",
            no_task="timeoff_trigger_var",
        )

        trigger_child_disableuser_timeoff_cr14 = rail.TriggerDagRunOperator(
            task_id='trigger_child_disableuser_timeoff_cr14',
            retries=0,
        ...
        timeoff_trigger_var = rail.PythonOperator(
            task_id='timeoff_trigger_var',
            python_callable=python_callable_method.get_timeoff_trigger_var
        )

        temp_to_regular_var = rail.PythonOperator(
            task_id='temp_to_regular_var',
            python_callable=python_callable_method.get_temp_to_regular_var
        )

        is_timeoff_trigger_yes = rail.IfOperator(
            task_id='is_timeoff_trigger_yes',
            test="{{ result('timeoff_trigger_var') == 'yes' }}",
            yes_task="trigger_timeoff_updateuser_cr2021_v1",
            no_task="if_regular_temp_equals_t_2",
        )

        trigger_timeoff_updateuser_cr2021_v1 = rail.TriggerDagRunOperator(
            task_id='trigger_timeoff_updateuser_cr2021_v1',
            retries=0,
            trigger_dag_id=f'adtalem_userimport_child_timeoff_update_user_cr2021_v1_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
                "lastname": "{{ dag_run.conf.lastname }}",
                "firstname": "{{ dag_run.conf.firstname }}",
                ...
                "filenumber": "{{ dag_run.conf.filenumber }}",
                "startdate": "{{ dag_run.conf.startdate }}",
                "rehiredate": "{{ current_time('%m/%d/%Y') if result('temp_to_regular_var') == 'yes' \
                    else dag_run.conf.rehiredate }}",
                "servicedate": "{{ dag_run.conf.servicedate }}",
                "colleaguednumber": "{{ dag_run.conf.colleaguednumber }}",
                "newmapperlookup": "{{ result('get_ususer_mappercombination') if dag_run.conf.ususer == 'yes' \
                    else dag_run.conf.paygroup }}",
                "worklocation": "{{ dag_run.conf.worklocation }}",
                "useruri": "{{ dag_run.conf.useruri }}",
                "terminationdate": "{{ dag_run.conf.terminationdate }}",
                "userstatus": "{{ result('parse_csv_user_data')['User Status'] }}",
                "ususer": "{{ dag_run.conf.ususer }}",
                "chamberlain": "{{ 'yes' if result('get_trigger_pto_policyupdate') == 'yes' else 'no' }}",
                'previousfullparttimevalue': "{{ result('parse_csv_user_data')['Full/Part Time'] }}",
                'previoushomestate': "{{ result('parse_csv_user_data')['Home State'] }}",
                'type': "{{ 'rehire' if result('enable_userprofile') else 'update' }}",
                'paygrade': '{{ dag_run.conf.paygrade }}',
                'flextobeassigned': "{{ 'yes' if result('get_flex_existing_new_vars') and \
                    result('get_flex_existing_new_vars').flex_existing_var != \
                        result('get_flex_existing_new_vars').flex_new_var else 'no' }}",
                'previousjobcode': "{{ result('parse_csv_user_data')['Job Code'] }}",
                'currentpaygroup': "{{ result('parse_csv_user_data')['Work Location'] }}"
            }
        )

        if_regular_temp_equals_t_2 = rail.IfOperator(
            task_id='if_regular_temp_equals_t_2',
            test="{{ result('timeoff_trigger_var') == 'no' and \
                result('get_trigger_pto_policyupdate') == 'yes' }}",
            yes_task="trigger_timeoff_updateuser_cr2021_v1_2",
            no_task="log_user_updated",
        )

        trigger_timeoff_updateuser_cr2021_v1_2 = rail.TriggerDagRunOperator(
            task_id='trigger_timeoff_updateuser_cr2021_v1_2',
            retries=0,
            trigger_dag_id=f'adtalem_userimport_child_timeoff_update_user_cr2021_v1_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
                ...
                'type': "{{ 'rehire' if result('enable_userprofile') else 'update' }}",
                'paygrade': '{{ dag_run.conf.paygrade }}',
                'flextobeassigned': "{{ 'yes' if result('get_flex_existing_new_vars').flex_existing_var != \
                    result('get_flex_existing_new_vars').flex_new_var else 'no' }}",
                'previousjobcode': "{{ result('parse_csv_user_data')['Job Code'] }}",
                'currentpaygroup': "{{ result('parse_csv_user_data')['Work Location'] }}"
            }
        ...
        is_activeleavestatus_contains_t = rail.IfOperator(
            ...
            yes_task="trigger_child_disableuser_timeoff_cr14_2",
            no_task="is_activeleave_status_contains_t_3",
        )

        trigger_child_disableuser_timeoff_cr14_2 = rail.TriggerDagRunOperator(
            task_id='trigger_child_disableuser_timeoff_cr14_2',
            retries=0,
            trigger_dag_id=f'adtalem_userimport_child_disable_user_timeoff_crv14.0_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        if_activeleavestatus_dropdown_present2 = rail.IfOperator(
            ...
            no_task="trigger_check_supervisor_directreports",
        )
        ...
        trigger_check_supervisor_directreports = rail.TriggerDagRunOperator(
            task_id='trigger_check_supervisor_directreports',
            retries=0,
            trigger_dag_id=f'adtalem_userimport_check_supervisor_direct_reports_crv2.0_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
                "useruri": "{{ dag_run.conf.useruri }}",
                "supervisorname": "{{ dag_run.conf.firstname }} {{ dag_run.conf.lastname }}"
            }
        )
        ...
        trigger_delete_timeoff_bookings_child_v1 = rail.TriggerDagRunOperator(
            ...
            conf=lambda dag_run: {
                ...
                "terminationdate": get_datetime_obj(dag_run.conf['terminationdate'])
            }
        )
        ...
        write_update_userlog = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                'login_name': dag_run.conf['loginname'],
                'status': rail.result('log_user_updated') if rail.result('log_user_updated') else (
                    rail.result('log_disabled') if rail.result('log_disabled') else rail.result('log_disabled2')),
                'failure_reason': ''
            }
        )
        ...
        can_run_batch_task >> rail.Label(
            'No') >> process_updateuser_cr2021 >> get_ususer_mappercombination >> get_user_customfieldgroupuri
        ...
        is_activeleavestatus_not_contains_t >> rail.Label(
            'Yes') >> get_replicon_mappercombination >> get_mapper_entries >> get_replicon_mapper_entries >> \
            if_userstatus_is_disabled
        if_userstatus_is_disabled >> rail.Label(
            'Yes') >> enable_userprofile >> get_employeetype_from_mapper
        if_userstatus_is_disabled >> rail.Label(
            'No') >> get_employeetype_from_mapper >> is_firstname_present
        ...
        is_department_uri_present >> rail.Label(
            'Yes') >> update_department_user >> get_authtype_from_mapper
        is_department_uri_present >> rail.Label(
            'No') >> get_authtype_from_mapper
        is_paygroup_present >> rail.Label(
            'No') >> get_authtype_from_mapper
        get_authtype_from_mapper >> is_authtype_mapper_present
        is_authtype_mapper_present >> rail.Label(
            'Yes') >> set_sso_auth_user >> get_required_policysets_to_assign
        is_authtype_mapper_present >> rail.Label(
        ...
        should_update_timeoff_approvalpath >> rail.Label(
            'Yes') >> update_timeoff_approvalpath_user >> is_employeetype_from_mapper_present
        should_update_timeoff_approvalpath >> rail.Label(
            'No') >> is_employeetype_from_mapper_present

        is_employeetype_from_mapper_present >> rail.Label(
        ...
        is_employeetype_timesheetperiod_uri_present >> rail.Label(
            'Yes') >> put_employeetype_timesheetperiod_uri >> get_required_departmenturi
        is_employeetype_timesheetperiod_uri_present >> rail.Label(
            'No') >> get_required_departmenturi

        is_timesheetperiod_employeetype >> rail.Label(
            'No') >> get_required_departmenturi

        get_required_departmenturi >> is_timesheetperiod_department
        ...
        is_jobcode_dropdown_present >> rail.Label(
            'Yes') >> update_jobcode_udf >> search_chamberlain_jobcode_mapper_newjobcode
        is_jobcode_dropdown_present >> rail.Label(
            'No') >> search_chamberlain_jobcode_mapper_newjobcode
        is_jobcode_present >> rail.Label(
            'No') >> search_chamberlain_jobcode_mapper_newjobcode

        search_chamberlain_jobcode_mapper_newjobcode >> search_chamberlain_jobcode_mapper_oldjobcode >> \
            get_trigger_pto_policyupdate >> required_batchid >> is_batchid_udf_present
        ...
        is_worklocation_present >> rail.Label(
            'Yes') >> update_worklocation_udf >> is_paygrade_present
        is_worklocation_present >> rail.Label(
            'No') >> is_paygrade_present
        is_paygrade_present >> rail.Label(
            'Yes') >> get_us_timeoff_policy_mapper_fto_s_existing >> \
            get_us_timeoff_policy_mapper_fto_any_existing >> get_us_timeoff_policy_mapper_fto_s_new >> \
            get_us_timeoff_policy_mapper_fto_any_new >> get_flex_existing_new_vars >> \
            update_salarygradecode_udf >> is_departmentnumber_present
        is_paygrade_present >> rail.Label(
        ...
        is_studentworker_dropdown_present >> rail.Label(
            'Yes') >> update_student_worker_udf >> is_regulartemp_equals_t
        is_studentworker_dropdown_present >> rail.Label(
            'No') >> is_regulartemp_equals_t
        is_studentworker_udf_present >> rail.Label(
            'No') >> is_regulartemp_equals_t
        is_jobcode_present2 >> rail.Label(
            'No') >> is_regulartemp_equals_t
        is_regulartemp_equals_t >> rail.Label(
            'Yes') >> trigger_child_disableuser_timeoff_cr14 >> timeoff_trigger_var
        is_regulartemp_equals_t >> rail.Label(
            'No') >> timeoff_trigger_var
        timeoff_trigger_var >> temp_to_regular_var >> is_timeoff_trigger_yes
        is_timeoff_trigger_yes >> rail.Label(
            'Yes') >> trigger_timeoff_updateuser_cr2021_v1 >> if_regular_temp_equals_t_2
        is_timeoff_trigger_yes >> rail.Label(
            'No') >> if_regular_temp_equals_t_2
        if_regular_temp_equals_t_2 >> rail.Label(
            'Yes') >> trigger_timeoff_updateuser_cr2021_v1_2 >> log_user_updated
        if_regular_temp_equals_t_2 >> rail.Label(
            'No') >> log_user_updated
        ...
        is_activeleavestatus_contains_t >> rail.Label(
            'Yes') >> trigger_child_disableuser_timeoff_cr14_2 >> is_activeleavestatus_not_same
        ...
        if_activeleavestatus_dropdown_present2 >> rail.Label(
            'Yes') >> update_activeleavestatus_udf2 >> trigger_check_supervisor_directreports
        if_activeleavestatus_dropdown_present2 >> rail.Label(
            'No') >> trigger_check_supervisor_directreports
        trigger_check_supervisor_directreports >> disable_login >> is_termination_date_present2
