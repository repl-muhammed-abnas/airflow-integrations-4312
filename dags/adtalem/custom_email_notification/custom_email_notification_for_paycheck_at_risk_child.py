...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'{config.company_key}_custom_email_notification_for_paycheck_at_risk_child_{config.instance}',
        description=f'Live|Adtalem_call recipe to send custom email notification for paycheck at risk {config.instance}',
        ...
        run_report_for_not_submitted_timesheet_group_entry, run_report_for_not_submitted_timesheet_group_exit = rail.run_report(
            ...
            report_params=request_payload.get_report2_params_for_paycheck
        )
        ...
        query_distinct_data_list = rail.QueryCollectionOperator(
            task_id='query_distinct_data_list',
            query="""SELECT DISTINCT  raw_data.usersupervisorname FROM  raw_data WHERE  raw_data.usersupervisorname IS NOT NULL""",
        )
        has_distinct_data = rail.IfOperator(
            task_id='has_distinct_data',
            test="{{ result('query_distinct_data_list', 'length') > 0 }}",
            yes_task='get_query_distinct_data',
            no_task='finish'
        )
        get_query_distinct_data = rail.PythonOperator(
            task_id="get_query_distinct_data",
            python_callable=python_callable.get_timesheet_period_formated_detail,
        )
        ...
        custom_email_notification_timesheet_reminder_for_paycheck_at_risk_child = rail.TriggerDagRunForEachItemOperator(
            task_id='custom_email_notification_timesheet_reminder_for_paycheck_at_risk_child',
            retries=0,
            items=lambda: rail.result('get_query_distinct_data'),
            trigger_dag_id=f'{config.company_key}_custom_email_notification_send_individual_for_paycheck_at_risk_child_{config.instance}',
            execution_timeout=timedelta(days=14),
            conf=lambda item: {
                "supervisor": item.get('supervisor'),
                "daterangevalue": item.get('daterangevalue'),
                "supervisoremail": item.get('supervisoremail'),
                "user": item.get('user'),
        ...
        wait_for_timesheet_reminder_for_paycheck_at_risk_child = rail.WaitForDagRunsSensor(
            task_id='wait_for_timesheet_reminder_for_paycheck_at_risk_child',
            execution_timeout=timedelta(days=14),
            dag_runs='{{ result("custom_email_notification_timesheet_reminder_for_paycheck_at_risk_child") }}'
        )
        ...
        send_log_email = rail.EmailOperator(
            ...
            html_content="templates/emails/email_for_timesheet_reminder_for_paycheck.html",
            params=None,
        )
        ...
        has_timesheet_period >> rail.Label(
            'No') >> finish
        get_not_submitted_timesheet_report_details >> run_report_for_not_submitted_timesheet_group_entry >> run_report_for_not_submitted_timesheet_group_exit >> load_report_data_csv >> compose_csv_with_headers >> create_collection_from_csv >> query_data_list >> query_distinct_data_list >> has_distinct_data
        has_distinct_data >> rail.Label(
            'Yes') >> get_query_distinct_data >> get_query_data
        has_distinct_data >> rail.Label(
            'No') >> finish
        get_query_data >> custom_email_notification_timesheet_reminder_for_paycheck_at_risk_child >> wait_for_timesheet_reminder_for_paycheck_at_risk_child >> get_all_logs
