...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'{config.company_key}_custom_email_notification_individual_timesheet_reminder_for_accelerated_child_{config.instance}',
        description=f'Live|Adtalem_call recipe to send individual custom email notification for Timesheet Reminder Accelerated {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.child_max_active_runs,
    ) as dag:

        html_body = "templates/emails/individual/email_for_accelerated_timesheet.html"
        ...
        send_reminder_email = rail.RepliconServiceOperator(
            ...
            data=request_payload.get_accelerated_payload_sendemail
        )
        ...
        log_success = rail.WriteLogOperator(
            ...
            properties={
                "JobID": "{{ dag_run_ecid() }}",
                "Date": datetime.now().strftime("%d/%m/%Y"),
                "Type": "Timesheet Reminder - Accelerated",
                "User Name | Supervisor Name": "{{ dag_run.conf.user }} | {{ dag_run.conf.supervisor }}",
