...
import rail
from adtalem.custom_email_notification.utils import request_payload
null = None
...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'{config.company_key}_custom_email_notification_send_individual_for_pay_at_risk_child_{config.instance}',
        description=f'Live|Adtalem_call recipe to send individual custom email notification for pay at risk {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=1,
    ) as dag:

        html_body = "templates/emails/individual/email_for_timesheet_reminder_pay_at_risk_format.html"
        ...
        send_reminder_email = rail.RepliconServiceOperator(
            ...
            data=request_payload.get_pay_payload_sendemail
        )
