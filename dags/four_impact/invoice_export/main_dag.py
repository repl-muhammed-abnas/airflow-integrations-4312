...
from four_impact.invoice_export.request_payload import get_queuedforsync_invoice


def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"4impact_invoice_export_master_{config.instance}",
        description=f'4Impact Invoice Export Master {config.instance}',
        ...
        trigger_invoice_child_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f"4impact_invoice_export_child_{config.instance}",
        ...
        log_dagrun_details_to_table = rail.PostDagRunDetailsToRepliconOperator(
            ...
            connector_name='xero',
            integration_type=f"4impact_invoice_export_master_{config.instance}"
        )
