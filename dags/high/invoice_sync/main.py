...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        ...
        description= f'Sync new/updated invoice in Replicon to Xero_{config.instance}',
        ...
        json_formatter = rail.PythonOperator(
            ...
            python_callable= python_callable.json_formatter_get_inoivce
        )
        ...
        get_queued_for_sync_invoice >> has_list_data >> rail.Label('Yes') >> get_required_invoices >>\
        has_invoice_data >> rail.Label('Yes') >> json_formatter >> process_child_dag >> wait_for_child >> update_lastsync_time
