...
from sasglobal.oef_import.oef_offerings_supported_import.tasks.send_logs import get_send_logs
from sasglobal.oef_import.oef_offerings_supported_import.utils.response_filters import filter_object_extension_tags, get_filtered_input_data
import rail
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'sasglobal_oef_offerings_supported_import_master_{config.instance}',
        description=f'SaSGlobal OEF Offerings Supported Import Master DAG {config.instance}',
        ...
        def offer_supported_oef_uri():
            return "urn:replicon-tenant:" + rail.get_tenant_slug() + ":object-extension-tag-definition:4f3d4684-5ca0-470b-98a5-2e65737868ae"
        ...
        archive_invalid_file = rail.SFTPMoveFileOperator(
            ...
            new_filename=config.archive_filepath +
                '/Skipped_offerings_supported_{{ current_time("%Y%m%d%H%M%S") }}_{{ result("new_file_sensor") | file_name }}',
        ...
        archive_file = rail.SFTPMoveFileOperator(
            ...
            new_filename=config.archive_filepath + '/Offerings_supported_{{ current_time("%Y%m%d%H%M%S") }}_{{ result("new_file_sensor") | file_name }}',
            existing_filename=config.input_filepath+'/{{ result("new_file_sensor") | file_name }}',
        )
        ...
        get_object_extension_tags = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "objectExtensionTagDefinitionUri": offer_supported_oef_uri()
            },
        ...
        query_not_offering_support_input = rail.QueryCollectionOperator(
            task_id='query_not_offering_support_input',
            query="SELECT * FROM inputdata WHERE name != 'Offerings Supported'"
        )

        log_input_name_not_offering_support = rail.WriteLogOperator(
            task_id='log_input_name_not_offering_support',
            items='{{ result("query_not_offering_support_input") }}',
            message="Name not set to Offerings Supported",
            severity='Skipped',
            properties={
                "name": "{{ item.name }}",
                "value": "{{ item.value }}",
                "status": "{{ item.status }}",
                "processing_status": "Skipped",
                "details": "Name not set to Offerings Supported"
            }
        )

        query_offering_support_input = rail.QueryCollectionOperator(
            task_id='query_offering_support_input',
            query="SELECT * FROM inputdata WHERE name = 'Offerings Supported'"
        )
        ...
        process_valid_objects = rail.TriggerDagRunForEachItemOperator(
            ...
            items='{{ result("query_offering_support_input") }}',
            trigger_dag_id=f'sasglobal_oef_offerings_supported_import_process_valid_oef_child_{config.instance}',
            conf=lambda item: {
                "offer_supported_oef_uri": offer_supported_oef_uri(),
                "object_data": item,
        ...
        get_object_extension_tags >> available_tags_list >> query_not_offering_support_input >> log_input_name_not_offering_support \
            >> query_offering_support_input >> load_available_tags >> process_valid_objects >> wait_for_process_valid_objects >> send_logs_entry
        send_logs_end
