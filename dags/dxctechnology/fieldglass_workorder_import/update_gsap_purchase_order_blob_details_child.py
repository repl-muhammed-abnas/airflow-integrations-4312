...
def create_airflow_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.update_gsap_purchase_order_blob_dag_id,
        description="purchase order blob details",
        ...
        query_valid_records_for_user_in_replicon = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM merged_report_and_input_gsap WHERE loginname='{{dag_run.conf["loginname"]}}' """
        )
        ...
        get_key_value_from_keystore = rail.RepliconServiceOperator(
            ...
            data={
                "keyNamespace": config.keyNamespace_gsap,
        ...
        put_new_records_as_key_value = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "keyValue": {
                    "key": dag_run.conf["key"],
                    "jsonValue": custom_methods.get_json_new_key_value_blob_gsap()
                }
            }
        )
        ...
        compose_existing_key_value_csv = rail.DataAdaptorOperator(
            ...
            data=custom_methods.get_existing_key_value_md5_gsap
        )
        ...
        create_new_key_value_blob_csv = rail.DataAdaptorOperator(
            ...
            data=custom_methods.get_new_key_value_md5_gsap
        ...
        query_existing_id_in_new_records_blob = rail.QueryCollectionOperator(
            ...
            query="""SELECT new.WorkerID,new.WorkOrderID,existing.WOStartDate,new.WOEndDate,
            new.RateType,new.RateUnit,new.md5,new.uniqueid from newblobrecords new, existingblobrecords existing
            WHERE new.uniqueid=existing.uniqueid""",
            name="existingdatawithidinnewdata"
        )

        if_new_records_or_existing_id = rail.IfOperator(
            task_id="if_new_records_or_existing_id",
            test='{{result("query_existing_id_in_new_records_blob", "length") > 0}}',
            yes_task="if_key_value_updated",
            no_task="write_purchase_order_failure_log"
        )

        if_key_value_updated = rail.IfOperator(
            task_id="if_key_value_updated",
            test='{{result("query_new_records_blob", "length") > 0}}' or '{{result("query_existing_id_in_new_records_blob", "length") > 0}}',
        ...
        put_key_value_exisitng_and_new_records = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "keyValue": {
                    "key": dag_run.conf["key"],
                    "jsonValue": custom_methods.get_json_key_value_blob_gsap()
                }
            }
        )
        ...
        query_new_records_blob >>\
        query_existing_id_in_new_records_blob >>\
        if_new_records_or_existing_id >> rail.Label("Yes") >>\
        if_key_value_updated >>rail.Label("Yes") >> put_key_value_exisitng_and_new_records >>\
        write_purchase_order_update_log >> write_purchase_order_failure_log
        if_key_value_updated >> rail.Label("No") >> write_purchase_order_failure_log
