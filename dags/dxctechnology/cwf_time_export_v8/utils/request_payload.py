...
def get_approved_timesheets_payload():
    approval_date_filter = rail.find_first_by_attr_and_get_attr(rail.result('get_timesheet_period_report_details')
        ['filterConfiguration']['enabledFilters'], 'displayText', 'ApprovalDateFilter', 'uri')
    return {
        "reportParameters": [
            {
                "reportUri": rail.result('get_timesheet_period_report_details')['uri'],
                "filterValues": [
                    {
