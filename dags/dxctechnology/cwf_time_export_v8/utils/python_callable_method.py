...
def get_zero_hours_timesheets(input_date_format, rate_types_list):
    results = []
    for row in rail.load_all_records(rail.result("query_zero_hours_timesheets")):
        start = datetime.strptime(row['timesheetstartdate'], REPORT_DATE_FORMAT)
        end = datetime.strptime(row['timesheetenddate'], REPORT_DATE_FORMAT)

        current = start
        while current <= end:
            for rate_type in rate_types_list:
                result_row = {}
                result_row['userlastname'] = row['userlastname']
                result_row['userfirstname'] = row['userfirstname']
                result_row['entrydate'] = current.strftime(input_date_format)
                result_row['timesheetperiod'] = row['timesheetperiod']
                result_row['timesheetstartdate'] = row['timesheetstartdate']
                result_row['attendancetypecode'] = ''
                result_row['ratetype'] = rate_type
                result_row['totalhrs'] = '0.00'
                result_row['companycodecode'] = row['companycodecode']
                result_row['loginname'] = row['loginname']
                result_row['location'] = row['location']
                result_row['costcenter'] = row['costcenter']
                result_row['useruri'] = row['useruri']
                result_row['company_code_current'] = row['company_code_current']
                result_row['employeeid'] = row['employeeid']
...
def get_query_to_merge_data():
    if rail.result("create_report_collection") and rail.result("generate_zero_hours_timesheets"):
        return "SELECT * FROM rawdata UNION ALL SELECT * FROM zero_hrs_generated_records"
    if rail.result("create_report_collection") and not rail.result("generate_zero_hours_timesheets"):
        return "SELECT * FROM rawdata"
    if not rail.result("create_report_collection") and rail.result("generate_zero_hours_timesheets"):
        return "SELECT * FROM zero_hrs_generated_records"
    return null
