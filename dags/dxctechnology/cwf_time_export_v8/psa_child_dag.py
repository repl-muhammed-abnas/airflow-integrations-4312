...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_cwf_time_export_psa_child_{config.instance}_v8',
        description=f'DXCTechnology_PSA WF39 Child for C1 Reg and Compass Reg v8 {config.instance}',
        ...
        query_filtered_data_for_reversal = rail.QueryCollectionOperator(
            ...
            query='''SELECT * FROM finaltimedata WHERE timeentryid NOT IN (SELECT DISTINCT timeentryid FROM query_list_filtered_data_psa) AND CAST(hours AS FLOAT) = 0'''
        )
