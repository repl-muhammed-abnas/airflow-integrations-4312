...
def get_compass_task(config, task_type, region, output_filename, compass_oef_name, internal_oef_name, unique_id):
    with rail.TaskGroup(group_id=f'compass_task_{task_type}', prefix_group_id=False) as compass_task:
        ...
        query_filtered_data_for_reversal = rail.QueryCollectionOperator(
            task_id=f'query_filtered_data_for_reversal_{task_type}',
            query=f'''SELECT * FROM initialrawdata WHERE region='{region}' AND (timeentryid NOT IN (SELECT DISTINCT timeentryid FROM 'query_list_data_{task_type}') AND CAST(hours AS FLOAT) = 0)'''
        )

        has_data = rail.IfOperator(
            task_id=f'has_data_{task_type}',
            test=f"{open_bracket} result('query_list_data_{task_type}','length') > 0 or result('query_filtered_data_for_reversal_{task_type}','length') > 0 {close_braket}",
        ...
        def map_compass_row(item):
            ...
            return {
                "column_0": item['timeentryid'] if item['timentryid2'] else item['timeoffbookingid'],
                "column_1": "REPLICON",
                "column_2": item['employeeid'],
                "column_3": datetime.strptime(item['entrydate'], config.entry_date_format).strftime("%Y%m%d"),
                "column_4": item['parentwbs'] if item['projecttype'] == "ES" or item['projecttype'] == "CP" and item['parentwbs'] else item['projectname'],
                "column_5": "400" if item['companycodecode'] == "C1" or item['companycodename'] in ['3001', '3124', '1602', '3118'] else
                item['attendancetypecode'] if item['attendancetypecode'] else
                item['timeofftypedescription'] if item['timeoffbookingid'] else '',
                "column_6": round(float(item['hours']), 2),
                "column_7": item['comments'],
                "column_8": "C1" if item['companycodecode'] == "C1" else 'GSAP' if item['companycodename'] in ['3001', '3124', '1602', '3118'] else '',
                "column_9": item['taskname'] if item['tasktype'] == "Attribute 1" else
                ...
                "column_10": item['taskname'] if item['tasktype'] == "Attribute 2" else
                ...
                "column_11": item['taskname'] if "PPMC" in item['tasktype'] else '',
                "column_12": item['customer1'],
                "column_13": item['customer2'],
                "column_14": item['customer3'],
                "column_15":  '' if item['projecttype'] == "ES" and item['projectname'].startswith("E-") else
                ...
                "column_16": '' if item['projecttype'] == "ES" and item['projectname'].startswith("E-") else
                "DT" if item['ratetype'] == "Double Time" else
                "OT" if item['ratetype'] == "Overtime" else
                "ST" if item['ratetype'] == "Straight Time" else
                '',
                "column_17": item['labortype'] if item['labortype'] else '',
                "column_18": region,
            }.values()
        ...
        log_message_filename_compass = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: (output_filename[:4] if (rail.get_company_key()).lower() ==
                "dxctechnology" else compass_oef_name[12:16]) + dag_run.conf['twbname']+'.xml'
        )
        ...
        create_reversal_csv_lines = rail.WriteCSVFileOperator(
            task_id=f'create_reversal_csv_lines_{task_type}',
            source=f"{open_bracket} result('query_filtered_data_for_reversal_{task_type}') {close_braket}",
            header=header,
            row=map_compass_row,
            thread_pool_size=config.write_csv_threadpool_size
        )

        load_csv_reversal = rail.LoadCSVFileOperator(
            task_id=f'load_csv_reversal_{task_type}',
            document=f"{open_bracket} result('create_reversal_csv_lines_{task_type}') {close_braket}",
        )

        create_collection_reversal = rail.CreateCollectionOperator(
            task_id=f'create_collection_reversal_{task_type}',
            source=f"{open_bracket} result('load_csv_reversal_{task_type}') {close_braket}",
            name=f"reversaldata{task_type}"
        )
        ...
        def do_get_dataset():
            ...
            reversal_records = rail.load_all_records(rail.result(
                f'create_collection_reversal_{task_type}'))
            records = [*unique_record, *all_records, *reversal_records]
            region = records[0]['region']
            return {
                'records': records,
                'region': [{'name': region}]
        ...
        check_current_export_ack >> rail.Label(
            "No") >> log_message_filename_compass >> query_list_data >> query_filtered_data_for_reversal >> has_data
        ...
        create_csv_line >> load_csv_create_list_from_csv >> create_collection_create_list_from_csv >> get_final_line >> \
            create_reversal_csv_lines >> load_csv_reversal >> create_collection_reversal >> \
