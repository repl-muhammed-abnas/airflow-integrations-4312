...
from dxctechnology.cwf_time_export_v8.task.compass_task import get_compass_task
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_cwf_time_export_compass_child_{config.instance}_v8',
        description=f'DXCTechnology_CWF Time export - Compass v8 {config.instance}',
        ...
        get_all_divisions_with_description = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response: list(map(lambda data: {
                "companycodename": data['cells'][0].get('textValue'),
                "description": data['cells'][1].get('textValue')
        ...
        # LRU cached functions for rail.result() calls to avoid repeated XCom data loading
        @functools.lru_cache(maxsize=1)
        def get_cached_map_division_list():
            return rail.result('map_division_list')

        @functools.lru_cache(maxsize=1)
        def get_cached_all_divisions_with_description():
            return rail.result('get_all_divisions_with_description')

        @functools.lru_cache(maxsize=1)
        def get_cached_map_location_list_data():
            return rail.result('map_location_list_data')

        @functools.lru_cache(maxsize=1)
        def get_cached_search_entries_location_map_compass():
            return rail.result('search_entries_location_map_compass')
        ...
        def get_region_from_mapper(item, companycodedesc, companycodedesc2):
            if item['projecttype'] == "ES" and item['projectname'].startswith("E-") and (companycodedesc == 'PN1' or companycodedesc2 == 'PN1'):
                return "AMER"
            if item['projecttype'] == "ES" and item['projectname'].startswith("E-") and (companycodedesc == 'P01' or companycodedesc2 == 'P01'):
                return "EMEA"
            if item['companycodecode'] == 'GSAP' and not item['locationname']:
                return "APAC"
            region_by_parent_location = rail.find_first_by_attr_and_get_attr(
                get_cached_search_entries_location_map_compass(), 'name',
                rail.find_first_by_attr_and_get_attr(
                    get_cached_map_location_list_data(), 'locationname', item['locationname'], 'parent'),
                'compassregion')
            return region_by_parent_location if region_by_parent_location else rail.find_first_by_attr_and_get_attr(
                get_cached_search_entries_location_map_compass(), 'name', item['locationname'], 'compassregion')

        def map_rows(item):
            companycodedesc = rail.find_first_by_attr_and_get_attr(get_cached_map_division_list(), 'companycodename', item['companycodename'], 'parent')
            companycodedesc2 = get_region_code_from_description(rail.find_first_by_attr_and_get_attr(get_cached_all_divisions_with_description(), 'companycodename', item['companycodename'], 'description'))
            return [
                ...
                item['ratetype'],
                item['timeentryid'],
                datetime.strptime(item['entrydate'], config.entry_date_format).strftime("%Y%m%d") + item['timeoffbookingid']
                if item['timeoffbookingid'] else '',
                item['comments'],
                item['wbstype'],
                item['tasktype'],
                item['newremainningwork'],
                item['customer1'],
                item['customer2'],
                item['customer3'],
                item['gsapbillableflag'],
                item['timeofftypedescription'],
                item['masterwbs'],
                item['projecttype'],
                item['iwoindicator'],
                item['parentwbs'],
                item['companycodename'],
                companycodedesc,
                companycodedesc2,
                item['taskfullpath'],
                len(item['taskfullpath'].split(" / ")),
                item['timentryid2'],
                item['employeetypename'],
                item['timesheetperiod'],
                item['locationname'],
                rail.find_first_by_attr_and_get_attr(get_cached_map_location_list_data(), 'locationname', item['locationname'], 'parent'),
                item['loginname'],
                item['user'],
                item['iwowbselement'],
                item['workorderid'],
                item['parentserviceorder'],
                item['c1cwfalternateid'],
                get_region_from_mapper(item, companycodedesc, companycodedesc2),
                item['attributecode1'],
                item['attributecode2'],
            ]
        ...
        create_csv_lines_estype = rail.WriteCSVFileOperator(
            ...
            header=['companycodecode','employeeid','perner','approvalstatus','entrydate','projectname','costcentercode','labortype','jobactivitytype','taskname','timetype','attendancetypecode',
                'billableindicator','hours','ratetype','timeentryid','timeoffbookingid','comments','wbstype','tasktype','newremainningwork','customer1','customer2','customer3','gsapbillableflag',
                'timeofftypedescription','masterwbs','projecttype','iwoindicator','parentwbs','companycodename','companycodedesc','companycodedesc2','taskfullpath','length','timentryid2','employeetype','timesheetperiod',
                'locationname','parentlocation','loginname','username','iwowbselement','workorderid','parentserviceorder','c1cwfalternateid','region','attributecode1','attributecode2'],
            row=map_rows,
        ...
        create_rawdata_csv_lines_estype = rail.WriteCSVFileOperator(
            task_id='create_rawdata_csv_lines_estype',
            source="{{ result('create_collection_create_list_from_csv_finaltimedata') }}",
            header=['companycodecode','employeeid','perner','approvalstatus','entrydate','projectname','costcentercode','labortype','jobactivitytype','taskname','timetype','attendancetypecode',
                'billableindicator','hours','ratetype','timeentryid','timeoffbookingid','comments','wbstype','tasktype','newremainningwork','customer1','customer2','customer3','gsapbillableflag',
                'timeofftypedescription','masterwbs','projecttype','iwoindicator','parentwbs','companycodename','companycodedesc','companycodedesc2','taskfullpath','length','timentryid2','employeetype','timesheetperiod',
                'locationname','parentlocation','loginname','username','iwowbselement','workorderid','parentserviceorder','c1cwfalternateid','region','attributecode1','attributecode2'],
        ...
        create_rawdata_collection_for_reversal = rail.CreateCollectionOperator(
            task_id='create_rawdata_collection_for_reversal',
            source="{{ result('create_rawdata_csv_lines_estype') }}",
            name="initialrawdata"
        )
        ...
        create_time_data_download_batch_compass >> batch_management_async_compass >> get_time_data_download_batch_results >> download_timedata_file >> \
            ...
            query_list_filtered_data_estype >> get_data_division_list_service >> map_division_list >> get_all_divisions_with_description >> get_data_location_list_service >> \
            map_location_list_data >> search_entries_location_map_compass >> create_csv_lines_estype >> \
            load_csv_create_list_from_estype >> create_collection_create_list_from_csv_estype >> create_rawdata_csv_lines_estype >> \
                create_rawdata_collection_for_reversal >> query_list_distinct_login_estype >> \
