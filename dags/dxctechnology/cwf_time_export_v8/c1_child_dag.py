...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_cwf_time_export_c1_child_{config.instance}_v8',
        description=f'DXCTechnology_CWF Time export - C1 v8 {config.instance}',
        ...
        query_list_filtered_data_c1 = rail.QueryCollectionOperator(
            ...
            query='''SELECT * FROM finaltimedata
                    WHERE
                        ...
                        OR
                        employeetypename LIKE '%Contractor%' AND companycodecode = 'C1' AND CAST(hours as INT) = 0
                        OR
                        (
        ...
        def map_row(item):
            ...
            if str(float(item['hours'])) == "0.0":
                reversal_check = True if "799" in item['attendancetypecode'] else False

            attendence_type = '' if reversal_check else item['attendancetypecode']
            ...
            return {
                ...
                "Labortype": None if item['projecttype'] == "ES" else item['labortype'].split("|")[0] if item[
                    'labortype'] and '|' in item['labortype'] else None,
                "Billableindicator": None if item['projecttype'] == "ES" else "X" if "|Billable" in item['labortype'] else None,
        ...
        query_filtered_data_for_reversal = rail.QueryCollectionOperator(
            ...
            query='''SELECT * FROM finaltimedata WHERE timeentryid NOT IN (SELECT DISTINCT timeentryid FROM query_list_filtered_data_c1) AND CAST(hours AS FLOAT) = 0'''
        )

        create_reversal_csv_lines_final_c1 = rail.WriteCSVFileOperator(
            task_id='create_reversal_csv_lines_final_c1',
        ...
        create_collection_create_list_from_reversal = rail.CreateCollectionOperator(
            ...
            source="{{ result('create_reversal_csv_lines_final_c1') }}",
            name="reversaldata"
        )
        ...
        has_rows_c1_query_list_filtered_data >> rail.Label(
            ...
            query_list_c1final >> get_final_line >> get_final_line_collection >> query_filtered_data_for_reversal >> \
                create_reversal_csv_lines_final_c1 >> create_collection_create_list_from_reversal >> get_final_export_data >> \
                    create_document_c1_xml >> upload_xmlfile_to_sftp
