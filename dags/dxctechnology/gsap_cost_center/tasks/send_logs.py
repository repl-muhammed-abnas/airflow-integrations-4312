    ...
    with rail.TaskGroup(group_id='send_logs', prefix_group_id=False):
        ...
        render_logs_csv = rail.WriteCSVFileOperator(
            ...
            header=['{{ current_time("%d/%m/%YT%H:%M:%S") }}',
                    'Number of Rows:' + '{{- result("get_logged_errors", key="length") + result("get_logged_success", key="length") +\
                      result("get_logged_exceptions", key="length") + result("get_logged_skipped", key="length")}}',
                    'Function: GSAP Cost Center', '', '', ''],
            row=['{{ item.properties | attr_or_default("costcentername", "") }}',
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('get_logged_errors', 'length') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | GSAP Cost Center Import - " }} \
