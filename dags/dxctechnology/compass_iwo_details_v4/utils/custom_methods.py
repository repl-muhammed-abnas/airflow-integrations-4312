...
def check_parent_c1_SO():
    master_wbs_oef_value = rail.find_first_by_attr_and_get_attr(
        rail.result("get_parent_project_details")['extensionFieldValues'],"definition.displayText","Master WBS (SO, WO)","textValue")
    return (True if master_wbs_oef_value =="SO" else False)

def extract_tasks_from_response(response):
    """
    Extract tasks from TaskListService response.
    The response has structure: {'d': {'rows': [...]}}
    Each row contains task information that needs to be parsed.
    """
    try:
        data = response.json()['d']
        ...
        # Extract task information from rows
        tasks = []
        for row in data['rows']:
            if row and row.get('cells'):
                # Get task name and URI from the cells
                task_info = {
                    'name': get_value(row, 0, 'textValue'),
                    'uri': get_value(row, 0, 'uri'),
                    'enabled': get_value(row, 3, 'textValue'),
                }
                # Only include enabled tasks with both name and URI
                if task_info['enabled'] == "True" and task_info['name'] and task_info['uri']:
