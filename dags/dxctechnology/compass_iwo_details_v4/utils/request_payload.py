...
def get_create_task_payload_optimized(item, parent_project_details):
    """
    Create task payload for optimized hierarchical task creation
    Matches the structure of get_put_task_payload

    Args:
        item: Task data from prepare_tasks_for_creation
        parent_project_details: Parent project details from API

    Returns:
        Formatted payload for PutTask API
    """
    # Get parent WBS task details if available
    parent_tasks = rail.result('get_all_tasks_of_parent_project') if rail.result('get_all_tasks_of_parent_project') else []
    master_task = None

    # Find the master task from parent project
    if parent_tasks and item.get('parent_wbs_task_uri'):
        for task in parent_tasks:
            if task.get('uri') == item.get('parent_wbs_task_uri'):
                master_task = task
                break

    # Get the task type custom field
    def get_custom_fields():
        if not item.get('task_type'):
            return []

        # Try to get dropdown value from master task
        dropdown_value = None
        if master_task and master_task.get('customFields'):
            dropdown_value = rail.find_first_by_attr_and_get_attr(
                master_task['customFields'],
        ...
        if not dropdown_value:
            dropdown_value = item.get('task_type')  # Fallback to task type name

        return [
            {
                "customField": {
                    "uri": item.get('task_type_uri', item.get('task_type')),
                },
                "dropDownOption": {
                    "name": dropdown_value,
                }
    ...
    # Format date range
    def get_time_entry_date_range():
        if not item.get('start_date') and not item.get('end_date'):
            return null
        return {
            "startDate": custom_methods.get_replicon_date(item.get('start_date'), "%d %B %Y") if item.get('start_date') else null,
            "endDate": custom_methods.get_replicon_date(item.get('end_date'), "%d %B %Y") if item.get('end_date') else null,
            "relativeDateRangeUri": null,
            "relativeDateRangeAsOfDate": null
        }

    # Get parent task URI for non-root tasks
    parent_uri = None
    if item.get('level') != '1' and item.get('parent_task_uri'):
        parent_uri = item.get('parent_task_uri')
    elif item.get('level') != '1' and item.get('parent'):
        # Try to find parent task URI from created tasks
        child_tasks = rail.result('get_all_tasks_of_child_project') if rail.result('get_all_tasks_of_child_project') else []
        for task in child_tasks:
            if task.get('taskname') == item.get('parent'):
                parent_uri = task.get('uri')
                break

    # Build the task payload matching original structure
    return {
        "project": {
            "uri": item.get('processing_wbs_uri'),
        },
        "task": {
            "target": {
                ...
                "parent": {
                    "uri": parent_uri,
                } if parent_uri else null,
            },
            "name": item.get('taskname'),
            "code": null if item.get('code') in ['None', None, ''] else item.get('code'),
            "description": null,
            "percentCompleted": "0",
            "timeEntryDateRange": get_time_entry_date_range(),
            "isTimeEntryAllowed": True,
            "isClosed": False,
            "customFieldValues": get_custom_fields(),
            "timeAndExpenseEntryTypeUri": master_task.get('timeAndExpenseEntryType', {}).get('uri')
            if master_task and master_task.get('timeAndExpenseEntryType') else null,
            "assignedResources": item.get('resources', [])  # Resources already formatted in prepare_tasks_for_creation
        }
    }
