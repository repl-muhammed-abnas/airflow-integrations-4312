...
from dxctechnology.compass_iwo_details_v4.utils import response_filter
from dxctechnology.compass_iwo_details_v4.utils import python_callable_method
from dxctechnology.compass_iwo_details_v4.task.generate_report_batch import report_batch
...
def create_iwo_details_master_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        parse_xml = rail.LoadXMLFileOperator(
            ...
            xsd_document='./dags/dxctechnology/compass_iwo_details_v4/xsdschema/input_schema.xsd'
        )
        ...
        get_unique_parent_wbs_with_all_child = rail.PythonOperator(
            task_id="get_unique_parent_wbs_with_all_child",
            python_callable=python_callable_method.get_unique_parent_wbs_with_all_child_py
        )

        process_iwoelement_update = rail.TriggerDagRunForEachItemOperator(
            task_id="process_iwoelement_update",
            retries=0,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            items=lambda: list(filter(None, rail.result(
                'get_unique_parent_wbs_with_all_child')['parent_list'])),
            trigger_dag_id=f'dxctechnology_compass_iwo_process_iwowbselement_child_{config.dag_id_postfix}',
            conf=lambda item: {
                "parent": item,
                "child_list": rail.find_first_by_attr_and_get_attr(
                    rail.result("get_unique_parent_wbs_with_all_child")['parent_child_list'], 'parent', item, "child"),
                "iwowbselement": rail.result('get_all_object_extension_field')['iwowbselement']
            }
        )

        wait_for_process_iwoelement_update = rail.WaitForDagRunsSensor(
            task_id='wait_for_process_iwoelement_update',
            dag_runs='{{ result("process_iwoelement_update") }}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
        )
        ...
        has_data >> rail.Label(
            ...
            >> [get_all_object_extension_field, get_custom_field_group] >> get_task_type_udf >> get_records_from_xml \
            >> get_unique_users_from_feed >> query_required_users >> get_query_required_users >> process_iwo_wbs_update \
            >> wait_for_process_iwo_wbs_update >> get_unique_parent_wbs_with_all_child \
            >> process_iwoelement_update >> wait_for_process_iwoelement_update >> generate_output_log \
        ...
        finish >> check_if_new_file_found >> log_to_sumo
