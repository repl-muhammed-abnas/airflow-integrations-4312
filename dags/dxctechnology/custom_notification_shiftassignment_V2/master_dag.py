...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_custom_notification_shiftassignment_master_V2_{config.instance}',
        description=f'DXC Custom notification for shiftassignment - India V2.0 {config.instance}',
        ...
        expected_report_columns = "User Name,Login Name,supervisorname,supervisoremail,scheduletype,useruri,supervisoruri,startdatetocheckforthursday,enddatetocheckforthursday,Company Code (Current),startdatetocheckforfriday,endatetocheckforfriday"
        ...
        query_basereportdata = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT supervisoruri,supervisorname,supervisoremail
                        ...
                        AND NULLIF(supervisoremail, '') IS NOT NULL"""
        )
        ...
        process_each_supervisor = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_custom_notification_shiftassignment_child_V2_{config.instance}',
        ...
        report_has_expected_columns >> rail.Label("Yes") >> \
            ...
            query_basereportdata >> process_each_supervisor >> wait_process_each_supervisor >> finish
