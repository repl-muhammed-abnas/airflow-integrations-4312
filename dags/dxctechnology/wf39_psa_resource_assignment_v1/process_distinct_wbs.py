...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_wf39_psa_resource_assignment_process_distinct_wbs_item_child_{config.instance}_v1',
        description=f'DXC_WF39 Resource Assignment Automation Child V1 - B1 {config.instance}',
        ...
        query_all_records_for_wbs = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM inputcombineddata WHERE wbs=:wbs AND role NOT IN (SELECT DISTINCT role FROM skippedlabourtypes)""",
            query_params={
                "wbs": project_name
            }
        )
        ...
        process_each_wbs_item = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_wf39_psa_resource_assignment_process_each_record_child_{config.instance}_v1',
        ...
        is_project_not_exists >> rail.Label(
            "No") >> get_division_detail >> is_project_c1
