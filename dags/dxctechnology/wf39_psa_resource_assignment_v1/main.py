...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_wf39_psa_resource_assignment_import_master_{config.instance}_v1',
        description=f'DXC_WF39 PSA Resource Assignment Master V1 - SFTP {config.instance}',
        ...
        get_all_records_for_role = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM billingratesforwbs WHERE LOWER(role) IN (SELECT DISTINCT LOWER(role) FROM skippedlabourtypes)"""
        )
        ...
        query_distinct_projects = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT wbs FROM billingratesforwbs WHERE role NOT IN (SELECT DISTINCT role FROM skippedlabourtypes)"""
        )
        ...
        process_billing_rate_for_each_wbs_item = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_wf39_psa_resource_assignment_process_distinct_wbs_item_child_{config.instance}_v1',
        ...
        render_logs_csv = rail.WriteCSVFileOperator(
            ...
            row=[
                ...
                '{{ item.properties.wbs }}',
                '{{ item.properties | attr_or_default("role", "") }}',
        ...
        has_labour_types >> rail.Label(
            "Yes") >> input_combined_list >> input_combined_data_collection >> distinct_labour_types >> \
