...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_australia_payrolldata_export_active_user_child_v3_{config.instance}',
        description=f'DXC_Australia_PayrollData_Export_Active_User_Child V3 {config.instance}',
        ...
        send_email_for_sftp_failure = rail.EmailOperator(
            ...
            bcc=config.internal_logs_email,
            # pylint: disable=line-too-long
            subject='{{ get_company_key() }} | Replicon payroll export for AUS Active User Balance export automation - SFTP failure for {{ dag_run.conf.location }} location  on - current_time_in_specified_tz() }}',
            params={
        ...
        send_email_for_export_completion = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | Replicon payroll export for AUS Active User Balance completed  on - {{ current_time_in_specified_tz() }}',
            params={
                'output_filepath': config.output_filepath,
        ...
        send_email_for_log_upload_failure = rail.EmailOperator(
            ...
            to=config.alert_email,
            subject='{{ get_company_key() }} | Replicon payroll export for AUS Active User Balance - SFTP failure for {{ dag_run.conf.location }} location {{ current_time_in_specified_tz() }}',
            params={
                'log_filepath': config.log_filepath
            },
        ...
        active_user_balance_report_data_collection >> query_final_payroll_collection >> query_active_user_balance_data >> query_list_in_final_payroll_collection
