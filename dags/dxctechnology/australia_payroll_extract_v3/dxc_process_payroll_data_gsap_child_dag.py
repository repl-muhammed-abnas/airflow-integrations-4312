...
from dxctechnology.australia_payroll_extract_v3.utils import response_filter


def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_australia_payrolldata_export_gsap_child_v3_{config.instance}',
        description=f'DXC_Australia_PayrollData_Export_GSAP_Child V3 {config.instance}',
        ...
        get_file_name = rail.PythonOperator(
            ...
            + "_AUREPL_REGS" + request_payload.get_sequence_no('sequence_no_for_2010') + "_DUT8G2I"
        )
        ...
        create_payroll_download_batch = rail.RepliconServiceOperator(
            ...
            data=lambda: request_payload.get_create_payroll_download_batch_gsap_payload(
                config.duration_days, config.cut_off_date)
        )
        ...
        send_email_for_no_payroll_data = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon payroll export is skipped for - {{dag_run.conf.location_name}} - {{dag_run.conf.division_name}} - {{current_time_in_specified_tz()}}',
            html_content="templates/email/gsap_blank_export.html"
        )
        ...
        create_payrun_batch = rail.RepliconServiceOperator(
            ...
            data=lambda: request_payload.get_create_payrun_batch_gsap_payload(
                config.duration_days, config.cut_off_date)
        )
        ...
        has_absence_item_data = rail.IfOperator(
            task_id='has_absence_item_data',
            test="{{ result('query_list_in_final_absence_payroll_collection','length') > 0 }}",
            yes_task='process_absence_taken',
            no_task='get_all_time_off_pay_codes_from_mapper'
        )

        process_absence_taken = rail.TriggerDagRunOperator(
            task_id='process_absence_taken',
            retries=0,
            trigger_dag_id=f'dxctechnology_australia_payrolldata_export_absence_taken_child_v3_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=request_payload.process_absence_taken_gsap_user_conf
        )
        ...
        query_list_in_final_payroll_collection = rail.QueryCollectionOperator(
            ...
            query='''SELECT * FROM final_payroll_item_data WHERE NULLIF(Pay_Code_Description, '') IS NOT NULL AND Pay_Code_Description = '2010' '''
        )
        ...
        # pylint: disable=line-too-long
        get_location_child_hierarchy_data >> get_contractor_employee_type_child_hierarchy_data >> create_log >> get_file_name >> process_start_time >> \
        ...
        has_empty_empid_data >> rail.Label(
            'No') >> query_list_in_final_absence_payroll_collection >> has_absence_item_data
        has_absence_item_data >>rail.Label("Yes") >> process_absence_taken >> get_all_time_off_pay_codes_from_mapper
        has_absence_item_data >>rail.Label("No") >> get_all_time_off_pay_codes_from_mapper >> query_list_in_final_payroll_collection >> has_item_data >> rail.Label(
            'Yes') >> compose_item_payroll_csv_file >> logging_no_of_records_exported >> no_of_records_size_including_header_footer >> create_document >>\
        ...
        has_item_data >> rail.Label('No') >> finish_export_no_payroll_data >> send_email_for_no_payroll_data
