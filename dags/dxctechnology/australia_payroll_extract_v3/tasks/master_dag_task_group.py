...
def get_master_dag_task_group(region, export, company_key, mapper):
    ...
    with rail.TaskGroup(group_id='master_dag_task_group', prefix_group_id=False):
        ...
        search_entries_companycode_mapper = rail.PythonOperator(
            ...
            python_callable=lambda: request_payload.companycode_from_gsap_mapper(
                export, mapper) if region == "GSAP" else (
                    request_payload.companycode_from_es_weekly_mapper(export, mapper) if region == "ES_WEEKLY"
                    else request_payload.companycode_from_es_mapper(export, mapper))
        )

        master_dag_task_group_loaded = rail.EmptyOperator(
            task_id='master_dag_task_group_loaded')

        master_dag_task_group_start >> [get_all_scripts, get_all_enabled_locations, get_all_enabled_divisions,
            get_all_enabled_employee_type_groups,search_entries_companycode_mapper] >> master_dag_task_group_loaded
