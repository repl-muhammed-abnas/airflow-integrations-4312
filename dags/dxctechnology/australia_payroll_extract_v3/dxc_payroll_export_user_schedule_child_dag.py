...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_australia_payrolldata_export_user_schedule_child_v3_{config.instance}',
        description=f'DXC_Australia_PayrollData_Export_User_Schedule_Child V3 {config.instance}',
        ...
        query_employee_data_collection = rail.QueryCollectionOperator(
            ...
            query="SELECT DISTINCT Employee_Id FROM active_userbalance WHERE NULLIF(Employee_Id, '') IS NOT NULL",
        )
        ...
        process_each_user_based_empid= rail.trigger_parallel_dagrun(
            ...
            trigger_dag_id=f'dxctechnology_australia_payroll_export_process_each_user_child_v3_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        gather_child_data = rail.GatherResultsFromDagRunsOperator(
            ...
            dagrun_task_id='get_dates_for_schedule',
            flatten=True,
        )
