...
def get_project_update_modifications():
    ...
    add_oef('projectofferinggroup')
    add_oef('wbsofferinggroup')

    def add_oef_psa(defuri,taguri):
        oefs.append({
            'definition': {'uri': defuri},
            'tag': {'uri': taguri} if taguri else None,
        })
    if conf['wbsofferinggroupvalue'] == 'Velocity Only':
        add_oef_psa(conf['psaflagdefinitionuri'],conf['psa_x_flaguri'])
    else :
        add_oef_psa(conf['psaflagdefinitionuri'],null)
    if conf['tmwbsindicator'] == 'X':
        add_oef_psa(conf['tmwbsindicatordefinitionuri'], conf["tm_x_flaguri"])
    else:
        add_oef_psa(conf["tmwbsindicatordefinitionuri"], null)
    oefs.append({
    ...
    if conf['serviceofferingid']:
        oefs.append({'definition': {'uri': conf['serviceofferingiddefinitionuri']}, 'textValue': conf['serviceofferingid']})
...
def get_psa_flag_oef_param(value,dag_run):
    return {
            "objectUri": dag_run.conf['childwbsuri'],
            "value": {
                "definition": {
                    "uri": dag_run.conf['psaflagdefinitionuri'],
                    "name": null
                },
                "tag": {
                    "uri": dag_run.conf['psa_x_flaguri'],
                }if value=='X' else null,
                "numericValue": null,
                "textValue": null,
                "fileValue": null,
                "jsonValue": null
...
def get_wbsofferinggrp_oef_param(dag_run):
    return {
            "objectUri": dag_run.conf['childwbsuri'],
            "value": {
                "definition": {
                    "uri": dag_run.conf['wbsofferinggroupdefinitionuri'],
                    "name": null
                },
                "tag": {
                    "uri": dag_run.conf['wbsofferinggrouptaguri'],
                }if dag_run.conf['wbsofferinggrouptaguri'] else null,
                "numericValue": null,
                "textValue": null,
                "fileValue": null,
                "jsonValue": null
...
def get_tmwbsindicator_oef_param(dag_run):
    return {
            "objectUri": dag_run.conf['childwbsuri'],
            "value": {
                "definition": {
                    "uri": dag_run.conf['tmwbsindicatordefinitionuri'],
                    "name": null
                },
                "tag": {
                    "uri": dag_run.conf['tm_x_flaguri'],
                }if dag_run.conf['tmwbsindicator'] == 'X' else null,
                "numericValue": null,
                "textValue": null,
                "fileValue": null,
                "jsonValue": null
