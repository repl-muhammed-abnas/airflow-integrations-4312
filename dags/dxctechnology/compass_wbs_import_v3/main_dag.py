...
import itertools
import rail
from dxctechnology.compass_wbs_import_v3.tasks.get_active_project_prereqs import get_active_project_prereqs
from dxctechnology.compass_wbs_import_v3.utils.response_filter import do_format_logs

#config : https://github.com/replicon/airflow-integrations/blob/main/dags/dxctechnology/compass_wbs_import_v3/config.py
...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        ...
        parse_xml = rail.LoadXMLFileOperator(
            ...
            xsd_document='./dags/dxctechnology/compass_wbs_import_v3/xml_schema/input_schema.xsd'
        )
        ...
        get_process_inactive_projects_dagrun_ids =rail.PythonOperator(
            task_id='get_process_inactive_projects_dagrun_ids',
            python_callable=lambda: list(itertools.chain(
                *list(map(lambda x: (rail.result(
                    f'process_inactive_projects_{x+1}') if rail.result(
                    f'process_inactive_projects_{x+1}') else []), range(config.trigger_parallel_dagrun_count))))),
            show_return_value_in_logs=False
        )

        gather_process_inactive_projects_logs = rail.GatherResultsFromDagRunsOperator(
            task_id='gather_process_inactive_projects_logs',
            dag_runs='{{ result("get_process_inactive_projects_dagrun_ids") }}',
        ...
        process_active_projects = rail.trigger_parallel_dagrun(
            ...
            conf={
                ...
                'tmwbsindicator': '{{item.TMWBSIndicator | sn }}',
                'tmwbsindicatordefinitionuri': '{{result("get_project_oefs").tmwbsindicator | sn}}',
                'tm_x_flaguri': '{{ result("get_tmwbsindicator_values") | filter_by_attr("name", "equals", "X") | first_or_default | attr_or_default("uri", "") }}',
                },
            execution_timeout=timedelta(days=config.execution_timeout_days),
        )

        get_process_active_projects_dagrun_ids =rail.PythonOperator(
            task_id='get_process_active_projects_dagrun_ids',
            python_callable=lambda: list(itertools.chain(
                *list(map(lambda x: (rail.result(
                    f'process_active_projects_{x+1}') if rail.result(
                    f'process_active_projects_{x+1}') else []), range(config.trigger_parallel_dagrun_count))))),
            show_return_value_in_logs=False
        )

        gather_process_active_projects_logs = rail.GatherResultsFromDagRunsOperator(
            task_id='gather_process_active_projects_logs',
            dag_runs='{{ result("get_process_active_projects_dagrun_ids") }}',
            dagrun_task_id='create_log',
            execution_timeout=timedelta(
                hours=config.gather_logs_timeout_hours),
            flatten=True
        ...
        format_log_records = rail.CreateCollectionOperator(
            ...
            source=do_format_logs,
            columns=["projectname", "projectcode", "status", "message", "ecid"],
        ...
        render_logs_csv = rail.WriteCSVFileOperator(
            ...
            header=[
                ...
                'Number of Rows: {{ result("format_log_records") | load_all_records | length }}',
                'Function: COMPASS WBS Master inbound',
                '',
                ''],
            row=[
                '{{ item | attr_or_default("projectname", "")  }}',
                '{{ item | attr_or_default("projectcode", "") }}',
                '{{ item | attr_or_default("status", "") }}',
                '{{ item.message }}',
                '{{ item.ecid }}'],
            footer=['Number of Records Processed Successfully: {{ result("format_log_records", key="get_success_projects") }}',
                    'Number of Records Processed with Exception: {{ result("format_log_records", key="get_exception_projects") }}',
                    'Number of Records with Error: {{ result("format_log_records", key="get_errored_projects") }}', '', ''],
        )
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc="{%- if result('format_log_records', key='get_errored_projects') == 0 -%}\
                    "+config.internal_logs_email+"\
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Replicon project sync for Compass WBS " }} \
                {%- if result("format_log_records", key="get_errored_projects") > 0 -%} \
                    completed with errors  \
                {%- else -%} \
                    {%- if result("format_log_records", key="get_exception_projects") > 0 -%} \
        ...
        new_file_sensor >> is_xml >> rail.Label(
            'Yes') >> download_file >> parse_xml >> has_data >> rail.Label('Yes') >> create_log >> create_project_collection
        ...
        has_any_active_projects >> rail.Label("No") >> generate_output_log >> format_log_records >> render_logs_csv
        query_inactive_projects >> process_inactive_projects >> get_process_inactive_projects_dagrun_ids \
            >> gather_process_inactive_projects_logs >> generate_output_log
        active_project_prereqs_group_exit >> process_active_projects >> get_process_active_projects_dagrun_ids \
            >> gather_process_active_projects_logs >> generate_output_log
