import rail
from dxctechnology.wf39_psa_resource_assignment_v2.utils import python_callable_method
from dxctechnology.wf39_psa_resource_assignment_v2.utils import response_filter
from dxctechnology.wf39_psa_resource_assignment_v2.utils import request_payload
# pylint: disable=too-many-statements


def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_wf39_psa_resource_assignment_process_each_record_child_{config.instance}_v2',
        description=f'DXC_WF39 PSA Resource Assignment Automation Child V2 {config.instance}',
        ...
        log_user_date_range_exception = rail.WriteLogOperator(
            ...
            message=lambda dag_run: str(python_callable_method.get_out_of_range_message(dag_run)),
            properties={
                'employeeid': '{{dag_run.conf.user}}',
                'wbs': '{{dag_run.conf.wbselement}}',
                'role': '{{dag_run.conf.labourtype}}',
        ...
        is_assignment_uri_present = rail.IfOperator(
            ...
            test=lambda: bool(rail.result('get_all_project_team_assignment')['user_data']),
        ...
        get_all_project_team_assignment >> get_all_project_labour_types >> is_labour_type_present_in_project
        ...
        can_assign_labour_type >> rail.Label("YES") >> bulkUpdateProjectTeamMemberBillingRatesAllowedForBillingTime
