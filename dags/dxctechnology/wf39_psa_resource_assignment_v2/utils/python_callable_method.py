...
from dxctechnology.wf39_psa_resource_assignment_v2.mapper.item_categories import item_categories_mapper
...
def is_LT_assigned():
    ...
    if not bool(rail.result('get_all_project_team_assignment')):
        return True
    data = rail.result('assignedBillingRates') if rail.result('assignedBillingRates') else []
...
def get_out_of_range_message(dag_run):
    ...
    if is_assignment_out_of_range(dag_run):
        log.append(
            'Labour Type is Not assigned to user due to Assignment start/end date outside the WBS start/end date')
    if is_user_out_of_range(dag_run):
        log.append(
            'Labour Type is Not assigned to user due to Assignment start/end date outside the user start/end date')
    return ';'.join(log)
...
def check_labour_type(dag_run):
    data = rail.result("get_all_project_labour_types")
    return (dag_run.conf['labourtype'] + '|Billable') in data or (dag_run.conf['labourtype'] + '|Non-Billable') in data
