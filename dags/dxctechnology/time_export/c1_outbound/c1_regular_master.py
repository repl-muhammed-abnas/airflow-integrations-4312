...
import pendulum
from dxctechnology.time_export.c1_outbound.tasks.update_export_status import cancel_time_export
from dxctechnology.time_export.c1_outbound.utils import request_payload, response_filters, custom_methods
...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.c1_regular_time_export_master_dagid,
        description=f"DXC - C1 Regular Time Export Master - {config.instance}",
        ...
        get_all_c1_divisions = rail.RepliconServicePageOperator(
            task_id="get_all_c1_divisions",
            endpoint="/services/DivisionListService1.svc/GetData",
            data=request_payload.get_c1_divisions_payload,
        ...
        get_project_type_oef_details= rail.RepliconServiceOperator(
            ...
            data_handler=lambda response: rail.find_first_by_attr_and_get_attr(response, 'name', 'Project Type', 'uri')
        )
        ...
        get_oef_drop_down_values = rail.RepliconServiceOperator(
            ...
            data={
                "objectExtensionTagDefinitionUri": "{{ result('get_project_type_oef_details') }}"
            }
        )

        get_filter_data_for_c1_reg_time_export = rail.PythonOperator(
            task_id='get_filter_data_for_c1_reg_time_export',
            python_callable=custom_methods.filter_data_for_c1_reg_time_export,
            op_args=[config]
        )
        ...
        get_data_for_all_past_time_exports_for_C1 = rail.RepliconServiceOperator(
            ...
            data=lambda: request_payload.get_all_past_time_export_data_payload("REG-C1", rail.result("get_filter_data_for_c1_reg_time_export")),
        ...
        create_timedata_exportbatch = rail.RepliconServiceOperator(
            ...
            data=request_payload.create_c1_reg_time_export_payload
        )
        ...
        update_cancelled_timedataexport_name = rail.RepliconServiceOperator(
            ...
            data={
                ...
                "name": 'C1_cancel_{{ dag_run_ecid() }}_Reg',
            }
        )
        ...
        process_c1_regular_time_export = rail.TriggerDagRunOperator(
            task_id='process_c1_regular_time_export',
            retries=0,
            trigger_dag_id=config.c1_process_time_export_child_dagid,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda: {
                'downloadurl': null,
                'fileformaturi': rail.result('get_filter_data_for_c1_reg_time_export')['fileformaturi'],
        ...
        process_psa_regular_time_export = rail.TriggerDagRunOperator(
            ...
            conf=lambda: {
                'downloadurl': null,
                'fileformaturi': rail.find_first_by_attr_and_get_attr(rail.result("get_time_download_script"), 'displayText', config.psa_reg_time_export_file_format, 'uri'),
                'timeexporturi': rail.result('get_timedataexport_batchresults')['timeDataExportUri'],
                'twbname': rail.result('log_current_export_name'),
                'postdata': "yes",
                'lasttwbname': rail.result('get_data_for_all_past_time_exports_for_C1')[0]['timeexport'],
        ...
        get_process_starttime >> get_all_c1_divisions >> get_all_filter_definitions \
            >> get_time_download_script >> get_employeetype_groups >> get_project_type_oef_details \
                >> get_oef_drop_down_values >> get_filter_data_for_c1_reg_time_export \
        ...
        batch_end >> rail.Label('on success') >> process_c1_regular_time_export \
