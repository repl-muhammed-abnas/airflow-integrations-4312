...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_aus_terminationbalance_child_{config.instance}_v2',
        description=f'DXC_AUS_terminationbalance_child {config.instance} V2',
        ...
        send_email_for_no_users_data = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon payroll export for AUS Termination file is skipped for {{ dag_run.conf.location }} location  on - {{ current_time_in_specified_tz() }}',
            params={
                'start_date': request_payload.get_start_date_begin_of_week(config),
        ...
        query_disabled_users_data = rail.QueryCollectionOperator(
            ...
            # pylint: disable=line-too-long
            query=f"SELECT * FROM getalluserdata WHERE DATE(userenddate) > DATE('{request_payload.get_start_date_begin_of_week(config)}') AND  DATE(userenddate) < DATE('{request_payload.get_end_date_begin_of_week()}') AND exported != 'Yes' "
        )
        ...
        send_email_for_no_termination_balance_data = rail.EmailOperator(
            ...
            bcc=config.internal_logs_email,
            # pylint: disable=line-too-long
            subject='{{ get_company_key() }} | Replicon payroll export for AUS Termination file is skipped for Australia location  on - {{ current_time_in_specified_tz() }}',
            params={
                'start_date': request_payload.get_start_date_begin_of_week(config),
                'end_date': request_payload.get_end_date_begin_of_week()
            },
        ...
        # pylint: disable=line-too-long
        termination_balance_expected_report_columns = "User Name,Time Off Type,Time Off Balance,Employee ID,International Assignee,International assignee start date,IA PERNER ID,Actual Employee ID,User End Date,UserUri"
        ...
        get_exported_custom_field = rail.RepliconServiceOperator(
            ...
            response_filter=lambda response: rail.find_first_by_attr_and_get_attr(
                response.json()['d'], 'displayText', 'Term Exported (AUS)', 'uri')
        )
        ...
        process_child_udf_update = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_aus_termination_balance_udf_update_child_{config.instance}_v2',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: {
        ...
        termination_balance_report_data_collection >> query_final_payroll_collection >> query_active_user_balance_data >> query_valid_termination_balance_data
