...
import rail
from dxctechnology.lcsc_les_uk_ireland_payroll_extract.utils import request_payload, response_filter
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        start_date=datetime(2025, 5, 1, tz=config.time_zone),
        ...
        logging_details = rail.PythonOperator(
            ...
            op_args=[config.duration_days, config.time_zone]
        )
        ...
        search_entries_companycode_mapper = rail.PythonOperator(
            ...
            python_callable=lambda: request_payload.companycode_from_mapper(
                config.time_00_30, config.export, config.frequency, config.locations_company_codes_mapper)
        )
        ...
        batch_task >> logging_details >> get_specific_scripts >> get_specific_enabled_locations \
            >> get_specific_enabled_divisions >> get_specific_enabled_employee_type_groups \
                >> search_entries_companycode_mapper >> process_payrolldata_export
