...
import pendulum
import rail
from dxctechnology.time_entry_auto_submission.utils import custom_methods
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        is_valid_schedule = rail.IfOperator(
            ...
            test=custom_methods.schedule_interval,
        ...
        get_location_details = rail.PythonOperator(
            task_id='get_location_details',
            python_callable=lambda: custom_methods.get_location_details(
                config.country)
        )
        ...
        load_users_data_from_report = rail.run_report(
            ...
            report_params=custom_methods.get_load_users_data_from_report
        )
        ...
        has_load_users_data_from_report_data = rail.IfOperator(
            ...
            no_task='send_email_for_no_report_data'
        )

        send_email_for_no_report_data = rail.EmailOperator(
            task_id='send_email_for_no_report_data',
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | Automatic TimeEntry Submission Completed on - {{ current_time_in_specified_tz() }}',
            params={
                'start_date': custom_methods.get_dates()['start_date'],
                'end_date': custom_methods.get_dates()['today']
            },
            html_content='templates/email/empty_records.html',
        )
        ...
        query_timesheet_report_collection = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM timesheetreport WHERE employeetype != "Contractor" AND (daydiff < -8 AND totalhours > 0.01 AND totalhours != totaltimeoffhours)
        ...
        upload_file_to_sftp = rail.SFTPUploadFileOperator(
            ...
            config.country+'_{{ dag_run_ecid()}}.csv',
        ...
        log_to_sumo = rail.DagRunLogToSumoOperator(
            ...
            extra_info={
                ...
                'Start Date - End Date': custom_methods.get_dates()['start_date'] + ' - ' + custom_methods.get_dates()['today']
            }
        )
        ...
        is_valid_schedule >> rail.Label(
            "Yes") >> get_location_details

        is_valid_schedule >> rail.Label(
            "No") >> delete_this_dagrun

        get_location_details >> get_report_details >> get_enabled_locations >> \
        ...
        has_load_users_data_from_report_data >> rail.Label(
            "No") >> send_email_for_no_report_data >> end
