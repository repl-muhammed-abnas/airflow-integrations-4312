...
import rail
from dxctechnology.cwf_time_export_v3.utils import python_callable_method
from dxctechnology.cwf_time_export_v3.utils import request_payload
...
def get_compass_task(config, task_type, region, output_filename, compass_oef_name, internal_oef_name, unique_id):
    with rail.TaskGroup(group_id=f'compass_task_{task_type}', prefix_group_id=False) as compass_task:
        ...
        process_acknowledgement_not_received = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_acknowledgement_not_received_notification_{config.instance}_v3',
        ...
        def map_compass_row(item):
            ...
            return {
                ...
                "column_4": item['iwowbselement'] if item['companycodecode'] == "C1" or
                item['companycodecode'] in ['3001', '3124', '1602', '3118'] else item['projectname'],
        ...
        log_message_filename_compass = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: (output_filename[:4] if '{{ get_company_key() }}' ==
