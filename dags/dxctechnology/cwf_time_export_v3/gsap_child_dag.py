...
from dxctechnology.cwf_time_export_v3.utils import response_filter
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_cwf_time_export_gsap_child_{config.instance}_v3',
        description=f'DXCTechnology_CWF Time export - GSAP V3 {config.instance}',
        ...
        create_collection_create_list_from_csv_gsap = rail.CreateCollectionOperator(
            ...
            columns={
                ...
                'CWF GSAP alternate ID': 'c1cwfalternateid',
        ...
        query_list_filtered_data_gsap = rail.QueryCollectionOperator(
            ...
            query='''SELECT * FROM finaltimedata
                    WHERE
                        employeetypename LIKE '%Contractor%' AND companycodename in ('3001', '3124', '1602', '3118') OR
                        (employeetypename LIKE '%Contractor%' AND projecttype='IC' AND projectname LIKE 'X-%') OR
                        (employeetypename LIKE '%Contractor%' AND projecttype='GS')
        ...
        def map_row(item):
            return {
                ...
                "Workday_PERNR": item['perner'],
                "Date": datetime.strptime(item['entrydate'], config.entry_date_format).strftime("%Y%m%d"),
                "GSAP_WBS": (item['parentwbs']).rjust(8, "0") if item['parentwbs'] else item['projectname'],
                "Billing_Key": item['taskfullpath'].split('/')[0].strip() if item['taskfullpath'] else None,
                "Billing_Indicator": "X" if item['gsapbillableflag'] == "Billable" else '',
                "Tasks": item['gsaptaskcode'] if item['gsaptask'] else item['taskfullpath'].split('/')[-1].strip() if '/' in item['taskfullpath'] else None,
                "Hours": format(float(item['hours']), '.2f') if float(item['hours']) != 0 else '0',
                "Attendance_Absence_Type": item['attendancetypecode'] if item['attendancetypename'] == 'Training' else '2082',
                "Status": 30,
        ...
        has_rows_gsap_query_list_filtered_data >> rail.Label(
            ...
            query_list_gsapfinal >> get_final_line >> get_final_line_collection >> get_final_export_data >> create_document_gsap_xml >> upload_xmlfile_to_sftp
