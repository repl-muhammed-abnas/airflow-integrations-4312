...
import rail
from dxctechnology.lcsc_les_uk_ireland_termination_balance_v2.utils import request_payload
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        # NOTE: UDF update runs before SFTP upload by design (existing behavior).
        # This marks users as "processed" for the export batch. If SFTP fails,
        # the DAG fails and alerts are sent. Re-running requires manual UDF reset.
        # Out of scope for payroll calendar CR - separate enhancement if needed.
