...
import rail
from dxctechnology.adhoc.cwf_time_export_v4_adhoc.utils import python_callable_method
...
def get_compass_time_export_conf(export_name, data, config):
    return {
        ...
        'last_twb_uri': rail.result("completed_exports_list")['uri'],
        'twb_list': get_twb_list(data),
        'payload_identifier_replicon_uniqueid_pn1': export_name + ("|PN1" if config.instance == "DXCTechnology" else '|NT1'),
        'payload_identifier_replicon_uniqueid_pj1': export_name + ("|PJ1" if config.instance == "DXCTechnology" else '|NT3'),
        'payload_identifier_replicon_uniqueid_p01': export_name + ("|P01" if config.instance == "DXCTechnology" else '|NT2'),
        ...
        'last_twb_unique_indentifier_pn1': rail.result("completed_exports_list")['Timeexport'] + ("|PN1" if config.instance == "DXCTechnology" else '|NT1'),
        'last_twb_unique_indentifier_p01': rail.result("completed_exports_list")['Timeexport'] + ("|P01" if config.instance == "DXCTechnology" else '|NT2'),
        'last_twb_unique_indentifier_pj1': rail.result("completed_exports_list")['Timeexport'] + ("|PJ1" if config.instance == "DXCTechnology" else '|NT3'),
