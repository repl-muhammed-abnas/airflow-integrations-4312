...
import rail
from dxctechnology.adhoc.cwf_time_export_v4_adhoc.mapper.location import location_mapper
from dxctechnology.adhoc.cwf_time_export_v4_adhoc.task.field_glass_compass_task import get_compass_task
from dxctechnology.adhoc.cwf_time_export_v4_adhoc.task.c1_task import get_c1_task
from dxctechnology.adhoc.cwf_time_export_v4_adhoc.task.gsap_task import get_gsap_task
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_cwf_time_export_field_glass_{config.instance}_v4_adhoc',
        description=f'DXCTechnology_CWF Time - Fieldglass report export V4 {config.instance}',
        ...
        get_report_start_date = rail.PythonOperator(
            ...
            python_callable=lambda: Variable.get(config.cwf_fieldglass_report_start_date)
        )

        get_report_end_date = rail.PythonOperator(
            task_id='get_report_end_date',
            python_callable=lambda: Variable.get(config.cwf_fieldglass_report_end_date),
        )
        ...
        query_list_c1userdata = rail.CreateCollectionOperator(
            ...
            source=lambda: list(filter(lambda item: item['companycodecode'] == 'C1' and
                                       ...
                                       rail.load_all_records(rail.result("create_report_collection")))),
        ...
        has_empty_report_data >> rail.Label('Yes') >> send_mail_nodata
        ...
        has_invalid_report_field >> rail.Label(
            'No') >> load_report_csv >> create_report_collection >> query_list_c1userdata >> c1_task >> query_list_compassusersdata >> has_compassuserdata
