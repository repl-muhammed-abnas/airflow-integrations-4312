...
from os import path
import rail
from dxctechnology.gsap_task_import_project_fields_v2.utils import request_payload, response_filter
from dxctechnology.gsap_task_import_project_fields_v2.task.get_system_level_attribute import get_system_level_attribute
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        parse_xml = rail.LoadXMLFileOperator(
            ...
            xsd_document='./dags/dxctechnology/gsap_task_import_project_fields_v2/xsdschema/input_schema.xsd'
        )
        ...
        log_past_end_date_records = rail.WriteLogOperator(
            ...
            log = "{{result('master_dag_log')}}",
            message="End date more than 90 days old",
            ...
            properties=lambda item:{
                ...
                'action': 'Validation',
                'status': "Skipped",
                'details': "End date more than 90 days old"
            }
        )
        ...
        def get_all_gsap_tasks_from_replicon_page_handler(request, result):
            if result['length'] > 0:
                request['page'] += 1
                return request
            return None

        def combine_the_results_of_all_itr(res):
            final_response = []
            for response in res:
                if response['result']:
                    final_response.extend(response['result'])
            return rail.write_json_artifact(final_response)
        ...
        def get_formatted_data(response):
            ...
            return {
                ...
                "length": len(response['rows'])
            }

        get_gsap_task_from_replicon = rail.RepliconServicePageOperator(
            task_id="get_gsap_task_from_replicon",
            endpoint= "services/ObjectExtensionTagListService1.svc/GetData",
            data= request_payload.get_gsap_task_from_replicon_payload,
            data_handler= get_formatted_data,
            page_handler=get_all_gsap_tasks_from_replicon_page_handler,
            all_result_data_handler= combine_the_results_of_all_itr,
            flatten=True
        )

        create_replicon_task_collection = rail.CreateCollectionOperator(
            task_id = "create_replicon_task_collection",
            source="{{ result('get_gsap_task_from_replicon') | load_json_artifact | to_json}}"
        )
        ...
        sync_unique_gsap_task = rail.trigger_parallel_dagrun(
            ...
            trigger_dag_id=config.add_gsap_task,
            conf=lambda item: {
        ...
        get_parent_wbs_column_uri = rail.RepliconServiceOperator(
            task_id="get_parent_wbs_column_uri",
        ...
        get_all_gsap_tasks_from_replicon = rail.RepliconServicePageOperator(
            task_id= "get_all_gsap_tasks_from_replicon",
            endpoint= "services/ObjectExtensionTagListService1.svc/GetData",
            data= request_payload.get_gsap_task_from_replicon_payload,
            data_handler= get_formatted_data,
            page_handler=get_all_gsap_tasks_from_replicon_page_handler,
        ...
        process_log_generation = rail.TriggerDagRunOperator(
            ...
            retries=0,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            trigger_dag_id=config.log_generation,
            conf=lambda dag_run:{
        ...
        is_wbs_xml_present >> rail.Label("Yes") >> sync_gsap_task_import_file_parallel >> get_parent_wbs_column_uri >> get_all_filter_definition\
              >> get_all_gsap_tasks_from_replicon >> sync_gsap_task_import_file >> generate_output_log
