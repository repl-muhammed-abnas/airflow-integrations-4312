...
import rail
from dxctechnology.c1_wbs_import import python_callable_method
from dxctechnology.c1_wbs_import import response_filter
from dxctechnology.c1_wbs_import import field_config
from dxctechnology.c1_wbs_import import request_payload
...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_c1_wbs_import_child_project{dag_id_postfix}',
        description=f'DXC_C1 WBS Automation Child V2.0 - B1 {config.instance}',
        ...
        update_iwo_wbs_oef = rail.RepliconServiceOperator(
            ...
            data=request_payload.get_iwo_wbs_oef_update_param
        )
        ...
        validate_wbs_source_input_reference = rail.IfOperator(
            ...
            yes_task='validate_changed_on_date',
        ...
        can_copy_project_from_icwbs = rail.IfOperator(
            ...
            # no_task="has_valid_status_if_new_project",
        ...
        is_project_exist >> rail.Label(
            "Yes") >> update_iwo_wbs_oef >> get_all_project_task >> get_project_info_based_on_wbs_element
        ...
        can_get_permission_sets_for_user >> rail.Label(
            "Yes") >> get_permission_sets_for_user >> get_assign_permissionset_to_user_param >> \
        ...
        is_project_created >> rail.Label(
            'yes') >> put_keyvalue_for_project >> can_update_time_tracking_oef
        is_project_created >> rail.Label('no') >> can_update_time_tracking_oef
        can_update_time_tracking_oef >> rail.Label(
            "Yes") >> update_time_tracking_oef >> update_russia_udf >> can_update_client
        can_update_time_tracking_oef >> rail.Label(
            "No") >> update_russia_udf >> can_update_client
        can_update_client >> rail.Label(
            "Yes") >> get_client_info >> update_client >> log_completion
        can_update_client >> rail.Label("No") >> log_completion
