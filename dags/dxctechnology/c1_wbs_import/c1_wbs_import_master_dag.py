...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        description=f'DXC_C1_WBS_Automation Master V1.0 - SFTP {config.instance}',
        ...
        process_wbs_item = rail.TriggerDagRunForEachItemOperator(
        ...
        wait_for_process_wbs_item = rail.WaitForDagRunsSensor(
            task_id='wait_for_process_wbs_item',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs='{{ result("process_wbs_item") }}',
        )
        ...
        has_clients >> rail.Label(
            "Yes") >> process_clients >> wait_for_process_clients >> query_unique_programs_from_payload

        query_unique_programs_from_payload >> has_programs
        has_programs >> rail.Label("No") >> get_all_cost_center
        has_programs >> rail.Label(
            "Yes") >> process_programs >> wait_for_process_programs >> get_all_cost_center

        get_all_cost_center >> get_unique_cost_center_to_add >> has_cost_center

        has_cost_center >> rail.Label("No") >> process_wbs_item
        has_cost_center >> rail.Label(
            "Yes") >> process_cost_center >> wait_for_process_cost_center >> process_wbs_item

        process_wbs_item >> wait_for_process_wbs_item >> generate_output_log >> [
