...
def create_update_userprofile_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.gsap_update_userprofiles_dagid,
        description=f'DXC_Fieldglass_GSAP_Child_Update User {config.instance}',
        ...
        check_if_non_australia_contractor = rail.IfOperator(
            ...
            yes_task='update_non_aus_user_enddate',
            no_task='get_effective_group_date'
        )

        update_non_aus_user_enddate = rail.RepliconServiceOperator(
            task_id='update_non_aus_user_enddate',
        ...
        disable_non_aus_user_login = rail.RepliconServiceOperator(
            task_id='disable_non_aus_user_login',
        ...
        log_non_australia_contractor = rail.WriteLogOperator(
            ...
            message="Non-Australia Contractor, Profile disabled",
            properties={
                'userid': '{{ dag_run.conf.hpid }}',
                'email': '{{ dag_run.conf.emailaddress }}',
                'action': 'Update',
                'status': 'Exception',
                'details': "Non-Australia Contractor, Profile disabled"
            }
        )

        get_effective_group_date = rail.PythonOperator(
            task_id='get_effective_group_date',
            python_callable=python_callable_method.get_gsap_effective_group_date_payload,
        )
        ...
        apply_user_modifications2 = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: request_payload.apply_user_modifications2_gsap(
        ...
        should_update_employeetype = rail.IfOperator(
            ...
            test=lambda dag_run: request_payload.should_update_employeetype_gsap(
        ...
        update_employeetype_group = rail.RepliconServiceOperator(
            ...
            endpoint='/services/ImportService1.svc/ApplyUserModifications2',
            data=request_payload.update_employeetype_gsap
        )

        should_update_timesheet_template = rail.IfOperator(
            task_id='should_update_timesheet_template',
            test=request_payload.should_update_timesheet_template_gsap,
        ...
        bulk_get_user >> check_if_non_australia_contractor >> rail.Label(
            'Yes') >> update_non_aus_user_enddate

        update_non_aus_user_enddate >> disable_non_aus_user_login >> log_non_australia_contractor >> catch_and_log_errors

        check_if_non_australia_contractor >> rail.Label(
            'No') >> get_effective_group_date >> is_user_disabled
