...
from dxctechnology.cwf_user_profile_v1.user_profile_sync.mapper.dxc_cwf_user_mapper import cwf_user_mapper
from dxctechnology.cwf_user_profile_v1.user_profile_sync.mapper.gsap_user_mapper import gsap_user_mapper
...
def get_gsap_user_integration_mapper_data():
    return list(filter(lambda x: x['function'] == 'GSAP User Integration', gsap_user_mapper))


def get_gsap_effective_group_date_payload():
    ...
    effective_group = america_time_now.subtract(
        days=gsap_date_mapper[america_time_now.day_of_week])
...
def get_blank_mandatory_field_log_gsap(item):
    logs = []
    field_map = {
        'hpid': 'HPID not available',
        'firstname': 'First Name not available',
    ...
    for key in [*field_map]:
        if not item[key]:
            logs.extend(field_map[key]) if isinstance(
                field_map[key], list) else logs.append(field_map[key])
        elif key == 'financesystem' and (item['financesystem'] != 'GSAP'):
            logs.append('Finance system is not allowed')
