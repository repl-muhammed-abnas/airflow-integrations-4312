...
import pendulum
import rail
from dxctechnology.cwf_user_profile_v1.user_profile_sync.mapper.dxc_cwf_location_list import cwf_location_list
...
def get_costcenter_schedule_to_update(finance_system, afm_costcenter, user_costcenters):
                ...
                'replacementCostCenterScheduleEntries': [
                    {
                        ...
                        'effectiveDate': get_today_date() if finance_system != 'GSAP' else rail.result('get_effective_group_date')
                    }
                ]
            }
        }
...
def get_division_schedule_to_update(finance_system, company_code, company_parent_uri, user_divisions):
...
def get_location_schedule_to_update(financesystem, afmcostcenter, user_locations, user_costcenters):
    ...
    if financesystem == 'GSAP' and current_user_location_displayname:
        return {
            'userLocationScheduleModificationOptionUri': 'urn:replicon:schedule-modification-option:update-schedule-over-date-range',
            'updateLocationScheduleOverDateRange': {
                'replacementLocationScheduleEntries': [
...
def apply_user_modifications2(dag_run, should_add_emailaddress):
    ...
    return {
        ...
        'modifications': {
            ...
            'divisionScheduleToApply': get_division_schedule_to_update(dag_run.conf['financesystem'],
            ...
            'costCenterScheduleToApply': get_costcenter_schedule_to_update(dag_run.conf['financesystem'],
...
def get_custom_field_values_gsap(
        pernerid, employee_type, workertype_uri, workertype_value_uri, financesystem_uri, financesystem_value_uri, pernerudf_uri=null):
    ...
    custom_field_values = [{
        ...
        'text': pernerid
    }] if pernerudf_uri and pernerid and not employee_type == 'Contractor | Agency Contractor' else []
    if workertype_value_uri:
        custom_field_values.append({
            'customField': {
                'uri': workertype_uri
...
def get_gsap_put_user_payload(dag_run, should_add_emailaddress):
    ...
    return {
        'user': {
            ...
            'securityConfiguration': {
                ...
                'isLoginEnabled': 'false' if dag_run.conf['timetracking'].lower() == 'no' or dag_run.conf['employee_type'] == 'Contractor | Agency Contractor'
                else 'true',
                'loginName': dag_run.conf['emailaddress'],
                'SSOName': dag_run.conf['emailaddress']
            },
            ...
            'customFieldValues': get_custom_field_values_gsap(dag_run.conf['pernerid'], dag_run.conf['employee_type'],
                                                              dag_run.conf['worker_type_uri'], dag_run.conf['worker_type_value_uri'],
                                                              dag_run.conf['finance_system_uri'], dag_run.conf['finance_system_value_uri'],
                                                              dag_run.conf['perner_udf_uri']),
            'assignedActivities': list(map(lambda x: {
                'name': x
            }, list(set(dag_run.conf['activities'].split(','))))) if dag_run.conf['activities'] else [],
            ...
            ] if dag_run.conf['afmcostcenter'] and dag_run.conf['employee_type'] != 'Contractor | Agency Contractor' else null,
            'employeeTypeGroupSchedule': [
                {
                    'employeeTypeGroup': {
                        'uri': dag_run.conf['employee_type_uri'],
...
def should_update_employeetype_gsap(employee_type_uri, user_employeetypes):
    current_employeetype = user_employeetypes[0] if user_employeetypes else null
    ...
    return not current_employeetype_displayname or (
        current_employeetype_uri and current_employeetype_uri != employee_type_uri)


def should_update_timesheet_template_gsap(dag_run):
...
def get_gsap_userprofile_child_dag_conf(item):
...
def get_office_schedule_to_update_gsap(office_schedule, current_schedule):
    current_office_schedule = current_schedule[-1] if current_schedule else null
    current_office_schedule_displayname = current_office_schedule[
        'officeSchedule']['displayText'] if current_office_schedule else null
    if not current_office_schedule_displayname or (office_schedule != current_office_schedule_displayname):
        return {
...
def get_custom_field_values_to_update_gsap(pernerid, employee_type,
                                           financesystem, workertype, workertype_uri, workertype_value_uri, financesystem_uri, financesystem_value_uri, bulkgetuser_customfields, pernerudf_uri):

    financesystem_value_to_add = null
    workertype_value_to_add = null
    pernerid_value_to_add = null

    user_financesystem_customfield = rail.find_first_by_attr_and_get_attr(
        bulkgetuser_customfields, 'customField.displayText', 'Finance System (CWF)', 'text')
    user_workertype_customfield = rail.find_first_by_attr_and_get_attr(
        bulkgetuser_customfields, 'customField.displayText', 'Worker Type', 'text')
    user_pernerid_customfield = rail.find_first_by_attr_and_get_attr(
        bulkgetuser_customfields, 'customField.displayText', 'PERNER', 'text')
    ...
    if workertype_value_uri and workertype.lower() != (
        ...
        workertype_value_to_add = workertype_value_uri
    if pernerid:
        if pernerudf_uri and pernerid.lower() != (
                user_pernerid_customfield.lower() if user_pernerid_customfield else null):
            pernerid_value_to_add = pernerid

    return get_custom_field_values_gsap(pernerid_value_to_add, employee_type,
                                        workertype_uri, workertype_value_to_add, financesystem_uri, financesystem_value_to_add, pernerudf_uri)


def apply_user_modifications2_gsap(dag_run, should_add_emailaddress):
    emailaddress_bulk_get_user = rail.result(
        'bulk_get_user')['userDetails']['emailAddress']
    loginname_bulk_get_user = rail.result(
        'bulk_get_user')['securityConfiguration']['loginName']
    ...
    return {
        ...
        'modifications': {
            'schedulePolicyToApply': get_office_schedule_to_update_gsap(dag_run.conf['office_schedule'], rail.result('bulk_get_user')['schedulePolicies']),
            'locationScheduleToApply': get_location_schedule_to_update(
                dag_run.conf['financesystem'], dag_run.conf['afmcostcenter'], rail.result(
                    'get_effective_group_membership')['locations'], rail.result(
                        'get_effective_group_membership')['costCenters']),
            ...
            'customFieldValuesToApply': get_custom_field_values_to_update_gsap(dag_run.conf['pernerid'], dag_run.conf['employee_type'],
                                                                               dag_run.conf['financesystem'], dag_run.conf['workertype'],
                                                                               dag_run.conf['worker_type_uri'], dag_run.conf['worker_type_value_uri'], dag_run.conf[
                'finance_system_uri'], dag_run.conf['finance_system_value_uri'], rail.result(
                'bulk_get_user')['userDetails']['customFieldValues'], dag_run.conf['perner_udf_uri']),
            'userDetailsToApply': {
                'firstName': firstname_to_apply,
                'lastName': lastname_to_apply,
                'emailAddress': {
...
def update_employeetype_gsap(dag_run):
    ...
    def get_effective_date():
        ...
        effective_group = america_time_now.add(
            days=7 - gsap_date_mapper[america_time_now.day_of_week])
