...
import pendulum
import rail
from dxctechnology.compass_wbs_file_merger_v2.utils import python_callable_method
from dxctechnology.compass_wbs_file_merger_v2.utils import custom_method
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_compass_wbs_file_merger_master_{config.dag_id_postfix}_v2',
        description=f'DXCtechnology Compass Wbs file merger master v2.0 {config.dag_id_postfix}_v2',
        ...
        process_reading_files = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_compass_wbs_file_merger_child_{config.dag_id_postfix}_v2',
            conf={
        ...
        query_wbs_data = rail.QueryCollectionOperator(
            ...
            query='''SELECT * FROM wbs_combined_data INNER JOIN (SELECT WBS as wbs_1, MAX(CAST (sequance_number AS UNSIGNED)) as
                    seq_number FROM wbs_combined_data GROUP BY WBS) as wbs_combined_data_1 on
                    wbs_combined_data.WBS = wbs_combined_data_1.wbs_1 AND
