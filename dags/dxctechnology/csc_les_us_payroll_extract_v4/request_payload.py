...
import rail
from dxctechnology.csc_les_us_payroll_extract_v4.mapper.attendancetypemapper import ATTENDANCE_TYPE_MAP, ATTENDANCE_TYPE_1010_MAP
...
def companycode_from_mapper(time, export, frequency, mapper):
    ...
    list_of_codes = list(filter(lambda item: item['Time'] == time and item['Export']
                                == export and item['Frequency'] == frequency, mapper))
...
def process_payrolldata_export_us_conf(item):
    return {
        ...
        'sequence_no': item['sequence no']
    }
def process_payrolldata_export_conf(item):
    return {
        'file_format_name': item['File format name'],
        'file_format_uri': (rail.find_first_by_attr_and_get_attr(rail.result("get_all_scripts"
            ), "displayText", item['File format name'], "uri")) if rail.result("get_all_scripts") else None,
...
def get_compose_item_payroll_us_data_row(items):
    time_type = ""
    final_time_type=items['Pay_Code_Code']
    dag_run_conf = get_conf()
    if dag_run_conf['division_name'] and dag_run_conf['division_name'].lower() not in ['usa7', 'usb7']:
        if items['Pay_Code_Code'] == "1010":
            time_type = list(filter(lambda x:  x['Employee_Type'] == items['Employee_Type_Name'], ATTENDANCE_TYPE_1010_MAP))
            if time_type:
                final_time_type=time_type[0]['TypeToExport'] if time_type[0]['TypeToExport'] else items['Pay_Code_Code']
        else:
            time_type = list(filter(
                lambda x: x['AAType'] == items['Pay_Code_Code'], ATTENDANCE_TYPE_MAP))
            if time_type:
                final_time_type=time_type[0]['TypeToExport'] if time_type[0]['TypeToExport'] else items['Pay_Code_Code']
    ...
    if items['Actual_Employee_ID']:
        ...
        personnelnumber = items['CLID']
    return ["P2010",
            personnelnumber,
            "US",
            items['ORDNO'] if items['ORDNO'] else None,
            "INS",
            "2010",
            final_time_type,
            dt.strptime(items['BEGDA'], "%d %B %Y").strftime("%Y%m%d") if items['BEGDA'] else None,
            dt.strptime(items['ENDDA'], "%d %B %Y").strftime("%Y%m%d") if items['ENDDA'] else None,
            items['OBJPS'] if items['OBJPS'] else None,
            items['SPRPS'] if items['SPRPS'] else None,
            items['SEQNR'] if items['SEQNR'] else None,
            items['EXTRA'] if items['EXTRA'] else None,
            final_time_type,
            items['STDAZ'] if items['STDAZ'] else None,
            items['BEGUZ'] if items['BEGUZ'] else None,
            items['ENDUZ'] if items['ENDUZ'] else None,
            items['BETRG'] if items['BETRG'] else None,
            items['WAERS'] if items['WAERS'] else None,
