...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_gsap_task_project_fields_file_merger_child_{config.instance}',
        description='DXCtechnology GSAP Tasks Project Fields file merger child',
        ...
        download_tasks_file = rail.SFTPDownloadFileOperator(
            task_id='download_tasks_file',
        ...
        parse_xml = rail.LoadXMLFileOperator(
            task_id="parse_xml",
            document="{{result('download_tasks_file')}}",
            xsd_document="./dags/dxctechnology/gsap_task_project_fields_file_merger/xml_schema/input_schema.xsd"
        )

        has_data = rail.IfOperator(
            task_id='has_data',
            test='{{ result("parse_xml") | xpath("Records") | length > 0 }}',
            yes_task='create_tasks_collection',
        ...
        download_tasks_file >> parse_xml >> has_data
        has_data >> rail.Label(
            "Yes") >> create_tasks_collection >> get_query_to_merge >> query_tasks_data >> move_files_to_archive >> finish
        has_data >> rail.Label(
