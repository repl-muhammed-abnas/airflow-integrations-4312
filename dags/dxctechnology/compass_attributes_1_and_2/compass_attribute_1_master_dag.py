...
def create_attribute_1_master_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_compass_attribute_1_master_{config.dag_id_postfix}',
        description=f'DXC_Compass_Attribute 1 Master V1.0 {config.dag_id_postfix}',
        ...
        is_attributes_1_file = rail.IfOperator(
            task_id='is_attributes_1_file',
        ...
        send_bad_file_name_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon task sync for Compass Attribute 1 - Incorrect File Name - {{ current_time() }}',
        ...
        send_bad_file_format_email = rail.EmailOperator(
            ...
            bcc=config.internal_logs_email,
            # pylint: disable=line-too-long
            subject='{{ get_company_key() }} | Replicon task sync for Compass Attribute 1 - Incorrect file format {{ current_time() }}',
            html_content='templates/email/bad_file_format.html',
            params={'attributenumber': '1'},
        )
        ...
        send_blank_payload_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | Replicon task sync for Compass Attribute 1 - Blank Payload  {{ current_time() }}',
        ...
        process_each_wbs_attribute = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_compass_attribute_1_process_wbs_child_{config.dag_id_postfix}',
        ...
        def file_upload_failed(context):
            subject = '{{ get_company_key() }} | Replicon task sync for Compass Attributes 1 - Uploading Logs to SFTP failed  {{ current_time() }}'
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('get_errored_logs', key='length') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Replicon task sync for Compass Attributes 1 -  " }} \
        ...
        new_file_sensor >> is_attributes_1_file >> rail.Label(
            'Yes') >> is_xml
        is_attributes_1_file >> rail.Label(
