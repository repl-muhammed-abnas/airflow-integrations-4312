...
import json
import rail
from dxctechnology.cwf_time_export.mapper.location import location_mapper
from dxctechnology.cwf_time_export.task.compass_task import get_compass_task
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_cwf_time_export_compass_child_{config.instance}',
        description=f'DXCTechnology_CWF Time export - Compass V2.0 {config.instance}',
        ...
        has_row_count_finaltimedata = rail.IfOperator(
            task_id='has_row_count_finaltimedata',
            test="{{ result('create_collection_create_list_from_csv_finaltimedata','length') >0 }}",
            yes_task="query_list_filtered_data_estype",
        )
            ...
            query='''SELECT *
                    ...
                    WHERE
                        employeetypename LIKE '%Contractor%' AND companycodecode='COMPASS' OR
                         (employeetypename LIKE '%Contractor%' AND projecttype='ES' AND projectname LIKE 'E-%')
                    ORDER BY CAST(hours as DECIMAL) ASC
                    ''',
        )

        has_rows_es_type = rail.IfOperator(
            task_id='has_rows_es_type',
            test="{{ result('query_list_filtered_data_estype','length') > 0  }}",
            yes_task="get_data_division_list_service",
        )
        ...
        log_message_filename_compass = rail.PythonOperator(
            task_id='log_message_filename_compass',
            python_callable=lambda: datetime.utcnow().strftime("%Y%m%d_%H%M%S")
        )
        ...
        emea_task = get_compass_task(
            config=config, region='EMEA', task_type='emea', output_filename='P01_ReplicontoCOMPASS')

        amer_task = get_compass_task(
            config=config, region='AMER', task_type='amer', output_filename='PN1_ReplicontoCOMPASS')

        apac_task = get_compass_task(
            config=config, region='APAC', task_type='apac', output_filename='PJ1_ReplicontoCOMPASS')
        ...
        create_time_data_download_batch_compass >> batch_management_async_compass >> get_time_data_download_batch_results >> download_timedata_file >> \
            load_csv_create_list_from_csv_finaltimedata >> create_collection_create_list_from_csv_finaltimedata >> has_row_count_finaltimedata
        has_row_count_finaltimedata >> rail.Label(
            'Yes') >> query_list_filtered_data_estype >> has_rows_es_type
        has_rows_es_type >> rail.Label(
            'Yes') >> get_data_division_list_service >> map_division_list >> get_data_location_list_service >> \
            map_location_list_data >> log_message_filename_compass >> search_entries_location_map_compass >> create_csv_lines_estype >> \
