...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f"dxctechnology_australia_termination_balance_gsap_master_{config.instance}_v1",
        description=f"DXC - AUS_termination_balance_GSAP_Master - {config.instance}",
        ...
        dxc_payroll_extract_mapper_aus_search_entries = rail.PythonOperator(
            ...
            python_callable=lambda:  list(
                filter(lambda x: x["Parent_Company_code"] == "GSAP", COMPANY_CODE_MAP_AUS))
        )
        ...
        has_mapper_data = rail.IfOperator(
            ...
            test=lambda: bool(list(filter(lambda x: x, map(lambda x: rail.find_first_by_attr_and_get_attr(rail.result('get_all_scripts'), 'displayText', x['fileformat_name'], 'uri'), rail.result(
                'dxc_payroll_extract_mapper_aus_search_entries'))))) and rail.result('dxc_payroll_extract_mapper_aus_search_entries') and rail.result('dxc_payroll_extract_mapper_aus_search_entries')[0]['type'] == 'Gsap',
        ...
        query_list_users_with_allowed_companycode = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM  terminateduserslist WHERE  iscompanycodeallowed="Yes" """,
        ...
        process_terminated_users = rail.TriggerDagRunOperator(
            ...
            conf=lambda: request_payload.get_child_dagrun_conf('GSAP')
        )
