...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_child_wbs_sync_attributes_1_2_file{dag_id_postfix}_{config.sub_erp}_{config.attribute}',
        description=f'Sync Child WBS Attributes 1 or 2 {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.child_wbs_dag_sync_attribute_1_2_max_active_runs,
    ) as dag:
        ...
        sync_each_atrribute_project_level = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda item: {
                'WBS': request_payload.get_conf()['childWbs'],
