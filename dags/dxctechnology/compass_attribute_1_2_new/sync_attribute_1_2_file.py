...
from dxctechnology.compass_attribute_1_2_new import python_callable_method


def create_child_sync_attribute_1_2_dag(config):
    dag_id_postfix = f'_{config.instance}' if config.instance else ''
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_child_sync_attributes_1_2_file{dag_id_postfix}_{config.sub_erp}_{config.attribute}',
        ...
        is_wbs_present = rail.IfOperator(
            ...
            yes_task="fetch_iwo_wbs_element",
            no_task="log_wbs_not_present",
        )

        fetch_iwo_wbs_element = rail.PythonOperator(
            task_id="fetch_iwo_wbs_element",
            python_callable=python_callable_method.get_iwo_wbs_element,
            op_args=['get_project_details']
        )

        is_child_wbs_present = rail.IfOperator(
            task_id="is_child_wbs_present",
            test="{{ result('fetch_iwo_wbs_element') | length > 0}}",
        ...
        sync_child_wbs_attribute_1_2 = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_child_wbs_sync_attributes_1_2_file_{config.instance}_{config.sub_erp}_{config.attribute}',
            conf=lambda: {
                ...
                'childWbs': rail.result("fetch_iwo_wbs_element")[0]['textValue']
            }
        )
        ...
        sync_each_atrribute_project_level = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(days=config.execution_timeout_days),
            trigger_dag_id=f'dxctechnology_child_sync_each_attribute_project_level_{config.instance}_{config.sub_erp}_{config.attribute}',
            conf=lambda item: {
                'WBS': item['WBS'],
        ...
        is_wbs_present >> rail.Label(
            "YES") >> fetch_iwo_wbs_element >> is_child_wbs_present
