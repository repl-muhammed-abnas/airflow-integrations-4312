...
import json
import rail
from dxctechnology.cwf_time_export_v7.mapper.location import location_mapper
from dxctechnology.cwf_time_export_v7.task.compass_task import get_compass_task
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_cwf_time_export_compass_child_{config.instance}_v7',
        description=f'DXCTechnology_CWF Time export - Compass v7 {config.instance}',
        ...
        def map_rows(item):
            return [
                ...
                rail.find_first_by_attr_and_get_attr(rail.result('map_division_list'), 'companycodename', item['companycodename'], 'parent'),
                item['taskfullpath'],
                ...
                item['locationname'],
                rail.find_first_by_attr_and_get_attr(rail.result('map_location_list_data'), 'locationname', item['locationname'], 'parent'),
                item['loginname'],
                item['user'],
                item['iwowbselement'],
                item['workorderid'],
        ...
        create_csv_lines_estype = rail.WriteCSVFileOperator(
            ...
            header=['companycodecode','employeeid','perner','approvalstatus','entrydate','projectname','costcentercode','labortype','jobactivitytype','taskname','timetype','attendancetypecode',
                ...
                'timeofftypedescription','masterwbs','projecttype','iwoindicator','parentwbs','companycodename','companycodedesc','taskfullpath','length','timentryid2','employeetype','timesheetperiod',
