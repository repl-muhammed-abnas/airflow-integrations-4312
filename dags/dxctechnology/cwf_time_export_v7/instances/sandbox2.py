...
from dxctechnology.cwf_time_export_v7.config import *
...
pgp_conn_id = 'DXCSandbox2_pgp_cwf_time_export'
pgp_conn_id_psa = 'DXCSandbox2_pgp_cwf_time_export_psa'

field_glass_sftp_conn_id = 'dxcsandbox2-sftp-628172_fieldglass'
field_glass_output_filepath = '/Test/Outbound/CWFTimesheets'

c1_sftp_conn_id = 'dxcsandbox2-sftp-628172_C1'
c1_output_filepath = '/Test/Outbound/C1TimeExtract'

compass_sftp_conn_id = 'dxcsandbox2-sftp-628172_Compass'
compass_output_filepath = '/Test/Outbound/COMPASSTimeExtract'

gsap_sftp_conn_id = 'sftp_dxcsandbox2_gsap'
gsap_output_filepath = '/Outbound/'
gsap_http_conn_id = 'dxcsandbox2_POQ_GSAPTimeData'

psa_sftp_conn_id = 'sftp_dxcsandbox2_psa'
...
compass_http_conn_id = 'dxcsandbox2_POQ_CompassTimeData'
...
psa_wf39_sql_query = '''SELECT * FROM finaltimedata WHERE
            ...
            OR
            (
                employeetypename LIKE '%Contractor%' AND companycodecode = 'C1' AND CAST(hours as INT) = 0 AND ParentWBS IS Null
            )
            OR
            ((
            ...
            OR
            (
                employeetypename LIKE '%Contractor%' AND companycodecode='COMPASS' AND CAST(hours as INT) = 0 AND ParentWBS IS Null
            ))))
            AND
            ((
                psaflag IN ('x','X')
