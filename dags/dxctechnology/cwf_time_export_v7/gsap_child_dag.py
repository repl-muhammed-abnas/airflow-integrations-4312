...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_cwf_time_export_gsap_child_{config.instance}_v7',
        description=f'DXCTechnology_CWF Time export - GSAP v7 {config.instance}',
        ...
        query_filtered_data_for_reversal = rail.QueryCollectionOperator(
            ...
            query='''SELECT * FROM finaltimedata WHERE (timeentryid NOT IN (SELECT DISTINCT timeentryid FROM query_list_filtered_data_gsap) AND hours IN (0, 0.0))'''
        )
