...
from dxctechnology.cwf_user_profile.user_profile_sync.task.process_costcenters import process_costcenters_task_group
from dxctechnology.cwf_user_profile.user_profile_sync.utils import request_payload
from dxctechnology.cwf_user_profile.user_profile_sync.utils.response_filter import page_handler, map_employeetypes
from dxctechnology.cwf_user_profile.user_profile_sync.utils.python_callable_method import get_blank_mandatory_field_log, get_cwf_user_integration_mapper_data
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        process_userprofile_child_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_cwf_userprofiles_child_{config.instance}',
        ...
        process_supervisor_child_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'dxctechnology_cwf_userprofiles_supervisor_child_{config.instance}',
        ...
        process_log_generation = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'dxctechnology_cwf_userprofiles_log_child_{config.instance}',
            conf=lambda: {
