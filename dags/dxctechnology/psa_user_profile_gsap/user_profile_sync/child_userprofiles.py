...
import rail
from dxctechnology.psa_user_profile_gsap.user_profile_sync.utils.request_payload import validate_email, get_search_user_param, get_adduser_updateuser_conf
from dxctechnology.psa_user_profile_gsap.user_profile_sync.utils.response_filter import map_user_details
from dxctechnology.psa_user_profile_gsap.user_profile_sync.task.process_updateuser import process_updateuser_task_group
...
def create_process_userprofile_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_psa_userprofiles_child_gsap_{config.instance}',
        description=f'DXC_PSA UserProfiles_Child {config.instance}',
        ...
        is_emailaddress_not_valid = rail.IfOperator(
            ...
            test=lambda dag_run: bool(
                validate_email(dag_run.conf['email'])),
        ...
        log_emailaddress_not_valid = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                'action': 'Validation',
                'status': 'Exception',
                'details': validate_email(dag_run.conf['email'])
            }
        )
        ...
        is_employeeid_present = rail.IfOperator(
            ...
            test="{{ result('get_user_data_from_employeeid') | filter_by_attr('employeeid', 'equals', dag_run.conf.contractorpern) | \
                length > 0 }}",
        ...
        log_multiple_employeeid_exception = rail.WriteLogOperator(
            ...
            properties={
                ...
                'action': 'Validation',
                'status': 'Exception',
                'details': 'Multiple users available with employee id: "{{ dag_run.conf.contractorpern }}"'
            }
        )

        update_user_employeeid, wait_for_update_user_employeeid = process_updateuser_task_group(
            config.execution_timeout_days, config.instance, 'emp_id')
        ...
        is_user_with_loginname_present = rail.IfOperator(
            ...
            test="{{ result('get_user_data_from_loginname') | find_first_by_attr_and_get_attr('loginname', dag_run.conf.email, 'uri') | \
                is_truthy }}",
        ...
        update_user_loginname, wait_for_update_user_loginname = process_updateuser_task_group(
            config.execution_timeout_days, config.instance, 'login_name')
        ...
        is_financesystemuri_workertypeuri_present = rail.IfOperator(
            ...
            yes_task='validate_contract_startdate_enddate',
        ...
        add_userprofile_child_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'dxctechnology_psa_userprofiles_add_child_gsap_{config.instance}',
        ...
        is_financesystemuri_workertypeuri_present >> rail.Label(
            'Yes') >> validate_contract_startdate_enddate
