...
from dxctechnology.psa_user_profile_gsap.user_profile_sync.task.process_costcenters import process_costcenters_task_group
from dxctechnology.psa_user_profile_gsap.user_profile_sync.utils import request_payload
from dxctechnology.psa_user_profile_gsap.user_profile_sync.utils.response_filter import page_handler, map_employeetypes
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_psa_userprofiles_gsap_master_{config.instance}',
        description='DXC_PSA UserProfiles_GSAP_Master',
        ...
        send_blank_payload_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon user sync for PSA user profile - No records to process - {{ current_time_in_specified_tz() }}',
        ...
        query_any_blankmandatory_check = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM rawdata
                    WHERE NULLIF(contractorpern,'') IS NULL OR NULLIF(firstname,'') IS NULL OR NULLIF(lastname,'') IS NULL OR NULLIF(email,'') IS NULL OR
                    NULLIF(costcenter,'') IS NULL OR NULLIF(companycode,'') IS NULL OR NULLIF(orgunitcode,'') IS NULL OR NULLIF(workertype,'') IS NULL OR
                    NULLIF(contractstartdate,'') IS NULL OR NULLIF(contractenddate,'') IS NULL OR (erp != 'C1' AND erp != 'ES' AND erp != "GSAP") OR
        ...
        write_blankmandatory_field_log = rail.WriteLogOperator(
            ...
            properties=lambda item: {
                'userid': item['contractorpern'],
                'email': item['email'],
                'action': 'Validation',
                'status': 'Skipped',
                'details': python_callable_method.get_blank_mandatory_field_log(item)
            }
        )

        get_mapper_data = rail.PythonOperator(
            task_id='get_mapper_data',
            python_callable=lambda: python_callable_method.get_psa_user_integration_mapper_data(
                config)
        )
        ...
        load_inputdata_from_rawdata = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM rawdata
                    WHERE NULLIF(contractorpern,'') IS NOT NULL AND NULLIF(workertype,'') IS NOT NULL AND NULLIF(firstname,'') IS NOT NULL AND NULLIF(lastname,'') IS NOT NULL AND
                    NULLIF(email,'') IS NOT NULL AND NULLIF(costcenter,'') IS NOT NULL AND NULLIF(companycode,'') IS NOT NULL AND NULLIF(orgunitcode,'') IS NOT NULL AND
                    NULLIF(contractstartdate,'') IS NOT NULL AND NULLIF(contractenddate,'') IS NOT NULL AND (erp = 'C1' OR erp = 'ES' OR erp = 'GSAP') AND
        ...
        should_process_userprofiles = rail.IfOperator(
            ...
            yes_task=['get_all_activities', 'get_all_permissionsets', 'get_financesystem_workertype_customfield',
                      'get_all_employeetypes', 'get_all_companycodes', 'get_all_department_groups', 'get_all_policysets',
                      'get_all_costcenters'],
        ...
        process_userprofile_child_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_psa_userprofiles_child_gsap_{config.instance}',
        ...
        process_supervisor_child_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'dxctechnology_psa_userprofiles_supervisor_child_gsap_{config.instance}',
        ...
        generate_output_log = rail.EmptyOperator(
            task_id='generate_output_log')
        ...
        process_log_generation = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'dxctechnology_psa_userprofiles_log_child_gsap_{config.instance}',
            conf=lambda: {
        ...
        get_time_for_file >> decrypt_file >> rail.Label(
        ...
        should_process_userprofiles >> rail.Label(
            'Yes') >> [get_all_activities, get_all_permissionsets, get_financesystem_workertype_customfield,
                       get_all_employeetypes, get_all_companycodes, get_all_department_groups, get_all_policysets, get_all_costcenters] >> \
        ...
        is_supervisorcheck_pending >> rail.Label(
            'Yes') >> process_supervisor_child_dag >> wait_for_supervisor_child_dag >> generate_output_log

        is_supervisorcheck_pending >> rail.Label(
            'No') >> generate_output_log

        generate_output_log >> gather_logs >> process_log_generation
