...
import rail
from dxctechnology.psa_user_profile_gsap.user_profile_sync.utils.python_callable_method import do_format_logs
...
def create_log_userprofile_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_psa_userprofiles_log_child_gsap_{config.instance}',
        description=f'DXC_PSA UserProfiles_Child_Log Generation GSAP {config.instance}',
        ...
        is_sftp_upload_failed = rail.IfOperator(
            ...
            test='{{get_task_state("upload_log_to_sftp") == "failed"}}',
            yes_task="send_time_data_to_sftp_failure_email",
            no_task="check_logs"
        )

        send_time_data_to_sftp_failure_email = rail.EmailOperator(
            task_id='send_time_data_to_sftp_failure_email',
            to=config.alert_email,
            # pylint: disable=line-too-long
            subject="{{ get_company_key() }} | Replicon user sync for PSA user profile - Uploading Logs to SFTP failed - {{ current_time_in_specified_tz() }}",
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('get_errored_logs', key='length') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Replicon user sync for PSA user profile - " }} \
        ...
        format_logs >> render_logs_csv >> upload_log_to_sftp >> is_sftp_upload_failed >> rail.Label(
            "Yes") >> send_time_data_to_sftp_failure_email >> check_logs >> \
        ...
        is_sftp_upload_failed >> rail.Label(
            "No") >> check_logs
        return dag
