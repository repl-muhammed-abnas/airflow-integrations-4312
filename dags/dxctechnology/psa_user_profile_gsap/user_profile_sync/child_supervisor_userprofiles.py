...
from dxctechnology.psa_user_profile_gsap.user_profile_sync.utils.response_filter import get_missing_permissions, map_supervisor_list_data
from dxctechnology.psa_user_profile_gsap.user_profile_sync.utils.python_callable_method import compose_supervisor_details, add_supervisorcheck_user_log
...
def create_supervisor_userprofile_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_psa_userprofiles_supervisor_child_gsap_{config.instance}',
        description=f'DXC_PSA UserProfiles_Child_Supervisor Assignment GSAP {config.instance}',
        ...
        is_supervisor_disabled = rail.IfOperator(
            ...
            yes_task='supervisor_empty_operator2',
        ...
        check_logs >> [filter_child_logs,
                       get_data_for_supervisor] >> get_matching_supervisor >> is_supervisor_present

        is_supervisor_present >> rail.Label(
            'Yes') >> is_supervisor_disabled

        is_supervisor_disabled >> rail.Label(
            'Yes') >> supervisor_empty_operator2 >> check_supervisor_enddate
        ...
        is_child_log_entries_present >> rail.Label(
            'No') >> write_supervisor_entry_complete >> on_error
