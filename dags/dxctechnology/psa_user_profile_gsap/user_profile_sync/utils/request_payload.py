...
import rail
from dxctechnology.psa_user_profile_gsap.user_profile_sync.mapper.dxc_psa_location_list import psa_location_list
        ...
        "costCenter": {
            "parent": {
                "uri": rail.find_first_by_attr_and_get_attr(rail.result(
                    "get_all_replicon_costcenters"), 'displayText', 'PSA Cost Center', 'uri')
            }
        },
        'modifications': {
...
def get_child_cost_centers_data():
    return {
        "page": "1",
        "pagesize": "111",
        "columnUris": [
...
def update_costcenter_in_replicon(item):
    return {
        "costCenter": {
            "uri": rail.find_first_by_attr_and_get_attr(rail.result(
                "get_all_replicon_costcenters"), 'displayText', item['costcenter'], 'uri')
        },
        "target": {
            "uri": rail.find_first_by_attr_and_get_attr(rail.result(
                "get_all_replicon_costcenters"), 'displayText', 'PSA Cost Center', 'uri')
...
def get_userprofile_child_dag_conf(item):
    ...
    filtered_mapper = list(filter(lambda x: x['workertype'] == item['workertype'] and x['financesystem']
                           == item['erp'], rail.result('get_mapper_data'))) if rail.result('get_mapper_data') else []
    ...
    if not filtered_mapper:
        ...
        financesystem_exception = 'Required Finance System is not present in mapper' if len(
            [x for x in rail.result('get_mapper_data') if x['financesystem'] == item['erp']]) == 0 else ''
    ...
    return {
        ...
        **{
            ...
            'finance_system_value_uri': rail.find_first_by_attr_and_get_attr(
                rail.result('get_financesystem_customfield_dropdowns'), 'displayText', item['erp'], 'uri'),
            ...
            'department_group_uri': rail.find_first_by_attr_and_get_attr(
                rail.result('get_all_department_groups'), 'displayText', item['orgunitcode'], 'uri'),
            'default_deparment_uri': rail.find_first_by_attr_and_get_attr(
                rail.result('get_all_department_groups'), 'displayText', 'DXC', 'uri'),
            'cost_center_uri': rail.find_first_by_attr_and_get_attr(
                rail.result('get_all_costcenters'), 'displayText', item['costcenter'], 'uri'),
            ...
            'parent_cost_center_uri': rail.find_first_by_attr_and_get_attr(
                rail.result("get_all_replicon_costcenters"), 'displayText', 'PSA Cost Center', 'uri')
        }}
...
def get_search_user_param(dag_run):
    ...
    return {
        ...
        'filterExpression': {
            ...
            'rightExpression': {
                'value': {
                    'text': dag_run.conf['email'] if login_name_check else dag_run.conf['contractorpern']
                }
            }
        }
    }
...
def get_adduser_updateuser_conf(dag_run, item):
    ...
    attribute_value_to_search = dag_run.conf['email']
    ...
    if rail.result('is_employeeid_not_unique') and rail.result('is_employeeid_not_unique') == 'update_userprofile_child_dag_emp_id':
        ...
        attribute_value_to_search = dag_run.conf['contractorpern']
...
def get_custom_field_values(
        workertype_uri, workertype_value_uri, financesystem_uri, financesystem_value_uri, pernerudf_uri=null, contractorpern=null):
...
def get_put_user_payload(dag_run, should_add_emailaddress):
    ...
    cwf_location_name = rail.find_first_by_attr_and_get_attr(
        psa_location_list, 'country', dag_run.conf['costcenter'][0:2], 'name') if dag_run.conf.get(
        'costcenter') and dag_run.conf.get('erp') == 'ES' else null
    ...
    return {
        'user': {
            ...
            'emailAddress': dag_run.conf['email'] if should_add_emailaddress else null,
            'employeeId': dag_run.conf['contractorpern'],
            ...
            'customFieldValues': get_custom_field_values(dag_run.conf['worker_type_uri'], dag_run.conf['worker_type_value_uri'],
                                                         ...
                                                         dag_run.conf['perner_udf_uri'], dag_run.conf['contractorpern']),
            ...
            ] if dag_run.conf['costcenter'] else null,
            "departmentGroupSchedule": [
                {
                    "departmentGroup": {
                        'uri': dag_run.conf['department_group_uri'] if dag_run.conf[
                            'department_group_uri'] else dag_run.conf['default_deparment_uri'],
                    }
                }
            ] if dag_run.conf['orgunitcode'] else null,
            ...
            'displayNameParameter': {
                'displayName': f"{dag_run.conf['lastname']}, {dag_run.conf['firstname']} {dag_run.conf['contractorpern']} {dag_run.conf['email']}"
            }
        }
    }
...
def validate_enddate_with_startdate(start_date, end_date, fmt=null):

    if fmt:
        return datetime.fromisoformat(end_date) < datetime.strptime(
            f"{start_date['year']}-{start_date['month']}-{start_date['day']}", fmt) if end_date and start_date else False
    return datetime.fromisoformat(end_date) < datetime.fromisoformat(start_date) if end_date else False


def update_supervisor_enddate(user_uri, start_date):
    return {
        'userUri': user_uri,
        'dateRange': {
...
def get_costcenter_schedule_to_update(costcenter, user_costcenters, cost_center_uri, parent_cost_center_uri):
    ...
    if not current_user_costcenter_displayname or (
            current_user_costcenter_displayname and current_user_costcenter_displayname.lower() != costcenter.lower()) and cost_center_uri is not None:
        return {
            ...
            'updateCostCenterScheduleOverDateRange': {
                'replacementCostCenterScheduleEntries': [
                    {
                        'costCenter': {
                            "parentUri": parent_cost_center_uri,
                            'name': costcenter
                        },
...
def get_organizational_unit_to_update(orgunitcode, user_orgunitcode, department_group_uri, default_deparment_uri):
    current_user_orgunitcode = user_orgunitcode[0] if user_orgunitcode else null
    current_user_orgunitcode_displayname = current_user_orgunitcode[
        'department']['department']['displayText'] if current_user_orgunitcode and current_user_orgunitcode.get(
            'departments') else null
    if not current_user_orgunitcode_displayname or (
            current_user_orgunitcode_displayname and current_user_orgunitcode_displayname.lower() != orgunitcode.lower()):
        return {
            ...
            "updateDepartmentGroupScheduleOverDateRange": {
                "replacementDepartmentGroupScheduleEntries": [
                    {
                        "departmentGroup": {
                            "uri": department_group_uri if department_group_uri else default_deparment_uri
                        },
...
def get_location_schedule_to_update(financesystem, costcenter, user_locations, user_costcenters):
    ...
    if financesystem == 'ES' and costcenter:
        current_user_costcenter = user_costcenters[0] if user_costcenters else null
        current_user_costcenter_displayname = current_user_costcenter[
            'costCenter']['costCenter']['displayText'] if current_user_costcenter and current_user_costcenter.get(
                'costCenter') else null
        if not current_user_costcenter_displayname or (
                current_user_costcenter_displayname and current_user_costcenter_displayname.lower() != costcenter.lower()):
            cwf_location_name = rail.find_first_by_attr_and_get_attr(
                psa_location_list, 'country', costcenter[0:2], 'name')
...
def apply_user_modifications2(dag_run, should_add_emailaddress):
    ...
    employeeid_to_apply = get_attribute_to_update(
        dag_run.conf['contractorpern'], ('userDetails', 'employeeId')) if dag_run.conf['contractorpern'] else null
    ...
    emailaddress_to_apply = dag_run.conf[
        'email'] if should_add_emailaddress and (not emailaddress_bulk_get_user or dag_run.conf[
            'email'].lower() != emailaddress_bulk_get_user.lower()) else null
    ...
    return {
        ...
        'modifications': {
            'locationScheduleToApply': get_location_schedule_to_update(
                dag_run.conf['erp'], dag_run.conf['costcenter'], rail.result(
            ...
            'costCenterScheduleToApply': get_costcenter_schedule_to_update(
                dag_run.conf['costcenter'], rail.result('get_effective_group_membership')['costCenters'], dag_run.conf[
                    'cost_center_uri'], dag_run.conf['parent_cost_center_uri']) if dag_run.conf['costcenter'] else null,
            "departmentGroupScheduleToApply": get_organizational_unit_to_update(
                dag_run.conf['orgunitcode'], rail.result('get_effective_group_membership')['departments'], dag_run.conf[
                    'department_group_uri'], dag_run.conf['default_deparment_uri']) if dag_run.conf[
                    'orgunitcode'] else null,
            ...
            } if dag_run.conf['email'] != loginname_bulk_get_user else null,
            'customFieldValuesToApply': get_custom_field_values_to_update(
                dag_run.conf['erp'], dag_run.conf['workertype'],
            ...
            'userDetailsToApply': {
                ...
                'displayNameParameter': {
                    'displayName': get_displayname(dag_run.conf['firstname'], dag_run.conf['lastname'], dag_run.conf['contractorpern'], dag_run.conf['email'])
