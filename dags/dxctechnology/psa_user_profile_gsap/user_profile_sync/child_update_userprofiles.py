...
from dxctechnology.psa_user_profile_gsap.user_profile_sync.utils import python_callable_method
from dxctechnology.psa_user_profile_gsap.user_profile_sync.utils import request_payload
...
def create_update_userprofile_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_psa_userprofiles_update_child_gsap_{config.instance}',
        description=f'DXC_PSA_Child_Update User GSAP {config.instance}',
        ...
        check_company_code_for_gsap = rail.IfOperator(
            ...
            test=python_callable_method.check_company_code,
            yes_task='get_effective_group_date',
        ...
        log_update_user_exception = rail.WriteLogOperator(
            task_id='log_add_user_exception',
            log='{{ dag_run.conf.log }}',
            message='User is Disabled in Replicon as received company code is not GSAP contractor company code',
            properties={
                'userid': '{{ dag_run.conf.contractorpern }}',
                'email': '{{ dag_run.conf.email }}',
                'action': 'Add',
                'status': 'Exception',
                'details': 'User is Disabled in Replicon as received company code is not GSAP contractor company code'
            }
        )
        ...
        is_user_disabled = rail.IfOperator(
            ...
            test=lambda dag_run: not rail.result('bulk_get_user')[
                'userDetails']['isEnabled'],
            yes_task='check_end_date_for_disabled_user',
            no_task='validate_contract_enddate_with_user_startdate'
        )

        check_end_date_for_disabled_user = rail.IfOperator(
            task_id='check_end_date_for_disabled_user',
            test=lambda dag_run: not request_payload.validate_enddate_with_startdate(datetime.now().strftime(
                "%Y-%m-%d"), dag_run.conf['contractenddate']),
            yes_task='enable_user_for_rehire',
            no_task='log_user_already_disabled_exception'
        )
        ...
        validate_contract_enddate_with_user_startdate = rail.IfOperator(
            ...
            test=lambda dag_run: request_payload.validate_enddate_with_startdate(rail.result(
                'bulk_get_user')['userDetails']['employmentDateRange']['startDate'], dag_run.conf['contractenddate'], '%Y-%m-%d'),
        ...
        is_contract_end_date_present = rail.IfOperator(
            ...
            yes_task='update_employee_date_range',
        ...
        enable_user_for_rehire = rail.IfOperator(
            ...
            test=lambda dag_run: not rail.result('bulk_get_user')[
                'userDetails']['isEnabled'] and not request_payload.validate_enddate_with_startdate(
        ...
        def validate_enddate_update(end_date):
            if end_date:
                ...
                user_end_datetime = datetime.strptime(
                    f"{user_end_date['year']}-{user_end_date['month']}-{user_end_date['day']}", '%Y-%m-%d') if user_end_date else 'No Date'
                validate_date = not request_payload.validate_enddate_with_startdate(
                    user_start_date, end_date, '%Y-%m-%d')
                return datetime.fromisoformat(end_date) != user_end_datetime and user_enabled and validate_date
            return False
        ...
        def validate_startdate_update(startdate):
            if startdate:
                user_start_date = rail.result('bulk_get_user')[
                    'userDetails']['employmentDateRange']['startDate']
                user_enabled = rail.result('bulk_get_user')[
                ...
                user_start_datetime = datetime.strptime(
                    f"{user_start_date['year']}-{user_start_date['month']}-{user_start_date['day']}", '%Y-%m-%d') if user_start_date else 'No Date'
                validate_date = not request_payload.validate_enddate_with_startdate(
                    user_start_date, startdate, '%Y-%m-%d') if datetime.fromisoformat(startdate) != user_start_date else False
                return datetime.fromisoformat(startdate) != user_start_datetime and user_enabled and validate_date
            return False

        startdate_update = rail.IfOperator(
            task_id='startdate_update',
            test=lambda dag_run: validate_startdate_update(
                dag_run.conf['contractstartdate']),
            yes_task='update_user_startdate',
            no_task='should_disable_user'
        )

        update_user_startdate = rail.RepliconServiceOperator(
            task_id='update_user_startdate',
            endpoint='/services/UserService1.svc/UpdateEmploymentDateRange',
            data=lambda dag_run: request_payload.get_update_employee_date_range_payload(
                dag_run.conf['user_uri'], dag_run.conf['contractenddate'], dag_run.conf['contractstartdate'])
        )
        ...
        should_disable_user = rail.IfOperator(
            ...
            test=lambda dag_run: rail.result('bulk_get_user')[
                    'userDetails']['isEnabled'] and dag_run.conf['contractenddate'],
            yes_task='is_contract_enddate_less_than_user_startdate_disable_user',
            no_task='get_effective_group_membership'
        )

        is_contract_enddate_less_than_user_startdate_disable_user = rail.IfOperator(
            task_id='is_contract_enddate_less_than_user_startdate_disable_user',
            test=lambda dag_run: not request_payload.validate_enddate_with_startdate(rail.result(
                'bulk_get_user')['userDetails']['employmentDateRange']['startDate'], dag_run.conf['contractenddate'], '%Y-%m-%d'),
        ...
        should_disable_login = rail.IfOperator(
            ...
            test=lambda dag_run: datetime.fromisoformat(
                dag_run.conf['contractenddate']) < datetime.fromisoformat(time.strftime('%Y-%m-%d')),
        ...
        (get_data_for_supervisor, update_supervisor_over_date_range, log_supervisor_check, is_supervisor_same, get_update_activities_payload) = process_supervisor_assignment_task_group(
            config.execution_timeout_days, is_update_user=True)
        ...
        should_update_workweek_start_day = rail.IfOperator(
            ...
            no_task='is_timeentry_approvalpath_exist'
        )
        ...
        get_exception_logs = rail.PythonOperator(
            ...
            op_args=['validate_contract_enddate_with_user_startdate', 'is_contract_enddate_less_than_user_startdate_disable_user',
                     'should_update_supervisor', 'check_supervisor_enddate']
        )
        ...
        check_company_code_for_gsap >> rail.Label(
            "Yes") >> get_effective_group_date >> bulk_get_user >> is_user_disabled

        check_company_code_for_gsap >> rail.Label(
            "No") >> disable_user_in_replicon >> log_update_user_exception >> catch_and_log_errors

        is_user_disabled >> rail.Label(
            'Yes') >> check_end_date_for_disabled_user >> rail.Label("Yes") >> enable_user_for_rehire

        check_end_date_for_disabled_user >> rail.Label(
            "No") >> log_user_already_disabled_exception >> catch_and_log_errors

        is_user_disabled >> rail.Label(
            'No') >> validate_contract_enddate_with_user_startdate
        ...
        is_contract_end_date_present >> rail.Label(
            'Yes') >> update_employee_date_range >> log_user_disabled_in_replicon >> catch_and_log_errors
        ...
        enddate_update >> rail.Label(
            'Yes') >> update_user_enddate >> startdate_update

        enddate_update >> rail.Label(
            'No') >> startdate_update

        startdate_update >> rail.Label(
            'Yes') >> update_user_startdate >> should_disable_user

        startdate_update >> rail.Label(
            'No') >> should_disable_user

        should_disable_user >> rail.Label(
            'Yes') >> is_contract_enddate_less_than_user_startdate_disable_user

        is_contract_enddate_less_than_user_startdate_disable_user >> rail.Label(
        ...
        should_update_employeetype >> rail.Label(
            'Yes') >> update_employeetype_group >> should_update_timesheet_template
        ...
        should_update_workweek_start_day >> rail.Label(
            'Yes') >> update_workweek_start_day >> is_timeentry_approvalpath_exist

        should_update_workweek_start_day >> rail.Label(
            'No') >> is_timeentry_approvalpath_exist

        is_timeentry_approvalpath_exist >> rail.Label(
            'Yes') >> get_user_timeentry_approval_path >> should_update_timeentry_approvalpath >> get_exception_logs

        should_update_timeentry_approvalpath >> rail.Label(
            'Yes') >> update_timeentry_approval_path >> get_exception_logs
