...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'adhoc_dxctechnology_australia_payrolldata_export_user_schedule_child_v1_{config.instance}',
        description=f'Adhoc_DXC_Australia_PayrollData_Export_User_Schedule_Child V1 {config.instance}',
        ...
        create_csv_lines_for_raw_data = rail.WriteCSVFileOperator(
            ...
            row=lambda item: [
                ...
                dt.strptime(item['Entry_Date'], '%d %B %Y').strftime("%Y-%m-%d"),
        ...
        query_invalid_user_schedule_balance_data = rail.QueryCollectionOperator(
            ...
            query='''SELECT * FROM user_schedule_balance_report_data_collection WHERE NULLIF(Employee_Id, '') IS NULL OR Employee_Id == '' OR (Schedule_Name == 'Shift Schedule' AND (NULLIF(Shift_Description, '') IS NULL OR Shift_Description == '' ))''',
        )
        ...
        query_employee_data_collection = rail.QueryCollectionOperator(
            ...
            query="SELECT DISTINCT Employee_Id,Shift_Description,Office_Schedule FROM active_userbalance WHERE NULLIF(Employee_Id, '') IS NOT NULL AND ((Schedule_Name == 'Shift Schedule' AND NULLIF(Shift_Description, '') IS NOT NULL) OR Schedule_Name != 'Shift Schedule')",
        )
