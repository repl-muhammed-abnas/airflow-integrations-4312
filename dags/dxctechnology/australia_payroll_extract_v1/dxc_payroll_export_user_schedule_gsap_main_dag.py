...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"dxctechnology_australia_payrollexport_gsap_user_schedule_master_dag_v1_{config.instance}",
        description=f"DXC_AUS_PayrollExport_GSAP_User_Schedule_Master V1 {config.instance}",
        ...
        process_user_schedule_payrolldata_export = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'dxctechnology_australia_payrolldata_export_user_schedule_child_v1_{config.instance}',
