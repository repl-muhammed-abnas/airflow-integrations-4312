...
def get_required_wbs_to_process():
    def get_proposed_wbs_type(item):
        if item['gsap_project_type'] == '19' and item['controlling_area'] == '1004' and item['sold_to_party'].startswith("I") \
            and item['parent_controlling_area'] == '1004':
            return 'DIWO'
        return 'GSAP'

    data = rail.load_all_records(rail.result('base_report_collection'))
    return list(filter(lambda x:x['wbs_type'] != x['proposed_wbs_type'],map(lambda item:{
        "project_name": item['project_name'],
        "project_uri": item['project_uri'],
        "gsap_project_type": item['gsap_project_type'],
        "sold_to_party": item['sold_to_party'],
        "controlling_area": item['controlling_area'],
        "parent_controlling_area": item['parent_controlling_area'],
        "wbs_type": item['wbs_type'],
        "proposed_wbs_type":  get_proposed_wbs_type(item)
    },data )))
