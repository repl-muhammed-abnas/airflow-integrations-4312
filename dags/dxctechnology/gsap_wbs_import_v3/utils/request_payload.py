...
def get_project_conf(item):
    ...
    return{
        ...
        'parentcontrollingareauri':rail.result('get_all_object_extension_fields')['parentcontrollingareauri'],
        'gsapchilduri': rail.result('get_all_object_extension_fields')['gsapchilduri'],
        'gsapchildtaguris': rail.result('get_oef_drop_down_values_gsap_child')['tags']
    }
...
def is_diwo_wbs(dag_run):
    if  dag_run.conf['gsapparentwbs']:
        if does_parent_project_exist():
            ...
            feed_file_parent_data = rail.load_all_records(rail.result('query_gsap_parent_wbs_data_from_feed'))
            if not feed_file_parent_data:
                if dag_run.conf['projecttype'] == '19' and dag_run.conf['controllingarea'] == '1004' and\
                    dag_run.conf['soldtoparty'].startswith("I"):
                    return True
            else:
                parent_controlling_area = rail.find_first_by_attr_and_get_attr(feed_file_parent_data, 'Controlling_Area', '1004','Controlling_Area')
                if parent_controlling_area:
                    return True
    return False
...
def get_oefs(dag_run):
    ...
    if dag_run.conf['gsapparentwbs']:
        ...
        feed_file_parent_data = rail.load_all_records(rail.result('query_gsap_parent_wbs_data_from_feed'))
        if feed_file_parent_data:
            parent_controlling_area = feed_file_parent_data[0]['Controlling_Area']
            add_text_oef(parent_controlling_area, dag_run.conf['parentcontrollingareauri'])
        else:
            add_text_oef(null, dag_run.conf['parentcontrollingareauri'])
    ...
    if is_diwo_wbs(dag_run):
        wbstypetaguri = rail.find_first_by_attr_and_get_attr(dag_run.conf['wbstypetaguris'], 'name', 'DIWO', 'uri')
    ...
    if dag_run.conf['gsapparentwbs']:
        gsap_child_oef_uri = rail.find_first_by_attr_and_get_attr(dag_run.conf['gsapchildtaguris'], 'name', 'YES', 'uri')
        add_dropdown_oef(dag_run.conf['gsapchilduri'], gsap_child_oef_uri)
...
def get_iwo_oefs(dag_run):
    ...
    if dag_run.conf['gsapparentwbs']:
        if current_parent_oef_values:
            parent_controlling_area_oef_value = rail.find_first_by_attr_and_get_attr(
                    current_parent_oef_values, 'definition.displayText', 'Controlling Area', 'textValue')
            add_text_oef(parent_controlling_area_oef_value,dag_run.conf['parentcontrollingareauri'])
        else:
            add_text_oef(null,dag_run.conf['parentcontrollingareauri'])
...
def get_process_child_wbs_conf(item, dag_run):
    return{
        ...
        'parentcontrollingareauri': dag_run.conf['parentcontrollingareauri']
    }
...
def update_oef_fields_c1_compass(dag_run):
    ...
    parent_controlling_area = rail.find_first_by_attr_and_get_attr(
        current_parent_oef_values, 'definition.displayText', 'Controlling Area', 'textValue')
    add_text_oef(parent_controlling_area, dag_run.conf['parentcontrollingareauri'])
...
def update_wbs_type_oef(item):
    tag_values = rail.result('get_oef_drop_down_values_wbs_type')['tags']
    def get_tag_uri():
        if item['proposed_wbs_type']=='DIWO':
            return rail.find_first_by_attr_and_get_attr(tag_values, 'name', 'DIWO', 'uri')
        return rail.find_first_by_attr_and_get_attr(tag_values, 'name', 'GSAP', 'uri')
    return {
        "objectUri": item['project_uri'],
        "value": {
            "definition": {
                "uri": rail.result('get_all_object_extension_fields')['wbstypeuri'],
                "name": None
            },
            "tag": {
                "uri": get_tag_uri()
