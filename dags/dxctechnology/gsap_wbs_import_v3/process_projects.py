...
def create_child_dag_wbs(config):
    with rail.create_airflow_dag(
        dag_id=config.process_wbs_dagid,
        ...
        is_parent_gsap = rail.IfOperator(
            task_id="is_parent_gsap",
            test=lambda dag_run : bool(dag_run.conf['gsapparentwbs']),
            yes_task="query_gsap_parent_wbs_data_from_feed",
            no_task="get_project_info_on_parentwbs",
        )

        query_gsap_parent_wbs_data_from_feed = rail.QueryCollectionOperator(
            task_id='query_gsap_parent_wbs_data_from_feed',
            name='gsap_parent_wbs_feed_data',
            query="""SELECT DISTINCT * FROM gsaprecordscollection WHERE WBS_Name == :gsap_parent_project_name LIMIT 1""",
             query_params = {
                'gsap_parent_project_name': '{{ dag_run.conf.gsapparentwbs }}'
            }
        )
        ...
        process_blob_update = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.process_blob_dagid,
        ...
        process_child_wbs = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.child_wait_execution_timeout_days),
            trigger_dag_id=config.process_child_projects_dagid,
        ...
        has_parent_wbs >> rail.Label(
            'Yes') >> is_parent_gsap >> rail.Label('No') >> get_project_info_on_parentwbs
        is_parent_gsap >> rail.Label('Yes') >> query_gsap_parent_wbs_data_from_feed >> get_project_info_on_parentwbs
        get_project_info_on_parentwbs >> does_parentwbs_exist >> rail.Label('Yes') >> get_project_info_based_on_wbs_element
