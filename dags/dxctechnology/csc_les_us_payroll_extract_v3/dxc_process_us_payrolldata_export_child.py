...
import rail
from dxctechnology.csc_les_us_payroll_extract_v3 import request_payload
from dxctechnology.csc_les_us_payroll_extract_v3 import response_filter


def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_lcsc_location_company_codewise_usa_payrolldata_export_child_v3_{config.instance}',
        ...
        get_file_name = rail.PythonOperator(
            ...
            python_callable=lambda: config.file_name_prefix +"_"+ dt.utcnow().strftime("%Y%m%d%H%M%S")\
                + "_USREPL_REPL" + request_payload.get_sequence_no() +"_DUT8G2I"
        )
        ...
        query_final_payroll_data_without_empid = rail.QueryCollectionOperator(
            ...
            query='SELECT * From finalpayrolldata WHERE CLID IS NULL OR CLID="" '
        )
        ...
        compose_item_payroll_csv_file = rail.WriteCSVFileOperator(
            ...
            "PayCodeHours","ZEINH"],
            row=request_payload.get_compose_item_payroll_us_data_row,
        )
