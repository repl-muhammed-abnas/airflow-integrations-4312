...
from dxctechnology.compass_wbs_import_v1.ensure_user_has_permissions import ensure_user_has_permissions
from dxctechnology.compass_wbs_import_v1.validate_persons_responsible import validate_persons_responsible

def create_child_active_airflow_dag(config):

    with rail.create_airflow_dag(
        dag_id=config.active_project_dagid,
        description='DXC_COMPASS_WBS_Automation Child V2.0 - Active Projects Processing',
        ...
        has_childs_wbs = rail.IfOperator(
            ...
            yes_task="process_time_tracking_attribute",
            no_task="log_completion",
        )

        process_time_tracking_attribute = rail.TriggerDagRunForEachItemOperator(
            task_id = 'process_time_tracking_attribute',
            retries=0,
            items="{{ result('get_data_of_child_wbs') | to_json }}",
            trigger_dag_id=config.process_time_tracking_attribute_dagid,
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
        ...
        has_childs_wbs >> rail.Label('Yes') >> process_time_tracking_attribute >> log_completion
