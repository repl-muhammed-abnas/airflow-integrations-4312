...
import rail
from dxctechnology.custom_notification_shiftassignment_india import request_payload, response_payload


def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_custom_notification_shiftassignment_child_{config.instance}',
        description=f'DXC Custom notification for shiftassignment - Child V1.0 {config.instance}',
        ...
        has_data = rail.IfOperator(
            task_id='has_data',
            test=lambda: bool(rail.result("list_of_usernames")),
        ...
        query_userdata >> get_user_shift_details >> list_of_usernames >> get_dates_for_email >> has_data >> rail.Label(
            "Yes ") >> render_logs_csv >> generate_download_link >> get_email_body >> get_final_payload >> send_email_supervisor
        send_email_supervisor >> finish
