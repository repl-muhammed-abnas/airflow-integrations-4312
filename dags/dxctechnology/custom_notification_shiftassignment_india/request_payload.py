...
def parse_list_of_users(data):
    ...
    filtered_data = list(filter(lambda item: item['count'] < 5, response))
    final_data = list(map(lambda x: {'user': x['username']}, filtered_data))
    return final_data


def get_shift_details(item):
    startdate_topass = datetime.strptime(item['startdatetocheck'], '%d %B %Y')
    enddate_topass = datetime.strptime(item['enddatetocheck'], '%d %B %Y')
    return {
        "search": {
            "userSearch": {
