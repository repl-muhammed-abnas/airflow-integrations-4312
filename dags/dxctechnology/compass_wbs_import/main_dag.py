...
import rail
from dxctechnology.compass_wbs_import.get_active_project_prereqs import get_active_project_prereqs

#config : https://github.com/replicon/airflow-integrations/blob/main/dags/dxctechnology/compass_wbs_import/config.py

def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_compass_wbs_import_{config.instance}',
        description=f'{config.instance}_COMPASS_WBS_Automation Master V3.0 - SFTP - P01',
        ...
        if config.debug:
            # when debugging this will cause the file to never get archived, so it
            # will just keep running over and over again
        ...
        parse_xml = rail.LoadXMLFileOperator(
            ...
            xsd_document='./dags/dxctechnology/compass_wbs_import/input_schema.xsd'
        )
        ...
        process_inactive_projects = rail.trigger_parallel_dagrun(
            ...
            trigger_dag_id=f'dxctechnology_compass_wbs_import_child_inactive_project_{config.instance}',
        ...
        process_programs = rail.trigger_parallel_dagrun(
            ...
            trigger_dag_id=f'dxctechnology_compass_wbs_import_child_program_{config.instance}',
        ...
        process_clients = rail.trigger_parallel_dagrun(
            ...
            trigger_dag_id=f'dxctechnology_compass_wbs_import_child_client_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf= lambda item: {
        ...
        process_active_projects = rail.trigger_parallel_dagrun(
            ...
            parallel_count=config.trigger_parallel_dagrun_count,
            trigger_dag_id=f'dxctechnology_compass_wbs_import_child_active_project_{config.instance}',
            conf={
                'wbs': '{{ item.WBS }}',
                ...
                'projecttypeuri': '{{ result("get_project_oefs").projecttype }}',
                ...
                'projecttype': '{{ result("get_projecttype_values") | filter_by_attr("name", "equals", item.ProjectType) | first_or_default | attr_or_default("uri", "") }}',
