...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        description=f'DXC_WF39 PSA Resource Assignment Master Compass - SFTP {config.instance}',
        ...
        query_valid_labour_type_data = rail.QueryCollectionOperator(
            task_id='query_valid_labour_type_data',
        ...
        has_valid_labour_types_data = rail.IfOperator(
            task_id="has_valid_labour_types_data",
            test="{{ result('query_valid_labour_type_data','length') > 0 }}",
            yes_task="distinct_labour_types",
        ...
        distinct_labour_types = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT role FROM labourtypesdata WHERE NULLIF(role, '') IS NOT NULL"""
        )
        ...
        has_labour_types_to_be_created = rail.IfOperator(
            ...
            no_task="get_billing_rates_after_create"
        )
        ...
        create_billing_rates = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.billing_rate_child_dagid,
            conf=lambda item: {
                'name': item['role']
            }
        )
        ...
        query_min_start_max_end_assignment_dates = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT
                        ...
                        ) as max_end_date, -- Gives the maximum date from the end_date field for the wbs-user records
                        (
                            SELECT MIN(l1.startdate) FROM labourtypesdata l1
                            WHERE l1.wbs = unique_wbs_user_data.wbs
                                AND l1.employeeid = unique_wbs_user_data.employeeid
                        ) as min_start_date, -- Gives the minimum date from the start_date field for the wbs-user records
                        (
                            SELECT GROUP_CONCAT(unique_roles."role", '{CONCAT_STRING_DELIMITER}') FROM (
                                SELECT DISTINCT l1."role" FROM labourtypesdata l1 WHERE l1.wbs = unique_wbs_user_data.wbs AND l1.employeeid = unique_wbs_user_data.employeeid
                                ) unique_roles -- Gives the unique roles(labour-types) for the wbs-user records
                        ) as roles -- returns ['a', 'b'] as a{CONCAT_STRING_DELIMITER}b
                    FROM (
        ...
        query_valid_user_data = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM raw_feed_report_data WHERE NULLIF(user_uri,'') IS NOT NULL and 
                    user_status == 'Enabled' and user_company_code LIKE 'COMPASS%' """,
        ...
        query_invalid_user_data = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM raw_feed_report_data WHERE NULLIF(user_uri,'') IS NULL or 
                    user_status != 'Enabled' or user_company_code NOT LIKE 'COMPASS%' """,
        ...
        def logMessage_user(item):
            ...
            if not str(item['user_company_code']).startswith("COMPASS"):
                return f"Required user {item['employeeid']} is not Compass User"
        ...
        log_invalid_user_records = rail.WriteLogOperator(
            ...
            items="{{result('query_invalid_user_data')}}",
            log='{{ result("create_master_log") }}',
        ...
        process_billing_rate_for_each_wbs_item = rail.trigger_parallel_dagrun(
            ...
            execution_timeout=timedelta(days=config.execution_timeout_days),
            trigger_dag_id=config.distinct_wbs_child_dagid,
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_log_records', key='get_errored_billing_rates') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Replicon Compass-PSA Resource Assignment for WF39 - " }} \
        ...
        dummy_load_data >> load_labour_types_data >> create_master_log >> \
        ...
        get_billing_rates_before_create >> existing_billing_rates_in_replicon >> [query_valid_labour_type_data, query_invalid_labour_type_data]
        query_valid_labour_type_data >> has_valid_labour_types_data

        has_valid_labour_types_data >> rail.Label("Yes") >> distinct_labour_types
        has_valid_labour_types_data >> rail.Label("Yes") >> format_log_records
        ...
        distinct_labour_types >> labour_types_to_be_created_in_replicon >> \
        ...
        has_labour_types_to_be_created >> rail.Label(
            "No") >> get_billing_rates_after_create
        ...
        load_report_data >> report_data_collection >> query_min_start_max_end_assignment_dates >> feed_report_data_combined
        feed_report_data_combined >> [query_valid_user_data, query_invalid_user_data]
        query_invalid_user_data >> has_invalid_user_data
        has_invalid_user_data >> rail.Label('Yes') >> log_invalid_user_records
        has_invalid_user_data >> rail.Label('No') >> format_log_records

        query_valid_user_data >> has_any_valid_records
        has_any_valid_records >> rail.Label('Yes') >> process_each_wbs_item
        has_any_valid_records >> rail.Label('No') >> format_log_records
        
        log_invalid_user_records >> format_log_records
        
        
        process_each_wbs_item >> process_billing_rate_for_each_wbs_item >> \
            get_process_each_wbs_dag_ids >> gather_process_billing_rates_logs \
