...
from dxctechnology.workday_user_import.user_import.common_utils.request_payload import get_todays_date_for_timezone_in_json, get_json_date_from_date_str, TIMESHEET_PERIOD_EFFECTIVE_DATE
from dxctechnology.workday_user_import.user_import.common_utils.custom_methods import get_todays_minus_specified_days_date_in_json, get_json_date_from_date
...
def _get_custom_fields_to_assign(dag_run, action):
    ...
    if action == "update":
        ...
        if (is_ia != rail.result("current_assigned_udf_values")['is_ia']):
            udfs_to_assign.append(_add_custom_field(custom_fields['international_assignee']['uri'], text=dag_run.conf['file_data']['is_ia']))
            if is_ia in [1,'1']:
                # If user is now an IA, ensure start date is provided and is not in the future
                if not dag_run.conf['file_data']['ia_start_date']:
                    udfs_to_assign.append(_add_custom_field(custom_fields['international_assignee_start_date']['uri'],
                                         date=get_todays_date_for_timezone_in_json()))
