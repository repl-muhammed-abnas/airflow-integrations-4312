...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        query_department_fields_from_input = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT vr.orgcode, vr.orgname FROM raw_valid_records vr WHERE NULLIF(vr.orgcode, '') IS NOT NULL""",
        ...
        query_costcenter_fields_from_input = rail.QueryCollectionOperator(
            ...
            query = """SELECT DISTINCT vr.costcenter, vr.costcentername FROM raw_valid_records vr WHERE NULLIF(vr.costcenter, '') IS NOT NULL"""
        )
        ...
        query_location_fields_from_input = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT DISTINCT vr.country, vr.state, vr.country || '{request_payload.LOCATION_DELIMITER}' || vr.state as full_path
                        FROM raw_valid_records vr WHERE NULLIF(vr.country, '') IS NOT NULL""",
        ...
        query_service_centers_field_from_input = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT paygroup FROM raw_data rd WHERE NULLIF(rd.paygroup, '') IS NOT NULL""",
        ...
        query_employee_data = rail.QueryCollectionOperator(
            ...
            query="SELECT DISTINCT rd.empgroupcode, rd.empgroupname, rd.empsubgroupcode, rd.empsubgroupname FROM raw_data rd",
        ...
        query_schedules_to_create = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT vr.workshift FROM raw_valid_records vr
