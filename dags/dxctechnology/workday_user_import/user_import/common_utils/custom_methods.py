...
def get_date_to_use_for_no_accrual(dag_run, default_return="", return_type='json'):
    if dag_run.conf['ia_updated'] in [True, 'true', 'True']:
        if dag_run.conf['is_ia'] in [1,'1']:
            if return_type == "str":
                return f"{dag_run.conf['ia_start_date']['year']}-{dag_run.conf['ia_start_date']['day']}-{dag_run.conf['ia_start_date']['month']}"
            return dag_run.conf['ia_start_date']
        if dag_run.conf['is_ia'] in [0,'0']:
            _end_date = convert_json_date_to_date(dag_run.conf['ia_end_date']) + timedelta(days=1)
            if return_type == "str":
