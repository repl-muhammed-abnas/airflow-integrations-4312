...
def create_us_sick_timeoff_non_california_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.usa_les_us_sick_leave_non_california_user_timeoff_assignment_dag_id,
        ...
        get_tenure = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: common_custom_methods.get_tenure_value(
                                                    ...
                                                    date_2= common_custom_methods.convert_json_date_to_date(dag_run.conf['effective_date_to_use_for_sick_timeoff']))
        )
        ...
        def get_policy_to_assign_update_callable(dag_run):
            ...
            schedule_changed_date = dag_run.conf['effective_date_to_use_for_sick_timeoff']
        ...
        def get_policy_to_assign_add_callable(dag_run):
            ...
            for policy in rail.result("get_default_timeoff_policy_set_schedule_for_timeoff_type"):
                ...
                value_to_be_gsubbed_from_policy = f"""{OPEN_BRACKETS}"keyUri": "urn:replicon:script-key:parameter:accrual-annual-amount", "value": {OPEN_BRACKETS}"number": {yearly_accrual_amount}{CLOSE_BRACKETS}{CLOSE_BRACKETS}"""
                ...
                if max_diff:
                    for glb_policy in policies_from_global:
                        if max_diff == glb_policy['diff']:
                            ...
                            _value_to_be_gsubbed_from_policy = f"""{OPEN_BRACKETS}"keyUri": "urn:replicon:script-key:parameter:accrual-annual-amount", "value": {OPEN_BRACKETS}"number": {_yearly_accrual_amount}{CLOSE_BRACKETS}{CLOSE_BRACKETS}"""
