...
from dxctechnology.workday_user_import.user_import_global.utils.custom_methods import is_profile_enabled, get_replicon_date
...
def get_user_custom_field_update_payload(dag_run, user_details):
    ...
    if (is_ia != rail.find_first_by_attr_and_get_attr(
        ...
        if is_ia in [1, '1']:
            custom_fields_payload.append(
                get_custom_fields_payload(uri=dag_run.conf['udfs']['international_assignee_start_date'].get('uri'), date_value=get_todays_date_in_json()))
    ...
    return custom_fields_payload, can_update_notifications, False, null
...
def prepare_update_user_payload_callable(dag_run, config):
    ...
    if dag_run.conf['timezone']['timezone'] and (dag_run.conf['timezone']['timezone_uri'] != _get_user_details['timeZone']['uri']):
