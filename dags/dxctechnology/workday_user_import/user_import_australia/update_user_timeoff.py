...
from dxctechnology.workday_user_import.user_import.common_utils.custom_methods import get_specified_json_date_minus_specified_days_months_years_date_in_json
from dxctechnology.workday_user_import.user_import_australia.utils.custom_methods import get_trigger_id_callable,\
...
from dxctechnology.workday_user_import.user_import_australia.tasks.assign_timeoff_lsl_prorata import assign_lsl_prorata_timeoff
from dxctechnology.workday_user_import.user_import_australia.tasks.assign_annual_leave_timeoff import assign_annual_leave_timeoff
from dxctechnology.workday_user_import.user_import.common_utils.custom_methods import convert_json_date_to_date, get_json_date_from_date
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        def get_required_details_for_timeoff_assignment_callable(dag_run):
            ...
            for assigned_timeoff in _all_assigned_timeoff_data:
                ...
                if assigned_timeoff['name'].startswith('[AUS] LSL Pro rata Accrual'):
                    ...
                    continue
            rail.set_result(key = "lsl_assigned", val=lsl_assigned)
            return {
        ...
        is_location_updated = rail.IfOperator(
            ...
            test=lambda dag_run: False,# dag_run.conf['location_updated'] and dag_run.conf['location_updated'].lower() == "yes",
        ...
        def is_uri_present_in_timeoff_to_be_assigned_list_test():
            ...
            return not (rail.find_first_by_attr_and_get_attr(rail.result('get_required_details_for_timeoff_assignment')['to_name_and_uri'],
                                                        'uri',
