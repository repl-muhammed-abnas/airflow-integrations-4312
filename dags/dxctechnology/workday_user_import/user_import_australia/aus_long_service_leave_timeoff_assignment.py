...
from dxctechnology.workday_user_import.user_import_australia.utils.response_filter import get_transactions_history_aus_prorata_accrual_timeoff_data_handler
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        def get_updated_policy_line(_policy):
        ...
        def get_policy_to_assign_callable(dag_run, add_new_policy_line=True):
            ...
            if add_new_policy_line:
                policy_to_assign.append(get_new_policy_line_to_add(dag_run, 
                    gbl_custom_methods.convert_json_date_to_date(dag_run.conf['json_formatted_dates']['schedule_change_date']) - timedelta(days=1)
                )
                                        )           
