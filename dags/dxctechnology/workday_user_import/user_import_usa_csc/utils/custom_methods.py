...
from dxctechnology.workday_user_import.user_import.common_utils.custom_methods import convert_json_date_to_date, compare_if_two_json_dates_are_same as _compare_if_two_json_dates_are_same
...
def _get_payrule_schedule_to_update(dag_run, _get_user_details, effective_date, mapper):
    if dag_run.conf['payrule']['payrule']:
        payrule_value = dag_run.conf['payrule']['payrule']
        if dag_run.conf['file_data']['work_shift'] and dag_run.conf['file_data']['parent_company'] == "C1":
            if dag_run.conf['file_data']['country'].lower() == "puerto rico":
                ...
                if temp_payrule_value:
                    payrule_value = temp_payrule_value
...
def get_update_user_payload(dag_run, config):
    ...
    payload = {
        ...
        "modifications": {
            ...
            "payRulesScheduleModifications": _get_payrule_schedule_to_update(dag_run, user_details, effective_date, config.DXC_WORKDAY_USER_SYNC_USER_MAPPER),
