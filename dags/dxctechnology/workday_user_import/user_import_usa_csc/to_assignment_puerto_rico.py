...
from dxctechnology.workday_user_import.user_import.common_utils \
    import custom_methods as common_custom_methods

OPEN_BRACKETS = '{'
CLOSE_BRACKETS = '}'

def create_us_puerto_rico_timeoff_assignment_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.usa_csc_us_puerto_rico_user_timeoff_assignment_dag_id,
        description="dxctechnology workday user sync process users child",
        replicon_conn_id=config.replicon_conn_id,
        company_key=config.company_key,
        start_date=datetime(2023, 9, 26),
        max_active_runs=config.max_run_puerto_rico_to_assignment
    ) as dag:
        ...
        get_tenure = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: common_custom_methods.get_tenure_value(
                date_1= common_custom_methods.convert_json_date_to_date(dag_run.conf['contineous_service_date']),
                date_2=common_custom_methods.convert_json_date_to_date(dag_run.conf['start_date']))
        )
        ...
        def get_policy_set_to_assign_update_callable(dag_run):
        ...
        get_policy_set_to_assign_update = rail.PythonOperator(
            task_id = "get_policy_set_to_assign_update",
            python_callable=get_policy_set_to_assign_update_callable
        )
        ...
        def get_policy_to_assign_add_callable(dag_run):
            ...
            continuous_service_date = dag_run.conf['contineous_service_date']

            policies_to_assign = []
            policies_sets = rail.result("get_policy_set_to_assign_update")

            fte = dag_run.conf['fte']

            if dag_run.conf['fte'] in [1, '1']:
                for policy_set in policies_sets:
                    if policy_set['first'] == "Yes":
                        policies_to_assign.append(
                            {
                                "description": f"Effective on {start_date['day']}/{start_date['month']/start_date['year']}",
                                "effectiveDate": start_date,
                                "policySet": policy_set['policy']
                            }
                        )
                    else:
                        effective_date_to_use = common_custom_methods.get_date_from_json_date(continuous_service_date) + relativedelta(years=policy_set['offset'])
                        policies_to_assign.append(
                            {
                ...
                for policy in policies_sets:
                    weekly_accrual_params = rail.find_first_by_attr_and_get_attr(policy['policy']['timeOffBalanceEventScripts'],
                        'script.name','Weekly Accrual','additionalParameters')
                    ...
                    accrual_amt_based_on_schedule = yearly_accrual_amount * float(fte)
                    ...
                    updated_policy =  loads(dumps(policy['policy']).replace(value_to_be_gsubbed_from_policy, actual_value_to_be_added))
        ...
        def get_policy_to_assign_update_callable(dag_run):
            ...
            if dag_run.conf['fte'] in [1, '1']:
                ...
                for policy in policies_sets:
                    ...
                    value_to_be_gsubbed_from_policy = f"""{OPEN_BRACKETS}"keyUri":"urn:replicon:script-key:parameter:accrual-annual-amount","value":{OPEN_BRACKETS}"number": {weekly_accrual_params}{CLOSE_BRACKETS}{CLOSE_BRACKETS}"""
                    actual_value_to_be_added = f"""{OPEN_BRACKETS}"keyUri":"urn:replicon:script-key:parameter:accrual-annual-amount","value":{OPEN_BRACKETS}"number": {accrual_amt_based_on_schedule}{CLOSE_BRACKETS}{CLOSE_BRACKETS}"""
                    updated_policy =  loads(dumps(policy['policySet']).replace(value_to_be_gsubbed_from_policy, actual_value_to_be_added))
        ...
        has_policy_to_assign = rail.IfOperator(
            ...
            test = lambda: rail.result("get_policy_to_assign_add") or rail.result("get_policy_to_assign_update"),
            yes_task = "put_user_time_off_account_policy_set_schedule",
            no_task = "catch_and_log_error"
        )

        def get_policy_to_consider(dag_run):
            return rail.result('get_policy_to_assign_add') if common_custom_methods.is_caller_add_update_rehire(
        ...
        put_user_time_off_account_policy_set_schedule = rail.RepliconServiceOperator(
            ...
            data = lambda dag_run: {
                    ...
                    "policySetScheduleEntries": loads(dumps(get_policy_to_consider(dag_run)
        ...
        get_default_timeoff_policy_set_schedule_for_timeoff_type >> get_policy_set_to_assign_update >> is_caller_add
        is_caller_add >> rail.Label('Yes') >> get_policy_to_assign_add >> has_policy_to_assign
        is_caller_add >> rail.Label('No') >>  get_policy_to_assign_update >> has_policy_to_assign

        has_policy_to_assign >> rail.Label("Yes") >> put_user_time_off_account_policy_set_schedule
        has_policy_to_assign >> rail.Label("No") >> catch_and_log_error

        put_user_time_off_account_policy_set_schedule >> catch_and_log_error

    return dag

rail.for_each_instance(create_us_puerto_rico_timeoff_assignment_dag)
