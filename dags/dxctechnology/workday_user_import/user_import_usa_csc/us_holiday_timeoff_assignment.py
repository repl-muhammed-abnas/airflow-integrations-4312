...
def create_us_holiday_timeoff_assignment_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.usa_csc_us_holiday_user_timeoff_assignment_dag_id,
        description="dxctechnology workday user sync process users child",
        replicon_conn_id=config.replicon_conn_id,
        company_key=config.company_key,
        start_date=datetime(2023, 9, 26),
        max_active_runs=config.max_run_holiday_to_assignment
    ) as dag:
        ...
        has_any_policy_set_to_assign = rail.IfOperator(
            ...
            test = lambda: bool(rail.result(
                "get_default_timeoff_type_policy_schedule_for_user") and rail.result("get_default_timeoff_type_policy_schedule_for_user")[0]['policySet']),
        ...
        def get_policy_to_assign_update_fte_not_one_callable(dag_run):
            ...
            for _policy in global_timeoff_policies:
                ...
                if float(_policy['startOffset']['offsetValue']) == tenure:
                    ...
                    yearly_accrual_amount = float(yearly_accrual_amount)
                    accrual_amt_based_on_schedule = yearly_accrual_amount * fte
                    value_to_be_gsubbed_from_policy =f"""{OPEN_BRACKETS}"keyUri":"urn:replicon:script-key:parameter:accrual-annual-amount","value":{OPEN_BRACKETS}"number": {weekly_accrual_params}{OPEN_BRACKETS}{OPEN_BRACKETS}"""
                    actual_value_to_be_added = f"""{OPEN_BRACKETS}"keyUri":"urn:replicon:script-key:parameter:accrual-annual-amount","value":{OPEN_BRACKETS}"number": {accrual_amt_based_on_schedule}{CLOSE_BRACKETS}{CLOSE_BRACKETS}"""
                    policy_sets.append(
                        {
                            "offset": _policy['startOffset']['offsetValue'],
        ...
        is_caller_not_add >> rail.Label("No") >> get_default_timeoff_policy_set_schedule_for_timeoff_type >> is_fte_one >> rail.Label("No") >>empty_fte_not_one
        ...
        has_any_policy_set_to_assign >> rail.Label("No") >> stop
        is_caller_add >> rail.Label("No") >> get_policy_to_assign_update >> has_policy_to_assign >> rail.Label("No") >> stop
...
rail.for_each_instance(create_us_holiday_timeoff_assignment_dag)
