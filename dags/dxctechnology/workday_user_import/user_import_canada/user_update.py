...
from dxctechnology.workday_user_import.user_import_canada.utils import custom_methods
from dxctechnology.workday_user_import.user_import_canada.utils.request_payload import prepare_update_user_payload_callable
...
from dxctechnology.workday_user_import.user_import_canada.task.supervisor_assignment_task import assign_supervisor
...
from dxctechnology.workday_user_import.user_import.tasks.update_timesheet_template import update_timesheet_template_task
...
def create_update_user_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_canada_users_update_user_child_dag,
        ...
        trigger_process_timeoff_policies = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.workday_user_import_canada_users_update_user_timeoff_process_child_dag_disable,
            ...
            conf=lambda item, dag_run: {
                ...
                "timeoff_name": item['timeOffType']["name"], 
        ...
        is_user_on_leave = rail.IfOperator(
            ...
            no_task="rehired_user"
        )

        rehired_user = rail.PythonOperator(
            task_id = "rehired_user",
            ...
            if dag_run.conf['mapper_data']['holiday_calendar']:
                if dag_run.conf['mapper_data']['holiday_calendar'] != holiday_calendar:
                    if dag_run.conf['mapper_data']['holiday_calendar_uri']:
                        return True
                    rail.set_result(key='exception_msg', val=f''''Holiday calendar "{dag_run.conf['mapper_data']['holiday_calendar']}" not available in Replicon''')
            return False
        ...
        update_user_holiday_calendar = rail.RepliconServiceOperator(
            ...
            data={
                    "userUri": "{{ dag_run.conf.user_uri}}",
                    "holidayCalendarUri": "{{ dag_run.conf.mapper_data.holiday_calendar_uri.uri}}"
                }
        )
        ...
        trigger_timeoff_update_for_user = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.workday_user_import_canada_users_update_user_timeoff_process_child_dag,
            conf=lambda dag_run: {
                "file_name": dag_run.conf["master_file_name"],
                "user_uri": dag_run.conf["user_uri"],
                ...
                "rehire": rail.result("rehired_user"),
                ...
                "prevent_balance_overdraw_uri": dag_run.conf["prevent_balance_overdraw_uri"],
                "parent_location" : rail.result("get_effective_group_membership")['parent_location'],
                "payrule": dag_run.conf['payrule']['payrule'],
                "payrule_updated": rail.result("update_user", 'payrule_updated') if rail.result("update_user", 'payrule_updated') else "No",
                "personnelsubarea": dag_run.conf["file_data"]['sub_area_code'],
                "employeegroup": dag_run.conf["file_data"]["emp_group_code"],
                "employeesubgroup": dag_run.conf["file_data"]["emp_subgroup_code"],
                "payrule_effective_date": dag_run.conf['json_formatted_dates']['payrule_effective_date'],
        ...
        def get_user_update_log_message(dag_run):
            exception_message = rail.result("prepare_update_user_payload", "exception_log") + rail.result('log_exception') + [rail.result('log_timesheet_template_exception')]
            return{
                "Jobid": "",
                "Userid": dag_run.conf['file_data']["emp_id"],
                "Email": dag_run.conf['file_data']["email_id"],
        ...
        get_user_details >> get_effective_group_membership >> get_user_timeoff_policy_summary >> get_assigned_permission_for_user
        ...
        is_user_on_leave >> rail.Label("No") >> rehired_user >> enable_login >> update_user_start_date >> is_user_for_long_leave_disable
        ...
        is_end_date_less_than_today >> rail.Label("No") >> process_user_update >> get_time_entry_approval_path >> prepare_update_user_payload >> can_process_update_user
        can_process_update_user >> rail.Label("No") >> log_ia_exception >> rail.Label("No") >> catch_and_log_error
        ...
        can_update_notification_preference >> rail.Label("No") >> is_profile_enabled >> rail.Label("No") >> trigger_timeoff_update_for_user
        is_profile_enabled >> rail.Label("Yes") >> can_update_holiday_calendar >> rail.Label("No") >> can_update_timeoff_template >> rail.Label(
            "No") >> log_exception >> trigger_timeoff_update_for_user
        can_update_holiday_calendar >> rail.Label("Yes") >> update_user_holiday_calendar >> can_update_timeoff_template
        can_update_timeoff_template >> rail.Label("Yes") >> update_timeoff_template >> log_exception

        trigger_timeoff_update_for_user >> wait_for_user_timeoff_update_completion >> supervisor_start 
        supervisor_end >> log_user_success >> rail.Label("No") >> catch_and_log_error
