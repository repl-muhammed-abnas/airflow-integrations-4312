...
from dxctechnology.workday_user_import_v1.user_import_hungary_v1.utils import custom_methods, request_payload
from dxctechnology.workday_user_import_v1.user_import_hungary_v1.tasks.supervisor_assignment import assign_supervisor
...
def create_add_user_dag(config):
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.workday_user_import_hungary_add_user_dag}{prefix}",
            description=config.workday_user_import_hungary_add_user_dag_description,
            company_key=config.company_key,
            replicon_conn_id=config.replicon_conn_id,
            max_active_runs=config.max_active_run_add_user_hungary,
        ) as dag:
            ...
            # Remove default timeoffs to assign Hungary-specific ones
            ...
            get_mapper_timeoff_data = rail.PythonOperator(
                ...
                python_callable=lambda:custom_methods.get_mapper_timeoff_data(config.TIMEOFF_MAPPER)
            )
            ...
            process_new_user_timeoff_assignment = rail.TriggerDagRunForEachItemOperator(
                ...
                trigger_dag_id= lambda dag_run, item : custom_methods.get_trigger_dag_id(
                    trigger_dag_id=config.workday_user_import_hungary_add_user_timeoff_assignment_dag, 
