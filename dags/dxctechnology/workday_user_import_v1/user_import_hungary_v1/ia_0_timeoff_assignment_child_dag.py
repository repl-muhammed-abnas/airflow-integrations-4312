...
from dxctechnology.workday_user_import_v1.user_import_hungary_v1.utils.custom_methods import convert_json_date_to_date, get_tenure_value, get_marital_status_effective_date_if_applicable  
...
def create_user_timeoff_assignment_dag(config):
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            ...
            max_active_runs=config.max_active_run_process_ia_0_timeoff_assignment
        ) as dag:
            ...
            def get_payload_to_process(dag_run):
                ...
                ia_end_date = convert_json_date_to_date(dag_run.conf['json_formatted_dates']['ia_end_date'])
                tenure = get_tenure_value(start_date, ia_end_date + timedelta(days=1))
                policy_sets = []
                policy_to_assign = []
