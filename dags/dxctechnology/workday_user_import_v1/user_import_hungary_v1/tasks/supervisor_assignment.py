...
from dxctechnology.workday_user_import_v1.user_import_hungary_v1.utils.custom_methods import is_profile_enabled
from dxctechnology.workday_user_import_v1.user_import_hungary_v1.utils.custom_methods import get_user_uri
    ...
    with rail.TaskGroup(group_id = group_id, prefix_group_id=False) as supervisor_group:
        ...
        def get_supervisor_details_data_handler(response, dag_run):
            if not response['rows']:
                return []
            return_response = list(filter(lambda emp_record: emp_record['emp_id'] == dag_run.conf['file_data']['supervisor_id'] and 
                    emp_record['login_name'] == dag_run.conf['file_data']['supervisor_email_id']
                ,map(lambda record: {
