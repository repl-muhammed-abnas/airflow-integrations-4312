...
from dxctechnology.workday_user_import_v1.user_import_hungary_v1.utils.custom_methods import get_trigger_dag_id, get_item_index
null = None
...
def create_dag(config):
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.workday_user_import_hungary_process_users_child_dag}{prefix}",
            description=config.workday_user_import_hungary_process_users_child_dag_description,
            ...
            log_no_mapper_values_found_exception = rail.WriteLogOperator(
                ...
                properties=lambda dag_run: {
                    ...
                    "Country": "Hungary",
                    "CompanyCode": dag_run.conf['file_data']['company_code']
                }
            )
            ...
            is_user_found = rail.IfOperator(
                ...
                yes_task="trigger_update_hungary_user",
                no_task="trigger_add_hungary_user"
            )
            ...
            trigger_update_hungary_user = rail.TriggerDagRunForEachItemOperator(
                task_id="trigger_update_hungary_user",
                items= [1],
                trigger_dag_id=lambda dag_run: get_trigger_dag_id(
                    config.workday_user_import_hungary_update_user_dag,
            ...
            wait_for_update_user_completion = rail.WaitForDagRunsSensor(
                ...
                dag_runs="{{result('trigger_update_hungary_user')}}",
                retries=0,
            ...
            trigger_add_hungary_user = rail.TriggerDagRunForEachItemOperator(
                task_id="trigger_add_hungary_user",
                items=[1],
                trigger_dag_id=lambda dag_run: get_trigger_dag_id(
                    config.workday_user_import_hungary_add_user_dag,
                    config.DAG_BATCH_COUNT,
                    item_index=get_item_index(dag_run, config.DAG_BATCH_COUNT)
                ),
            ...
            wait_for_add_user_completion = rail.WaitForDagRunsSensor(
                ...
                dag_runs="{{result('trigger_add_hungary_user')}}",
                retries=0,
                execution_timeout=timedelta(days=1)
            )
            ...
            is_user_found >> rail.Label("Yes") >> trigger_update_hungary_user >> wait_for_update_user_completion >> gather_logs
            is_user_found >> rail.Label("No") >> trigger_add_hungary_user >> wait_for_add_user_completion >> gather_logs
