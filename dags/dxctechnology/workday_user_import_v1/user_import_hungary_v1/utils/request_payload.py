...
from dxctechnology.workday_user_import_v1.user_import.common_utils.region_fields_config import DXC_HUNGARY
...
def get_psa_user_udf_add_update_payload(dag_run, current_udf_value, caller, current_user_groups, effective_date):
    ...
    if dag_run.conf['groups']['cost_center'].get('uri'):
        if dag_run.conf['groups']['cost_center'].get('parent'):
...
def _get_holiday_calendar_to_assign(dag_run, exception_log):
    if dag_run.conf['holiday_calendar'].get('holiday_calendar')=="NA":
        return null
        ...
        policy_sets.append(
            {
                "policySet": {
                    "uri": null,
                    "name": dag_run.conf['user_policies']['timeoff_template']['timeoff_template']
                }
            }
        )
...
def _get_employee_type_uri_to_assign(dag_run):
    if dag_run.conf['groups']['employee_type'].get('employee_type_name'):
        return [
            {
                "employeeTypeGroup": {
                    "uri": dag_run.conf['groups']['employee_type']['employee_type_uri'],
                    "parent": null,
                    "name": null,
                    "parameterCorrelationId": null
                },
...
def _get_timesheet_period_schedule_to_apply_add_user(dag_run):
    timesheet_period_name = dag_run.conf['user_policies']['timesheet_period']['timesheet_period']
        ...
        return [
            {
                ...
                "effectiveDate": _get_timesheet_template_period_effective_date(
                    ...
                    work_shift_eff_date=convert_json_date_to_date(dag_run.conf['json_formatted_dates']['work_shift_effective_date']),
                    caller = "add",
...
def _get_timesheet_template_period_effective_date(hire_date:date, timesheet_period_eff_date:date, work_shift_eff_date: date, caller, timesheet_not_assigned_to_user:bool, true_caller:str="add"):
    if caller == "add":
        ...
        if timesheet_not_assigned_to_user:
            ...
            return {
                "day": work_shift_eff_date.day,
                "month": work_shift_eff_date.month,
                "year": work_shift_eff_date.year
            }
    else:
        raise
...
def _get_update_timesheet_template_update_payload(dag_run, caller):
    ...
    work_shift_eff_date = convert_json_date_to_date(json_formatted_dates.get('work_shift_effective_date'))  # mapping to be checked
    if caller == "update":
        effective_date = work_shift_eff_date
...
def _update_assignment_type_txt_udf(dag_run, input_field_name, udf_display_text_value, udf_key_name, custom_fields_payload, current_custom_fields_values):
    file_data = dag_run.conf.get('file_data', {})
    udfs = dag_run.conf.get('udfs', {})
    input_data = file_data.get(input_field_name)
    current_value = rail.find_first_by_attr_and_get_attr(
...
def get_ia_update_payload_for_udf_update(dag_run, custom_fields_payload, current_custom_fields_values, update_txt_udf:callable, update_date_udf: callable):
    ...
    if (is_ia != rail.find_first_by_attr_and_get_attr(
        ...
        if not ia_start_date and _is_international_assignee(is_ia):
        ...
        if _is_international_assignee(is_ia) and (convert_json_date_to_date(ia_start_date) < today_minus_five_days):
        ...
        if is_ia in [0,'0']:
            ...
            effective_date = get_json_date_from_date(convert_json_date_to_date(ia_end_date) + timedelta(days=1))
...
def _update_custom_fields_for_user(dag_run, current_user_groups):
    ...
    _update_assignment_type_txt_udf(dag_run, 'assignment_type', 'assignment_type', 'assignment_type', custom_fields_payload, current_custom_fields_values)
    ...
    # If user is an International Assignee, clear UDFs that are not applicable to Hungary region
...
def _get_employee_type_update_payload(dag_run, current_effective_grps, effective_date):
    if dag_run.conf['groups']['employee_type'] and dag_run.conf['groups']['employee_type']['employee_type_uri'] and (dag_run.conf['groups']['employee_type']['employee_type_uri'] != current_effective_grps['employeeType'].get('uri', '')):
        return {
...
def _get_timesheet_period_schedule_to_apply(dag_run, user_details, effective_date, exception_log, ia_updated=False):
    ...
    # NOTE: No ia_updated check - applies to ALL IA=1 users
...
def _get_timesheetperiod_update_payload(dag_run, user_details, effective_date, exception_log, ia_updated=False):
    if not user_details['timesheetPeriodSchedule'] and dag_run.conf['user_security_config']['profile_status'].lower() == "enabled":
        ...
        if data:
           return data
    return _get_timesheet_period_schedule_to_apply(dag_run, user_details, effective_date, exception_log, ia_updated)
...
def _can_update_punch_entry_policies(dag_run):
    ...
    return not bool(list(filter(lambda time_punch_policy: time_punch_policy['policySet']['name'] == dag_run.conf['user_policies']['punch_entry_policy'].get('name', ''), 
...
def _get_payrule_schedule_to_update(dag_run, _user_details, effective_date, exception_log, ia_updated=False):
    is_ia = dag_run.conf['file_data'].get('is_ia')
...
def _get_shift_assignment_to_update(dag_run, user_details, config, exception_log, effective_date):
    ...
    if dag_run.conf['schedule']['schedule_name'] == "Shift Schedule":
        ...
        if dag_run.conf['schedule']['schedule_name']:
            ...
            if _is_international_assignee(dag_run.conf['file_data']['is_ia']):
                exception_log.append(f"""Office schedule {dag_run.conf['schedule']['schedule_name']} not available for IA=1 in Replicon. Assigning none schedule""")
                return {
                    "userSchedulePolicyScheduleModificationOptionUri": "urn:replicon:schedule-modification-option:update-schedule-over-date-range",
                    "replacementSchedule": [],
                    "updateScheduleOverDateRange": {
...
def get_update_user_payload(dag_run, config):
    ...
    ia_updated = rail.result("ia_updated")
    payload = {
        "user": {
            "uri": dag_run.conf['user_uri'],
            "loginName": null,
        ...
        "modifications": {
            ...
            "timesheetPeriodScheduleToApply": _get_timesheetperiod_update_payload(dag_run, user_details, effective_date, exceptions, ia_updated), # workweek
            ...
            "payRulesScheduleModifications": _get_payrule_schedule_to_update(dag_run, user_details, effective_date, exceptions, ia_updated), #workweek
    ...
    # If user is an International Assignee, clear OEFs that are not applicable to Hungary region
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
