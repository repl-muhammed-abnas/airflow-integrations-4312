...
from dxctechnology.workday_user_import_v1.user_import_canada.utils.custom_methods import is_profile_enabled, get_replicon_date
...
# UDF fields that are NOT applicable to Canada region (same as Global)
CANADA_EXCLUDED_UDFS = (
    "Work Shift",
    "Date of Birth",
    "Time Type",
    "Middle Name",
...
def _get_udf_fields_to_clear_for_ia(dag_run, current_custom_fields_values, exception_log):
    ...
    for udf_display_text in CANADA_EXCLUDED_UDFS:
        for cf in current_custom_fields_values:
            if cf.get('customField', {}).get('displayText') == udf_display_text:
                ...
                if has_value:
                    exception_log.append(f"Clearing UDF '{udf_display_text}' for IA=1 user (not applicable to Canada region)")
...
def get_user_custom_field_update_payload(dag_run, user_details, exception_log):
    ...
    # For IA=1 users, clear UDF fields that are not applicable to Canada region
    ...
    return custom_fields_payload, can_update_notifications, effective_date
...
def prepare_update_user_payload_callable(dag_run, config):
    ...
    custom_field_update_payload, can_update_notification_pref, effective_date = get_user_custom_field_update_payload(dag_run, user_details, logger)
