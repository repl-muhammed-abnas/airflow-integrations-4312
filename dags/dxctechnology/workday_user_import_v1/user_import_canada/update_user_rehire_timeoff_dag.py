...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_canada_users_update_user_rehire_timeoff_process_child_dag,
        ...
        def get_policy_to_assign_callable(dag_run):
            ...
            if not first_policy_found:
                ...
                max_daydiff = max([offset['day_diff'] for offset in off_set_list])
                max_offset_data = rail.find_first_by_attr_and_get_attr(off_set_list, 'day_diff', max_daydiff, default={})
                policy_sets.append(
                    {
                        "offset": max_offset_data['offset'],
                        "policy": max_offset_data['policy'],
                        "first": "Yes"
                    }
                )
