...
# UDF fields that are NOT applicable to Philippines region (same as Hungary)
# Philippines uses Work Shift, Date of Birth, Time Type, Middle Name, and PERNER
PHILIPPINES_EXCLUDED_UDFS = (
    "Personnel Area Code",
...
def _get_udf_fields_to_clear_for_ia(dag_run, current_custom_fields_values, exception_log):
    ...
    for udf_display_text in PHILIPPINES_EXCLUDED_UDFS:
        for cf in current_custom_fields_values:
            if cf.get('customField', {}).get('displayText') == udf_display_text:
                ...
                if has_value:
                    exception_log.append(f"Clearing UDF '{udf_display_text}' for IA=1 user (not applicable to Philippines region)")
...
def _get_update_timesheet_template_update_payload(dag_run, caller):
    ...
    timesheet_not_assigned_to_user = bool(rail.result('get_user_details')['timesheetTemplate'])
    if caller == "add":
...
def _update_custom_fields_for_user(dag_run, current_user_groups, exception_log):
    ...
    # For IA=1 users, clear UDF fields that are not applicable to Philippines region
