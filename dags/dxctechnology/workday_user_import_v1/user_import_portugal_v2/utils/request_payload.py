...
def _get_shift_assignment_to_update(dag_run, user_details, config, exception_log:list, effective_date):
...
def _get_policies_to_update_payload(dag_run, user_details, exception_log):
    ...
    if _can_update_timeoff_template(dag_run, user_details):
        if dag_run.conf['policy_sets']['timeoff_template'].get('uri'):
            ...
            if not dag_run.conf['policy_sets']['timeoff_template']:
...
def _get_holiday_calendar_update_payload(dag_run, user_details, profile_status_is_enabled, exception_log):
    if profile_status_is_enabled:
        ...
        if not dag_run.conf['mapper_data']['holiday_calendar'] and _is_international_assignee(dag_run.conf['file_data']['is_ia']):
            return {
                "holidayCalendar": {
                    "uri": null,
                    "name": NONE_DEFAULT_VALUE
