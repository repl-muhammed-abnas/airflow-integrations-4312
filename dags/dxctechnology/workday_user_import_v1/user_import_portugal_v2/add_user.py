...
from dxctechnology.workday_user_import_v1.user_import_portugal_v2.utils import request_payload, custom_methods
from dxctechnology.workday_user_import_v1.user_import_portugal_v2.tasks.supervisor_assignment import assign_supervisor

def create_add_user_dag(config):
    
    with rail.create_airflow_dag(
        dag_id = config.portugal_add_user_dag_id,
        ...
        def query_timeoff_data_from_mapper(dag_run):
            work_shift = dag_run.conf['file_data']['work_shift']
            _work_shift = "BPSOT" if work_shift.startswith('BPSOT') else ("BPS" if work_shift.startswith('BPS') else work_shift)
            return list(filter(lambda row: row['Type']=='Timeoff' and\
                                                                    ...
                                                                    row['personnelsubarea'] == _work_shift and\
                                                                    row['employeegroup'] == dag_run.conf['file_data']['gender'], config.MAPPER))

        query_timeoff_data = rail.PythonOperator(
            task_id = "query_timeoff_data",
            python_callable=query_timeoff_data_from_mapper
        )
        ...
        process_new_user_timeoff_assignment = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.portugal_add_user_timeoff_assignment_dag_id,
            conf=lambda dag_run:{
                ...
                "workshift": "BPSOT" if dag_run.conf['file_data']['work_shift'].startswith("BPSOT") else("BPS" if dag_run.conf['file_data']['work_shift'].startswith("BPS") else dag_run.conf['file_data']['work_shift']),
