...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_process_portugal_users_child_dag,
        ...
        trigger_update_user = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.portugal_update_user_dag_id,
            conf=lambda dag_run: {
                **{
        ...
        trigger_add_user = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.portugal_add_user_dag_id,
            conf=lambda dag_run: {
                **{
                    "user_log": rail.result('create_user_log')
                },
