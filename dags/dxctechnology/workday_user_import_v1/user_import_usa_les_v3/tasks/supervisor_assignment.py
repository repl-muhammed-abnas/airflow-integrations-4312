...
def assign_supervisor(group_id, caller):
    with rail.TaskGroup(group_id=group_id, prefix_group_id=False):
        ...
        is_supervisor_found = rail.IfOperator(
            ...
            yes_task="is_supervisor_disabled_empty_task"
        )
        ...
        log_supervisor_for_reassignment_check = rail.WriteLogOperator(
            ...
            properties = lambda dag_run: {
                ...
                "supervisor_end_user_permission": {
                    "name": dag_run.conf['mapper_data']['supervisor_end_user_permission_name'],
                    "uri": dag_run.conf['mapper_data']['supervisor_end_user_permission_uri']
                } if dag_run.conf['mapper_data']['supervisor_end_user_permission_uri'] else {},
                "supervisor_user_permission":{
                    "name": dag_run.conf['mapper_data']['supervisor_user_permission_name'],
                    "uri": dag_run.conf['mapper_data']['supervisor_user_permission_uri']
                } if dag_run.conf['mapper_data']['supervisor_user_permission_uri'] else {},
                'aus_supervisor_end_user_permission': {},'aus_supervisor_end_user_permission': {},
                'parent_company': dag_run.conf['file_data']['parent_company']
            }
        )

        is_supervisor_disabled_empty_task = rail.EmptyOperator(
            task_id = "is_supervisor_disabled_empty_task"
        )

        has_division = rail.IfOperator(
            task_id = "has_division",
            test = lambda : bool(rail.result("get_supervisor_details")['division']),
            yes_task = "is_supervisor_disabled",
            no_task = "dummy_has_division_no_task"
        )

        dummy_has_division_no_task = rail.EmptyOperator(
            task_id= "dummy_has_division_no_task"
        )
        ...
        def get_supervisor_permission_details_data_handler(response, dag_run):
            ...
            return {
                "end_user_permission": rail.find_first_by_attr_and_get_attr(response,
                                            'permissionSet.name', dag_run.conf['mapper_data']['supervisor_end_user_permission_name']),
                "supervisor_permission": rail.find_first_by_attr_and_get_attr(response,
                                            'permissionSet.name', dag_run.conf['mapper_data']['supervisor_user_permission_name'])
            }
        ...
        assign_supervisor_permission = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "userUri": rail.result('get_supervisor_details')['user_uri'],
                "permissionSetUri": dag_run.conf['user_permission_sets']['supervisor_user_permission']
            }
        )
        ...
        assign_end_user_permission = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "userUri": rail.result('get_supervisor_details')['user_uri'],
                "permissionSetUri": dag_run.conf['user_permission_sets']['supervisor_end_user_permission']
            }
        )
        ...
        is_supervisor_found >> rail.Label("Yes") >> is_supervisor_disabled_empty_task >> has_division >> rail.Label("Yes") >> is_supervisor_disabled
        has_division >> rail.Label("No") >> dummy_has_division_no_task >> log_supervisor_for_reassignment_check
        ...
        is_end_user_permission_missing >> rail.Label("No") >> assign_supervisor_to_user
        
        assign_supervisor_to_user >> finish
