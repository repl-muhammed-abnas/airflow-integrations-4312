...
from dxctechnology.workday_user_import_v1.user_import.common_utils.region_fields_config import DXC_USA_LES
from dxctechnology.workday_user_import_v1.user_import_usa_les_v3.utils.request_payload import _add_custom_field
...
def _update_custom_fields_for_user(dag_run):
    ...
    # If user is an International Assignee, clear UDFs that are not applicable to USA LES region
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
...
def _get_timesheet_period_schedule_to_apply(dag_run, user_details, effective_date):
    if user_details['timesheetPeriodSchedule']:
        ...
        elif _is_international_assignee(dag_run.conf['file_data'].get('is_ia')) and not dag_run.conf['mapper_data']['timesheet_period']:
...
def _get_payrule_schedule_to_update(dag_run, _get_user_details, effective_date, exception_log):
        ...
        if not current_payrule_schedule or current_payrule_schedule['payRuleScript']['displayText'] != payrule_name:
            if dag_run.conf['file_data']['management_lvl'] not in ['L1', 'L2']:
                ...
                if _is_international_assignee(is_ia) and payrule_name == NONE_DEFAULT_VALUE:
                    ...
                    payrule_eff_date = payrule_effective_date
                return {
                    "scheduleEntries": [
                        {
                            "payRuleScript": {
...
def get_update_user_payload(dag_run, config):
    ...
    payload = {
        ...
        "modifications": {
            ...
            "payRulesScheduleModifications": _get_payrule_schedule_to_update(dag_run, user_details, effective_date, exceptions),
    ...
    # If user is an International Assignee, clear OEFs that are not applicable to USA LES region
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
