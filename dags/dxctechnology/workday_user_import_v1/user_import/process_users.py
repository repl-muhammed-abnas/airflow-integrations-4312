...
def create_dag(config):
    dag_ids =[]
    for erp in config.DXC_ERPS:
        with rail.create_airflow_dag(
            dag_id=config.workday_user_import_process_users_child_dag_dag_ids_per_erp[erp],
            ...
            can_process_users = rail.IfOperator(
                task_id = "can_process_users",
                test=lambda dag_run: dag_run.conf.get("process_record", "Yes") == "Yes",
                yes_task =  "get_user_details_via_emp_id",
                no_task = "processing_stopped_no_valid_combination" 
            )

            processing_stopped_no_valid_combination = rail.EmptyOperator(
                task_id = "processing_stopped_no_valid_combination"
            )
            ...
            is_user_found = rail.IfOperator(
                ...
                yes_task="trigger_update_gbl_user",
                no_task="trigger_add_gbl_user"
            )
            
            trigger_update_gbl_user = rail.TriggerDagRunOperator(
                task_id = "trigger_update_gbl_user",
                trigger_dag_id=config.ADD_UPDATE_CHILD_DAGS_PER_ERP[erp]['update'],
                conf=lambda dag_run: {
                    **{
            ...
            wait_for_update_user_completion = rail.WaitForDagRunsSensor(
                ...
                dag_runs="{{result('trigger_update_gbl_user')}}",
                retries = 0,
                execution_timeout = timedelta(days=1)
            )

            trigger_add_gbl_user = rail.TriggerDagRunOperator(
                task_id = "trigger_add_gbl_user",
                trigger_dag_id=config.ADD_UPDATE_CHILD_DAGS_PER_ERP[erp]['add'],
                conf=lambda dag_run: {
                    **{
                        "user_log": rail.result('create_user_log')
                    },
            ...
            wait_for_add_user_completion = rail.WaitForDagRunsSensor(
                ...
                dag_runs="{{result('trigger_add_gbl_user')}}",
                retries = 0,
                execution_timeout = timedelta(days=1)
            )
            ...
            create_user_log >> can_process_users >> rail.Label("Yes") >> get_user_details_via_emp_id >> is_user_found
            can_process_users >> rail.Label("No") >> processing_stopped_no_valid_combination >> catch_and_log_error
            is_user_found >> rail.Label("Yes") >> trigger_update_gbl_user >> wait_for_update_user_completion >> catch_and_log_error
            is_user_found >> rail.Label("No") >> trigger_add_gbl_user >> wait_for_add_user_completion >> catch_and_log_error
