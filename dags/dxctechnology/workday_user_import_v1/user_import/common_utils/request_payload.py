...
def get_canada_user_process_conf_v2(item, dag_run, config):
    # Used by process_canada_users_v2.py: employee group specific mapper values win over the generic Canada ones
    conf = get_canada_user_process_conf(item, dag_run, config)
    policy_data = rail.result('get_all_policy_sets')

    specific_timesheet_period = item['mapper_canada_timesheet_period_specific']
    if specific_timesheet_period:
        conf['mapper_data']['timesheet_period'] = specific_timesheet_period
        conf['mapper_data']['timesheet_period_canada'] = specific_timesheet_period
        conf['policy_sets']['timesheet_period'] = rail.find_first_by_attr_and_get_attr(
            policy_data, "name", specific_timesheet_period)

    specific_timesheet_approval = item['mapper_canada_timesheet_approval_specific']
    if specific_timesheet_approval:
        conf['mapper_data']['timesheet_approval_path'] = specific_timesheet_approval
        conf['mapper_data']['timesheet_approval_path_canada'] = specific_timesheet_approval
        conf['policy_sets']['timesheet_approval_path'] = rail.find_first_by_attr_and_get_attr(
            policy_data, "name", specific_timesheet_approval)

    specific_time_entry_approval = item['mapper_canada_time_entry_approval_path_specific']
    if specific_time_entry_approval:
        conf['mapper_data']['time_entry_approval_path_name'] = specific_time_entry_approval
...
def get_psa_udf_value(dag_run, current_custom_fields_values, current_user_groups, custom_fields_payload:list, _get_custom_fields_payload:Callable, _get_cost_center_update_payload:Callable, _get_department_update_payload:Callable, caller):
    psa_flag = get_psa_user_udf_add_update_payload(
        dag_run = dag_run, 
        current_udf_value = rail.find_first_by_attr_and_get_attr(
            current_custom_fields_values, "customField.displayText", "PSA User", 'text', default=""), 
        caller = caller,
