...
from dxctechnology.workday_user_import_v1.user_import.common_utils.request_payload import get_global_user_process_conf
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_process_gbl_data_child_dag,
        ...
        get_global_data = rail.QueryCollectionOperator(
            task_id = "get_global_data",
            ...
            query=f"""SELECT
                    ...
                    sgugd.*,
                        ...
                        (SELECT m.Value FROM mapper m WHERE
                            ...
                            m."Source" ="C1" LIMIT 1) as mapper_timesheet_period_canada,
                        (SELECT m.Value FROM mapper m WHERE
                            m."Type" == 'TimeZone' AND
                            m.Country == sgugd._country_to_use_for_query AND
                            m."Source" ="C1" AND
                            m.uri == sgugd.companycode AND
                            m.personnelsubarea = sgugd.areacode AND
                        ...
                        (SELECT m."Value" FROM mapper m WHERE
                            ...
                            m.employeegroup  == sgugd.empgroupcode AND
                            m.employeesubgroup  == sgugd.empgroupcode AND
                        ...
                        (SELECT m."Source" FROM mapper m 
                            WHERE m."Type" == "Holiday Calendar" 
                                ...
                                AND m.personnelsubarea == sgugd."_state_to_use_for_query" LIMIT 1) as mapper_prt_not_compass_holidaycalander_canada_holidaycalendar,
		            ...
		            FROM splitter_gbl_union_global_data_new sgugd"""
        )

        start_task, end_task = get_all_required_fields("gbl_get_all_records", config)

        trigger_process_user = rail.trigger_parallel_dagrun(
            task_id = "trigger_process_user",
            items="{{result('get_global_data')}}",
            trigger_dag_id=config.workday_user_import_process_users_child_dag_dag_ids_per_erp['global'],
            parallel_count=config.process_users_parallel_count,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf = lambda item, dag_run : get_global_user_process_conf(item, dag_run, config)
        )
