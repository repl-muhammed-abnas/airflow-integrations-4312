...
from dxctechnology.workday_user_import_v1.user_import.common_utils import response_filter
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        send_blank_file_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | Replicon user import for workday  - No records to process - {{ current_time_in_specified_tz() }}',
            html_content="templates/emails/blank_file.html"
        )
        ...
        get_philipines_data = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT * FROM valid_records vr
                        WHERE vr._country_to_use_for_query = 'Philippines' and vr.companycode IN ("PHES", "PHET")
            """
        )
        ...
        get_hungary_data = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT * FROM valid_records vr
                        WHERE vr._country_to_use_for_query = 'Hungary' and vr.companycode IN ("HU00", "HU00")
            """
        )
        ...
        get_uki_csc_data = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT * FROM valid_records vr
                        WHERE vr._country_to_use_for_query IN ("United Kingdom", "Ireland") AND vr.companycode IN ("0201", "0290", "1627", "0250", "1629", "1639", "1631", "1630", "1628", "0237")
            """
        )
        ...
        get_uki_es_data = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT * FROM valid_records vr
                        WHERE vr._country_to_use_for_query IN ("United Kingdom", "Ireland") and vr.companycode IN ("GBC5", "GBA5", "IEEU", "IEES")
            """
        )
        ...
        get_other_than_philipines_hungary_uki_data = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT * FROM valid_records vr
                        WHERE NOT (vr._country_to_use_for_query = 'Philippines' and vr.companycode IN ("PHES", "PHET")) AND
                              NOT (vr._country_to_use_for_query = 'Hungary' and vr.companycode IN ("HU00", "HU00")) AND
                              NOT (vr._country_to_use_for_query IN ("United Kingdom", "Ireland") AND vr.companycode IN ("0201", "0290", "1627", "0250", "1629", "1639", "1631", "1630", "1628", "0237")) AND
                              NOT (vr._country_to_use_for_query IN ("United Kingdom", "Ireland") and vr.companycode IN ("GBC5", "GBA5", "IEEU", "IEES"))
            """
        )
        ...
        get_gbl_data_new = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT * FROM get_other_than_philipines_hungary_uki_data vr
                        WHERE 
                        (LOWER(vr._parent_company_code) == 'ftp')
                        OR
                        (LOWER(vr._country_to_use_for_query) NOT IN ('united states of america', 'puerto rico', 'india', 'portugal', 'costa rica', 'australia', 'canada'))
                        OR
                        (LOWER(vr._country_to_use_for_query) == 'australia' AND LOWER(vr._parent_company_code) == 'gsap' AND vr.companycode NOT IN {config.gsap_company_codes_lcsc})
                        OR
                        (LOWER(vr._country_to_use_for_query) == 'australia' AND LOWER(vr._parent_company_code) NOT IN ('gsap', 'compass'))
                        OR
                        (LOWER(vr._country_to_use_for_query) == 'united states of america' AND LOWER(vr._parent_company_code) NOT IN ('c1', 'compass'))
                        OR
                        (LOWER(vr._country_to_use_for_query) == 'puerto rico' AND LOWER(vr._parent_company_code) != 'c1')
                        OR
                        (LOWER(vr._country_to_use_for_query) == 'costa rica' AND LOWER(vr._parent_company_code) != 'compass')
                        OR
                        (LOWER(vr._country_to_use_for_query) == 'india' AND LOWER(vr._parent_company_code) != 'compass')
                        OR
                        (LOWER(vr._country_to_use_for_query) == 'portugal' AND LOWER(vr._parent_company_code) != 'compass')
                        OR
                        (LOWER(vr._country_to_use_for_query) == 'canada' AND LOWER(vr._parent_company_code) != 'c1')
                        """,
        ...
        combine_australia_data = rail.QueryCollectionOperator(
            ...
            query = f"""SELECT *, 'GSAP_AUS' as _origin
                    FROM get_other_than_philipines_hungary_uki_data vr
                    WHERE
        ...
        get_c1_canada_data = rail.QueryCollectionOperator(
            ...
            query = """SELECT * FROM get_other_than_philipines_hungary_uki_data vr WHERE 
        ...
        trigger_canada_processing_dag = rail.TriggerDagRunOperator(
            ...
            # Instances that opted into process_canada_users_v2.py are routed there
            trigger_dag_id=getattr(config, "workday_user_import_process_canada_data_child_dag_v2", None)
                            or config.workday_user_import_process_canada_data_child_dag,
            conf = lambda : {
                "file_name": path.split(rail.result("new_file_sensor"))[1],
                "starting_balance_set_to_uri": rail.result("get_starting_balance_script"),
                "prevent_balance_overdraw_uri": rail.result("get_prevent_balance_overdraw_script"),
        ...
        wait_trigger_pre_chec_dag >> get_philipines_data >> has_any_philipines_data

        has_any_philipines_data >> rail.Label("Yes") >> create_phl_data_csv_file >> upload_phl_data_to_sftp >> get_hungary_data
        has_any_philipines_data >> rail.Label("No") >> get_hungary_data >> has_any_hungary_data

        has_any_hungary_data >> rail.Label("Yes") >> create_hun_data_csv_file >> upload_hun_data_to_sftp >> get_uki_csc_data
