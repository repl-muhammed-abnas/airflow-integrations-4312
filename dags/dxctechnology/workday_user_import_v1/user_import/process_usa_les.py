...
from dxctechnology.workday_user_import_v1.user_import.common_utils.request_payload import get_usa_les_user_process_conf
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_process_usa_les_data_child_dag,
        description="dxctechnology workday user sync - USA LES",
        ...
        start_task, end_task = get_all_required_fields("usa_les_get_all_records", config)

        trigger_process_user = rail.trigger_parallel_dagrun(
            task_id = "trigger_process_user",
            items=lambda dag_run:dag_run.conf['compass_usa_les_data'],
            trigger_dag_id=config.usa_les_process_users_child_dag_id,
            parallel_count=config.process_users_parallel_count,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf = lambda item, dag_run : get_usa_les_user_process_conf(item, dag_run, config)
        )
