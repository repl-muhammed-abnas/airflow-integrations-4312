# Version 2 of process_canada_users.py: Timesheet Period and WorkWeek can also be mapped per personnel sub area / employee group / employee sub group / company code.
...
from dxctechnology.workday_user_import_v1.user_import.common_utils.request_payload import get_canada_user_process_conf_v2
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import get_all_run_ids_callable

def create_dag(config):

    # Only instances defining this config key get this version, the rest stay on process_canada_users.py
    if not getattr(config, "workday_user_import_process_canada_data_child_dag_v2", None):
        return None

    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_process_canada_data_child_dag_v2,
            ...
            query=f"""SELECT
                    ...
                    sgugd.*,
                        ...
                        (SELECT m.value FROM mapper m WHERE (
                                ...
                                m.employeesubgroup  == sgugd.empsubgroupcode AND
                                m.status == sgugd.companycode
                            ) LIMIT 1) as mapper_canada_timesheet_period_specific,
                        (SELECT m.value FROM mapper m WHERE (
                                m."Type" == "Timesheet Approval" AND
                                m.Country == sgugd._country_to_use_for_query AND
                                m."Source" == "C1" AND
                                ...
                                m.employeesubgroup  == sgugd.empsubgroupcode AND
                                m.status == sgugd.companycode
                            ) LIMIT 1) as mapper_canada_timesheet_approval_specific,
                        (SELECT m.value FROM mapper m WHERE (
                                m."Type" == "Time Entry Approval Path" AND
                                m.Country == sgugd._country_to_use_for_query AND
                                m."Source" == "C1" AND
                                ...
                                m.employeesubgroup  == sgugd.empsubgroupcode AND
                                m.status == sgugd.companycode
                            ) LIMIT 1) as mapper_canada_time_entry_approval_path_specific,
                        (SELECT m.value FROM mapper m WHERE (
                            ...
                            WHEN (SELECT m."Source" FROM mapper m WHERE m."Type" == "Company Code" AND m.uri == sgugd."companycode" LIMIT 1) IS NOT NULL
                                THEN 
                                    CASE 
                                        ...
                                        ELSE (SELECT m."value" FROM mapper m WHERE m."Type" == "WorkWeek" AND m."Source" == (
                        ...
                        CASE 
                            WHEN (SELECT m."Source" FROM mapper m WHERE m."Type" == "Company Code" AND m.uri == sgugd."companycode" LIMIT 1) IS NOT NULL
                                THEN 
                                    CASE 
                                        WHEN (SELECT m."uri" FROM mapper m WHERE (
                                                m."Type" == "WorkWeek" AND
                                                m.Country == sgugd._country_to_use_for_query AND
                                                m."Source" == "C1" AND
                                                m.personnelsubarea  == sgugd.subareacode AND
                                        ...
                                        ELSE (SELECT m."uri" FROM mapper m WHERE m."Type" == "WorkWeek" AND m."Source" == (
                                                SELECT m."Source" FROM mapper m WHERE m."Type" == "Company Code" AND m.uri == sgugd."companycode" LIMIT 1))
                                    END
                            ELSE 
                                NULL
        ...
        trigger_process_user = rail.trigger_parallel_dagrun(
            ...
            conf = lambda item, dag_run : get_canada_user_process_conf_v2(item, dag_run, config)
        )
