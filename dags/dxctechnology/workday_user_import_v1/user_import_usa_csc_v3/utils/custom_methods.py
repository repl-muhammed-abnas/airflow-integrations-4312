...
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import convert_json_date_to_date, compare_if_two_json_dates_are_same as _compare_if_two_json_dates_are_same, get_excluded_udf_clear_payloads, get_excluded_oef_clear_payloads
from dxctechnology.workday_user_import_v1.user_import.common_utils.request_payload import get_todays_date_in_json
from dxctechnology.workday_user_import_v1.user_import.common_utils.region_fields_config import DXC_USA_CSC
...
def map_mapper_replicon_timeoffs(val):
    replicon_timeoffs = rail.result("get_all_timeoffs")
    mapper_timeoffs = rail.result(val)
    ...
    filtered_timeoff_list = list(filter(lambda x: bool(x['uri']), mapped_timeoff_data))
    
    timeoff_unique_uri_list_to_assign = list(set(map(lambda record: record['uri'], filtered_timeoff_list)))

    rail.set_result(key = "mapped_timeoff_data", val = filtered_timeoff_list)
...
def _get_shift_assignment_to_update(dag_run, user_details, config, exception_log, effective_date, isia_update_done=False):
    ...
    if dag_run.conf['schedule_data']['work_schedule']:
        if (not current_office_schedule) or (not current_office_schedule['officeSchedule']) or (
                current_office_schedule['officeSchedule']['displayText'] != dag_run.conf['schedule_data']['work_schedule']):
            if dag_run.conf['schedule_data']['schedule_uri']:
                ...
                exception_log.append(f"""Office schedule {dag_run.conf['schedule_data']['office_schedule']} not available in Replicon""")
    else:
        # NONE fallback for IA users when schedule is not available from mapper
        if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
            if isia_update_done:
...
def _get_employee_type_update_payload(dag_run, current_effective_grps, effective_date):
    emp_type_uri = dag_run.conf['groups']['employee_type']['employee_type_uri_for_all']
...
def _update_txt_udf(dag_run, file_data_input_field_name, replicon_udf_display_text_value, conf_key_name_for_udf, custom_fields_payload, current_custom_fields_values):
    # this field is derived by mapper
    if file_data_input_field_name == 'termination_reason_code':
        input_data = dag_run.conf['mapper_data']['termination_reason_code']
    elif file_data_input_field_name=="job_level" and dag_run.conf['file_data']['country'] == "Canada":
        input_data = f"H{dag_run.conf['file_data']['job_level']}"
    elif isinstance(file_data_input_field_name, list):
        input_data = rail.smartjoin_by_delim([dag_run.conf['file_data'][field_name] for field_name in file_data_input_field_name], separator='|')
    else:
        input_data = dag_run.conf['file_data'][file_data_input_field_name]
    if input_data:
        if input_data != rail.find_first_by_attr_and_get_attr(
            current_custom_fields_values, "customField.displayText", replicon_udf_display_text_value, 'text', default=""):
            custom_fields_payload.append(
                _get_custom_fields_payload(uri=dag_run.conf['udfs'][conf_key_name_for_udf].get('uri'), txt_value=input_data))
            if file_data_input_field_name == "management_lvl":
...
def _update_custom_fields_for_user(dag_run):
    ...
    if dag_run.conf['file_data']['is_ia']:
        if dag_run.conf['file_data']['is_ia'] != rail.find_first_by_attr_and_get_attr(
            current_custom_fields_values, "customField.displayText", 'International Assignee', 'text', default=""):
    ...
    _update_txt_udf(dag_run, 'emp_group_code', 'EE Group', 'ee_group', custom_fields_payload, current_custom_fields_values)

    # If user is an International Assignee, clear UDFs that are not applicable to USA CSC region
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
...
def _get_timesheet_period_schedule_to_apply(dag_run, user_details, effective_date):
    if user_details['timesheetPeriodSchedule']:
        ...
        if dag_run.conf['mapper_data']['timesheet_period_c1'] and dag_run.conf['mapper_data']['timesheet_period_c1'] != current_timesheet_period['timesheetPeriod']['displayText']:
        ...
        elif _is_international_assignee(dag_run.conf['file_data'].get('is_ia')) and not dag_run.conf['mapper_data']['timesheet_period_c1']:
            if current_timesheet_period and current_timesheet_period['timesheetPeriod']['displayText'] != NONE_DEFAULT_VALUE:
                timesheet_effective_date = _get_effective_date_based_on_work_week(dag_run.conf['mapper_data']['work_week'], ['saturday', 'sunday'])
                return {
                    "userTimesheetPeriodScheduleModificationOptionUri": "urn:replicon:schedule-modification-option:update-schedule-over-date-range",
                    "replacementTimesheetPeriodSchedule": [],
...
def _can_update_timesheet_template(dag_run, user_details):
    if dag_run.conf['mapper_data']['c1_timesheet_template']:
        timesheet_template = user_details['timesheetTemplate']['name'] if user_details['timesheetTemplate'] else ''
        if (not timesheet_template) or (timesheet_template != dag_run.conf['mapper_data']['c1_timesheet_template']):
            return True
    return False
...
def _can_update_timeoff_template(dag_run, user_details):
    if dag_run.conf['mapper_data']['timeoff_template']:
        ...
        if not user_details['timeOffTemplate'] or timeoff_template != dag_run.conf['mapper_data']['timeoff_template']:
            return True
    return False
...
def _get_policies_to_update_payload(dag_run, user_details, exception_log):
    ...
    if _can_update_timesheet_template(dag_run, user_details):
        if dag_run.conf['file_data']['management_lvl'] not in ['L1', 'L2']: 
            if dag_run.conf['policy_sets']['c1_timesheet_template'].get('uri'):
                policies_to_add.append(dag_run.conf['policy_sets']['c1_timesheet_template']['uri'])
            else:
    ...
    if not dag_run.conf['mapper_data']['timeoff_template'] and _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
...
def _get_activities_update_payload(dag_run, user_details):
    if dag_run.conf['activities']['c1_activity']:
        activity_list = dag_run.conf['activities']['c1_activity'].split('|')
        user_activities = user_details['assignedActivities']
        if any(_activity not in {a['name'] for a in user_activities} for _activity in activity_list):
...
def get_new_limited_payrule_value(item, dxc_mapper, caller):
    parent_company = item['file_data']['parent_company']
    if caller == "puerto rico":
        return next(filter(lambda row: row['Type'] == "Limited Payrule" and\
                                row['Function'] == "Workday User Sync" and\
                                row['Country'] == item['file_data']['country'] and\
                                row['Source'] == parent_company and\
                                row['URI'] == item['file_data']["work_shift"] and\
                                row['employeesubgroup'] == item['file_data']['emp_subgroup_code'], dxc_mapper), {}).get('Value')
    ...
    if caller == "usa":
        return next(filter(lambda row: row['Type'] == "Limited Payrule" and\
                                ...
                                row['Source'] == parent_company and\
                                row['URI'] == item['file_data']["work_shift"] and\
                                row['employeesubgroup'] == item['file_data']['emp_subgroup_code'] and\
                                row['status'] == item['file_data']['state'], dxc_mapper), {}).get('Value')


def _get_payrule_schedule_to_update(dag_run, _get_user_details, effective_date, exception_log, mapper):
    is_ia = dag_run.conf['file_data'].get('is_ia')
    payrule_name = dag_run.conf['payrule']['payrule']

    # Check for limited payrule value based on work_shift and country
    if payrule_name and dag_run.conf['file_data']['work_shift'] and dag_run.conf['file_data']['parent_company'] == "C1":
        if dag_run.conf['file_data']['country'].lower() == "puerto rico":
            temp_payrule_value = get_new_limited_payrule_value(dag_run.conf, mapper, "puerto rico")
            if temp_payrule_value:
                payrule_name = temp_payrule_value
        if dag_run.conf['file_data']['country'].lower() == "united states of america" and dag_run.conf['file_data']['state'].lower() == "puerto rico":
            temp_payrule_value = get_new_limited_payrule_value(dag_run.conf, mapper, "usa")
            if temp_payrule_value:
                payrule_name = temp_payrule_value
        ...
        if not current_payrule_schedule or current_payrule_schedule['payRuleScript']['displayText'] != payrule_name:
            if dag_run.conf['file_data']['management_lvl'] not in ['L1', 'L2']:
                ...
                if _is_international_assignee(is_ia) and payrule_name == NONE_DEFAULT_VALUE:
                    ...
                    payrule_eff_date = effective_date if effective_date else {
...
def get_update_user_payload(dag_run, config):
    ...
    payload = {
        ...
        "modifications": {
            ...
            "schedulePolicyToApply": _get_shift_assignment_to_update(dag_run, user_details, config, exceptions, effective_date, isia_update_done),
            ...
            "payRulesScheduleModifications": _get_payrule_schedule_to_update(dag_run, user_details, effective_date, exceptions, config.DXC_WORKDAY_USER_SYNC_USER_MAPPER),
    ...
    # If user is an International Assignee, clear OEFs that are not applicable to USA CSC region
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
