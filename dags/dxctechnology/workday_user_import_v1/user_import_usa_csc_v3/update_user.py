...
from dxctechnology.workday_user_import_v1.user_import_usa_csc_v3.utils import request_payload, custom_methods
from dxctechnology.workday_user_import_v1.user_import_usa_csc_v3.tasks.supervisor_assignment import assign_supervisor
...
def create_update_user_dag(config):
    with rail.create_airflow_dag(
        dag_id = config.usa_csc_update_user_dag_id,
        description = "add user",
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs = config.max_run_update_user
    ) as dag:
        ...
        can_update_user_end_date_step_28 = rail.IfOperator(
            task_id = "can_update_user_end_date_step_28",
            test=custom_methods.can_update_user_end_date_test,
            yes_task= "update_user_end_date_step_29",
            no_task="trigger_process_timeoff_policies"
        )

        update_user_end_date_step_29 = rail.RepliconServiceOperator(
            task_id = "update_user_end_date_step_29",
            endpoint="/services/UserService1.svc/UpdateEmploymentDateRange",
            data=lambda dag_run: {
                "userUri": dag_run.conf['user_uri'],
                "dateRange": {
        ...
        def has_any_user_exception_callable(dag_run):
            ...
            isia_user_value = rail.find_first_by_attr_and_get_attr(
            current_custom_fields_values, "customField.displayText", 'International Assignee', 'text', default="")
            if dag_run.conf['file_data']['is_ia'] and dag_run.conf['file_data']['is_ia'] != isia_user_value:
        ...
        can_update_user = rail.IfOperator(
            ...
            test = lambda : bool(rail.result("has_any_user_exception")),
            yes_task = "log_user_processing_exception",
            no_task = "update_user"
        )
        ...
        is_parent_company_c1_and_empgrpcode_not_equal_3 = rail.IfOperator(
            ...
            yes_task="process_update_user_timeoff_assignment_us_csc",
            no_task= "process_update_user_timeoff_assignment_us_les"
        )

        process_update_user_timeoff_assignment_us_csc = rail.TriggerDagRunOperator(
            task_id = "process_update_user_timeoff_assignment_us_csc",
            trigger_dag_id=config.usa_csc_update_user_timeoff_assignment_dag_id,
            conf=lambda dag_run:{
                "file_name": dag_run.conf['master_file_name'],
                ...
                "is_ia_updated": rail.result("update_user","ia_updated"),
                "ia_updated": rail.result("update_user","ia_updated"), 
                ...
                "prevent_balance_overdraw_uri": dag_run.conf['prevent_balance_overdraw_uri'],
                "starting_balance_set_to_uri": dag_run.conf["starting_balance_set_to_uri"],
                "personnal_sub_area":dag_run.conf['file_data']['sub_area_code'],
                ...
                "old_ee_group": rail.find_first_by_attr_and_get_attr(rail.result("get_user_details")['userDetails']['customFieldValues'],
                    "customField.displayText", "EE Group", "text", ""),
                "contineous_service_date":dag_run.conf['file_data']['service_date'],
                "fte_updated": (("No" if dag_run.conf['file_data']['fte'] == rail.find_first_by_attr_and_get_attr(rail.result("get_user_details")['userDetails']['customFieldValues'],
                    "customField.displayText", "FTE", "text", "") else "Yes") if dag_run.conf['file_data']['fte'] else "No"),
        ...
        wait_for_timeoff_assignment_us_csc = rail.WaitForDagRunsSensor(
            ...
            dag_runs="""{{ result('process_update_user_timeoff_assignment_us_csc') }}""",
            execution_timeout=timedelta(
                    days=config.execution_timeout_days),
            retries=0
        )

        process_update_user_timeoff_assignment_us_les = rail.TriggerDagRunOperator(
            task_id = "process_update_user_timeoff_assignment_us_les",
        ...
        wait_for_timeoff_assignment_us_les = rail.WaitForDagRunsSensor(
            ...
            dag_runs="""{{ result('process_update_user_timeoff_assignment_us_les') }}""",
            execution_timeout=timedelta(
                    days=config.execution_timeout_days),
        )
        ...
        is_termination_date_present >> rail.Label("No") >> user_does_not_have_admin_and_payroll_permission >> rail.Label("Yes") >> disable_user_login_2 \
            >> can_update_user_end_date_step_28 >> rail.Label("No") >> trigger_process_timeoff_policies
        can_update_user_end_date_step_28 >> rail.Label("Yes") >> update_user_end_date_step_29 >> trigger_process_timeoff_policies
        ...
        can_update_user_start_date >> rail.Label("No") >> should_disable_user >> rail.Label("No") >> process_user_update >> has_any_user_exception
        ...
        is_end_date_less_than_today >> rail.Label("Yes") >> process_user_update 
        ...
        end_supervisor_assignment >> is_parent_company_c1_and_empgrpcode_not_equal_3
        is_parent_company_c1_and_empgrpcode_not_equal_3 >> rail.Label('Yes') >> process_update_user_timeoff_assignment_us_csc
        is_parent_company_c1_and_empgrpcode_not_equal_3 >> rail.Label('No') >> process_update_user_timeoff_assignment_us_les
        
        process_update_user_timeoff_assignment_us_csc >> wait_for_timeoff_assignment_us_csc >> log_user_completion >> should_trigger_delete_time_and_timeoff >> rail.Label("Yes") >> trigger_cleanup_for_disabled_user >> wait_for_trigger_cleanup_for_disabled_user >> catch_and_log_error
        process_update_user_timeoff_assignment_us_les >> wait_for_timeoff_assignment_us_les >> log_user_completion
