...
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import get_date_to_use_for_no_accrual, get_replicon_date, get_json_date_from_date, convert_json_date_to_string_date, convert_json_date_to_date, get_date_to_use_for_no_accrual
    ...
    with rail.create_airflow_dag(
        ...
        def get_required_details_2_callable(dag_run):
            ...
            return {
                ...
                "timeofftypestobeassigned": [toa for toa in timeofftypestobeassigned if toa['uri']],
        ...
        trigger_sick_non_california_timeoff_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda dag_run: {
                ...
                "company_code": dag_run.conf['company_code'],
                "source": dag_run.conf['parent_company_code'],
                "old_start_date": dag_run.conf['start_date'],
                "country": dag_run.conf['country'],
                "contineous_service_date": dag_run.conf['start_date'],
                "timeoff_uri": rail.result('process_each_assigned_timeoffs')['uri'],
                "timeoff_name": rail.result('process_each_assigned_timeoffs')['name'],
                ...
                "employee_group": dag_run.conf['employee_group'],
                "employee_sub_group":dag_run.conf['employee_sub_group'],
                **sick_non_california_timeoff_dag_config(dag_run, 'start_date')
            }
        )
        ...
        def employee_group_test(dag_run):
            return dag_run.conf['employee_group'] in ['3', '4', '5']

        def puerto_rico_emp_group_subgroup_test(dag_run):
            if dag_run.conf['country'].lower() == "puerto rico" or dag_run.conf['state'].lower() == "puerto rico":
                if dag_run.conf['employee_group'] in ['1', '7']:
                    if dag_run.conf['employee_sub_group'].upper() in ['F6', 'F7', 'P7', 'P6']:
                        return True
                    return True
            return False

        def sick_non_california_timeoff_dag_config(dag_run, dag_run_conf_date_key, ia_updated=False):
            if not employee_group_test(dag_run) and not puerto_rico_emp_group_subgroup_test(dag_run):
                _date_to_use = get_date_to_use_for_no_accrual(dag_run) if dag_run.conf['fte_updated'] != "Yes" else get_todays_date_in_json()
                return {
                    "start_date": _date_to_use,
                    "fte_update": dag_run.conf['fte_updated'],
                    "effective_date_to_use_for_sick_timeoff": get_todays_date_in_json()
                }
            return {
                    "start_date": dag_run.conf['start_date'],
                    "fte_update": dag_run.conf['fte_updated'],
                    "effective_date_to_use_for_sick_timeoff":  get_todays_date_in_json() if dag_run.conf['schedule_change'] == "Yes" else dag_run.conf[dag_run_conf_date_key]
                }
        ...
        trigger_sick_non_california_timeoff_dag2 = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda dag_run: {
                ...
                "employee_group": dag_run.conf['employee_group'],
                "employee_sub_group":dag_run.conf['employee_sub_group'],
                **sick_non_california_timeoff_dag_config(dag_run, 'schedule_changed_date')
            }
        )
        ...
        def get_effective_date(dag_run, default_date, return_type="json", validate_schedule_change=True):
            ...
            if dag_run.conf['is_ia_updated'].lower() == "yes":
                ...
                if dag_run.conf['is_ia'] in ['0',0]:
                    _end_date = get_replicon_date(dag_run.conf['ia_end_date']) + timedelta(days=1)
                    if return_type == "json":
            ...
            if return_type == "json":
                    return get_json_date_from_date_str(default_date) if default_date else None
            return default_date
        ...
        is_timeoff_sick_time = rail.IfOperator(
            ...
            yes_task="trigger_us_sick_non_california_timeoff_assignment",
        ...
        trigger_us_sick_non_california_timeoff_assignment = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda dag_run: {
                ...
                "company_code": dag_run.conf['company_code'],
                "source": dag_run.conf['parent_company_code'],
                "old_start_date": rail.result('date_to_considered', 'ia_date_to_use') if rail.result('date_to_considered', 'ia_date_to_use') else dag_run.conf['start_date'],
                "country": dag_run.conf['country'],
                "contineous_service_date": dag_run.conf['contineous_service_date'],
                "timeoff_uri": rail.result("for_each_timeoff")['uri'],
                "timeoff_name": rail.result("for_each_timeoff")['name'],
                ...
                "employee_group": dag_run.conf['employee_group'],
                "employee_sub_group":dag_run.conf['employee_sub_group'],
                **sick_non_california_timeoff_dag_config(dag_run, 'ia_date_to_use', True)
            }
        )
        ...
        is_timeoff_name_us_sick_time >> rail.Label('Yes') >> trigger_sick_non_california_timeoff_dag
        ...
        process_each_assigned_timeoffs >> end_process_each_assigned_timeoffs >> is_schedule_changed_and_not_rehire_not_ia_updated
        ...
        is_timeoff_name_usa_holiday2 >> rail.Label('No') >> is_timeoff_name_us_sick_time2 >> rail.Label("Yes") >> trigger_sick_non_california_timeoff_dag2
        is_timeoff_name_us_sick_time2 >> rail.Label("No") >> is_timeoff_name_vacation_accrued_2
        trigger_sick_non_california_timeoff_dag2 >> add_dag_run_id_to_wait7 >> is_timeoff_name_vacation_accrued_2
        ...
        process_usa_holiday_and_sick_timeoffs >> end_process_each_assigned_timeoffs2 >> has_any_timeoff_to_be_assign 
        ...
        is_timeoff_sick_time >> rail.Label('Yes') >> trigger_us_sick_non_california_timeoff_assignment >> add_dag_run_id_to_wait10 >> is_timeoff_es_holiday
