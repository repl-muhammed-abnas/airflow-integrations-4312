...
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import INPUT_DATE_FORMAT as DATE_FORMAT, TIMEZONE, get_replicon_date

null = None

def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.costa_rica_process_users_child_dag_id,
        ...
        def get_work_week_date_costa_rica(timezone, work_week):
            ...
            return pendulum.now(timezone
                                ...
                                ).subtract(days=get_required_days_to_subtract()
                                ).strftime(DATE_FORMAT
                                )
        
        def get_update_user_conf(dag_run):
            work_week_date = get_work_week_date_costa_rica(TIMEZONE, dag_run.conf['mapper_data']['workweek'])
            dag_run.conf['json_formatted_dates']['work_week_date'] = get_replicon_date(work_week_date)
            return {
                **{
        ...
        trigger_update_user = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.costa_rica_update_user_dag_id,
            conf=get_update_user_conf
        )
        ...
        trigger_add_user = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.costa_rica_add_user_dag_id,
            conf=lambda dag_run: {
                **{
                    "replicon_field": "true" if dag_run.conf['file_data']['status'] == "1" else "false", 
                    "todays_date" : (pendulum.now(TIMEZONE)).strftime(DATE_FORMAT),
