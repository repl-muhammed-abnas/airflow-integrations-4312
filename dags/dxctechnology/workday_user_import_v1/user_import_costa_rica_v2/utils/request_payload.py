...
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import convert_json_date_to_date, get_todays_minus_specified_days_date_in_json, get_json_date_from_date, get_excluded_udf_clear_payloads, get_excluded_oef_clear_payloads
from dxctechnology.workday_user_import_v1.user_import.common_utils.request_payload import get_todays_date_for_timezone_in_json, get_json_date_from_date_str, TIMESHEET_PERIOD_EFFECTIVE_DATE, get_psa_user_udf_add_update_payload, get_psa_udf_value
from dxctechnology.workday_user_import_v1.user_import.common_utils.region_fields_config import DXC_COSTA_RICA
...
def _get_shift_from_mapper(dag_run, config):
    ...
    return list(filter(lambda row: row['Type'] == "Schedule Type" and\
                                ...
                                row['Source'] == source,config.DXC_WORKDAY_USER_SYNC_USER_MAPPER))

def _get_schedule_policy_to_assign(dag_run, config, exception_log:list):
    if dag_run.conf['schedule_name'] == "ES-CR-ROT(CRI)":
    ...
    if dag_run.conf['schedule_name']:
        if dag_run.conf['schedule_uri']:
            return [
                {
                        "schedulePolicy": {
                            "officeScheduleUri": null,
                            "name": dag_run.conf['schedule_name'],
                            "officeSchedule": {
                                "officeScheduleUri": null,
                                "name": dag_run.conf['schedule_name']
                            },
        ...
        exception_log.append(f"""Office schedule "{dag_run.conf['schedule_name']}" not available in Replicon. Hence default shift assigned""")
        return [
            {
                "schedulePolicy": {
                    "officeScheduleUri": null,
...
def _get_policy_sets_to_assign(dag_run):
    ...
    if dag_run.conf['punch_entry_policy_uri']:
        policy_sets.append({
            "name": null,
...
def _add_custom_field(uri, text=null, date=null, drop_down_uri=null, drop_down_value_name=null, number=null):
    return {
        "customField": {
            "uri" : uri
        },
        ...
        } if drop_down_uri or drop_down_value_name else null,
        "number": number
    }
...
def _get_custom_fields_to_assign(dag_run, action):
    ...
    if action =="add":
        ...
        udfs_to_assign.append(_add_custom_field(custom_fields['perner']['uri'], text=dag_run.conf['file_data']['emp_id']))
        ...
        if dag_run.conf['file_data']['time_type']:
            udfs_to_assign.append(_add_custom_field(custom_fields['time_type']['uri'], drop_down_value_name=dag_run.conf['file_data']['time_type']))
        ...
        pas_flag = get_psa_user_udf_add_update_payload(dag_run = dag_run, 
                                                    current_udf_value = '',
                                                    caller = 'add',
                                                    current_user_groups = [],
                                                    effective_date = None,
                                                    _get_cost_center_update_payload = None,
                                                    _get_department_update_payload = None)
        if pas_flag:
            udfs_to_assign.append(_add_custom_field(custom_fields['psa_user']['uri'], drop_down_value_name=pas_flag))

        # Only add IA date fields if user is an International Assignee
        if str(dag_run.conf['file_data'].get('is_ia', '0')) == '1':
    ...
    if action == "update":
        ...
        if dag_run.conf['file_data']['fte_pct'] and dag_run.conf['file_data']['fte_pct'] != rail.result("current_assigned_udf_values")['ftepct']:
            udfs_to_assign.append(_add_custom_field(custom_fields['ftepct']['uri'], text=dag_run.conf['file_data']['fte_pct']))
        ...
        get_psa_udf_value(
            ...
            custom_fields_payload = udfs_to_assign, 
        ...
        # Only parse IA dates if user is actually an IA user
        # Store both dictionary format (for custom fields) and JSON format (for effective_date)
        ia_start_date_dict = get_replicon_date(dag_run.conf['file_data']['ia_start_date']) if is_ia  in [1, '1'] and dag_run.conf['file_data']['ia_start_date'] else None
        ia_end_date_dict = get_replicon_date(dag_run.conf['file_data']['ia_end_date']) if is_ia in [0, '0'] and dag_run.conf['file_data']['ia_end_date'] else None
        
        # Get JSON formatted dates for effective_date usage
        ia_start_date_json = dag_run.conf['json_formatted_dates'].get('ia_start_date') if is_ia in [1, '1'] and dag_run.conf['file_data']['ia_start_date'] else None
        ia_end_date_json = dag_run.conf['json_formatted_dates'].get('ia_end_date') if is_ia in [0, '0'] and dag_run.conf['file_data']['ia_end_date'] else None
        
        today_minus_five_days = convert_json_date_to_date(get_todays_minus_specified_days_date_in_json(5)) 

        if is_ia and (is_ia != rail.result("current_assigned_udf_values")['is_ia']):
            udfs_to_assign.append(_add_custom_field(custom_fields['international_assignee']['uri'], text=dag_run.conf['file_data']['is_ia']))
        
            if not ia_start_date_dict and is_ia in [1, '1']:
                rail.set_result(key="ia_updated", val=False)
                rail.set_result(key = "ia_exception_msg", val="User processing skipped as IAStart date not available for IA=1")

            if not ia_end_date_dict and is_ia in [0, '0']:
                rail.set_result(key="ia_updated", val=False)
                rail.set_result(key = "ia_exception_msg", val="User processing skipped as IAEnd date not available for IA=0")

            if is_ia in [1,'1'] and ia_start_date_json and (convert_json_date_to_date(ia_start_date_json) < today_minus_five_days):
                rail.set_result(key="ia_updated", val=False)
                rail.set_result(key = "ia_exception_msg", val="User processing skipped as IAStart date in past for IA=1")

            if is_ia in [0,'0'] and ia_end_date_json and (convert_json_date_to_date(ia_end_date_json) < today_minus_five_days):
                rail.set_result(key="ia_updated", val=False)
                rail.set_result(key = "ia_exception_msg", val="User processing skipped as IAEnd date in past for IA=0")

            if is_ia in [1,'1'] and ia_start_date_json:
                rail.set_result(key="ia_updated", val=True)
                ia_updated =True
                rail.set_result(key="effective_date", val=ia_start_date_json)
                effective_date = ia_start_date_json
                udfs_to_assign.append(_add_custom_field(custom_fields['international_assignee_start_date']['uri'],
                    date=ia_start_date_dict))

            if is_ia in [0,'0'] and ia_end_date_json:
                rail.set_result(key="ia_updated", val=True)
                ia_updated =True
                rail.set_result(key="effective_date", val=get_json_date_from_date(convert_json_date_to_date(ia_end_date_json) + timedelta(days=1)))
                udfs_to_assign.append(_add_custom_field(custom_fields['international_assignee_end_date']['uri'],
                    date=ia_end_date_dict))
                effective_date = get_json_date_from_date(convert_json_date_to_date(ia_end_date_json) + timedelta(days=1))
        else:
            rail.set_result(key="ia_updated", val=False)
            # Only update IA dates if user is an IA user
            if is_ia in [1, '1'] and dag_run.conf['file_data']['ia_start_date'] and _compare_if_two_json_dates_are_same(date_1=dag_run.conf['json_formatted_dates']['ia_start_date'],
            ...
            if is_ia in [0, '0'] and dag_run.conf['file_data']['ia_end_date'] and _compare_if_two_json_dates_are_same(date_1=dag_run.conf['json_formatted_dates']['ia_end_date'],
                date_2=rail.find_first_by_attr_and_get_attr(current_custom_fields_values,
        ...
        # If user is an International Assignee, clear UDFs that are not applicable to Costa Rica region
        if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
            excluded_udf_payloads = get_excluded_udf_clear_payloads(
                region=DXC_COSTA_RICA,
                current_custom_fields_values=current_custom_fields_values,
                udfs_config=dag_run.conf.get('udfs', {})
            )
            udfs_to_assign.extend(excluded_udf_payloads)

    return udfs_to_assign, effective_date, ia_updated
...
def _get_timesheet_period_schedule_to_assign(dag_run):
    if dag_run.conf['mapper_data']['timesheet_period']:
        json_formatted_date = get_json_date_from_date_str(dag_run.conf['mapper_data']['timesheet_period_effective_date'], TIMESHEET_PERIOD_EFFECTIVE_DATE)

        if dag_run.conf['mapper_data']['timesheet_period_effective_date']:
            if convert_json_date_to_date(json_formatted_date) >\
                ...
                return [
                    {
                        ...
                        "effectiveDate": json_formatted_date
                    }
                ]
        return [
            {
...
def create_user_payload(dag_run, config):
    ...
    payload = {
        "user": {
            ...
            "customFieldValues": _get_custom_fields_to_assign(dag_run,"add")[0],
...
def _can_update_email(dag_run, user_details, config, login_name_check=True):
    if config.instance in ["prod", "production"]:
...
def _get_payrule_schedule_to_update(dag_run, _get_user_details, effective_date):
    if dag_run.conf['mapper_data']['payrule']:
        ...
        if not current_payrule_schedule or dag_run.conf['mapper_data']['payrule'] != current_payrule_schedule['payRuleScript']['displayText']:
            if dag_run.conf['file_data']['management_lvl'] not in ['L1', 'L2']:
                payrule_effective_date = dag_run.conf['json_formatted_dates'].get('work_week_date', dag_run.conf.get('work_week_date'))
                return {
                    "scheduleEntries": [
                        {
                            "payRuleScript": {
    ...
    elif _is_international_assignee(dag_run.conf['file_data']['is_ia']) and not dag_run.conf['mapper_data']['payrule']:
        return {
                    "scheduleEntries": [
                        {
                            "payRuleScript": {
...
def _get_shift_assignment_to_update(dag_run, user_details, config, exception_log, effective_date):
    ...
    if dag_run.conf['schedule_name'] == "ES-CR-ROT(CRI)":
        ...
        if dag_run.conf['schedule_name']:
            if (not current_office_schedule) or (not current_office_schedule['officeSchedule']) or (current_office_schedule['officeSchedule']['displayText'] != dag_run.conf['schedule_name']):
                if dag_run.conf['schedule_uri']:
                    return {
                        "userSchedulePolicyScheduleModificationOptionUri": "urn:replicon:schedule-modification-option:update-schedule-over-date-range",
                        "replacementSchedule": [],
...
def _can_update_punch_entry_policies(dag_run):
    if not dag_run.conf['punch_entry_policy_name']:
        return False
    current_assigned_policies = rail.result("get_user_assigned_policy")
    return not bool(list(filter(lambda time_punch_policy: time_punch_policy['policySet']['name'] == dag_run.conf['punch_entry_policy_name'],
...
def _get_timesheet_period_schedule_to_apply(dag_run, user_details, effective_date):
    ...
    if user_details['timesheetPeriodSchedule']:
        ...
        if dag_run.conf['mapper_data']['timesheet_period'] and dag_run.conf['mapper_data']['timesheet_period'] != \
            ...
            timesheet_effective_date = dag_run.conf['json_formatted_dates'].get('work_week_date', dag_run.conf.get('work_week_date'))
            return {
    ...
    if _is_international_assignee(dag_run.conf['file_data']['is_ia']) and not dag_run.conf['mapper_data']['timesheet_period']:
        return {
            "userTimesheetPeriodScheduleModificationOptionUri": "urn:replicon:schedule-modification-option:update-schedule-over-date-range",
            "replacementTimesheetPeriodSchedule": [],
            "updateTimesheetPeriodScheduleOverDateRange": {
...
def _get_department_update_payload(dag_run, current_effective_grps, effective_date):
    if dag_run.conf['file_data']['org_code'] and dag_run.conf['file_data']['org_code'] != current_effective_grps['department'].get('displayText', ''):
        work_week_eff_date = dag_run.conf['json_formatted_dates'].get('work_week_date', dag_run.conf.get('work_week_date'))
        return {
                ...
                "replacementDepartmentGroupScheduleEntries": [
                    {
                        ...
                        "effectiveDate": effective_date if effective_date else work_week_eff_date
                    }
                ],
                "endDate": null
            }
...
def _get_policies_to_update_payload(dag_run, user_details, exception_log, profile_status_is_enabled, ia_updated, is_ia):
    policies_to_add = []
    policies_to_remove = []
    ...
    if _can_update_timeoff_template(dag_run, user_details, profile_status_is_enabled):
        ...
        if not dag_run.conf['mapper_data']['timeoff_template'] and ia_updated and is_ia in [1,'1']:
    ...
    # Costa Rica does not have any schedule Policies if in future added, this needs to be updated

    shift_schedule_policy = _get_assigned_shift_schedule_policy()
    if shift_schedule_policy and _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
...
def get_update_user_payload(dag_run, config):
    ...
    custom_felids_to_apply, effective_date, ia_updated = _get_custom_fields_to_assign(dag_run, "update")
    payload = {
        "user": {
        ...
        "modifications": {
            ...
            "timesheetPeriodScheduleToApply": _get_timesheet_period_schedule_to_apply(dag_run, user_details, effective_date),
            ...
            "policySetsToApply": _get_policies_to_update_payload(dag_run, user_details, exceptions, profile_status_is_enabled, ia_updated, dag_run.conf['file_data']['is_ia']),
            ...
            "customFieldValuesToApply": custom_felids_to_apply,
    ...
    # If user is an International Assignee, clear OEFs that are not applicable to Costa Rica region
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
