...
from dxctechnology.workday_user_import_v1.user_import_costa_rica_v2.utils import request_payload, custom_methods
from dxctechnology.workday_user_import_v1.user_import_costa_rica_v2.tasks.supervisor_assignment import assign_supervisor

null = None

def create_add_user_dag(config):
    with rail.create_airflow_dag(
        dag_id = config.costa_rica_add_user_dag_id,
        description = "DXC Workday User Import Costa Rica - Process Add User",
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs = config.max_active_run_add_user_costa_rica,
    ) as dag:
        ...
        get_mapper_timeoff_data = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: list(filter(lambda row: row['Type']=='Timeoff' and\
                ...
                row['personnelsubarea'] == dag_run.conf['file_data']['exempt'] and\
                row['employeegroup'] == (dag_run.conf['file_data']['work_shift'] if dag_run.conf['file_data']['work_shift'] =="ES-CR-ROT(CR)" else "Others")
        ...
        process_new_user_timeoff_assignment = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.costa_rica_add_user_timeoff_assignment_dag_id,
            conf=lambda dag_run:{
