...
from dxctechnology.workday_user_import_v1.user_import.common_utils.request_payload import get_todays_date_in_json, get_psa_udf_value, get_psa_user_udf_add_update_payload
...
# UDF fields that are NOT applicable to Australia region
# These fields should be cleared for IA=1 users coming to Australia from other regions
AUSTRALIA_EXCLUDED_UDFS = (
    "Work Shift",
    "Management Level",
    "EE Group",
)
...
def _get_udf_fields_to_clear_for_ia(dag_run, current_custom_fields_values, exception_log):
    ...
    for udf_display_text in AUSTRALIA_EXCLUDED_UDFS:
        for cf in current_custom_fields_values:
            if cf.get('customField', {}).get('displayText') == udf_display_text:
                ...
                if has_value:
                    exception_log.append(f"Clearing UDF '{udf_display_text}' for IA=1 user (not applicable to Australia region)")
...
def get_custom_fields_to_update_callable(dag_run):
    ...
    # For IA=1 users, clear UDF fields that are not applicable to Australia region
    ...
    if exception_log:
        rail.set_result(key="udf_ia_exceptions", val=exception_log)
