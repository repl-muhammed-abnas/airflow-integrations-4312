...
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import get_tenure_value, convert_json_date_to_date
...
def create_rehire_user_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.costa_rica_rehire_user_timeoff_assignment_dag_id,
        ...
        get_user_tenure_for_timeoff_type = rail.PythonOperator(
            ...
            python_callable= lambda dag_run: get_tenure_value(
                convert_json_date_to_date(dag_run.conf['continuous_service_date_json_format']),
                convert_json_date_to_date(dag_run.conf['start_date_json_format']))
        )
        ...
        def get_policy_to_assign_callable(dag_run):
            ...
            start_date = convert_json_date_to_date(dag_run.conf['start_date_json_format'])
            continue_service_date = convert_json_date_to_date(dag_run.conf['continuous_service_date_json_format'])
            ...
            current_timeoff_policies = dag_run.conf['current_timeoff_policies']
