...
# UDF fields that are NOT applicable to India region (same as Global)
INDIA_EXCLUDED_UDFS = (
    "Work Shift",
    "Date of Birth",
    "Time Type",
...
def _get_udf_fields_to_clear_for_ia(dag_run, current_custom_fields_values, exception_log):
    ...
    for udf_display_text in INDIA_EXCLUDED_UDFS:
        for cf in current_custom_fields_values:
            if cf.get('customField', {}).get('displayText') == udf_display_text:
                ...
                if has_value:
                    exception_log.append(f"Clearing UDF '{udf_display_text}' for IA=1 user (not applicable to India region)")
...
def _get_custom_fields_to_assign(dag_run, action, exception_log=None):
    ...
    if action == "update":
        ...
        # For IA=1 users, clear UDF fields that are not applicable to India region
...
def create_user_payload(dag_run, config):
    ...
    payload = {
        "user": {
            ...
            "customFieldValues": _get_custom_fields_to_assign(dag_run,"add", exception_log),
...
def _get_shift_assignment_to_update(dag_run, user_details, config, exception_log):
    ...
    if dag_run.conf['file_data']['work_shift'] == "ES-Rotational Work shift":
        if not current_office_schedule or (current_office_schedule['officeSchedule']['scheduleTypeUri'] != mapper_shift_details[0].get('URI', '')):
            return {
...
def get_update_user_payload(dag_run, config):
    ...
    payload = {
        ...
        "modifications": {
            ...
            "customFieldValuesToApply": _get_custom_fields_to_assign(dag_run, "update", exceptions),
