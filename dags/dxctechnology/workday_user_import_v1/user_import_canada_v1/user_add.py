...
from dxctechnology.workday_user_import_v1.user_import_canada_v1.utils import request_payload
...
def create_add_user_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_canada_users_add_user_child_dag,
        ...
        supervisor_start_dummy = rail.EmptyOperator(
            task_id = "supervisor_start_dummy"
        )
        ...
        perform_timeoff_assignment = rail.IfOperator(
            ...
            no_task="supervisor_start_dummy"
        )

        process_new_user_timeoff_assignment_canada = rail.TriggerDagRunOperator(
            task_id = "process_new_user_timeoff_assignment_canada",
            trigger_dag_id=config.workday_user_import_canada_users_add_user_timeoff_process_child_for_canada_dag,
            conf=lambda dag_run: {
                **dag_run.conf,
                **{
                    ...
                    "status": dag_run.conf['file_data']['company_code']
                }
            },
        ...
        wait_for_timeoff_assignment = rail.WaitForDagRunsSensor(
            ...
            dag_runs="""{{ result('process_new_user_timeoff_assignment_canada')}}""",
            execution_timeout=timedelta(
                    days=config.execution_timeout_days),
            retries=0
        )
        ...
        perform_timeoff_assignment >> rail.Label("Yes, Canada & C1") >> process_new_user_timeoff_assignment_canada\
            >> wait_for_timeoff_assignment >> supervisor_start_dummy
        perform_timeoff_assignment >> supervisor_start_dummy >> supervisor_start
