...
from dxctechnology.workday_user_import_v1.user_import_canada_v1.utils import custom_methods
from dxctechnology.workday_user_import_v1.user_import_canada_v1.utils.request_payload import prepare_update_user_payload_callable
...
from dxctechnology.workday_user_import_v1.user_import_canada_v1.task.supervisor_assignment_task import assign_supervisor
...
def create_update_user_dag(config):
    with rail.create_airflow_dag(
        ...
        def get_user_update_log_message(dag_run):
            # Drop falsy entries: log_timesheet_template_exception is skipped on the happy path
            exception_message = [
                message for message in (
                    (rail.result("prepare_update_user_payload", "exception_log") or [])
                    + (rail.result('log_exception') or [])
                    + [rail.result('log_timesheet_template_exception')]
                ) if message
            ]
            return{
                "Jobid": "",
        ...
        check_remove_timeoff_template >> rail.Label("Yes") >> remove_timeoff_template >> log_exception
