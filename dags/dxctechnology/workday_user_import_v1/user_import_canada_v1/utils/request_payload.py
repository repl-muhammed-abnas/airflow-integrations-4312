...
import rail
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import convert_json_date_to_date, get_ia_update_payload_for_udf_update, compare_if_two_json_dates_are_same, get_excluded_udf_clear_payloads, get_excluded_oef_clear_payloads
...
from dxctechnology.workday_user_import_v1.user_import.common_utils.region_fields_config import DXC_CANADA
from dxctechnology.workday_user_import_v1.user_import_canada_v1.utils.custom_methods import is_profile_enabled, get_replicon_date
...
def _add_custom_field(custom_field_uri, text=null, date=null, drop_down_name=null, drop_down_uri=null, number=null):
    return {
        "customField": {
...
def _get_payrule_script_schedule_to_assign(dag_run):
    ...
    # Only assign payrule for Canada non-L1/L2
...
def get_timeoff_assignment_payload_for_update_user(dag_run):
    # Normal flow: at least one mapper-eligible timeoff is missing from the user's
    # current assignments — send the full mapper URI list so the missing ones get added.
    if rail.result("get_timeoff_to_disable_and_to_assign"):
        return {
            "userUri": dag_run.conf['user_uri'],
            "timeOffTypeUris": rail.result("map_mapper_replicon_timeoff")
        }
    # RIT-20720: pure-disable case — nothing new to add, but the user (non-Yukon) still
    # holds [CAN] Company Holiday. Send (current enabled URIs minus Company Holiday) so
    # Replicon's replace-semantics PUT drops only that one URI; manual assignments survive.
    ch_uri = rail.result("get_timeoff_to_disable_and_to_assign", "company_holiday_uri_to_disable")
    current_uris = [p['uri'] for p in rail.result("get_timeoff_to_disable_and_to_assign", "current_timeoffs_policies")]
    return {
        "userUri": dag_run.conf['user_uri'],
        "timeOffTypeUris": [u for u in current_uris if u != ch_uri]
    }
...
def get_custom_fields_payload(uri, txt_value=null, date_value=null, drop_down_value=null, drop_down_value_name=null, number_value=null):
    return {
        "customField": {
            "uri": uri,
            "name": null,
...
def get_user_custom_field_update_payload(dag_run, user_details):
    ...
    # If user is an International Assignee, clear UDFs that are not applicable to Canada region
    ...
    return custom_fields_payload, can_update_notifications, effective_date, ia_updated
...
def prepare_update_user_payload_callable(dag_run, config):
    ...
    custom_field_update_payload, can_update_notification_pref, effective_date, ia_updated = get_user_custom_field_update_payload(dag_run, user_details)
    ...
    if dag_run.conf['mapper_data']['time_entry_approval_path_name']:
        ...
        if not current_timeentry_approval_path or \
            current_timeentry_approval_path['displayText'] != dag_run.conf['mapper_data']['time_entry_approval_path_name']:
    ...
    if _is_international_assignee(is_ia) and not timesheet_period_name:
        ...
        if timesheet_period_name != current_timesheet_period['timesheetPeriod']['displayText']:
    ...
    if dag_run.conf['activities']['activity']:
        ...
        current_activities = [activity['name'] for activity in (_get_user_details['assignedActivities'] or [])]
        # Assign whenever the mapper list differs. This previously needed the user to already hold
        # one of the mapper activities, so a user with none never got them assigned.
        if sorted(current_activities) != sorted(activity_list):
    ...
    if _is_international_assignee(is_ia) and not dag_run.conf['mapper_data'].get('holiday_calendar'):
    ...
    # If user has a shift-schedule policy assigned, remove it
    # Canada does not have any schedule Policies if in future added, this needs to be updated
    ...
    if shift_schedule_policy and _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
        payload['modifications']['policySetsToApply'] = {
            "policySetUrisToAssign": [],
            "policyUrisToRemovePolicySet": [],
            "policySetUrisToRemove": [shift_schedule_policy['policySet']['uri']]
        }

    # If user is an International Assignee, clear OEFs that are not applicable to Canada region
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
