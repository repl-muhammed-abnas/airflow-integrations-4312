...
# UDF fields that are NOT applicable to Hungary region
# Hungary uses Work Shift, Date of Birth, Time Type, Middle Name, and PERNER
HUNGARY_EXCLUDED_UDFS = (
    "Personnel Area Code",
...
def _get_udf_fields_to_clear_for_ia(dag_run, current_custom_fields_values, exception_log):
    ...
    for udf_display_text in HUNGARY_EXCLUDED_UDFS:
        for cf in current_custom_fields_values:
            if cf.get('customField', {}).get('displayText') == udf_display_text:
                ...
                if has_value:
                    exception_log.append(f"Clearing UDF '{udf_display_text}' for IA=1 user (not applicable to Hungary region)")
...
def _update_custom_fields_for_user(dag_run, current_user_groups, exception_log):
    ...
    # For IA=1 users, clear UDF fields that are not applicable to Hungary region
