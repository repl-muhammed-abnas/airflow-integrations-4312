...
from dxctechnology.workday_user_import_v1.user_import_hungary.utils import request_payload, custom_methods
from dxctechnology.workday_user_import_v1.user_import_hungary.tasks.supervisor_assignment import assign_supervisor
from dxctechnology.workday_user_import_v1.user_import_hungary.utils.response_filter import get_effective_grp_membership_data_handler
from dxctechnology.workday_user_import_v1.user_import_hungary.utils.custom_methods import should_trigger_delete_time_and_timeoff_for_disabled_user
...
def create_update_user_dag(config):
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            dag_id = f"{config.workday_user_import_hungary_update_user_dag}{prefix}",
            description = config.workday_user_import_hungary_update_user_dag_description,
            company_key=config.company_key,
            replicon_conn_id=config.replicon_conn_id,
            max_active_runs = config.max_active_run_update_user_hungary
        ) as dag:
            ...
            trigger_rehire_timeoff_assignment = rail.TriggerDagRunForEachItemOperator(
                ...
                trigger_dag_id=lambda dag_run: custom_methods.get_trigger_dag_id(
                    config.workday_user_import_hungary_user_rehire_timeoff_process_dag,
                    config.DAG_BATCH_COUNT,
                    item_index=(custom_methods.get_item_index(dag_run, config.DAG_BATCH_COUNT))
                ),
            ...
            def get_filtered_timeoff_types_to_disable():
                return rail.result("get_required_timeoff_type_details")["timeoffs_to_disable"]
            ...
            process_timeoffs = rail.TriggerDagRunForEachItemOperator(
                    ...
                    trigger_dag_id=lambda dag_run, item: custom_methods.get_trigger_dag_id(
                        config.workday_user_import_hungary_update_user_timeoff_assignment_dag,
                        config.DAG_BATCH_COUNT,
                        item_index=item['to_index']#(custom_methods.get_item_index(dag_run, config.DAG_BATCH_COUNT))
                    ),
                    conf=lambda dag_run, item: {
            ...
            should_trigger_delete_time_and_timeoff = rail.IfOperator(
                ...
                test=config.instance != "prod" and should_trigger_delete_time_and_timeoff_for_disabled_user,
            ...
            log_user_completion >> should_trigger_delete_time_and_timeoff
