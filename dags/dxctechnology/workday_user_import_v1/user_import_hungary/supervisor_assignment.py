...
import rail
from dxctechnology.workday_user_import_v1.user_import_hungary.utils.request_payload import get_todays_date_in_json
from dxctechnology.workday_user_import_v1.user_import_hungary.utils.custom_methods import convert_json_date_to_date
from dxctechnology.workday_user_import_v1.user_import_hungary.utils.custom_methods import get_date_from_replicon_date
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        max_active_runs=config.max_active_run_process_each_users_hungary
    ) as dag:
        ...
        def get_supervisor_details_data_handler(response, dag_run):
            ...
            parsed_records = list(map(lambda record: {
                "user": record['cells'][0],
                ...
                'enddate': get_date_from_replicon_date(record['cells'][5]['dateValue']).strftime("%m-%d-%Y") if record['cells'][5]['textValue'] != "" else None
            }, response['rows']))

            exact_match = list(filter(lambda emp_record: 
                emp_record['emp_id'] == supervisor_id and emp_record['login_name'] == login_name, 
                parsed_records))
            
            login_match = list(filter(lambda emp_record: emp_record['login_name'] == login_name, parsed_records))
            emp_id_match = list(filter(lambda emp_record: emp_record['emp_id'] == supervisor_id, parsed_records))

            validation_message = ""
            if not login_match or not emp_id_match:
                validation_message = "Supervisor login name and ID do not match"
            if not login_match and not emp_id_match:
                validation_message = "Supervisor information doesn’t exist in Replicon"

            rail.set_result(key="exception_message", val=validation_message)

            return exact_match[0] if exact_match else {"user_uri": None}
        ...
        is_supervisor_end_date_in_past = rail.IfOperator(
            ...
            test=lambda: datetime.now() >  datetime.strptime(
                rail.result('get_supervisor_details')['enddate'], "%m-%d-%Y")
            if rail.result('get_supervisor_details') and rail.result('get_supervisor_details')['enddate'] else False,
        ...
        log_supervisor_end_date_in_past = rail.WriteLogOperator(
            ...
            properties = lambda dag_run: {
                ...
                "Status": "Exception",
                "Details": "Supervisor not added due to end date in past",
            }
        )
        ...
        log_exception_user_not_found = rail.WriteLogOperator(
            ...
            properties = lambda dag_run: {
                ...
                "Action": dag_run.conf['Action'],
                "Status": "Exception",
                "Details": rail.result('get_supervisor_details', key='exception_message') if rail.result('get_supervisor_details', key='exception_message') 
                        else "Supervisor not found in Replicon",
            }
        )
        ...
        is_user_not_found >> rail.Label("No") >> is_supervisor_end_date_in_past
        is_user_disabled >> rail.Label("No") >> get_user_assigned_permissions

        is_user_disabled >> rail.Label("Yes") >> enable_login >> get_user_assigned_permissions

        is_supervisor_end_date_in_past >> rail.Label("Yes") >> log_supervisor_end_date_in_past >> rail.Label("On Error") >> catch_and_log_error
        is_supervisor_end_date_in_past >> rail.Label("No") >> is_user_disabled
        
        get_user_assigned_permissions >> is_user_does_not_have_supervisor_permission
