...
from dxctechnology.workday_user_import_v1.user_import_hungary.utils.custom_methods \
...
def create_dag(config):
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.workday_user_import_hungary_user_rehire_timeoff_process_dag}{prefix}",
            description=config.workday_user_import_hungary_user_rehire_timeoff_process_dag_description,
            replicon_conn_id=config.replicon_conn_id,
            company_key=config.company_key,
            start_date=datetime(2025, 1, 1),
            max_active_runs=config.max_active_run_rehire_user_timeoff_assignement_hungary
        ) as dag:
