...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.usa_les_process_users_child_dag_id,
        description="DXC Workday User Import Costa Rica - Process Each Users",
        replicon_conn_id=config.replicon_conn_id,
        company_key=config.company_key,
        start_date=datetime(2023, 9, 26),
        max_active_runs=config.max_active_run_process_each_users_costa_rica
    ) as dag:
        ...
        trigger_update_user = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.usa_lse_update_user_dag_id,
            conf=lambda dag_run: {
                **{
        ...
        trigger_add_user = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.usa_lse_add_user_dag_id,
            conf=lambda dag_run: {
                **{
                    "user_log": rail.result('create_user_log')
                },
