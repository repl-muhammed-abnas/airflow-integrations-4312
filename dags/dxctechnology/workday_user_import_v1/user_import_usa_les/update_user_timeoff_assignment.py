...
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import convert_json_date_to_date, get_json_date_from_date, get_date_to_use_for_no_accrual
    ...
    with rail.create_airflow_dag(
        ...
        trigger_sick_california_timeoff_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda dag_run: {
                ...
                "timeoff_uri": rail.result('process_each_assigned_timeoffs')['uri'],
                "timeoff_name": rail.result('process_each_assigned_timeoffs')['name'],
                "secondary_timeoff_uri": rail.find_first_by_attr_and_get_attr(rail.result("get_all_timeoffs "), "name", "[USA] ES CA Sick Time", 'uri', ''),
        ...
        is_state_california3 = rail.IfOperator(
            task_id = "is_state_california3",
            test = "{{ dag_run.conf.state == 'California'}}",
            yes_task = "trigger_sick_california_timeoff",
            no_task = "trigger_sick_non_california_timeoff"
        )

        trigger_sick_california_timeoff = rail.TriggerDagRunForEachItemOperator(
            task_id = "trigger_sick_california_timeoff",
            items = [1],
            trigger_dag_id = config.usa_les_us_sick_leave_california_user_timeoff_assignment_dag_id,
            conf=lambda dag_run: {
                "user_log": dag_run.conf['user_log'],
        ...
        add_dag_run_id_to_wait10 = rail.SetVariableOperator(
            ...
            name= lambda: rail.result('set_variable_to_store_run_id')['name'],
            value=lambda: rail.result("trigger_sick_california_timeoff"),
            append=True
        )
        ...
        is_timeoff_name_usa_holiday >> rail.Label("No") >> is_timeoff_name_us_sick_time >> rail.Label("yes") >> is_state_california >> rail.Label("Yes") >> trigger_sick_california_timeoff_dag >> add_dag_run_id_to_wait3 >> end_process_each_assigned_timeoffs
        is_state_california >> rail.Label("Yes") >> trigger_sick_non_california_timeoff_dag >> add_dag_run_id_to_wait4 >> end_process_each_assigned_timeoffs
        process_each_assigned_timeoffs >> end_process_each_assigned_timeoffs
        is_timeoff_name_us_sick_time >> rail.Label("N0") >> end_process_each_assigned_timeoffs
        ...
        is_timeoff_name_us_sick_time2 >> rail.Label("Yes") >> is_state_california2 >> rail.Label("Yes") >> trigger_sick_california_timeoff_dag2 >> add_dag_run_id_to_wait6 >> end_process_each_assigned_timeoffs2
        is_state_california2 >> rail.Label("No") >> trigger_sick_non_california_timeoff_dag2 >> add_dag_run_id_to_wait7 >> end_process_each_assigned_timeoffs2
        ...
        is_timeoff_name_not_us_holiday_us_sick_leave >> rail.Label("No") >> is_timeoff_usa_sick_time >> rail.Label("Yes") >> is_state_california3 >> rail.Label(
            "Yes") >> trigger_sick_california_timeoff >> add_dag_run_id_to_wait10 >> for_each_end
        ...
        is_state_california3 >> rail.Label("No") >> trigger_sick_non_california_timeoff >> add_dag_run_id_to_wait11 >> for_each_end >> empty_gather_all_triggered_dag_runs
