...
from dxctechnology.workday_user_import_v1.user_import_usa_les.utils.request_payload import _add_custom_field
...
def _update_custom_fields_for_user(dag_run):
    ...
    # For IA=1 users, clear UDF fields that are not applicable to USA LES region
    exception_log = []
    from dxctechnology.workday_user_import_v1.user_import_usa_les.utils.request_payload import _get_udf_fields_to_clear_for_ia
    ...
    if exception_log:
        existing_log = rail.result("exception_log")
        existing_log.extend(exception_log)
        rail.set_result(key="exception_log", val=existing_log)
