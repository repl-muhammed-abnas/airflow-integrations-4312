...
from dxctechnology.workday_user_import_v1.user_import.common_utils.request_payload import get_todays_date_in_json, get_psa_user_udf_add_update_payload
import rail
...
# UDF fields that are NOT applicable to USA LES region (same as Global)
USA_LES_EXCLUDED_UDFS = (
    "Work Shift",
    "Date of Birth",
    "Time Type",
    "Middle Name",
...
def _get_udf_fields_to_clear_for_ia(dag_run, current_custom_fields_values, exception_log):
    ...
    for udf_display_text in USA_LES_EXCLUDED_UDFS:
        for cf in current_custom_fields_values:
            if cf.get('customField', {}).get('displayText') == udf_display_text:
                ...
                if has_value:
                    exception_log.append(f"Clearing UDF '{udf_display_text}' for IA=1 user (not applicable to USA LES region)")
...
def _get_policy_sets_to_assign(dag_run):
    ...
    if dag_run.conf['mapper_data']['profile_status'] in ['true', True, 'True'] or dag_run.conf['mapper_data']['profile_status'] == "enabled":
        ...
        if dag_run.conf['mapper_data']['timesheet_template'] and dag_run.conf['mapper_data']['profile_status']:
...
def _get_is_login_enabled(dag_run):
    if (not dag_run.conf['allowed_country']) or (dag_run.conf['allowed_country'].lower() != "enable") or\
        (not dag_run.conf['file_data']['parent_company']) or (not dag_run.conf['mapper_data']['profile_status']) or\
            (dag_run.conf['mapper_data']['profile_status'].lower() != "enabled"):
        return False
