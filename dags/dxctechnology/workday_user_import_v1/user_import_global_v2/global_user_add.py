...
from dxctechnology.workday_user_import_v1.user_import_global_v2.utils import request_payload
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_global_v2_users_add_user_child_dag,
        ...
        process_new_user_timeoff_assignment_canada = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.workday_user_import_global_v2_users_add_user_timeoff_process_child_for_canada_dag,
            conf=lambda dag_run: {
        ...
        process_new_user_timeoff_assignment = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.workday_user_import_global_v2_users_add_user_timeoff_process_child_dag,
            conf=lambda dag_run:{
                "file_name": dag_run.conf['master_file_name'],
                "user_uri": rail.result('create_user')['uri'],
                "loginName": rail.result('create_user')['loginName'],
