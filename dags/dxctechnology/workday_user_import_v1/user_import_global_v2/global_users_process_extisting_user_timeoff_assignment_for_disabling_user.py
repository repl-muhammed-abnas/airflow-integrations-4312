...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_global_v2_users_update_user_timeoff_process_child_dag_disable,
        ...
        def get_effective_date_callable(dag_run):
            ...
            if rail.result("get_timeoff_details")[0]['displayText'].startswith("[AUS]"):
                _effective_date = _effective_date + timedelta(days=1)
                return {
        ...
        get_timeoff_details >> get_effective_date >> get_balance_summary >> put_user_time_off_account_policy_set_schedule >> catch_and_log_error
