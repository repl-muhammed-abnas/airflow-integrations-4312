...
from dxctechnology.workday_user_import_v1.user_import_global_v2.utils.custom_methods import is_profile_enabled
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import get_user_uri
...
def assign_supervisor(group_id, caller):
    with rail.TaskGroup(group_id=group_id, prefix_group_id=False):
        ...
        is_caller_update = rail.IfOperator(
            ...
            no_task="get_supervisor_details_dummy"
        )

        get_supervisor_details_dummy = rail.EmptyOperator(
            task_id="get_supervisor_details_dummy"
        )
        ...
        is_supervisor_found = rail.IfOperator(
            ...
            no_task="is_supervisor_found_log_supervisor_for_reassignment_check_no_task",
            yes_task="is_company_code_present_for_supervisor_empty_task"
        )

        is_supervisor_found_log_supervisor_for_reassignment_check_no_task = rail.EmptyOperator(
            task_id = "is_supervisor_found_log_supervisor_for_reassignment_check_no_task"
        )
        ...
        is_company_code_present_for_supervisor_empty_task = rail.EmptyOperator(
            task_id = "is_company_code_present_for_supervisor_empty_task"
        )

        is_company_code_present_for_supervisor = rail.IfOperator(
            task_id = "is_company_code_present_for_supervisor",
            test=lambda: bool(rail.result('get_supervisor_details')['parent_div']),
            yes_task="is_supervisor_disabled",
            no_task="is_company_code_present_for_supervisor_log_supervisor_for_reassignment_check"
        )

        is_company_code_present_for_supervisor_log_supervisor_for_reassignment_check = rail.EmptyOperator(
            task_id="is_company_code_present_for_supervisor_log_supervisor_for_reassignment_check"
        )
        ...
        def get_supervisor_permission_details_data_handler(response, dag_run):
            ...
            return {
                "end_user_permission": rail.find_first_by_attr_and_get_attr(response,
                                        'permissionSet.name', dag_run.conf['user_permission_sets']['supervisor_end_user_permission'].get('name')),
                "supervisor_permission":rail.find_first_by_attr_and_get_attr(response,
                                        'permissionSet.name', dag_run.conf['user_permission_sets']['supervisor_user_permission'].get('name')),
            }
        ...
        assign_supervisor_permission = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "userUri": rail.result('get_supervisor_details')['user_uri'],
                "permissionSetUri": dag_run.conf['user_permission_sets']['supervisor_user_permission'].get('uri')
            }
        )
        ...
        assign_end_user_permission = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "userUri": rail.result('get_supervisor_details')['user_uri'],
                "permissionSetUri": dag_run.conf['user_permission_sets']['supervisor_end_user_permission'].get('uri')
            }
        )
        ...
        def get_data_access_scope_to_assign_callable(dag_run):
            ...
            _division = list(map(lambda division: {
                                    ...
                                    , [item for item in rail.load_json_artifact(dag_run.conf['division_data']) if item['parent']==dag_run.conf['file_data']['parent_company']])
                                )
        ...
        is_profile_status_enabled >> rail.Label("No") >> finish
        is_profile_status_enabled >> rail.Label("Yes") >> is_supervisor_id_user_id_same

        is_supervisor_id_user_id_same >> rail.Label("Yes") >> log_supervisor_id_user_id_same >> finish
        is_supervisor_id_user_id_same >> rail.Label("No") >> is_caller_update >> rail.Label("No") >> get_supervisor_details_dummy >> get_supervisor_details
        ...
        can_update_supervisor >> rail.Label("No") >> finish

        get_supervisor_details >> is_supervisor_found >> rail.Label("No") >> is_supervisor_found_log_supervisor_for_reassignment_check_no_task >> log_supervisor_for_reassignment_check
        is_supervisor_found >> rail.Label("Yes") >> is_company_code_present_for_supervisor_empty_task >> is_company_code_present_for_supervisor

        is_company_code_present_for_supervisor >> rail.Label("Yes") >> is_supervisor_disabled >> rail.Label("No") >> get_supervisor_permission_details >> is_supervisor_permission_missing
        is_company_code_present_for_supervisor >> rail.Label("No") >> is_company_code_present_for_supervisor_log_supervisor_for_reassignment_check >> log_supervisor_for_reassignment_check
        
        is_supervisor_disabled >> rail.Label("Yes") >> log_supervisor_for_reassignment_check >> finish
        ...
        is_end_user_permission_missing >> rail.Label("No") >> assign_supervisor_to_user >> finish


        return is_profile_status_enabled, finish