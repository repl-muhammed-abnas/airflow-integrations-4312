...
import rail
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import convert_json_date_to_date, get_ia_update_payload_for_udf_update, compare_if_two_json_dates_are_same, get_json_date_from_date, get_excluded_udf_clear_payloads, get_excluded_oef_clear_payloads
from dxctechnology.workday_user_import_v1.user_import.common_utils.region_fields_config import DXC_GLOBAL
from dxctechnology.workday_user_import_v1.user_import.common_utils.request_payload import get_todays_date_in_json,TIMESHEET_PERIOD_EFFECTIVE_DATE, get_psa_udf_value, get_psa_user_udf_add_update_payload
from dxctechnology.workday_user_import_v1.user_import_global_v2.utils.custom_methods import is_profile_enabled, get_replicon_date
...
def _get_holiday_calendar_to_assign(dag_run):
    if not dag_run.conf['mapper_data']["holiday_calendar"]:
        return null
    if dag_run.conf['mapper_data']["holiday_calendar"] and dag_run.conf['mapper_data']["holiday_calendar"].lower() == "null":
        return null
...
def _get_custom_fields_to_assign(dag_run):
    ...
    if pas_flag:
        udfs_to_assign.append(_add_custom_field(custom_fields['psa_user']['uri'], drop_down_name=pas_flag))
...
def _get_employee_type_schedule_to_assign(dag_run):
    if dag_run.conf['groups']['employee_type']:
        return [
            {
                "employeeTypeGroup": {
                    "uri": dag_run.conf['groups']['employee_type']['exempt_salaried'  if dag_run.conf['groups']['employee_type']["is_exempt"] else 'non_exempt_hourly']['uri'],
                    "parent": null,
                    "name": null,
                    "parameterCorrelationId": null
                },
...
def _get_payrule_script_schedule_to_assign(dag_run):
    payrule_name = dag_run.conf['payrule']['payrule']

    # Only assign payrule for Canada and non-L1/L2
...
def get_custom_fields_payload(uri, txt_value=null, date_value=null, drop_down_value_name=null, drop_down_value=null, number_value=null):
    return {
        "customField": {
            "uri": uri,
            "name": null,
        ...
        "dropDownOption": {
            ...
            } if drop_down_value or drop_down_value_name else null,
        "number": number_value
    }
...
def get_user_custom_field_update_payload(dag_run, user_details):
    ...
    get_psa_udf_value(
        ...
        _get_cost_center_update_payload = get_cost_center_update_payload,
        _get_department_update_payload = get_department_update_payload,
        caller= "update")

    # If user is an International Assignee, clear UDFs that are not applicable to Global region
    # This handles cases where IA users come from other regions (e.g., USA CSC, Australia)
    # and have UDF values that should not exist in Global
    ...
    return custom_fields_payload, can_update_notifications, ia_updated, effective_date

def get_department_update_payload(dag_run, current_effective_grps, work_week_effective_date, effective_date_to_use=None):
...
def get_effective_date_based_on_work_week(work_week, work_week_starts_with_check:list):
    is_start_with_saturday = work_week.lower().split(" ")[0] == "saturday" if work_week else False
    today = datetime.now()
...
def _build_group_schedule_payload(schedule_type, group_key, uri, effective_date,
                                   parent_key="parent", name=null, include_param_id=True):
    # Build the modification option URI key and value
    option_key = f"user{schedule_type[0].upper()}{schedule_type[1:]}ScheduleModificationOptionUri"
    option_value = "urn:replicon:schedule-modification-option:update-schedule-over-date-range"

    # Build replacement and update keys
    replacement_key = f"replacement{schedule_type[0].upper()}{schedule_type[1:]}Schedule"
    update_key = f"update{schedule_type[0].upper()}{schedule_type[1:]}ScheduleOverDateRange"
    entries_key = f"replacement{schedule_type[0].upper()}{schedule_type[1:]}ScheduleEntries"
    ...
    if include_param_id:
        group_entry[group_key]["parameterCorrelationId"] = null

    return {
        option_key: option_value,
        replacement_key: [],
        update_key: {
            entries_key: [group_entry],
            "endDate": null
        }
    }
...
def prepare_update_user_payload_callable(dag_run, config):
    ...
    custom_field_update_payload, can_update_notification_pref, ia_updated, effective_date = get_user_custom_field_update_payload(dag_run, user_details)
    payload['modifications']['customFieldValuesToApply'] = custom_field_update_payload

    # If user is an International Assignee, clear OEFs that are not applicable to Global region
    # This handles cases where IA users come from regions with OEFs (e.g., UKI)
    if _is_international_assignee(is_ia):
        ...
        if current_oef_values:
            ...
            if excluded_oef_payloads:
                payload['modifications']['objectExtensionFieldValuesToApply'] = excluded_oef_payloads

    # Payrule update logic with IA=1 NONE fallback
    payrule_name = dag_run.conf['payrule']['payrule']
    should_update_payrule = bool(payrule_name) or (_is_international_assignee(is_ia))

    if should_update_payrule:
        ...
        if payrule_name and (not current_payrule_schedule or (current_payrule_schedule['payRuleScript']['displayText'] != payrule_name)):
            if dag_run.conf['file_data']['management_lvl'] not in ['L1', 'L2'] or (_is_international_assignee(is_ia)):
    ...
    if dag_run.conf['file_data']['work_shift']:
        ...
        if _is_international_assignee(is_ia):
            logger.append(f"Schedule not available in mapper for IA=1. Assigning {NONE_DEFAULT_VALUE} schedule.")
            payload['modifications']['schedulePolicyToApply'] = {
                "userSchedulePolicyScheduleModificationOptionUri": "urn:replicon:schedule-modification-option:update-schedule-over-date-range",
                "replacementSchedule": [],
                "updateScheduleOverDateRange": {
    ...
    # Timesheet period update with IA=1 NONE fallback
    timesheet_period_name = dag_run.conf['mapper_data']['timesheet_period']
    should_update_timesheet = bool(timesheet_period_name) or _is_international_assignee(is_ia)

    if should_update_timesheet and _get_user_details['timesheetPeriodSchedule']:
        ...
        if timesheet_period_name and (not current_timesheet_period or current_timesheet_period['timesheetPeriod']['displayText'] != timesheet_period_name):
    ...
    # Department schedule update
    if dag_run.conf['file_data']['org_code'] and (dag_run.conf['file_data']['org_code'] != current_effective_grps['department'].get('displayText', '')):
        payload['modifications']['departmentGroupScheduleToApply'] = _build_group_schedule_payload(
            schedule_type="departmentGroup",
            group_key="departmentGroup",
            uri=dag_run.conf['groups']['department'].get('uri'),
            effective_date=effective_date if effective_date else today,
            parent_key="parent",
            include_param_id=True
        )
    ...
    if dag_run.conf['groups']['location'] and (dag_run.conf['groups']['location'].get('uri' ,'') != current_effective_grps['location'].get('uri' ,'')):
        payload['modifications']['locationScheduleToApply'] = _build_group_schedule_payload(
            schedule_type="location",
            group_key="location",
            uri=dag_run.conf['groups']['location'].get('uri'),
            effective_date=effective_date if effective_date else dag_run.conf['json_formatted_dates']['location_effective_date'],
            parent_key="parentUri",
            include_param_id=False
        )

    # Cost center schedule update
    if dag_run.conf['file_data']['cost_center'] and (dag_run.conf['file_data']['cost_center'] != current_effective_grps['costCenter'].get('displayText', '')):
        payload['modifications']['costCenterScheduleToApply'] = _build_group_schedule_payload(
            schedule_type="costCenter",
            group_key="costCenter",
            uri=dag_run.conf['groups']['cost_center'].get('uri' ,''),
            effective_date=effective_date if effective_date else dag_run.conf['json_formatted_dates']['cost_center_effective_date'],
            parent_key="parentUri",
            include_param_id=False
        )

    # Division schedule update
    if dag_run.conf['groups']['division'].get('uri' ,'') and (dag_run.conf['groups']['division'].get('uri' ,'') != current_effective_grps['division'].get('uri' ,'')):
        division_effective_date = effective_date if effective_date else (
            dag_run.conf['json_formatted_dates']['job_change_effective_date'] if dag_run.conf['file_data']['job_change_effective_date']
            else dag_run.conf['json_formatted_dates']['cost_center_effective_date']
        )
        payload['modifications']['divisionScheduleToApply'] = _build_group_schedule_payload(
            schedule_type="division",
            group_key="division",
            uri=dag_run.conf['groups']['division'].get('uri' ,''),
            effective_date=division_effective_date,
            parent_key="parentUri",
            include_param_id=False
        )

    # Employee type schedule update
    employee_type_uri = dag_run.conf['groups']['employee_type']['exempt_salaried'] if dag_run.conf['groups']['employee_type']['is_exempt'] else dag_run.conf['groups']['employee_type']['non_exempt_hourly']
    if dag_run.conf['groups']['employee_type'] and (current_effective_grps['employeeType'].get('uri', '') != employee_type_uri.get('uri', '')):
        if employee_type_uri.get('uri', ''):
            payload['modifications']['employeeTypeGroupScheduleToApply'] = _build_group_schedule_payload(
                schedule_type="employeeTypeGroup",
                group_key="employeeTypeGroup",
                uri=employee_type_uri['uri'],
                effective_date=effective_date if effective_date else dag_run.conf['json_formatted_dates']['employee_type_effective_date'],
                parent_key="parent",
                include_param_id=True
            )
    ...
    if dag_run.conf['file_data']['pay_group'] and (dag_run.conf['file_data']['pay_group'] != current_effective_grps['serviceCenter'].get('displayText', '')):
        payload['modifications']['serviceCenterScheduleToApply'] = _build_group_schedule_payload(
            schedule_type="serviceCenter",
            group_key="serviceCenter",
            uri=null,
            effective_date=effective_date if effective_date else today,
            parent_key="parentUri",
            name=dag_run.conf['file_data']['pay_group'],
