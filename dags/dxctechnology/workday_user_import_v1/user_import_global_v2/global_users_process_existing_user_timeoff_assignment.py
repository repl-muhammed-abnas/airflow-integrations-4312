...
from dxctechnology.workday_user_import_v1.user_import_global_v2.utils.request_payload import get_update_timeoff_policies_payload_update_user, get_timeoff_assignment_payload_for_update_user
from dxctechnology.workday_user_import_v1.user_import_global_v2.utils.custom_methods import map_mapper_replicon_timeoffs_update_user
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import get_date_to_use_for_no_accrual, convert_json_date_to_date, get_json_date_from_date

# Non- Canada
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_global_v2_users_update_user_timeoff_process_child_dag,
        ...
        def get_ia_zero_timeoff_conf(dag_run):
            ia_end_date = dag_run.conf['ia_end_date']
            # Calculate ia_end_date + 1 for the new policy effective date
            ia_end_date_plus_one = get_json_date_from_date(
                convert_json_date_to_date(ia_end_date) + timedelta(days=1)
            )
            return {
                "file_name": dag_run.conf['file_name'],
                "login_name": dag_run.conf['loginName'],
                ...
                "ia_end_date": ia_end_date,
                "ia_end_date_plus_one": ia_end_date_plus_one,  # New policy starts on ia_end_date + 1
                "country": dag_run.conf['country'],
                "personnel_subarea": "",
                "employee_group":"",
                "employee_subgroup": "",
                ...
                "json_formatted_dates": {
                    ...
                    "ia_end_date": ia_end_date,
                    "ia_end_date_plus_one": ia_end_date_plus_one  # Effective date for new policy
                }
            }
