...
def create_add_user_dag(config):
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.workday_user_import_uki_csc_add_user_dag}{prefix}",
            description="DXC Technology Workday User Sync UK&I CSC Add User",
            company_key=config.company_key,
            replicon_conn_id=config.replicon_conn_id,
            start_date=pendulum.datetime(2025, 4, 1),
            max_active_runs=config.max_active_run_add_user_uki_csc
        ) as dag:
            ...
            can_run_batch_task = rail.IfOperator(
                ...
                test=lambda: Variable.get(
                    config.can_run_batch_task_var_name_uki_csc, default_var='false').lower() == 'true',
                yes_task="batch_task",
                no_task="create_user"
            )
            ...
            create_user = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run: request_payload.create_user_payload_uki_csc(dag_run, config)
            )
            ...
            remove_timeoffs = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run: request_payload.get_timeoff_to_assign_remove_payload_uki_csc(dag_run, mode="hard-remove")
            )
            ...
            update_product_assignment = rail.RepliconServiceOperator(
                ...
                data=request_payload.get_product_assignment_payload_uki_csc
            )
            ...
            update_time_entry_path = rail.RepliconServiceOperator(
                ...
                data=request_payload.update_time_entry_path_payload_uki_csc
            )
            ...
            timeoff_to_assign = rail.PythonOperator(
                ...
                python_callable=custom_methods.timeoff_to_assign_uki_csc
            )
            ...
            assign_timeoff_to_user = rail.RepliconServiceOperator(
                ...
                endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
                data=lambda dag_run: request_payload.get_timeoff_to_assign_remove_payload_uki_csc(dag_run, mode="assign")
            )
            ...
            process_new_user_timeoff_assignment = rail.TriggerDagRunForEachItemOperator(
                ...
                trigger_dag_id=lambda dag_run: custom_methods.get_trigger_dag_id(
                    config.workday_user_import_uki_csc_add_user_timeoff_assignment_dag,
            ...
            reassign_timeoff_to_user = rail.RepliconServiceOperator(
                ...
                endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
                data=lambda dag_run: request_payload.get_timeoff_to_assign_remove_payload_uki_csc(dag_run, mode="soft-remove")
            )
