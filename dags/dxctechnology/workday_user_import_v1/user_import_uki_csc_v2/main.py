...
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.tasks.get_all_required_fields import get_all_required_fields
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.custom_methods import get_process_uki_csc_user_data_config, cached_write_json_artifact, get_all_run_ids_callable, get_trigger_dag_id, get_item_index

def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_process_uki_csc_data_child_dag,
        ...
        create_uki_csc_user_data_collection = rail.CreateCollectionOperator(
            task_id="create_uki_csc_user_data_collection",
            ...
            columns={
                ...
                # NEW: Column 68 - Default Weekly Hours OEF for overtime users
        ...
        get_valid_uki_csc_data = rail.QueryCollectionOperator(
            task_id="get_valid_uki_csc_data",
            query="""SELECT *, ROW_NUMBER() OVER(ORDER BY ROWID) as user_record_index FROM raw_user_data rd WHERE
                    LOWER(rd.companycode) IN ('0201', '0290', '0250', '0237', '1627', '1628', '1629', '1630', '1631', '1639')""",
            name="valid_uki_csc_data"
        )

        has_any_valid_data = rail.IfOperator(
            task_id = "has_any_valid_data",
            test=lambda: rail.result(get_valid_uki_csc_data.task_id, 'length') > 0,
        ...
        process_users = rail.trigger_parallel_dagrun(
            ...
            items="{{result('get_valid_uki_csc_data')}}",
            trigger_dag_id=lambda item: get_trigger_dag_id(config.workday_user_import_process_uki_csc_user_records_child_dag,
            ...
            conf=lambda dag_run, item: get_process_uki_csc_user_data_config(dag_run, item, config)
        )
        ...
        generate_logs = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.workday_user_import_uki_csc_log_generation_dag,
            wait_for_completion=True,
            poke_interval=5,
            conf=lambda: {
                "file_name": path.split(rail.result("new_file_sensor"))[1],
                'logs': rail.result("gather_all_logs"),
                "total_record_count": rail.result("create_uki_csc_user_data_collection", "length") if rail.result("create_uki_csc_user_data_collection", "length") else 0,
                "location": "UK&I CSC",
                "location_code": "UKI_CSC",
                **get_log_details()
            }
        )
        ...
        load_data >> create_supervisor_log >> create_uki_csc_user_data_collection >> get_data_start
        get_data_end >> get_valid_uki_csc_data >> has_any_valid_data
