...
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.constants import (
...
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.date_utils import (
    parse_date,
    build_json_formatted_dates
)
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.request_payload import get_todays_date_in_json, convert_json_date_to_date, get_replicon_date, INPUT_DATE_FORMAT

from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.region_fields_config import get_excluded_fields
...
def get_activities_to_assign(item, config):
    return list(map(lambda filtered_activity: filtered_activity['activity_name'] ,filter(lambda activity: activity['company_code'] == item['companycode'], config.ACTIVITY_MAPPER)))

def get_process_uki_csc_user_data_config(dag_run, item, config):
    # Get mapper data for the user
    # from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.mapper import MASTER_MAPPER
    ...
    return {
        ...
        "activities": {
            "activity_list" : get_activities_to_assign(item, config)
        },
        "user_policies": {
            "punch_entry_policy": {},
            "timeoff_template": {
...
def get_mapper_derived_data(item, config):
    from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.mapper.mapper import get_user_assignments
    ...
    assignment = get_user_assignments(user_data, config.CSC_ASSIGNMENT_MAPPER)
    if assignment:
...
# this is added as workday_user_import_v1 is not in proudction yet
...
def get_excluded_oef_clear_payloads(region: str, current_oef_values: list) -> list:
    from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.region_fields_config import _OEF_CONFIG
