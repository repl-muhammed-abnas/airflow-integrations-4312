...
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.date_utils import _get_effective_date_based_on_work_week
from airflow.exceptions import AirflowException
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.region_fields_config import DXC_UKI, DXC_UKI_C1
...
# UDF fields that are NOT applicable to UKI region
...
def _get_udf_fields_to_clear_for_ia(dag_run, current_custom_fields_values, exception_log):
    ...
    # Check each excluded UDF and add clear payload if it has a value
    for udf_display_text in UKI_EXCLUDED_UDFS:
        # Find if this UDF has a current value
        for cf in current_custom_fields_values:
            if cf.get('customField', {}).get('displayText') == udf_display_text:
                # Check if field has a value (text or date)
                has_value = cf.get('text') or cf.get('date')
                if has_value:
                    exception_log.append(f"Clearing UDF '{udf_display_text}' for IA=1 user (not applicable to UKI region)")
                    # Add payload to clear the field (set text to empty string)
...
def _update_custom_fields_for_user(dag_run, current_user_groups, exception_log):
    ...
    _update_txt_udf(dag_run, 'additional_job_classifications', 'Additional Job Classifications', 'additional_job_classifications', custom_fields_payload, current_custom_fields_values)
    ...
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
        from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.custom_methods import get_excluded_udf_clear_payloads
        excluded_udf_payloads = get_excluded_udf_clear_payloads(
            region=DXC_UKI_C1 if dag_run.conf['file_data']['parent_company'].lower() not in ['compass', 'ftp'] else DXC_UKI,
...
def _get_object_extension_fields_to_update_payload(dag_run, current_oef_fields_values, exception_log):
    ...
    # Existing: Additional Job Classifications
    ...
    # For IA=1 users without value in mapper, clear if currently has value
    elif _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
        current_value = rail.find_first_by_attr_and_get_attr(
            current_oef_fields_values, "definition.displayText", "Additional Job Classifications", 'textValue', default="")
        if current_value:
            exception_log.append("Clearing OEF 'Additional Job Classifications' for IA=1 user (no value in mapper)")
            updated_oef_payload.append(
                generate_oef_payload(
                    dag_run.conf['oefs']['additional_job_classifications']['uri'],
                    text_value=""
    ...
    # Existing: Employee Representative Effective Date
    ...
    # For IA=1 users without value in mapper, clear if currently has value
        ...
        current_value = rail.find_first_by_attr_and_get_attr(
            current_oef_fields_values, "definition.displayText", "Employee Representative Effective Date", 'textValue', default="")
        if current_value:
            exception_log.append("Clearing OEF 'Employee Representative Effective Date' for IA=1 user (no value in mapper)")
            updated_oef_payload.append(
                generate_oef_payload(
                    dag_run.conf['oefs']['employee_representative_effective_date']['uri'],
                    text_value=""
    ...
    # Existing: Employee Representative Status
    ...
    # For IA=1 users without value in mapper, clear if currently has value
        ...
        current_value = rail.find_first_by_attr_and_get_attr(
            current_oef_fields_values, "definition.displayText", "Employee Representative Status", 'tag.displayText', default="")
        if current_value:
            exception_log.append("Clearing OEF 'Employee Representative Status' for IA=1 user (no value in mapper)")
            # Clear by setting tag_uri to None
            updated_oef_payload.append({
                "definition": {
                    "uri": dag_run.conf['oefs']['employee_representative_status']['uri'],
                    "displayText": null
                },
                "tag": null,
                "textValue": null,
    ...
    # For IA=1 users without value in mapper, clear if currently has value
        ...
        current_value = rail.find_first_by_attr_and_get_attr(
            current_oef_fields_values, "definition.displayText", "Default Weekly Hours", 'textValue', default="")
        if current_value:
            exception_log.append("Clearing OEF 'Default Weekly Hours' for IA=1 user (no value in mapper)")
            updated_oef_payload.append(
                generate_oef_payload(
                    dag_run.conf['oefs']['default_weekly_hours']['uri'],
                    text_value=""
    ...
    return updated_oef_payload if updated_oef_payload else null

def get_update_user_payload_uki_csc(dag_run, config):
    exceptions = []
    ...
    current_oef_values = user_details['userDetails']['extensionFieldValues']
    custom_fields, effective_date = _update_custom_fields_for_user(dag_run, current_user_groups, exceptions)
    payload = {
        "user": {
        ...
        "modifications": {
            ...
            "objectExtensionFieldsToApply": _get_object_extension_fields_to_update_payload(dag_run, current_oef_values, exceptions),
    ...
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
        if current_oef_values:
            from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.custom_methods import get_excluded_oef_clear_payloads
...
def get_update_custom_fields_payload_uki_csc(dag_run):
...
def get_notification_preference_to_assign_payload_uki_csc(dag_run, mode="update"):
    return get_notification_preference_to_assign_payload(dag_run, mode)

def get_product_assignment_payload_uki_csc(dag_run):
...
def get_user_end_date_update_payload_uki_csc(dag_run):
...
def get_timeoff_to_assign_remove_payload_uki_csc(dag_run, mode="assign"): # pylint: disable=unused-argument
    """
...
def get_default_timeoff_policy_payload_uki_csc(dag_run):
...
def get_put_user_timeoff_policy_set_payload_uki_csc(dag_run):
...
def get_update_timesheet_template_update_payload_uki_csc(dag_run, caller):
    ...
    work_week_eff_date = convert_json_date_to_date(json_formatted_dates.get('work_week'))
...
def create_user_payload_uki_csc(dag_run, config):
    exception_log = []
    payload = {
        "user": {
...
__all__ = [
    'get_update_user_payload_uki_csc',
    'get_update_custom_fields_payload_uki_csc',
    'get_notification_preference_to_assign_payload',
    'get_notification_preference_to_assign_payload_uki_csc',
    'get_product_assignment_payload_uki_csc',
    'get_user_end_date_update_payload_uki_csc',
    'get_timeoff_to_assign_remove_payload_uki_csc',
    'get_default_timeoff_policy_payload_uki_csc',
    'get_put_user_timeoff_policy_set_payload_uki_csc',
    'get_update_timesheet_template_update_payload_uki_csc',
    'get_add_user_payload_uki_csc',
    'get_todays_date_in_json',
    'get_todays_date_for_timezone_in_json'
]


def update_time_entry_path_payload_uki_csc(dag_run):
    return {
        "user": {
...
def update_user_start_date_remove_end_date_uki_csc(dag_run):
    return {
        "userUri": dag_run.conf['user_uri'],
        "dateRange": {
            "startDate": dag_run.conf['json_formatted_dates']['hire_date'],
