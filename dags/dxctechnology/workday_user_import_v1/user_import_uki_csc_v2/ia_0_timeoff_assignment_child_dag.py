...
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.custom_methods import convert_json_date_to_date, get_tenure_value, get_marital_status_effective_date_if_applicable  
...
def create_user_timeoff_assignment_dag(config):
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.workday_user_import_uki_csc_ia_zero_timeoff_assignment_child_dag}{prefix}",
            ...
            def get_payload_to_process(dag_run):
                ...
                ia_end_date = convert_json_date_to_date(dag_run.conf['json_formatted_dates']['ia_end_date']) + timedelta(days=1)
                ...
                for policy in policies_from_caller:
                    if convert_json_date_to_date( policy['effectiveDate']) < (ia_end_date + timedelta(days=1)):
                ...
                for each_policy in policy_sets:
                    if each_policy['first'] == "Yes":
                        ...
                        # Check if we should use marital status effective date
                        marital_status_date = get_marital_status_effective_date_if_applicable(timeoff_name, dag_run)
                        if marital_status_date:
                            # Convert date dict to datetime object
                            effective_date = datetime(marital_status_date['year'], marital_status_date['month'], marital_status_date['day'])
