...
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils import request_payload, custom_methods
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.custom_methods import (get_effective_grp_membership_data_handler)
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.utils.request_payload import (get_todays_date_for_timezone_in_json)

from dxctechnology.workday_user_import_v1.user_import_uki_csc_v2.tasks.supervisor_assignment import assign_supervisor
...
def create_update_user_dag(config):
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            dag_id = f"{config.workday_user_import_uki_csc_update_user_dag}{prefix}",
            description = "DXC Technology Workday User Sync UK&I CSC Update User",
            company_key=config.company_key,
            replicon_conn_id=config.replicon_conn_id,
            max_active_runs = config.max_active_run_update_user_uki_csc
        ) as dag:
            ...
            can_run_batch_task = rail.IfOperator(
                ...
                test=lambda: Variable.get(
                    config.can_run_batch_task_var_name_uki_csc, default_var='true').lower() == 'true',
                yes_task="batch_task",
            ...
            has_no_direct_reports_for_user = rail.IfOperator(
                ...
                yes_task="is_division_uki_csc",
                no_task="is_user_already_disabled_41"
            )

            # UK&I CSC specific: Check if division is CSC (GSAP-C1 or FTP)
            is_division_uki_csc = rail.IfOperator(
                task_id = "is_division_uki_csc",
                test=custom_methods.is_division_uki_csc_test,
            ...
            update_user_end_date_15 = rail.RepliconServiceOperator(
                ...
                data=request_payload.get_user_end_date_update_payload_uki_csc
            )
            ...
            trigger_process_timeoff_policies_no_accrual_19 = rail.TriggerDagRunForEachItemOperator(
                ...
                trigger_dag_id=lambda dag_run: custom_methods.get_trigger_dag_id(
                    config.workday_user_import_uki_csc_process_time_off_accrual_dag,
                ...
                conf=lambda item, dag_run: {
                    ...
                    "user_end_date_json_proration": dag_run.conf['json_formatted_dates']['term_date'],
                    ...
                    "user_start_date_json": dag_run.conf['json_formatted_dates']['hire_date']
                }
            )
            ...
            log_disabled_user_20 =  rail.WriteLogOperator(
                ...
                properties=lambda dag_run:{
                    ...
                    "Details": '''User disabled in Replicon as the required user's company code and country not in allowed status.
                                User's company code is UK&I CSC (GSAP-C1/FTP). User has an end date in the feed file'''
                }
            )
            ...
            user_does_not_have_admin_and_payroll_permission = rail.IfOperator(
                ...
                test=custom_methods.user_does_not_have_admin_and_payroll_permission_test_uki_csc,
            ...
            can_update_user_end_date_25 = rail.IfOperator(
                ...
                test=custom_methods.can_update_user_end_date_test_uki_csc,
            ...
            log_disabled_user_30 =  rail.WriteLogOperator(
                ...
                properties=lambda dag_run:{
                    ...
                    "Status": "Success",
                    "Details": '''User disabled in Replicon as the required user's company code and country not in allowed status.
                                User's company code is UK&I CSC (GSAP-C1/FTP). User does not have payroll or admin permission'''
                }
            )
            ...
            is_user_already_disabled_41 = rail.IfOperator(
                ...
                test=custom_methods.is_user_already_disabled_test_uki_csc,
            ...
            is_user_rehire = rail.IfOperator(
                ...
                test=custom_methods.is_user_rehire_test_uki_csc,
            ...
            update_user_start_date_48 = rail.RepliconServiceOperator(
                ...
                endpoint="/services/UserService1.svc/UpdateEmploymentDateRange",
                data=request_payload.update_user_start_date_remove_end_date_uki_csc
            )

            can_update_user_start_date = rail.IfOperator(
                task_id = "can_update_user_start_date",
                test= custom_methods.can_update_user_start_date_test_uki_csc,
            ...
            should_disable_user = rail.IfOperator(
                ...
                test=custom_methods.should_disabled_user_test_uki_csc,
            ...
            is_end_date_less_than_today = rail.IfOperator(
                ...
                test=custom_methods.is_end_date_less_than_today_test_uki_csc,
            ...
            trigger_process_timeoff_policies_no_accrual_58 = rail.TriggerDagRunForEachItemOperator(
                ...
                conf=lambda item, dag_run: {
                    ...
                    "timeoff_type_name": item['timeOffType']["name"],
                    "policy_set": dumps(item['policySetSchedule']).replace("[[{", "[{").replace("}]]", "}]"),
                    "end_date": request_payload.convert_json_date_to_string_date(custom_methods.get_specified_json_date_minus_specified_days_months_years_date_in_json(dag_run.conf['json_formatted_dates']['term_date'], days_in_number=-1)),
            ...
            prepare_update_payload = rail.PythonOperator(
                ...
                python_callable=lambda dag_run: request_payload.get_update_user_payload_uki_csc(dag_run, config)
            )
            ...
            can_update_timesheet_template = rail.IfOperator(
                ...
                no_task="can_update_notification_settings"
            )
            ...
            update_timesheet_template_with_effective_date = rail.RepliconServiceOperator(
                ...
                endpoint = "/services/ImportService1.svc/ApplyUserModifications3",
                data = lambda dag_run: request_payload.get_update_timesheet_template_update_payload_uki_csc(dag_run, "update")
            )
            ...
            update_notification_preference = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run: request_payload.get_notification_preference_to_assign_payload_uki_csc(dag_run, "update")
            )

            can_remove_timesheet_template = rail.IfOperator(
                task_id = "can_remove_timesheet_template",
                test = lambda : rail.result("get_user_details")['timesheetTemplate'] and rail.result("get_user_details")['timesheetTemplate'].get('name'),
                        ...
                        "policySetsScheduleToApply": [
                            {
                                ...
                                "schedule": [
                                    {
                                        "policySetUri": null,
                                        "effectiveDate": dag_run.conf['json_formatted_dates']['work_week']
                                    }
                                ],
                                "policySetSchedule": []
                            }
            ...
            trigger_rehire_timeoff_assignment = rail.TriggerDagRunForEachItemOperator(
                ...
                trigger_dag_id=lambda dag_run: custom_methods.get_trigger_dag_id(
                    config.workday_user_import_uki_csc_user_rehire_timeoff_process_dag,
                    config.DAG_BATCH_COUNT,
                    item_index=(custom_methods.get_item_index(dag_run, config.DAG_BATCH_COUNT))
                ),
            ...
            def timeoff_assignment_data(dag_run):
                mapper_timeoff_details_to_disable_after_assignment = dag_run.conf['mapper_data']['timeoff_to_assign_and_disable']
                ...
                timeoff_list =  list(map(lambda timeoff: {
                        ...
                        "uri": rail.find_first_by_attr_and_get_attr(
                            replicon_timeoffs, 'name', timeoff.strip(), 'uri'),
                        "mapper_data": timeoff
                    }, mapper_timeoffs))
                ...
                timeoff_to_disable_after_assignment = [ timeoff for timeoff in timeoff_list if timeoff['name'] not in mapper_timeoff_details_to_disable_after_assignment]
                return {
            ...
            def get_required_details_callable(dag_run):
                _timeoff_assignment_data = timeoff_assignment_data(dag_run)
                ...
                # If a timeoff is in the mapper's "assign and disable" list but is
                # currently enabled in Replicon (manually enabled by HR/payroll),
                # preserve it by adding it to the reassignment list.
                mapper_disable_timeoffs = dag_run.conf['mapper_data']['timeoff_to_assign_and_disable']
                for timeoff in _timeoff_assignment_data['timeoff_list_mapped_as_per_replicon']:
                    if timeoff['name'] in mapper_disable_timeoffs:
                        is_currently_enabled = rail.find_first_by_attr_and_get_attr(
                            current_assigned_timeoffs, 'uri', timeoff['uri'], 'enabled'
                        )
                        if is_currently_enabled == True:
                            timeoff_to_disable_after_assignment.append(timeoff)

                _timeoff_assignment_data['timeoff_to_disable_after_assignment'] = timeoff_to_disable_after_assignment
                ...
                # TODO:
                # Add enumerate and add index to timeoff
                # and update the timeoff processing dag
                return {
                    "final_time_off_list": final_timeoff_list,
                    "timeoffs_to_assign": [timeoff for timeoff in timeoffs_to_assign if timeoff['uri']],
                    "timeoffs_to_disable": [timeoff for timeoff in timeoffs_to_disable if timeoff['uri']],
                    "current_assigned_timeoffs": current_assigned_timeoffs
                }
            ...
            assign_timeoff_to_user = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run :{
                    "userUri": dag_run.conf['user_uri'],
                    "timeOffTypeUris":(([to['uri'] for to in rail.result("get_required_timeoff_type_details")['timeoffs_to_assign']]) if rail.result("get_required_timeoff_type_details")['timeoffs_to_assign'] else ([to['uri'] for to in rail.result("get_required_timeoff_type_details")['current_assigned_timeoffs']]))
                }
            )
            ...
            process_timeoff_no_accrual = rail.TriggerDagRunForEachItemOperator(
                ...
                trigger_dag_id=lambda dag_run: custom_methods.get_trigger_dag_id(
                    ...
                    **{
                        ...
                        "user_end_date_json": request_payload.date_to_use_for_disable(dag_run, return_as_json_date=True),
                        "user_end_date_json_proration": dag_run.conf['json_formatted_dates']['term_date'],
                        "end_date": request_payload.date_to_use_for_disable(dag_run, return_as_json_date=False),
                        "start_date": dag_run.conf['file_data']['hire_date'],
                        "user_start_date_json": dag_run.conf['json_formatted_dates']['hire_date']
                    }
                },
            ...
            process_timeoffs = rail.TriggerDagRunForEachItemOperator(
                    ...
                    trigger_dag_id=lambda dag_run, item: custom_methods.get_trigger_dag_id(
                        config.workday_user_import_uki_csc_update_user_timeoff_assignment_dag,
                        config.DAG_BATCH_COUNT,
                        item_index=(custom_methods.get_item_index(dag_run, config.DAG_BATCH_COUNT))
                    ),
                    conf=lambda dag_run, item: {
                    **{
                    "feed_file_name": dag_run.conf["file_name"],
                        ...
                        "effective_date":  dag_run.conf['file_data'].get("employee_representative_effective_date", ""),
            ...
            reassign_timeoff_to_user = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run :{
                    "userUri": dag_run.conf['user_uri'],
                    "timeOffTypeUris": [timeoff['uri'] for timeoff in rail.result("get_required_timeoff_type_details", "timeoff_assignment_data")['timeoff_to_disable_after_assignment'] if timeoff['uri']]
                }
            )
            ...
            should_trigger_delete_time_and_timeoff = rail.IfOperator(
                ...
                test=custom_methods.should_trigger_delete_time_and_timeoff_for_disabled_user,
            ...
            has_no_direct_reports_for_user >> rail.Label('Yes') >> is_division_uki_csc

            is_division_uki_csc >> rail.Label('Yes') >> is_termination_date_present >> rail.Label('Yes') >> update_user_end_date_15
            ...
            is_division_uki_csc >> rail.Label('No') >> disable_user_login_33 >> can_update_user_end_date_34
            ...
            current_assigned_udf_values >> get_time_entry_approval_path_name >> get_user_assigned_policy  >> prepare_update_payload >> can_process_user >> rail.Label("Yes") >> update_user >> can_update_timesheet_template\
                >> rail.Label("No") >> can_update_notification_settings
            can_update_timesheet_template >> rail.Label("Yes") >> update_timesheet_template_with_effective_date >> can_update_notification_settings >> rail.Label("Yes") >> update_notification_preference
            can_process_user >> rail.Label("No") >> log_ia_exception >> should_trigger_delete_time_and_timeoff

            update_notification_preference >> can_remove_timesheet_template >> rail.Label("Yes") >> remove_timesheet_template >> dummy_process_supervisor
            can_remove_timesheet_template >> rail.Label("No") >> dummy_process_supervisor
            can_update_notification_settings >> rail.Label("No") >> dummy_process_supervisor
