...
import rail
from dxctechnology.workday_user_import_v1.disable_user_v2.utils.data_handler import get_user_timeoff_type_policy_summary_data_handler
from dxctechnology.workday_user_import_v1.disable_user_v2.utils.request_payload import REPORT_DATE_FORMAT
from dxctechnology.workday_user_import_v1.disable_user_v2.utils.custom_methods import INPUT_DATE_FORMAT
    ...
    for idx in range(config.process_disable_user_dag_count):
        ...
        with rail.create_airflow_dag(
            ...
            validate_employee_type = rail.IfOperator(
                ...
                yes_task="if_country_allowed"
            )

            if_country_allowed = rail.IfOperator(
                task_id="if_country_allowed",
                test=lambda dag_run: "ALL" in config.allowed_countries or dag_run.conf.get('country') in config.allowed_countries,
            ...
            validate_employee_type >> rail.Label("Yes") >> if_country_allowed

            if_country_allowed >> rail.Label("Yes") >> trigger_delete_future_entries
            if_country_allowed >> rail.Label("No") >> disable_user_login

            # Main workflow - trigger deletion child DAG then proceed with disable user flow
            trigger_delete_future_entries >> wait_for_trigger_delete_future_entries >> disable_user_login \
