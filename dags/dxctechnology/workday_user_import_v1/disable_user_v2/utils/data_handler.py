...
def get_time_entry_response_data(response):
    if not response:
        return []

    return [
        {
            "timeEntryUri": find_first_by_attr_and_get_attr(
                row['cells'], 'dataType', 'urn:replicon:list-type:object', 'uri'
            ),
            "timesheetUri": find_first_by_attr_and_get_attr(
                row['cells'], 'objectType', 'urn:replicon:object-type:timesheet', 'uri', None
            ),
            "entryDate": find_first_by_attr_and_get_attr(
                row['cells'], 'dataType', 'urn:replicon:list-type:date', 'textValue', None
            ),
            "status": find_first_by_attr_and_get_attr(
                row['cells'], 'objectType', 'urn:replicon:object-type:time-entry-status', 'textValue', None
            )
        } for row in response['rows']
    ] if response['rows'] else []

def get_time_punch_response_data(response):
    if not response:
        return []

    return [
        {
            "timePunchUri": find_first_by_attr_and_get_attr(
                row['cells'], 'objectType', 'urn:replicon:object-type:time-punch', 'uri'
            )
        } for row in response['rows']
    ] if response['rows'] else []
