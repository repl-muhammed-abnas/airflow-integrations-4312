...
from dxctechnology.workday_user_import_v1.disable_user_v2.utils.custom_methods\
    import get_timeoff_polices_to_assign_callable, format_timeoff_polices_to_assign_callable, convert_json_date_to_date, get_annual_brought_sold_holiday_leave_list

from dxctechnology.workday_user_import_v1.disable_user_v2.utils.request_payload \
    import get_update_policy_payload, get_user_timeoff_balance_summary_payload
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_time_off_accrual,
        ...
        if_country_is_uk_or_ireland = rail.IfOperator(
            ...
            test = lambda dag_run: dag_run.conf['country'].lower() in ['united kingdom', 'ireland'],
        ...
        is_timeoff_bought_or_sold = rail.IfOperator(
            ...
            test=lambda dag_run: dag_run.conf['timeoff_type_name'] in ['[UK] Bought A/L', '[UK] P/T Bought A/L Hrs', '[IRL] Bought A/L', '[IRL] P/T Bought A/L Hrs', '[IRL] P/T Sold A/L Hrs', '[IRL] Sold A/L', '[UK] P/T Sold A/L Hrs', '[UK] Sold A/L', '[UK] P/T Public Holiday Hrs', '[IRL] P/T Public Holiday Hrs'],
        ...
        def get_annual_bought_sold_leave_policy_summary_data_handler(response, dag_run):
            ...
            is_uk, is_irl = dag_run.conf['country'].lower() == "united kingdom", dag_run.conf['country'].lower() == "ireland"

            if not is_uk and not is_irl:
                raise AirflowException("Action should not be performed, as its only for UK and IRL")

            is_user_part_time = dag_run.conf['fte_pct'] != '100'
            ...
            get_holidays_timeoff_hours = 0
            if is_uk:
                if is_user_part_time:
                    get_holidays_timeoff_hours = res.get('[UK] P/T Public Holiday Hrs', 0)
                else:
                    get_holidays_timeoff_hours = 0
            else:
                if is_user_part_time:
                    get_holidays_timeoff_hours = res.get('[IRL] P/T Public Holiday Hrs', 0)
                else:
                    get_holidays_timeoff_hours = 0
            return  {
                "res": res,
                ...
                "holidays": get_holidays_timeoff_hours
            }
        ...
        def standard_round(num):
            return float(math.floor(num + 0.5))
        ...
        def calculate_prorated_timeoff(entitlement, termination_date, hire_date=None, leaves_used=0, is_hours=False):
            ...
            # Deduct used leaves
            final_balance = float(prorated) - float(leaves_used)
            # Standard rounding: .0-.4 rounds down, .5-.9 rounds up
            if final_balance > 0:
                final_balance = standard_round(final_balance)
        ...
        def calculate_required_balance(dag_run):
            ...
            _timeoffs = get_annual_brought_sold_holiday_leave_list(
                return_for="country",
                country="UK" if dag_run.conf['country'].lower() == "united kingdom" else "IRL",
                part_time_full_time="parttime" if dag_run.conf['fte_pct'] != '100' else "fulltime",
                return_type="json"
            )
            ANNUAL_TIMEOFF_LIST = [_timeoffs['annual']]
            BOUGHT_TIMEOFF_LIST = [_timeoffs['bought']]
            HOLIDAY_TIMEOFF_LIST = [_timeoffs['holiday']] if _timeoffs.get('holiday') else []

            balance_summary = rail.result("get_annual_bought_sold_leave_balance_summary")

            annual_leave_used, bought_leave_used, holiday_leave_used = 0, 0, 0
            for summary in balance_summary:
                ...
                if summary['account']['timeOffType']['name'] in HOLIDAY_TIMEOFF_LIST:
                    holiday_leave_used = summary['timeTakenForPeriod']
            ...
            # For part-time users, holiday consumed leaves are also subtracted
            leaves_used = annual_leave_used + bought_leave_used + holiday_leave_used
