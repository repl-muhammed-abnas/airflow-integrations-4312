...
from dxctechnology.workday_user_import_v1.disable_user_v2.utils import custom_methods
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        report_has_expected_columns = rail.IfOperator(
            ...
            test="{{ (result('load_timesheet_deletion_report.get_report_result') | load_json_artifact).reportGenerationResults[0].payload | starts_with('%s') }}" % config.expected_report_columns,
        ...
        load_report_data_to_csv = rail.LoadCSVFileOperator(
            ...
            document="{{ (result('load_timesheet_deletion_report.get_report_result') | load_json_artifact).reportGenerationResults[0].payload }}",
        )
        ...
        get_report_details >> load_timesheet_deletion_report >> report_has_data
