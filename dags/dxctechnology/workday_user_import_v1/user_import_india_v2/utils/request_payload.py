...
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import convert_json_date_to_date, get_excluded_udf_clear_payloads, get_excluded_oef_clear_payloads
from dxctechnology.workday_user_import_v1.user_import.common_utils.request_payload import get_todays_date_for_timezone_in_json, get_psa_udf_value, get_psa_user_udf_add_update_payload
from dxctechnology.workday_user_import_v1.user_import.common_utils.region_fields_config import DXC_INDIA
...
def _get_custom_fields_to_assign(dag_run, action):
    ...
    if action == "update":
        ...
        # If user is an International Assignee, clear UDFs that are not applicable to India region
...
def _get_payrule_schedule_to_update(dag_run, _get_user_details):
    payrule = dag_run.conf['mapper_data']['payrule']
    if not payrule and _is_international_assignee(dag_run.conf['file_data']['is_ia']):
        payrule = NONE_DEFAULT_VALUE
    if payrule:
        ...
        if not current_payrule_schedule or current_payrule_schedule['payRuleScript']['displayText'] != payrule:
            if dag_run.conf['file_data']['management_lvl'] not in ['L1', 'L2']:
...
def _get_shift_assignment_to_update(dag_run, user_details, config, exception_log):
    ...
    if dag_run.conf['file_data']['work_shift'] == "ES-Rotational Work shift":
        ...
        if dag_run.conf['file_data']['work_shift']:
            ...
            # For IA users without work_shift from file, assign NONE schedule
            if _is_international_assignee(dag_run.conf['file_data']['is_ia']):
                if current_office_schedule and current_office_schedule.get('officeSchedule', {}).get('displayText') != NONE_DEFAULT_VALUE:
                    return {
                        "userSchedulePolicyScheduleModificationOptionUri": "urn:replicon:schedule-modification-option:update-schedule-over-date-range",
                        "replacementSchedule": [],
                        "updateScheduleOverDateRange": {
                            "replacementScheduleEntries": [
                                {
                                    ...
                                    "effectiveDate": get_replicon_date(dag_run.conf['file_data']['ia_start_date']) if dag_run.conf['file_data']['is_ia'] in ['1',1]
                                        else get_replicon_date(dag_run.conf['file_data']['ia_end_date'])
                                }
                            ],
                            "endDate": null
...
def _get_timesheet_period_schedule_to_apply(dag_run, user_details):
    ...
    if user_details['timesheetPeriodSchedule']:
        ...
        if not dag_run.conf['mapper_data']['timesheet_period']:
            if _is_international_assignee(dag_run.conf['file_data']['is_ia']) and current_timesheet_period['timesheetPeriod']['displayText'] != NONE_DEFAULT_VALUE:
                return {
                    "userTimesheetPeriodScheduleModificationOptionUri": "urn:replicon:schedule-modification-option:update-schedule-over-date-range",
                    "replacementTimesheetPeriodSchedule": [],
                    "updateTimesheetPeriodScheduleOverDateRange": {
                        "replacementTimesheetPeriodScheduleEntries": [
                            {
                                ...
                                "effectiveDate": get_replicon_date(dag_run.conf['file_data']['ia_start_date'])
                            }
                        ],
                        "endDate": null
                    }
...
def _get_department_update_payload(dag_run, current_effective_grps, effective_date = None):
...
def _get_cost_center_update_payload(dag_run, current_effective_grps, effective_date = None):
    ...
    if profile_status_is_enabled:
        ...
        if dag_run.conf['mapper_data']['holiday_calender_name']:
            ...
            if _is_international_assignee(dag_run.conf['file_data']['is_ia']):
                ...
                if not mapper_holiday_calendar_details:
...
def _get_policies_to_update_payload(dag_run, user_details, exception_log, profile_status_is_enabled):
    ...
    if _can_update_timeoff_template(dag_run, user_details, profile_status_is_enabled):
        ...
        if _is_international_assignee(dag_run.conf['file_data']['is_ia']) and not dag_run.conf['mapper_data']['timeoff_template']:
...
def get_update_user_payload(dag_run, config):
    ...
    # If user is an International Assignee, clear OEFs that are not applicable to India region
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
