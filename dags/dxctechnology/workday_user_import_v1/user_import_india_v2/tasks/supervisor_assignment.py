...
from dxctechnology.workday_user_import_v1.user_import_india_v2.utils.custom_methods import is_profile_enabled
...
def assign_supervisor(group_id, caller):
    with rail.TaskGroup(group_id=group_id, prefix_group_id=False):
        ...
        is_supervisor_disabled = rail.IfOperator(
            ...
            test=lambda: not bool(rail.result("get_supervisor_details")["status"]),
        ...
        def get_supervisor_permission_details_data_handler(response, dag_run):
            ...
            return {
                ...
                "schedule_manager_permission": rail.find_first_by_attr_and_get_attr(response,
                    'permissionSet.name', dag_run.conf['mapper_data']['supervisor_schedule_manager_name']),
            }
        ...
        assign_supervisor_permission = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "userUri": rail.result('get_supervisor_details')['user_uri'],
                "permissionSetUri": dag_run.conf['mapper_data']['supervisor_user_permission_uri']
            }
        )
        ...
        assign_end_user_permission = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "userUri": rail.result('get_supervisor_details')['user_uri'],
                "permissionSetUri": dag_run.conf['mapper_data']['supervisor_end_user_permission_uri']
            }
        )
        ...
        def get_data_access_scope_to_assign_callable(dag_run):
            ...
            _employee_type = list(map(lambda employee_type: {
                                    ...
                                    , rail.load_json_artifact(dag_run.conf['employee_type_list'])
                                ))
            _division = list(map(lambda division: {
                ...
                , [item for item in rail.load_json_artifact(dag_run.conf['company_code_list'])
        ...
        log_supervisor_for_reassignment_check = rail.WriteLogOperator(
            ...
            properties = lambda dag_run: {
                ...
                'parent_company': dag_run.conf['mapper_data']['parent_company']
            }
        )
        ...
        get_supervisor_details >> is_supervisor_found >> rail.Label("No") >> log_supervisor_for_reassignment_check  >> finish
        is_supervisor_found >> rail.Label("Yes") >> is_supervisor_company_present >> rail.Label("Yes") >> is_supervisor_disabled
        is_supervisor_company_present >> rail.Label("No") >> log_supervisor_for_reassignment_check  >> finish
        ...
        is_end_user_permission_missing >> rail.Label("Yes") >> assign_end_user_permission >> get_data_access_scope_to_assign
