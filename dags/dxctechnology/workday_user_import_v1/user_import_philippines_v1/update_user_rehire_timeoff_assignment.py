...
from dxctechnology.workday_user_import_v1.user_import_philippines_v1.utils.custom_methods \
    import get_tenure_value
from dxctechnology.workday_user_import_v1.user_import_philippines_v1.utils.custom_methods import convert_json_date_to_date
...
def create_dag(config):
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.workday_user_import_philippines_user_rehire_timeoff_process_dag}{prefix}",
            description=config.workday_user_import_philippines_user_rehire_timeoff_process_dag_description,
            replicon_conn_id=config.replicon_conn_id,
            company_key=config.company_key,
            start_date=datetime(2025, 1, 1),
            max_active_runs=config.max_active_run_rehire_user_timeoff_assignement_philippines
        ) as dag:
            ...
            can_run_batch_task = rail.IfOperator(
                ...
                yes_task="batch_task",
                no_task="view_dagrun_conf"
            )

            batch_task = rail.BatchTaskRunOperator(
                task_id = "batch_task",
                start_task="view_dagrun_conf",
            ...
            def get_policy_to_assign_callable(dag_run):
                ...
                continue_service_date = convert_json_date_to_date(dag_run.conf['json_formatted_dates']['continuous_service_date'])
                ...
                current_timeoff_policies = loads(dag_run.conf['current_timeoff_policies']) if not isinstance(dag_run.conf['current_timeoff_policies'], list) else dag_run.conf['current_timeoff_policies']
                for policy_line in current_timeoff_policies:
                    if convert_json_date_to_date(policy_line['effectiveDate']) < start_date:
            ...
            can_run_batch_task >> rail.Label("No") >> view_dagrun_conf
            view_dagrun_conf >> get_default_policy_for_timeoff >> get_policy_to_assign >> has_policy_to_assign
