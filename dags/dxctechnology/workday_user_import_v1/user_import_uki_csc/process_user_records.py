...
from dxctechnology.workday_user_import_v1.user_import_uki_csc.utils.custom_methods import get_trigger_dag_id, get_item_index
null = None
...
def create_dag(config):
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.workday_user_import_process_uki_csc_user_records_child_dag}{prefix}",
            description="DXC Technology Workday User Sync UK&I CSC Process User Records Child",
            replicon_conn_id=config.replicon_conn_id,
            company_key=config.company_key,
            start_date=datetime(2025, 4, 1),
            max_active_runs=config.max_active_run_process_each_users_uki_csc
        ) as dag:
            ...
            validate_company_code = rail.IfOperator(
                ...
                test=lambda dag_run: dag_run.conf['file_data'].get('company_code', '') in [
                    '0201', '0290', '0250', '0237', '1627', '1628', '1629', '1630', '1631', '1639'
                ],
            ...
            log_invalid_company_code = rail.WriteLogOperator(
                ...
                properties=lambda dag_run: {
                    ...
                    "Action": "Validation",
                    "Status": "Error",
                    "Details": f"Invalid company code: {dag_run.conf['file_data'].get('company_code', 'Unknown')} for UK&I CSC processing"
                }
            )
            ...
            is_user_found = rail.IfOperator(
                ...
                yes_task="trigger_update_uki_csc_user",
                no_task="trigger_add_uki_csc_user"
            )
            ...
            trigger_update_uki_csc_user = rail.TriggerDagRunForEachItemOperator(
                task_id="trigger_update_uki_csc_user",
                items=[1],
                trigger_dag_id=lambda dag_run: get_trigger_dag_id(
                    config.workday_user_import_uki_csc_update_user_dag,
            ...
            wait_for_update_user_completion = rail.WaitForDagRunsSensor(
                ...
                dag_runs="{{result('trigger_update_uki_csc_user')}}",
                retries=0,
            ...
            trigger_add_uki_csc_user = rail.TriggerDagRunForEachItemOperator(
                task_id="trigger_add_uki_csc_user",
                items=[1],
                trigger_dag_id=lambda dag_run: get_trigger_dag_id(
                    config.workday_user_import_uki_csc_add_user_dag,
                    config.DAG_BATCH_COUNT,
                    item_index=get_item_index(dag_run, config.DAG_BATCH_COUNT)
                ),
            ...
            wait_for_add_user_completion = rail.WaitForDagRunsSensor(
                ...
                dag_runs="{{result('trigger_add_uki_csc_user')}}",
                retries=0,
                execution_timeout=timedelta(hours=1)
            )
            ...
            catch_and_log_error = rail.WriteLogOperator(
                ...
                properties=lambda dag_run: {
                    ...
                    "Action": "Process",
                    "Status": "Failed",
                    "Details": "An error occurred during user processing"
                },
            ...
            is_user_found >> trigger_update_uki_csc_user >> wait_for_update_user_completion >> end_task
            is_user_found >> trigger_add_uki_csc_user >> wait_for_add_user_completion >> end_task
