...
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods \
    import convert_json_date_to_date, compare_two_dates, get_work_week_based_effective_date, get_ia_update_payload_for_udf_update, compare_if_two_json_dates_are_same, get_excluded_udf_clear_payloads, get_excluded_oef_clear_payloads
from dxctechnology.workday_user_import_v1.user_import.common_utils.request_payload import get_todays_date_in_json, get_psa_user_udf_add_update_payload, get_psa_udf_value
from dxctechnology.workday_user_import_v1.user_import.common_utils.region_fields_config import DXC_AUSTRALIA
...
def _get_custom_fields_to_assign_for_aus_user(dag_run):
    ...
    if pas_flag:
        udfs_to_assign.append(_add_custom_field_for_aus_user(custom_fields['psa_user']['uri'], drop_down_name=pas_flag))
...
def get_custom_fields_to_update_callable(dag_run):
    ...
    get_psa_udf_value(
        ...
        _get_department_update_payload = lambda dr, ceg, wwed: get_department_update_payload(dr, ceg, wwed, check_only=True),
        caller= "update")

    # If user is an International Assignee, clear UDFs that are not applicable to Australia region
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
        excluded_udf_payloads = get_excluded_udf_clear_payloads(
            region=DXC_AUSTRALIA,
            current_custom_fields_values=current_custom_fields_values,
...
def get_payrule_to_apply(dag_run, user_details, config, effective_date_to_use, exception_log, ia_updated=False):
    ...
    if not mapper_value:
        ...
        if _is_international_assignee(is_ia):
            mapper_value = [{'Value': NONE_DEFAULT_VALUE}]
    ...
    if _is_international_assignee(is_ia) and not payrule_name:
        if not effective_date_to_use:
            effective_date_to_use = dag_run.conf['json_formatted_dates']['ia_start_date']
...
def get_schedule_to_update(dag_run, user_details, exception_log, ia_updated):
    ...
    if dag_run.conf['schedule_data']['schedule_name'] == 'Shift':
        ...
        if dag_run.conf['schedule_data']['schedule_name']:
            ...
            if _is_international_assignee(is_ia = dag_run.conf['file_data']['is_ia']):
                office_schedule_name = current_office_schedule['officeSchedule']['name']
                if ia_updated:
                    return {
                            "userSchedulePolicyScheduleModificationOptionUri": "urn:replicon:schedule-modification-option:update-schedule-over-date-range",
                            "replacementSchedule": [],
                ...
                if office_schedule_name != NONE_DEFAULT_VALUE:
                        return {
                            "userSchedulePolicyScheduleModificationOptionUri": "urn:replicon:schedule-modification-option:update-schedule-over-date-range",
                            "replacementSchedule": [],
                            "updateScheduleOverDateRange": {
...
def get_timesheet_period_to_apply(dag_run, user_details, work_week_effective_date, effective_date_to_use, exception_log, ia_updated=False):
    is_ia = dag_run.conf['file_data'].get('is_ia')
    timesheet_period_name = dag_run.conf['mapper_data']['timesheet_period']
    ...
    if timesheet_period_name:
        ...
        if not current_timesheet_period or current_timesheet_period['timesheetPeriod']['displayText'] != timesheet_period_name:
            return {
...
def get_department_update_payload(dag_run, current_effective_grps, work_week_effective_date, effective_date_to_use=None, check_only=False):
    if dag_run.conf['file_data']['org_code'] and dag_run.conf['file_data']['org_code'] != current_effective_grps['department'].get('displayText', ''):
        if check_only:
            return True
        effective_date = effective_date_to_use if effective_date_to_use else {
            "year": work_week_effective_date.year,
            "month": work_week_effective_date.month,
            "day": work_week_effective_date.day
        }
        return _build_group_schedule_payload(
...
def get_location_update_payload(dag_run, current_effective_grps, effective_date_to_use):
    ...
    if dag_run.conf['groups']['location'] and dag_run.conf['groups']['location'].get('uri' ,'') != current_effective_grps['location'].get('uri' ,''):
        ...
        return _build_group_schedule_payload(
            ...
            effective_date=effective_date_to_use if effective_date_to_use else dag_run.conf['json_formatted_dates']['location_effective_date'],
...
def get_cost_center_update_payload(dag_run, current_effective_grps, effective_date_to_use):
    if dag_run.conf['file_data']['cost_center'] and dag_run.conf['file_data']['cost_center'] != current_effective_grps['costCenter'].get('displayText', ''):
        return _build_group_schedule_payload(
            ...
            effective_date=effective_date_to_use if effective_date_to_use else dag_run.conf['json_formatted_dates']['cost_center_effective_date'],
            parent_key="parentUri",
            include_param_id=False
        )
    return null
...
def get_employee_type_update_payload(dag_run, current_effective_grps, effective_date_to_use):
    if dag_run.conf['groups']['employee_type']['uri'] and dag_run.conf['groups']['employee_type']['uri'] != current_effective_grps['employeeType'].get('uri', ''):
        return _build_group_schedule_payload(
            ...
            uri=dag_run.conf['groups']['employee_type']['uri'],
            effective_date=effective_date_to_use if effective_date_to_use else dag_run.conf['json_formatted_dates']['employee_type_effective_date'],
            parent_key="parent",
            include_param_id=True
        )
    return null
...
def get_division_update_payload(dag_run, current_effective_grps, effective_date_to_use):
    ...
    if dag_run.conf['groups']['division'].get('uri' ,'') and dag_run.conf['groups']['division'].get('uri' ,'') != current_effective_grps['division'].get('uri' ,''):
        ...
        division_effective_date = effective_date_to_use if effective_date_to_use else (
...
def get_timesheetperiod_update_payload(dag_run, user_details, effective_date_to_use, exception_log, ia_updated=False):
    ...
    if user_details['timesheetPeriodSchedule']:
        return get_timesheet_period_to_apply(dag_run, user_details, work_week_effective_date, effective_date_to_use, exception_log, ia_updated)
    if dag_run.conf['mapper_data']['profile_status'].lower() == "enabled" and not user_details['timesheetPeriodSchedule']:
...
def get_assigned_schedule_policy(dag_run):
...
def get_policies_to_update_payload(dag_run, user_details, exception_log, ia_updated, is_ia):
    policies_to_add = []
    policies_to_remove = []
    ...
    if get_assigned_schedule_policy(dag_run) and _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
        policies_to_remove.append(get_assigned_schedule_policy(dag_run))
    ...
    return {
            ...
            } if policies_to_add or policies_to_remove else null
...
def update_user_payload(dag_run, config):
    ...
    if ia_updated:
        effective_date_to_use = rail.result("get_custom_fields_to_update", "effective_date")
    else:
    ...
    payload = {
        ...
        "modifications": {
            ...
            "schedulePolicyToApply": get_schedule_to_update(dag_run, user_details, exception_log, ia_updated),
            ...
            "timesheetPeriodScheduleToApply": get_timesheetperiod_update_payload(dag_run, user_details, effective_date_to_use, exception_log, ia_updated),
            ...
            "policySetsToApply": get_policies_to_update_payload(dag_run, user_details, exception_log, ia_updated, dag_run.conf['file_data']['is_ia']),
            ...
            "payRulesScheduleModifications": get_payrule_to_apply(dag_run, user_details, config, effective_date_to_use, exception_log, ia_updated),
    ...
    # If user is an International Assignee, clear OEFs that are not applicable to Australia region
    if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
