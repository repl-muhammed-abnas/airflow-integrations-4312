...
from dxctechnology.workday_user_import_v1.user_import_australia_v3.utils import request_payload
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        def update_user_permission_payload(dag_run):
            permission_to_add = dag_run.conf['user_permission_sets']['connect_employee'].get('uri')
            if dag_run.conf['connect_employee'] == 'No' and rail.result('get_custom_fields_to_update','permission_change')['update_permission_general'] is True:
                permission_to_add = dag_run.conf['user_permission_sets']['end_user_permission'].get('uri')
            return {
                "userUri" : dag_run.conf['user_uri'],
        ...
        log_user_completion = rail.WriteLogOperator(
            ...
            properties = lambda dag_run: {
                ...
                "Email": dag_run.conf['file_data']['email_id'],
                "Action": "Update",
                "Status": "Success" if not bool(rail.result('update_user', 'exception_log')) else "Exception",
                "Details": get_update_log_message()
            }
        )
        ...
        get_user_details >> get_user_assigned_policy >> get_effective_group_membership >> get_time_entry_approval_path >> get_user_time_off_type_policy_summary >> get_custom_fields_to_update
        ...
        has_any_timeoffs_to_process >> rail.Label("No") >> add_for_supervisor_assignment >> log_user_completion >> should_trigger_delete_time_and_timeoff
        has_any_timeoffs_to_process >> rail.Label("Yes") >> process_timeoff_assignment >> wait_for_process_timeoff_assignment >> add_for_supervisor_assignment >> log_user_completion >> should_trigger_delete_time_and_timeoff
