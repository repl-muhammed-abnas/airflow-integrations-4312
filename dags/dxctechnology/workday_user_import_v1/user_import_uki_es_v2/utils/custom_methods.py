...
from dxctechnology.workday_user_import_v1.user_import_uki_es_v2.utils.constants import (
    LOCATION_DELIMITER, EMPLOYEE_TYPE_DELIMITER, PATH_DELIMITER,
    DEFAULT_TIMEZONE, DEFAULT_TIMEZONE_URI, DEFAULT_WORK_WEEK, DEFAULT_SCHEDULE_TYPE,
    DATA_LOAD_OPTION_URI, USER_STATUS_OPTION_INCLUDE_ALL, PASSWORD_OPTION_USER_SETS,
    POLICY_ADMINISTRATION, POLICY_PAYROLL, POLICY_PROJECT_MANAGEMENT,
    CUSTOM_FIELD_PREFIX, DATE_FORMAT_YYYY_MM_DD, DATE_FORMAT_YYYYMMDD_HHMMSS, TIMEOFF_CONFIG
)

from dxctechnology.workday_user_import_v1.user_import_uki_es_v2.utils.date_utils import (
    parse_date, parse_workday_date, format_date_yyyy_mm_dd, format_workday_date_to_yyyy_mm_dd,
    format_timestamp, get_today_yyyy_mm_dd, get_week_start_date, is_date_in_past, 
    build_json_formatted_dates
)
from dxctechnology.workday_user_import_v1.user_import_uki_es_v2.utils.request_payload import get_todays_date_in_json, convert_json_date_to_date, get_replicon_date, INPUT_DATE_FORMAT
from dxctechnology.workday_user_import_v1.user_import_uki_es_v2.utils.region_fields_config import get_excluded_fields
...
def get_activities_to_assign_from_mapper(ajc, company_code, ACTIVITY_MAPPER_ES):
    activities = list(filter(lambda item: ajc.lower() in lower_list(item["additional_job_classifications"]) and company_code.lower() in lower_list(item["company_code"]), ACTIVITY_MAPPER_ES))
    if activities:
        return activities[0]["activity_types"]
    else:
        return []

def get_process_uki_es_user_data_config(dag_run, item, config):
    # Get mapper data for the user
    # from dxctechnology.workday_user_import_v1.user_import_uki_es_v2.mapper import MASTER_MAPPER
    ...
    activities_to_assign = get_activities_to_assign_from_mapper(item['additionaljobclassifications'], item['companycode'], config.ACTIVITY_MAPPER_ES)
    ...
    return {
        ...
        "activities": {
            "activity_list" : activities_to_assign
        },
        "user_policies": {
            "punch_entry_policy": {},
            "timeoff_template": {
...
def get_mapper_derived_data(item, config):
    from dxctechnology.workday_user_import_v1.user_import_uki_es_v2.mapper.mapper import get_user_assignments
    ...
    assignment = get_user_assignments(user_data, config.DXC_ASSIGNMENT_MAPPER)
...
def get_file_data_mapping(item):
    return {
        ...
        "default_weekly_hours": item.get("defaultweeklyhours", "")
    }
...
def _get_user_oefs_details():
    ...
    return {
        ...
        # NEW: Default Weekly Hours OEF
        "default_weekly_hours": replicon_oef_list.get("default_weekly_hours", {})
    }
...
def _get_user_udfs_details():
    ...
    return {
        ...
        # UK&I specific UDFs
        "additional_job_classifications": replicon_udf_list.get("additional_job_classifications", {}),
        "holiday_schedule_calendar": replicon_udf_list.get("holiday_schedule_calendar", {}),
        "employee_representative_status": replicon_udf_list.get("employee_representative_status", {}),
        "employee_representative_effective_date": replicon_udf_list.get("employee_representative_effective_date", {}),
        "personnel_area_name": replicon_udf_list.get("personnel_area_description", {}),
        "personnel_area_code": replicon_udf_list.get("personnel_area_code", {}),
        # UKI Export UDF
        "term_exported_australia": replicon_udf_list.get("term_exported_(aus)", {})
    }
...
def get_item_index(dag_run, batch_count, item=None, use_item= False, use_idx= False, idx=None):
    if use_idx and idx is not None:
        return (int(idx) % batch_count) + 1
...
def can_update_user_end_date_test_uki_es(dag_run):
...
def user_does_not_have_admin_and_payroll_permission_test_uki_es():
...
def is_user_already_disabled_test_uki_es(dag_run):
...
def is_user_rehire_test_uki_es(dag_run):
    user_details = rail.result('get_user_details')
...
def can_update_user_start_date_test_uki_es(dag_run):
...
def get_policy_set_to_assign_uki_es():
...
def timeoff_to_assign_uki_es(dag_run):
...
def get_excluded_oef_clear_payloads(region: str, current_oef_values: list) -> list:
    from dxctechnology.workday_user_import_v1.user_import_uki_es_v2.utils.region_fields_config import _OEF_CONFIG
