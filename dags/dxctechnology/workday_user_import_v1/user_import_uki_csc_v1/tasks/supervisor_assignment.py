...
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v1.utils.custom_methods import is_profile_enabled, get_user_uri
    ...
    with rail.TaskGroup(group_id=group_id, prefix_group_id=False):
        ...
        def get_supervisor_details_data_handler(response, dag_run):
            if not response['rows']:
                return []
            return_response = list(filter(lambda emp_record: emp_record['emp_id'] == dag_run.conf['file_data']['supervisor_id'] and emp_record['login_name'] == dag_run.conf['file_data']['supervisor_email_id']
                ,map(lambda record: {
        ...
        is_supervisor_disabled = rail.IfOperator(
            ...
            test=lambda: (rail.result("get_supervisor_details")["status"]).lower() == "false",
        ...
        def get_supervisor_permission_details_data_handler(response, dag_run):
            ...
            return {
                ...
                "supervisor_permission": rail.find_first_by_attr_and_get_attr(response,
                    'permissionSet.name', dag_run.conf['user_permissions']['supervisor_end_user_supervision_permission']['name']),
            }
        ...
        assign_supervisor_permission = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "userUri": rail.result('get_supervisor_details')['user_uri'],
                "permissionSetUri": dag_run.conf['user_permissions']['supervisor_end_user_supervision_permission']['uri']
            }
        )
        ...
        def get_data_access_scope_to_assign_callable(dag_run):
            ...
            _employee_type = list(map(lambda employee_type: {
                                    ...
                                    , rail.load_json_artifact(dag_run.conf['employee_type_data'])
                                ))
        ...
        log_supervisor_for_reassignment_check = rail.WriteLogOperator(
            ...
            properties = lambda dag_run: {
                ...
                "supervisor_end_user_permission": dag_run.conf['user_permissions']['supervisor_end_user_supervision_permission'],
                "supervisor_user_permission":dag_run.conf['user_permissions']['supervisor_end_user_permission'],
