...
from dxctechnology.workday_user_import_v1.user_import_uki_csc_v1.utils.date_utils import _get_effective_date_based_on_work_week
...
from dxctechnology.workday_user_import_v1.user_import.common_utils.region_fields_config import DXC_UKI
...
def _get_holiday_calendar_update_payload(dag_run, user_details, profile_status_is_enabled, exception_log):
    ...
    if not holiday_calendar:
        if _is_international_assignee(dag_run.conf['file_data'].get('is_ia')):
            if user_holiday_calendar and user_holiday_calendar != NONE_DEFAULT_VALUE:
