...
import rail
from os import path
from dxctechnology.workday_user_import_v1.user_import_philippines_v3.tasks.get_all_required_fields import get_all_required_fields
from dxctechnology.workday_user_import_v1.user_import_philippines_v3.utils.custom_methods import (
    get_process_philippines_user_data_config,
    cached_write_json_artifact,
    get_trigger_dag_id,
    get_item_index,
    get_all_run_ids_callable
)

def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.workday_user_import_process_philippines_data_child_dag,
        description="DXC Technology Workday User Sync Philippines Data Child",
        replicon_conn_id=config.replicon_conn_id,
        schedule_interval=config.schedule_interval,
        company_key=config.company_key,
        start_date=datetime(2023, 9, 26),
        max_active_runs=1,#config.max_active_run_master,
        default_args={
        ...
        send_bad_file_format_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon user import for workday PHL  - Incorrect File Format - {{ current_time_in_specified_tz() }}',
        ...
        create_phl_user_data_collection = rail.CreateCollectionOperator(
            task_id = "create_phl_user_data_collection",
        ...
        updated_country_state_based_on_ia = rail.DataAdaptorOperator(
            ...
            source="{{result('create_phl_user_data_collection')}}",
        ...
        create_input_collection2 = rail.CreateCollectionOperator(
            ...
            name = "philippines_data",
        )

        get_data_start, get_data_end = get_all_required_fields("get_replicon_details", config)

        # Filter for records with company_code values PHES or PHET
        get_valid_philippines_data = rail.QueryCollectionOperator(
            task_id="get_valid_philippines_data",
            query="""SELECT *, ROW_NUMBER() OVER(ORDER BY ROWID) as user_record_index FROM philippines_data pdr WHERE
                    LOWER(pdr.companycode) IN ('phes', 'phet') and LOWER(pdr._country_to_use_for_query) == 'philippines'""",
            name="valid_philippines_data"
        )


        has_any_valid_data = rail.IfOperator(
            task_id = "has_any_valid_data",
            test=lambda: rail.result(get_valid_philippines_data.task_id, 'length') > 0,
        ...
        process_users = rail.trigger_parallel_dagrun(
            ...
            items="{{result('get_valid_philippines_data')}}",
            trigger_dag_id=lambda item: get_trigger_dag_id(
                config.workday_user_import_philippines_process_users_child_dag,
                config.DAG_BATCH_COUNT,
                item_index=int(item['user_record_index'])
            ),
            parallel_count=config.process_users_parallel_count,
            execution_timeout=timedelta(days=1),
            conf=lambda dag_run, item: get_process_philippines_user_data_config(dag_run, item, config)
        )
        ...
        trigger_log_generation = rail.TriggerDagRunOperator(
            ...
            conf=lambda: {
                ...
                "total_record_count": rail.result("create_phl_user_data_collection", "length") if rail.result("create_phl_user_data_collection", "length") else 0,
        ...
        load_data >> create_supervisor_log >> create_phl_user_data_collection >> updated_country_state_based_on_ia >> create_input_collection2 >> get_data_start
        get_data_end >> get_valid_philippines_data

        get_valid_philippines_data >> has_any_valid_data >> rail.Label("Yes") >> process_users_start >> process_users
