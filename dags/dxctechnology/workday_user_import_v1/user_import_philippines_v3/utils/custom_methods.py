...
from dxctechnology.workday_user_import_v1.user_import_philippines_v3.utils.request_payload import (
...
def add_new_policy_line(dag_run):
    ...
    balance_to_add = 0
    if "add_balance_as_zero" in dag_run.conf:
        if dag_run.conf['add_balance_as_zero'] == 'no':
            balance_to_add = exp_to_decimal_best(str(rail.result("get_user_timeoff_balance_summary")['timeRemaining']))
    return {
        "effectiveDate": {
            "day": _date.day,
            "month": _date.month,
