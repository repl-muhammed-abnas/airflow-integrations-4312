...
def create_add_user_dag(config):
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            dag_id = f"{config.workday_user_import_philippines_add_user_dag}{prefix}",
            description = config.workday_user_import_philippines_add_user_dag_description,
            company_key=config.company_key,
            replicon_conn_id=config.replicon_conn_id,
            max_active_runs = config.max_active_run_add_user_philippines,
        ) as dag:
            ...
            create_user = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run: request_payload.create_user_payload(dag_run, config)
            )

            remove_timeoffs = rail.RepliconServiceOperator(
                task_id = "remove_timeoffs",
                endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
                data = lambda dag_run: request_payload.get_timeoff_to_assign_remove_payload(dag_run, mode="hard-remove")
            )
            ...
            update_product_assignment = rail.RepliconServiceOperator(
                ...
                data=request_payload.get_product_assignment_payload
            )
            ...
            update_time_entry_path = rail.RepliconServiceOperator(
                ...
                data=request_payload.update_time_entry_path_payload
            )
            ...
            # Update the content in the function
            ...
            is_user_on_leave = rail.IfOperator(
                ...
                test = lambda dag_run: dag_run.conf['file_data']['on_leave'] in [1,'1'],
                yes_task="log_user_completion",
            ...
            timeoff_to_assign = rail.PythonOperator(
                ...
                python_callable=custom_methods.timeoff_to_assign
            )
            ...
            assign_timeoff_to_user = rail.RepliconServiceOperator(
                ...
                endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
                data = lambda dag_run: request_payload.get_timeoff_to_assign_remove_payload(dag_run, mode="assign")
            )
            ...
            def get_process_timeoff_conf(dag_run, item):
                ...
                return {
                    ...
                    "file_data": file_data  # Pass entire file_data for marital status fields
                }
            ...
            process_new_user_timeoff_assignment = rail.TriggerDagRunForEachItemOperator(
                ...
                trigger_dag_id= lambda dag_run, item : custom_methods.get_trigger_dag_id(
                    trigger_dag_id=config.workday_user_import_philippines_add_user_timeoff_assignment_dag, 
                    max_dag_batch_count=config.DAG_BATCH_COUNT, 
            ...
            # to disabled specific timeoffs after assignment
            reassign_timeoff_to_user = rail.RepliconServiceOperator(
                task_id = "reassign_timeoff_to_user",
                endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
                data = lambda dag_run: request_payload.get_timeoff_to_assign_remove_payload(dag_run, mode="soft-remove")
            )

            def get_log_message(dag_run):
                exception_msg = rail.result('create_user', 'exception_log') or []
                timesheet_exception = [f'Timesheet Template {dag_run.conf["user_policies"]["timesheet_template"]["timesheet_template"]} not present in Replicon'] if not bool(dag_run.conf.get('user_policies', {}).get('timesheet_template', {}).get('uri')) else []
            ...
            create_user >>  remove_timeoffs >>  update_notification_preference
            ...
            supervisor_end >> is_user_on_leave >> rail.Label("No") >> get_all_timeoffs
            is_user_on_leave >> rail.Label("Yes") >> log_user_completion
