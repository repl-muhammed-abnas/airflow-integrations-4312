...
from dxctechnology.workday_user_import_v1.user_import_philippines_v3.utils import request_payload, custom_methods
from dxctechnology.workday_user_import_v1.user_import_philippines_v3.tasks.supervisor_assignment import assign_supervisor
from dxctechnology.workday_user_import_v1.user_import_philippines_v3.utils.response_filter import get_effective_grp_membership_data_handler
from dxctechnology.workday_user_import_v1.user_import_philippines_v3.utils.custom_methods import should_trigger_delete_time_and_timeoff_for_disabled_user
from dxctechnology.workday_user_import_v1.user_import_philippines_v3.utils.request_payload import get_todays_date_for_timezone_in_json
    ...
    for batch_index in range(1, config.DAG_BATCH_COUNT + 1):
        ...
        with rail.create_airflow_dag(
            ...
            is_user_disabled_for_non_go_live_country = rail.IfOperator(
                ...
                yes_task = "get_assigned_permission_for_user_dummy",
                no_task = "is_user_already_disabled_41"
            )

            get_assigned_permission_for_user_dummy = rail.EmptyOperator(
                task_id="get_assigned_permission_for_user_dummy"
            )
            ...
            is_user_rehire = rail.IfOperator(
                ...
                yes_task="is_on_leave_status_one",
                no_task="is_user_for_long_leave_disable_dummy"
            )

            is_user_for_long_leave_disable_dummy = rail.EmptyOperator(
                task_id = "is_user_for_long_leave_disable_dummy"
            )

            is_on_leave_status_one = rail.IfOperator(
                task_id = "is_on_leave_status_one",
                test=lambda dag_run: dag_run.conf['file_data']['on_leave'] == "1",
            ...
            can_update_term_date_2 = rail.IfOperator(
                task_id = "can_update_term_date_2",
                test = lambda dag_run: bool(dag_run.conf['json_formatted_dates']['term_date']),
                yes_task = "update_user_end_date_5",
                no_task = "log_disabled_user_6"
            )
            ...
            log_disabled_user_6 =  rail.WriteLogOperator(
                task_id = "log_disabled_user_6",
                log = "{{dag_run.conf.user_log}}",
                message="User Disable",
                severity="Sucess",
                properties=lambda dag_run:{
            ...
            def is_user_for_long_leave_disable_test(dag_run):
                ...
                user_not_project_manager = not bool(rail.find_first_by_attr_and_get_attr(
                    rail.result("get_assigned_permission_for_user"), "policyUri", "urn:replicon:policy:project-management"
                ))
                user_is_enabled = user_details['userDetails']['isEnabled']
                user_is_on_leave = dag_run.conf['file_data']['on_leave'] in [1, '1']
                return (user_not_project_manager and user_is_enabled and user_is_on_leave) 

            is_user_for_long_leave_disable = rail.IfOperator(
                task_id = "is_user_for_long_leave_disable",
                test=is_user_for_long_leave_disable_test,
                yes_task="disable_user_login_4_dummy",
                no_task="can_update_user_start_date"
            )

            disable_user_login_4_dummy = rail.EmptyOperator(
                task_id = "disable_user_login_4_dummy"
            )
            ...
            can_update_term_date = rail.IfOperator(
                task_id = "can_update_term_date",
                test = lambda dag_run: bool(dag_run.conf['json_formatted_dates']['term_date']),
                yes_task = "update_user_end_date_4",
                no_task = "trigger_process_timeoff_policies_no_accrual_5"
            )
            ...
            trigger_process_timeoff_policies_no_accrual_5 = rail.TriggerDagRunForEachItemOperator(
                task_id = "trigger_process_timeoff_policies_no_accrual_5",
                trigger_dag_id=lambda dag_run: custom_methods.get_trigger_dag_id(
                    config.process_time_off_accrual,
                    config.DAG_BATCH_COUNT,
                    item_index=(custom_methods.get_item_index(dag_run, config.DAG_BATCH_COUNT))
                ...
                conf=lambda item, dag_run: {
                    ...
                    "timeoff_type_uri": item['timeOffType']["uri"],
                    "policy_set": dumps(item['policySetSchedule']).replace("[[{", "[{").replace("}]]", "}]"),
                    "end_date": custom_methods.convert_json_date_to_string_date(custom_methods.get_todays_date_in_json()),
                    "user_end_date_json": custom_methods.get_todays_date_in_json(),
                    "starting_balance_set_to_uri": dag_run.conf["starting_balance_set_to_uri"],
                    "prevent_balance_overdraw_uri": dag_run.conf["prevent_balance_overdraw_uri"],
                    "login_name": dag_run.conf['file_data']['email_id'],
                    "parent_location" : rail.result("get_effective_group_membership")['parent_location'],
            ...
            wait_for_process_timeoff_policies_5 = rail.WaitForDagRunsSensor(
                task_id = "wait_for_process_timeoff_policies_5",
                dag_runs="{{result('trigger_process_timeoff_policies_no_accrual_5')}}",
                execution_timeout = timedelta(days=config.execution_timeout_days)
            )

            log_disabled_user_5 =  rail.WriteLogOperator(
                task_id = "log_disabled_user_5",
                log = "{{dag_run.conf.user_log}}",
                message="User Disable",
                severity="Sucess",
                properties=lambda dag_run:{
            ...
            get_user_timeoff_policy_summary >> get_assigned_permission_for_user >> is_user_disabled_for_non_go_live_country >> rail.Label("No") >> is_user_already_disabled_41
            ...
            is_user_disabled_for_non_go_live_country >> rail.Label("Yes") >> get_assigned_permission_for_user_dummy >> has_no_project_management_permission
            ...
            is_user_rehire >> rail.Label("Yes") >> is_on_leave_status_one >> rail.Label("No") >> enable_login >> update_user_start_date_48 >> can_update_user_start_date
            is_on_leave_status_one >> rail.Label("Yes") >> empty_can_update_user_end_date_2_task >> can_update_term_date_2
            can_update_term_date_2 >> rail.Label("Yes") >> update_user_end_date_5 >> log_disabled_user_6
            can_update_term_date_2 >> rail.Label("No")>> log_disabled_user_6 >> should_trigger_delete_time_and_timeoff
            is_user_for_long_leave_disable >> rail.Label(
                "Yes") >> disable_user_login_4_dummy >> disable_user_login_4 >> can_update_term_date >> rail.Label(
                "Yes") >> update_user_end_date_4 >> trigger_process_timeoff_policies_no_accrual_5 >> wait_for_process_timeoff_policies_5 >> log_disabled_user_5 >> should_trigger_delete_time_and_timeoff
            can_update_term_date >> rail.Label("No") >> trigger_process_timeoff_policies_no_accrual_5 
            is_user_rehire >> rail.Label("No") >> is_user_for_long_leave_disable_dummy >> is_user_for_long_leave_disable >> rail.Label("No") >> can_update_user_start_date >> rail.Label('No') >> should_disable_user
