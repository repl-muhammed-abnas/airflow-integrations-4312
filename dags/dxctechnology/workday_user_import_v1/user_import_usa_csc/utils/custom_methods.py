...
from dxctechnology.workday_user_import_v1.user_import_usa_csc.utils.request_payload import _add_custom_field, _get_udf_fields_to_clear_for_ia
...
def _update_custom_fields_for_user(dag_run):
    ...
    # For IA=1 users, clear UDF fields that are not applicable to USA CSC region
    ...
    # Store exception log for later retrieval
    if exception_log:
        rail.set_result(key="udf_clearing_exceptions", val=exception_log)
...
def get_update_user_payload(dag_run, config):
    ...
    # Include UDF clearing exceptions if any
    if rail.result("udf_clearing_exceptions", default=None):
        exceptions.extend(rail.result("udf_clearing_exceptions"))
