...
from dxctechnology.workday_user_import_v1.user_import_portugal.utils.custom_methods import get_effective_grp_membership_data_handler
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.usa_csc_process_users_child_dag_id,
        description="DXC Workday User Import US CSC - Process Each Users",
        replicon_conn_id=config.replicon_conn_id,
        company_key=config.company_key,
        start_date=datetime(2023, 9, 26),
        max_active_runs=config.max_run_process_each_users
    ) as dag:
        ...
        def get_work_week_date_usa_les(dag_run):
            ...
            def get_required_days_to_subtract():
                work_week = dag_run.conf['mapper_data']['work_week'].lower().split(" ")[0]
                if pendulum.now('America/Los_Angeles').day_of_week==7:
                    if work_week == "saturday":
                        return 1
                    if work_week == "sunday":
                        return 0
                    return 7

                elif pendulum.now('America/Los_Angeles').day_of_week==1:
                        if work_week == "saturday":
                            return 2
                        if work_week == "sunday":
                            return 1
                ...
                elif pendulum.now('America/Los_Angeles').day_of_week==2:
                    if work_week == "saturday":
                        return 3
                    if work_week == "sunday":
                        return 2
                ...
                elif pendulum.now('America/Los_Angeles').day_of_week==3:
                    if work_week == "saturday":
                        return 4
                    if work_week == "sunday":
                        return 3
                ...
                elif pendulum.now('America/Los_Angeles').day_of_week==4:
                    if work_week == "saturday":
                        return 5
                    if work_week == "sunday":
                        return 4
                ...
                elif pendulum.now('America/Los_Angeles').day_of_week==5:
                    if work_week == "saturday":
                        return 6
                    if work_week == "sunday":
                        return 5
            ...
            return str(pendulum.now('America/Los_Angeles').date().subtract(days=get_required_days_to_subtract()))
        
        trigger_update_user = rail.TriggerDagRunOperator(
            task_id = "trigger_update_user",
            trigger_dag_id=config.usa_csc_update_user_dag_id,
            conf=lambda dag_run: {
                **{
                    ...
                    "work_week_date": get_work_week_date_usa_les(dag_run),
        ...
        trigger_add_user = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.usa_csc_add_user_dag_id,
            conf=lambda dag_run: {
                **{
                    "replicon_field": "true" if dag_run.conf['file_data']['status'] == "1" else "false", 
                    "todays_date" : (pendulum.now(TIMEZONE)).strftime(DATE_FORMAT),
                    "user_log": rail.result('create_user_log')
                },
                **dag_run.conf
            }
