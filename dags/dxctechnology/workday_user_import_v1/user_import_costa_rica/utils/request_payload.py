...
from dxctechnology.workday_user_import_v1.user_import.common_utils.custom_methods import get_todays_minus_specified_days_date_in_json, get_json_date_from_date
...
# UDF fields that are NOT applicable to Costa Rica region (same as Global)
COSTA_RICA_EXCLUDED_UDFS = (
    "Work Shift",
    "Date of Birth",
    "Time Type",
    "Middle Name",
...
def _get_udf_fields_to_clear_for_ia(dag_run, current_custom_fields_values, exception_log):
    ...
    for udf_display_text in COSTA_RICA_EXCLUDED_UDFS:
        for cf in current_custom_fields_values:
            if cf.get('customField', {}).get('displayText') == udf_display_text:
                ...
                if has_value:
                    exception_log.append(f"Clearing UDF '{udf_display_text}' for IA=1 user (not applicable to Costa Rica region)")
...
def _get_custom_fields_to_assign(dag_run, action):
    ...
    if action =="add":
        ...
        if str(dag_run.conf['file_data'].get('is_ia', '0')) == '1':
            if dag_run.conf['file_data']['ia_start_date']:
                ...
                # Default to today if no start date provided for IA users
    ...
    if action == "update":
        ...
        is_ia = dag_run.conf['file_data']['is_ia'] in [1, '1']
        ...
        ia_start_date_dict = get_replicon_date(dag_run.conf['file_data']['ia_start_date']) if is_ia and dag_run.conf['file_data']['ia_start_date'] else None
        ia_end_date_dict = get_replicon_date(dag_run.conf['file_data']['ia_end_date']) if not is_ia and dag_run.conf['file_data']['ia_end_date'] else None
        
        # Get JSON formatted dates for effective_date usage
        ia_start_date_json = dag_run.conf['json_formatted_dates'].get('ia_start_date') if is_ia and dag_run.conf['file_data']['ia_start_date'] else None
        ia_end_date_json = dag_run.conf['json_formatted_dates'].get('ia_end_date') if not is_ia and dag_run.conf['file_data']['ia_end_date'] else None
        
        today_minus_five_days = convert_json_date_to_date(get_todays_minus_specified_days_date_in_json(5)) 

        if (is_ia != rail.result("current_assigned_udf_values")['is_ia']):
            udfs_to_assign.append(_add_custom_field(custom_fields['international_assignee']['uri'], text=dag_run.conf['file_data']['is_ia']))
            ...
            if is_ia and dag_run.conf['file_data']['ia_start_date'] and _compare_if_two_json_dates_are_same(date_1=dag_run.conf['json_formatted_dates']['ia_start_date'],
            ...
            if not is_ia and dag_run.conf['file_data']['ia_end_date'] and _compare_if_two_json_dates_are_same(date_1=dag_run.conf['json_formatted_dates']['ia_end_date'],
                date_2=rail.find_first_by_attr_and_get_attr(current_custom_fields_values,
        ...
        # For IA=1 users, clear UDF fields that are not applicable to Costa Rica region
        udf_fields_to_clear = _get_udf_fields_to_clear_for_ia(dag_run, current_custom_fields_values, exception_log)
        udfs_to_assign.extend(udf_fields_to_clear)

    return udfs_to_assign, effective_date
                            ...
                            "replacementScheduleEntries": [
                                {
                                    "schedulePolicy": {
                                        ...
                                        "scheduleTypeUri": dag_run.conf['schedule_uri']
                                    },
...
def get_update_user_payload(dag_run, config):
    ...
    custom_felids_to_apply, effective_date = _get_custom_fields_to_assign(dag_run, "update")
    payload = {
        "user": {
