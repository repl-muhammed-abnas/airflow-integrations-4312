...
from dxctechnology.gsap_wbs_import.utils import python_callable_methods


# pylint: disable=too-many-statements
def create_child_dag_wbs(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_gsap_wbs_import_child_process_projects_{config.instance}',
        ...
        process_blob_update = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=f'dxctechnology_gsap_wbs_import_child_process_blob_{config.instance}',
        ...
        process_child_wbs = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.child_wait_execution_timeout_days),
            trigger_dag_id=f'dxctechnology_gsap_wbs_import_child_process_child_projects_{config.instance}',
        ...
        update_division >> put_eligibleprojectteammember_dataaccessscopes
        put_eligibleprojectteammember_dataaccessscopes >> has_client
        has_client >> rail.Label(
            'No') >> remove_client >> can_be_potential_parent
        has_client >> rail.Label(
            'Yes') >> get_client_info >> update_client >> can_be_potential_parent
