...
def get_eligibleprojectteammember_dataaccessscopes(dag_run):
    diwo_divisions = ['3001', '3124', '3118', '1602']
    diwo_division_details = list(filter(lambda x: x['parent'] == 'GSAP' and (
        x['name'] in diwo_divisions), dag_run.conf['enableddivisions']))
    diwo_division_uris = list(
        map(lambda item: {'uri': item['uri']}, diwo_division_details))
    ...
    return {
        ...
        {
            "locations": [],
            "divisions": diwo_division_uris if is_diwo_wbs(dag_run) and not is_parent_psa_flag_exist() else [
                {
                    "uri": get_division(),
                    "parentUri": null,
                    "name": null
