...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_aus_terminationbalance_child_{config.instance}',
        description=f'DXC_AUS_terminationbalance_child {config.instance}',
        ...
        load_user_report = rail.run_report(
            ...
            report_params=lambda dag_run: request_payload.get_run_us_user_report_payload(dag_run,config.report_data)
        )
        ...
        query_disabled_users_data = rail.QueryCollectionOperator(
            ...
            # pylint: disable=line-too-long
            query=f"SELECT * FROM getalluserdata WHERE DATE(userenddate) > DATE('{request_payload.get_start_date_begin_of_week(config)}') AND  DATE(userenddate) < DATE('{request_payload.get_end_date_begin_of_week()}')"
        )
        ...
        # pylint: disable=line-too-long
        termination_balance_expected_report_columns = "User Name,Time Off Type,Time Off Balance,Employee ID,International Assignee,International assignee start date,IA PERNER ID,Actual Employee ID,User End Date"
