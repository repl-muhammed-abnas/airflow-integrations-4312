...
def get_logging_details(pta_weeks, time_zone):
    now = pd.now(time_zone)
...
def companycode_from_mapper(export, mapper, time_zone, lcsc_payroll_calendar, les_payroll_calendar):
    ...
    list_of_codes = list(filter(lambda item:
        item['export'] == export and item['region'] in valid_regions, mapper))
