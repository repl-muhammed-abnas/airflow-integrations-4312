...
from dxctechnology.time_export_v1.compass_outbound.tasks.update_export_status import cancel_time_export
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.compass_regular_time_export_master_dagid,
        description=f"DXC - Compass Regular Time Export Master - {config.instance}",
        company_key=config.company_key,
        start_date=datetime(2024, 11, 1, tz=config.utc_timezone),
        schedule_interval=config.reg_schedule_interval,
        ...
        if_to_skip_scheduled_run = rail.IfOperator(
            ...
            test=lambda: pendulum.now(config.utc_timezone).day_of_week == config.skip_run_weekday and pendulum.now(config.utc_timezone).hour == config.reg_skip_run_hour,
        ...
        get_all_compass_divisions = rail.RepliconServicePageOperator(
            task_id="get_all_compass_divisions",
            endpoint="/services/DivisionListService1.svc/GetData",
            data=request_payload.get_compass_divisions_payload,
        ...
        get_oef_details= rail.RepliconServiceOperator(
            task_id='get_oef_details',
        ...
        get_oef_drop_down_values_iwo_indicator = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "objectExtensionTagDefinitionUri": rail.find_first_by_attr_and_get_attr(
                    rail.result("get_oef_details"), 'name', 'IWO Indicator', 'uri')
            }
        )
        ...
        get_oef_drop_down_values_project_type = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "objectExtensionTagDefinitionUri": rail.find_first_by_attr_and_get_attr(
                    rail.result("get_oef_details"), 'name', 'Project Type', 'uri')
            }
        )

        get_filter_data_for_compass_reg_time_export = rail.PythonOperator(
            task_id='get_filter_data_for_compass_reg_time_export',
            python_callable=custom_methods.filter_data_for_compass_reg_time_export,
            op_args=[config]
        )

        get_past_time_exports_data_for_compass = rail.RepliconServiceOperator(
            task_id='get_past_time_exports_data_for_compass',
            endpoint='/services/TimeDataExportListService1.svc/GetData',
            data=lambda: request_payload.get_past_time_export_data_payload("REG-CP",
                rail.result("get_filter_data_for_compass_reg_time_export")),
        ...
        create_timedata_exportbatch = rail.RepliconServiceOperator(
            ...
            data=request_payload.create_compass_reg_time_export_payload
        )
        ...
        update_cancelled_timedataexport_name = rail.RepliconServiceOperator(
            ...
            data={
                ...
                "name": 'CP_cancel_{{ dag_run_ecid() }}_Reg',
            }
        )
        ...
        process_compass_regular_time_export = rail.TriggerDagRunOperator(
            task_id='process_compass_regular_time_export',
            retries=0,
            trigger_dag_id=config.compass_process_time_export_child_dagid,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda: {
                'downloadurl': null,
                'fileformaturi': rail.result('get_filter_data_for_compass_reg_time_export')['fileformaturi'],
                'timeexporturi': rail.result('get_timedataexport_batchresults')['timeDataExportUri'],
                'twbname': rail.result('log_current_export_name'),
                'postdata': "yes",
                'lasttwbname': rail.result('get_past_time_exports_data_for_compass')[0]['timeexport'],
                'lasttwburi': rail.result('get_past_time_exports_data_for_compass')[0]['uri'],
                ...
                'lasttwbuniqueindentifier_PN1': rail.result('get_past_time_exports_data_for_compass')[0]['timeexport'] + ("|PN1"
                    if (config.company_key).lower() == "dxctechnology" else "|NT1"),
                'lasttwbuniqueindentifier_PJ1': rail.result('get_past_time_exports_data_for_compass')[0]['timeexport'] + ("|PJ1"
                    if (config.company_key).lower() == "dxctechnology" else "|NT3"),
                'lasttwbuniqueindentifier_P01': rail.result('get_past_time_exports_data_for_compass')[0]['timeexport'] + ("|P01"
                    if (config.company_key).lower() == "dxctechnology" else "|NT2"),
                ...
                'twblist': rail.result('get_past_time_exports_data_for_compass'),
                'process_start_time': rail.result('get_process_starttime')
            }
        )


        process_psa_regular_time_export = rail.TriggerDagRunOperator(
            task_id='process_psa_regular_time_export',
            retries=0,
            trigger_dag_id=config.psa_outbound_time_export_reg_c1_compass_child_dagid,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda: {
                'downloadurl': null,
                'fileformaturi': rail.find_first_by_attr_and_get_attr(rail.result("get_time_download_script"),
                    'displayText', config.psa_reg_time_export_file_format, 'uri'),
                'timeexporturi': rail.result('get_timedataexport_batchresults')['timeDataExportUri'],
                'twbname': rail.result('log_current_export_name'),
                'postdata': "yes",
                'lasttwbname': rail.result('get_past_time_exports_data_for_compass')[0]['timeexport'],
                ...
                'lasttwbuniqueindentifier': rail.result('get_past_time_exports_data_for_compass')[0]['timeexport'] + "|PSA",
        ...
        get_process_starttime >> get_all_compass_divisions >> get_all_filter_definitions \
            >> get_time_download_script >> get_employeetype_groups >> get_oef_details \
                >> get_oef_drop_down_values_iwo_indicator >> get_oef_drop_down_values_project_type >> get_filter_data_for_compass_reg_time_export \
                    >> get_past_time_exports_data_for_compass \
        ...
        batch_end >> rail.Label('on success') >> process_compass_regular_time_export \
            >> process_psa_regular_time_export
