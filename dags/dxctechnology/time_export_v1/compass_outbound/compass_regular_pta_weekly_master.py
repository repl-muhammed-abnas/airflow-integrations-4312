...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.compass_regular_time_export_pta_weekly_master_dagid,
        description=f"DXC - Compass Regular Time Export PTA Weekly Master - {config.instance}",
        company_key=config.company_key,
        start_date=datetime(2024, 11, 1, tz=config.utc_timezone),
        schedule_interval=config.reg_pta_weekly_schedule_interval,
        ...
        get_filter_data_for_compass_reg_time_export = rail.PythonOperator(
            ...
            python_callable=custom_methods.filter_data_for_compass_reg_pta_weekly_time_export,
            op_args=[config]
        )
        ...
        get_past_time_exports_data_for_compass = rail.RepliconServiceOperator(
            ...
            data=lambda: request_payload.get_past_time_export_data_payload("REGP-CP",
        ...
        update_cancelled_timedataexport_name = rail.RepliconServiceOperator(
            ...
            data={
                ...
                "name": 'CP_cancel_{{ dag_run_ecid() }}_RegPTA',
            }
        )
        ...
        get_process_starttime >> get_all_compass_divisions >> get_all_filter_definitions \
            >> get_time_download_script >> get_employeetype_groups >> get_oef_details \
                >> get_oef_drop_down_values_iwo_indicator >> get_oef_drop_down_values_project_type \
                    >> get_filter_data_for_compass_reg_time_export >> get_past_time_exports_data_for_compass \
        ...
        batch_end >> rail.Label('on success') >> process_psa_regular_time_export \
            >> process_compass_regular_time_export
