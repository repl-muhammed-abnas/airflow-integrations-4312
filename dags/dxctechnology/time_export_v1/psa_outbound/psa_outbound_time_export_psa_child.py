...
null = None
export_columns = ["employeeid","date","wbs","task","hours","repliconuniqueid",
    "comments","billingkey","projectkey","attendancetypecode","childprojectwbs",
    "childprojecterp","iwowbsflag","parentprojecterp","labortype","homeerp","projecttypeflag"]

def create_child_dag(config):
    # pylint: disable=too-many-statements line-too-long
    with rail.create_airflow_dag(
        dag_id=config.psa_outbound_time_export_psa_child_dagid,
        description=f"DXC - PSA Outboud Time Export Create PSA Outbound Time export child - {config.instance}",
        ...
        query_entries_without_shortid_current = rail.QueryCollectionOperator(
            task_id='query_entries_without_shortid_current',
        ...
        query_entries_without_shortid_hours = rail.QueryCollectionOperator(
            task_id='query_entries_without_shortid_hours',
            query="SELECT * FROM finaltimedata_with_hours WHERE NULLIF(timeentryid, '') IS NULL"
        )

        if_short_id_not_exists = rail.IfOperator(
            task_id='if_short_id_not_exists',
            test='{{ result("query_entries_without_shortid_current", "length") > 0 or result("query_entries_without_shortid_hours", "length") > 0 }}',
            yes_task='fail_missing_short_id',
            no_task='if_collections_have_data'
        )
        ...
        get_psa_cost_center = rail.RepliconServiceOperator(
            task_id='get_psa_cost_center',
            endpoint="/services/CostCenterService1.svc/GetAllCostCenters",
            data_handler=response_filters.get_psa_cost_center_uri
        )

        get_psa_cost_center_child_hierarchy = rail.RepliconServiceOperator(
            task_id='get_psa_cost_center_child_hierarchy',
            endpoint="/services/CostCenterListService1.svc/GetChildHierarchyData",
            data=request_payload.get_psa_cost_center_child_hierarchy_payload,
        ...
        get_query_to_filter_time_export_data = rail.PythonOperator(
            ...
            python_callable=custom_methods.get_query_to_filter_psa_time_export_data,
            op_args=[config.timeoff_types_to_export]
        )

        query_filter_gsap_reg_time_export_data = rail.QueryCollectionOperator(
            task_id='query_filter_gsap_reg_time_export_data',
            query='{{ result("get_query_to_filter_time_export_data").gsap_regular }}',
            name='filtered_gsap_reg_time_export_data'
        )

        query_filter_gsap_iwo_time_export_data = rail.QueryCollectionOperator(
            task_id='query_filter_gsap_iwo_time_export_data',
            query='{{ result("get_query_to_filter_time_export_data").gsap_iwo }}',
            name='filtered_gsap_iwo_time_export_data'
        )

        query_filter_c1_iwo_time_export_data = rail.QueryCollectionOperator(
            task_id='query_filter_c1_iwo_time_export_data',
            query='{{ result("get_query_to_filter_time_export_data").c1_iwo }}',
            name='filtered_c1_iwo_time_export_data'
        )

        query_filter_compass_iwo_time_export_data = rail.QueryCollectionOperator(
            task_id='query_filter_compass_iwo_time_export_data',
            query='{{ result("get_query_to_filter_time_export_data").compass_iwo }}',
            name='filtered_compass_iwo_time_export_data'
        )

        if_filtered_timedata_has_data = rail.IfOperator(
            task_id='if_filtered_timedata_has_data',
            test=lambda: rail.result("query_filter_gsap_reg_time_export_data", key="length") > 0 or
                rail.result("query_filter_gsap_iwo_time_export_data", key="length") > 0 or
                rail.result("query_filter_c1_iwo_time_export_data", key="length") > 0 or
                rail.result("query_filter_compass_iwo_time_export_data", key="length") > 0,
            yes_task='query_merge_gsap_c1_compass_time_export_data',
            no_task='empty_filtered_data'
        )

        query_merge_gsap_c1_compass_time_export_data = rail.QueryCollectionOperator(
            task_id='query_merge_gsap_c1_compass_time_export_data',
            query='{{ result("get_query_to_filter_time_export_data").merge_gsap_reg_iwo_c1_compass_iwo }}',
            name='filtered_gsap_c1_compass_time_export_data'
        )
        ...
        query_ineligible_and_reversals = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM finaltimedata ftd WHERE ftd.timeentryid
                NOT IN (SELECT DISTINCT filtd.timeentryid FROM filtered_gsap_c1_compass_time_export_data filtd)
        ...
        query_filter_psa_time_export_data = rail.QueryCollectionOperator(
            task_id='query_filter_psa_time_export_data',
            query='{{ result("get_query_to_filter_time_export_data").psa_data }}',
            name='filtered_psa_time_export_data'
        )

        merge_filtered_and_reversals = rail.QueryCollectionOperator(
            task_id='merge_filtered_and_reversals',
            query="SELECT * FROM filtered_psa_time_export_data UNION ALL SELECT * FROM ineligible_and_reversals UNION ALL SELECT * FROM iwo_reversals",
            name='filtered_and_reversals'
        )

        if_filtered_psa_timedata_has_data = rail.IfOperator(
            task_id='if_filtered_psa_timedata_has_data',
            test=lambda: rail.result("merge_filtered_and_reversals", key="length") > 0,
            yes_task='final_data_for_processing',
            no_task='empty_filtered_data'
        )
        ...
        final_data_for_processing = rail.CreateCollectionOperator(
            ...
            source=lambda: custom_methods.get_final_psa_time_export_data(config.timeoff_types_to_export),
        ...
        create_document_psa_xml = rail.RenderTemplateOperator(
            ...
            target='artifact',
            template_file='xml_schema/psa_outbound.xml',
            dataset=custom_methods.add_starting_line_to_gsap_final_data
        )
        ...
        send_no_data_email = rail.EmailOperator(
            ...
            bcc=config.internal_logs_email,
            subject="{{ get_company_key() }} | Replicon time extract for PSA - No records to export - {{ current_time_in_specified_tz() }}",
        ...
        send_psa_export_complete_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject="{{ get_company_key() }} | Replicon time extract for PSA - Completed Successfully - {{ current_time_in_specified_tz() }}",
        ...
        can_export_psa_regular_timedata >> rail.Label("Yes") >> data_existence_var >> get_last_time_export_psa_details \
        ...
        create_download_batch >> execute_download_batch >> wait_for_download_batch >> get_download_url \
            >> download_export >> load_export >> create_final_time_data_collection >> query_entries_without_shortid_current \
            ...
            >> download_export_with_hours >> load_export_with_hours >> create_final_time_data_collection_with_hours >> query_entries_without_shortid_hours \
            >> if_short_id_not_exists

        if_short_id_not_exists >> rail.Label("Yes") >> fail_missing_short_id
        if_short_id_not_exists >> rail.Label("No") >> if_collections_have_data

        if_collections_have_data >> rail.Label("Yes") >> get_psa_cost_center >> get_psa_cost_center_child_hierarchy \
            >> get_psa_org_unit >> get_psa_org_unit_child_hierarchy >> get_query_to_filter_time_export_data \
                >> query_filter_gsap_reg_time_export_data >> query_filter_gsap_iwo_time_export_data \
                    >> query_filter_c1_iwo_time_export_data >> query_filter_compass_iwo_time_export_data >> if_filtered_timedata_has_data

        if_collections_have_data >> rail.Label("No") >> empty_final_data >> log_no_data_var

        if_filtered_timedata_has_data >> rail.Label("Yes") >> query_merge_gsap_c1_compass_time_export_data \
            >> query_ineligible_and_reversals >> query_iwo_reversals >> query_filter_psa_time_export_data >> merge_filtered_and_reversals >> if_filtered_psa_timedata_has_data

        if_filtered_psa_timedata_has_data >> rail.Label("Yes") >> final_data_for_processing \
            >> log_data_exist_var >> get_data_existence_var

        if_filtered_psa_timedata_has_data >> rail.Label("No") >> empty_filtered_data
        if_filtered_timedata_has_data >> rail.Label("No") >> empty_filtered_data
        empty_filtered_data >> log_no_data_var
