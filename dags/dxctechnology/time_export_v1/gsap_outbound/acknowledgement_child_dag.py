...
from dxctechnology.time_export_v1.gsap_outbound.utils import response_filters, custom_methods

null = None

def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.gsap_acknowledgement_child_dagid,
        description=f'DXC - GSAP Regular Time Export Acknowledgement Notification Not Received Child {config.instance}',
        ...
        if_erp_equals_gsap = rail.IfOperator(
            task_id='if_erp_equals_gsap',
            test=lambda dag_run: dag_run.conf["erp"].lower() == "gsap",
        ...
        get_unackn_email_content = rail.RenderTemplateOperator(
            ...
            template_file='templates/emails/gsap_ackn_template.html',
            target='result'
        )

        send_unackn_email = rail.EmailOperator(
            task_id='send_unackn_email',
            to=config.gsap_acknowledgement_email,
        ...
        log_twb_creation_time >> if_erp_equals_gsap
        if_erp_equals_gsap >> rail.Label("Yes") >> create_log_twb_without_acknowledge >> for_each_time_export >> check_twb_and_createdate
        if_erp_equals_gsap >> rail.Label("No") >> batch_end
