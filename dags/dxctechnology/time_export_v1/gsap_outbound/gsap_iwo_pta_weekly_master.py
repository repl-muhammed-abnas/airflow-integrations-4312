...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.gsap_iwo_time_export_pta_weekly_master_dagid,
        description=f"DXC - GSAP IWO Time Export PTA Weekly Master - {config.instance}",
        company_key=config.company_key,
        start_date=datetime(2024, 11, 1, tz=config.utc_timezone),
        schedule_interval=config.iwo_pta_weekly_schedule_interval,
        ...
        get_c1_compass_divisions = rail.RepliconServicePageOperator(
            task_id="get_c1_compass_divisions",
            endpoint="/services/DivisionListService1.svc/GetData",
            data=request_payload.get_c1_compass_divisions_payload,
        ...
        get_filter_data_for_gsap_iwo_time_export = rail.PythonOperator(
            task_id='get_filter_data_for_gsap_iwo_time_export',
            python_callable=custom_methods.filter_data_for_gsap_iwo_pta_weekly_time_export,
            op_args=[config]
        )
        ...
        get_past_time_exports_data_for_GSAP = rail.RepliconServiceOperator(
            ...
            data=lambda: request_payload.get_past_time_export_data_payload("IWOP-GS", rail.result("get_filter_data_for_gsap_iwo_time_export")),
        ...
        create_timedata_exportbatch = rail.RepliconServiceOperator(
            ...
            data=request_payload.create_gsap_iwo_time_export_payload
        )
        ...
        update_cancelled_timedataexport_name = rail.RepliconServiceOperator(
            ...
            data={
                ...
                "name": 'GS_Cancel_{{ dag_run_ecid() }}_IWOPTA',
            }
        )
        ...
        process_gsap_iwo_time_export = rail.TriggerDagRunOperator(
            ...
            conf=lambda: {
                'downloadurl': null,
                'fileformaturi': rail.result('get_filter_data_for_gsap_iwo_time_export')['fileformaturi'],
                'hoursfileformaturi': rail.result('get_filter_data_for_gsap_iwo_time_export')['hoursfileformaturi'],
        ...
        process_c1_iwo_time_export = rail.TriggerDagRunOperator(
            ...
            conf=lambda: {
                ...
                'lasttwbuniqueindentifier': rail.result('get_past_time_exports_data_for_GSAP')[0]['timeexport'] + "|C1",
                'twblist': rail.result('get_past_time_exports_data_for_GSAP'),
                'type': "IWO",
                'process_start_time': rail.result('get_process_starttime')
            }
        ...
        process_compass_iwo_time_export = rail.TriggerDagRunOperator(
            ...
            conf=lambda: {
                ...
                'lasttwbuniqueindentifier_PN1': rail.result('get_past_time_exports_data_for_GSAP')[0]['timeexport'] + ("|PN1"
                    if (config.company_key).lower() == "dxctechnology" else "|NT1"),
                'lasttwbuniqueindentifier_PJ1': rail.result('get_past_time_exports_data_for_GSAP')[0]['timeexport'] + ("|PJ1"
                    if (config.company_key).lower() == "dxctechnology" else "|NT3"),
                'lasttwbuniqueindentifier_P01': rail.result('get_past_time_exports_data_for_GSAP')[0]['timeexport'] + ("|P01"
                    if (config.company_key).lower() == "dxctechnology" else "|NT2"),
        ...
        get_process_starttime >> get_c1_compass_divisions >> get_all_filter_definitions \
            >> get_time_download_script >> get_employeetype_groups >> get_all_oef_details \
                >> get_oef_drop_down_values_project_type >> get_filter_data_for_gsap_iwo_time_export >> get_past_time_exports_data_for_GSAP \
        ...
        batch_end >> rail.Label('on success') >> process_gsap_iwo_time_export \
            >> process_c1_iwo_time_export \
                >> process_compass_iwo_time_export \
                    >> process_psa_iwo_time_export
