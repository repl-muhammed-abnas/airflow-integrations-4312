...
def filter_data_for_gsap_reg_time_export(config):
    ...
    return {
        ...
        "hoursfileformaturi": rail.find_first_by_attr_and_get_attr(time_data_formats, 'displayText', config.gsap_c1_cp_time_export_hours_file_format, 'uri'),
...
def get_query_to_filter_reg_time_export_data():
    ...
    return """SELECT * FROM finaltimedata
        WHERE (companycodecode = 'GSAP' AND breaktypename NOT IN ('Meal', 'Rest')
            ...
            AND supplementalpay <> 'Stand by' AND profsupplementalpay <> "Stand by"
