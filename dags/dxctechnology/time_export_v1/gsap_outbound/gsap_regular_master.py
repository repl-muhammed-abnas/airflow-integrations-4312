...
from dxctechnology.time_export_v1.gsap_outbound.tasks.update_export_status import cancel_time_export
from dxctechnology.time_export_v1.gsap_outbound.utils import request_payload, response_filters, custom_methods
import rail
...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.gsap_regular_time_export_master_dagid,
        description=f"DXC - GSAP Regular Time Export Master - {config.instance}",
        ...
        get_all_gsap_divisions = rail.RepliconServicePageOperator(
            task_id="get_all_gsap_divisions",
            endpoint="/services/DivisionListService1.svc/GetData",
            data=request_payload.get_gsap_divisions_payload,
        ...
        get_oef_drop_down_values_project_type = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "objectExtensionTagDefinitionUri": rail.find_first_by_attr_and_get_attr(
                    rail.result("get_all_oef_details"), 'name', 'Project Type', 'uri')
            }
        )
        ...
        get_oef_drop_down_values_iwo_indicator = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "objectExtensionTagDefinitionUri": rail.find_first_by_attr_and_get_attr(
                    rail.result("get_all_oef_details"), 'name', 'IWO Indicator', 'uri')
            }
        )

        get_filter_data_for_gsap_reg_time_export = rail.PythonOperator(
            task_id='get_filter_data_for_gsap_reg_time_export',
            python_callable=custom_methods.filter_data_for_gsap_reg_time_export,
            op_args=[config]
        )

        get_past_time_exports_data_for_GSAP = rail.RepliconServiceOperator(
            task_id='get_past_time_exports_data_for_GSAP',
            endpoint='/services/TimeDataExportListService1.svc/GetData',
            data=lambda: request_payload.get_past_time_export_data_payload("REG-GS", rail.result("get_filter_data_for_gsap_reg_time_export")),
        ...
        create_timedata_exportbatch = rail.RepliconServiceOperator(
            ...
            data=request_payload.create_gsap_reg_time_export_payload
        )
        ...
        update_cancelled_timedataexport_name = rail.RepliconServiceOperator(
            ...
            data={
                ...
                "name": 'GS_cancel_{{ dag_run_ecid() }}_Reg',
            }
        )
        ...
        process_gsap_regular_time_export = rail.TriggerDagRunOperator(
            task_id='process_gsap_regular_time_export',
            retries=0,
            trigger_dag_id=config.gsap_process_time_export_child_dagid,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda: {
                'downloadurl': null,
                'fileformaturi': rail.result('get_filter_data_for_gsap_reg_time_export')['fileformaturi'],
                'timeexporturi': rail.result('get_timedataexport_batchresults')['timeDataExportUri'],
                'twbname': rail.result('log_current_export_name'),
                'postdata': "yes",
                'lasttwbname': rail.result('get_past_time_exports_data_for_GSAP')[0]['timeexport'],
                'lasttwburi': rail.result('get_past_time_exports_data_for_GSAP')[0]['uri'],
                ...
                'lasttwbuniqueindentifier': rail.result('get_past_time_exports_data_for_GSAP')[0]['timeexport'] + "|GS",
                'twblist': rail.result('get_past_time_exports_data_for_GSAP'),
        ...
        process_psa_regular_time_export = rail.TriggerDagRunOperator(
            ...
            conf=lambda: {
                'downloadurl': null,
                'fileformaturi': rail.result('get_filter_data_for_gsap_reg_time_export')['fileformaturi'],
                'hoursfileformaturi': rail.result('get_filter_data_for_gsap_reg_time_export')['hoursfileformaturi'],
                'timeexporturi': rail.result('get_timedataexport_batchresults')['timeDataExportUri'],
                'twbname': rail.result('log_current_export_name'),
                'postdata': "yes",
                'lasttwbname': rail.result('get_past_time_exports_data_for_GSAP')[0]['timeexport'],
                ...
                'lasttwbuniqueindentifier': rail.result('get_past_time_exports_data_for_GSAP')[0]['timeexport'] + "|PSA",
                'oefuri': rail.find_first_by_attr_and_get_attr(rail.result("get_all_object_extension_field_bindings"),
        ...
        get_process_starttime >> get_all_gsap_divisions >> get_all_filter_definitions \
            >> get_time_download_script >> get_employeetype_groups >> get_all_oef_details >> get_oef_drop_down_values_project_type \
                >> get_oef_drop_down_values_iwo_indicator >> get_filter_data_for_gsap_reg_time_export >> get_past_time_exports_data_for_GSAP \
        ...
        batch_end >> rail.Label('on success') >> process_gsap_regular_time_export \
