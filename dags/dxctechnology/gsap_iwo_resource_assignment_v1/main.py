...
def create_attribute_1_master_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        parse_xml = rail.LoadXMLFileOperator(
            ...
            xsd_document='./dags/dxctechnology/gsap_iwo_resource_assignment_v1/xsdschema/input_schema.xsd'
        )
        ...
        # Group records by WBS for optimized processing (reduces redundant API calls)
        group_records_by_wbs = rail.PythonOperator(
            task_id='group_records_by_wbs',
            python_callable=python_callable_method.group_records_by_wbs
        )

        check_all_valid_wbs = rail.IfOperator(
            task_id='check_all_valid_wbs',
            test=lambda: len(rail.result('group_records_by_wbs')) > 0,
        ...
        log_no_record_to_process = rail.WriteLogOperator(
            ...
            items='{{ result("group_records_by_wbs", key="blank_records") | to_json }}',
            properties={
                'wbs': 'na',
                'empid': '',
                'taskcode': '',
        ...
        process_each_wbs_attribute = rail.trigger_parallel_dagrun(
            ...
            items='{{ result("group_records_by_wbs") | to_json }}',
        ...
        get_active_user >> \
            get_wbs_records_from_xml >> group_records_by_wbs >> check_all_valid_wbs
