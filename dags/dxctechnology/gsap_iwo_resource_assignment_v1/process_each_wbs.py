...
def create_attribute_1_process_wbs_child_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        # WBS validation - done ONCE per WBS (not per employee)
        ...
        # Log WBS not found for ALL employees using items parameter
        log_wbs_not_available = rail.WriteLogOperator(
            ...
            severity='Exception',
            items='{{ dag_run.conf.employees | to_json }}',
            properties={
        ...
        # Log WBS archived for ALL employees using items parameter
        ...
        # Log parent project not present for ALL employees using items parameter
        ...
        # Log division not available for ALL employees using items parameter
        ...
        # Process C1/Compass assignment - trigger for each employee in the WBS
        process_c1_compass_assignment = rail.TriggerDagRunForEachItemOperator(
            task_id='process_c1_compass_assignment',
            retries=0,
            items=lambda dag_run: dag_run.conf['employees'],
            execution_timeout=timedelta(days=config.execution_timeout_days),
            trigger_dag_id=config.process_c1_compass_assignment_dag_id,
        ...
        # Process GSAP assignment - trigger for each employee in the WBS
        process_gsap_assignment = rail.TriggerDagRunForEachItemOperator(
            task_id='process_gsap_assignment',
            retries=0,
            items=lambda dag_run: dag_run.conf['employees'],
            execution_timeout=timedelta(days=config.execution_timeout_days),
            trigger_dag_id=config.process_gsap_assignment_dag_id,
            conf=request_payload.get_c1_compass_conf
        )
        ...
        def get_log_to_sumo_extra_conf(dag_run):
            ...
            return {
                ...
                "wbs": dag_run.conf['wbs'],
                "message": get_message(),
                "resource_count": len(dag_run.conf.get('employees', []))
            }
        ...
        # GSAP path - process all employees
        check_parent_project_division >> rail.Label(
