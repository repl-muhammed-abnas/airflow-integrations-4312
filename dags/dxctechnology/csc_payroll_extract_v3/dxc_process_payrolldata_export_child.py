...
from dxctechnology.csc_payroll_extract_v3 import response_filter


def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_lcsc_location_company_codewise_payrolldata_export_child_v3_{config.instance}',
        description=f'DXC_Location_Company_Codewise_PayrollData_Export_Child - v3.0{config.instance}',
        ...
        create_document = rail.RenderTemplateOperator(
            ...
            target='artifact',
            template_file='schema/csc_payroll_export_data.txt',
        ...
        send_email_for_sftp_failure = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon payroll export for Canada - SFTP failure for {{dag_run.conf.location_name}}-'+config.company_codes+'-{{current_time_in_specified_tz()}}',
            params={
                'output_filepath': config.output_filepath,
                'company_code': config.company_codes,
            },
        ...
        send_email_for_export_copmpletion = rail.EmailOperator(
            ...
            bcc=config.internal_logs_email,
            # pylint: disable=line-too-long
            subject='{{ get_company_key() }} | Replicon payroll export for Canada completed for - {{dag_run.conf.location_name}} - '+config.company_codes+' - {{current_time_in_specified_tz()}}',
            params={
                'output_filepath': config.output_filepath,
        ...
        send_email_for_log_upload_failure = rail.EmailOperator(
            ...
            files=[
                ("log_{{ result('get_file_name')}}.txt", '{{result("log_file_data_to_csv")}}')]
        )
        # pylint: disable=line-too-long
        ...
        has_empty_empid_data >> rail.Label(
            ...
            'Yes') >> compose_item_payroll_csv_file >> logging_no_of_records_exported >> no_of_records_size_including_header_footer>> create_document >>\
                upload_payroll_item_file_secondary_sftp>> [pgp_encyrpt_item_file, can_upload_to_tertiary_sftp]

        can_upload_to_tertiary_sftp >> rail.Label('Yes') >> pgp_encyrpt_for_tertiary_sftp >> upload_encrypted_payroll_file_tertiary_sftp
        ...
        pgp_encyrpt_item_file >> upload_encrypted_payroll_item_file_sftp
