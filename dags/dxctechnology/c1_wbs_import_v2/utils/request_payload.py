...
def get_assign_permissionset_to_user_param():
    ...
    for user_uri in get_projectleader_uris():
...
def can_copy_project_from_icwbs():
    return bool(rail.result('get_project_info_based_on_wbs_element') and not rail.result('get_project_info_based_on_wbs_element')[0]['projectDetails'] and
                ...
                ( len(rail.result('get_all_project_task')) > 0 or
                  ...
                  len(rail.result('get_all_attribute_2_project_dependant_fields')) > 0
                )
            )
...
def create_projectorapply_modification_param(is_client_dag_triggered):
    ...
    if (not is_wbs_project_exist() or can_copy_project_from_icwbs()):
...
def get_project_dag_confg(item):
    ...
    return {
        ...
        'serviceofferinguri': rail.find_first_by_attr_and_get_attr(rail.result("get_all_object_extension_field"), "name", "Service Offering SFDC Major Code", "uri"),
        ...
        'projectidentifieruri': rail.find_first_by_attr_and_get_attr(rail.result("get_all_object_extension_field"), "name", "C1 Project Identifier", "uri"),
        ...
        "teamMemberDataAccessScopes": [
            {
                "divisions": [
                    {
                        "uri": dag_run.conf['psacompanycodeuri'],
                    }
                ]
            }
        ]
