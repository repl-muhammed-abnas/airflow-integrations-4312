...
def get_wbs_dataset():
    ...
    raw_file = rail.load_all_records(query_wbs_data)
    final_wbs_data = []
...
def get_wbs_md5_data(wbs_datas):
    ...
    for wbs_data in wbs_datas:
        wbs_data['md5'] = hashlib.md5(
            (str(wbs_data['WBS_Name']) + "," + str(wbs_data['sequance_number'])).encode('utf-8')).hexdigest()
...
def get_data_from_compared_wbs_files():
    query_wbs_data = rail.result("query_wbs_data")
    consolidated_wbs_data = rail.load_all_records(query_wbs_data)
    raw_wbs_data = rail.result("merge_all_wbs_files")
    for raw_wbs in raw_wbs_data:
        if rail.find_first_by_attr_and_get_attr(consolidated_wbs_data, "md5", raw_wbs['md5'], "WBS_Name"):
            raw_wbs['ignored'] = 'No'
        else:
            raw_wbs['ignored'] = 'Yes'
    return raw_wbs_data
