...
import rail
import rail.lib.collection
from airflow.models import Variable
from dxctechnology.ftp_time_export_pta_v4.utils import request_payload
from dxctechnology.ftp_time_export_pta_v4.utils import response_filter
from dxctechnology.ftp_time_export_pta_v4.utils import custom_method
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'Export time data for FTP V4.0 {config.instance}',
                            ...
                            'leftExpression': {
                                ...
                                'rightExpression': {
                                    'value': {
                                        'dateRange': {
                                            'startDate': {
                                                'year': '{{ (data_interval_end - macros.timedelta(days=365)).strftime(\'%Y\') }}',
                                                'month': '{{(data_interval_end - macros.timedelta(days=365)).strftime(\'%m\')}}',
                                                'day': '{{(data_interval_end - macros.timedelta(days=365)).strftime(\'%d\')}}'
                                            },
                                            'endDate': {
        ...
        process_all_unackn_exports = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.process_unackn_child_dag_id,
            conf=lambda item: {
        ...
        send_unackn_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() + " | Priority 2 : Payload acknowledgement not received for FTP PTA " }}',
        ...
        empty_export_email_content = """
            <p>Hello,<br /> <br />
            The Replicon time extract for FTP is completed successfully at {{ data_interval_end }}. 
            There are no records to export. The payload identifier is """ + getExportName() + """.</p>
            <p>For any queries, please contact our support team at 
            <a href='https://support.deltek.com/'>https://support.deltek.com/</a><br /><br />
            <strong>This is an automated mail from Deltek Integration. Please do not reply to this email.</strong><br /><br />
            Regards,<br />
            Deltek Inc</p>
        """
        empty_export_email_subject = '{{ get_company_key() }} | Weekly Replicon time extract for FTP - No records to export - {{ data_interval_end }}'
        ...
        filter_for_ftp_company_code = rail.DataAdaptorOperator(
            ...
            # TODO:this result is a bit weird cause it's getting the uri from deep in the task_group
        ...
        create_ftp_company_code_data_collection = rail.CreateCollectionOperator(
            task_id='create_ftp_company_code_data_collection',
            source="{{ result('filter_for_ftp_company_code') }}",
            name="ftp_company_code_data"
        )

        def build_exclusion_query():
            # Build timeoff types exclusion part
            timeoff_exclusions = "', '".join(config.timeoff_types_to_exclude_mapper)
            timeoff_filter = f"Time_Off_Type_Name NOT IN ('{timeoff_exclusions}')"
            
            # Build standby units exclusion part
            standby_conditions = []
            for time_type, standby_units in config.timetype_standby_units_to_exclude_mapper.items():
                # Replace spaces and special characters in column names with underscores
                column_name = rail.lib.collection.normalize_identifier(time_type)
                standby_units_str = "', '".join(standby_units)
                standby_conditions.append(f"{column_name} NOT IN ('{standby_units_str}')")
            
            standby_filter = " AND ".join(standby_conditions)
            
            return f"SELECT * FROM ftp_company_code_data WHERE ({timeoff_filter}) AND ({standby_filter})"

        query_exclude_specific_timeoff_and_standby_data = rail.QueryCollectionOperator(
            task_id='query_exclude_specific_timeoff_and_standby_data',
            query=build_exclusion_query(),
            name="filtered_time_data_on_timeoff_and_standby"
        )
        ...
        def translate_row(row):
            if row:
                return {
                    'Entryid': row['Short_Time_Entry_ID'] if row['Time_Entry_ID'] != '' else row['Entry_Date'] + row['Time_Off_Booking_ID'],
                    'Uniqueid': 'REPLICON',
                    'Employeeid': row['Actual_Employee_ID'] if row['Actual_Employee_ID'] else row['Employee_ID'],
                    'Date': datetime.strptime(row['Entry_Date'], '%Y%m%d').strftime('%m%d%Y'),
                    'Wbs': row['WBS___SO_Name'] if row['Master_WBS__SO__WO_'] == 'WBS' else '',
                    'Vblen': row['WBS___SO_Name'][0:10] if row['Master_WBS__SO__WO_'] == 'RO' else '',
                    'Salesitem': row['WBS___SO_Name'][-3:] if row['Master_WBS__SO__WO_'] == 'RO' else '',
                    'Attendancetype': '1010' if row['IWO_Indicator'] == 'X'
                        else row['Time_Type_US'].split('-')[0] if row['Time_Type_US'] != ''
                        else row['Attendance_Type_Code'] if row['Attendance_Type_Code'] != ''
                        else custom_method.get_timeoff_attendance_type(
                            row['Time_Off_Type_Name'],
                            row['Time_Off_Type_Description'],
                            row.get('Location_Name__Full_Path_', '')
                        ) if row['Time_Off_Booking_ID'] != '' 
                        else '1010',
                    'Hours': round(float(row['Hours__Current_']), 2),
                    'Comments': '0 Hour' if float(row['Hours__Current_']) == 0 else 'Time Off' if row['Time_Off_Booking_ID'] != '' else row['Comments'][0:39]
                }
        ...
        translate_csv = rail.DataAdaptorOperator(
            ...
            source='{{result(\'' + query_exclude_specific_timeoff_and_standby_data.task_id + '\')}}',
            data=translate_row,
            columns=[
                'Entryid',
                'Uniqueid',
        ...
        def file_upload_failed(context):
            subject = '{{ get_company_key() }} | Weekly Replicon time extract for FTP - SFTP upload failure - {{ data_interval_end }}'
    ...
    <p>Please find the attached file to be uploaded to sftp. Upload the file to the given sftp and debug the issue.<br /> <br /> Regards,<br /> Deltek Inc</p>
        ...
        send_success_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_email,
            subject='{{ get_company_key() }} | Weekly Replicon time extract for FTP - Completed Successfully - {{ data_interval_end }}',
            html_content="""
            <p>Hello,<br /> <br />The Replicon time extract for FTP is completed successfully.<br />
            ...
            <p>For any queries, please contact our support team at <a href="https://support.deltek.com/">https://support.deltek.com/</a><br />
                <br />
                <strong>This is an automated mail from Deltek Integration. Please do not reply to this email.</strong><br />
                <br /><br />
                Regards,<br />
                Deltek Inc
            </p>"""
        ...
        log_to_sumo = rail.SendToSumoOperator(
            ...
            data={
                'exporttype': 'Employee',
                'downstreamapp': 'FTP',
                'twbrowcount': '{{result(\'' + query_exclude_specific_timeoff_and_standby_data.task_id + '\',key=\'length\')}}',
                'twbname': getExportName(),
        ...
        start >> [get_ftp_divisions, employee_type_contractor,
                  employee_type_agency_contractor, time_export_download_script,
                  get_actual_start_time] >> get_all_past_time_export >> resolve_file_name >> log_filename_and_twbname >> export[0]
        ...
        export_has_data >> rail.Label('Yes') >> create_raw_data_collection >> filter_for_ftp_company_code \
            >> create_ftp_company_code_data_collection >> query_exclude_specific_timeoff_and_standby_data
        ...
        query_exclude_specific_timeoff_and_standby_data >> filtered_export_has_data
