...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_australia_payrolldata_export_absence_taken_child_{config.instance}',
        description=f'DXC_Australia_PayrollData_Export_Absence_taken_Child {config.instance}',
        ...
        get_file_name = rail.PythonOperator(
            ...
            + "_AUREPL_RE"+ dag_run.conf['file_diff'] + request_payload.get_sequence_no('sequence_no_for_2001') + "_DUT8G2I"
        )
        ...
        query_list_in_final_payroll_collection = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM final_payroll_item_data WHERE NULLIF(Pay_Code_Code, '') IS NOT NULL AND Pay_Code_Code IN ({{result('get_all_pay_codes_from_mapper')}})"""
        )
        ...
        query_list_in_final_payroll_collection >> compose_item_payroll_csv_file >> logging_no_of_records_exported >> no_of_records_size_including_header_footer >> create_document >>\
