...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'adhoc_dxctechnology_australia_payrolldata_export_user_schedule_child_{config.instance}',
        description=f'Adhoc_DXC_Australia_PayrollData_Export_User_Schedule_Child {config.instance}',
        ...
        load_user_schedule_balance_report = rail.run_report(
            ...
            report_params=request_payload.get_adhoc_run_user_schedule_balance_report_payload
        )
        ...
        query_invalid_user_schedule_balance_data = rail.QueryCollectionOperator(
            ...
            query='''SELECT * FROM user_schedule_balance_report_data_collection WHERE NULLIF(Employee_Id, '') IS NULL OR Employee_Id == '' ''',
        )
        ...
        query_employee_data_collection = rail.QueryCollectionOperator(
            ...
            query="SELECT DISTINCT Employee_Id,Shift_Description,Office_Schedule FROM active_userbalance WHERE NULLIF(Employee_Id, '') IS NOT NULL",
        )

        has_valid_data = rail.IfOperator(
            task_id='has_valid_data',
            test='{{ result("query_employee_data_collection", "length") > 0 }}',
            yes_task="process_each_user_based_empid",
            no_task="finish_export_no_valid_data"
        )

        process_each_user_based_empid= rail.TriggerDagRunForEachItemOperator(
            task_id= 'process_each_user_based_empid',
            retries=0,
            items=lambda: rail.result('query_employee_data_collection'),
            trigger_dag_id=f'dxctechnology_australia_payroll_export_process_each_user_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        wait_for_process_each_user_based_empid = rail.WaitForDagRunsSensor(
            task_id='wait_for_process_each_user_based_empid',
            dag_runs='{{ result("process_each_user_based_empid") }}',
        ...
        query_employee_data_collection >> has_valid_data >> rail.Label("Yes"
                ) >> process_each_user_based_empid >> wait_for_process_each_user_based_empid >> gather_child_data >> \
