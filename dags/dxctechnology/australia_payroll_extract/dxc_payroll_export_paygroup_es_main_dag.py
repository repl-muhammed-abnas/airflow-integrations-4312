...
from dxctechnology.australia_payroll_extract.tasks import master_dag_task_group
from dxctechnology.australia_payroll_extract.mapper.company_code_mapper_usles_uscsc import COMPANY_CODE_MAP_ES


def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"dxctechnology_australia_payrollexport_es_master_dag_{config.instance}",
        description=f"DXC_AUS_PayrollExport_ES_Master {config.instance}",
        company_key=config.company_key,
        start_date=datetime(2022, 4, 1, tz=config.ist_timezone),
        ...
        process_payrolldata_export = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_australia_payrolldata_export_es_child_{config.instance}',
        ...
        process_active_user = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'dxctechnology_australia_payrolldata_export_active_user_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
        ...
        process_cashout_annual_payroll_export= rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=lambda: rail.result('search_entries_companycode_mapper'),
            trigger_dag_id=f'dxctechnology_australia_payrolldata_export_sellback_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
