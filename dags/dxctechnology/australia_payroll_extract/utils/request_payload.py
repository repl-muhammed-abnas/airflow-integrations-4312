...
import json
import rail
from dxctechnology.australia_payroll_extract.mapper.time_off_balance_mapper import Time_off_mappper
...
def process_cashout_gsap_user_conf(item):
    return{
        ...
        'location_name': item['Location'],
        'region': 'GSAP',
        'division_name': ['3001', '3124', '1602', '3118'],
        'sequence_no_for_0416': item['sequence_no_for_0416'],
        'file_diff': 'GS'
    }
...
def get_dates():
    ...
    five_days_before_end_of_month = (dt(current_date.year,current_date.month, last_day_of_the_month) - timedelta(days=5)).strftime("%d")
    ...
    if today == five_days_before_end_of_month:
        start_date = dt.now().replace(day=1).strftime("%m/%d/%Y")
        end_date = (dt(current_date.year,current_date.month, last_day_of_the_month)).strftime("%m/%d/%Y")
...
def get_adhoc_run_user_schedule_balance_report_payload(dag_run):
    get_specific_report_details = rail.result(
...
def get_create_payroll_download_batch_gsap_payload(duration_days):
    conf = rail.get_current_context()['dag_run'].conf
    begin_of_week = dt.utcnow() - timedelta(days=dt.utcnow().weekday())
    null = None
...
def get_create_payrun_batch_gsap_payload(duration_days):
    conf = rail.get_current_context()['dag_run'].conf
    null = None
    begin_of_week = dt.utcnow() - timedelta(days=dt.utcnow().weekday())
    child_location_uris = get_child_hierarchy_data(rail.result(
...
def get_active_user_balance_us_data_row(items):
    ...
    return ["P2006",
            ...
            "",
            time_off_balance[0]['Quote Type'],
            items['Time_Off_Balance'].split("- ")[-1].replace(",",""),
            "",
            "",
            "",
            "",
