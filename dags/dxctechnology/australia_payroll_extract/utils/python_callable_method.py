...
def check_schedule_for_user_schedule():
    ...
    return bool(today_date in ('3', '8', five_days_before_end_of_month))
...
def calculate_dates():
    ...
    if data[0]['Shift_Description']:
        ...
        if derived_end_date == get_filter_end_date:
            ...
            check_friday= datetime.strptime(data[0]['MAX_Entry_Date_'], "%Y-%m-%d")
            if check_friday.weekday() == 4:
                end_date= (check_friday + timedelta(days=2)).strftime('%Y-%m-%d')
            else:
...
def get_required_data():
    query_data= rail.result("get_query_data")
...
def get_required_date_format(entry_date):
    date_format = datetime.strptime(entry_date, '%d %B %Y')
...
def get_converted_query_data(item):
    ...
    return {
        'Entry_Date': get_required_date_format(item['Entry_Date']),
