...
from dxctechnology.csc_payroll_extract_v2 import master_dag_task_group


def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"dxctechnology_lcsc_payrollexport_19_15_master_dag_v2_{config.instance}",
        ...
        master_dag_task_group_entry, master_dag_task_group_exit = master_dag_task_group.get_master_dag_task_group(
            config.time_19_15, config.export, config.frequency, config.company_key)
        ...
        process_payrolldata_export = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_lcsc_location_company_codewise_payrolldata_export_child_v2_{config.instance}',
