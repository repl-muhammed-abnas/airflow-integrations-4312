...
def report_batch(config):
    ...
    report_has_data = rail.IfOperator(
                ...
                test= "{{ result('run_iwo_report.get_report_result','has_data')}}",
    ...
    load_report_data = rail.LoadCSVFileOperator(
        ...
        document = "{{ result('run_iwo_report.get_report_result').reportGenerationResults[0].payload }}",
    )
    ...
    run_report_group_exit >> report_has_data >> rail.Label("Yes") >> load_report_data >> create_report_collection
    report_has_data >> rail.Label("No") >> fail_no_report_data

    return get_report_details,create_report_collection
