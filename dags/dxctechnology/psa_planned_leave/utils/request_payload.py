...
def get_australia_division_uris():
    return {
        "page": "1",
        "pagesize": "1000000",
        "columnUris": [
...
def get_filter_values():
    ...
    report_filter_uri_cost_center = rail.find_first_by_attr_and_get_attr(filterconfiguration, 'displayText', 'CurrentCostCenterFilter', 'uri')
    replicon_psa_costcenters_list = rail.result('get_all_psa_cost_centers')
    cost_center = list(map(lambda item: {
        "reportFilterUri": report_filter_uri_cost_center,
        "value": item['uri'].split(':')[-1],
    }, replicon_psa_costcenters_list))
    ...
    data_divison = rail.result('get_australia_division_uris')
    division = list(map(lambda item: {
    ...
    date_range = [
        ...
        {
            "reportFilterUri": report_filter_uri_date_range,
            "value": str((datetime.strptime(rail.result('get_export_data')['report_start_date'], "%m/%d/%Y") + timedelta(days=1)).strftime('%m/%d/%Y'))
        },
        {
            "reportFilterUri": report_filter_uri_date_range,
            "value": null
    ...
    return cost_center + division + date_range
...
def translate_row(row):
    ...
    return {
        ...
        'absencetype': get_paycode_code_value(row['TimeOffTypeUri']) if row['LeaveTypeName'].lower() != 'holiday' else '25Z0',
