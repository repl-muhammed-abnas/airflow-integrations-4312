...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_lcsc_terminationbalance_child_{config.instance}',
        description=f'DXC_LCSC_terminationbalance_child - V2.0 {config.instance}',
        ...
        # pylint: disable=line-too-long
        termination_balance_expected_report_columns = "User Name,Time Off Type,Time Off Balance,Employee ID,International Assignee,International assignee start date,IA PERNER ID"
        ...
        is_encryption_required = rail.IfOperator(
            ...
            yes_task="pgp_encyrpt_item_file",
        ...
        send_email_for_log_upload_failure = rail.EmailOperator(
            ...
            html_content="email_for_sftp_failure.html",
            files=[
                ("log_"+'{{ dag_run.conf.file_name}}', '{{result("final_termination_balance_data_to_csv")}}')]
        )
        ...
        # pylint: disable=line-too-long
        create_log>>process_start_time >> logging_job_start_time >> get_user_report_details >> load_user_report >> has_data >> rail.Label("Yes"
        ...
        query_valid_termination_balance_data >> has_valid_data >> rail.Label("Yes"
                ) >> final_termination_balance_data_to_csv >> create_document>>is_encryption_required>>rail.Label("Yes"
        ...
        upload_encrypted_export_data_to_sftp >> rail.Label(
            "on_success") >> logging_no_of_valid_records
        upload_export_data_to_sftp >> rail.Label(
            "on_success") >> logging_no_of_valid_records >> logging_file_creation
