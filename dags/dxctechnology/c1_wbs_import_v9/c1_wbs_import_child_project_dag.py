...
from dxctechnology.c1_wbs_import_v9.tasks import field_config
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        can_update_enddate_for_projects = rail.IfOperator(
            task_id="can_update_enddate_for_projects",
            test=lambda: request_payload.can_copy_project_from_icwbs(),
            yes_task="update_enddate_for_projects",
            no_task="get_project_created_status",
        )

        update_enddate_for_projects = rail.RepliconServiceOperator(
            task_id='update_enddate_for_projects',
            endpoint='/services/ProjectService1.svc/CreateProjectOrApplyModifications',
            data=request_payload.update_enddate_payload
        )
        ...
        can_update_psa_flag = rail.IfOperator(
            ...
            test=lambda dag_run : bool(dag_run.conf['ProjectType'] == 'RP' or dag_run.conf['ProjectType'] in config.project_type),
        ...
        create_projectorapply_modifications >> can_update_enddate_for_projects

        can_update_enddate_for_projects >> rail.Label(
            "Yes") >> update_enddate_for_projects >> get_project_created_status >> can_assign_comanager
        can_update_enddate_for_projects >> rail.Label(
            "No") >> get_project_created_status >> can_assign_comanager
