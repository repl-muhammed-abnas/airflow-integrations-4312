...
import rail
from dxctechnology.cwf_time_export_v6.utils import python_callable_method
from dxctechnology.cwf_time_export_v6.utils import request_payload
from dxctechnology.cwf_time_export_v6.utils import response_filter
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_cwf_time_export_psa_child_{config.instance}_v6',
        description=f'DXCTechnology_PSA WF39 Child for C1 Reg and Compass Reg v6 {config.instance}',
        ...
        process_acknowledgement_not_received = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_acknowledgement_not_received_notification_{config.instance}_v6',
            ...
            conf=lambda item: {
                ...
                "twbname": python_callable_method.get_dag_run_conf()['twbname']+'_WF39',
                "erp": 'PSA',
                'sender': 'PSA'
            }
        )
        ...
        get_final_line_no_data = rail.PythonOperator(
            ...
            python_callable=python_callable_method.get_psa_final_line_data
        )
        ...
        send_time_no_data_to_sftp = rail.SFTPUploadFileOperator(
            ...
            '/{{ dag_run.conf.twbname }}_WF39.xml.pgp',
        ...
        query_list_filtered_data_psa = rail.QueryCollectionOperator(
            ...
            query=config.psa_wf39_sql_query,
        )
        ...
        def map_row(item):
            c1_attendance_type = '1010'
            compass_attendance_type = item['attendancetypecode'] if item['attendancetypecode'] else None
            gsap_attendance_type = item['attendancetypecode'] if item['attendancetypename'] == 'Training' else '2082'

            return {
                "PERSNO": item['employeeid'],
                "DATE": datetime.strptime(item['entrydate'], config.entry_date_format).strftime("%Y%m%d"),
                "WBS": (item['parentwbs']).rjust(8, "0") if item['parentwbs'] else item['projectname'],
                "AttendanceAbsenceType": c1_attendance_type if item['companycodecode']=='C1' else (
                    compass_attendance_type if item['companycodecode']=='COMPASS' else
                   ( gsap_attendance_type if item['companycodecode']=='GSAP' else None)  ),
                "LaborType": item['labortype'].split("|")[0] if item['labortype'] and '|' in item['labortype'] else None,
                "BILLABLEINDICATOR": item['labortype'].split("|")[1] if item['labortype'] and '|' in item['labortype'] else None,
                "Hours": format(float(item['hours']), '.2f') if float(item['hours']) != 0 else '0',
                "TASKS": None if item['tasktype'] == 'GSAP Billing Key' else item['taskname'],
                "Attribute1": item['attributecode1'] if  item['attributecode1'] else None,
                "Attribute2": item['attributecode2'] if  item['attributecode2'] else None,
                "SHORTTEXT": item['comments'] if item['comments'] else None,
                "BillingKey": item['taskname'] if item['tasktype'] == 'GSAP Billing Key' else None,
                "GSAPTask": item['gsaptaskcode'] if item['gsaptask'] else None,
                "GSAPBillableFlag":item['gsapbillableflag'] if item['gsapbillableflag'] else None,
                "RepliconUniqueID": item['timeentryid'] if item['timeentryid'] else item['timeoffbookingid'],
                "Home_ERP": item['companycodecode'] if  item['companycodecode'] else None,
                "Child_Project_ERP": item['companycodecode'] if  item['companycodecode'] else None,
                "Home_Location": item['locationcode'] if item['locationcode'] else None,
                "TimeOff": "N",
                "Project_Type_Flag": 'X' if item['psaflag'] == 'X' else None
            }.values()
        ...
        get_final_line_collection = rail.CreateCollectionOperator(
            ...
            columns={
                ...
                'DATE': 'DATE',
                'WBS': 'WBS',
                'AttendanceAbsenceType': 'AttendanceAbsenceType',
                'LaborType': 'LaborType',
        ...
        send_mail_timedatafileexportfailed_psa = rail.EmailOperator(
            ...
            files=[
                ("{{dag_run.conf.twbname}}_WF39.xml",
                 '{{result("pgp_encyrpt_time_data")}}')
            ]
        ...
        has_rows_psa_query_list_filtered_data >> rail.Label(
            ...
            query_list_psa_final >> get_final_line >> get_final_line_collection >> get_final_export_data >> create_document_psa_xml >> \
