...
from dxctechnology.ftp_time_export_pta_v2.utils import request_payload
from dxctechnology.ftp_time_export_pta_v2.utils import response_filter
from dxctechnology.ftp_time_export_pta_v2.utils import custom_method
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_time_export_ftp_pta_v2_{config.instance}',
        description=f'Export time data for FTP {config.instance}',
        ...
        process_all_unackn_exports = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_ftp_pta_child_v4_process_all_unackn_export_{config.instance}',
            conf=lambda item: {
        ...
        export_has_data = rail.HasDataOperator(
            ...
            yes_task="filter_for_ftp_company_code",
        ...
        def translate_row(row):
            if row:
                return {
                    ...
                    # pylint: disable=line-too-long
                    'Attendancetype': '1010' if row['IWO Indicator'] == 'X' else row['Time Type US'].split('-')[0] if row['Time Type US'] != '' else row['Attendance Type Code'] if row['Attendance Type Code'] != '' else row['Time Off Type Description'] if row['Time Off Booking ID'] != '' else '1010',
        ...
        translate_csv = rail.DataAdaptorOperator(
            ...
            data=translate_row,
            columns=['Entryid', 'Uniqueid', 'Employeeid', 'Date', 'Wbs',
                     'Vblen', 'Salesitem', 'Attendancetype', 'Hours', 'Comments']
        )
    ...
    Deltek Inc.
        ...
        export_has_data >> rail.Label('Yes') >> filter_for_ftp_company_code
        ...
        filtered_export_has_data >> rail.Label(
            'Yes') >> translate_csv >> create_export_data_collection >> query_export_data >> get_final_line >> get_final_line_collection >> get_final_export_data >> generate_xml_time_data >> \
