...
from dxctechnology.gsap_wbs_import_v1.utils import response_filter
...
def create_child_dag_wbs(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_gsap_wbs_import_child_process_projects_{config.instance}_v1',
        ...
        is_parent_c1 = rail.IfOperator(
            ...
            test=lambda dag_run: dag_run.conf['c1compassparentwbs'] and request_payload.check_parent_division(
                dag_run) == 'C1',
        ...
        process_blob_update = rail.TriggerDagRunOperator(
            ...
            execution_timeout=timedelta(
                days=config.child_wait_execution_timeout_days),
            trigger_dag_id=f'dxctechnology_gsap_wbs_import_child_process_blob_{config.instance}_v1',
        ...
        process_child_wbs = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.child_wait_execution_timeout_days),
            trigger_dag_id=f'dxctechnology_gsap_wbs_import_child_process_child_projects_{config.instance}_v1',
        ...
        is_project_manager_available >> is_user_found_using_perner >> rail.Label(
            "Yes") >> empty_get_user_info >> get_user_info >> is_end_date_in_past
        ...
        is_end_date_in_past >> rail.Label('No') >> can_enable_project_manager >> rail.Label(
            'No') >> is_project_manager_contractor
        can_enable_project_manager >> rail.Label(
            'Yes') >> enable_project_manager >> is_project_manager_contractor
        ...
        does_parentwbs_exist >> rail.Label(
            'No') >> log_parent_doesnt_exist >> get_project_info_based_on_wbs_element
        can_copy_project_from_parent >> rail.Label(
            'Yes') >> create_project_copy_batch
        create_project_copy_batch >> project_copy_batch_group_entry
        project_copy_batch_group_exit >> get_projectcopy_batch_results >> is_parent_c1 >> rail.Label(
            'No') >> create_projectorapply_modifications
        is_parent_c1 >> rail.Label(
            'Yes') >> get_all_labour_types >> create_projectorapply_modifications
        can_copy_project_from_parent >> rail.Label(
            'No') >> create_projectorapply_modifications >> can_update_blob >> rail.Label('No') >> update_division
        can_update_blob >> rail.Label(
            'Yes') >> process_blob_update >> wait_for_process_blob_update >> update_division
