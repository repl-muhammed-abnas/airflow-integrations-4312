...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_wf39_psa_resource_assignment_compass_process_each_record_child_{config.instance}_v2',
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            test=lambda: Variable.get("dxctechnology_wf39_psa_resource_assignment_compass_batch_task_var_prod", default_var='true').lower() == 'true',
            yes_task="batch_task",
            no_task="create_log"
        )
        ...
        is_user_compass = rail.IfOperator(
            task_id="is_user_compass",
            test=python_callable_method.is_user_compass,
            yes_task="is_assignemnt_date_out_of_range",
            no_task="log_user_not_compass",
        )

        log_user_not_compass = rail.WriteLogOperator(
            task_id='log_user_not_compass',
            log='{{ result("create_log") }}',
            message="Required user '{{dag_run.conf.user}}' is not Compass User",
            properties={
                'employeeid': '{{dag_run.conf.user}}',
                'wbs': '{{dag_run.conf.wbselement}}',
                'role': '{{dag_run.conf.labourtype}}',
        ...
        create_log >> validate_field >> rail.Label(
        ...
        get_project_info_based_on_wbs_element >> get_user_info >> get_user_division_detail >> is_user_compass >> rail.Label(
        ...
        is_user_compass >> rail.Label("No") >> log_user_not_compass >> catch_and_log_errors
