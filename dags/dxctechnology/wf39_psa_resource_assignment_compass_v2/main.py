...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_wf39_psa_resource_assignment_compass_import_master_{config.instance}_v2',
        description=f'DXC_WF39 PSA Resource Assignment Master Compass V2.0 - SFTP {config.instance}',
        ...
        create_billing_rates = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'dxctechnology_wf39_psa_resource_assignment_compass_create_billing_rate_child_{config.instance}_v2',
            conf=lambda item: {
                'name': item['role']
            }
        )
        ...
        process_billing_rate_for_each_wbs_item = rail.trigger_parallel_dagrun(
            ...
            execution_timeout=timedelta(days=config.execution_timeout_days),
            trigger_dag_id=f'dxctechnology_wf39_psa_resource_assignment_compass_process_distinct_wbs_item_child_{config.instance}_v2',
        ...
        gather_process_each_wbs_record_logs = rail.GatherResultsFromDagRunsOperator(
            task_id='gather_process_each_wbs_record_logs',
            dag_runs='{{ result("get_process_each_wbs_dag_ids") }}',
            dagrun_task_id='gather_all_wbs_logs',
            execution_timeout=timedelta(
                hours=config.gather_logs_timeout_hours),
            flatten=True
        )
        ...
        load_report_data >> process_billing_rate_for_each_wbs_item >> \
            get_process_each_wbs_dag_ids >> gather_process_billing_rates_logs \
                >> gather_process_each_wbs_record_logs >> format_log_records >> render_logs_csv
