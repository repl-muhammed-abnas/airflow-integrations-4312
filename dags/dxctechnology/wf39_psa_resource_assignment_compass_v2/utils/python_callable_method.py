...
def check_labour_type(dag_run):
    data = rail.result("get_all_project_team_assignment")['labour_type_data']
    if dag_run.conf['labourtype'] in data:
        return True
    return False
...
def do_format_logs():
    ...
    logs = rail.result("gather_process_billing_rates_logs") + rail.result("gather_process_each_wbs_record_logs")
