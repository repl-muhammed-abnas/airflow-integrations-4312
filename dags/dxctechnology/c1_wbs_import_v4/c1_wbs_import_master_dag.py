...
import rail
from dxctechnology.c1_wbs_import_v4.utils import python_callable_method
from dxctechnology.c1_wbs_import_v4.utils import response_filter
from dxctechnology.c1_wbs_import_v4.utils import request_payload

# config : https://github.com/replicon/airflow-integrations/blob/main/dags/dxctechnology/c1_wbs_import_v4/config.py
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'DXC_C1_WBS_Automation Master - SFTP {config.instance}',
        ...
        parse_xml = rail.LoadXMLFileOperator(
            ...
            xsd_document='./dags/dxctechnology/c1_wbs_import_v4/xml_schema/input_schema.xsd'
        )
        ...
        get_details_from_xml = rail.XMLAdaptorOperator(
            ...
            adaptor=[
                'Records',
                {
                    ...
                    "ProjectIdentifier": "ProjectIdentifier/text()",
        ...
        get_compass_company_codes = rail.CreateCollectionOperator(
            task_id='get_compass_company_codes',
            source=lambda: list(filter(lambda x: "COMPASS" in (x['name'],x['parent']), rail.result(
                'get_enabled_divisions_company_codes'))),
        )
        ...
        get_oef_drop_down_values_project_identifier = rail.RepliconServiceOperator(
            task_id="get_oef_drop_down_values_project_identifier",
            endpoint="/services/ObjectExtensionTagDefinitionService1.svc/GetObjectExtensionTagDefinitionDetails",
            data={
                "objectExtensionTagDefinitionUri": "{{ result('get_all_object_extension_field_projects').projectidentifier }}"},
        )

        get_oef_drop_down_values_psaflag = rail.RepliconServiceOperator(
            task_id="get_oef_drop_down_values_psaflag",
            endpoint="/services/ObjectExtensionTagDefinitionService1.svc/GetObjectExtensionTagDefinitionDetails",
            data={
                "objectExtensionTagDefinitionUri": "{{ result('get_all_object_extension_field_projects').psaflag }}"},
        )
        ...
        def file_upload_failed(context):
            ...
            email = rail.EmailOperator(
                ...
                params={
                    'dag_id': f'dxctechnology_c1_wbs_import_master_{config.instance}'
                },
                files=[
        ...
        get_details_from_xml >> create_project_collection >> \
            ...
            get_rusia_custom_field_dropdown_options >> get_gsap_company_codes >> get_compass_company_codes >> get_all_object_extension_field_projects
        get_all_object_extension_field_projects >> get_oef_drop_down_values_project_type >> get_oef_drop_down_values_item_category
        get_oef_drop_down_values_item_category >> get_oef_drop_down_values_service_order_type >> get_oef_drop_down_values_project_identifier >> \
            get_oef_drop_down_values_psaflag >> get_new_project_types_to_add >> has_new_project_types
