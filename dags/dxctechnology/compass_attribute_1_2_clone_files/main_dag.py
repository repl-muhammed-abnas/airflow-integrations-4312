...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"dxctechnology_clone_compass_attribute_1_2_files_{config.sub_erp}_{config.instance}",
        description="testing purpose bla",
        ...
        is_attributes_1_2_file = rail.IfOperator(
            ...
            yes_task=['upload_1', 'is_file_attribute1'],
        ...
        upload_1 = rail.SFTPUploadFileOperator(
            task_id="upload_1",
            content="{{result('download_file')}}",
            remote_filepath=config.copy_filepath_1 +
        ...
        is_file_attribute1 = rail.IfOperator(
            ...
            yes_task=['upload_for_attribute1', 'upload_for_attribute1_old'],
        ...
        is_file_attribute2 = rail.IfOperator(
            ...
            yes_task=['upload_for_attribute2', 'upload_for_attribute2_old']
        )
        ...
        upload_for_attribute1_old = rail.SFTPUploadFileOperator(
            task_id="upload_for_attribute1_old",
            content="{{result('download_file')}}",
            remote_filepath=config.attribute1_old_filepath +
            "/{{ result('new_file_sensor') | file_name}}"
        )

        upload_for_attribute2_old = rail.SFTPUploadFileOperator(
            task_id="upload_for_attribute2_old",
            content="{{result('download_file')}}",
            remote_filepath=config.attribute2_old_filepath +
            "/{{ result('new_file_sensor') | file_name}}"
        )
        ...
        is_attributes_1_2_file >> rail.Label("Yes") >> \
            [upload_1, is_file_attribute1]
        is_file_attribute1 >> rail.Label(
            "Yes") >> [upload_for_attribute1, upload_for_attribute1_old] >> archive_file
        is_file_attribute1 >> rail.Label("No") >> is_file_attribute2 >> rail.Label("Yes") \
            >> [upload_for_attribute2, upload_for_attribute2_old] >> archive_file
