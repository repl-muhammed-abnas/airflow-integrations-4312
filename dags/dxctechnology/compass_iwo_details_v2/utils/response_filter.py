import rail
from dxctechnology.compass_iwo_details_v2.utils.custom_methods import get_value, get_full_path
...
def get_oef_details(response):
    ...
    return {
        ...
        "iwoindicator": rail.find_first_by_attr_and_get_attr(data, 'name', 'IWO Indicator', 'uri')
    }

def division_code_check(response):
    dag_run_conf = get_dag_run_conf()
    response = response.json()['d']['rows']
    if not response:
        return []

    return list(filter(lambda x: x['code'] == dag_run_conf['parentcompanycode'],list(map(lambda item: {
