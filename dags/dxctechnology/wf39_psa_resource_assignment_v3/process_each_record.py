import rail
from dxctechnology.wf39_psa_resource_assignment_v3.utils import python_callable_method
from dxctechnology.wf39_psa_resource_assignment_v3.utils import response_filter
from dxctechnology.wf39_psa_resource_assignment_v3.utils import request_payload
# pylint: disable=too-many-statements


def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_wf39_psa_resource_assignment_process_each_record_child_{config.instance}_v3',
        description=f'DXC_WF39 PSA Resource Assignment Automation Child v3 {config.instance}',
        ...
        is_user_c1 = rail.IfOperator(
            ...
            yes_task="is_assignemnt_date_out_of_range",
        ...
        is_assignemnt_date_out_of_range = rail.IfOperator(
            ...
            no_task="project_exist_validation"
        )

        log_user_date_range_exception = rail.WriteLogOperator(
            task_id='log_user_date_range_exception',
            message=lambda dag_run: str(
                python_callable_method.get_out_of_range_message(dag_run)),
            properties={
                'employeeid': '{{dag_run.conf.user}}',
                'wbs': '{{dag_run.conf.wbselement}}',
                'role': '{{dag_run.conf.labourtype}}',
        ...
        check_labour_type_in_feed_file = rail.IfOperator(
            task_id="check_labour_type_in_feed_file",
            test="{{ dag_run.conf.labourtype | is_truthy }}",
            yes_task="get_all_project_labour_types",
            no_task="is_uri_present_2"
        )

        is_uri_present_2 = rail.IfOperator(
            task_id='is_uri_present_2',
            test=lambda: len(rail.result(
                'get_all_project_team_assignment')['user_data']) > 0,
            yes_task="is_assignment_date_present_2",
            no_task="assign_user_to_project_2"
        )

        assign_user_to_project_2 = rail.RepliconServiceOperator(
            task_id="assign_user_to_project_2",
            endpoint="/services/ProjectService1.svc/UpdateProjectTeamMemberAssignment",
            data=request_payload.get_assign_user_payload
        )

        is_assignment_date_present_2 = rail.IfOperator(
            task_id="is_assignment_date_present_2",
            test=lambda: (bool(request_payload.get_dag_run_conf()['startdate']) or bool(
                request_payload.get_dag_run_conf()['enddate'])),
            yes_task="updateProjectTeamMemberAssignmentDateRange_2",
            no_task="log_success_if_labour_type_not_present_in_feed_file"
        )

        updateProjectTeamMemberAssignmentDateRange_2 = rail.RepliconServiceOperator(
            task_id="updateProjectTeamMemberAssignmentDateRange_2",
        ...
        log_success_if_labour_type_not_present_in_feed_file = rail.WriteLogOperator(
            task_id='log_success_if_labour_type_not_present_in_feed_file',
        ...
        get_all_project_labour_types = rail.RepliconServiceOperator(
            task_id='get_all_project_labour_types',
            endpoint='/services/ImportService1.svc/BulkGetProjects2',
            data={"projects": [{
                    "uri": "{{ result('get_project_info_based_on_wbs_element')[0]['projectDetails']['uri'] }}"
            }]},
            data_handler=response_filter.get_all_labour_types
        )

        is_labour_type_present_in_project = rail.IfOperator(
            task_id="is_labour_type_present_in_project",
            test=python_callable_method.check_labour_type,
            yes_task="is_uri_present",
            no_task="log_labour_type_validation",
        )

        log_labour_type_validation = rail.WriteLogOperator(
            task_id='log_labour_type_validation',
            message="Labour Type is Not Associated to the WBS '{{ dag_run.conf.wbselement }}' ",
            properties={
                'employeeid': '{{dag_run.conf.user}}',
                'wbs': '{{dag_run.conf.wbselement}}',
                'role': '{{dag_run.conf.labourtype}}',
        ...
        is_assignment_uri_present = rail.IfOperator(
            ...
            test=lambda: bool(rail.result(
                'get_all_project_team_assignment')['user_data']),
            yes_task="assignedBillingRates",
            no_task="is_labour_type_not_assigned"
        )
        ...
        is_labour_type_not_assigned = rail.IfOperator(
            task_id="is_labour_type_not_assigned",
            test=python_callable_method.is_LT_assigned,
            yes_task="can_assign_labour_type",
            no_task="is_success_record"
        )

        can_assign_labour_type = rail.IfOperator(
            task_id='can_assign_labour_type',
            test=python_callable_method.check_can_assign_labour_type,
        ...
        def successPayload():
            ...
            can_assign_labor_type = python_callable_method.check_can_assign_labour_type()
            data_item = request_payload.get_dag_run_conf()['labourtype']
            return {
                'employeeid': '{{dag_run.conf.user}}',
                'wbs': '{{dag_run.conf.wbselement}}',
                'role': '{{dag_run.conf.labourtype}}',
                'billingtype': [data_item+extension for extension in ["|Billable", "|Non-Billable"]]
                if value and can_assign_labor_type else "nill",
                'status': 'Success',
                'action': 'Add',
                'employee': '{{dag_run.conf.name}}'
            }
        ...
        get_project_info_based_on_wbs_element >> get_user_info >> get_user_division_detail >> is_user_c1 >> rail.Label(
            "Yes") >> is_assignemnt_date_out_of_range
        is_assignemnt_date_out_of_range >> rail.Label(
            'YES') >> log_user_date_range_exception >> catch_and_log_errors
        is_assignemnt_date_out_of_range >> rail.Label(
            'No') >> project_exist_validation
        is_user_c1 >> rail.Label(
            "No") >> log_user_not_c1 >> catch_and_log_errors
        ...
        get_all_project_team_assignment >> check_labour_type_in_feed_file

        check_labour_type_in_feed_file >> rail.Label("No") >> is_uri_present_2
        check_labour_type_in_feed_file >> rail.Label(
            "Yes") >> get_all_project_labour_types

        is_uri_present_2 >> rail.Label(
            "No") >> assign_user_to_project_2 >> is_assignment_date_present_2
        is_uri_present_2 >> rail.Label("Yes") >> is_assignment_date_present_2

        is_assignment_date_present_2 >> rail.Label(
            "No") >> log_success_if_labour_type_not_present_in_feed_file
        is_assignment_date_present_2 >> rail.Label(
            "Yes") >> updateProjectTeamMemberAssignmentDateRange_2 >> log_success_if_labour_type_not_present_in_feed_file

        log_success_if_labour_type_not_present_in_feed_file >> catch_and_log_errors

        get_all_project_labour_types >> is_labour_type_present_in_project
        is_labour_type_present_in_project >> rail.Label(
            "YES") >> is_uri_present
        is_labour_type_present_in_project >> rail.Label(
            "No") >> log_labour_type_validation >> catch_and_log_errors
        ...
        is_assignment_uri_present >> rail.Label(
            "YES") >> assignedBillingRates >> is_labour_type_not_assigned
        is_assignment_uri_present >> rail.Label(
            "NO") >> is_labour_type_not_assigned
        is_labour_type_not_assigned >> rail.Label(
            "YES") >> can_assign_labour_type
        can_assign_labour_type >> rail.Label(
            "YES") >> bulkUpdateProjectTeamMemberBillingRatesAllowedForBillingTime
        can_assign_labour_type >> rail.Label("NO") >> is_success_record
        bulkUpdateProjectTeamMemberBillingRatesAllowedForBillingTime >> is_success_record
