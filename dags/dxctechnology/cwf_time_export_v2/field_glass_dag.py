...
import json
import rail
from dxctechnology.cwf_time_export_v2.mapper.location import location_mapper
from dxctechnology.cwf_time_export_v2.task.field_glass_compass_task import get_compass_task
from dxctechnology.cwf_time_export_v2.task.c1_task import get_c1_task
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_cwf_time_export_field_glass_v2_{config.instance}',
        description=f'DXCTechnology_CWF Time - Fieldglass report export V2 {config.instance}',
        ...
        has_invalid_report_field = rail.IfOperator(
            ...
            test=lambda: not rail.result("get_field_glass_report_batch_results")['reportGenerationResults'][0]['payload'].startswith(
                'User Last Name,User First Name,Entry Date,Timesheet Period,Timesheet Start Date,Attendance Type Code,Rate Type,Total Hrs,Company Code Code'),
        ...
        has_compassuserdata >> rail.Label(
            ...
            load_csv_datawithlocation >> create_collection_datawithlocation >> emea_task >> amer_task >> apac_task
        return dag
