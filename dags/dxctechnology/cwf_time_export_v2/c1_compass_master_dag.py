...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'dxctechnology_cwf_time_export_c1_compass_master_v2_{config.instance}',
        description=f'DXCTechnology_CWF Time export - Master V2 {config.instance}',
        ...
        process_c1_time_export = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'dxctechnology_cwf_time_export_c1_child_v2_{config.instance}',
            execution_timeout=timedelta(days=14),
            conf=lambda: request_payload.get_c1_time_export_conf(rail.result(
                "current_export_name"), rail.result("get_data_For_all_past_time_exports_for_C1"))
        )

        process_compass_time_export = rail.TriggerDagRunOperator(
            task_id='process_compass_time_export',
            retries=0,
            trigger_dag_id=f'dxctechnology_cwf_time_export_compass_child_v2_{config.instance}',
            execution_timeout=timedelta(days=14),
            # pylint: disable=line-too-long
            conf=lambda: request_payload.get_compass_time_export_conf(rail.result(
                "current_export_name"), rail.result("get_data_For_all_past_time_exports_for_C1"), config)
        )
        ...
        get_all_time_download_scripts >> get_all_employee_groups >> log_message_fileformat_uri >> log_message_contractor_uri >> \
            ...
            get_cwf_data >> get_data_For_all_past_time_exports_for_C1 >> completed_exports_list >> current_export_name >> \
