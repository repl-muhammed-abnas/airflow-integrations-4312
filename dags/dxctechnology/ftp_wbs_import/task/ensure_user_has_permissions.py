...
def end_date_check(end_date):
    end_date_format = str(dateutil.parser.parse(
        end_date)).split(' ', maxsplit=1)[0]
...
def ensure_user_has_permissions(field):
    ...
    with rail.TaskGroup(group_id=f'ensure_{field}_has_permissions', prefix_group_id=False):
            ...
            if field == 'projectmanager':
                ...
                if useruri:
                    if status == 'True':
                        ...
                        if end_date is None or end_date == '' or end_date_check(end_date):
                            should_enable = True
                            should_apply = True
                        else:
        ...
        determine_updates >> enable_user_needed >> rail.Label(
            'Yes') >> enable_user >> should_apply_to_project >> rail.Label('Yes') >> get_permission_sets
        determine_updates >> is_user_permanently_disabled >> rail.Label(
            "Yes") >> record_user_disabled >> should_apply_to_project
        get_permission_sets >> has_admin_policy >> rail.Label(
            'Yes') >> needed_permissions_granted
        get_permission_sets >> has_projectmanagement_policy >> rail.Label(
            'Yes') >> needed_permissions_granted
        get_permission_sets >> has_manager_permissions >> rail.Label(
            'Yes') >> needed_permissions_granted
        ...
        has_admin_policy >> rail.Label(
            'No') >> assign_admin_policy >> needed_permissions_granted
        has_projectmanagement_policy >> rail.Label(
            'No') >> assign_projectmanager_policy >> needed_permissions_granted
        has_manager_permissions >> rail.Label(
            'No') >> assign_manager_permissions >> needed_permissions_granted
