...
def validate_timexport_report_start_end_date_format(dag_run):
    export_start_date = dag_run.conf['timeexport']['start_date']
    if not isinstance(export_start_date, dict):
        export_start_date_format = {"day": 1, "month": 1, "year": 2024}
        raise AirflowFailException(f"Expected timeexport start date to be dict in {export_start_date_format} format. Received {dag_run.conf['timeexport']['start_date']}")
    datetime(year=export_start_date['year'],
             month=export_start_date['month'],
             day=export_start_date['day'])
    # This part will fail if the format is not correct
    datetime.strptime(dag_run.conf['report']['start_date'], "%Y/%m/%d")
    datetime.strptime(dag_run.conf['report']['end_date'], "%Y/%m/%d")

def get_time_export_name(config, dag_run) -> dict:
    validate_timexport_report_start_end_date_format(dag_run)
