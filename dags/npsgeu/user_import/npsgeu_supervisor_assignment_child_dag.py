...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'npsgeu_user_import_supervisor_assignment_child_{config.instance}',
        description=f'NPSGEU_Supervisor Assignment {config.instance}',
        ...
        def get_supervisoruser_details(response, dag_run):
            ...
            return {
                ...
                'todayday': int(datetime.now().strftime('%d')),
                'todaymonth': int(datetime.now().strftime('%m')),
                'todayyear': int(datetime.now().strftime('%Y')),
        ...
        update_supervisor_assignment_schedule_over_date_range_19 = rail.RepliconServiceOperator(
            ...
            data={
                ...
                "dateRange": {
                    "startDate": {
                        "year": "{{ dag_run.conf.supeffectivedate_DS.year }}",
                        "month": "{{ dag_run.conf.supeffectivedate_DS.month }}",
                        "day": "{{ dag_run.conf.supeffectivedate_DS.day }}"
                    },
                    "endDate": null,
        ...
        if_request_action_equals_to_update_17 >> rail.Label(
            'Yes') >> update_supervisor_assignment_schedule_over_date_range_19 >> if_error_present
        if_request_action_equals_to_update_17 >> rail.Label(
            'No') >> if_error_present
