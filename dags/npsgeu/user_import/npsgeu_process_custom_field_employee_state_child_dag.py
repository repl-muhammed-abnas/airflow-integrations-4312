...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'npsgeu_user_import_process_custom_field_employee_state_child_{config.instance}',
        description=f'NPSGEU_Process Custom field (employee state)- Child V1.0 {config.instance}',
        ...
        create_list_5 = rail.CreateCollectionOperator(
            ...
            name="employeestatefromreplicon",
        )

        query_list_checkfornewdropdownvalues_6 = rail.QueryCollectionOperator(
            task_id='query_list_checkfornewdropdownvalues_6',
            query="""SELECT employeestatefromfeed.employeestate FROM  employeestatefromfeed WHERE
                LOWER( employeestatefromfeed.employeestate) NOT IN ( SELECT DISTINCT LOWER( employeestatefromreplicon.displayText) FROM
                employeestatefromreplicon )""",
        )
