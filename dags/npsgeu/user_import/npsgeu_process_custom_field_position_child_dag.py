...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'npsgeu_user_import_process_custom_field_position_child_{config.instance}',
        description=f'NPSGEU_Process Custom field (Position)- Child V1.0 {config.instance}',
        ...
        create_list_5 = rail.CreateCollectionOperator(
            ...
            name="positionfromreplicon",
        )

        query_list_checkfornewdropdownvalues_6 = rail.QueryCollectionOperator(
            task_id='query_list_checkfornewdropdownvalues_6',
            query="""SELECT  positionfromfeed.positionoption FROM positionfromfeed WHERE
                LOWER( positionfromfeed.positionoption) NOT IN ( SELECT DISTINCT LOWER( positionfromreplicon.displayText) FROM  positionfromreplicon )""",
        )
