...
from npsgeu.user_import.mappers.npsg_permission_mapper_mapper import npsg_permission_mapper
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'npsgeu_user_import_master_{config.instance}',
        description=f'NPSGEU_User import - Master {config.instance}',
        ...
        if_5c3be11b_name_starts_with_replicon_eu_skills_4 = rail.IfOperator(
            task_id='if_5c3be11b_name_starts_with_replicon_eu_skills_4',
            test='''{{ result('new_file_sensor') | file_name | starts_with('Replicon_EU_Skills') }}''',
            yes_task="parse_csv_11",
            no_task="parse_csv_15",
        )
        ...
        create_csv_lines_12 = rail.WriteCSVFileOperator(
            ...
            row=lambda item: [
                ...
                item['Skill Level'] or '',
                item['License / Certificate'] or '',
                item['Issuing Organization'] or '',
                item['Issue Date'] or '',
                item['Expiry Date'] or '',
                rail.find_first_by_attr_and_get_attr(rail.result(
                    'load_enabled_users'), 'Login Name', item['Login Name'], 'UserUri', '')
            ],
        )

        trigger_skills_processing_child = rail.TriggerDagRunOperator(
            task_id='trigger_skills_processing_child',
            retries=0,
            trigger_dag_id=f'npsgeu_user_import_skills_processing_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
                "inputdata": "{{result('create_csv_lines_12')}}",
        ...
        create_csv_lines_create_m_d5filefor_inputfile_16 = rail.WriteCSVFileOperator(
            ...
            row=lambda item: [
                ...
                item['Payrules'] or '',
                item['Effective Date'] or '',
                item['Primary Role'] or '',
                item['Additional Roles'] or '',
                hashlib.md5(str(str(item['First Name']) + "_" + str(item['Last Name']) + "_" + str(item['Email Address']) + "_" +
                    ...
                    str(item['Time Zone']) + "_" + str(item['Holiday Calendar']) + "_" + str(item['Payrules']) + "_" + str(item['Effective Date']) + "_" +
                    str(item['Primary Role']) + "_" + str(item['Additional Roles'])).encode('utf-8')).hexdigest()
            ],
        )
        ...
        download_downloadthereferencefile_17 = rail.SFTPDownloadFileOperator(
            ...
            sftp_conn_id=config.sftp_conn_id,
            remote_filepath=config.reference_filepath + 'npsgeu_reference.csv'
        )
        ...
        if_query_list_changedrecordswith_mandatoryfields_29_rows_greater_than_0_30 = rail.IfOperator(
            ...
            yes_task="get_all_roles_31",
            no_task="search_log_entries",
        )

        get_all_roles_31 = rail.RepliconServiceOperator(
            task_id='get_all_roles_31',
            endpoint="/services/ProjectRoleService1.svc/GetAllRoles",
        )

        def get_additional_roles(records):
            additionalroles = []
            for record in records:
                additionalroles + [role.strip() for role in (record['Additional Roles']).split('|')]
            return list(set(additionalroles))

        def get_projectroles():
            input_file_data = rail.load_all_records(
                rail.result('parse_csv_15'))
            all_roles_in_replicon = rail.result('get_all_roles_31')
            primaryroles = list({record['Primary Role'] for record in input_file_data}
                                ) if input_file_data and input_file_data[0]['Primary Role'] else []
            additionalroles = get_additional_roles(
                input_file_data) if input_file_data and input_file_data[0]['Additional Roles'] else []
            existingroles = [role['name'] for role in all_roles_in_replicon]
            merge_list = primaryroles + additionalroles
            add_list = (set(merge_list).difference(existingroles))
            final_list = list(map(lambda item: {"name": item}, list(add_list)))
            return final_list

        get_project_roles = rail.PythonOperator(
            task_id='get_project_roles',
            python_callable=get_projectroles
        )

        if_output_rolestoadd_greater_than_0_33 = rail.IfOperator(
            task_id='if_output_rolestoadd_greater_than_0_33',
            test=lambda: len(rail.result('get_project_roles')) > 0,
            yes_task="foreach_output_34",
            no_task="get_all_custom_fields_42",
        )

        foreach_output_34 = rail.ForEachOperator(
            task_id='foreach_output_34',
            items=lambda: rail.result('get_project_roles'),
            start_task='put_project_role_35',
            end_task='foreach_output_34_end'
        )

        put_project_role_35 = rail.RepliconServiceOperator(
            task_id='put_project_role_35',
            endpoint="/services/ProjectRoleService1.svc/PutProjectRole",
            data={
                "projectRoleUri": {
                    "target": {
                        "uri": null,
                        "name": "{{ result('foreach_output_34').name }}"
                    },
                    "name": "{{ result('foreach_output_34').name }}",
                    "description": null,
                    "isArchived": "false",
        ...
        foreach_output_34_end = rail.EmptyOperator(
            task_id='foreach_output_34_end',
        )
        ...
        trigger_child_to_process_division = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'npsgeu_user_import_process_custom_field_division_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        trigger_child_to_process_position = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'npsgeu_user_import_process_custom_field_position_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        trigger_child_to_process_employeestate = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'npsgeu_user_import_process_custom_field_employee_state_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        npsg_permission_mapper_search_entries_63 = rail.PythonOperator(
            ...
            python_callable=lambda:  list(
                filter(lambda x: x["allowed"] == "yes", npsg_permission_mapper))
        )
        ...
        invoke_custom_ruby_code_65 = rail.PythonOperator(
            ...
            python_callable=lambda: {
                "user": rail.find_first_by_attr_and_get_attr(rail.result('load_enabled_users'), 'Employee ID', rail.result(
                    'foreach_query_list_changedrecordswith_mandatoryfields_29_64')['employeeid'], 'User Name', ''),
                "useruri": rail.find_first_by_attr_and_get_attr(rail.result('load_enabled_users'), 'Employee ID', rail.result(
                    'foreach_query_list_changedrecordswith_mandatoryfields_29_64')['employeeid'], 'UserUri', ''),
        ...
        def get_required_permission_uris_to_assign():
            permissionsets = rail.smartjoin_by_delim([entry['permission_set'] for entry in rail.result(
                ...
                [entry['permission_set'] for entry in rail.result('npsg_permission_mapper_search_entries_63') if entry['job_family'] == rail.result(
        ...
        if_output_user_blank_73 = rail.IfOperator(
            ...
            yes_task="trigger_dag_run_npsgeu_user_import_npsgeu_create_user_v1_0async_74",
            no_task="trigger_dag_run_npsgeu_user_import_npsgeu_update_user_v1_0async_76",
        )

        def get_date_obj(datestring):
            dateobj = datetime.strptime(
                datestring, '%m/%d/%Y') if datestring else ''
            return {
                'day': dateobj.day if datestring else '',
                'month': dateobj.month if datestring else '',
                'year': dateobj.year if datestring else ''
            }
            ...
            conf = {
                ...
                "employeestateuri": rail.find_first_by_attr_and_get_attr(rail.result(
                    'get_all_custom_field_drop_down_optionsemployeestate_53'), 'displayText', user['employeestate'], 'uri', '') if user[
                        'employeestate'] else '',
                "startdate_DS": get_date_obj(user['startdate']),
                "enddate_DS": get_date_obj(user['enddate']),
                "today_DS": get_today_date(),
                "effective_DS": get_date_obj(user['effectivedate']),
        ...
        trigger_dag_run_npsgeu_user_import_npsgeu_create_user_v1_0async_74 = rail.TriggerDagRunOperator(
            task_id='trigger_dag_run_npsgeu_user_import_npsgeu_create_user_v1_0async_74',
            retries=0,
            trigger_dag_id=f'npsgeu_user_import_create_user_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
        ...
        add_create_user_dag_id = rail.SetVariableOperator(
            ...
            append=True,
            #pylint: disable = line-too-long
            value="{{result('trigger_dag_run_npsgeu_user_import_npsgeu_create_user_v1_0async_74')}}"
        )

        trigger_dag_run_npsgeu_user_import_npsgeu_update_user_v1_0async_76 = rail.TriggerDagRunOperator(
            task_id='trigger_dag_run_npsgeu_user_import_npsgeu_update_user_v1_0async_76',
            retries=0,
        ...
        add_update_user_dag_id = rail.SetVariableOperator(
            ...
            append=True,
            #pylint: disable = line-too-long
            value="{{result('trigger_dag_run_npsgeu_user_import_npsgeu_update_user_v1_0async_76')}}"
        )
        ...
        trigger_dag_run_supervisor_assignment_child = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'npsgeu_user_import_supervisor_assignment_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: {
                ...
                "supeffectivedate_DS": get_date_obj(item['properties']['effectivedate']),
                "userimportlogtable": rail.result('create_user_import_log_table'),
                "supervisorlookup": rail.result('create_supervisor_check_lookup_table')
            }
        )
        ...
        rename_archivethereferencefile_99 = rail.SFTPMoveFileOperator(
            ...
            new_filename=config.archive_filepath +
            '''{{ dag_run_ecid() }}_npsgeu_reference.csv''',
            existing_filename=config.reference_filepath + '''npsgeu_reference.csv''',
        )
        ...
        can_run_batch_task >> rail.Label(
            'No') >> log_formatted_time >> get_enabled_users_report_details >> run_enabled_users_report_details
        ...
        if_generate_report_5_payload_not_starts_with_usernameloginnameemployeeiduseruriuserenddatedaydiff_8 >> rail.Label(
            'No') >> parse_csv_10 >> load_enabled_users >> if_5c3be11b_name_starts_with_replicon_eu_skills_4
        if_5c3be11b_name_starts_with_replicon_eu_skills_4 >> rail.Label(
            'Yes') >> parse_csv_11 >> create_csv_lines_12 >> trigger_skills_processing_child >> log_to_sumo
        if_5c3be11b_name_starts_with_replicon_eu_skills_4 >> rail.Label(
            'No') >> parse_csv_15 >> create_csv_lines_create_m_d5filefor_inputfile_16 >> download_downloadthereferencefile_17
        ...
        if_query_list_changedrecordswith_mandatoryfields_29_rows_greater_than_0_30 >> rail.Label(
            'Yes') >> get_all_roles_31 >> get_project_roles >> if_output_rolestoadd_greater_than_0_33
        if_output_rolestoadd_greater_than_0_33 >> rail.Label(
            'Yes') >> foreach_output_34 >> put_project_role_35 >> foreach_output_34_end
        foreach_output_34 >> foreach_output_34_end >> get_all_custom_fields_42
        if_output_rolestoadd_greater_than_0_33 >> rail.Label(
            'No') >> get_all_custom_fields_42 >> query_users_in_file_tobe_assigned_as_supervisor
        ...
        if_output_user_blank_73 >> rail.Label(
            'Yes') >> trigger_dag_run_npsgeu_user_import_npsgeu_create_user_v1_0async_74 >> add_create_user_dag_id
        add_create_user_dag_id >> foreach_query_list_changedrecordswith_mandatoryfields_29_64_end
        if_output_user_blank_73 >> rail.Label(
            'No') >> trigger_dag_run_npsgeu_user_import_npsgeu_update_user_v1_0async_76 >> add_update_user_dag_id
