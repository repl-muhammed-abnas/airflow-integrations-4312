...
def create_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.sigroup_locations_dag_id,
       description="sigroup user import locations child",
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task="batch_task",
            no_task="get_enabled_locations"
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id="batch_task",
            execution_timeout=timedelta(days=config.execution_timeout_days),
            start_task="get_enabled_locations",
            end_task="batch_end"
        )
        ...
        create_existing_locations_collection = rail.CreateCollectionOperator(
            task_id="create_existing_locations_collection",
            source='{{result("get_enabled_locations")|to_json}}',
            name="existing_locations"
        )

        query_new_locations = rail.QueryCollectionOperator(
            task_id="query_new_locations",
            query="""SELECT * FROM query_locations_from_feed_file WHERE location NOT IN
            (SELECT DISTINCT displayText from existing_locations )"""
        )

        if_new_locations = rail.IfOperator(
            task_id="if_new_locations",
            test='{{result("query_new_locations", "length") > 0}}',
            yes_task="create_new_locations_in_replicon",
            no_task="batch_end"
        )

        create_new_locations_in_replicon = rail.RepliconServiceCallForEachItemOperator(
            task_id="create_new_locations_in_replicon",
        ...
        get_enabled_locations >>\
            create_existing_locations_collection >>\
            query_new_locations >>\
            if_new_locations >> rail.Label("Yes") >>\
            create_new_locations_in_replicon >> batch_end
        if_new_locations >> rail.Label("No") >> batch_end
