...
from sigroup.payroll_export_singapore.mappers.sigroup_payelements_mapper import sigroup_payelements_mapper
from sigroup.payroll_export_singapore.mappers.sigroup_singapore_calendar_mapper import sigroup_singapore_calendar
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'sigroup_payroll_export_singapore_per_payroll_id_child_{config.instance}',
        description=f'SiGroup - Singapore_per_payroll_id_child {config.instance}',
        ...
        get_entries_from_calendar_mapper=rail.PythonOperator(
            ...
            python_callable= lambda:  (list(filter(lambda x: x["export"] == "Yes" and x["fileformat"] == "Singapore" and x["repliconextractdate"] == datetime.now().strftime("%d-%m-%Y"), sigroup_singapore_calendar)))
        )
