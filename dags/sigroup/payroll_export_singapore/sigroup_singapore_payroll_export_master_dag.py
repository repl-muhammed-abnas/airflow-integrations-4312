...
from sigroup.payroll_export_singapore.mappers import sigroup_singapore_calendar_mapper
import rail
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'sigroup_payroll_export_singapore_master_{config.instance}',
        description=f'SiGroup_Payroll_Export_Singapore_Master {config.instance}',
        ...
        def check_schedule_present():
            """
            Production-ready schedule validation for Singapore.
            Checks today's date against Singapore calendar mapper entries.
            """
            ...
            return list(filter(
                lambda schedule: (
                    ...
                    schedule['fileformat'] == "Singapore" and
                    schedule['export'] == "Yes"
                ),
                sigroup_singapore_calendar_mapper.sigroup_singapore_calendar
            ))
        ...
        filter_valid_entries_from_schedule = rail.PythonOperator(
            ...
            python_callable=lambda: list(filter(lambda entry: (
                rail.result('get_all_scripts') and
                rail.find_first_by_attr_and_get_attr(rail.result('get_all_scripts'), 'displayText', entry['fileformat'], 'uri', '')
            ) and (
                rail.result('get_enabled_divisions') and 
                rail.find_first_by_attr_and_get_attr(rail.result('get_enabled_divisions'), 'displayText', entry['businessunit'], 'uri', '')
            ) and (
                entry['fileformat'] == 'Singapore'
            ), rail.result('get_schedule_from_calendar')))
        )

        trigger_singapore_business_unit_payroll_data_export_child = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_singapore_business_unit_payroll_data_export_child',
            retries=0,
            items=lambda: rail.result('filter_valid_entries_from_schedule'),
            trigger_dag_id=f'sigroup_payroll_export_singapore_business_unit_child_{config.instance}',
        ...
        wait_for_trigger_singapore_business_unit_payroll_data_export_child = rail.WaitForDagRunsSensor(
            task_id='wait_for_trigger_singapore_business_unit_payroll_data_export_child',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs='{{ result("trigger_singapore_business_unit_payroll_data_export_child") }}'
        )
        ...
        filter_valid_entries_from_schedule >> trigger_singapore_business_unit_payroll_data_export_child
        trigger_singapore_business_unit_payroll_data_export_child >> wait_for_trigger_singapore_business_unit_payroll_data_export_child
        wait_for_trigger_singapore_business_unit_payroll_data_export_child >> finish >> log_to_sumo
