...
import rail
from sigroup.payroll_export_singapore.mappers.sigroup_valid_paycodenames_mapper import valid_paycodename
null=None

def create_dag(config):
    # pylint: disable=too-many-statements
    with rail.create_airflow_dag(
        dag_id=f'sigroup_payroll_export_singapore_business_unit_child_{config.instance}',
        description=f'SiGroup - Singapore Business Unit Payroll Export_Child {config.instance}',
        ...
        update_pay_run_name=rail.RepliconServiceOperator(
            ...
            data={
              ...
              "name": "Singapore_{{ result('log_required_filename') }}"
            }
        )
        ...
        write_csv_for_payrolldata_with_required_dateformat = rail.WriteCSVFileOperator(
            ...
            row=lambda item:[
                item['Employee ID'],
                item['Entry Date'] if item['Entry Date'] else '',
        ...
        trigger_child_per_distinct_payroll_id_including_timeoffdata=rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'sigroup_payroll_export_singapore_per_payroll_id_child_{config.instance}',
