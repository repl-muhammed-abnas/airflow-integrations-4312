...
from sigroup.payroll_export_uk.mappers.sigroup_payelements_mapper import sigroup_payelements_mapper
from sigroup.payroll_export_uk.mappers.sigroup_uk_calendar_mapper import sigroup_uk_calendar
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'sigroup_payroll_export_uk_per_payroll_id_child_{config.instance}',
        description=f'SiGroup - UK_per_payroll_id_child {config.instance}',
        ...
        create_payload_per_payrollid = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM finaldata WHERE finaldata.cloudpaypaycode = '{{dag_run.conf.payrollid}}'"""
        )
        ...
        get_valid_mapper_entries=rail.PythonOperator(
            ...
            python_callable= lambda: (list(filter(lambda x: x["exported"] == "Yes" and x['country'] == 'UK' , sigroup_payelements_mapper)))
        )
        ...
        get_entries_from_calendar_mapper=rail.PythonOperator(
            ...
            python_callable= lambda:  (list(filter(lambda x: x["export"] == "Yes" and x["fileformat"] == "UK" and x["repliconextractdate"] == datetime.now().strftime("%d-%m-%Y"), sigroup_uk_calendar)))
        )

        get_file_name=rail.PythonOperator(
            task_id='get_file_name',
            python_callable= lambda dag_run:  "SIGTT" + "-" + dag_run.conf['filenamecounter'] + "-" +
