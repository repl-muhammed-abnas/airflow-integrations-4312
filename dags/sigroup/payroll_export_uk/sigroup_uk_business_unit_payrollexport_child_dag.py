...
import rail
from sigroup.payroll_export_uk.mappers.sigroup_valid_paycodenames_mapper import valid_paycodename
null=None

def create_dag(config):
    # pylint: disable=too-many-statements
    with rail.create_airflow_dag(
        dag_id=f'sigroup_payroll_export_uk_business_unit_child_{config.instance}',
        description=f'SiGroup - UK Business Unit Payroll Export_Child {config.instance}',
        ...
        update_pay_run_name=rail.RepliconServiceOperator(
            ...
            data={
              ...
              "name": "UK_{{ result('log_required_filename') }}"
            }
        )
        ...
        get_valid_paycode_names = rail.PythonOperator(
            ...
            python_callable=lambda: get_all_required_pacodes(valid_paycodename)
        )

        query_list_final_data=rail.QueryCollectionOperator(
            task_id='query_list_final_data',
            name='finaldata',
            query="""SELECT * FROM  finalpayrolldata ORDER BY  finalpayrolldata.employeeid ASC""",
        )
        ...
        query_users_with_enddate=rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM  reportdtata WHERE  reportdtata.status="Disabled" AND  NULLIF(userenddate,'') IS NOT NULL  AND
                  reportdtata.businessunit='{{ dag_run.conf.businessunit }}'""",
        )
        ...
        if_users_with_enddate_present=rail.IfOperator(
            ...
            no_task="if_timeoffreportdata_has_records",
        )
        ...
        query_end_user_data=rail.QueryCollectionOperator(
            ...
            name='enduserdata',
            query="""SELECT * FROM  timeoffreportdtata WHERE   timeoffreportdtata.timeofftypedescription LIKE 'Export Balance on Termination%'"""
        )

        if_timeoffreportdata_has_records=rail.IfOperator(
            task_id='if_timeoffreportdata_has_records',
        ...
        query_distinct_payrollid_with_timeoff_data=rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT finaldata.cloudpaypaycode FROM  finaldata WHERE NOT  finaldata.cloudpaypaycode= '""' AND
                  NULLIF(cloudpaypaycode,'') IS NOT NULL UNION SELECT DISTINCT  timeoffreportdtata.cloudpaypaycode FROM  timeoffreportdtata WHERE
                  NOT  timeoffreportdtata.cloudpaypaycode= '""' AND  NULLIF(cloudpaypaycode,'') IS NOT NULL AND
                  timeoffreportdtata.timeofftypedescription  LIKE 'Export Balance on Termination%'"""
        )
        ...
        if_distinct_payrollids_including_timeoffdata_present = rail.IfOperator(
            task_id = 'if_distinct_payrollids_including_timeoffdata_present',
            ...
            yes_task='trigger_child_per_distinct_payroll_id_including_timeoffdata',
            no_task='trigger_child_per_payroll_id'
        )

        trigger_child_per_distinct_payroll_id_including_timeoffdata=rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_child_per_distinct_payroll_id_including_timeoffdata',
            retries=0,
            items="{{ result('query_distinct_payrollid_with_timeoff_data') }}",
            trigger_dag_id=f'sigroup_payroll_export_uk_per_payroll_id_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        wait_for_completion_of_trigger_child_per_distinct_payroll_id_including_timeoffdata = rail.WaitForDagRunsSensor(
            task_id='wait_for_completion_of_trigger_child_per_distinct_payroll_id_including_timeoffdata',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs='{{ result("trigger_child_per_distinct_payroll_id_including_timeoffdata") }}'
        )
        ...
        if_query_distinct_payrollid_has_no_data=rail.IfOperator(
            ...
            #pylint: disable = line-too-long
            test='''{{ result('query_distinct_payrollid','length') < 1 and (result('query_distinct_payrollid_with_timeoff_data') | is_falsy or result('query_distinct_payrollid_with_timeoff_data','length') < 1)}}''',
        ...
        send_mail_no_data_for_business_unit=rail.EmailOperator(
            ...
            #pylint: disable = line-too-long
            subject='''{{ get_company_key() }}| Payroll extract completed with no data for {{ dag_run.conf.businessunit }} - {{ result('get_time_in_required_formats').format1 }} ''',
        ...
        create_collection_timeoff_report_data >> query_end_user_data >> if_timeoffreportdata_has_records
        if_query_users_to_consider_has_data >> rail.Label('No') >> if_timeoffreportdata_has_records
        if_users_with_enddate_present >> rail.Label('No') >> if_timeoffreportdata_has_records
        if_timeoffreportdata_has_records >> rail.Label('Yes')  >> query_distinct_payrollid_with_timeoff_data >> query_distinct_payrollid
        if_timeoffreportdata_has_records >> rail.Label('No') >> query_distinct_payrollid >> if_distinct_payrollids_including_timeoffdata_present
        if_distinct_payrollids_including_timeoffdata_present >> rail.Label('Yes') >> trigger_child_per_distinct_payroll_id_including_timeoffdata
        if_distinct_payrollids_including_timeoffdata_present >> rail.Label('No') >> trigger_child_per_payroll_id
        trigger_child_per_distinct_payroll_id_including_timeoffdata >> wait_for_completion_of_trigger_child_per_distinct_payroll_id_including_timeoffdata
        wait_for_completion_of_trigger_child_per_distinct_payroll_id_including_timeoffdata >> if_query_distinct_payrollid_has_no_data
