...
import rail
from sigroup.payroll_export_korea.mappers.sigroup_valid_paycodenames_mapper import valid_paycodename_korea
null=None

def create_dag(config):
    # pylint: disable=too-many-statements
    with rail.create_airflow_dag(
        dag_id=f'sigroup_payroll_export_korea_business_unit_child_{config.instance}',
        description=f'SiGroup - Korea Business Unit Payroll Export_Child {config.instance}',
        ...
        update_pay_run_name=rail.RepliconServiceOperator(
            ...
            data={
              ...
              "name": "Korea_{{ result('log_required_filename') }}"
            }
        )
        ...
        get_valid_paycode_names = rail.PythonOperator(
            ...
            python_callable=lambda: get_all_required_pacodes(valid_paycodename_korea)
        )
        ...
        query_users_with_enddate=rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM  reportdata WHERE  reportdata.status="Disabled" AND  NULLIF(userenddate,'') IS NOT NULL  AND
                  reportdata.businessunit='{{dag_run.conf.businessunit}}'""",
        )
        ...
        trigger_child_per_payroll_id_with_timeoffdata=rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'sigroup_payroll_export_korea_per_payroll_id_child_{config.instance}',
