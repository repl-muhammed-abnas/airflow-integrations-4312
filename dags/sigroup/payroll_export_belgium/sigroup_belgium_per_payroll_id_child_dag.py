...
from sigroup.payroll_export_belgium.mappers.sigroup_payelements_mapper import sigroup_payelements_mapper
from sigroup.payroll_export_belgium.mappers.sigroup_belgium_calendar_mapper import sigroup_belgium_calendar
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'sigroup_payroll_export_belgium_per_payroll_id_child_{config.instance}',
        description=f'SiGroup - Belgium_per_payroll_id_child {config.instance}',
        ...
        get_entries_from_calendar_mapper=rail.PythonOperator(
            ...
            python_callable= lambda:  (list(filter(lambda x: x["export"] == "Yes" and x["fileformat"] == "Belgium" and x["repliconextractdate"] == datetime.now().strftime("%d-%m-%Y"), sigroup_belgium_calendar)))
        )
