...
from sigroup.payroll_export_china.mappers.sigroup_payelements_mapper import sigroup_payelements_mapper
from sigroup.payroll_export_china.mappers.sigroup_china_calendar_mapper import sigroup_china_calendar
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'sigroup_payroll_export_china_per_payroll_id_child_{config.instance}',
        description=f'SiGroup - China_per_payroll_id_child {config.instance}',
        ...
        compose_csv_for_data_with_paycode_type = rail.WriteCSVFileOperator(
            ...
            row=lambda item:[
                ...
                item['paycodehours'],
                item['paycodepay'],
                rail.find_first_by_attr_and_get_attr(rail.result('get_valid_mapper_entries'),'identifier',item['paycodecode'],'value','') or 'Temporary Pay Element'
            ]
        )
        ...
        get_entries_from_calendar_mapper=rail.PythonOperator(
            ...
            python_callable= lambda:  (list(filter(lambda x: x["export"] == "Yes" and x["fileformat"] == "China" and x["repliconextractdate"] == datetime.now().strftime("%d-%m-%Y"), sigroup_china_calendar)))
        )
