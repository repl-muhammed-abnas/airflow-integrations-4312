...
import rail
from sigroup.payroll_export_china.mappers.sigroup_valid_paycodenames_mapper import valid_paycodename_china
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'sigroup_payroll_export_china_business_unit_child_{config.instance}',
        description=f'SiGroup - China Business Unit Payroll Export_Child {config.instance}',
        ...
        update_pay_run_name=rail.RepliconServiceOperator(
            ...
            data={
              ...
              "name": "China_{{ result('log_required_filename') }}"
            }
        )
        ...
        get_valid_paycode_names = rail.PythonOperator(
            ...
            python_callable=lambda: get_all_required_pacodes(valid_paycodename_china)
        )
        ...
        if_payload_doesnt_match_required_column_order=rail.IfOperator(
            ...
            test='''{{not (result('run_user_details_report.get_report_result')| load_json_artifact).reportGenerationResults[0].payload | starts_with('User Name,Employee ID,User End Date,User Status,User URI,CurrentHourlyPayroll') }}''',
        ...
        query_users_with_enddate=rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM  reportdtata WHERE  reportdtata.status="Disabled" AND  NULLIF(userenddate,'') IS NOT NULL"""
        )
        ...
        if_distinct_payrollid_including_timeoff_data_present = rail.IfOperator(
            task_id = 'if_distinct_payrollid_including_timeoff_data_present',
            ...
            yes_task='trigger_child_per_payrollid_including_timeoffdata',
            no_task='trigger_child_per_payroll_id'
        )

        trigger_child_per_payrollid_including_timeoffdata = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_child_per_payrollid_including_timeoffdata',
            retries=0,
            items="{{ result('query_distinct_payrollid_with_timeoff_data') }}",
            trigger_dag_id=f'sigroup_payroll_export_china_per_payroll_id_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        wait_for_completion_of_trigger_child_per_payroll_id_including_timeoffdata = rail.WaitForDagRunsSensor(
            task_id='wait_for_completion_of_trigger_child_per_payroll_id_including_timeoffdata',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs='{{ result("trigger_child_per_payrollid_including_timeoffdata") }}'
        )
        ...
        if_timeoffreportdata_has_records >> rail.Label('No') >> query_distinct_payrollid >> if_distinct_payrollid_including_timeoff_data_present
        if_distinct_payrollid_including_timeoff_data_present >> rail.Label('Yes') >> trigger_child_per_payrollid_including_timeoffdata
        trigger_child_per_payrollid_including_timeoffdata >> wait_for_completion_of_trigger_child_per_payroll_id_including_timeoffdata
        wait_for_completion_of_trigger_child_per_payroll_id_including_timeoffdata >> if_query_distinct_payrollid_has_no_data
        if_distinct_payrollid_including_timeoff_data_present >> rail.Label('No') >> trigger_child_per_payroll_id
        trigger_child_per_payroll_id >> wait_for_completion_of_trigger_child_per_payroll_id >> if_query_distinct_payrollid_has_no_data
        if_query_distinct_payrollid_has_no_data >> rail.Label('Yes')  >> send_mail_no_data_for_business_unit >> send_success_mail
