...
from sigroup.payroll_export_china.mappers import sigroup_china_calendar_mapper
import rail
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'sigroup_payroll_export_china_master_{config.instance}',
        description=f'SiGroup_Payroll_Export_China_Master {config.instance}',
        ...
        def check_schedule_present():
            ...
            Checks today's date against calendar mapper entries for China payroll exports.
            This replicates the Workato filter:
            lookup("SiGroup_PayrollExport_Calendar", 
                   repliconextractdate: scheduled_time.strftime("%d-%m-%Y"), 
                   fileformat: ['Singapore','China'])
            """
            today = datetime.now().strftime("%d-%m-%Y")
            
            # Match Workato logic: look for entries with China fileformat and today's date
            valid_schedules = list(filter(
                lambda schedule: (
                    schedule['repliconextractdate'] == today and
                    schedule['fileformat'] == "China" and  # Focus on China for this DAG
                    schedule['export'] == "Yes"
                ),
                sigroup_china_calendar_mapper.sigroup_china_calendar
            ))
        ...
        # CORRECTED: Add child DAG triggering logic that matches Workato master recipe
        trigger_china_child_dags = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_china_child_dags',
            retries=0,
            items="{{ result('get_schedule_from_calendar') | to_json }}",
            trigger_dag_id=f'sigroup_payroll_export_china_business_unit_child_{config.instance}',
        ...
        wait_for_child_dags = rail.WaitForDagRunsSensor(
            ...
            execution_timeout=timedelta(days=14),
            dag_runs='{{ result("trigger_china_child_dags") }}'
        )
        ...
        get_all_scripts >> get_enabled_divisions >> get_schedule_from_calendar >> trigger_china_child_dags >> wait_for_child_dags >> finish >> log_to_sumo
