...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"solvaycore_project_import_to_replicon_process_update_project_child_{config.instance}",
        ...
        if_cost_type_and_object_class_are_present = rail.IfOperator(
            ...
            test=lambda dag_run:bool((dag_run.conf["costtype"].upper() == "OPEX"
                                        and dag_run.conf["objectclass"].upper() == "OVERHEAD") or
                                    (dag_run.conf["costtype"].upper() == "CAPEX" and
                                    dag_run.conf["objectclass"].upper() == "INVESTMENT")),
            yes_task="get_bulk_project_details",
        ...
        cost_type_and_object_class_are_present_errlog = rail.WriteLogOperator(
            ...
            log="{{dag_run.conf.lookuptable}}",
            message="Invalid Object class/Cost type.",
            severity="Exception",
        ...
        get_bulk_project_details = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response:
                                ...
                                if response[0]["projectDetails"] is not None
                                and "uri" in response[0]["projectDetails"]
                                else None
        )
        ...
        if_accolade_project_and_projecturi_not_present = rail.IfOperator(
            ...
            yes_task="write_exception_project_import_log",
            no_task="get_object_extension_definitions_for_project"
        )
        ...
        query_service_center_collection = rail.QueryCollectionOperator(
            ...
            query="""select * from servicecenterlist where code='{{dag_run.conf["companycode"]}}'"""
        )
        ...
        object_extension_tags_for_psfamily_errlog = rail.WriteLogOperator(
            ...
            log="{{dag_run.conf.lookuptable}}",
            message="Source System is not available",
            severity="Exception",
            properties=lambda dag_run:{
                "projectcode" : dag_run.conf["projectcode"],
                "projectdescription": dag_run.conf["projectdescription"],
        ...
        if_object_extension_tags_for_gbu = rail.IfOperator(
            ...
            yes_task="if_accolade_project_is_present",
        ...
        object_extension_tags_for_gbu_errlog = rail.WriteLogOperator(
            ...
            log="{{dag_run.conf.lookuptable}}",
            message="Global Business unit not available in Replicon.",
            severity="Exception",
            properties=lambda dag_run:{
                "projectcode" : dag_run.conf["projectcode"],
                "projectdescription": dag_run.conf["projectdescription"],
        ...
        if_accolade_project_is_present = rail.IfOperator(
            task_id="if_accolade_project_is_present",
            test=lambda dag_run:bool(dag_run.conf["accoladeprojectid"]),
            yes_task="update_project",
            no_task="update_project_with_enddate"
        )

        update_project = rail.RepliconServiceOperator(
            task_id="update_project",
            endpoint="/services/ProjectService1.svc/CreateProjectOrApplyModifications",
            data=request_payload.update_project_request
        )

        update_project_with_enddate = rail.RepliconServiceOperator(
            task_id="update_project_with_enddate",
            endpoint="/services/ProjectService1.svc/CreateProjectOrApplyModifications",
            data=request_payload.update_project_with_date_request,
        ...
        get_bulk_task_details = rail.RepliconServiceOperator(
            task_id="get_bulk_task_details",
            endpoint="/services/ProjectService1.svc/BulkGetTaskDetails",
            data=lambda: json.dumps({
                    "pageIndex": "1",
                    "pageSize": "5000",
                    "projectUris":[rail.result("get_bulk_project_details")]
                })
        )
        ...
        if_accolade_project_is_present1 = rail.IfOperator(
            task_id="if_accolade_project_is_present1",
            test=lambda dag_run:bool(dag_run.conf["accoladeprojectid"]),
            yes_task="search_object_type_in_mapper",
            no_task="update_project_task"
        )

        search_object_type_in_mapper = rail.PythonOperator(
            task_id="search_object_type_in_mapper",
            python_callable=custom_methods.get_list_of_subtasks
        )

        check_if_subtasks_exist = rail.IfOperator(
            task_id="check_if_subtasks_exist",
            test='{{result("search_object_type_in_mapper") | length > 0}}',
            yes_task="update_project_task",
            no_task="object_class_errlog"
        )

        object_class_errlog = rail.WriteLogOperator(
            task_id="object_class_errlog",
            log="{{dag_run.conf.lookuptable}}",
            message="Invalid Object class, Error while creating task.",
            severity="Exception",
            properties=lambda dag_run:{
                "projectcode" : dag_run.conf["projectcode"],
                "projectdescription": dag_run.conf["projectdescription"],
                ...
                "Task Code": dag_run.conf["wbscode"],
                "Status": "Exception",
                "Reason":"Invalid Object class, Error while creating task.",
                "Child jobid": rail.render_template('{{ecid()}}')
            }
        )
        update_project_task = rail.RepliconServiceOperator(
            task_id="update_project_task",
            endpoint="/services/TaskService1.svc/CreateTaskOrApplyModifications",
            data=request_payload.update_task_request,
            data_handler=lambda response:response["uri"]
        )

        if_accolade_project_is_present2 = rail.IfOperator(
            task_id="if_accolade_project_is_present2",
            test=lambda dag_run:bool(dag_run.conf["accoladeprojectid"]),
            yes_task="get_updated_task_details",
            no_task="write_sucess_project_import_log"
        )

        get_updated_task_details = rail.RepliconServiceOperator(
            task_id="get_updated_task_details",
            endpoint="/services/TaskService1.svc/GetChildrenTaskDetails",
            data=lambda: json.dumps({
                "parentUri":rail.result("update_project_task")
            })
        )

        update_task_hierarchy_for_accolade_project = rail.RepliconServiceOperator(
            task_id="update_task_hierarchy_for_accolade_project",
            endpoint="/services/TaskService1.svc/CreateTaskHierarchyOrApplyModifications",
            data=request_payload.accolade_task_hierarchy_request
        )

        write_exception_project_import_log = rail.WriteLogOperator(
            task_id="write_exception_project_import_log",
            log="{{dag_run.conf.lookuptable}}",
            message="Accolade Project is not available in replicon.",
            severity="Exception",
            properties=lambda dag_run:{
                "projectcode" : dag_run.conf["projectcode"],
                "projectdescription": dag_run.conf["projectdescription"],
        ...
        if_cost_type_and_object_class_are_present>>\
        rail.Label("Yes") >>get_bulk_project_details >>\
        get_project_leader_uri_if_present>>\
        if_accolade_project_and_projecturi_not_present>>\
        rail.Label("Yes") >> write_exception_project_import_log
        if_accolade_project_and_projecturi_not_present>>\
        rail.Label("No") >> \
        ...
        if_object_extension_tags_for_gbu >>\
        rail.Label("Yes") >> \
        if_accolade_project_is_present >>\
        rail.Label("Yes") >> update_project >>\
        get_object_extension_definitions_for_task >> get_bulk_task_details >>\
        get_object_extension_tags_for_log_system >>\
        if_accolade_project_is_present1 >>\
        rail.Label("Yes") >> search_object_type_in_mapper >>\
        check_if_subtasks_exist >> \
        rail.Label("Yes") >> update_project_task >>\
        if_accolade_project_is_present2 >>\
        rail.Label("Yes") >> get_updated_task_details >>\
        update_task_hierarchy_for_accolade_project >> write_sucess_project_import_log >> catch_and_log_errors
        if_accolade_project_is_present >>\
        rail.Label("No") >> update_project_with_enddate >>\
        get_object_extension_definitions_for_task >> get_bulk_task_details >>\
        get_object_extension_tags_for_log_system >>\
        if_accolade_project_is_present1 >>\
        rail.Label("No") >> update_project_task >>\
        if_accolade_project_is_present2 >>\
        rail.Label("No") >> write_sucess_project_import_log >> catch_and_log_errors
        check_if_subtasks_exist >> \
        rail.Label("No") >> object_class_errlog >> catch_and_log_errors >> log_to_sumo
    return dag
