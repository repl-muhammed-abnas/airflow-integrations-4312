...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.child_create_office_schedule_dag_id,
        description=f"Dataaxle User Import - Create Office Schedule child DAG {config.instance}",
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.max_active_run_create_office_schedules_child
    ) as dag:
        ...
        update_name = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: request_payload.update_name_request_payload(dag_run)
        )

        apply_schedule_pattern = rail.RepliconServiceOperator(
            task_id="apply_schedule_pattern",
            endpoint="/services/OfficeScheduleService1.svc/PutSimpleSchedulePattern",
            data=lambda dag_run: request_payload.apply_schedule_pattern_request_payload(dag_run)
        )
        ...
        can_use_batch >> rail.Label("No") >> start_process >> \
            create_new_draft >> update_name >> apply_schedule_pattern >> publish_draft >> finish
