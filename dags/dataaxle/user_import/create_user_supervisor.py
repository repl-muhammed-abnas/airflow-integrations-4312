...
def create_create_user_supervisor_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.child_create_user_supervisor_dag_id,
        description=f"Dataaxle User Import - Create User Supervisor child DAG {config.instance}",
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.max_active_run_create_user_supervisor_child,
        default_args={
        ...
        # ── Extract login name and check for duplicates (Workato steps 2-7) ────
        ...
        search_user_by_login = rail.RepliconServiceOperator(
            ...
            data=lambda: request_payload.build_bulk_get_users_payload(),
        )
        ...
        if_duplicate_login_name = rail.IfOperator(
            ...
            yes_task="stop_duplicate_user_creation",
            no_task="create_user",
        )
        ...
        # ── Create user via PutUser2 ───────────────────────────────────────────
        create_user = rail.RepliconServiceOperator(
            task_id="create_user",
            endpoint="/services/ImportService1.svc/PutUser2",
            data=lambda dag_run: request_payload.build_create_supervisor_user_payload(dag_run)
        )
        ...
        if_department_present = rail.IfOperator(
            ...
            no_task="if_any_supervisor_modification_present",
        )
        ...
        # ── Steps 11-22: Apply all user attribute modifications in one API call ─
        # Combines: holiday calendar, timezone, location, division, employee type
        # group, service center, and schedule policy. Uses update-schedule-over-
        # date-range with effectiveDate=null.
        if_any_supervisor_modification_present = rail.IfOperator(
            task_id="if_any_supervisor_modification_present",
            test="{{ (dag_run.conf.holiday_calendar | is_truthy) or "
                 ...
                 "(dag_run.conf.standard_hours | is_truthy) or "
                 "((dag_run.conf.payroll_dept_no | is_truthy) and (dag_run.conf.payroll_dept_name | is_truthy)) }}",
            yes_task="apply_supervisor_attribute_modifications",
            no_task="if_payroll_dept_no_cf_present",
        )

        apply_supervisor_attribute_modifications = rail.RepliconServiceOperator(
            task_id="apply_supervisor_attribute_modifications",
            endpoint="/services/ImportService1.svc/ApplyUserModifications3",
            data=lambda dag_run: request_payload.build_combined_supervisor_modifications_payload(dag_run),
        )

        # ── Custom field: Payroll Dept # ───────────────────────────────────────
        if_payroll_dept_no_cf_present = rail.IfOperator(
            task_id="if_payroll_dept_no_cf_present",
            test=lambda dag_run: dag_run.conf.get("payroll_department_number_uri") and dag_run.conf.get("payroll_department_no_drop_down_uri"),
        ...
        # ── Dependency chain ───────────────────────────────────────────────────
        can_use_batch >> rail.Label("Yes") >> batch_wrapper >> catch_and_log_error
        can_use_batch >> rail.Label("No") >> start_process >> extract_login_name >> search_user_by_login >> check_duplicate_login >> if_duplicate_login_name
        if_duplicate_login_name >> rail.Label("Yes") >> stop_duplicate_user_creation >> catch_and_log_error
        if_duplicate_login_name >> rail.Label("No") >> create_user

        create_user >> if_department_present
        if_department_present >> rail.Label("Yes") >> apply_department >> if_any_supervisor_modification_present
        if_department_present >> rail.Label("No") >> if_any_supervisor_modification_present

        # Steps 11-22: Apply all user attribute modifications in one combined call
        if_any_supervisor_modification_present >> rail.Label("Yes") >> apply_supervisor_attribute_modifications >> if_payroll_dept_no_cf_present
        if_any_supervisor_modification_present >> rail.Label("No") >> if_payroll_dept_no_cf_present
        ...
        if_supervisor_found >> rail.Label("No") >> add_user_creation_log >> catch_and_log_error
...
rail.for_each_instance(create_create_user_supervisor_dag)
