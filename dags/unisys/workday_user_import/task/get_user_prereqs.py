"""
Get User Prerequisites Task Group - Unisys Workday User Import

Retrieves all prerequisite data from Replicon needed for user processing.
This task group fetches reference data including locations, divisions, user types,
schedules, permissions, and other configuration data required for user creation/updates.

Key features:
    - Fetches all organizational hierarchies (locations, divisions, user types)
    - Retrieves timesheet periods and approval paths
    - Gets permission sets and policy sets
    - Fetches timezones and holiday calendars
    - Retrieves office schedules and activities
    - Creates new schedules when needed
    - Uses pagination for large datasets
    - Returns filtered and formatted data

Functions:
    get_user_prereqs_task_group(config): Creates the prerequisites task group
"""
from datetime import timedelta
import rail
from unisys.workday_user_import.utils import request_payload, response_filters
...
def get_user_prereqs_task_group(config):
    ...
    Replicon API Endpoints Used:
        - /services/DivisionListService1.svc/GetData: Division/cost center data
        - /services/LocationListService1.svc/GetData: Location hierarchy data
        - /services/EmployeeTypeGroupListService1.svc/GetData: User type groups
        - /services/TimesheetPeriodListService1.svc/GetData: Timesheet periods
        - /services/CustomFieldService1.svc/GetAllCustomFields: User custom fields
        - /services/PermissionSetService1.svc/GetAllPermissionSets: Permission sets
        - /services/PolicySetService1.svc/GetAllPolicySets: Policy sets
        - /services/TimesheetApprovalService1.svc/GetAllApprovalPaths: Approval paths
        - /services/InternationalizationService1.svc/GetAllTimeZones: Timezones
        - /services/OfficeScheduleService1.svc/GetAllOfficeSchedules: Office schedules
        - /services/HolidayCalendarService1.svc/GetAllHolidayCalendars: Holiday calendars
        - /services/DepartmentService1.svc/GetEnabledDepartments: Departments/PO IDs
        - /services/ActivityService1.svc/GetAllActivities: Activity types
        - /services/CustomFieldService1.svc/GetAllCustomFieldDropDownOptions: Dropdown options

    Task Group Results:
        - get_enabled_co_costcenters: Filtered division/cost center data
        - get_location_details: Filtered location hierarchy data
        - get_employeetype_groups_data: Filtered user type group data
        - get_all_timesheet_period_list: Timesheet period list
        - get_user_customfields: User custom field URIs
        - get_permission_sets: Permission set data
        - get_all_policy_sets: Policy set data
        - get_timesheet_approval_paths: Approval path data
        - get_all_timezones: Timezone data
        - get_all_office_schedule: Office schedule list
        - get_all_holiday_calendars: Holiday calendar data
        - get_all_enabled_purchase_order: Department/PO data
        - get_all_activity_uris: Activity URI list
        - get_all_user_status_dropdowns: User status dropdown options
    """
    ...
    with rail.TaskGroup(group_id='get_user_prereqs', prefix_group_id=False) as get_user_prereqs:
        ...
        get_enabled_co_costcenters = rail.RepliconServicePageOperator(
            task_id='get_enabled_co_costcenters',
            endpoint="/services/DivisionListService1.svc/GetData",
            data=request_payload.get_division_payload,
            page_handler=page_handler,
            all_result_data_handler=response_filters.filter_all_co_costcenters_data
        )
        ...
        get_all_enabled_purchase_order = rail.RepliconServiceOperator(
            task_id='get_all_enabled_purchase_order',
        ...
        get_all_user_status_dropdowns = rail.RepliconServiceOperator(
            ...
            data={
                'customFieldUri': "{{ result('get_user_customfields').user_status_uri }}"
            }
        )

        get_all_leave_type_dropdowns = rail.RepliconServiceOperator(
            task_id='get_all_leave_type_dropdowns',
            endpoint='/services/CustomFieldService1.svc/GetAllCustomFieldDropDownOptions',
            data={
                'customFieldUri': "{{ result('get_user_customfields').leave_type_uri }}"
            }
        )
        ...
        dummy_get_user_prereqs >> [get_enabled_co_costcenters, get_location_details, get_employeetype_groups_data,
        get_all_timesheet_period_list,get_user_customfields, get_permission_sets, get_all_payrule_scripts, get_all_policy_sets,
        get_timesheet_approval_paths, get_all_timezones, get_all_office_schedule, get_all_holiday_calendars, get_all_enabled_purchase_order,
        get_all_activity_uris] >> get_dropdowns_values_start >> [get_all_user_status_dropdowns, get_all_leave_type_dropdowns] >> \
