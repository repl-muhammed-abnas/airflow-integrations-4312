...
def get_missing_permission_sets(assigned_permissions, project_mgmt_perm_set_uri, resource_mgmt_perm_set_uri):
    ...
    if assigned_permissions:
        for assigned_perm in assigned_permissions:
            ...
            elif policy_uri == 'urn:replicon:policy:resource-management':
    ...
    if not has_resource_management and resource_mgmt_perm_set_uri:
        missing_permission_sets.append(resource_mgmt_perm_set_uri)
...
def extract_newly_added_task_uris_from_batches():
    ...
    if isinstance(batched_results, list):
        for batch in batched_results:  # Iterate through each batch
            if isinstance(batch, list):  # Each batch contains task results
