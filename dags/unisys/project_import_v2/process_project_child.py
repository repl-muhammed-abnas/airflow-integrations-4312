...
from unisys.project_import_v2.utils import request_payload, response_filter, custom_method
        ...
        with rail.create_airflow_dag(
            ...
            determine_missing_permissions = rail.PythonOperator(
                ...
                python_callable=lambda dag_run: custom_method.get_missing_permission_sets(
                    ...
                    dag_run.conf.get('resource_management_permission_set_uri', '')
                )
            )
            ...
            is_project_manager_available >> rail.Label('Yes') >> get_assigned_pm_permissions >> determine_missing_permissions >> has_missing_permissions
            is_project_manager_available >> rail.Label('No') >> log_project_success >> is_new_project

            # If permissions are missing, assign them; otherwise skip to project manager assignment
            has_missing_permissions >> rail.Label('Yes') >> assign_missing_permissions >> assign_project_manager_to_project >> log_project_success
            has_missing_permissions >> rail.Label('No') >> assign_project_manager_to_project >> log_project_success
            ...
            has_resource_batches_to_assign >> rail.Label("Yes") >> trigger_add_resource_child_dags >> wait_for_resource_assignment >> log_task_added >> create_update_task_batches
            has_resource_batches_to_assign >> rail.Label("No") >> log_task_added >> create_update_task_batches
