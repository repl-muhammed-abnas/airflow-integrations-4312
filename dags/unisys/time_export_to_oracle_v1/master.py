...
import pendulum
from unisys.time_export_to_oracle_v1.tasks.time_data_export import time_data_export
from unisys.time_export_to_oracle_v1.tasks.update_export_status import cancel_time_export
from unisys.time_export_to_oracle_v1.utils import custom_methods
from unisys.time_export_to_oracle_v1.utils import request_payload
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        generate_empty_export_log_file = rail.WriteCSVFileOperator(
            task_id="generate_empty_export_log_file",
            source=lambda: custom_methods.build_log_message(config.export_csv_filepath, config.secondary_export_csv_filepath, config.sftp_logs_filepath),
            header=None,
            row=[
        ...
        upload_empty_export_log_file_to_sftp = rail.SFTPUploadFileOperator(
            task_id="upload_empty_export_log_file_to_sftp",
            content='{{ result("generate_empty_export_log_file") }}',
            remote_filepath=config.sftp_logs_filepath + '/Log_{{ result("logging_details").time_export_filename }}.txt'
        )
        ...
        send_empty_export_email = rail.EmailOperator(
            ...
            params={
                ...
                'log_file_path': config.sftp_logs_filepath
            }
        )
        ...
        time_export_batch_end >> get_file_generated_time >> create_timeexport_collection >> finish_time_export_batch_creation >> has_data
        ...
        write_time_data_to_csv >> generate_log_file >> upload_log_file_to_sftp >> send_valid_export_complete_email
        ...
        has_data >> rail.Label("No") >> update_export_name_to_no_data >> \
            generate_empty_export_log_file >> upload_empty_export_log_file_to_sftp >> \
                send_empty_export_email >> dagrun_log_to_sumo
