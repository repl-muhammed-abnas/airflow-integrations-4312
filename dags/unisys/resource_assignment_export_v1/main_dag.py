...
import rail
from unisys.resource_assignment_export_v1.utils import custom_methods
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        # Get export file generated time in UTC
        get_file_generated_time = rail.PythonOperator(
            task_id="get_file_generated_time",
            python_callable=lambda: rail.render_template("{{ ts }}")
        )
        ...
        upload_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=f"{config.sftp_remote_path}/{{{{ result('get_file_name') }}}}"
        )

        get_ops_file_name = rail.PythonOperator(
            task_id="get_ops_file_name",
            python_callable=lambda: custom_methods.get_ops_file_name_for_instance(config)
        )

        get_log_file_name = rail.PythonOperator(
            task_id="get_log_file_name",
            python_callable=lambda: custom_methods.get_log_file_name(config)
        )

        encrypt_ops_file = rail.PGPEncryptionOperator(
            task_id="encrypt_ops_file",
            pgp_conn_id=config.secondary_pgp_conn_id,
            source="{{ result('generate_csv') }}"
        )

        upload_ops_file_to_sftp = rail.SFTPUploadFileOperator(
            task_id="upload_ops_file_to_sftp",
            content='{{ result("encrypt_ops_file") }}' if config.enable_encryption else '{{ result("generate_csv") }}',
            remote_filepath=f"{config.sftp_ops_remote_path}/{{{{ result('get_ops_file_name') }}}}"
        )

        query_allocation_data_records = rail.QueryCollectionOperator(
            task_id="query_allocation_data_records",
            query="""SELECT
                        v.*,
                        p.Project_Code as project_code,
                        u.Employee_ID as employee_id
                            FROM resource_allocation_events v
                            INNER JOIN project_report_collection p
                                ON v.project_uri = p.ProjectUri
                            INNER JOIN user_report_collection u
                                ON v.user_uri = u.UserUri""",
            name="base_allocation_data"
        )

        create_allocation_results = rail.CreateCollectionOperator(
            task_id="create_allocation_results",
            name="export_data",
            source="{{ result('gather_allocation_results') | to_json }}",
        )

        query_allocation_for_log = rail.QueryCollectionOperator(
            task_id="query_allocation_for_log",
            query="""SELECT 
                    BAD.*,
                    ED.AllocationStartDate,
                    ED.AllocationEndDate,
                    ED.AllocatedHours 
                    FROM base_allocation_data BAD
                    LEFT JOIN Export_data ED ON BAD.event_type = ED.Action 
                    AND BAD.Project_Code = ED.Project_Code 
                    AND BAD.Employee_ID = ED.Employee_ID AND
                    BAD.modified_date = ED.latest_modified_date"""
        )

        generate_base_extract_csv = rail.WriteCSVFileOperator(
            task_id="generate_base_extract_csv",
            source="{{ result('query_allocation_for_log') }}",
            header=[
                'Allocation Id',
                'Action',
                'Project Code',
        ...
        get_base_extract_file_name = rail.PythonOperator(
            task_id="get_base_extract_file_name",
            python_callable=lambda: custom_methods.get_base_extract_file_name(config)
        )

        encrypt_base_extract = rail.PGPEncryptionOperator(
            task_id="encrypt_base_extract",
            pgp_conn_id=config.pgp_conn_id,
            source="{{ result('generate_base_extract_csv') }}"
        )

        upload_base_extract_to_sftp = rail.SFTPUploadFileOperator(
            task_id="upload_base_extract_to_sftp",
            content= '{{ result("encrypt_base_extract") }}' if config.enable_encryption else '{{ result("generate_base_extract_csv") }}',
            remote_filepath=f"{config.sftp_logs_filepath}/{{{{ result('get_base_extract_file_name') }}}}"
        )

        get_sftp_log_file_details = rail.PythonOperator(
            task_id="get_sftp_log_file_details",
            python_callable=lambda: custom_methods.build_log_message(config)
        )

        generate_log_file = rail.WriteCSVFileOperator(
            task_id="generate_log_file",
            source="{{ result('get_sftp_log_file_details') }}",
            header=None,
            row=[
        ...
        upload_log_data_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=f"{config.sftp_logs_filepath}/{{{{ result('get_log_file_name') }}}}"
        )
        ...
        send_success_email = rail.EmailOperator(
            ...
            params={
                'export_filepath': config.sftp_remote_path,
                'ops_filepath': config.sftp_ops_remote_path,
                'log_filepath': config.sftp_logs_filepath,
            }
        )
        ...
        gather_allocation_results >> generate_csv >> get_file_generated_time >> get_file_name >> encrypt_file >> \
        upload_to_sftp >> get_ops_file_name >> get_log_file_name >> encrypt_ops_file >> upload_ops_file_to_sftp >> \
        query_allocation_data_records >> create_allocation_results >> query_allocation_for_log >> generate_base_extract_csv >> get_base_extract_file_name >> encrypt_base_extract >> upload_base_extract_to_sftp >> \
        get_sftp_log_file_details >> generate_log_file >> upload_log_data_to_sftp >> \
        send_success_email
