...
def _get_timesheet_period_to_assign_impl(dag_run, timesheetperioduri, user_action, log, effective_date):
    ...
    current_timesheetperiod = get_effective_timesheet_period(user_data['effectiveTimesheetPeriodSchedule'])
    ...
    if user_status in ['Active - No Time', 'Unpaid Leave']:
        if current_user_status in ['Active - No Time', 'Unpaid Leave']:
            return []
        date_source = effective_date if user_status == 'Active - No Time' else dag_run.conf['latest_leave_start_date']
        effective_date = get_replicon_date(date_source) or get_today_date()
        return [{
    ...
    current_period_uri = (
        current_timesheetperiod['timesheetPeriod']['uri']
        if current_timesheetperiod and current_timesheetperiod['timesheetPeriod']
        else None
    )
...
def get_timesheet_period_to_assign(dag_run, timesheetperioduri, user_action, log, effective_date):
    ...
    """
    return _get_timesheet_period_to_assign_impl(dag_run, timesheetperioduri, user_action, log, effective_date)
...
def get_active_policy_entry(schedule_list, change_effective_date=null):
    """
    Generic helper to find the currently active entry from any schedule/policy list.

    Matches the entry whose effectiveDate/endDate range covers today:
        - effectiveDate=null, endDate set   → active if today <= endDate
        - effectiveDate=null, endDate=null  → always active (initial/permanent)
        - effectiveDate set,  endDate=null  → active if today >= effectiveDate
        - effectiveDate set,  endDate set   → active if effectiveDate <= today <= endDate

    Args:
        schedule_list (list): Any list of Replicon schedule/policy entries that
                              contain 'effectiveDate' and 'endDate' dict fields.

    Returns:
        dict or None: The first matching entry, or None if nothing is active today.
    """
...
def get_effective_timesheet_period(schedule):
    """
    Get the currently effective timesheet period from effectiveTimesheetPeriodSchedule.

    Finds the entry whose effectiveDate is the latest date that is <= today.
    Entries with a future effectiveDate are ignored. If timesheetPeriod is null
    on the winning entry, returns that entry as-is (period intentionally unset).

    Args:
        schedule (list): effectiveTimesheetPeriodSchedule from user data response.

    Returns:
        dict or None: The effective schedule entry, or None if schedule is empty
                      or no entry qualifies.
    """
    if not schedule:
        return None
    today = datetime.strptime(pendulum.now().strftime(DATE_FORMAT), DATE_FORMAT)
    ...
    for entry in schedule:
        ...
        if effective_date <= today:
            if best_date is None or effective_date > best_date:
...
def get_permissionsets_to_assign(dag_run, permissionsets, user_action, exceptions_co_code, instance):
    resp = []
...
def get_create_update_user_payload(config, dag_run, user_action):
    ...
    put_user_payload = {
        ...
        "modifications": {
            ...
            "policySets": [],
            "permissionSets": get_permissionsets_to_assign(dag_run, dag_run.conf['permissionsetdetails'], user_action, config.EXCEPTIONS_CO_CODE, config.instance),
            "bankedTimePolicies": [],
            "timeOffTypes": [],
...
def get_old_user_type_payload(dag_run):
    user_data = rail.result('get_user_payload_data')['user_data']
    rplcn_usertypes = rail.load_all_records(dag_run.conf['replicon_usertypes_details'])
    effective_usertype = rail.result('get_effective_user_groups', 'employeetype')
    current_usertype = rail.find_first_by_attr_and_get_attr(rplcn_usertypes, 'uri', effective_usertype.get('uri'))
    user_type = get_zero_indx_value(user_data['user_type'])
    if current_usertype: 
        user_type = get_zero_indx_value(current_usertype.get('fullpath', ''))
    return {
        "target": {
            "uri": rail.result('get_user_by_empl_id')[0]['userDetails']['uri'],
        },
        ...
        "modifications": {
            "loginName": {
                "value": f"{user_data['login_name']}_{user_type}"
            },
            "employeeId": {
                "value": f"{dag_run.conf['employee_id']}_{user_type}"
            },
