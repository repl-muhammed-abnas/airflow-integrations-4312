...
import rail
from unisys.workday_user_import_v1.mappers.user_sync_mapper import user_sync_mapper
from unisys.workday_user_import_v1.mappers.timezone_mapper import timezone_mapper
from unisys.workday_user_import_v1.mappers.workweek_mapper import workweek_mapper
from unisys.workday_user_import_v1.mappers.project_mapper import project_mapper
...
def has_company_code_changed(dag_run):
    effective_company_code = rail.result('get_effective_user_groupmembership', 'division')
    effective_usertype = rail.result('get_effective_user_groupmembership', 'employeetype')
...
def has_user_type_changed(dag_run):
    effective_usertype = rail.result('get_effective_user_groups', 'employeetype')
    if not effective_usertype: return False

    rplcn_usertypes = rail.load_all_records(dag_run.conf['replicon_usertypes_details'])
    
    user_data = rail.load_all_records(rail.result('query_user_data'))[0]
    new_usertype = strip_extra_spaces(user_data['user_type'])
