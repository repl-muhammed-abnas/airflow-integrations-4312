...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"standard_xero_connector_{config.region.replace('-', '_')}_invoice_status_update_billed_{config.instance}",
        description=f'Xero Connector {config.region} Invoice Billed Status Update {config.instance}',
        ...
        search_new_updated_invoices = rail.XeroAPIOperator(
            ...
            request_method='GET',
            filters='''?where=STATUS=="AUTHORISED" AND SentToContact==true''',
        ...
        has_invoice_data = rail.IfOperator(
            ...
            test="{{ result('parse_required_invoices') | length > 0 }}",
        ...
        trigger_invoice_status_update_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f"standard_xero_connector_{config.region.replace('-', '_')}_invoice_status_update_billed_child_dag_{config.instance}",
        ...
        log_dagrun_details_to_table = rail.PostDagRunDetailsToRepliconOperator(
            ...
            connector_name='xero',
            integration_type='invoice_status_update_billed'
        )
        ...
        get_lastsync_time >> search_new_updated_invoices >> update_lastsync_time >> parse_required_invoices
