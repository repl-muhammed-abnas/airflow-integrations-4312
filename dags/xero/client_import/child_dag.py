...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"standard_xero_connector_{config.region.replace('-', '_')}_client_import_child_dag_{config.instance}",
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task='batch_task',
            no_task='create_update_client_in_replicon'
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id='batch_task',
            start_task='create_update_client_in_replicon',
            ...
            return {
                ...
                'modifications': {
                    ...
                    'clientAddressToApply': {
                        'address': {
                            'value': dag_run.conf['client_address'] if dag_run.conf['client_address'] else null
                        },
                        'city': {'value': dag_run.conf['client_city']} if dag_run.conf['client_city'] else null,
                        'stateProvince': {'value': dag_run.conf['client_region']} if dag_run.conf['client_region'] else null,
                        ...
                        'faxNumber': {'value': dag_run.conf['client_fax_number'] if dag_run.conf['client_fax_number'] else null},
                    },
                    'billingAddressToApply': {
                        ...
                        'stateProvince': {'value': dag_run.conf['billing_region']} if dag_run.conf['billing_region'] else null,
        ...
        create_update_client_in_replicon = rail.RepliconServiceOperator(
            task_id='create_update_client_in_replicon',
        ...
        can_run_batch_task >> rail.Label(
            'No') >> create_update_client_in_replicon >> rail.Label(
                'on Error') >> catch_client_error
