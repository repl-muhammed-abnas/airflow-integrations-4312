...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        log_invalid_pers_act_cd = rail.WriteLogOperator(
            ...
            log = '{{ result("create_user_log") }}',
            message = "Profile is not present in replicon.",
            severity='Exception',
            properties = lambda dag_run: {
                'lastname': dag_run.conf['last_name'],
                'firstname': dag_run.conf['first_name'],
                ...
                'action': 'Validation',
                'status': 'Exception',
                'details': "Profile is not present in replicon."
            }
        )
        ...
        has_valid_data >> rail.Label('Yes') >> check_add_or_update_user >> if_add_or_update_is_valid
        ...
        if_setup_new_profile_is_no >> rail.Label('Yes') >> process_update_user >> wait_for_process_update_user >> catch_and_log_errors
