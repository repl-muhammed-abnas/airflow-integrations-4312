...
from deltek_northstar.user_sync_polaris.utils import request_payload, response_filter, python_callable
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        get_user_data >> if_termination_date_present >> rail.Label("No") >> is_user_disabled
        ...
        is_enddate_greater_than_start_date >> rail.Label("No") >> update_employee_endate >> if_can_be_disabled

        if_can_be_disabled >> rail.Label('Yes') >> disable_login >> log_disabled_success >> catch_and_log_errors
        if_can_be_disabled >> rail.Label('No') >> catch_and_log_errors
        ...
        get_effective_user_groupmembership >> get_user_assigned_role_from_replicon >> update_existing_user >> is_supervisor_in_api_response
