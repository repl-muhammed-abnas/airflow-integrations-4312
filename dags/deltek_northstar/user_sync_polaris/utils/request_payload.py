...
import pendulum
import rail
from deltek_northstar.user_sync_polaris.utils.python_callable import get_current_date, parse_date_json
...
def get_timesheet_period_name(config, timesheet_cycle):
    return rail.find_first_by_attr_and_get_attr(config.TIMESHEET_PERIOD_MAPPER, "Timesheet Cycle", timesheet_cycle, "Timesheet Period", "")

def get_licences_to_be_assigned(config, polaris_role):
    licenses = rail.find_first_by_attr_and_get_attr(config.POLARIS_ROLES_MAPPER, "Polaris Roles", polaris_role, "Licenses", [])
    resp = []
...
def timezone_name(config, country_cd):
    return rail.find_first_by_attr_and_get_attr(config.TIMEZONE_MAPPER, 'Country_CD', country_cd, 'TimeZone')


def get_timesheet_template(config, polaris_role):
    return rail.find_first_by_attr_and_get_attr(config.POLARIS_ROLES_MAPPER, "Polaris Roles", polaris_role, "Timesheet Template", None)

def get_timesheet_approval_path(config, polaris_role):
    return rail.find_first_by_attr_and_get_attr(config.POLARIS_ROLES_MAPPER, "Polaris Roles", polaris_role, "Timesheet Approval Path", None)

def get_expense_template(config, polaris_role):
    return rail.find_first_by_attr_and_get_attr(config.POLARIS_ROLES_MAPPER, "Polaris Roles", polaris_role, "Expense Template", None)

def get_expense_approval_path(config, polaris_role):
    return rail.find_first_by_attr_and_get_attr(config.POLARIS_ROLES_MAPPER, "Polaris Roles", polaris_role, "Expense Approval Path", None)

def get_timeoff_template(config, polaris_role):
    return rail.find_first_by_attr_and_get_attr(config.POLARIS_ROLES_MAPPER, "Polaris Roles", polaris_role, "Time Off Template", None)

def get_timeoff_approval_path(config, polaris_role):
    return rail.find_first_by_attr_and_get_attr(config.POLARIS_ROLES_MAPPER, "Polaris Roles", polaris_role, "Time Off Approval Path", None)
...
def get_process_users_conf(config, item):
    ...
    def get_all_permissionseturis(item):
        ...
        permissions = rail.find_first_by_attr_and_get_attr(config.POLARIS_ROLES_MAPPER, "Polaris Roles", item['polaris_role'], "Permissions", {})
    ...
    return {
        ...
        **{
            ...
            'timezone': timezone_name(config, item['country']),
            'licences': get_licences_to_be_assigned(config, item['polaris_role']),
            'timezoneuri': rail.find_first_by_attr_and_get_attr(rail.result('get_all_timezones'), 'displayText', timezone_name(config, item['country']), 'uri'),
            'permissionsetdetails': get_all_permissionseturis(item),
            'supervisor_permission_uri': rail.find_first_by_attr_and_get_attr(rail.result('get_all_permission_set'),'displayText',"Supervisor DPS",'uri'),
            ...
            'servicecenteruri': rail.find_first_by_attr_and_get_attr(rail.result('get_updated_employee_types'),'name',taxable_entity_name(item),'uri'),
            'project_role_uri':rail.find_first_by_attr_and_get_attr(rail.result('get_all_roles'), 'code', item['plc'],'uri'),
            'timesheetperiod':get_timesheet_period_name(config, item['timesheet_cycle']),
            'timesheet_period_uri':rail.find_first_by_attr_and_get_attr(rail.result('get_all_timesheet_period_list'), 'name', get_timesheet_period_name(config, item['timesheet_cycle']),'uri'),
            'timesheettemplateuri': rail.find_first_by_attr_and_get_attr(rail.result("get_all_policy_sets"),'displayText',get_timesheet_template(config, item['polaris_role']),"uri")
                if get_timesheet_template(config, item['polaris_role']) else null,
            'timesheettemplate': get_timesheet_template(config, item['polaris_role']),
            'timesheetapprovalpath': get_timesheet_approval_path(config, item['polaris_role']),
            'timesheetapprovalpathuri': rail.find_first_by_attr_and_get_attr(rail.result("get_timesheet_approval_paths"),'displayText',get_timesheet_approval_path(config, item['polaris_role']),"uri") if get_timesheet_approval_path(config, item['polaris_role']) else null,
            'expensetemplate': get_expense_template(config, item['polaris_role']),
            'expensetemplateuri': rail.find_first_by_attr_and_get_attr(rail.result("get_all_policy_sets"),'displayText',get_expense_template(config, item['polaris_role']),"uri")
                if get_expense_template(config, item['polaris_role']) else null,
            'expenseapprovalpath': get_expense_approval_path(config, item['polaris_role']),
            'expenseapprovalpathuri': rail.find_first_by_attr_and_get_attr(rail.result("get_expense_approval_paths"),'displayText',get_expense_approval_path(config, item['polaris_role']),"uri")
                if get_expense_approval_path(config, item['polaris_role']) else null,
            'timeofftemplate': get_timeoff_template(config, item['polaris_role']),
            'timeofftemplateuri': rail.find_first_by_attr_and_get_attr(rail.result("get_all_policy_sets"),'displayText',get_timeoff_template(config, item['polaris_role']),"uri")
                if get_timeoff_template(config, item['polaris_role']) else null,
            'timeoffapprovalpath': get_timeoff_approval_path(config, item['polaris_role']),
            'timeoffapprovalpathuri': rail.find_first_by_attr_and_get_attr(rail.result("get_timeoff_approval_paths"),'displayText',get_timeoff_approval_path(config, item['polaris_role']),"uri")
                if get_timeoff_approval_path(config, item['polaris_role']) else null,
...
def get_timesheet_template_to_assign(log, dag_run, user_action):
...
def get_timeoff_types(config, work_schedule, user_action):
    resp = []
    if not work_schedule:
        return resp
    timeoff_types = rail.find_first_by_attr_and_get_attr(config.TIMEOFF_TYPE_MAPPER, "work_schedule", work_schedule, "timeoff_types", [])
    if user_action == "add_user":
        ...
        for timeoff_type in timeoff_types:
            if timeoff_type not in current_timeoff_type_names:
                resp.append({
                        "timeOffType": {
                            "uri": null,
                            "name": timeoff_type
...
def get_schedule_type_to_assign(work_schedule, user_action, effective_date):
    if not work_schedule:
        return []
    if user_action == "add_user":
        return [
        ...
        if not current_schedule_type or not current_schedule_type[-1]['officeSchedule'] or (current_schedule_type and (
            work_schedule != current_schedule_type[-1]['officeSchedule']['displayText'])):
            return [
                {
                    "dateRange": {
                        "startDate": get_replicon_date(effective_date),
...
def get_create_update_user_payload(config, dag_run, user_action):
    ...
    put_user_payload = {
        ...
        "modifications": {
            ...
            "timesheetApprovalPath": get_timesheet_approvalpath(log,dag_run, "add_user") if user_action == "add_user" else get_timesheet_approvalpath(log,dag_run, "update_user"),
            "timeEntryApprovalPath": null,
            "workAuthorizationApprovalPath": null,
            "timeoffApprovalPath": get_timeoff_approvalpath(log,dag_run, "add_user") if user_action == "add_user" else get_timeoff_approvalpath(log,dag_run, "update_user"),
            ...
            "timeZone": get_timezone_uri(log, dag_run, "add_user") if user_action == "add_user" else get_timezone_uri(log, dag_run, "update_user"),
            "workWeekStartDay": null,
            "defaultBillingRate": null,
            "userPreferences": null,
            "formattings": null,
            "notificationPreferences": null,
            "timesheetTemplate": get_timesheet_template_to_assign(log, dag_run, "add_user") if user_action == "add_user" else get_timesheet_template_to_assign(log, dag_run, "update_user"),
            ...
            "extensionFields": [],
            "customFields": get_udfs('add_user', dag_run) if user_action == "add_user" else get_udfs('update_user', dag_run),
            "products": [],
            "skills": [],
            "activities": [],
            "policySets": [],
            "permissionSets": get_permissionsets_to_assign(dag_run.conf['permissionsetdetails'], "add_user") if user_action == "add_user" else get_permissionsets_to_assign(dag_run.conf['permissionsetdetails'], "update_user"),
            "bankedTimePolicies": [],
            "timeOffTypes": get_timeoff_types(config, dag_run.conf['work_schedule'], "add_user") if user_action == "add_user" else get_timeoff_types(config, dag_run.conf['work_schedule'], "update_user"),
            "locationSchedule": get_location_schedule_to_assign(dag_run.conf['country_name'], "add_user", dag_run.conf['effect_date']) if user_action == "add_user" else get_location_schedule_to_assign(dag_run.conf['country_name'], "update_user", dag_run.conf['effect_date']),
            "divisionSchedule": [],
            "costCenterSchedule": get_costcenter_schedule_to_assign(dag_run.conf['costcenteruri'],"add_user", dag_run.conf['effect_date']) if user_action == "add_user" else get_costcenter_schedule_to_assign(dag_run.conf['costcenteruri'], "update_user", dag_run.conf['effect_date']),
            "serviceCenterSchedule": get_servicecenter_schedule_to_assign(dag_run.conf['servicecenteruri'],"add_user", dag_run.conf['effect_date']) if user_action == "add_user" else get_servicecenter_schedule_to_assign(dag_run.conf['servicecenteruri'], "update_user", dag_run.conf['effect_date']),
            "departmentGroupSchedule": get_department_schedule_to_assign(dag_run.conf['department_uri'],"add_user", dag_run.conf['effect_date']) if user_action == "add_user" else get_department_schedule_to_assign(dag_run.conf['department_uri'], "update_user", dag_run.conf['effect_date']),
            "employeeTypeGroupSchedule": get_employeetype_schedule_to_assign(dag_run.conf['employee_type_uri'],"add_user", dag_run.conf['effect_date']) if user_action == "add_user" else get_employeetype_schedule_to_assign(dag_run.conf['employee_type_uri'], "update_user", dag_run.conf['effect_date']),
            "supervisorSchedule": [],
            "timesheetPeriodSchedule": get_timesheet_period_to_assign(dag_run.conf['timesheet_period_uri'], "add_user", dag_run.conf['effect_date']) if user_action == "add_user" else get_timesheet_period_to_assign(dag_run.conf['timesheet_period_uri'], "update_user", dag_run.conf['effect_date']),
            "holidayCalendarSchedule": [],
            "scheduleTypeSchedule": get_schedule_type_to_assign(dag_run.conf['work_schedule'], "add_user", dag_run.conf['effect_date']) if user_action == "add_user" else get_schedule_type_to_assign(dag_run.conf['work_schedule'], "update_user", dag_run.conf['effect_date']),
            "payRuleSchedule": [],
            "placeSchedule": [],
            "payRateSchedule": [],
            "projectRoleSchedule": get_project_role_to_assign(dag_run.conf['project_role_uri'], "add_user", dag_run.conf['effect_date']) if user_action == "add_user" else get_project_role_to_assign(dag_run.conf['project_role_uri'], "update_user", dag_run.conf['effect_date']),
            ...
            "hourlyRatesSchedule": [
            {
                ...
                "item": {
                "hourlyRate": {
                    "amount": round((float(rail.result('get_cost_rate_by_emp_id')[0]['BILL_RT_AMT'])), 2),
                    "currency": {
                        "uri": null,
                        "name": null,
                        "symbol": rail.result('get_cost_rate_by_emp_id')[0]['CURRENCY']
                    }
                }
                }
            }
            ] if user_action == "add_user" and rail.result('get_cost_rate_by_emp_id') and rail.result('get_cost_rate_by_emp_id')[0].get('BILL_RT_AMT') else [],
...
def get_cost_rate_by_emp_id(dag_run):
    return {
        "filter":{
            "id":"it_polaris_cost",
            "where":[{
                "rsWhere":{
                    "rsId":"XT_EMPL_SRC_POL_COST",
                    "conditions":[{
                        "joinWithParent": "N",
                        "relations": [
                            {
...
def get_update_existing_user_profile(dag_run, time_zone, user_details, update_existing_profile):
    empl_daterange = user_details['employmentDateRange']
    user_uri = user_details['uri']
    term_date = get_term_date(dag_run, empl_daterange, time_zone)
    return {
        "target": {
            "uri": user_uri
        },
        "modifications": {
            **update_existing_profile,
            "employmentDateRange": {
            "value": {
                "startDate": empl_daterange['startDate'],
                "endDate": {
                "year": term_date.year,
