...
def get_updated_start_date():
    prev_timesheet_end_date = result("get_timesheet_details")
    return (parse_date_json(prev_timesheet_end_date) + timedelta(days=1)).strftime(DATE_FORMAT) if prev_timesheet_end_date else None
...
def do_get_last_run_date(config):
    ...
    if_conf_data_used = Variable.get(config.can_use_conf_payload_var_name, default_var='false').lower() == 'true'
    if not if_conf_data_used:
...
def check_add_or_update_user(dag_run, time_zone):
    ...
    if not user_data_resp:
        ...
        if pers_act_cd in ["HI-NWHIR","HI-INTRN", "HI-ACQUS"]:
            resp = {
                "setup_new_profile":"Yes",
                "update_existing_profile":{}
            }
    else:
        empl_id_does_not_match = False if result('get_user_by_empl_id') else True
        loginName = user_data_resp['securityConfiguration']['loginName']
        current_empl_id = user_data_resp['userDetails']['employeeId']
        current_email_id = user_data_resp['userDetails']['emailAddress']
        ...
        elif empl_id_does_not_match and pers_act_cd in ["HI-REHIR", "HI-REINT"]:
            resp = {
                "setup_new_profile":"Yes",
                "update_existing_profile":{
                    "loginName": {
                        "value": f"{current_email_id}_{get_update_value(dag_run, current_empl_daterange, time_zone)}"
                    },
                    "employeeId": {
        ...
        elif pers_act_cd in ["CH-ORGNU","CH-TRANS"]:
            resp = {
                "setup_new_profile":"No",
                "update_existing_profile":{}
            }
