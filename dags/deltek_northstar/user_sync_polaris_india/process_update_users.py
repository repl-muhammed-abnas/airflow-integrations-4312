...
from deltek_northstar.user_sync_polaris_india.utils import request_payload, response_filter, python_callable
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        if_can_be_disabled = rail.IfOperator(
            ...
            yes_task="get_current_udf_values_for_disable_users",
            no_task="empty_is_user_disabled"
        )

        get_current_udf_values_for_disable_users = rail.PythonOperator(
            task_id='get_current_udf_values_for_disable_users',
            python_callable=lambda: rail.result('get_user_data')[0][
                'userDetails']['customFieldValues']
        )

        get_effective_user_groupmembership_for_disable_users = rail.RepliconServiceOperator(
            task_id='get_effective_user_groupmembership_for_disable_users',
        ...
        get_user_assigned_role_from_replicon_for_disable_users = rail.RepliconServiceOperator(
            task_id='get_user_assigned_role_from_replicon_for_disable_users',
        ...
        update_existing_user_for_disable_users = rail.RepliconServiceOperator(
            task_id='update_existing_user_for_disable_users',
        ...
        if_supervisor_exists = rail.IfOperator(
            ...
            yes_task='send_termination_email',
            no_task='log_disabled_success'
        )

        send_termination_email = rail.EmailOperator(
            task_id='send_termination_email',
            to="{{ result('get_supervisor_details').email }}",
            bcc=config.internal_logs_email,
            subject="Deltek India - User\'s {{dag_run.conf.display_name}}, {{dag_run.conf.empl_id}} Termination Notice",
            html_content="templates/emails/termination_mail.html"
        )
        ...
        if_can_be_disabled >> rail.Label('Yes') >> get_current_udf_values_for_disable_users >> \
        get_effective_user_groupmembership_for_disable_users >> get_user_assigned_role_from_replicon_for_disable_users >> \
        update_existing_user_for_disable_users >> disable_login >> get_supervisor_details >> if_supervisor_exists
        if_supervisor_exists >> rail.Label('Yes') >> send_termination_email >> log_disabled_success
        if_supervisor_exists >> rail.Label('No') >> log_disabled_success
        log_disabled_success >> catch_and_log_errors
