...
import pendulum
import rail
from deltek_northstar.user_sync_polaris_philippines_v1.utils.python_callable import get_current_date, parse_date_json, get_updated_start_date

null = None
DATE_FORMAT = "%Y-%m-%dT%H:%M:%S"
ALL_NOTIFICATIONS = ['expense-sheet', 'holiday', 'pay-rule-script', 'project', 'time-entry-revision-group', 'time-off', 'timesheet', 'user']
...
def get_process_users_conf(config, item):
    ...
    return {
        ...
        **{
            ...
            'timezone': get_mapper_values(config, item['pay_period_code'], 'Time Zone'),
            'schedule_type': get_mapper_values(config, item['pay_period_code'], 'Schedule Type'),
            'pay_rule': get_mapper_values(config, item['pay_period_code'], 'Pay Rule'),
            'licences': get_licences_to_be_assigned(config, item['pay_period_code']),
            'timezoneuri': rail.find_first_by_attr_and_get_attr(rail.result('get_all_timezones'), 'displayText', get_mapper_values(config, item['pay_period_code'], 'Time Zone'), 'uri'),
            ...
            'timesheetapprovalpath': get_mapper_values(config, item['pay_period_code'], 'Timesheet Approval Path'),
            'timesheetapprovalpathuri': rail.find_first_by_attr_and_get_attr(rail.result("get_timesheet_approval_paths"),'displayText',get_mapper_values(config, item['pay_period_code'], 'Timesheet Approval Path'),"uri") if get_mapper_values(config, item['pay_period_code'], 'Timesheet Approval Path') else null,
...
def get_timesheet_template_to_assign(log, dag_run, user_action, effective_date):
    ...
    if user_action == "add_user":
        ...
        # Skip timesheet template update if "On call Allowance" UDF is "Yes"
        custom_field_values = rail.result('get_user_data')[0]['userDetails']['customFieldValues']
        on_call_allowance = rail.find_first_by_attr_and_get_attr(
            custom_field_values, 'customField.displayText', 'On call Allowance', 'text')
        if on_call_allowance and on_call_allowance.strip().lower() == 'yes':
            return []
...
def get_timeoff_types(timeoff_types_available, user_action):
    resp = []
    if not timeoff_types_available:
        return resp
    # timeoff_types = rail.find_first_by_attr_and_get_attr(config.TIMEOFF_TYPE_MAPPER, "employee_type", employee_type, "timeoff_types", [])
    if user_action == "add_user":
...
def get_schedule_type_to_assign(schedule_type, user_action, effective_date=null):
    if not schedule_type:
        return []
    if schedule_type == "Shift Schedule":
        item = {
    ...
    if user_action == "add_user":
        ...
        if current_schedule_type and (current_schedule_type[-1]['scheduleTypeUri'] == "urn:replicon:schedule-type:shift" and schedule_type == "Shift Schedule"):
            return []
...
def get_pay_rule_to_assign(pay_rule, user_action, effective_date=null):
    if not pay_rule:
        return []
    if user_action == "add_user":
        return [
...
def get_create_update_user_payload(config, dag_run, user_action):
    ...
    put_user_payload = {
        ...
        "modifications": {
            ...
            "formattings": null,
            "notificationPreferences": get_notification_settings(config, dag_run.conf['pay_period_code']),
            ...
            "bankedTimePolicies": [],
            "timeOffTypes": get_timeoff_types(dag_run.conf['timeoff_types_available'], "add_user") if user_action == "add_user" else get_timeoff_types(dag_run.conf['timeoff_types_available'], "update_user"),
            ...
            "divisionSchedule": [],
            "costCenterSchedule": get_costcenter_schedule_to_assign(dag_run.conf['costcenteruri'],"add_user", dag_run.conf['todaysdate']) if user_action == "add_user" else get_costcenter_schedule_to_assign(dag_run.conf['costcenteruri'], "update_user", dag_run.conf['todaysdate']),
            "serviceCenterSchedule": get_servicecenter_schedule_to_assign(dag_run.conf['servicecenteruri'],"add_user", dag_run.conf['todaysdate']) if user_action == "add_user" else get_servicecenter_schedule_to_assign(dag_run.conf['servicecenteruri'], "update_user", dag_run.conf['todaysdate']),
            "departmentGroupSchedule": get_department_schedule_to_assign(dag_run.conf['department_uri'],"add_user", dag_run.conf['todaysdate']) if user_action == "add_user" else get_department_schedule_to_assign(dag_run.conf['department_uri'], "update_user", dag_run.conf['todaysdate']),
            "employeeTypeGroupSchedule": get_employeetype_schedule_to_assign(dag_run.conf['employee_type_uri'],"add_user", dag_run.conf['todaysdate']) if user_action == "add_user" else get_employeetype_schedule_to_assign(dag_run.conf['employee_type_uri'], "update_user", dag_run.conf['todaysdate']),
            "supervisorSchedule": [],
            "timesheetPeriodSchedule": get_timesheet_period_to_assign(dag_run.conf['timesheet_period_uri'], "add_user", dag_run.conf['todaysdate'])  if user_action == "add_user" else [],
            "holidayCalendarSchedule": [],
            "scheduleTypeSchedule": get_schedule_type_to_assign(dag_run.conf['schedule_type'], "add_user") if user_action == "add_user" else get_schedule_type_to_assign(dag_run.conf['schedule_type'], "update_user", dag_run.conf['next_timesheet_start_date']),
            "payRuleSchedule": get_pay_rule_to_assign(dag_run.conf['pay_rule'], "add_user") if user_action == "add_user" else get_pay_rule_to_assign(dag_run.conf['pay_rule'], "update_user", dag_run.conf['next_timesheet_start_date']),
            "placeSchedule": [],
            "payRateSchedule": [],
            "projectRoleSchedule": get_project_role_to_assign(dag_run.conf['project_role_uri'], "add_user", dag_run.conf['todaysdate']) if user_action == "add_user" else get_project_role_to_assign(dag_run.conf['project_role_uri'], "update_user", dag_run.conf['todaysdate']),
            "costNormalizationRuleSchedule": [],
            "hourlyRatesSchedule": [],
            "substituteUserSchedule": [],
            "policySetsScheduleToApply": get_timesheet_template_to_assign(log, dag_run, "add_user", dag_run.conf['todaysdate']) if user_action == "add_user" else get_timesheet_template_to_assign(log, dag_run, "update_user", dag_run.conf['todaysdate']),
        },
...
def get_update_existing_user_profile(dag_run, time_zone, user_details, update_existing_profile):
    ...
    term_date = get_term_date(dag_run, {}, time_zone)  # Pass empty dict since we're not using empl_daterange
    ...
    return {
        ...
        "modifications": {
            **update_existing_profile,
            "employmentDateRange": {
                "value": {
                    "startDate": current_hire_date,  # Use current_hire_date from config
                    "endDate": {
                        "year": term_date.year,
...
def get_customfield_dropdown_option(existing_dropdowns_task_id, list_to_add_key):
    ...
    for udf_name in dropdowns_list_add:
        if udf_name and udf_name.strip():
...
def get_notification_settings(config, pay_period_code):
    notification_settings = rail.find_first_by_attr_and_get_attr(config.PAY_PERIOD_MAPPER, "Pay Period Code", pay_period_code, "Notification Settings", [])
    notification_settings = [notification_settings] if  isinstance(notification_settings, str) else notification_settings
