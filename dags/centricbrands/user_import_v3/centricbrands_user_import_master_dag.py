...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        description=f'CentricBrands_User_Import_Master',
        ...
        trigger_child_add_cost_centers_locations = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=config.child_cost_centers_locations_add_dag_id,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
                "downloadedfile": "{{ result('parse_csv') }}",
                "groupslogslookuptable": "{{result('create_centric_brands_groups_logs_lookuptable')}}",
        ...
        trigger_child_add_locations_teams = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=config.child_locations_teams_add_dag_id,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
                "downloadedfile": "{{ result('parse_csv') }}",
                "groupslogslookuptable": "{{result('create_centric_brands_groups_logs_lookuptable')}}",
        ...
        trigger_child_assign_supervisor_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.child_assign_supervisor_dag_id,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        if_rawinput_has_no_data >> rail.Label(
            'Yes') >> send_mail_no_data_in_file >> finish
        if_rawinput_has_no_data >> rail.Label(
        ...
        if_no_delta_records >> rail.Label(
            'Yes') >> send_mail_no_changes_found >> archive_old_reference_file >> upload_new_reference_file >> finish
        if_no_delta_records >> rail.Label(
        ...
        if_log_entries_not_present >> rail.Label(
            'Yes') >> fail_dag_with_error >> finish
        if_log_entries_not_present >> rail.Label('No') >> finish
        if_users_tobe_added_or_users_in_replicon_present >> rail.Label(
            'No') >> finish
        if_report_has_data >> rail.Label(
            'No') >> fail_with_report_has_no_data >> finish
    return dag
