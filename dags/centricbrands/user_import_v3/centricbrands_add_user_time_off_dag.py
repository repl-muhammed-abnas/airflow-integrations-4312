...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.child_add_user_time_off_dag_id,
        description=f'CentricBrands User Import - Add User Time Off Child',
        ...
        search_matching_timeoff_hk = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: list(filter(lambda entry: entry['country'].lower() == (
                dag_run.conf['location']).lower() and entry['hong_kong_levels'].lower() == (
                dag_run.conf['hongkonglevels']).lower(), config.TO_TYPE_ASSIGNMENT_MAPPER))
        )
        ...
        search_matching_timeoff_china = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: list(filter(lambda entry: entry['country'].lower() == (
                dag_run.conf['location']).lower() and entry['state'].lower() == (
                dag_run.conf['stateprovince']).lower(), config.TO_TYPE_ASSIGNMENT_MAPPER))
        )
        ...
        search_matching_timeoff_type_in_mapper = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: list(filter(lambda entry: entry['country'].lower() == (
                dag_run.conf['location']).lower() and entry['state'].lower() == (
                dag_run.conf['stateprovince']).lower() and entry['employeetype'].lower() == (
                dag_run.conf['employeetype']).lower(), config.TO_TYPE_ASSIGNMENT_MAPPER))
        )
        ...
        def get_modified_date(offsetvalue, offsetUnitUri, integration_date):
            modified_policy_effectivedate = (datetime.strptime(integration_date, "%Y-%m-%d") + (
                relativedelta(
                    months=offsetvalue * 12) if 'years' in offsetUnitUri else (relativedelta(
                        months=offsetvalue) if 'months' in offsetUnitUri else relativedelta(
                        days=offsetvalue)))).strftime("%Y-%m-%d")

            return rail.parse_date(modified_policy_effectivedate, "%Y-%m-%d")

        def get_timeoffpolicies_to_assign(integration_date):
            default_setschedule = rail.result(
                'get_default_timeoff_policysetschedule_for_timeofftype')
            return [{
                "effectiveDate": get_modified_date(schedule['startOffset']['offsetValue'], schedule['startOffset']['offsetUnitUri'], integration_date),
                "description": "Effective on " + datetime(**get_modified_date(
                    schedule['startOffset']['offsetValue'], schedule['startOffset']['offsetUnitUri'], integration_date)).strftime("%Y-%m-%d"),
        ...
        get_list_of_timeoffpolicies_to_assign = rail.PythonOperator(
            ...
            python_callable=lambda: get_timeoffpolicies_to_assign(
                rail.result('get_integrationdate_object')['date'])
        )
        ...
        search_timeoff_policy_in_mapper_for_timeofftype_china_hongkong = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: list(filter(lambda policy: policy['timeofftype'] == rail.result('foreach_timeofftype_assigned')['name'] and
                ...
                "%B"), config.TO_POLICY_STARTING_BALANCE_CHINA_HK_MAPPER))
        )
        ...
        search_timeoff_policy_in_mapper_for_timeofftype = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: list(filter(lambda policy: policy['timeofftype'] == rail.result('foreach_timeofftype_assigned')['name'] and
                                                        policy['offset'] == '0' and policy['month'] == (
                                                            datetime.strptime(dag_run.conf['integrationdate'], '%m/%d/%Y')).strftime(
                "%B"), config.TO_POLICY_STARTING_BALANCE_MAPPER))
        )

        check_if_location_uk = rail.IfOperator(
            task_id='check_if_location_uk',
            test='''{{dag_run.conf.location | lower == "united kingdom"}}''',
            yes_task='if_timeoff_is_pto_uk',
            no_task='if_timeoff_isnt_pto_sick_vacation'
        )

        if_timeoff_is_pto_uk = rail.IfOperator(
            task_id='if_timeoff_is_pto_uk',
            test=lambda: 'PTO' in rail.result(
                'foreach_timeofftype_assigned')['name'],
            yes_task="get_default_policy_modified_uk_pto",
            no_task="if_default_policy_set_present_uk",
        )

        def get_modified_default_policy_uk():
        ...
        get_default_policy_modified_uk_pto = rail.PythonOperator(
            task_id='get_default_policy_modified_uk_pto',
            python_callable=get_modified_default_policy_uk
        )

        put_user_timeoff_accountpolicy_set_schedule_uk_pto = rail.RepliconServiceOperator(
            task_id='put_user_timeoff_accountpolicy_set_schedule_uk_pto',
            endpoint="/services/TimeOffPolicyService2.svc/PutUserTimeOffAccountPolicySetSchedule",
            data=lambda dag_run: {
                "timeOffAccount": {
                    "userUri": dag_run.conf['uri'],
                ...
                "policySetScheduleEntries": [rail.result('get_default_policy_modified_uk_pto')]
            }
        )

        if_default_policy_set_present_uk = rail.IfOperator(
            task_id='if_default_policy_set_present_uk',
            test=lambda: bool(rail.result('log_default_policyset')),
            yes_task="get_default_policy_modified_uk_non_pto",
            no_task="foreach_timeofftype_assigned_end",
        )

        get_default_policy_modified_uk_non_pto = rail.PythonOperator(
            task_id='get_default_policy_modified_uk_non_pto',
            python_callable=get_default_policy
        )

        put_user_timeoff_account_policysetschedule_uk_non_pto = rail.RepliconServiceOperator(
            task_id='put_user_timeoff_account_policysetschedule_uk_non_pto',
            endpoint="/services/TimeOffPolicyService2.svc/PutUserTimeOffAccountPolicySetSchedule",
            data=lambda dag_run: {
                "timeOffAccount": {
                    "userUri": dag_run.conf['uri'],
                ...
                "policySetScheduleEntries": [rail.result('get_default_policy_modified_uk_non_pto')]
            }
        )
        ...
        search_timeoff_policy_in_mapper_for_timeofftype >> check_if_location_uk

        check_if_location_uk >> rail.Label(
            'No') >> if_timeoff_isnt_pto_sick_vacation
        check_if_location_uk >> rail.Label(
            'Yes') >> if_timeoff_is_pto_uk

        if_timeoff_is_pto_uk >> rail.Label(
            'No') >> if_default_policy_set_present_uk
        if_timeoff_is_pto_uk >> rail.Label(
            'Yes') >> get_default_policy_modified_uk_pto >> put_user_timeoff_accountpolicy_set_schedule_uk_pto >> foreach_timeofftype_assigned_end

        if_default_policy_set_present_uk >> rail.Label(
            'No') >> foreach_timeofftype_assigned_end
        if_default_policy_set_present_uk >> rail.Label(
            'Yes') >> get_default_policy_modified_uk_non_pto >> put_user_timeoff_account_policysetschedule_uk_non_pto >> foreach_timeofftype_assigned_end
