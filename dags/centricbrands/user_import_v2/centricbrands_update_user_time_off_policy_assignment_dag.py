...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'centricbrands_user_import_update_user_time_off_policy_assignment_{config.instance}_v2',
        description=f'CentricBrands_Update User - Time Off policy assignment {config.instance}_v2',
        ...
        search_matching_timeoff_policy = rail.PythonOperator(
            ...
            python_callable=lambda dag_run:  list(filter(lambda entry: entry['timeofftype'] == dag_run.conf['timeofftypename'] and entry[
                ...
                'get_integrationdate_effective_month'), centric_brands_time_off_policy_starting_policy_mapper))
        )
        ...
        put_user_time_off_account_policy_set_schedule_withhistoricalpolicies = rail.RepliconServiceOperator(
            ...
            data={
                ...
                "policySetScheduleEntries": ["{{ result('get_existing_policies') }}",
                                             {
                                                 "effectiveDate": {
                                                     "year": "{{ result('get_today_dateobject').year }}",
                                                     "month": "{{ result('get_today_dateobject').month }}",
                                                     "day": "{{ result('get_today_dateobject').day }}"
                                                 },
                                                 "description": "Effective On {{ result('get_today_dateobject').datestring }}",
                                                 "policySet": "{{ result('get_default_policy_modified') }}"
                                             }
                                             ]
            }
        )
        ...
        search_timeoffpolicy_for_timeofftype_and_month = rail.PythonOperator(
            ...
            ).strftime('%B'), centric_brands_time_off_policy_starting_policy_mapper))
        )
                ...
                "policySetScheduleEntries": ["{{ result('get_existing_policies') }}",
                                             {
                                                 ...
                                                 "description": "Effective On {{ result('get_integrationdate_object').datestring }}",
                                                 "policySet": "{{ result('get_defaultpolicy_modified') }}"
                                             }
                                             ]
            }
        )
                ...
                "policySetScheduleEntries": [
                    {
                        ...
                        "policySet": rail.result('get_defaultpolicy_modified')
                    }
                ]
            }
        )
        ...
        search_timeoff_policy_with_matching_offset = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: list(filter(lambda entry: entry['timeofftype'] == dag_run.conf['timeofftypename'] and entry[
                'offset'] == rail.result(
                'get_final_offset') and entry['month'] == datetime.now().strftime("%B"), centric_brands_time_off_policy_starting_policy_mapper))
        )
        ...
        if_timeoff_other_than_pto_vacation_sick >> rail.Label(
            'Yes') >> get_today_dateobject >> if_default_policy_set_present
