...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'centricbrands_user_import_update_user_{config.instance}_v2',
        description=f'CentricBrands_Update_User {config.instance}_v2',
        ...
        trigger_dag_to_disable_user = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'centric_brands_child_workflow_to_disable_user_{config.instance}_v2',
        ...
        log_department_notfound = rail.PythonOperator(
            ...
            python_callable=lambda dag_run:  "User not updated - Department not found '" +
            dag_run.confd['epartmentname'] + "';"
        )
        ...
        trigger_child_timeofftype_assignment_for_rehireduser = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'centricbrands_user_import_rehire_user_time_off_type_assignment_master_{config.instance}_v2',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
                "uri": "{{ dag_run.conf.uri }}",
                "firstname": "{{ dag_run.conf.firstname }}",
        ...
        trigger_child_timeofftype_assignment_for_updateuser = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'centricbrands_update_user_time_off_type_assignment_{config.instance}_v2',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
                "uri": "{{ dag_run.conf.uri }}",
                "firstname": "{{ dag_run.conf.firstname }}",
        ...
        if_required_officeschedule_uri_not_present >> rail.Label(
            'Yes') >> log_office_schedule_notfound >> if_location_china_hongkong
        if_required_officeschedule_uri_not_present >> rail.Label(
        ...
        if_locationandemployeetypebasedchange_variable_notequal_no >> rail.Label(
            'No') >> get_final_exceptions >> get_final_logs >> add_final_log_for_user >> catch_log_error >> finish >> log_to_sumo
