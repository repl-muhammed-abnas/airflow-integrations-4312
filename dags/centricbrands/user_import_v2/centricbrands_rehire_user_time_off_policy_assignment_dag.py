...
from centricbrands.user_import_v2.mappers.centric_brands_time_off_policy_starting_policy_mapper_mapper import centric_brands_time_off_policy_starting_policy_mapper
import rail
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'centricbrands_user_import_rehire_user_time_off_policy_assignment_{config.instance}_v2',
        description=f'CentricBrands_Rehire User - Time Off policy assignment {config.instance}_v2',
        ...
        def get_modified_default_policy():
            ...
            timeoff_policies_to_assign = json.loads(json.dumps(timeoff_policies_to_assign).replace(
                '[[', '').replace(']]', ''))
