...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'centricbrands_user_import_assign_supervisor_{config.instance}_v2',
        description=f'Live|CentricBrands_Assign Supervisor {config.instance}_v2',
        ...
        if_entry_found >> rail.Label(
            'Yes') >> update_userimport_log_entry >> log_to_sumo
        if_entry_found >> rail.Label('No') >> log_to_sumo
