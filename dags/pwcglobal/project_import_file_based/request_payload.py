...
def get_process_project_conf(item):
    ...
    client_uri = null
    if rail.result('get_client_data')['client_code'] and rail.result('get_client_data')['client_name']:
    ...
    return {
        **{k.lower(): v if v else '' for k, v in item.items() if k != 'CurrentStatus'},
        ...
        **{
            ...
            'sender_environment': rail.result('get_sender_identifier').get('SenderEnvironment'),
            'identifier': rail.result('get_sender_identifier').get('Identifier'),
            'openfortime': (item['CurrentStatus']['OpenForTime']).lower() if item.get('CurrentStatus', {}).get('OpenForTime') else null,
            'confidential_flag': rail.result('for_each_workmanagement').get('ConfidentialFlag'),
            ...
            'confidentiality_flag_dropdown_uri': rail.find_first_by_attr_and_get_attr(
                ...
                if rail.result('for_each_workmanagement').get('ConfidentialFlag') and
                rail.result('for_each_workmanagement').get('ConfidentialFlag') == "true" else "No", "uri"),
...
def get_user_list_for_team_members(item):
    return {
        "page": "1",
        "pagesize": "1000000",
        "columnUris": [
    ...
    }  if rail.get_company_key().lower() == 'pwc' else {
        "page": "1",
        "pagesize": "100000000",
        "columnUris": [
            "urn:replicon:user-list-column:user",
...
def get_update_text_effective_udf_value_payload(dag_run):
    return {
        ...
        rail.result('create_project_with_payload'),
...
def get_apply_department_resource_to_project_payload():
    return {
        "target": {
            "uri": rail.result('create_project_with_payload')
        },
        "modifications": {
            "resourceAssignmentModifications": {
                "resourcesToAdd": [
