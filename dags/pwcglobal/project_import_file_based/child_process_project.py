...
import rail
from pwcglobal.project_import_file_based.custom_method import get_source_input_reference, compare_payload_with_sourceinputkey
from pwcglobal.project_import_file_based.request_payload import get_project_uri, get_add_update_project_conf
from pwcglobal.project_import_file_based.task.log_mandatory_field_exception import log_mandatory_field_exception_task


# config :
# https://github.com/replicon/airflow-integrations/blob/main/dags/pwcglobal/project_import_file_based/config.py


def create_child_process_project_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'pwc_project_import_child_project_flat_file_based_{config.instance}',
        ...
        update_project = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'pwc_project_import_child_update_project_flat_file_based_{config.instance}',
        ...
        create_project = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'pwc_project_import_child_create_project_flat_file_based_{config.instance}',
            conf=get_add_update_project_conf
        )
