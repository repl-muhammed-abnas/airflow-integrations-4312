...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"pwcglobal_user_import_australia_user_allowance_child_process_each_cost_center_records_{config.instance}",
        description=f"PwCGlobal User Import Australia User Allowance child process each cost center records {config.instance}",
        ...
        add_schedule_for_user = rail.RepliconServiceOperator(
            task_id="add_cost_center_schedule_for_user",
            endpoint="services/ImportService1.svc/ApplyUserModifications2",
            data=request_payload.get_add_cost_center_schedule_for_user_applyModification
        )
        ...
        is_both_dates_present >> rail.Label("Yes") >> add_schedule_for_user >> log_success
