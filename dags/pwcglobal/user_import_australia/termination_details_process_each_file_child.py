...
import rail
from pwcglobal.user_import_australia.send_logs_termination import get_send_logs


def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"pwcglobal_user_import_australia_termination_details_process_each_file_child_{config.instance}",
        description=f"PwCGlobal User Import Australia Termination Details process each file child {config.instance}",
        ...
        create_termination_log = rail.CreateLogOperator(
            task_id="create_termination_log"
        )
        ...
        process_each_records = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f"pwcglobal_user_import_australia_termination_details_child_process_each_records_{config.instance}",
            ...
            conf=lambda item, dag_run: {
                ...
                "log": rail.result("create_termination_log")
            },
            retries=0,
        ...
        create_termination_log >> is_file_csv >> rail.Label("Yes") >> download_file >> parse_input_file >> input_data_collection >> input_data >> has_data \
            >> rail.Label("Yes") >> process_each_records >> wait_for_process_each_records >> send_logs
        has_data >> rail.Label("No") >> send_logs >> log_to_sumo
