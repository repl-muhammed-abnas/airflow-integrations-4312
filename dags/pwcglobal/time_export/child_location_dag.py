...
import rail
from pwcglobal.time_export import python_callable_method, request_payload, response_filter
from pwcglobal.time_export.task.current_timesheet_period import current_timesheet_period_task
...
def create_child_location_dag(config):
    ...
    for location in config.location_codes:
        with rail.create_airflow_dag(
            dag_id=f'pwc_time_export_child_location_{location}_{config.instance}',
            ...
            trigger_process_user_batch = rail.TriggerDagRunForEachItemOperator(
                ...
                trigger_dag_id=f'pwc_time_export_user_batch_child_{location}_{config.instance}',
