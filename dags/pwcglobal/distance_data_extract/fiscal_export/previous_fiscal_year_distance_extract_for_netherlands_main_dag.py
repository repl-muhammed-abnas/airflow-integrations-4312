...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id= f"{config.instance}_previous_fiscal_year_distance_data_extract_report_for_Netherlands",
        description= f"PwC - Previous Fiscal Year Distance Extract for Netherlands {config.instance}",
        ...
        get_file_name = rail.PythonOperator(
            ...
            python_callable=lambda: "Fiscal_Distance_Extract_"  +
        ...
        process_previous_fiscal_year_extract_upload_file = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'{config.instance}_process_previous_fiscal_year_extract_upload_file_Child_dag',
            ...
            conf= lambda item: {
                ...
                'report_filter_uri' : rail.find_first_by_attr_and_get_attr(rail.result('get_specific_report_details')["filterConfiguration"]["enabledFilters"],'displayText', config.filter_name, 'uri'),
