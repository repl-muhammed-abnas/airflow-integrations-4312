...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id= f"pwcglobal_previous_fiscal_year_distance_data_extract_report_for_netherlands_{config.instance}_v1",
        description= f"PwC - Previous Fiscal Year Distance Extract for Netherlands {config.instance} V1",
        ...
        process_start_time = rail.PythonOperator(
            ...
            python_callable=lambda: pendulum.now(config.europe_timezone).strftime("%Y%m%d%H%M%S")
        )

        get_file_name = rail.PythonOperator(
            task_id="get_file_name",
            python_callable=lambda: "Fiscal_Distance_Extract_" + rail.result("process_start_time") + "_NLD.csv"
        )
        ...
        def last_day_of_month(item):
            start_of_the_previous_year = dt(dt.now().year -1, 1,1)
            start_of_the_previous_year_plus_sixmonths = start_of_the_previous_year + relativedelta(months = 6)
            months_to_add = int(((12/ config.batch_size) * (item+1)) - 1)
            end_date_to_use=start_of_the_previous_year_plus_sixmonths + relativedelta(months=months_to_add)
            res = end_date_to_use + relativedelta(day=31)
            return res.date()

        def start_date_of_month(item):
            start_of_the_previous_year = dt(dt.now().year -1, 1,1)
            start_of_the_previous_year_plus_sixmonths = start_of_the_previous_year + relativedelta(months = 6)
            months_to_add = int(((12/ config.batch_size) * item))
            end_date_to_use=start_of_the_previous_year_plus_sixmonths + relativedelta(months=months_to_add)
            res = end_date_to_use + relativedelta(day=1)
            return res.date()

        process_previous_fiscal_year_extract_upload_file = rail.TriggerDagRunForEachItemOperator(
            task_id='process_previous_fiscal_year_extract_upload_file',
            retries=0,
            items=list(range(0, config.batch_size)),
            trigger_dag_id=f'pwcglobal_process_previous_fiscal_year_extract_upload_file_child_dag_{config.instance}_v1',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf= lambda item: {
                'index':  item,
                'batch_size': config.batch_size,
                'report_uri': rail.result('get_specific_report_details')['uri'],
                'report_filter_uri' : rail.find_first_by_attr_and_get_attr(
                    rail.result('get_specific_report_details')["filterConfiguration"]["enabledFilters"],
                        'displayText', config.filter_name, 'uri'),
                'start_date' :  str(start_date_of_month(item).strftime("%m/%d/%Y")),
                'end_date'  : str(last_day_of_month(item).strftime("%m/%d/%Y")),
                'sequence_no' : item + 1,
                'file_name' : rail.result('get_file_name')
            }
        )

        wait_to_process_previous_fiscal_year_extract = rail.WaitForDagRunsSensor(
            task_id='wait_to_process_previous_fiscal_year_extract',
           execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs='{{ result("process_previous_fiscal_year_extract_upload_file") }}',
        )
        ...
        final_single_report_data_to_csv = rail.WriteCSVFileOperator2(
            ...
            header=["TransactionDate","TimeEntryID","PartyID","ResourceGrade","LegalEntityPartyID",
                    "WorkDayId","TimesheetStartDate","TimesheetEndDate","Mileage","ChargeCode",
                    "WorkItemType","Comments","Distance"],
        ...
        combine_report_data = rail.QueryCollectionOperator(
            ...
            query='''SELECT * FROM report_data_batch_I LEFT JOIN report_data_batch_II
                    USING (TimeEntryID) UNION ALL SELECT * FROM report_data_batch_II
                    LEFT JOIN report_data_batch_I USING (TimeEntryID)'''
        )
        ...
        process_start_time>> get_file_name >> get_specific_report_details >> process_previous_fiscal_year_extract_upload_file \
        >> wait_to_process_previous_fiscal_year_extract >> get_data_from_child >> child_has_data >> both_export_data_is_available
