...
import pendulum
from pwcglobal.distance_data_extract.daily_export_v1 import request_payload
import rail

def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id= f"pwcglobal_distance_data_extract_daily_report_for_netherlands_{config.instance}_v1",
        description= f"Daily Distance Extract for Netherlands {config.instance} V1",
        ...
        get_file_name = rail.PythonOperator(
            ...
            python_callable=lambda: "Daily_Distance_Extract_" + rail.result("process_start_time") + "_NLD.csv"
        )
        ...
        final_data_to_csv = rail.WriteCSVFileOperator2(
            ...
            header=["TransactionDate","TimeEntryID","PartyID","ResourceGrade",
                    "LegalEntityPartyID","WorkDayId","TimesheetStartDate","TimesheetEndDate",
                    "Mileage","ChargeCode","WorkItemType","Comments","Distance"],
