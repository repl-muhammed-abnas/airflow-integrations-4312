...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"pwcglobal_previous_calendar_year_distance_data_extract_report_for_netherlands_{config.instance}_v1",
        description=f"Calendar Year Distance Extract for Netherlands {config.instance} V1",
        ...
        get_file_name = rail.PythonOperator(
            ...
            python_callable=lambda: "Yearly_Distance_Extract_" + rail.result("process_start_time") + "_NLD.csv"
        )
        ...
        process_previous_calendar_year_extract_upload_file = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'pwcglobal_process_previous_calendar_year_extract_upload_file_child_dag_{config.instance}_v1',
        ...
        final_single_report_data_to_csv = rail.WriteCSVFileOperator2(
            ...
            header=["TransactionDate", "TimeEntryID", "PartyID", "ResourceGrade", "LegalEntityPartyID",
                    "WorkDayId", "TimesheetStartDate", "TimesheetEndDate", "Mileage", "ChargeCode", "WorkItemType", "Comments", "Distance"],
