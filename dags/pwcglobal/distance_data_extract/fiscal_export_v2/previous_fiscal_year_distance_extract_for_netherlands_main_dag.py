...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f"PwC - Previous Fiscal Year Distance Extract for Netherlands",
        ...
        def is_scheduled_run_date(dag_run, config):
            """
            Validates if the DAG is running on the scheduled date (July 15).
            ...
            # Check if current date is July 15
            current_day = current_date_in_tz.day
            current_month = current_date_in_tz.month

            # Should only run on 15th day of July (7)
            return current_day == 15 and current_month == 7
        ...
        def last_day_of_month(item):
            ...
            start_of_the_previous_year_plus_sixmonths = start_of_the_previous_year + \
                relativedelta(months=6)
            months_to_add = int(((12 / config.batch_size) * (item+1)) - 1)
            end_date_to_use = start_of_the_previous_year_plus_sixmonths + \
        ...
        process_previous_fiscal_year_extract_upload_file = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda item: {
                ...
                'report_filter_uri': rail.find_first_by_attr_and_get_attr(
                    rail.result('get_specific_report_details')[
        ...
        send_export_complete_email = rail.EmailOperator(
            ...
            subject="{{ get_company_key() }} | Previous Fiscal Year Distance Extract for Netherlands -{{result('process_start_time')}}",
        ...
        process_start_time >> get_file_name >> get_specific_report_details >> process_previous_fiscal_year_extract_upload_file \
            >> wait_to_process_previous_fiscal_year_extract >> get_data_from_child >> child_has_data
