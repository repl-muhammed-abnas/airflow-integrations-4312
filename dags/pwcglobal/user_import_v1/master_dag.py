...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'pwcglobal_user_import_master_{config.instance}_v1',
        description=f'PwCGlobal - User Import_Master_Process XML input file {config.instance} V1',
        ...
        process_new_scheduels = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'{config.schedule_dag_id}_{config.instance}_v1',
            conf={
        ...
        process_new_locations = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'{config.location_dag_id}_{config.instance}_v1',
            conf={
        ...
        trigger_parallel_dagrun = rail.trigger_parallel_dagrun(
            ...
            execution_timeout=timedelta(days=config.execution_timeout_days),
            trigger_dag_id=f'{config.user_dag_id}_{config.instance}_v1',
        ...
        process_users = rail.PythonOperator(
            ...
            python_callable=lambda: list(itertools.chain(
                *list(map(lambda x: rail.result(
                    f'trigger_parallel_dagrun_{x+1}'), range(parallel_count)))))
        )
        ...
        process_supervisor = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'{config.supervisor_dag_id}_{config.instance}_v1',
            conf={
        ...
        has_invalid_users >> rail.Label(
            'Yes') >> log_invalid_users >> dummy_invalid_users >> get_errored_master_logs

        has_invalid_users >> rail.Label(
            'No') >> dummy_invalid_users >> get_errored_master_logs
        ...
        create_user_collection >> query_valid_users >> has_valid_users >> rail.Label('Yes') >> query_distinct_schedule >> \
            [query_distinct_country, get_all_office_schedule, get_all_location, get_all_division,
             get_enabled_emp_groups, get_dept_group, get_all_policy_sets, get_all_approval_paths, get_all_holiday_calendars,
             get_all_timezones, get_all_timeofftypes, get_all_permissionset, get_all_customfields] >> has_grade_customfield

        has_grade_customfield >> rail.Label("Yes") >> get_grade_options >> has_profilestatus_customfield
        has_grade_customfield >> rail.Label("No") >> has_profilestatus_customfield

        has_profilestatus_customfield >> rail.Label(
            'Yes') >> get_profilestatus_options >> process_office_and_location
        has_profilestatus_customfield >> rail.Label(
            'No') >> process_office_and_location
        ...
        get_conf_uris >> trigger_parallel_dagrun >> process_users >> gather_supervisor >> gather_logs >> \
            process_supervisor >> wait_for_process_supervisor >> \
            get_errored_master_logs >> load_master_log

        load_master_log >> format_logs >> get_errored_logs >> get_exception_logs >> \
            write_xml_file >> upload_xml_to_sftp >> can_upload_xml_to_secondary_filepath >> rail.Label('Yes') >> upload_xml_to_secondary_filepath

        can_upload_xml_to_secondary_filepath >> rail.Label('No') >> generate_download_link
