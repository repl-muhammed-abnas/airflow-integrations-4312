...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'pwc_project_client_master_flat_file_based_{config.instance}_v1',
        ...
        get_file_manually = rail.IfOperator(
            ...
            test=lambda: Variable.get(f"pwc_project_client_master_flat_file_based_get_file_manually_variable_{config.instance}_v1", default_var="false") == "true",
        ...
        def get_download_file_name(dag_run):
            if Variable.get(f"pwc_project_client_master_flat_file_based_get_file_manually_variable_{config.instance}_v1", default_var="false") == "true":
        ...
        process_projects = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'pwc_project_import_child_project_flat_file_based_{config.instance}_v1',
        ...
        process_log_generation = rail.TriggerDagRunOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'pwc_project_import_child_log_flat_file_based_{config.instance}_v1',
            conf=lambda: {
