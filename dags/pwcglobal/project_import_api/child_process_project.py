...
import rail
from pwcglobal.project_import_api.custom_method import get_source_input_reference, compare_payload_with_sourceinputkey
from pwcglobal.project_import_api.request_payload import get_project_uri, get_add_update_project_conf
from pwcglobal.project_import_api.task.log_mandatory_field_exception import log_mandatory_field_exception_task
...
def create_child_process_project_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'pwc_project_import_child_process_project_b1_{config.instance}',
        ...
        update_project = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=datetime.timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'pwc_project_import_child_update_project_b1_{config.instance}',
        ...
        create_project = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=datetime.timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'pwc_project_import_child_create_project_b1_{config.instance}',
            conf=get_add_update_project_conf
        )
