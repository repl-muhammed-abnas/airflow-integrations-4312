...
from pwcglobal.project_import_api.custom_method import get_unique_sender_ids_by_companykey
...
def create_log_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'pwc_project_import_master_log_scheduled_{config.instance}',
        ...
        def get_dagruns_to_process(session=NEW_SESSION):
            ...
            dag_runs_to_filter = (
                ...
                .select_from(DagRun)
                .filter(
                    DagRun.dag_id == f'pwc_project_import_child_log_pregeneration_{config.instance}', DagRun.state.in_(
        ...
        process_logs_by_sender = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'pwc_project_import_child_log_{config.instance}',
            conf={
                'sender_id': "{{ item }}",
