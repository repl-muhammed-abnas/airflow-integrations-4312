...
import rail
from pwcglobal.user_import.mapper.timesheet_policy import timesheet_policy_mapper
from pwcglobal.user_import.mapper.general import general_mapper
null = None
true = True
false = False
...
def get_process_user_conf(item):
    ...
    return {
        ...
        **{
            ...
            'useruri': rail.find_first_by_attr_and_get_attr(rail.result('load_all_report_users') or [], 'Login_Name', item['LoginName'], 'uri'),
        }
    }
...
def get_timesheet_schedule_param():
    schedules = list(map(lambda x:
                         ...
                         filter(lambda x: not x["effectiveDate"] or
                                get_date_from_replicon_date(
                             x["effectiveDate"]) != get_date_from_replicon_date(get_today_date()),
                             rail.result('get_current_timesheetperiod_schedule'))))
...
def get_office_schedule_param():
    schedules = list(map(lambda x:
                         ...
                         filter(lambda x: not x["effectiveDate"] or
                                get_date_from_replicon_date(
                                    x["effectiveDate"]) != get_date_from_replicon_date(get_today_date()),
                                rail.result('bulk_get_user3')['schedulePolicies'])))
...
def get_policy_sets():
    ...
    timeoff_policy = next(filter(lambda x: x['Country'] == 'Global' and x['Type'] == 'Time off Template'
                                 and x['Identifier'] == get_conf()['country'] and x['Value'], general_mapper), None)
...
def get_custom_field_values():
    ...
    if conf['grade'] and customfielduri['grade']:
        custom_fields.append({
            'customField': {
...
def get_work_week():
    return next(filter(lambda x: x['Country'] == 'Global' and
                       x['Type'] == 'workweek' and
                       x['Identifier'] == get_conf()['workweek'] and
                       x['Value'],
                       general_mapper), {}).get('Value', 'urn:replicon:day-of-week:sunday')
...
def get_create_user_data():
    ...
    return {
        "user": {
            ...
            "emailAddress": conf['emailaddress'] if conf['emailaddress'] and 'emailaddress' not in validationlog else null,
            "employeeId": conf['employeeid'],
            "department": null,
            "supervisorAssignmentSchedule": null,
            "schedulePolicySchedule": get_schedule_param(),
            "workWeekStartDayUri": get_work_week(),
            "employmentDateRange": {
