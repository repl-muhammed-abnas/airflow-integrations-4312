import rail
from pwcglobal.time_export_v3.request_payload import get_paris_timenow_in_fmt, get_transaction_date_list, get_final_extract_data_row
from pwcglobal.time_export_v3.python_callable_method import get_api_payload, gather_all_api_logs
from pwcglobal.time_export_v3.response_filter import retrieve_export_uri
from pwcglobal.time_export_v3.task.time_data_export import time_data_export
from pwcglobal.time_export_v3.task.post_to_endpoint import get_post_to_endpoint
...
def create_child_current_past_period_dag(config):
    ...
    for location in config.location_codes:
        with rail.create_airflow_dag(
            dag_id=f'pwc_time_export_child_current_past_period_{location}_{config.instance}_v3',
            ...
            is_valid_data_present_to_log = rail.IfOperator(
                ...
                test="{{ result('valid_extracted_data', 'length') > 0 }}",
                yes_task="get_api_endpoint_details_for_location",
                no_task="process_invalid_records"
            )

            def get_api_endpoint_details_for_location_from_mapper(dag_run):
                return rail.find_first_by_attr_and_get_attr(config.api_details_mapper, "location", dag_run.conf['location'], 'api_details')

            get_api_endpoint_details_for_location = rail.PythonOperator(
                task_id="get_api_endpoint_details_for_location",
                python_callable=get_api_endpoint_details_for_location_from_mapper
            )

            can_post_to_api = rail.IfOperator(
                task_id="can_post_to_api",
                test=lambda: (bool(rail.find_first_by_attr_and_get_attr(
                    rail.result('get_api_endpoint_details_for_location'), 'can_post_to_api', 'Yes')) and rail.result('valid_extracted_data', 'length') > 0),
                yes_task="prepare_api_payload",
                no_task="get_export_file_log_count"
            )

            prepare_api_payload = rail.PythonOperator(
                task_id="prepare_api_payload",
                python_callable=get_api_payload
            )

            # the endpoint index needs to be start from 1 onwards
            post_to_endpoint_1_start, finish_1, post_to_endpoint_1_end = get_post_to_endpoint(
                config, endpoint_index="1")
            post_to_endpoint_2_start, finish_2, post_to_endpoint_2_end = get_post_to_endpoint(
                config, endpoint_index="2")
            post_to_endpoint_3_start, finish_3, post_to_endpoint_3_end = get_post_to_endpoint(
                config, endpoint_index="3")

            def get_export_log_count(dag_run):
                paris_time_now = get_paris_timenow_in_fmt()
                # pylint: disable=cell-var-from-loop
                api_log = gather_all_api_logs()
                logs = [
                    ...
                    {'log': f'{paris_time_now} {api_log}'} if api_log else {
                        'log': None},
            ...
            send_invalid_record_email = rail.EmailOperator(
                ...
                params={
                    'dag_id': f'pwc_time_export_child_current_past_period_{location}_{config.instance}_v3',
                    'parent_dag_id': f'pwc_time_export_child_location_{location}_{config.instance}_v3'
                }
            )
            ...
            is_valid_data_present_to_log >> rail.Label(
                "Yes") >> get_api_endpoint_details_for_location >> can_post_to_api >> rail.Label("No") >> get_export_file_log_count >> render_logs_csv

            can_post_to_api >> rail.Label(
                "Yes") >> prepare_api_payload

            prepare_api_payload >> [post_to_endpoint_1_start, post_to_endpoint_2_start,
                                    post_to_endpoint_3_start]
            [finish_1, finish_2, finish_3] >> get_export_file_log_count
            [post_to_endpoint_1_end, post_to_endpoint_2_end, post_to_endpoint_3_end
             ] >> get_export_file_log_count
            ...
            catch_and_log_errors >> dagrun_log_to_sumo >> should_fail_dag
