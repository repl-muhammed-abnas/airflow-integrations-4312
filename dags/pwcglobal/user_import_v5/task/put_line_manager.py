...
def put_line_manager_udf(status):
    ...
    with rail.TaskGroup(group_id='put_line_manager', prefix_group_id=False) as put_line_manager:
        ...
        has_valid_linemanager = rail.IfOperator(
            ...
            )["linemanagerpartyid"] != request_payload.get_conf()['employeeid'],
        ...
        if_multiple_users_found_with_legal_entity_id = rail.IfOperator(
            task_id="if_multiple_users_found_with_legal_entity_id",
            test=lambda: rail.result("get_user_with_legal_entity_id","users_count") > 1,
            yes_task="line_manager_complete",
            no_task="if_line_manager_with_legal_entity_id"
        )
        ...
        line_manager_complete = rail.EmptyOperator(
            task_id="line_manager_complete")

        has_valid_linemanager >> rail.Label("Yes") >>\
            get_user_with_line_manager_partyid >>\
            if_line_manager_in_replicon >> rail.Label("Yes") >> get_user_with_legal_entity_id >>\
                if_multiple_users_found_with_legal_entity_id
        if_multiple_users_found_with_legal_entity_id >> rail.Label(
            "Yes") >> line_manager_complete
        if_multiple_users_found_with_legal_entity_id >> rail.Label(
            "No") >> if_line_manager_with_legal_entity_id
        ...
        if_line_manager_in_replicon >> rail.Label(
            "No") >> line_manager_complete
