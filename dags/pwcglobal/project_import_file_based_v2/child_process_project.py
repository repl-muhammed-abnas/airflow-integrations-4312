...
import rail
from pwcglobal.project_import_file_based_v2.custom_method import get_source_input_reference, compare_payload_with_sourceinputkey
from pwcglobal.project_import_file_based_v2.request_payload import get_project_uri, get_add_update_project_conf
from pwcglobal.project_import_file_based_v2.task.log_mandatory_field_exception import log_mandatory_field_exception_task
...
def create_child_process_project_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'pwc_project_import_child_project_flat_file_based_{config.instance}_v2',
        description=f'PWC Process Project {config.instance}',
        ...
        update_project = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'pwc_project_import_child_update_project_flat_file_based_{config.instance}_v2',
        ...
        create_project = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'pwc_project_import_child_create_project_flat_file_based_{config.instance}_v2',
            conf=get_add_update_project_conf
        )
        ...
        log_dagrun_to_sumo = rail.EmptyOperator(
        ...
        create_log >> get_md5_from_payload >> get_project_details_from_code >> is_project_exists
