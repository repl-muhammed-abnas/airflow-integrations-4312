import rail
from pwcglobal.project_import_file_based_v2 import custom_method
...
def get_client_name_code_partyid():
    client_name = null
    client_code = null
    client_party_id = null
    work_management_data = rail.result('for_each_workmanagement')
    ...
    if client_data:
        ...
        client_code = client_data['PartyAlternateIdentifierValue'] if client_data.get(
...
def add_to_respective_user_list(role_types):
    def get_user_uris(item):
        user_uris = []
        if custom_method.get_user_from_list_by_interperson_id(item, user_list):
            user_uris = list(set(x['uri'] for x in custom_method.get_user_from_list_by_interperson_id(item, user_list)))

        return {
            'user_uri': "".join(user_uris),
            'count': len(user_uris)
        }
    ...
    if role_types[0].endswith("Manager") or role_types[0].endswith("Partner"):
        user_list = [item for item in rail.result('get_user_list_for_manager_and_comanager') if item]
    ...
    return list(filter(lambda x: x['user_uri'] and (x['assignment_required'] == "Yes" or
        ...
        list(map(lambda item: {
            ...
            "user_uri": get_user_uris(item)['user_uri'],
            "user_count": get_user_uris(item)['count'],
...
def add_to_individual_team_member_list():
    ...
    return list(set(team_member_uris))

def get_user_uris_from_list(result):
    return [item['user_uri'] for item in result if item['user_count'] == 1] if result else []


def get_permission_user_uri(manager_type, task_id):
    user_uris = [x['user_uri']
                 for x in rail.result(task_id) if x['user_count'] == 1] if rail.result(task_id) else null
...
def do_get_exception_logs_add(dag_run):
    ...
    if rail.result("get_project_manager_to_assign") and rail.result("get_project_manager_to_assign")[0]['user_count'] > 1:
        logs.append(
            "Multiple Users found in replicon for Project manager with the same 'User Party ID' and 'Legal Entity Party ID'")

    elif not rail.result('get_permissionuri_useruri_project_manager'):
        logs.append(
            'Project manager not assigned since user with given party ID was not found or from a different country as of the Project.')

    if rail.result("get_project_co_manager_to_assign") and rail.result("get_project_co_manager_to_assign")[0]['user_count'] > 1:
        logs.append(
            "Multiple Users found in replicon for Project Delivery Partner with the same 'User Party ID' and 'Legal Entity Party ID'")

    elif not rail.result('get_permissionuri_useruri_project_comanager'):
        logs.append(
            'Project Delivery Partner not found with party ID and legal entity ID or from a different country as of the Project.')
...
def get_required_department_parent_data(response):
    if response:
        parent_department_list = response['parentDepartments']
        if len(parent_department_list) > 1:
            return response['parentDepartments'][1]['uri']
        return rail.result('map_department_list_data')['companycodeuri']
    return None
