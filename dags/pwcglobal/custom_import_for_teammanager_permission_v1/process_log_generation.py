...
import rail
from pwcglobal.custom_import_for_teammanager_permission_v1.utils.python_callable import do_format_logs

def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_log_generation,
        description=f'PwC Custom Import for Team Manager Supervisory Org Restriction Process Log Generation {config.instance}',
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_logs', 'error_record_count') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Custom Import to Team Manager Permission with Supervisory Org Restriction " }} \
