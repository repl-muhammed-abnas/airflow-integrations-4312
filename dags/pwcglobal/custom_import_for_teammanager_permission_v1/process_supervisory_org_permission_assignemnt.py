...
import rail
from pwcglobal.custom_import_for_teammanager_permission_v1.utils import python_callable, request_payload
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        get_assigned_supervisory_org_child_levels_uri = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response, dag_run: python_callable.get_required_supervisory_org_level_uris(response, dag_run)
        )

        check_for_current_run_logs = rail.FilterLogEntriesOperator(
            task_id='check_for_current_run_logs',
            log="{{ dag_run.conf.current_run_sup_org_log }}",
            properties={
        ...
        append_sup_org_to_logged_data = rail.PythonOperator(
            task_id='append_sup_org_to_logged_data',
            python_callable=python_callable.append_supervisory_org_to_logged_data
        )
        ...
        log_current_run_sup_org_to_assign = rail.WriteLogOperator(
            task_id="log_current_run_sup_org_to_assign",
            log = "{{ dag_run.conf.current_run_sup_org_log }}",
            message = "success",
            properties={
                "guid": "{{ dag_run.conf.guid }}",
                "supervisory_org_level": '{{ result("append_sup_org_to_logged_data") }}'
            }
        )
        ...
        if_supervisory_org_level_path_present >> rail.Label('Yes') >> get_preassigned_restrictions >> remove_pre_assigned_restrictions >> \
            get_assigned_supervisory_org_child_levels_uri >> check_for_current_run_logs >> append_sup_org_to_logged_data >> assign_supervisory_orgs >> \
                log_current_run_sup_org_to_assign >> log_project_process
