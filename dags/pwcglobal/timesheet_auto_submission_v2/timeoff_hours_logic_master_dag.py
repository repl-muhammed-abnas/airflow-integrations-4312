...
import rail
from pwcglobal.timesheet_auto_submission_v2.task.timeoff_hours_specific_scenarios import get_specific_scenarios
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'pwcglobal_timesheet_auto_submission_timeoff_hours_v4_master_{config.instance}',
        description=f'PwCGlobal - Timesheet auto submission timeoff hours Master_v4.0 {config.instance}',
        ...
        query_distinct_statistical_record = rail.QueryCollectionOperator(
            ...
            query='''SELECT DISTINCT
                        timesheeturi,totalhours,Scheduled_Hrs__In_Period_,projecttype,useruri,daydiff,Timesheet_Start_Date,Timesheet_End_Date,User_Name
        ...
        has_log_data = rail.IfOperator(
            ...
            test='{{ (result("query_distinct_valid_timesheet", "length") | is_truthy and result("query_distinct_valid_timesheet", "length") > 0) or (result("query_final_data", "length") | is_truthy and result("query_final_data", "length")) > 0 }}',
        ...
        report_has_data >> rail.Label("Yes") >> \
            ...
            query_timeoff_user >> query_distinct_statistical_record >> query_distinct_nonstatistical_record >> query_distinct_valid_timesheet
        query_distinct_valid_timesheet >> process_timesheet >> wait_for_process_timesheet >> specific_scenarios >> has_log_data
