...
import rail
from pwcglobal.user_import_v7.config import timeoff_approval_path_mapper
from pwcglobal.user_import_v7.config import language_mapper
from pwcglobal.user_import_v7.config import work_compliance_policy_mapper
null = None
true = True
false = False
...
def get_process_user_conf(item,config):
    # Descope/termination (IsLoginEnabled = 'No'): reduced payload may omit optional columns, so pass through what it carries plus only the keys the disable flow needs.
    if (item.get('IsLoginEnabled') or '').strip() == 'No':
        return {
            'startdate': '',  # update_end_date reads conf['startdate']; payload may omit <StartDate> (blank => keep existing)
            **{k.lower(): v for k, v in item.items()},
            'has_loaded_report_users': bool(rail.result('load_all_report_users')),
            'useruri': rail.find_first_by_attr_and_get_attr(  # search_user reads conf['useruri'] directly (get_user_uri)
                rail.result('load_all_report_users') or [], 'Login_Name',
                item.get('LoginName'), 'useruri'),
            'is_descope_event': true,  # update child DAG gates on this flag
        }
