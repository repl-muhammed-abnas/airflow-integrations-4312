        ...
        map(lambda item:
            {
                ...
                'fullpath': [x['textValue'] for x in item['cells'][2]['cellCollection']] if item['cells'][2]['cellCollection'] else null
            }, data)
    )
...
def map_supervisor_list(res):
    data = res.json()['d']['rows']
    return list(
        filter(lambda x: x['employeeid'] == request_payload.get_conf()['supervisor'].split('||')[0], map(
            lambda item: {
...
def clean_value(value):
    # Empty XML fields arrive as the string "None"; treat those (and blanks/whitespace-only)
    # as empty. Genuine values are returned unchanged so distinct ids are not merged.
    if value is None or str(value).strip() in ('', 'None'):
        return ''
    return value

def get_user_log_key(record):
    # Group by user party id, falling back to login name when it's missing (e.g. disabled users).
    properties = record['properties']
    return clean_value(properties.get('userpartyid')) or clean_value(properties.get('loginname')) or ''
...
def do_format_logs():
    ...
    group_keys = list(set(map(get_user_log_key, master_log)))
    logs = []
    # pylint: disable=cell-var-from-loop
    for group_key in group_keys:
        user_logs = list(
            filter(lambda x: get_user_log_key(x) == group_key and x['properties'].get('message', ''), master_log))
        error_logs = list(
        ...
        if len(user_logs) > 0:
            ...
            login_name = next((clean_value(log_entry['properties'].get('loginname')) for log_entry in user_logs
                               if clean_value(log_entry['properties'].get('loginname'))), '')
            logs.append({
                # 2022-04-29 08:20:49
                'Date': parser.parse(first['timestamp']).strftime('%Y-%m-%d %H:%M:%S'),
                'LegalentityID': clean_value(first['properties'].get('legalentityid')),
                'UserPartyID': clean_value(first['properties'].get('userpartyid')),
...
def user_with_legal_entity_id():
    rail.set_result(key='users_count', val=len([item for item in rail.result("get_user_with_line_manager_partyid") if item[
        'legalentityuri'] == request_payload.get_conf()["linemanagerlegalentityuri"]]))
...
def do_get_exception_logs():
    ...
    if not request_payload.get_conf()["country"] and request_payload.get_conf()["toil"] == "Y":
        logs.append(
            "TOIL time off type is not assigned as country is not specified")
    if rail.result("if_fte_effective_date_in_past") == "get_user_details" and \
        ...
        logs.append(
            "Product licencse not assigned for user hence fte percent with past effective date not updated")
    if rail.result("if_timesheet_template_assigned") and\
            not rail.result("get_current_timesheet_period"):
        logs.append(
            "Time sheet not assigned for user hence fte percent with past effective date not updated")
    if rail.result("if_line_manager_with_legal_entity_id") == "line_manager_complete":
        logs.append(
            "Mulitple users with same linemanager party id or legal entity id")
    ...
    if rail.result("if_multiple_users_found_with_legal_entity_id") == "line_manager_complete":
        logs.append("Multiple users found with same Line Manager Party ID and Legal Entity ID")
    return logs


def get_user_data(response):
    return list(filter(lambda x: request_payload.get_conf()["linemanagerpartyid"] == x["partyid"], list(map(
        lambda i: {
...
def get_permission_set(permission_add):
    ...
    user_permission_sets = list(map(
        lambda i: {
            "uri": i["permissionSet"]["uri"],
            "displayText": i["permissionSet"]["displayText"]
        }, user_permission_sets))
    if permission_add == "remove":
        permission_set = list(filter(
            lambda i: i["displayText"] == "ZT User without collectors", user_permission_sets))
        ...
        if request_payload.get_conf()["zerotimeuserpermissionseturi"] not in permission_set:
            permission_set.append(request_payload.get_conf()[
...
def check_non_zt_country(dag_run):
    current_country = request_payload.get_attr_value(rail.result(
        "get_effective_user_groupmembership"), 'locations.0.location.location.displayText')
...
def check_update_zerotime(dag_run):
    current_country = request_payload.get_attr_value(rail.result(
        "get_effective_user_groupmembership"), 'locations.0.location.location.uri')
...
def is_current_payrule_matching(pay_rule_schedule, expected_uri):
    """
    Checks if the current active pay rule matches the expected pay rule URI.

    Args:
        pay_rule_schedule (list): List of pay rule entries with effectiveDate and payRuleScript.
        expected_uri (str): The expected pay rule URI to compare.

    Returns:
        bool: True if current active pay rule URI matches expected_uri, else False.
    """
    from datetime import date

    if not pay_rule_schedule:
        return False

    today = date.today()
    active_rules = []

    for rule in pay_rule_schedule:
        eff_date = rule.get("effectiveDate")
        if eff_date:
            try:
                eff_date_obj = date(eff_date["year"], eff_date["month"], eff_date["day"])
                if eff_date_obj <= today:
                    active_rules.append((eff_date_obj, rule["payRuleScript"]["uri"]))
            except Exception:
                continue  # Skip malformed dates
        else:
            # Rule with no effectiveDate (initial assignment)
            active_rules.append((None, rule["payRuleScript"]["uri"]))

    if not active_rules:
        return False

    # Prefer rules with a valid date ≤ today
    dated_rules = [r for r in active_rules if r[0] is not None]

    if dated_rules:
        latest_uri = max(dated_rules, key=lambda x: x[0])[1]
    else:
        latest_uri = active_rules[0][1]  # fallback to the initial rule

    return latest_uri == expected_uri


def do_map_logs():
    """
    Map and format logs from successful tasks and validation logs.
    Builds logs_map internally to avoid closure issues.

    Returns:
        str: Semicolon-separated log messages.
    """
    # Build logs_map with hardcoded task IDs
    logs_map = {
        'update_start_date': 'Start Date updated',
        'update_timesheet_periodtype': 'Timesheet period overwritten with new hire date',
        'enable_login': 'User re-enabled',
        ...
        'update_partyid_udf': 'Party ID updated',
        ...
        'update_emptype': 'Employee type updated',
        'update_companycode': 'Agencies (Department group) updated',
        ...
        'update_displayname': 'Display name updated',
        'put_timesheetperiodschedule_for_user': 'Timesheet period updated',
        'update_supervisor_assignmentschedule_overdaterange': 'Supervisor updated',
        'update_timezone': 'Time zone updated',
        'update_timesheet_template': 'Timesheet template updated',
        'update_timesheet_path': 'Timesheet approval path updated',
        'update_holiday_calendar': 'Holiday calendar updated',
        'update_office_schedule': 'Office schedule updated',
        'update_login_name': 'Login name updated',
        'update_timeofftypes': "Toil Time off type updated",
        'assign_initial_payrule_for_user': "Payrule updated",
        'assign_payrules_for_user': "Payrule updated",
        'put_time_entry_approval_path': "Time entry approval path updated",
        'update_toil_udf': "TOIL updated",
        'update_ftepercent_udf': "FTEpercent updated",
        'update_line_manager': "Line manager updated",
        'put_system_approval_time_entry_path': "Time entry approval path updated",
        'put_new_key_value_to_ftevalue_space': "FTE effective date updated",
        'put_key_value_to_ftevalue_space': "FTE effective date updated",
        'assign_zerotime_license_and_permission': "Zerotime license and ZT user permission updated",
        'unassign_zerotime_license': "Zerotime license and ZT user permission removed",
    }

    logs = []
    if rail.result("if_multiple_users_found_with_legal_entity_id") == "line_manager_complete":
        request_payload.get_conf()['validationlog'].append(
            {"message": "Multiple users found with same Line Manager Party ID and Legal Entity ID"})
    if rail.result("has_valid_linemanager") == "line_manager_complete":
        request_payload.get_conf()['validationlog'].append(
            {"message": "Line manager party id and user party id are the same"})
    if len(request_payload.get_conf().get('validationlog', [])) > 0:
        logs.extend(list(
            map(lambda item: item['message'], request_payload.get_conf()['validationlog'])))
    if rail.result('get_timesheetperiodtype_uri', 'log'):
        logs.append(rail.result('get_timesheetperiodtype_uri', 'log'))
    if rail.result('get_location_details') and not rail.result('get_location_details').get('code'):
