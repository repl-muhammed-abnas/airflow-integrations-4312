...
def get_update_supervisor(user_uri, can_queue_assignment=True):
    ...
    with rail.TaskGroup(group_id='update_supervisor', prefix_group_id=False) as update_supervisor:
        ...
        def do_map_updated_supervisor_details():
            if rail.result(search_supervisor_by_legalentity.task_id):
                if len(rail.result(search_supervisor_by_legalentity.task_id)) == 1:
                    return rail.result(search_supervisor_by_legalentity.task_id)[0]
                if len(rail.result(search_supervisor_by_legalentity.task_id)) > 1:
                    return [user for user in rail.result(search_supervisor_by_legalentity.task_id) if user['enabled']][0]
        ...
        map_supervisor_details = rail.PythonOperator(
            ...
            python_callable=lambda: do_map_supervisor_details() if rail.get_company_key().lower() == 'pwc' else do_map_updated_supervisor_details()
        )
        ...
        has_supervisor_legalentity_uri >> rail.Label('Yes') >> search_supervisor_by_legalentity >> \
            map_supervisor_details >> can_assign_supervisor
