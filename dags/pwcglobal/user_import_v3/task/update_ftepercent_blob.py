...
def update_ftepercent_blob(config):
    with rail.TaskGroup(group_id="update_fte_blob", prefix_group_id=False) as update_blob:
        ...
        if_fte_date_in_past = rail.IfOperator(
            ...
            test=lambda:bool(request_payload.check_past_date() and request_payload.check_past_date() > 0),
        ...
        if_key_value_exists_for_user = rail.IfOperator(
            ...
            test=lambda :bool(rail.result("get_all_key_value_for_fte_value") and "jsonValue" in rail.result("get_all_key_value_for_fte_value")),
        ...
        if_product_license_present >> rail.Label("No") >> update_validation_log_for_product_fte >> fte_end
        if_timesheet_template_assigned >> rail.Label("No") >> update_validation_log_for_ts_fte >> fte_end
