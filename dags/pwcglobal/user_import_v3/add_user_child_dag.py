...
from pwcglobal.user_import_v3.task.update_user_setting import get_update_user_setting
from pwcglobal.user_import_v3.task.put_table_view_setting import get_put_table_view_setting
from pwcglobal.user_import_v3.task.update_supervisor import get_update_supervisor
from pwcglobal.user_import_v3.task.put_line_manager import put_line_manager_udf
from pwcglobal.user_import_v3.task.assign_default_toil_to_policy import add_toil_default_policy

# pylint: disable=too-many-statements
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'pwcglobal_user_import_add_user_child_{config.instance}_v3',
        description=f'PwCGlobal_User_Import_Child_User Add {config.instance} V3',
        ...
        if_user_belongs_to_zt_country = rail.IfOperator(
            ...
            test=lambda:bool(request_payload.get_conf()["zerotimeuserpermissionseturi"]),
        ...
        def get_time_off_req():
            ...
            if request_payload.get_conf()['toiltimeofftypeuri'] and request_payload.get_conf()["toil"] == "Y":
                time_off_types.append(request_payload.get_conf()['toiltimeofftypeuri'])
            return {
        ...
        if_toil_time_off = rail.IfOperator(
            ...
            test=lambda:request_payload.get_conf()['toiltimeofftypeuri'] and request_payload.get_conf()["toil"] == "Y",
            yes_task="start_default_policy",
            no_task="end_default_policy"
        )

        start_default_policy = rail.EmptyOperator(task_id="start_default_policy")
        ...
        get_location_details >> create_user >> \
        if_user_belongs_to_zt_country >> rail.Label("Yes") >> start_user_setting
