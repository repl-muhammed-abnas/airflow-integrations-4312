...
from pwcglobal.user_import_v3.validate_field import validate_field, field_config_add, field_config_update
from pwcglobal.user_import_v3.task.search_user import get_search_user_task
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'pwcglobal_user_import_user_child_{config.instance}_v3',
        description=f'PwCGlobal_User_Import Process User {config.instance} V3',
        ...
        add_user = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'{config.user_add_dag_id}_{config.instance}_v3',
            conf=lambda item: {
                **item, **{
        ...
        update_user = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=lambda: [request_payload.get_conf()],
            trigger_dag_id=f'{config.user_update_dag_id}_{config.instance}_v3',
            execution_timeout=timedelta(days=config.execution_timeout_days),
        ...
        get_supervisor_assignment >> catch_and_log_errors
