...
import rail
from pwcglobal.user_import_v3 import request_payload, custom_method
from pwcglobal.user_import_v3.task.update_country import get_update_country
from pwcglobal.user_import_v3.task.update_timesheet_period import get_update_timesheet_period
from pwcglobal.user_import_v3.task.update_supervisor import get_update_supervisor
from pwcglobal.user_import_v3.task.update_timesheet_template import get_update_timesheet_template
from pwcglobal.user_import_v3.task.update_custom_field import get_update_custom_field
from pwcglobal.user_import_v3.task.update_timeoff_policy import get_update_timeoff_policy
from pwcglobal.user_import_v3.task.update_ftepercent_blob import update_ftepercent_blob
from pwcglobal.user_import_v3.task.assign_default_toil_to_policy import add_toil_default_policy

def create_dag(config):
    # pylint: disable=too-many-statements
    with rail.create_airflow_dag(
        dag_id=f'pwcglobal_user_import_update_user_child_{config.instance}_v3',
        description=f'PwCGlobal_User_Import_Child_User Update {config.instance} V3',
        ...
        time_sheet_period_end = rail.EmptyOperator(task_id="time_sheet_period_end")
        ...
        get_user_location_uri = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response: list(map(lambda i: i["location"]["displayText"], response))[-1] if response else null
        )
        ...
        is_ftepercent_blob_changed = rail.IfOperator(
            ...
            test=lambda: request_payload.get_conf()['ftepercent'] and request_payload.get_conf()["customfielduri"]['ftepercenturi'] and
                    str(request_payload.get_conf()['ftepercent']) !=
                    rail.find_first_by_attr_and_get_attr(
                    rail.result('bulk_get_user3')[
                        'userDetails']['customFieldValues'],
                    'customField.displayText',
        ...
        update_fte_blob_start = rail.EmptyOperator(task_id="update_fte_blob_start")
        ...
        if_new_payrules_for_user = rail.IfOperator(
            ...
            test=lambda:(sorted(list(map(lambda i: {
                    "displayText": i['payRuleScript']['displayText'],
                    "uri": i['payRuleScript']['uri'],
                    "effective_date": datetime(**i["effectiveDate"]) if i["effectiveDate"] and\
                          datetime(**i["effectiveDate"]) <= now(tz="Europe/London").replace(tzinfo=None) else datetime.min
                }, rail.result("bulk_get_user3")["payRuleScriptSchedule"])), key= lambda x:x["effective_date"])[-1]["uri"] !=
                request_payload.get_conf()["payruleuri"]),
        ...
        if_product_license_present_payrule = rail.IfOperator(
            ...
            test=lambda:bool(len(rail.result("bulk_get_user3")["assignedProducts"])>0),
        ...
        if_timesheet_template_assigned_payrule = rail.IfOperator(
            ...
            test=lambda:bool(rail.result("bulk_get_user3")["timesheetTemplate"]),
        ...
        get_current_timesheet_period_payrule = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response: rail.parse_date(response["displayText"].split("/")[1], "%Y-%m-%d") if response else null
        )
        ...
        update_zerotime_start = rail.EmptyOperator(task_id="update_zerotime_start")
        ...
        logs_map = {
            ...
            update_timeofftypes.task_id: "Toil Time off type updated",
            assign_initial_payrule_for_user.task_id: "Payrule updated",
            assign_payrules_for_user.task_id: "Payrule updated",
            put_time_entry_approval_path.task_id: "Time entry approval path updated",
            "update_toil_udf": "TOIL updated",
            "update_ftepercent_udf": "FTEpercent updated",
            "update_line_manager": "Line manager updated",
            put_system_approval_time_entry_path.task_id: "Time entry approval path updated",
        ...
        def do_map_logs():
            ...
            if rail.result("if_line_manager_with_legal_entity_id") == "line_manager_complete":
                request_payload.get_conf()['validationlog'].append(
                    {"message":"Mulitple users with same linemanager party id or legal entity id"})
        ...
        is_businessunit_changed >> rail.Label(
            'Yes') >> update_businessunit >> is_country_present
        ...
        has_time_entry_changed >> rail.Label("Yes") >> put_time_entry_approval_path >> is_ftepercent_blob_changed
        ...
        if_timesheet_template_assigned_payrule >> rail.Label("No") >> update_validation_log_for_ts >> update_zerotime_start
        if_product_license_present_payrule >> rail.Label("No") >> update_validation_log_for_product >>update_zerotime_start
        ...
        unassign_zerotime_license >> has_any_zerotime_permission_to_remove >> rail.Label(
            "Yes") >> unassign_zerotime_permission >> end_zerotime
