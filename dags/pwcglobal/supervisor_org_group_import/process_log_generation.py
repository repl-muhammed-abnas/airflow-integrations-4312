...
import rail
from pwcglobal.supervisor_org_group_import.utils.python_callable import do_format_logs

def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_log_generation,
        description=f'PwC Supervisory Org Custom Import Process Log Generation {config.instance}',
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_logs', 'error_record_count') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Supervisory Org Custom Import " }} \
