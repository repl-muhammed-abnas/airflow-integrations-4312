...
import rail
from pwcglobal.project_import_api_v5.task.get_team_members_project_manager import get_team_members_project_managers
from pwcglobal.project_import_api_v5.task.add_user_permission_policy import get_add_permission_policy
from pwcglobal.project_import_api_v5.task.child_dag_exception_logs import log_exception_field_task
from pwcglobal.project_import_api_v5 import request_payload
from pwcglobal.project_import_api_v5 import custom_method
from pwcglobal.project_import_api_v5 import response_filter
from pwcglobal.project_import_api_v5 import python_callable_method

# config :
# https://github.com/replicon/airflow-integrations/blob/main/dags/pwcglobal/project_import_api_v5/config.py


# pylint:disable = too-many-statements
def create_child_add_project_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.project_import_api_create_project_child_dag_id,
        ...
        get_project_team_managers = get_team_members_project_managers('add')
        ...
        if_project_success = rail.IfOperator(
            task_id="if_project_success",
            trigger_rule="all_done",
            test=lambda:python_callable_method.get_task_state("create_project_with_payload") == "success",
            no_task="if_existing_project",
            yes_task="get_project_uri"
        )

        if_existing_project = rail.IfOperator(
            task_id="if_existing_project",
            test=lambda : python_callable_method.get_task_state("create_project_with_payload") == "failed" and\
                rail.result("create_project_with_payload","error") and rail.result(
                'create_project_with_payload','error')['response']['json']['error']['details']['notifications'][0]['displayText'] == \
                    "Project Code should be unique. A Project already exists with specified project code.",
            yes_task="get_exception_logs",
            no_task="log_project_creation_failure"
        )

        log_project_creation_failure = rail.WriteLogOperator(
            task_id="log_project_creation_failure",
            log='{{ dag_run.conf.log }}',
            ...
            properties=lambda dag_run:{
                ...
                'Task Name|Task Code': 'nil',
                'status': 'Error',
                'details': rail.result("create_project_with_payload","error"),
        ...
        get_project_uri = rail.PythonOperator(
            ...
            python_callable=lambda:rail.result("create_project_with_payload").get("uri")
        )
        ...
        update_engagement_party_udf_value = rail.RepliconServiceOperator(
            ...
            data={
                "objectUri": "{{ result('get_project_uri') }}",
        ...
        get_department_parent_details = rail.RepliconServiceOperator(
            task_id='get_department_parent_details',
            endpoint='/services/DepartmentService1.svc/GetDepartmentDetails',
            data={
                "departmentUri": "{{ result('map_department_list_data').companycodeuri }}"
            },
            data_handler=python_callable_method.get_required_department_parent_data
        )

        if_department_uri_present = rail.IfOperator(
            task_id="if_department_uri_present",
            test="{{ result('get_department_parent_details') | is_truthy }}",
            yes_task="set_assign_team_from_respective_department",
            no_task="should_update_location"
        )

        set_assign_team_from_respective_department = rail.RepliconServiceOperator(
            task_id='set_assign_team_from_respective_department',
                ...
                "teamMemberDataAccessScopes": [
                    {
                        "departmentGroups": [
                            {
                                "uri": "{{ result('get_department_parent_details') }}"
                            }
                        ]
                    }
                ]
        ...
        batch_task >> get_department_list >> is_department_not_present
        ...
        create_project_with_payload >>\
        if_project_success >> rail.Label("No") >>\
        if_existing_project  >> rail.Label("Yes") >> get_exception_logs
        if_existing_project >> rail.Label("No") >> log_project_creation_failure >> catch_and_log_errors
    
        if_project_success >> rail.Label("Yes") >> get_project_uri >> should_update_engagement_party_udf_value
        ...
        should_process_company_code >> rail.Label(
            "Yes") >> update_company_code_for_project >> get_department_parent_details >> if_department_uri_present

        should_process_company_code >> rail.Label(
            "No") >> should_update_location

        if_department_uri_present >> rail.Label(
            "Yes") >> set_assign_team_from_respective_department >> should_update_location
        if_department_uri_present >> rail.Label(
            "No") >> should_update_location
