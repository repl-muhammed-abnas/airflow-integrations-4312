...
import rail
from pwcglobal.project_import_api_v5.custom_method import get_unique_sender_ids_by_companykey
...
def create_log_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.project_import_api_log_schedule_master_dag_id,
        ...
        def get_dagruns_to_process(session=NEW_SESSION):
            ...
            dag_runs_to_filter = (
                ...
                .select_from(DagRun)
                .filter(
                    DagRun.dag_id == config.project_import_api_log_generation_child_dag_id, DagRun.state.in_(
        ...
        process_logs_by_sender = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.project_import_api_log_child_dag_id,
            conf={
                'sender_id': "{{ item }}",
