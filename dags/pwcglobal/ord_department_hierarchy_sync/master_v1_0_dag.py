...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'pwc_ord_department_group_hierarchy_sync_master_v10_{config.instance}',
        description=f'PwC-ORD Department Group Hierarchy Sync Master V1.0 {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=datetime(2022, 1, 1, tz=config.schedule_timezone_Paris),
        ...
        get_current_time_in_tz = rail.PythonOperator(
            ...
            python_callable=lambda: now(
                config.schedule_timezone_Paris).isoformat()
        )
        ...
        pwc_ord_mapper_search_entries_7 = rail.PythonOperator(
            ...
            python_callable=python_callable_method.get_ord_mapper_non_specific_values,
        ...
        get_current_time_in_tz >> get_enabled_department_groups_4 \
            >> pwc_ord_mapper_search_entries_7 \
