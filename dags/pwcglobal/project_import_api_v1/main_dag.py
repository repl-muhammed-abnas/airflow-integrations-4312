...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        ...
        process_projects = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=datetime.timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'pwc_project_import_child_process_project_b1_{config.instance}_v1',
        ...
        process_log_pregeneration = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=datetime.timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f'pwc_project_import_child_log_pregeneration_{config.instance}_v1',
            conf=lambda item: {
