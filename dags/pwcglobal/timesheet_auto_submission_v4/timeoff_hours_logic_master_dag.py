...
import rail
from pwcglobal.timesheet_auto_submission_v4.task.timeoff_hours_specific_scenarios import get_specific_scenarios
    ...
    with rail.create_airflow_dag(
        ...
        query_distinct_wholeweek_timeoff_record = rail.QueryCollectionOperator(
            ...
            query=f'''SELECT DISTINCT
                        timesheeturi,totalhours,useruri,daydiff,Timesheet_Start_Date,Timesheet_End_Date,User_Name,Time_Off_Hrs,Country__Current_
                     FROM query_timeoff_user
                     WHERE LOWER(Country__Current_) IN ('japan')
                     AND (CAST(totalhours as decimal) > CAST(Scheduled_Hrs__In_Period_ as decimal) OR CAST(totalhours as decimal) = CAST(Scheduled_Hrs__In_Period_ as decimal))
                     AND (CAST(Time_Off_Hrs as decimal) > 0 AND CAST(totalhours as decimal) = CAST(Time_Off_Hrs as decimal))
                     AND CAST(daydiff as decimal) < 1
                    '''
        )
