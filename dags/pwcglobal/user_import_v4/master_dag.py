...
from pwcglobal.user_import_v4.utils import custom_method, request_payload
        ...
        process_users = rail.PythonOperator(
            ...
            python_callable=lambda: list(itertools.chain(
                *list(map(lambda x: rail.result(
                    f'trigger_parallel_dagrun_{x+1}'), range(config.process_each_user_trigger_parallel_count)))))
        )
