...
from pwcglobal.absense_data_pre_population_v3.utils import response_filter
from pwcglobal.absense_data_pre_population_v3.utils import request_payload
from pwcglobal.absense_data_pre_population_v3.utils import python_callable_method
from pwcglobal.absense_data_pre_population_v3.utils import custom_method
...
def create_main_absense_data_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        description=f'PWC Absense Data Pre-population Master V3.0 {config.instance}',
        ...
        def get_process_timeentry_trigger_id(item):
            ...
            if modulo == 0:
                return f'{config.child_dag_id}_{config.version}'
            return f'{config.child_dag_id}_batch_{str(modulo)}_{config.version}'
        ...
        upload_xml_to_sftp = rail.SFTPUploadFileOperator(
            ...
            '/{{get_company_key() | lower}}_time_prepopulation_' + config.api_unique_code + '_{{current_time("%Y%m%d%H%M%S")|replace(":","")|replace("-","")|replace(" ","")}}_{{dag_run_ecid()|replace(":","-")}}_logs.xml',
        ...
        generate_download_link = rail.GeneratePresignedDownloadUrlOperator(
            ...
            # pylint: disable=line-too-long
            output_file_name='{{get_company_key() | lower}}_time_prepopulation_' + config.api_unique_code + '_{{current_time("%Y%m%d%H%M%S")|replace(":","")|replace("-","")|replace(" ","")}}_{{dag_run_ecid()|replace(":","-")}}_logs.xml',
        ...
        log_to_sumo = rail.DagRunLogToSumoOperator(
            ...
            extra_info=lambda dag_run: {
                'NoOfTransactions': len(dag_run.conf['webhook']['data']['PersonResourceActualTime'])
                if dag_run.conf['webhook'].get('data') and (dag_run.conf['webhook']['data']).get('PersonResourceActualTime') else 0
            }
        )
