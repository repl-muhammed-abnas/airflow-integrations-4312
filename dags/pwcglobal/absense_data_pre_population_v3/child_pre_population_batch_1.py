...
from pwcglobal.absense_data_pre_population_v3.task.hours_quantity_check import get_hours_quantity_check
from pwcglobal.absense_data_pre_population_v3.task.build_oef_values import get_build_oef_values
from pwcglobal.absense_data_pre_population_v3.task.put_timeentry import get_put_time_entry
from pwcglobal.absense_data_pre_population_v3.task.update_or_delete_timeentry import get_update_or_delete_time_entry
...
def create_child_pre_population_project_dag(config):
    ...
    get_postfix = "_batch_1"

    with rail.create_airflow_dag(
        dag_id=f'{config.child_dag_id}{get_postfix}_{config.version}',
        description='PwC_Time Prepopulatation_child_v3.0',
        ...
        is_timesheet_present = rail.IfOperator(
            ...
            test= lambda: bool(rail.result('get_timesheet_info')),
            yes_task='is_timesheet_status_open',
            no_task='is_transactiondate_present'
        )

        is_timesheet_status_open = rail.IfOperator(
            task_id='is_timesheet_status_open',
            test=lambda: bool(rail.result('get_timesheet_info')[0]['timesheet_status_2_uri'] == 'urn:replicon:timesheet-status-2:open' 
        ...
        is_useruri_present >> rail.Label(
            'Yes') >> get_timesheet_info >> is_timesheet_present
        
        is_timesheet_present >> rail.Label(
            'Yes') >> is_timesheet_status_open
        is_timesheet_present >> rail.Label(
            'No') >> is_transactiondate_present
        is_useruri_present >> rail.Label(
