...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        ...
        def get_unhandled_error_message():
            context = rail.get_current_context()
            failed_task_ids = rail.lib.errors.get_failed_task_ids(context)

            if 'create_client_in_replicon' in failed_task_ids:
                client_error_executed = python_callable_method.get_task_state('client_error') == 'success'

                if client_error_executed:
                    pass
                else:
                    failed_task_ids = [tid for tid in failed_task_ids if tid != 'create_client_in_replicon']
        ...
        process_projects = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=datetime.timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=config.project_import_api_process_project_child_dag_id,
        ...
        process_log_pregeneration = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=datetime.timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=config.project_import_api_log_generation_child_dag_id,
            conf=lambda item: {
        ...
        can_fail_dag = rail.IfOperator(
            ...
            test=lambda: bool(get_unhandled_error_message()),
            yes_task="fail_dagrun",
        )

        fail_dagrun = rail.FailOperator(
            task_id="fail_dagrun",
            message=lambda: get_unhandled_error_message()
        )

        batch_task >> import_end
        
        batch_task >> was_triggered_by_pwc >> get_all_permission_sets >> get_all_legal_entities >> get_all_locations >> \
