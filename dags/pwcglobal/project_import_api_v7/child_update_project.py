...
def create_child_update_project_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.project_import_api_update_project_child_dag_id,
        ...
        batch_task = rail.BatchTaskRunOperator(
            ...
            start_task='bulk_get_project_team_members',
        ...
        should_update_project_date_range = rail.IfOperator(
            ...
            yes_task="check_for_valid_end_date",
            no_task="should_process_confidential_flag"
        )

        check_for_valid_end_date = rail.IfOperator(
            task_id="check_for_valid_end_date",
            test=lambda: (
                lambda dr: date(
                    dr['endDate']['year'],
                    dr['endDate']['month'],
            ...
            )(rail.result('get_project_date_range_logs')['date_range']),
        ...
        log_end_date_exception = rail.WriteLogOperator(
            ...
            severity='Exception',
            message="Project not updated because End Date is earlier than Start Date.",
            properties=lambda dag_run:{
                ...
                'status': 'Exception',
                'details': f"Invalid project date range. Start Date: {rail.result('get_project_date_range_logs')['date_range']['startDate']} | End Date: {rail.result('get_project_date_range_logs')['date_range']['endDate']}. End Date must not be earlier than Start Date.",
        ...
        should_process_client_update = rail.IfOperator(
            ...
            test=lambda dag_run: dag_run.conf['client_uri'] and
            (not rail.find_first_by_attr_and_get_attr(dag_run.conf['load_project']['clients'],
                                                      'client.uri', dag_run.conf['client_uri'], 'client.uri')),
        ...
        update_tasks_in_replicon = rail.RepliconServiceCallForEachItemOperator(
            ...
            items=lambda: request_payload.get_data_from_document(
        ...
        update_allow_timeentry_against_task_only_add = rail.RepliconServiceOperator(
            task_id="update_allow_timeentry_against_task_only_add",
            endpoint="/services/ProjectService1.svc/UpdateAllowTimeEntryAgainstTasksOnly",
            data={
                "projectUri": "{{ dag_run.conf.project_uri }}",
                "allowTimeEntryAgainstTasksOnly": "1"
            }
        )
        ...
        batch_task >> bulk_get_project_team_members >> \
        ...
        should_update_project_date_range >> rail.Label(
            "Yes") >> check_for_valid_end_date
        
        check_for_valid_end_date >> rail.Label(
            "Yes") >> update_project_date_range >> should_process_confidential_flag
        
        check_for_valid_end_date >> rail.Label(
            "No") >> log_end_date_exception >> should_process_confidential_flag
        ...
        has_tasks_to_add >> rail.Label(
            "Yes") >> add_tasks_in_replicon >> update_allow_timeentry_against_task_only_add >> log_added_tasks_in_replicon >> \
        ...
        should_update_project_team_members >> rail.Label(
            ...
            update_project_team_member_billing_rate >> put_key_value_to_project >> get_success_logs

        should_update_project_team_members >> rail.Label(
            "No") >> put_key_value_to_project >> get_success_logs
