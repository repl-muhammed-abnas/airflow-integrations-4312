...
import rail
from pwcglobal.project_import_api_v4 import request_payload, custom_method
...
def get_add_permission_policy(caller, action_type='create'):
    with rail.TaskGroup(group_id=f'add_permission_policy_{caller}', prefix_group_id=False) as add_permission_policy:
        ...
        if action_type == 'update' and caller == 'project_manager':
            ...
            update_project_manager_approval_required = rail.RepliconServiceOperator(
                task_id="update_project_manager_approval_required",
        ...
        if caller == 'project_comanager':
            ...
            assign_comanager_to_project = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run: {
                    "projectUri": rail.result('get_project_uri') if action_type == 'create' else dag_run.conf['project_uri'],
        ...
        if action_type == 'update' and caller == 'project_manager':
            impersonate_and_create_interactive_session >> update_project_manager >> update_project_manager_approval_required >> \
