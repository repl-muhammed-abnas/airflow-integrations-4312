...
import rail
from pwcglobal.project_import_ury_and_arg.utils.python_callable import do_format_logs

def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_log_generation,
        description='PwC Project Import - Process Log Generation',
        ...
        render_logs_csv = rail.WriteCSVFileOperator(
            ...
            row=[
                ...
                "{{ item.clientcode }}",
                "{{ item.taskcode }}",
                "{{ '|'.join(item.taskname) if item.taskname | is_truthy else '' }}",
                "{{ item.action }}",
                "{{ item.status }}",
                "{{ item.details }}",
                "{{ item.ecid }}",
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_logs', 'error_record_count') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | LAN AC Project Import " }} \
