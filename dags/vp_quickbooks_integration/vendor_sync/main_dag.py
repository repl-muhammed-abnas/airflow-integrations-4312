"""
Main DAG for VP QBO Vendor Sync.
"""
...
def create_dag(config):
    """
    Create main DAG for VP QBO Vendor Sync.

    Args:
        config: Configuration object with instance settings
    """
    with rail.create_airflow_dag(
        dag_id=f'vp_qbo_vendor_sync_main_{config.instance}',
        description='Processor DAG for VP QBO Vendor Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=Variable.get(
            f'vp_qbo_vendor_sync_schedule_interval_{config.instance}',
            '*/5 * * * *'  # By default, it runs every 5 minutes
        ),
        max_active_runs=config.max_active_runs,
        tags=['vantagepoint_quickbooks', 'vendor_sync', 'main'],
        default_args={
        ...
        process_customers = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=(
                f'vp_qbo_vendor_sync_dispatcher_{config.instance}'
            ),
            conf=lambda item: {
                **item,
