"""
Dispatcher DAG for QBO -> VP Invoice Payment Sync.

Per-tenant: polls QBO for new/updated invoice payments by MetaData.LastUpdatedTime,
filters out zero-amount entries, then fans out one worker DAG per payment.
Mirrors `vp_quickbooks_integration/customer_sync/dispatcher_dag.py`.

Replaces Workato recipe 014_503_psa_poll_quickbooks_invoice_payment.
"""
...
from vp_quickbooks_integration.invoice_payment_sync.utils.python_callable_method import (
    ...
    extract_and_filter_payments_method,
)
...
def create_dag(config):
    """Per-tenant dispatcher: poll QBO payments, filter zero-amount, fan out."""
    with rail.create_airflow_dag(
        dag_id=f'vp_qbo_invoice_payment_sync_dispatcher_{config.instance}',
        description=(
            'Poll QBO for new invoice payments and trigger per-payment worker DAG'
        ),
        ...
        tags=['vantagepoint_quickbooks', 'invoice_payment_sync', 'dispatcher'],
        default_args={
        ...
        # Mirrors Workato CFG_DisableInvoicePaymentIntegration account property.
        check_disabled_flag = rail.IfOperator(
            task_id='check_disabled_flag',
            test=lambda: is_integration_enabled_method(config.instance),
            yes_task='get_recently_changed_payments',
            no_task='skip_run'
        )

        skip_run = rail.EmptyOperator(task_id='skip_run')

        # Single QBO poll by MetaData.LastUpdatedTime captures new payments.
        get_recently_changed_payments = rail.QuickBooksPaymentOperator(
            task_id='get_recently_changed_payments',
            intuit_conn_id="{{ dag_run.conf.connections.intuit }}",
            operation='search_payment',
            query=(
        ...
        # Extract list and apply zero-amount filter (TotalAmt != 0).
        # Mirrors Workato recipe step 1 `if TotalAmt != 0`.
        extract_payments = rail.PythonOperator(
            task_id='extract_payment_list',
            python_callable=extract_and_filter_payments_method
        )

        check_if_records_exist = rail.IfOperator(
            task_id='check_if_records_exist',
            test=lambda: len(rail.result('extract_payment_list')) > 0,
        ...
        log_no_records = rail.PythonOperator(
            ...
            python_callable=lambda: logger.info(
                'No new non-zero invoice payments in this poll window.'
            )
        )

        # Fan out to worker DAG — one run per payment.
        # Conf forwards PaymentID, InvoiceID, connections, and customerId
        # so the worker is fully self-contained.
        process_records = rail.TriggerDagRunForEachItemOperator(
            task_id='process_records',
            items=lambda: rail.result('extract_payment_list'),
            trigger_dag_id=(
                f'vp_qbo_invoice_payment_sync_create_{config.instance}'
            ),
            conf=lambda item: {
                **item,
                'connections': (
        ...
        gather_create_dag_errors = rail.GatherResultsFromDagRunsOperator(
            ...
            dag_runs="{{ result('process_records') }}",
            dagrun_task_id='catch_payment_dag_error',
            flatten=True
        )
        ...
        (
            ...
            get_recently_changed_payments >> extract_payments >>
        ...
        (
            ...
            fail_payment_sync >> update_sync_time
        )
