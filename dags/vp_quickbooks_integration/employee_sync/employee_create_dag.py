"""
Employee Create DAG for VP QBO Employee Sync.
Creates a new employee in Vantagepoint from a QBO Employee record.

Mirrors the Workato create branch (`014_503_psa_vantagepoint_upsert_employee`):
  GET  /codeTable/CFGEmployeeTitle             (look up Title code)
  POST /codeTable/CFGEmployeeTitle             (create Title if missing)
  POST /employee                               (create employee)
  Insert row into employee map (lookup table)
"""
...
from vp_quickbooks_integration.employee_sync.utils.python_callable_method import (
    ...
    build_create_employee_body,
    add_employee_to_employee_map,
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'vp_qbo_employee_sync_create_{config.instance}',
        description='Create employee in Vantagepoint from QuickBooks',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=None,
        max_active_runs=config.max_active_runs,
        tags=['vantagepoint_quickbooks', 'employee_sync', 'create_employee'],
        default_args={
        ...
        create_employee_in_vp = rail.VantagepointEmployeeOperator(
            ...
            request_method='POST',
            request_body=lambda: build_create_employee_body(config.instance),
            trigger_rule='none_failed'
        )

        update_employee_map_lookup = rail.PythonOperator(
            task_id='update_employee_map_lookup',
            python_callable=add_employee_to_employee_map
        )
