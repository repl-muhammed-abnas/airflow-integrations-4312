"""
Main (scheduler) DAG for QBO -> VP Bill Payment Sync.
...
Mirrors `vp_quickbooks_integration/invoice_payment_sync/main_dag.py`.
"""
...
def _schedule_for(instance):
    env_key = (
        f"AIRFLOW_VAR_VP_QBO_BILL_PAYMENT_SYNC_SCHEDULE_"
        f"{instance.upper()}"
    )
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'vp_qbo_bill_payment_sync_main_{config.instance}',
        description='Scheduler DAG for QBO -> VP Bill Payment Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=_schedule_for(config.instance),
        max_active_runs=config.max_active_runs,
        tags=['vantagepoint_quickbooks', 'bill_payment_sync', 'main'],
        default_args={
        ...
        fetch_customers_by_integration = rail.SimpleHttpOperator(
            ...
            data={
                'dag_id': (
                    f'vp_qbo_bill_payment_sync_dispatcher_{config.instance}'
                ),
                'integration_type': 'bill_payment_sync',
                'status': 'enabled'
            },
