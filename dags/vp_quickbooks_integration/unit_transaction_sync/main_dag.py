"""
Main (scheduler) DAG for VP -> QBO Unit Transaction Sync.
...
vendor_sync/main_dag.py and timesheets_sync/main_dag.py.
"""
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'vp_qbo_unit_transaction_sync_main_{config.instance}',
        description='Scheduler DAG for VP -> QBO Unit Transaction Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=Variable.get(
            f'vp_qbo_unit_transaction_sync_schedule_interval_'
            f'{config.instance}',
            default_schedule_interval,
        ),
        max_active_runs=config.max_active_runs,
        tags=[
            'vantagepoint_quickbooks', 'unit_transaction_sync', 'main'
        ],
        default_args={
        ...
        fetch_customers_by_integration = rail.SimpleHttpOperator(
            ...
            data={
                'dag_id': (
                    f'vp_qbo_unit_transaction_sync_main_{config.instance}'
                ),
                'integration_type': 'unit_transaction_sync',
                'status': 'enabled'
            },
