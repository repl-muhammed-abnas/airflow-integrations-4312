"""
Main DAG for VP -> QBO Journal Entry Sync.

Scheduled cron entry point. Fetches enabled tenants from the middleware
and triggers the dispatcher DAG per tenant. Mirrors the wiring of
`vp_qbo_employee_sync_upsert_main_dag.py` but targets the VP-source /
QBO-sink direction for posted journal entries (PSALedger TransType=JE).
"""
...
def create_dag(config):
    """Create main scheduler DAG for VP -> QBO Journal Entry Sync."""
    with rail.create_airflow_dag(
        dag_id=f'vp_qbo_journal_entry_sync_main_{config.instance}',
        description='Scheduler DAG for VP -> QBO Journal Entry Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=Variable.get(
            f'vp_qbo_journal_entry_sync_schedule_interval_{config.instance}',
        ...
        fetch_customers_by_integration = rail.SimpleHttpOperator(
            ...
            data={
                'dag_id': (
                    f'vp_qbo_journal_entry_sync_dispatcher_{config.instance}'
                ),
                'integration_type': 'journal_entry_sync',
                'status': 'enabled'
            },
