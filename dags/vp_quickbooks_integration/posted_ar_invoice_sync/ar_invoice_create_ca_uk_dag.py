"""
CA/UK Worker DAG for VP PSA -> QBO Posted AR Invoice Sync.

Per-batch: fetches all PSA Ledger 'IN' rows for the given Batch, resolves
the QBO NoTaxCodeID for the tenant, resolves project clients and firm
mappings, searches or creates the QBO 'Sales' product item, then iterates
every invoice in the batch via ForEachOperator:
  - Checks Airflow Variable outstanding_sales_invoices for duplicates
  - Builds the QBO invoice/credit-memo body (CA/UK tax logic)
...
CA/UK tax logic:
  IsTaxGroup=true  → TaxGroupCodes field used as TaxCodeRef.
  IsTaxGroup=false → NoTaxCodeID resolved from QBO (fallback 'NON').
...
Triggered by router_dag with:
  ...
  dag_run.conf.config      — integration config dict (CFG_NoTaxCode, etc.)

Replaces Workato recipes:
  014-503 PSA Vantagepoint AR Invoice Exports to QuickBooks CA UK
  014-503 PSA Post Invoice to QuickBooks CA UK
"""
...
from vp_quickbooks_integration.posted_ar_invoice_sync.utils.python_callable_method import (  # noqa: E501
    ...
    build_invoice_body_ca_uk_method,
...
def create_dag(config):
    """Per-batch CA/UK worker: fetch NoTaxCodeID + resolve mappings + post QBO invoices."""
    with rail.create_airflow_dag(
        dag_id=f'vp_qbo_ar_invoice_sync_create_ca_uk_{config.instance}',
        description=(
            'Post VP PSA AR invoice batch to QBO using CA/UK IsTaxGroup tax '
            'logic and record revenue recognition JournalEntry.'
        ),
        ...
        # Fetch the QBO TaxCode whose Id is used as NoTaxCodeID for non-taxable
        # lines (IsTaxGroup=false). Name comes from CFG_NoTaxCode in the
        # integration config passed via dag_run.conf.
        fetch_no_tax_code_id = rail.QuickBooksTaxCodeOperator(
            task_id='fetch_no_tax_code_id',
            intuit_conn_id="{{ dag_run.conf.connections.intuit }}",
            query=(
                "select * from TaxCode WHERE Name = '"
                "{{ dag_run.conf.config.CFG_NoTaxCode | default('') }}'"
            ),
            retries=2,
            retry_delay=timedelta(seconds=10)
        )

        resolve_no_tax_code_id = rail.PythonOperator(
            task_id='resolve_no_tax_code_id',
            python_callable=resolve_no_tax_code_id_method
        )
        ...
        build_invoice_body = rail.PythonOperator(
            ...
            python_callable=build_invoice_body_ca_uk_method
        )
        ...
        # Phase 1: parallel data fetches; NoTaxCodeID resolved before Sales item
        fetch_invoice_batch >> fetch_project_clients
        fetch_no_tax_code_id >> resolve_no_tax_code_id

        # Phase 2: Sales item resolution (waits for project clients + NoTaxCodeID + firm mapping)
        (
            [fetch_project_clients, resolve_no_tax_code_id, fetch_firm_mapping] >>
            search_sales_item
        )
        ...
        fetch_no_tax_code_id >> catch_create_dag_error
        resolve_no_tax_code_id >> catch_create_dag_error
