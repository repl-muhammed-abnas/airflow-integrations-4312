"""
Main DAG for VP -> QBO Employee Sync.
...
`vp_qbo_employee_sync_main_dag.py` but targets the VP-source / QBO-sink
direction.
"""
...
def create_dag(config):
    """Create main scheduler DAG for VP -> QBO Employee Sync."""
    with rail.create_airflow_dag(
        dag_id=f'vp_qbo_employee_sync_upsert_main_{config.instance}',
        description='Scheduler DAG for VP -> QBO Employee Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=Variable.get(
            f'vp_qbo_employee_sync_upsert_schedule_interval_{config.instance}',
        ...
        fetch_customers_by_integration = rail.SimpleHttpOperator(
            ...
            data={
                'dag_id': (
                    f'vp_qbo_employee_sync_upsert_dispatcher_{config.instance}'
                ),
                'integration_type': 'employee_sync_upsert',
                'status': 'enabled'
            },
