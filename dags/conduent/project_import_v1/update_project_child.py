...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        is_project_manager_changed = rail.IfOperator(
            task_id='is_project_manager_changed',
            test=lambda dag_run: bool(request_payload.get_updated_project_leader(dag_run)),
            yes_task='get_project_manager_blob_data',
            no_task='if_project_type_overhead'
        )

        get_project_manager_blob_data = rail.RepliconServiceOperator(
            task_id='get_project_manager_blob_data',
            endpoint='/services/GenericKeyValueStoreService1.svc/GetKeyValue',
            data=lambda:{
                "keyNamespace": 'project_manager_change_effective_date',
                "key": rail.result('create_projectorapply_modifications')['uri']
            }
        )

        update_project_manager_data_to_blob = rail.RepliconServiceOperator(
            task_id='update_project_manager_data_to_blob',
            endpoint='/services/GenericKeyValueStoreService1.svc/PutKeyValue',
            data=lambda: request_payload.update_project_data_to_blob_param(config.project_manager_blob_key_name)
        )
        ...
        if_project_type_changed >> rail.Label('No') >>\
            create_projectorapply_modifications >> is_project_manager_changed        
        is_project_manager_changed >> rail.Label(
            "Yes") >> get_project_manager_blob_data >> update_project_manager_data_to_blob >> if_project_type_overhead
        is_project_manager_changed >> rail.Label(
            "No") >> if_project_type_overhead
