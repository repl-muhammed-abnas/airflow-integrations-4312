...
# config :
# https://github.com/replicon/airflow-integrations/blob/main/dags/oxfordfinancial/client_import_household/config.py


# pylint: disable=too-many-statements
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'oxfordfinancial_client_import_household_master_dag_{config.instance}',
        description=f'New Client import file in SFTP_Household {config.instance}',
        ...
        is_household_file = rail.IfOperator(
            task_id='is_household_file',
            test="{{ result('new_file_sensor') | file_base | lower | matches('household') }}",
        ...
        was_new_file_found = rail.IfOperator(
            ...
            test="{{ get_task_state('new_file_sensor') == 'success' and result('is_household_file') == 'download_file' }}",
        ...
        archive_file = rail.SFTPMoveFileOperator(
            ...
            "/{{ dag_run_ecid() | replace(':', '-') }}_{{ result('new_file_sensor') | file_base }}_{{ result('get_time_for_file') }}.csv"
        )
        ...
        parse_csv = rail.LoadCSVFileOperator(
            ...
            headers=['SF_18_Digit_ID', 'Household_Name', 'Record_Type', 'Subtype', 'Client_ID', 'Market',
                     'Advisory_Agreement_Date', 'SF_15_Digit_ID', 'IsDeleted']
        )

        def get_row(item):
            def get_md5(item):
                column_vals = [str(v) if v else '' for k, v in item.items() if k in (
                    'SF_18_Digit_ID', 'Household_Name', 'Record_Type',
                    'Subtype', 'Client_ID', 'Market',
                    'Advisory_Agreement_Date', 'SF_15_Digit_ID', 'IsDeleted')]
        ...
        get_required_client_custom_fields = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response: {
                ...
                'subtype_uri': rail.find_first_by_attr_and_get_attr(response, 'displayText', 'Subtype', 'uri', ''),
                'record_type_uri': rail.find_first_by_attr_and_get_attr(response, 'displayText', 'Record Type', 'uri', ''),
                'market_uri': rail.find_first_by_attr_and_get_attr(response, 'displayText', 'Market', 'uri', ''),
                'advisory_agreement_date_uri': rail.find_first_by_attr_and_get_attr(response, 'displayText', 'Advisory Agreement Date', 'uri', '')
            }
        )
        ...
        trigger_client_import_create_update_household_client = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_client_import_create_update_household_client',
            retries=0,
            items="{{ result('query_changed_records') }}",
            trigger_dag_id=f'oxfordfinancial_client_import_household_child_create_update_client_{config.instance}',
            ...
            conf={
                ...
                "Market": "{{ item.Market }}",
                "Advisory_Agreement_Date": "{{ item.Advisory_Agreement_Date }}",
                ...
                "Subtype_Custom_Field": "{{ result('get_required_client_custom_fields').subtype_uri }}",
                "Record_Type_Custom_Field": "{{ result('get_required_client_custom_fields').record_type_uri }}",
                "Market_Custom_Field": "{{ result('get_required_client_custom_fields').market_uri }}",
                "Advisor_Agreement_Date_Custom_Field": "{{ result('get_required_client_custom_fields').advisory_agreement_date_uri }}",
                "Service_Name_Custom_Field": "{{ result('get_required_project_custom_fields').service_name_uri }}"
            }
        )

        wait_for_client_import_create_update_household_client = rail.WaitForDagRunsSensor(
            task_id='wait_for_client_import_create_update_household_client',
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            dag_runs='{{ result("trigger_client_import_create_update_household_client") }}'
        )
        ...
        process_log_generation = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'oxfordfinancial_client_import_household_child_log_{config.instance}',
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            conf=lambda: {
        ...
        new_file_sensor >> is_household_file

        is_household_file >> rail.Label(
        ...
        is_changed_records >> rail.Label(
            ...
            trigger_client_import_create_update_household_client >> \
            wait_for_client_import_create_update_household_client >> gather_logs >> \
