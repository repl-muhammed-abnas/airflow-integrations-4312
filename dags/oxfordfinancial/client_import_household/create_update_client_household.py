...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'oxfordfinancial_client_import_household_child_create_update_client_{config.instance}',
        description=f'Create/Update Household Client in Replicon {config.instance}',
        ...
        def get_clientdetails(response, dag_run):
            client_name = dag_run.conf['Household_Name']
        ...
        search_clients = rail.RepliconServicePageOperator(
            ...
            data=lambda dag_run: {
                ...
                'filterExpression': {
                    ...
                    'rightExpression': {
                        'value': {
                            'text': dag_run.conf['Household_Name'],
                        }
                    }
                }
            },
        ...
        if_client_text_value_present = rail.IfOperator(
            ...
            no_task='is_record_type_not_contains_prospect'
        )
        ...
        def get_custom_field_values(dag_run):
            custom_field_values = [
                ...
                {
                    "customField": {
                        "uri": dag_run.conf['Subtype_Custom_Field']
                    },
                    "text": dag_run.conf['Subtype']
                },
                {
                    "customField": {
                        "uri": dag_run.conf['Record_Type_Custom_Field']
                    },
                    "text": dag_run.conf['Record_Type']
                },
                {
                    "customField": {
                        "uri": dag_run.conf['Market_Custom_Field']
                    },
                    "text": dag_run.conf['Market']
                }
            ]
            if dag_run.conf['Advisory_Agreement_Date']:
                advisory_agreement_date = datetime.strptime(
                    dag_run.conf['Advisory_Agreement_Date'], '%m/%d/%Y')
                custom_field_values.append({
                    "customField": {
                        "uri": dag_run.conf['Advisor_Agreement_Date_Custom_Field']
                    },
                    "text": f'{advisory_agreement_date.year}-{advisory_agreement_date.month}-{advisory_agreement_date.day}'
                })
        ...
        def get_projectdetails(response, dag_run):
            project_name = f"Business Development ({dag_run.conf['Household_Name']})"
            flatten_rows = list(itertools.chain(
                *list(map(lambda x: x['rows'], response))))
            filtered_project = [
        ...
        search_project = rail.RepliconServicePageOperator(
            ...
            data=lambda dag_run: {
                ...
                'filterExpression': {
                    ...
                    'rightExpression': {
                        'value': {
                            'text': f"Business Development ({dag_run.conf['Household_Name']})",
                        }
                    }
                }
            },
        ...
        create_project = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "target": {
                    "name": f"Business Development ({dag_run.conf['Household_Name']})"
                },
                "projectInfo": {
                    "name": f"Business Development ({dag_run.conf['Household_Name']})",
                    "projectStatusLabel": {
        ...
        is_record_type_not_contains_prospect = rail.IfOperator(
            task_id='is_record_type_not_contains_prospect',
            test=lambda dag_run: 'Prospect' not in dag_run.conf['Record_Type'],
            yes_task='create_client',
            no_task='create_client_2'
        )
        ...
        create_client = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "client": {
                    "target": {
                        "name": dag_run.conf['Household_Name']
                    },
                    "name": dag_run.conf['Household_Name'],
                    "isActive": True,
                    "customFieldValues": get_custom_field_values(dag_run)
        ...
        write_create_client_success = rail.WriteLogOperator(
            task_id='write_create_client_success',
            log="{{ result('create_log') }}",
            message='Success',
            severity='Created Non-Prospect',
            properties={
                'sf18digitid': '{{ dag_run.conf.SF_18_Digit_ID }}',
                'status': 'Created Non-Prospect',
                'reason': 'Success'
            }
        )

        create_client_2 = rail.RepliconServiceOperator(
            task_id='create_client_2',
            endpoint='/services/ClientService1.svc/PutClient',
            data=lambda dag_run: {
                "client": {
                    "target": {
        ...
        update_project_client_2 = rail.RepliconServiceOperator(
            ...
            data={
                ...
                "clientUri": "{{ result('create_client_2').uri }}",
                "optionUri": "urn:replicon:project-apply-new-client-option:keep-existing-billing-rates-and-expense-codes"
            }
        )
        ...
        write_update_client_success_2 = rail.WriteLogOperator(
            ...
            log="{{ result('create_log') }}",
            message='Success',
            severity='Created Prospect',
            properties={
                'sf18digitid': '{{ dag_run.conf.SF_18_Digit_ID }}',
                'status': 'Created Prospect',
                'reason': 'Success'
            }
        )
        ...
        if_client_text_value_present >> rail.Label(
            'No') >> is_record_type_not_contains_prospect
        is_record_type_not_contains_prospect >> rail.Label(
            'Yes') >> create_client >> write_create_client_success >> catch_and_log_errors
        is_record_type_not_contains_prospect >> rail.Label(
            'No') >> create_client_2 >> search_project_2 >> is_project_slug_blank_2
