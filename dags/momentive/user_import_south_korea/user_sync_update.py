...
from momentive.user_import_south_korea.utils.python_callable import get_current_data

def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'momentive_userimport_user_sync_update_child_{config.instance}',
        description=f'momentive_userimport_user_sync_update_child_{config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.user_sync_update_child_dag_active_runs,
        default_args={
        ...
        log_user_import_not_created = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "action": "Update",
                "status": "Exception",
                'details': "User not updated," + rail.result('get_exception_log')['exc_value'],
        ...
        check_dob_mismatch = rail.IfOperator(
            ...
            test=lambda dag_run: bool(datetime.strptime(dag_run.conf['date_of_birth'], '%Y-%m-%d') != \
        ...
        validate_hiredate_to_today = rail.IfOperator(
            ...
            test=lambda: bool(python_callable.validate_hiredate),
        ...
        validate_employeetype = rail.IfOperator(
            ...
            test="{{ result('get_effectiveusergroupmembership').employeeTypes[0].employeeType.employeeType | is_truthy and \
        ...
        trigger_update_user_timeoff = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=f'momentive_userimport_update_user_timeoff_assign_child_{config.instance}',
