...
import rail
from momentive.user_import_belgium.utils import request_payload, python_callable
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.momentive_othercountries_user_sync_bel_policy_rehire_child_dag_id,
        description=f'momentive_othercountries_user_sync_bel_policy_rehire_child_{config.instance}',
        ...
        # Recipe step 4: the default policy-set schedule for the time-off type.
        ...
        # Recipe nodes 14-19: re-anchor each default tier (offset 0/2/3) onto the hire
        # year (Jan 1 of hire year / +1 / +2), append to the preserved past entries,
        # rename script->scriptTarget.
        build_rehire_policy_entries = rail.PythonOperator(
            task_id='build_rehire_policy_entries',
            python_callable=lambda dag_run: python_callable.build_bel_rehire_policy_entries(
                rail.result('get_past_rehire_entries'),
                rail.result('get_default_policyset_schedule'),
                dag_run.conf['startdate'])
        )

        # Recipe node 20: only write when at least one entry was built.
        ...
        # Recipe nodes 22-23: PutUserTimeOffAccountPolicySetSchedule (past + re-anchored tiers).
        ...
        # Recipe node 20 (empty list): nothing to assign.
        ...
        catch_error = rail.PythonOperator(
            ...
            python_callable=lambda: rail.render_template(
                "BEL time off policy update rehire - Dag_Run Error - {{ get_error_message() }}")
        )
