...
def user_import_data(item):
    ...
    same shared other-countries report as Japan/UAE; the Japan-flag column exists in the
    report but is unused for Belgium)."""
    return [
...
def process_each_user_payload(item):
    ...
    pre-fetched lists — Belgium valid records are already filtered to BENELUX BV, so the
    blank-legalentity default only guards against malformed rows. The supervisor log artifact
    is the master's, so deferred supervisor entries land in one place for the master's fan-out;
    the per-user log is created by the run itself and gathered afterwards."""
    ...
    from momentive.user_import_belgium import config as _cfg
    legalentity_name = item['legalentity'] if item['legalentity'] else _cfg.eligible_legal_entity
    legalentityuri = ''
...
def _base_child_payload(dag_run):
    ...
    run's conf. 'logger' is THIS run's log artifact (gathered by the master after all per-user
    runs finish); 'supervisor_logger' is the master's shared artifact."""
    item = dag_run.conf
    return {
        'userid': item['userid'],
...
def process_supervisor_mapper_data(item):
    ...
    entry ({'properties': {...}}). 'logger' is the per-user run's log artifact (held in the
    supervisor entry) so the child can find and rewrite the correct Add/Update entry."""
    return {
...
# Routing-branch -> action label for catch_and_log_errors in process_each_user.
...
def get_mandatory_fields_exception_message(item):
    """Detail string for the validation-skip entries: 'User ID must be present'."""
    missing_fields = []
...
def get_invalid_record(item):
    """Validation-skip row; stamps country 'Belgium' on these entries."""
...
# ---------------------------------------------------------------------------
# BEL Time off policy update_rehire child payloads (recipe 1362490). This child is
# triggered by the common update_user_timeoff_assign child with conf:
# {useruri, startdate, timeoffuri, username}.
# ---------------------------------------------------------------------------


def get_user_timeoff_policy_summary_payload(dag_run):
    """Recipe step 3: the user's per-type time-off policy summary."""
    return {"userUri": dag_run.conf['useruri']}


def get_default_policyset_schedule_for_type_payload(dag_run):
    """Recipe step 4: GetDefaultTimeOffPolicySetScheduleForTimeOffType for the type being
    re-anchored (returns the seniority-tier default schedule)."""
    return {"timeOffTypeUri": dag_run.conf['timeoffuri']}


def put_rehire_policy_schedule_payload(dag_run):
    """Recipe steps 22-23: PutUserTimeOffAccountPolicySetSchedule with the preserved past
    entries plus the re-anchored tiers."""
    return {
        "timeOffAccount": {
