...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.momentive_belgium_user_sync_master_dag_id,
        description=f'Momentive user import Belgium - Master {config.instance}',
        ...
        get_master_jobid = rail.PythonOperator(
            task_id='get_master_jobid',
            python_callable=lambda: rail.render_template('{{ dag_run_ecid() }}')
        )
        ...
        send_mail_no_change_records = rail.EmailOperator(
            ...
            subject='''{{get_company_key() }} - Belgium | User import completed- No change records found - {{ current_time() }} ''',
        ...
        # Belgium processes exempt BENELUX BV workers only.
        query_valid_records = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT * FROM workdayuserdata WHERE (NULLIF(userid, '') IS NOT NULL AND \
                legalentity == '{config.eligible_legal_entity}' AND exemptionstatus == '{config.eligible_exemption_status}' )""",
            name="valid_records"
        )
        ...
        # Per-user routing now lives in process_each_user_dag, fanned out in parallel lanes.
        # Each lane triggers one run per user; URIs resolved here from the pre-fetched lists
        # are passed via conf so the per-user runs never re-fetch them.
        process_each_user_parallel_dagrun = rail.trigger_parallel_dagrun(
            ...
            trigger_dag_id=config.momentive_belgium_user_sync_process_each_user_dag_id,
        ...
        # all_done: a per-user run that fails its lane still lets the log CSV and email proceed.
        ...
        # Supervisor fan-out: per-user children deferred entries into the master supervisor log.
        ...
        # Merge per-user logs + master validation entries; publish error/exception counts.
        ...
        compose_logs_csv = rail.WriteCSVFileOperator(
            ...
            header=['jobid', 'childjobid', 'userid', 'username', 'action', 'status', 'details'],
            row=lambda item: [
                rail.result('get_master_jobid'),
                item.get('childjobid', ''),
                item.get('userid', ''),
                item.get('username', ''),
        ...
        upload_logs_to_sftp = rail.SFTPUploadFileOperator(
            ...
            '''/belgium_userimport_log_{{ result('log_todaysdate_2') }}.csv''',
        )

        generate_download_link = rail.GeneratePresignedDownloadUrlOperator(
            task_id='generate_download_link',
            artifact_name="{{ result('compose_logs_csv')}}",
            output_file_name='''belgium_userimport_log_{{ result('log_todaysdate_2') }}.csv''',
        ...
        send_alert_mail_log_upload_unsuccessful = rail.EmailOperator(
            ...
            subject='''{{get_company_key() }} - Belgium |  Failed while uploading User import Logs to SFTP  - {{ current_time() }} ''',
            html_content='''templates/log_upload_failure.html''',
            params=None,
        )

        # Belgium email rules: Skipped does NOT count as exception; bcc alert only on errors.
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_logs', key='error_record_count') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " - Belgium | User import" }} \
        ...
        get_write_csv_task_source >> log_todaysdate_2 >> get_master_jobid >> create_csv_lines_12
