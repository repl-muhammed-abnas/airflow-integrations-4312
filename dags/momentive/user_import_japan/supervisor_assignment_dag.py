...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.momentive_japan_user_sync_supervisor_assignment_dag_id,
        description=f'Momentive_User Import Japan_Supervisor assignment {config.instance}',
        ...
        search_for_user_with_empid = rail.RepliconServiceOperator(
            ...
            data=request_payload.seachfor_supervisor_payload,
            data_handler=python_callable.search_userdata_list_for_managerid
        )
        ...
        if_supervisorloginname_not_equals_to_loginname_8 >> rail.Label(
            'No') >> catch_and_log_error_find_entry
        if_supervisorloginname_not_equals_to_loginname_8 >> rail.Label(
            'Yes') >> search_for_user_with_empid >> check_if_multiple_manageruseruri_present
