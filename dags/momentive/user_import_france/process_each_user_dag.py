...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.momentive_france_user_sync_process_each_user_dag_id,
        description=f'Momentive user import France - Process Each User {config.instance}',
        ...
        # Existing-user state (useruri/status/startdate/enddate) and all resolved uris are
        # pre-computed by the master (from the bulk userreferencereport + pre-fetched lists)
        # and arrive via dag_run.conf, so this task is a conf pass-through with no API calls.
        ...
        create_user_log >> get_all_req_uri_details_40 >> if_get_req_useruri_present_45
