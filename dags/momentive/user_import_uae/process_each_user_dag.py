...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.momentive_uae_user_sync_process_each_user_dag_id,
        description=f'Momentive user import UAE - Process Each User {config.instance}',
        ...
        # Recipe nodes 27-31: the search sits in a try with a 10s retry interval, and the
        # catch only logs an Error when the message is a transport failure ('503 service
        # unavailable', '504 gateway timeout', 'bad gateway', 'connection reset by peer',
        # 'failed to open tcp connection', 'server broke connection', 'timed out connecting
        # to server'). It is NOT a branch on the normal path - a search that succeeds, empty
        # result included, never reaches it. So the retries live here and a genuine failure
        # falls through to catch_and_log_errors, which records the Error for this user; the
        # other users are unaffected because each one is its own dag run.
