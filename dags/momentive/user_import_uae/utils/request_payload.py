...
def user_import_data(item):
    ...
    same shared other-countries report as Japan/Thailand; the Japan-flag column exists in
    the report but is unused for UAE)."""
    return [
...
def get_user_by_search_payload(text_search_term):
    """UserList GetData text-search by login (per foreach user)."""
    return {
        "page": "1",
        "pagesize": "100000",
        "columnUris": [
...
def process_each_user_payload(item):
    ...
    Carries the canonical row plus the four uris resolved HERE from the master's
    pre-fetched lists - incl. the UAE fallbacks of recipe nodes 57/62/67 (blank legalentity
    -> division 'MPM FZE', blank paygroup -> service center 'MPM FZE PAYGROUP', cost center
    has no default) - so the per-user runs never re-fetch those lists. The supervisor log
    artifact is the master's, so deferred supervisor entries land in one place for the
    master's fan-out; the per-user log is created by the run itself and gathered afterwards."""
    ...
    legalentity_name = item['legalentity'] if item['legalentity'] else 'MPM FZE'
    legalentityuri = ''
    if rail.result('get_all_enabled_divisions'):
        legalentityuri = rail.find_first_by_attr_and_get_attr(
            rail.result('get_all_enabled_divisions'), 'displayText', legalentity_name, 'uri', '')

    paygroup_name = item['paygroup'] if item['paygroup'] else 'MPM FZE PAYGROUP'
    paygroupuri = ''
    if rail.result('get_enabled_service_centers'):
        paygroupuri = rail.find_first_by_attr_and_get_attr(
            rail.result('get_enabled_service_centers'), 'displayText', paygroup_name, 'uri', '')
...
def _base_child_payload(dag_run):
    """The 30-field base conf every add/update/disable child receives, sourced from this
    per-user run's conf. 'logger' is THIS run's log artifact (gathered by the master after
    all per-user runs finish); 'supervisor_logger' is the master's shared artifact."""
    item = dag_run.conf
    return {
        'userid': item['userid'],
...
def process_disable_user_payload(dag_run):
    """Disable child conf. Recipe node 53: when the user is already disabled in Replicon
    (status false) the termination date sent is the existing Replicon end date."""
    item = dag_run.conf
...
def process_supervisor_mapper_data(item):
    ...
    'logger' must be the per-user run's log - the one holding this user's Add/Update entry,
    which the child filters out and rewrites with the supervisor outcome. Handing it the
    master's logger_list (which only holds validation skips now that each user logs into its
    own artifact) made that filter return nothing, so every supervisor result was dropped."""
    return {
...
# Which routing branch each task belongs to, so a failure is logged against the action that
# was actually being attempted. Tasks before the routing decision (search / uri resolution)
# are not listed - they keep 'Add/Update', which is the recipe's own wording for that catch.
...
def get_failure_log_entry(dag_run):
    ...
    Names the failing task and derives the action from the branch that task belongs to, so a
    failed disable is not reported as an 'Add/Update' error. A failure at the search keeps the
    recipe's exact wording (nodes 27-31). Works under BatchTaskRunOperator too: once retries
    are exhausted the batch runner writes the task's FAILED state, so get_failed_task_ids
    finds it; if it finds nothing we still log the bare error rather than break the entry."""
    ...
    if 'search_user' in failed_task_ids:
        details = f'Error while searching the user {error_message}'
...
def log_process_user_payload(dag_run):
    """The Skipped log entry for the router's dead-end branches (same detail strings as
    the recipe's lookup-table entries)."""
    item = dag_run.conf
    uris = rail.result('get_all_req_uri_details_40')
    action = "Add"
    ...
    if uris['useruri']:
        ...
        if str(uris['status']).lower() == 'false':
            if uris['enddate']:
                ...
                # startdate can be '' when the search row carries no start date - guard it the
                # same way the routing gate does rather than dying on strptime('').
...
def get_mandatory_fields_exception_message(item):
    """Recipe node 18 detail string: 'User ID must be present'."""
    missing_fields = []
...
def get_invalid_record(item):
    """Validation-skip row; the UAE recipe stamps country 'UAE' on these entries."""
