...
import rail
from momentive.user_import_india.utils.request_payload import final_policyset_schedule_entry
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.momentive_india_user_sync_child_put_remaining_balance_for_payout_dag_id,
        ...
        update_variable_20 = rail.SetVariableOperator(
            ...
            value=lambda: str(rail.result(
                'log_balancevalueleft_5')['decimal_left']) + ".5"
        )
        ...
        update_variable_22 = rail.SetVariableOperator(
            ...
            value=lambda: str(int(rail.result(
                'log_balancevalueleft_5')['decimal_left']) + 1) + ".0"
        )
        ...
        append_new_entry_to_be_added_in_policysetschedule = rail.PythonOperator(
            ...
            python_callable=final_policyset_schedule_entry
        )
        ...
        if_policysetschedule_exists >> rail.Label('No') >> finish
        if_policysetschedule_exists >> rail.Label(
            'No') >> get_past_policysetschedule_entries >> if_urn_in_scheduleentries

        if_urn_in_scheduleentries >> rail.Label(
            'No') >> log_no_policy_error >> finish
        if_urn_in_scheduleentries >> rail.Label(
            'Yes') >> append_new_entry_to_be_added_in_policysetschedule

        append_new_entry_to_be_added_in_policysetschedule >> put_timeoffpolicy_with_initial_balance_as_remaining_balance_38 >> finish
