...
from momentive.common_recipes_userimport.utils.python_callable import get_exceptions, get_supervisor_status_escalation

def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.momentive_othercountries_user_sync_supervisor_assignment_dag_id,
        description=f'momentive_othercountries_user_sync_supervisor_assignment_child_{config.instance}',
        ...
        add_updated_log = rail.WriteLogOperator(
            ...
            severity=lambda: 'Error' if 'Error' in rail.result('load_found_logs_entry')[0]['properties']['status'] else (
                    'Exception' if get_supervisor_status_escalation() else rail.result('load_found_logs_entry')[0]['properties']['status']),
            properties=lambda: {
                ...
                # Recipe col5: only multiple-match / disabled escalate to Exception; foreign
                # created / not-received keep the existing status.
                "status": 'Error' if 'Error' in rail.result('load_found_logs_entry')[0]['properties']['status'] else (
                    'Exception' if get_supervisor_status_escalation() else rail.result('load_found_logs_entry')[0]['properties']['status']),
                # Recipe col6: append the branch message to the original update details.
                # get_exceptions covers multiple-match / disabled / not-received (all soft
                # failures). log_created_and_updated covers the foreign supervisor created path.
                # Both must be APPENDed, not replace, so "User updated..., Foreign supervisor
                # created" both appear. When neither ran (local supervisor, no message),
                # the original details are preserved unchanged.
                "details": (rail.result('load_found_logs_entry')[0]['properties']['details'] + ',' + get_exceptions()) if get_exceptions() else (
                    (rail.result('load_found_logs_entry')[0]['properties']['details'] + ',' + rail.result('log_created_and_updated'))
                    if rail.result('log_created_and_updated')
                    else rail.result('load_found_logs_entry')[0]['properties']['details'])
            }
        )
