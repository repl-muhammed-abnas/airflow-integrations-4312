...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.momentive_othercountries_user_sync_disable_user_child_dag_id,
        description=f'momentive_othercountries_user_sync_disable_user_child_{config.instance}',
        ...
        # Recipe nodes 3-4: guard - never disable the integration user itself. (Routes to the
        # one_failed catch, so on a clean self-user match it is simply skipped = silent stop.)
        ...
        get_balance_summary_foraccount = rail.RepliconServiceOperator(
            ...
            data=request_payload.disable_balance_summary_payload,
        ...
        put_remaining_balance_for_payout = rail.TriggerDagRunOperator(
            ...
            conf=request_payload.disable_payout_annual_payload,
            execution_timeout=timedelta(days=config.execution_timeout_days)
        )
        ...
        put_remaining_balance_for_payout_as_0 = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.momentive_othercountries_user_sync_put_zero_balance_payout_child_dag_id,
            conf=request_payload.disable_payout_zero_payload,
            execution_timeout=timedelta(days=config.execution_timeout_days)
        )
        ...
        foreach_policiesby_timeofftype >> foreach_policiesby_timeofftype_end >> log_user_import_with_enddate >> catch_and_log_error
