...
import rail
from terraconconsultants.user_import.utils.python_callable_method import construct_policyschedule
from terraconconsultants.user_import.utils.request_payload import get_put_timeoffpolicywithinitialbalance
...
def create_timeoff_balance(config):
    with rail.create_airflow_dag(
        dag_id=f'terraconconsultants_userimport_child_timeoff_put_0_balance_{config.instance}',
        description=f'TerraconConsultants User Sync Child Time Off Put 0 balance {config.instance}',
        ...
        is_first_description_present = rail.IfOperator(
            ...
            test="{{ result('get_existingpolicy_schedule_for_timeoff') | length > 0 and \
                result('get_existingpolicy_schedule_for_timeoff') | first_or_default | \
        ...
        is_first_description_present >> rail.Label(
            'Yes') >> past_policyset_schedule >> get_startingbalance_script_uri >> \
            put_timeoffpolicy_with_initial_balance_as_0 >> dagrun_log_to_sumo
        is_first_description_present >> rail.Label(
...
rail.for_each_instance(create_timeoff_balance)
