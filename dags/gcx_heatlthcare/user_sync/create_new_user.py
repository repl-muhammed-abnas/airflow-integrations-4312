...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.create_new_user_child_dag_id,
        description=f"GCX CREATE NEW USER {config.instance}",
        ...
        has_mandatory_fields = rail.IfOperator(
            ...
            yes_task="is_manager_value_present",
        ...
        is_manager_value_present = rail.IfOperator(
            task_id="is_manager_value_present",
        ...
        if_manager_present = rail.IfOperator(
            ...
            no_task="create_user"  # CHANGED: Go to create_user instead of logging exception
        )
        ...
        log_supervisor_disbaled = rail.WriteLogOperator(
                ...
                message="User creation is skipped since the supervisor disabled in Replicon",
                ...
                properties=lambda dag_run:{
                    ...
                    'action': "Add",
                    'status': "Exception",
                    'details': "User creation is skipped since the supervisor disabled in Replicon",
        ...
        create_user = rail.RepliconServiceOperator(
            task_id="create_user",
            endpoint="/services/ImportService2.svc/CreateUserOrApplyModifications",
            data=request_payload.create_new_user_payload
        )
        ...
        has_mandatory_fields >> rail.Label("Yes") >> is_manager_value_present >> rail.Label("No") >> search_manager_user >> if_manager_present >> rail.Label("Yes") >> check_manager_status >> rail.Label("Yes") >> if_manager_as_supervisor >> rail.Label('No') >> assign_supervisor_perminssion >> create_user

        if_manager_present >> rail.Label("No") >> create_user  # CHANGED: Create user even if manager not found

        is_manager_value_present >> rail.Label("Yes") >> create_user

        check_manager_status >> rail.Label("No") >> log_supervisor_disbaled >> catch_and_log_errors

        if_manager_as_supervisor >> rail.Label('Yes') >> create_user >> update_to_log_success >> catch_and_log_errors
