...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"genoa_design_employee_work_hours_by_pay_codes_report_export_master_dag_{config.instance}",
        description=f"Genoa Design Employee Work Hours By Pay Codes Report Export Master Dag {config.instance}",
        ...
        get_report_details = rail.RepliconReportDetailsOperator(
            ...
            report_name=config.employee_work_hours_report,
        )
        ...
        list_sftp_files = rail.SFTPListFilesOperator(
            ...
            paths=[config.employee_work_hours_export_path]
        )

        has_files = rail.IfOperator(
            task_id='has_files',
            test=lambda: bool(rail.result('list_sftp_files').get(
                config.employee_work_hours_export_path)),
            yes_task='sftp_files',
        ...
        archive_files = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda item: {
                "export_path": config.employee_work_hours_export_path,
                "item": item
            }
        )
        ...
        upload_csv_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.employee_work_hours_export_path +
                "/Report 1 – Employee work hours by pay codes_{{current_time_in_specified_tz('Canada/Newfoundland', '%Y%m%dT%H%M%S')}}.csv",
        )
