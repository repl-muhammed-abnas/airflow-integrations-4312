...
from tsystems.user_import_v1.utils import custom_methods, request_payload
...
def create_process_each_user_child(config):
    ...
    for idx in range(0, config.PROCESS_USER_BATCH_COUNT):
        with rail.create_airflow_dag(
            dag_id=f'{config.process_user_record_child_dag_id}_batch_{idx+1}',
            description=f"T-Systems User Import Process User Record Child {config.instance}",
            ...
            can_run_batch_task = rail.IfOperator(
                ...
                yes_task="batch_task",
                no_task="create_user_child_log"
            )

            batch_task = rail.BatchTaskRunOperator(
                task_id="batch_task",
                start_task="create_user_child_log",
                end_task="catch_and_log_errors"
            )

            create_user_child_log = rail.CreateLogOperator(
                task_id='create_user_child_log'
            )
            ...
            process_update_user = rail.TriggerDagRunForEachItemOperator(
                ...
                items=[0],
                trigger_dag_id=lambda dag_run: get_process_users_batch_dag_id(config.update_user_child_dag_id, int(dag_run.conf['modulo'])),
                conf=lambda dag_run:
                {
                    **dag_run.conf,
                    "log_artifact": rail.result("create_user_child_log")
                },
            ...
            process_add_user = rail.TriggerDagRunForEachItemOperator(
                ...
                items=[0],
                trigger_dag_id=lambda dag_run: get_process_users_batch_dag_id(config.add_user_child_dag_id, int(dag_run.conf['modulo'])),
                conf=lambda dag_run:
                {
                    **dag_run.conf,
                    "log_artifact": rail.result("create_user_child_log")
            ...
            catch_and_log_errors = rail.WriteLogOperator(
                ...
                log='{{ result("create_user_child_log") }}',
            ...
            can_run_batch_task >> rail.Label("No") >> create_user_child_log

            create_user_child_log >> get_user_details >> if_user_exists
            if_user_exists >> rail.Label("Yes") >> process_update_user \
