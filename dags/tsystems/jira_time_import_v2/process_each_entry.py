...
from tsystems.jira_time_import_v2.utils import request_payload, response_filters, custom_methods
...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        get_all_project_details = rail.RepliconServiceOperator(
            ...
            data_handler=lambda res: res["results"][0] if (res and res["results"]) else null
        )
        ...
        log_project_not_found = rail.WriteLogOperator(
            ...
            properties={
                ...
                'employee_email': '{{ dag_run.conf.input_data.user_email }}',
                ...
                'project_manager_email': '{{ dag_run.conf.input_data.project_manager_email }}',
        ...
        log_user_not_assigned_to_project = rail.WriteLogOperator(
            ...
            log='{{ dag_run.conf.user_log }}',
            severity='Exception',
            message='User is not assigned to the project_id: {{ dag_run.conf.input_data.project_id }}',
            properties={
                'unique_id': '{{ dag_run.conf.input_data.unique_id }}',
                'employee_id': '{{ dag_run.conf.input_data.employee_id }}',
                'employee_email': '{{ dag_run.conf.input_data.user_email }}',
                ...
                'status': 'Exception',
                'action': 'Add',
                'details': 'User is not assigned to the project_id: {{ dag_run.conf.input_data.project_id }}'
            },
        )
        ...
        check_user_assigned_to_project >> rail.Label('Yes') >> get_required_tasks_for_project
