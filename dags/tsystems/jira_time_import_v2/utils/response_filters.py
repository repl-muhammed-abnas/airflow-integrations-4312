...
def filter_time_entries(response, dag_run):
    ...
    for entry in response:
        ...
        hours = 0
        if 'interval' in entry and entry['interval'] and entry['interval'].get('hours'):
...
def format_project_task_details(response, dag_run):
    ...
    tasks = list(map(lambda item: {
        ...
        "time_and_expense_entry_type": item['timeAndExpenseEntryType']['displayText'] if item['timeAndExpenseEntryType'] else null,
        "time_and_expense_entry_type_uri": item['timeAndExpenseEntryType']['uri'] if item['timeAndExpenseEntryType'] else null
