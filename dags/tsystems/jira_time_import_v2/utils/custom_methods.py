...
def check_user_assigned_to_project(dag_run):
    ...
    # Resource type URIs for group-based assignments
    RESOURCE_TYPE_USER = 'urn:replicon:resource-type:user'
    RESOURCE_TYPE_LOCATION = 'urn:replicon:resource-type:location'
    RESOURCE_TYPE_SERVICE_CENTER = 'urn:replicon:resource-type:service-center'
    RESOURCE_TYPE_DEPARTMENT_GROUP = 'urn:replicon:resource-type:department-group'
    RESOURCE_TYPE_EMPLOYEE_TYPE = 'urn:replicon:resource-type:employee-type-group'
    ...
    for team_member in project_details.get('team', []):
        ...
        # Check 1: Direct user assignment
        if resource_type_uri == RESOURCE_TYPE_USER:
        ...
        # Check 2: Org Structure (Location) group assignment
        elif resource_type_uri == RESOURCE_TYPE_LOCATION:
            user_location_uri = user_groups.get('location_uri')
            if user_location_uri:
                location = resource.get('location')
                if location and location.get('uri') == user_location_uri:
                    return True
                if resource.get('uri') == user_location_uri:
                    return True

        # Check 3: Service Center group assignment
        elif resource_type_uri == RESOURCE_TYPE_SERVICE_CENTER:
            user_service_center_uri = user_groups.get('service_center_uri')
            if user_service_center_uri:
                service_center = resource.get('serviceCenter')
                if service_center and service_center.get('uri') == user_service_center_uri:
                    return True
                if resource.get('uri') == user_service_center_uri:
                    return True

        # Check 4: Department Group assignment
        elif resource_type_uri == RESOURCE_TYPE_DEPARTMENT_GROUP:
            user_department_uri = user_groups.get('department_uri')
            if user_department_uri:
                department_group = resource.get('departmentGroup')
                if department_group and department_group.get('uri') == user_department_uri:
                    return True
                if resource.get('uri') == user_department_uri:
                    return True

        # Check 5: Employee Type group assignment
        elif resource_type_uri == RESOURCE_TYPE_EMPLOYEE_TYPE:
            user_employee_type_uri = user_groups.get('employee_type_uri')
            if user_employee_type_uri:
                employee_type = resource.get('employeeTypeGroup')
                if employee_type and employee_type.get('uri') == user_employee_type_uri:
                    return True
                if resource.get('uri') == user_employee_type_uri:
                    return True
...
def check_billing_rate_allowed_for_user(dag_run):
    ...
    if billing_type_uri == 'urn:replicon:billing-type:non-billable':
        return True
    ...
    for team_member in project_details.get('team', []):
        ...
        if resource:
            ...
            if user_found:
                ...
                billing_rates = team_member.get('billingRatesAllowedForBillingTime') or []
...
def get_submitted_timesheet_uris():
    ...
    for timesheet in timesheet_data:
        if timesheet.get('timesheet_status') not in ['Not Submitted', 'Rejected']:
