...
def create_metadata_payload(dag_run, task_details):
    """
    ...
    project_details = rail.result('get_all_project_details')['project']

    is_billable = False

    project_billing_type = project_details['billingType']['uri'] if project_details['billingType'] else null
    task_time_and_entry_type = task_details['time_and_expense_entry_type_uri']

    project_specific_billing_type_uri = 'urn:replicon:project-specific-billing-rate'

    # Determine billing based on project type and task configuration
    if project_billing_type == 'urn:replicon:billing-type:time-and-material':
        # For time and material projects, check task time and expense entry configuration
        if task_time_and_entry_type == 'urn:replicon:time-and-expense-entry-type:billable-and-non-billable':
            # Billable and non-billable: choose billable
            is_billable = True
        elif task_time_and_entry_type == 'urn:replicon:time-and-expense-entry-type:billable':
            # Billable only: choose billable
            is_billable = True
        elif task_time_and_entry_type == 'urn:replicon:time-and-expense-entry-type:non-billable':
            # Non-billable only: choose non-billable
            is_billable = False
            project_specific_billing_type_uri = null
        else:
            # Default for time and material if no specific configuration
            is_billable = False

    elif project_billing_type == 'urn:replicon:billing-type:fixed-bid':
        # For fixed bid projects: always billable
        is_billable = True

    elif project_billing_type == 'urn:replicon:billing-type:non-billable':
        # For non-billable projects: always non-billable
        is_billable = False
        project_specific_billing_type_uri = null

    else:
        # Default case if billing type is not recognized
        is_billable = False

    metadata.append({
        'keyUri': 'urn:replicon:time-entry-metadata-key:task',
        'value': {
            'uri': task_details['uri']
        }
    })
    ...
    if project_specific_billing_type_uri:
        metadata.append({
            'keyUri': 'urn:replicon:time-entry-metadata-key:billing-rate',
            'value': {
                'uri': project_specific_billing_type_uri
            }
        })
...
def put_time_entry_payload(dag_run, ENTRY_DATE_FORMAT):
    ...
    """
    task_details = rail.find_first_by_attr_and_get_attr(rail.result('get_required_tasks_for_project'), 'full_task_name', dag_run.conf['input_data']['full_task_path'])
    ...
    return {
        "timeEntryRevisionGroup": {
            ...
            "customMetadata": create_metadata_payload(dag_run, task_details),
