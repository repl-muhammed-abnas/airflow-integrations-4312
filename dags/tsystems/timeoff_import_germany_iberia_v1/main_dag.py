"""
T-Systems Germany/Iberia Time Off Import - Master DAG
Imports approved time off bookings from SAP BTP webhook data to Replicon
Supports Germany (6206, 6207, 6208) and Iberia (0370, 0377, 1046) organizations
"""

from datetime import timedelta
from pendulum import now
import rail
from tsystems.timeoff_import_germany_iberia_v1.utils import custom_methods, request_payload, response_filter

def create_master_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.master_dag_id,
        description='T-Systems Germany/Iberia Time Off Import - MASTER',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        schedule_interval=None,  # Triggered only by webhook
        max_active_runs=config.max_active_runs_master,
        default_args={'sftp_conn_id': config.sftp_conn_id}
    ) as dag:

        # View the webhook data passed from webhook DAG
        ...
        create_timeoff_collection = rail.CreateCollectionOperator(
            ...
            source=lambda dag_run: dag_run.conf['webhook_data']['TimeOff'],
        ...
        has_timeoff_data = rail.IfOperator(
            ...
            no_task='send_empty_payload_email'
        )
        ...
        query_valid_timeoff_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT ROW_NUMBER() OVER(ORDER BY ROWID) AS record_id,* FROM input_timeoff_data_collection                        WHERE
        ...
        send_empty_payload_email = rail.EmailOperator(
            task_id='send_empty_payload_email',
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject="{{ get_company_key() }} | Replicon TimeOff Import Germany/Iberia - Empty payload | {{ current_time_in_specified_tz() }}",
            html_content="templates/emails/empty_payload.html"
        )
        ...
        def get_process_timeoff_child_dag_id(record_id):
            modulo = int(record_id) % config.TIMEOFF_BATCH_COUNT
            if modulo == 0:
                return config.process_timeoff_child_dag_id
            return f"{config.process_timeoff_child_dag_id}_batch_{str(modulo)}"
        ...
        process_timeoff_records = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=lambda item: get_process_timeoff_child_dag_id(item.get("record_id")),            conf=lambda item: {
                **item,
        ...
        send_completion_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_logs', 'error_record_count') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject="""{{ get_company_key() + ' | Replicon TimeOff Import Germany/Iberia - '}} \
        ...
        # Task dependencies
        log_start_time >> create_timeoff_collection
        create_timeoff_collection >> has_timeoff_data
        
        # Empty payload path
        has_timeoff_data >> rail.Label("No") >> send_empty_payload_email
