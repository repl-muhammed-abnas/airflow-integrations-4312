...
import rail
from tsystems.project_import_file_based_v3.utils import request_payload, response_filter
    ...
    with rail.create_airflow_dag(
        ...
        extract_client_info = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: {
                ...
                'client_name_from_payload': dag_run.conf.get('client_name', ''),
                'client_name': f"{dag_run.conf['client_code']}_{dag_run.conf['client_name']}"
                    if dag_run.conf.get('client_name', '').strip()
                    else f"Unknown({dag_run.conf['client_code']})"
            }
        )
