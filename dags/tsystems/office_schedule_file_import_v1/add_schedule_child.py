...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.schedule_add_dag_id,
        description=f'{config.company_key} office schedule file import schedule add child{config.instance}',
        ...
        log_exception = rail.WriteLogOperator(
            ...
            properties={
                ...
                'status': 'Exception',
                'details': "{{ dag_run.conf.scheduletype + ' Schedule pattern is received for incorrect number of days' }}"
            }
        )
        ...
        has_valid_scheduletype >> rail.Label(
            'no') >> log_exception >> catch_and_log_errors
