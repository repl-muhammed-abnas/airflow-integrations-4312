...
# Import utilities
from tsystems.time_import.utils import custom_methods, request_payload
from tsystems.time_import.task.get_prereqs import get_prereqs_task_group
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        # Task: Validate required fields in all CSV records
        # Checks that Employee ID and Entry Date are present and valid
        validate_records = rail.PythonOperator(
            task_id='validate_records',
            python_callable=lambda: custom_methods.validate_and_separate_records(
                rail.load_all_records(rail.result('create_csv_collection'))
            )
        )
        ...
        store_valid_records = rail.QueryCollectionOperator(
            ...
            query="SELECT * FROM time_import_records WHERE NULLIF(employee_id, '') IS NOT NULL AND NULLIF(entry_date, '') IS NOT NULL",
            name="valid_entries"
        )
        ...
        store_invalid_records = rail.QueryCollectionOperator(
            ...
            query="SELECT * FROM time_import_records WHERE NULLIF(employee_id, '') IS NULL OR NULLIF(entry_date, '') IS NULL",
        ...
        has_any_records >> rail.Label("Yes") >> create_records_log >> validate_records
        has_any_records >> rail.Label("No") >> send_no_data_email
        
        # Store and validate records: Mandatory field validation and separation
        validate_records >> [store_valid_records, store_invalid_records] >> log_invalid_records >> has_valid_records
