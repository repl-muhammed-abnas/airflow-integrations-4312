...
from tsystems.time_import.utils import request_payload, custom_methods, response_filters
...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        log_invalid_entry_date = rail.WriteLogOperator(
            ...
            severity='Exception',
            message='Entry date is not in required format (DD/MM/YYYY) or Entry date is before user start date',
            properties={
                ...
                'status': 'Exception',
                'action': 'Validation',
                'details': 'Entry date is not in required format (DD/MM/YYYY) or Entry date is before user start date'
            },
        )
        ...
        log_user_missing_in_replicon = rail.WriteLogOperator(
            ...
            log='{{ result("create_process_user_log") }}',
            severity='Exception',
            message='User is not present or disabled in replicon for Employee Id: {{ dag_run.conf.employee_id }}',
            properties={
                ...
                'status': 'Exception',
                'action': 'Validation',
                'details': 'User is not present or disabled in replicon for Employee Id: {{ dag_run.conf.employee_id }}'
            },
        )
        ...
        get_timesheet_details = rail.RepliconServiceCallForEachItemOperator(
            ...
            items="{{ result('validate_entry_date').valid_records | to_json }}",
        ...
        trigger_process_each_inout_entry = rail.TriggerDagRunForEachItemOperator(
            ...
            items="{{ result('validate_entry_date').unique_entry_dates | to_json }}",
            ...
            conf=lambda item, dag_run: {
                ...
                **dag_run.conf,
                ...
                'user_uri': rail.result('get_user_details')['userDetails']['uri'],
                'user_ts_type': config.TIMESHEET_TEMPLATES.get(rail.result('get_user_details')['timesheetTemplate']['displayText'], ''),
        ...
        if_user_uri_present >> rail.Label("Yes") >> validate_entry_date >> log_invalid_entry_date >> get_timesheet_details
