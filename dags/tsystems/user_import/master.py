...
from tsystems.user_import.tasks.process_user_groups_data import get_all_groups_data
from tsystems.user_import.utils import custom_methods, request_payload, response_filters
from tsystems.user_import.tasks.process_user_prerequisites import get_all_prerequisites_data
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        can_use_user_api_source = rail.IfOperator(
            ...
            no_task='create_users_payload_collection'
        )
        ...
        get_user_details_from_source = rail.SimpleHttpOperator(
            ...
            response_filter=lambda response: response_filters.filter_users_data_from_source(response, config.api_keys_mapper)
        )
        ...
        create_users_payload_collection = rail.CreateCollectionOperator(
            ...
            columns=[
                ...
                "enddate", "supervisorempid", "orgstructure", "costcenter", "legalunit",
        ...
        process_create_holiday_calendars = rail.TriggerDagRunOperator(
            task_id="process_create_holiday_calendars",
            trigger_dag_id=config.create_holiday_calendar_child_dag_id,
        ...
        wait_for_process_create_holiday_calendars = rail.WaitForDagRunsSensor(
            task_id='wait_for_process_create_holiday_calendars',
            dag_runs='{{ result("process_create_holiday_calendars") }}',
            execution_timeout=timedelta(days=config.execution_timeout_days)
        )
        ...
        process_user_record = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda item: {
                **custom_methods.create_users_payload_from_variable(item, config),
                "oef_data": {
                    oef["oef_name"]: {
                        "oef_uri": rail.result("get_all_user_oefs")[f"{oef['field_name']}_oef_uri"],
                        "oef_tag_uri": (
                            rail.find_first_by_attr_and_get_attr(
                                rail.result(f'get_{oef["field_name"]}_oef_values', []),
                                "name",
                ...
                "holiday_calendar_uri": rail.find_first_by_attr_and_get_attr(
                    ...
                    "displayText",
                    f'{item.get("country_of_employment", "")}_{item.get("city_of_employment", "")}', 
                    "uri"
                ),
        ...
        trigger_log_generation = rail.TriggerDagRunOperator(
            ...
            conf=lambda: {
                **rail.result("logging_details"),
                ...
                'total_record_count': rail.result("query_mandatory_fields", key="length")
            }
        )
        ...
        #Define task flow
        logging_details >> create_log >> can_use_user_api_source
        can_use_user_api_source >> rail.Label("Yes") >> get_user_details_payload >> get_user_details_from_source >> create_users_payload_collection \
            >> if_user_payload_exists
        if_user_payload_exists >> rail.Label("Yes") >> query_blank_mandatory_fields
        if_user_payload_exists >> rail.Label("No") >> format_logs
        can_use_user_api_source >> rail.Label("No") >> create_users_payload_collection
        query_blank_mandatory_fields >> if_blank_mandatory_fields
        if_blank_mandatory_fields >> rail.Label("Yes") >> write_blank_mandatory_fields_log >> query_mandatory_fields >> if_mandatory_fields_data_exists
        if_mandatory_fields_data_exists >> rail.Label("Yes") >> prereq_data_start \
            >> process_create_holiday_calendars >> wait_for_process_create_holiday_calendars \
                >> process_create_oef_tags >> wait_for_process_create_oef_tags >> groups_data_start >> process_get_groups_data >> process_get_prerequisites_data \
                    >> process_user_record >> wait_for_process_user_record >> gather_user_logs >> format_logs
        if_mandatory_fields_data_exists >> rail.Label("No") >> format_logs
        if_blank_mandatory_fields >> rail.Label("No") >> query_mandatory_fields

        format_logs >> has_any_entries_in_log >> rail.Label("Yes") >> trigger_log_generation >> finish_import
        has_any_entries_in_log >> rail.Label("No") >> finish_import
