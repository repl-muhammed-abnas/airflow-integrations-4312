...
def get_basic_user_details_update(dag_run, YMD_DATE_FORMAT, REP_DATE_FORMAT):
    ...
    if is_rehire:
        ...
        if start_date_str:
            start_date = datetime.strptime(start_date_str, REP_DATE_FORMAT if dag_run.conf.get("startdate") else REP_DATE_FORMAT)
    ...
    elif not should_be_active and current_security.get("isLoginEnabled"):
    ...
    elif should_be_active and not current_security.get("isLoginEnabled"):
        # Enable based on mapper - check if end date prevents activation
        end_date_prevents_activation = false
        if existing_end_date:
            existing_end = datetime.strptime(existing_end_date, REP_DATE_FORMAT)
...
def get_updated_supervisor(dag_run):
    ...
    supervisor_assignment = rail.result("get_supervisor_assignment_details", "supervisor")
...
def get_exception_logs(dag_run, is_update_scenario, config):
    ...
    if dag_run.conf.get("supervisorempid") and not rail.result("get_supervisor_details"):
