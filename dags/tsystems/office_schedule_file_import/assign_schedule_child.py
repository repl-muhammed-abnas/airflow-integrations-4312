...
from tsystems.office_schedule_file_import.utils import request_payload, custom_methods
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        if_user_is_active = rail.IfOperator(
            ...
            test="{{result('get_user_details')[0].userDetails.isEnabled | is_truthy}}",
            yes_task='if_valid_from_date_is_less_than_user_start_date',
            no_task='log_exception_user_not_active'
        )

        log_exception_user_not_active = rail.WriteLogOperator(
            task_id='log_exception_user_not_active',
            log='{{ result("create_assignment_log") }}',
            message="User is not active in Replicon",
            severity='Exception',
            properties=lambda dag_run: {
                'employee_id': dag_run.conf['employee_id'],
                'schedule_name': dag_run.conf['schedule_name'],
                'action': 'Validation',
                'status': 'Exception',
                "details": "User is not active in Replicon"
            }
        )
        ...
        log_user_schedule_assignment_successful = rail.WriteLogOperator(
            task_id='log_user_schedule_assignment_successful',
            log='{{ result("create_assignment_log") }}',
            message="User schedule assignment successful",
            severity='Success',
            properties=lambda dag_run: {
                'employee_id': dag_run.conf['employee_id'],
                'schedule_name': dag_run.conf['schedule_name'],
                ...
                'status': 'Success',
                'details': 'User schedule assignment was successful'
            },
        )
        ...
        if_user_availablein_replicon >> rail.Label('Yes') >> if_user_is_active
        if_user_is_active >> rail.Label('Yes') >> if_valid_from_date_is_less_than_user_start_date
        if_valid_from_date_is_less_than_user_start_date >> rail.Label('Yes') >>update_user_with_schedule >> log_user_schedule_assignment_successful >> catch_and_log_errors
        if_valid_from_date_is_less_than_user_start_date >> rail.Label('No') >> log_exception_before_start_date >> catch_and_log_errors
        if_user_is_active >> rail.Label('No') >> log_exception_user_not_active >> catch_and_log_errors
