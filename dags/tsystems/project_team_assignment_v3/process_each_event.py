...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        if_project_is_time_and_material_and_user_has_billing_rate = rail.IfOperator(
            task_id='if_project_is_time_and_material_and_user_has_billing_rate',
            test=lambda: bool(rail.result("get_project_from_costobject_id")["project_billingtype"] == "Time & Materials" and len(
                rail.result("get_user_assigned_to_project_for_billingrate")["project_rate_uri"]) > 0),
            yes_task="unassign_project_rate",
            no_task='get_capacity_items'
        )

        unassign_project_rate = rail.RepliconServiceOperator(
            task_id='unassign_project_rate',
            endpoint='/services/TimeAndMaterialsProjectService1.svc/PutProjectTeamMemberBillingRatesAllowedForBillingTime3',
            data=request_payload.unassign_project_rate_payload
        )
        ...
        get_user_assigned_to_project_for_billingrate = rail.RepliconServiceOperator(
            task_id='get_user_assigned_to_project_for_billingrate',
            endpoint='/services/ProjectService1.svc/GetProjectTeamMemberDetails',
            data={
                "projectUri": "{{ result('get_project_from_costobject_id')['project_uri'] }}",
                "resourceUri": "{{ result('get_user_from_individual_id')[0]['user_uri'] }}"
        ...
        update_user_assignment_date_range >> get_user_assigned_to_project_for_billingrate >> if_project_is_time_and_material_and_user_has_billing_rate

        if_project_is_time_and_material_and_user_has_billing_rate >> rail.Label("Yes") >> unassign_project_rate >> get_capacity_items
        if_project_is_time_and_material_and_user_has_billing_rate >> rail.Label("No") >> get_capacity_items
