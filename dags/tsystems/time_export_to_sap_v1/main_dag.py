...
import rail
import json
from tsystems.time_export_to_sap_v1.utils.python_callable import check_export_date_matches
...
def create_unified_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        get_project_type_oef_uri = rail.RepliconServiceOperator(
            task_id='get_project_type_oef_uri',
            endpoint='/services/ObjectExtensionService1.svc/GetAllObjectExtensionFieldDetails',
            data = {
        ...
        get_required_project_type_value  = rail.RepliconServiceOperator(
            task_id='get_required_project_type_value',
            ...
            data={
                ...
                "filterExpression": {
                    ...
                    "rightExpression": {
                        "value": {
                            "uri": "{{ result('get_project_type_oef_uri') }}"
                        }
                    }
                }
            },
            data_handler= lambda response : list(map(lambda d: d['cells'][0]['uri'], list(filter(lambda x: x['cells'][0].get('textValue') in ['Non SAP Project'], response['rows']))))
        )

        get_required_time_export_filter_definition = rail.RepliconServiceOperator(
            task_id='get_required_time_export_filter_definition',
            endpoint='/services/TimeDataExportService1.svc/GetAllFilterDefinitions',
            data_handler=lambda response: {
                'legal_unit_filter_uri' : rail.find_first_by_attr_and_get_attr(response, 'name', "Legal Unit", 'uri'),
                'project_type_filter_uri' : rail.find_first_by_attr_and_get_attr(response, 'name', "Project Type", 'uri')
            }
        )
        ...
        determine_export >> rail.Label("Export Scheduled") >> get_fileformat_script >> get_user_oef_uri >> \
            get_project_type_oef_uri >> get_required_project_type_value >> get_required_time_export_filter_definition >> \
                trigger_time_export_to_sap_process_export_child
