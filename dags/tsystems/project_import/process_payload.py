...
import rail
from tsystems.project_import.utils import custom_methods, response_filter
...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        query_any_blankmandatory_check = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM inputdata WHERE NULLIF(project_code,'') IS NULL OR NULLIF(project_name,'') IS NULL"""
        )
        ...
        write_blank_mandatory_field_log = rail.WriteLogOperator(
            ...
            properties=lambda item: {
                ...
                "action": "Validation",
                "status": "Exception",
                "details": "Mandatory field project_code or project_name is missing"
            }
        )
        ...
        query_valid_data_from_rawdata = rail.QueryCollectionOperator(
            ...
            query="""SELECT ROW_NUMBER() OVER(ORDER BY ROWID) AS record_id,* FROM inputdata WHERE
                NULLIF(project_code,'') IS NOT NULL AND NULLIF(project_name,'') IS NOT NULL"""
        )
        ...
        gather_prerequisites >> get_cost_center_as_department_groups >> get_org_structure_as_location_groups >> get_service_centers_as_department_groups >> \
            get_project_manager_permission_set >> get_project_oef_fields >> get_oef_drop_down_project_oefs >> query_distinct_clients
