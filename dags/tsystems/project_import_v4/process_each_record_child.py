...
import rail
from tsystems.project_import_v4.utils import request_payload, custom_methods
...
def create_process_each_record_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        get_assigned_pm_permissions = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response, dag_run: custom_methods.get_missing_pm_permission_sets(
                response,
                dag_run.conf.get('project_manager_permission_set', '')
            )
        )

        has_missing_pm_permission = rail.IfOperator(
            task_id='has_missing_pm_permission',
            test=lambda: len(rail.result('get_assigned_pm_permissions')) > 0,
        ...
        # Version 1.7: Check if restriction-based team eligibility is required
        # Runs when assign_from_department_uris OR assign_from_employee_type_uris OR assign_from_cost_center_uris are present
        has_restrictions = rail.IfOperator(
            ...
            test=lambda dag_run: bool(
                ...
                dag_run.conf.get('team_departments', {}).get('assign_from_employee_type_uris', []) or
                dag_run.conf.get('team_departments', {}).get('assign_from_cost_center_uris', [])
            ),
            yes_task='put_eligible_project_team_member',
            no_task='get_existing_project_team_members'
        )
        ...
        get_existing_project_team_members = rail.RepliconServiceOperator(
            task_id='get_existing_project_team_members',
            endpoint='/services/ProjectService1.svc/BulkGetAllProjectTeamMembers2',
            data=lambda: {
                "projectUris": [rail.result("create_or_update_project")['uri']]
            },
            data_handler=lambda response, dag_run: custom_methods.get_new_team_member_uris(
                response,
                dag_run.conf.get('team_departments', {}).get('uris', [])
            )
        )

        has_new_team_members = rail.IfOperator(
            task_id='has_new_team_members',
            test=lambda: bool(rail.result('get_existing_project_team_members')),
        ...
        assign_team_members_to_tasks = rail.RepliconServiceCallForEachItemOperator(
            ...
            data=lambda item: {
                "taskUri": item['uri'],
                "resourceUris": rail.result('get_existing_project_team_members'),
                "isAssigned": True
            }
        )
        ...
        is_project_manager_valid >> rail.Label("Yes") >> get_assigned_pm_permissions >> has_missing_pm_permission
        is_project_manager_valid >> rail.Label("No") >> create_or_update_project >> should_create_task

        has_missing_pm_permission >> rail.Label("Yes") >> assign_manager_permission_set >> create_or_update_project
        has_missing_pm_permission >> rail.Label("No") >> create_or_update_project
        ...
        # Version 1.6: Restrictions branch - set eligible team members if restrictions exist
        has_restrictions >> rail.Label("Yes") >> put_eligible_project_team_member >> get_existing_project_team_members
        has_restrictions >> rail.Label("No") >> get_existing_project_team_members >> has_new_team_members

        has_new_team_members >> rail.Label("Yes") >> get_all_tasks_for_project >> \
            assign_team_members_to_tasks >> log_project_success
        has_new_team_members >> rail.Label("No") >> log_project_success
