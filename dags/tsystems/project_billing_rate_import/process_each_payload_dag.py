...
def create_process_each_payload_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        if_billing_rate_description_exists_in_replicon = rail.IfOperator(
            ...
            yes_task='if_billing_rate_name_or_amount_changed',
            no_task='check_if_billing_rate_name_already_exists_in_replicon',
        )

        if_billing_rate_name_or_amount_changed = rail.IfOperator(
            task_id='if_billing_rate_name_or_amount_changed',
            test=lambda: (rail.result('log_payload_data')['existing_billing_rate_name'] != rail.result(
                'log_payload_data')['final_billing_rate_name']) or (float(rail.result('log_payload_data')['Billing_Rate_Value']) != float(rail.result(
                    'log_payload_data')['existing_billing_rate_amount'])),
            yes_task="trigger_update_billing_rate_dag",
            no_task="log_no_changes_detected"
        )

        log_no_changes_detected = rail.WriteLogOperator(
            task_id='log_no_changes_detected',
            log="{{ result('create_log') }}",
            message="No changes in billing rate",
            severity="Skipped",
            properties=lambda: {
                "billing_rate_id": rail.result('log_payload_data')['Billing_Rate_ID'],
                "billing_rate_name": rail.result('log_payload_data')['final_billing_rate_name'],
                ...
                "action": "Validation",
                "status": "Skipped",
                "details": "No changes in billing rate",
            }
        )
        ...
        if_billing_rate_description_exists_in_replicon >> rail.Label(
            'Yes') >> if_billing_rate_name_or_amount_changed
        if_billing_rate_description_exists_in_replicon >> rail.Label(
            'No') >> check_if_billing_rate_name_already_exists_in_replicon

        if_billing_rate_name_or_amount_changed >> rail.Label(
            'Yes') >> trigger_update_billing_rate_dag
        if_billing_rate_name_or_amount_changed >> rail.Label(
            'No') >> log_no_changes_detected >> catch_and_log_errors
