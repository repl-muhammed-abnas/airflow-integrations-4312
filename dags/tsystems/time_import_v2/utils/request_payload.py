...
def get_bulk_user_data_payload(employee_ids: List[str]) -> Dict[str, Any]:
    """
    Build payload for retrieving multiple users' details from Replicon in one call.

    Used to resolve display names for a batch of employee IDs up front (e.g. for
    records that get rejected in the master DAG before per-employee processing).

    Args:
        employee_ids: List of employee IDs to look up

    Returns:
        Dict[str, Any]: API request payload for the bulk user details lookup
    """
    return {
        "users": [
            {
...
def create_metadata_payload(dag_run, task_uri: Optional[str]) -> List[Dict[str, Any]]:
    ...
    # Handle billing rate - for fixed bid projects use project-specific billing rate
    project_details = rail.result('get_all_project_details')
    project = project_details.get('project', project_details) if project_details else {}
    billing_type = project.get('billingType')
    billing_type_uri = billing_type.get('uri') if billing_type else None

    billing_rate_name = dag_run.conf['entry_data'].get('billing_rate_name', '')
    if billing_type_uri == 'urn:replicon:billing-type:fixed-bid' and billing_rate_name.lower() == 'project rate':
        metadata.append({
    ...
    elif dag_run.conf["billing_rate_uri"]:
        metadata.append({
            "keyUri": "urn:replicon:time-entry-metadata-key:billing-rate",
            "value": {
...
def put_time_entry_payload(dag_run, task_uri: Optional[str]) -> Dict[str, Any]:
    ...
    allocation_type = ["urn:replicon:time-allocation-type:project"]
    ...
    return {
        "timeEntryRevisionGroup": {
            ...
            "timeAllocationTypeUris": allocation_type,
            "interval": {
                "hours": get_interval_from_hours(time_entries["hours"]) if time_entries["hours"] and bool(float(time_entries["hours"])) else null,
                "timePair": null
            },
