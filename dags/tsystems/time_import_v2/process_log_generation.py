...
import rail
from tsystems.time_import_v2.utils import custom_methods
...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        # Username column is currently gated to instances with config.include_username_in_logs (trial only)
        log_header = ['Employee ID']
        log_row = ["{{ item.employee_id }}"]
        if config.include_username_in_logs:
            log_header.append('Username')
            log_row.append("{{ item.user_name }}")
        log_header += ['Entry Date', 'Project ID', 'Task Name', 'Activity', "Action", "Status", "Details", "ECID | Run ID"]
        log_row += [
            "{{ item.entry_date }}",
            "{{ item.project_id }}",
            "{{ item.task_name }}",
