...
from tsystems.time_import_v2.utils import request_payload, custom_methods, response_filters
...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        batch_task = rail.BatchTaskRunOperator(
            ...
            end_task='if_submit_reopen_failed',
        )
        ...
        log_user_missing_in_replicon = rail.WriteLogOperator(
            ...
            properties={
                ...
                'user_name': '{{ (result("get_user_details").userDetails.customDisplayName or result("get_user_details").userDetails.displayText or "") if result("get_user_details") else "" }}',
        ...
        # Task: Verify reported by user is the same user or part of user's substitute users
        # Allows self-import (user importing for themselves) or substitute user import
        if_reported_user_is_substitute_user = rail.IfOperator(
            task_id ='if_reported_user_is_substitute_user',
            test=lambda dag_run: (
                dag_run.conf["reported_by_user_uri"] == rail.result('get_user_details')['userDetails']['uri'] or
                rail.find_first_by_attr_and_get_attr(rail.result("get_user_substitute_users_details"),
        ...
        log_reported_by_user_is_not_substitute_in_replicon = rail.WriteLogOperator(
            ...
            properties={
                'row_number': '{{ item.row_number }}',
                'employee_id': '{{ item.employee_id }}',
                'user_name': '{{ result("get_user_details").userDetails.customDisplayName or result("get_user_details").userDetails.displayText or "" }}',
                'entry_date': '{{ item.entry_date }}',
                'project_id': '{{ item.project_id }}',
                'task_name': '{{ item.task_name }}',
                'activity': '{{ item.activity }}',
        ...
        if_valid_records_exists = rail.IfOperator(
            ...
            no_task="if_submit_reopen_failed"
        )
        ...
        # Task: Group timesheet URIs by submission status
        # Returns {'submitted': [...], 'all': [...]} for downstream selective processing
        group_ts_uris_by_status = rail.PythonOperator(
            task_id='group_ts_uris_by_status',
            python_callable=lambda: custom_methods.group_timesheet_uris_by_status(rail.result('get_timesheet_details'))
        )
        ...
        reopen_timesheets = rail.RepliconServiceCallForEachItemOperator(
            ...
            items="{{result('group_ts_uris_by_status').submitted | to_json}}",
        ...
        trigger_process_each_inout_entry = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda item, dag_run: {
                ...
                **dag_run.conf,
                ...
                'user_uri': rail.result('get_user_details')['userDetails']['uri'],
                'user_name': rail.result('get_user_details')['userDetails'].get('customDisplayName') or rail.result('get_user_details')['userDetails'].get('displayText') or '',
        ...
        submit_reopen_timesheets = rail.RepliconServiceCallForEachItemOperator(
            ...
            items="{{result('group_ts_uris_by_status').all | to_json}}",
        ...
        # Task: Check if submit_reopen_timesheets failed
        # Fires only when an upstream task fails; routes to exception log on failure, skips on success
        if_submit_reopen_failed = rail.IfOperator(
            task_id='if_submit_reopen_failed',
            trigger_rule='one_failed',
            test=custom_methods.is_submit_reopen_timesheets_failed,
            yes_task='log_submit_reopen_exception',
            no_task='catch_and_log_errors'
        )

        # Task: Log exception when re-submitting timesheets fails
        log_submit_reopen_exception = rail.WriteLogOperator(
            task_id='log_submit_reopen_exception',
            log='{{ result("create_process_user_log") }}',
            severity='Exception',
            message='Failed to submit timesheet for EmployeeId: {{ dag_run.conf.employee_id }}',
            properties={
                'row_number': '00',
                ...
                'details': 'Failed to submit timesheet after time import'
            },
        )
        ...
        can_run_batch_task >> rail.Label('Yes') >> batch_task >> if_submit_reopen_failed
        ...
        if_user_uri_present >> rail.Label("Yes") >> get_user_current_group_assignments >> get_user_substitute_users_details >> if_reported_user_is_substitute_user
        if_user_uri_present >> rail.Label("No") >> log_user_missing_in_replicon >> if_submit_reopen_failed
        ...
        if_reported_user_is_substitute_user >> rail.Label("No") >> log_reported_by_user_is_not_substitute_in_replicon \
            >> if_submit_reopen_failed
        
        if_valid_records_exists >> rail.Label("No") >> if_submit_reopen_failed
        if_valid_records_exists >> rail.Label("Yes") >> get_timesheet_details

        get_timesheet_details >> group_ts_uris_by_status >> \
        ...
        wait_for_process_each_entry >> submit_reopen_timesheets >> if_submit_reopen_failed
        if_submit_reopen_failed >> rail.Label("Yes") >> log_submit_reopen_exception
        if_submit_reopen_failed >> rail.Label("No") >> catch_and_log_errors
