...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        if_timesheet_template_present_in_mapper = rail.IfOperator(
            ...
            yes_task='if_activity_assigned_to_user',
        ...
        log_timesheet_type_not_supported = rail.WriteLogOperator(
            ...
            properties={
                'row_number': '{{ dag_run.conf.entry_data.row_number }}',
        ...
        if_activity_assigned_to_user = rail.IfOperator(
            ...
            test=lambda dag_run: bool(dag_run.conf["entry_data"]["activity"] and dag_run.conf["activity_uri"]),
            yes_task="if_project_task_present",
        ...
        log_activity_not_assigned_to_user = rail.WriteLogOperator(
            ...
            log='{{ dag_run.conf.user_log }}',
            severity='Exception',
            message='Activity {{ dag_run.conf.entry_data.activity }} is not assigned to user',
            properties={
                'row_number': '{{ dag_run.conf.entry_data.row_number }}',
                'employee_id': '{{ dag_run.conf.entry_data.employee_id }}',
                'user_name': '{{ dag_run.conf.user_name or "" }}',
                ...
                'status': 'Exception',
                'action': 'Add',
                'details': 'Activity {{ dag_run.conf.entry_data.activity }} is not assigned to user'
            },
        )
        ...
        get_all_project_details = rail.RepliconServiceOperator(
            ...
            data_handler=lambda res: res["results"][0] if (res and res["results"]) else None
        )
        ...
        check_project_exists = rail.IfOperator(
            ...
            yes_task="check_user_assigned_to_project",
        ...
        # Task: Check if user is assigned to the project team
        check_user_assigned_to_project = rail.IfOperator(
            task_id='check_user_assigned_to_project',
            test=custom_methods.check_user_assigned_to_project,
            yes_task='check_billing_rate_assigned_to_project',
            no_task='log_user_not_assigned_to_project'
        )
        
        # Task: Log error when user is not assigned to the project
        log_user_not_assigned_to_project = rail.WriteLogOperator(
            task_id='log_user_not_assigned_to_project',
            log='{{ dag_run.conf.user_log }}',
            severity='Exception',
            message='User is not assigned to the project_id: {{ dag_run.conf.entry_data.project_id }}',
            properties={
                'row_number': '{{ dag_run.conf.entry_data.row_number }}',
                'employee_id': '{{ dag_run.conf.entry_data.employee_id }}',
                'user_name': '{{ dag_run.conf.user_name or "" }}',
                ...
                'status': 'Exception',
                'action': 'Add',
                'details': 'User is not assigned to the project_id: {{ dag_run.conf.entry_data.project_id }}'
            },
        )

        check_billing_rate_assigned_to_project = rail.IfOperator(
            task_id='check_billing_rate_assigned_to_project',
            test=custom_methods.check_billing_rate_allowed_for_user,
            yes_task='get_all_tasks_for_project',
            no_task='log_billing_rate_not_assigned_to_project'
        )

        # Task: Log error when billing rate is not allowed for the user on this project
        # Records billing rate validation failure for reporting and troubleshooting
        log_billing_rate_not_assigned_to_project = rail.WriteLogOperator(
            task_id='log_billing_rate_not_assigned_to_project',
            log='{{ dag_run.conf.user_log }}',
            severity='Exception',
            message='Billing rate is not associated to the Project/user',
            properties={
                'row_number': '{{ dag_run.conf.entry_data.row_number }}',
                'employee_id': '{{ dag_run.conf.entry_data.employee_id }}',
                'user_name': '{{ dag_run.conf.user_name or "" }}',
                ...
                'status': 'Exception',
                'action': 'Add',
                'details': 'Billing rate is not associated to the Project/user'
            },
        )
        ...
        get_all_tasks_for_project = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "parentUri": rail.result('get_all_project_details')['project']['uri']
            },
        ...
        check_task_exists = rail.IfOperator(
            ...
            test=lambda dag_run: bool(rail.find_first_by_attr_and_get_attr(
                rail.result('get_all_tasks_for_project'), 'full_task_name', dag_run.conf['entry_data']['task_name'], 'uri', False
            )),
        ...
        log_missing_ts_based_fields = rail.WriteLogOperator(
            ...
            log='{{ dag_run.conf.user_log }}',
            severity='Error',
            message='{{ result("has_required_ts_based_fields") }}',
            properties={
                'row_number': '{{ dag_run.conf.entry_data.row_number }}',
                'employee_id': '{{ dag_run.conf.entry_data.employee_id }}',
                'user_name': '{{ dag_run.conf.user_name or "" }}',
                ...
                'status': 'Exception',
                'action': 'Add',
                'details': '{{ result("has_required_ts_based_fields") }}'
            },
        )
        ...
        if_inout_or_hour_present = rail.IfOperator(
            ...
            no_task="log_missing_hours_or_inout"
        )

        log_missing_hours_or_inout = rail.WriteLogOperator(
            task_id='log_missing_hours_or_inout',
            log='{{ dag_run.conf.user_log }}',
            severity='Exception',
            message='Neither Hours nor Clock In/Clock Out times are provided for time entry.',
            properties={
                'row_number': '{{ dag_run.conf.entry_data.row_number }}',
                'employee_id': '{{ dag_run.conf.entry_data.employee_id }}',
                'user_name': '{{ dag_run.conf.user_name or "" }}',
                ...
                'status': 'Exception',
                'action': 'Add',
                'details': 'Neither Hours nor Clock In/Clock Out times are provided for time entry'
            }
        )
        ...
        add_time_entry = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: request_payload.put_time_entry_payload(
                dag_run,
                rail.find_first_by_attr_and_get_attr(rail.result('get_all_tasks_for_project'), 'full_task_name', dag_run.conf['entry_data']['task_name'], 'uri', ''),
            )
        )
        ...
        if_timesheet_template_present_in_mapper >> rail.Label('Yes') >> if_activity_assigned_to_user
        if_timesheet_template_present_in_mapper >> rail.Label('No') >> log_timesheet_type_not_supported >> catch_and_log_errors

        if_activity_assigned_to_user >> rail.Label('Yes') >> if_project_task_present
        if_activity_assigned_to_user >> rail.Label('No') >> log_activity_not_assigned_to_user >> if_project_task_present
        ...
        check_project_exists >> rail.Label('Yes') >> check_user_assigned_to_project
        check_project_exists >> rail.Label('No') >> log_project_not_found >> process_time_entry_details

        check_user_assigned_to_project >> rail.Label('Yes') >> check_billing_rate_assigned_to_project
        check_user_assigned_to_project >> rail.Label('No') >> log_user_not_assigned_to_project \
            >> catch_and_log_errors

        check_billing_rate_assigned_to_project >> rail.Label("Yes") >> get_all_tasks_for_project
        check_billing_rate_assigned_to_project >> rail.Label("No") >> log_billing_rate_not_assigned_to_project \
            >> catch_and_log_errors
        ...
        if_inout_or_hour_present >> rail.Label('No') >> log_missing_hours_or_inout >> catch_and_log_errors
