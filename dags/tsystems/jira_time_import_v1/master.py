...
from tsystems.jira_time_import_v1.utils import custom_methods
    ...
    with rail.create_airflow_dag(
        ...
        log_start_time_and_filename = rail.PythonOperator(
            ...
            python_callable=lambda: {
                ...
                "log_filename": rail.render_template('{{ dag_run_ecid() | replace(":", "-") }}_{{ result("new_file_sensor") | file_name | replace(".csv", "") }}_') + \
        ...
        get_project_details_report = rail.RepliconReportDetailsOperator(
            task_id='get_project_details_report',
            report_name=config.project_details_report
        )

        generate_project_details_report = rail.run_report2(
            group_id='generate_project_details_report',
            report_params={
                "reportParameters": [
                    {
                        "reportUri": "{{ result('get_project_details_report').uri }}",
                        "filterValues": [],
        ...
        load_project_details_csv = rail.LoadCSVFileOperator(
            task_id='load_project_details_csv',
            document="{{ (result('generate_project_details_report.get_report_result') | load_json_artifact).reportGenerationResults[0].payload }}",
            delimiter=config.csv_separator
        )

        create_project_details_collection = rail.CreateCollectionOperator(
            task_id='create_project_details_collection',
            source='{{ result("load_project_details_csv") }}',
            name='project_details'
        )

        # Task: Get user details report
        ...
        generate_user_details_report = rail.run_report2(
            group_id='generate_user_details_report',
            report_params={
                "reportParameters": [
                    {
        ...
        load_user_details_csv = rail.LoadCSVFileOperator(
            task_id='load_user_details_csv',
            document="{{ (result('generate_user_details_report.get_report_result') | load_json_artifact).reportGenerationResults[0].payload }}",
            delimiter=config.csv_separator
        )

        create_user_details_collection = rail.CreateCollectionOperator(
            task_id='create_user_details_collection',
            source='{{ result("load_user_details_csv") }}',
            name='user_details'
        )
        ...
        # Python-based join to handle duplicates properly
        create_input_users_projects_data = rail.CreateCollectionOperator(
            task_id='create_input_users_projects_data',
            source=custom_methods.join_user_and_project_details,
            name='filtered_input_time_data'
        )
        ...
        store_valid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM filtered_input_time_data
        ...
        get_working_time_activity_uri = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response: rail.find_first_by_attr_and_get_attr(response, "displayText", config.activity_name, "uri")
        )
        ...
        trigger_log_generation = rail.TriggerDagRunOperator(
            ...
            conf=lambda: {
                ...
                'log_file_suffix': rail.result("log_start_time_and_filename")["log_filename"],
        ...
        has_any_records >> rail.Label("Yes") >> add_entrydate_for_sql_format \
            >> create_processed_input_data_collection
        has_any_records >> rail.Label("No") >> send_no_data_email
        
        # Run both reports in parallel
        create_processed_input_data_collection >> get_project_details_report \
            >> generate_project_details_report >> load_project_details_csv \
                >> create_project_details_collection
        
        create_processed_input_data_collection >> get_user_details_report \
            >> generate_user_details_report >> load_user_details_csv \
                >> create_user_details_collection
        
        # Join all data after both reports are ready - using Python function instead of SQL queries
        [create_project_details_collection, create_user_details_collection] \
            >> create_records_log >> create_input_users_projects_data

        # Store and validate records: Mandatory field validation and separation
        create_input_users_projects_data >> [store_valid_records, store_invalid_records] >> has_invalid_records
