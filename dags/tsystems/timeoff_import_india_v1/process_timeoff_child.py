"""
T-Systems ICT India Time Off Import - Child DAG
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        description='T-Systems ICT India Time Off Import - Process Individual Record',
        ...
        log_invalid_dates = rail.WriteLogOperator(
            ...
            properties=lambda dag_run:{
                ...
                'transaction_id': dag_run.conf.get('transaction_id'),
                'action':'Validation',
                'status': 'Exception',
                'details': custom_methods.get_invalid_datetime_exception(dag_run)
            },
        )
        ...
        is_timeoff_for_delete = rail.IfOperator(
            ...
            test=lambda dag_run: dag_run.conf['duration_hours'] in [0.00, 0 , "0.00", "0"],
        ...
        get_time_off_approval_status = rail.RepliconServiceOperator(
            ...
            data={
                "timeOffUri": "{{ result('get_time_off_details_on_transaction_id').0.timeoff_uri \
                    if result('get_time_off_details_on_transaction_id') else result('put_timeoff_entry').uri }}"
            },
        ...
        force_approve_time_off_entry = rail.RepliconServiceOperator(
            ...
            data= lambda :{
                "timeOffUri": rail.result('get_time_off_details_on_transaction_id')[0]['timeoff_uri']
                    if rail.result('get_time_off_details_on_transaction_id') else rail.result('put_timeoff_entry')['uri'],
                "unitOfWorkId": str(uuid4()),
        ...
        # Batch Task Dependencies
        can_run_batch_task >> rail.Label("Yes") >> batch_task >> catch_and_log_errors
        can_run_batch_task >> rail.Label("No") >> create_log

        # Initial Validations
        create_log >> is_timeoff_type_available_in_mapper >> rail.Label('No') >> log_timeoff_type_not_available_in_mapper >> catch_and_log_errors
        is_timeoff_type_available_in_mapper >> rail.Label('Yes') >> get_user_on_empid
        get_user_on_empid >> is_user_present >> rail.Label('No') >> log_user_not_present >> catch_and_log_errors
        is_user_present >> rail.Label('Yes') >> get_user_info >> is_date_valid >> rail.Label('Yes') >> is_timeoff_template_present
        ...
        is_transaction_id_available >> rail.Label('Yes') >> is_update_required

        # Create/Modify Timeoff Booking
        is_update_required >> rail.Label('Yes') >> reopen_timeoff >> put_reopen_timeoff >> get_time_off_approval_status >> is_timeoff_approved
        is_update_required >> rail.Label('No') >> log_no_update_required >> catch_and_log_errors
        is_transaction_id_available >> rail.Label('No') >> put_timeoff_entry
        put_timeoff_entry >> get_time_off_approval_status >> is_timeoff_approved
        ...
        # Error handling for log_timeoff_success
