...
import rail
from tsystems.office_schedule_file_import_v2.utils import custom_methods
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        description=f'{config.company_key} office schedule file import Master {config.instance}',
        ...
        send_invalid_format_email = rail.EmailOperator(
            ...
            subject="{{ get_company_key() }} | Replicon Office Schedule Import - Invalid Format | {{ current_time_in_specified_tz() }}",
        ...
        load_json = rail.PythonOperator(
            task_id = 'load_json',
            python_callable=lambda: rail.load_json_artifact(rail.result('download_file'))
        )

        create_schedule_list_collection = rail.CreateCollectionOperator(
            task_id='create_schedule_list_collection',
            source="{{ result('load_json')['Schedule WorkTime'] | to_json }}",
            name='schedule_list',
            columns={
                "CID": "employee_id",
                "Valid From": "valid_from",
                "Schedule Name": "schedule_name",
                "Holiday Calendar name": "holiday_calendar_name",
                "Holiday Calendar effective date": "holiday_calendar_effective_date"
            }
        )
        ...
        log_add_invalid_records = rail.WriteLogOperator(
            ...
            properties=lambda item: {
                ...
                "holiday_calendar_name": item['holiday_calendar_name'] or '',
                "action": "Validation",
                "status": "Skipped",
        ...
        def get_create_new_holiday_calendars_not_inreplicon_by_name():
            valid_records = rail.load_all_records(rail.result('query_valid_records'))
            existing_calendar_names = [calendar['displayText'] for calendar in rail.result('get_all_holiday_calendars')]
            new_calendars = []
            seen = set()
            for record in valid_records:
                calendar_name = (record['holiday_calendar_name'] or '').strip()
                effective_date = (record['holiday_calendar_effective_date'] or '').strip()
                if calendar_name and effective_date and calendar_name not in existing_calendar_names and calendar_name not in seen:
                    new_calendars.append({"holiday_calendar_name": calendar_name})
                    seen.add(calendar_name)
            return new_calendars

        get_create_new_holiday_calendars = rail.PythonOperator(
            task_id='get_create_new_holiday_calendars',
            python_callable=get_create_new_holiday_calendars_not_inreplicon_by_name
        )

        foreach_create_holiday_calendars = rail.ForEachOperator(
            task_id='foreach_create_holiday_calendars',
            items="{{ result('get_create_new_holiday_calendars') | to_json }}",
            start_task='create_holiday_calendar',
            end_task='foreach_create_holiday_calendars_end'
        )

        create_holiday_calendar = rail.RepliconServiceOperator(
            task_id='create_holiday_calendar',
            endpoint='/services/HolidayCalendarService2.svc/PutHolidayCalendar',
            data=lambda: {
                "calendar": {
                    "target": {
                        "uri": null,
                        "name": rail.result('foreach_create_holiday_calendars')['holiday_calendar_name']
                    },
                    "name": rail.result('foreach_create_holiday_calendars')['holiday_calendar_name'],
                    "details": []
                }
        ...
        foreach_create_holiday_calendars_end = rail.EmptyOperator(
            task_id='foreach_create_holiday_calendars_end'
        )

        get_all_updated_holiday_calendars = rail.RepliconServiceOperator(
            task_id='get_all_updated_holiday_calendars',
            endpoint='/services/HolidayCalendarService1.svc/GetAllHolidayCalendars'
        )
        ...
        trigger_assign_schedule_to_employee = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda item: {
                ...
                'holiday_calendar_effective_date': item['holiday_calendar_effective_date'],
                'holiday_calendar_uri': rail.find_first_by_attr_and_get_attr(
                    rail.result('get_all_updated_holiday_calendars'),
                    'displayText',
                    (item['holiday_calendar_name'] or '').strip(),
                    'uri',
                    ''
                ) if (item['holiday_calendar_name'] or '').strip() else ''
            }
        )
        ...
        job_started_time >> new_file_sensor >> is_json
        is_json >> rail.Label('Yes') >> download_file
        is_json >> rail.Label('No') >> send_invalid_format_email
        download_file >> was_new_file_found
        was_new_file_found >> rail.Label('Yes') >> archive_file 
        download_file >> create_log >> load_json >> create_schedule_list_collection >> if_schedule_list_collection_has_records
        ...
        wait_for_create_new_schedules >> get_all_updated_office_schedules >> get_all_holiday_calendars >> get_create_new_holiday_calendars >> foreach_create_holiday_calendars
        foreach_create_holiday_calendars >> create_holiday_calendar >> foreach_create_holiday_calendars_end
        foreach_create_holiday_calendars >> foreach_create_holiday_calendars_end
        foreach_create_holiday_calendars_end >> get_all_updated_holiday_calendars >> trigger_assign_schedule_to_employee >> wait_for_assign_schedule_to_employee >> gather_assignment_logs >> format_logs
