...
def process_clock_records():
    ...
    return list(map(lambda record:{
        ...
        'Clock Out': f"{record['clock_out'].split(':')[0].zfill(2)}:{record['clock_out'].split(':')[1].zfill(2)}" if ':' in record['clock_out'] else record['clock_out'],
        'Number of Hours': "{:05.2f}".format(float(record['hours'].replace(',', '.'))) if record['hours'] else "00.00",
        'Work Type': get_work_type(record),
    }, report_data
    ))

def get_sumo_log_data(config):
    """
    Generate data for logging to Sumo Logic.
    
    Returns:
        dict: Data dictionary for Sumo Logic logging
    """
    return {
        'job_start_time': rail.result('process_start_time'),
        'job_end_time': pendulum.now(config.timezone).strftime("%Y-%m-%dT%H:%M:%S"),
        'export_date': pendulum.now(config.timezone).strftime(config.date_format_export),
        'export_filename': rail.result('generate_filename') if rail.result('create_json_export') else 'N/A',
        'number_of_records': rail.result('process_clock_data', 'length') if rail.result('process_clock_data') else 0,
        'export_status': 'success' if rail.result('post_to_client_api') or rail.result('upload_to_sftp') else 'failed',
