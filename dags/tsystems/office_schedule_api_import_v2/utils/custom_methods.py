...
def get_missing_holiday_field_message(dag_run):
    error_msg = []
    if not (dag_run.conf.get('holiday_calendar_name') or '').strip():
        error_msg.append("Holiday Calendar name is not present")
    if not (dag_run.conf.get('holiday_calendar_effective_date') or '').strip():
        error_msg.append("Holiday Calendar effective date is not present")
    return rail.smartjoin_by_delim(error_msg, ";")


def get_assignment_log_status(dag_run):
    name = (dag_run.conf.get('holiday_calendar_name') or '').strip()
    date = (dag_run.conf.get('holiday_calendar_effective_date') or '').strip()
    uri = dag_run.conf.get('holiday_calendar_uri')

    if not name and not date:
        return 'Success', 'Schedule Assignment', 'Schedule assigned successfully'
    if not (name and date):
        return 'Exception', 'Validation', 'Schedule assigned successfully. ' + get_missing_holiday_field_message(dag_run)
    if not uri:
        return 'Exception', 'Holiday Calendar Assignment', 'Schedule assigned successfully. Holiday calendar could not be resolved in Replicon'
    return 'Success', 'Schedule and Holiday Calendar Assignment', 'Schedule and holiday calendar assigned successfully'


def get_assignment_log_severity(dag_run):
    return get_assignment_log_status(dag_run)[0]


def get_assignment_log_properties(dag_run):
    status, action, details = get_assignment_log_status(dag_run)
    return {
...
def get_email_details_callable(dag_run, timezone):
    ...
    return {
        ...
        "log_file_name": "Log_"+ dag_run.conf['payload']['FileName'] + _now.strftime('%y%m%dT%H%M%S')+".csv"
    }
