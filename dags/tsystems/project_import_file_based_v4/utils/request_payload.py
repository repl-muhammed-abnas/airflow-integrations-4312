...
import rail
from tsystems.project_import_file_based_v4.utils import custom_methods
...
def get_create_client_payload(dag_run):
    """
    Version 1.7: Use formatted client name from extract_client_info
    Format: {code}_{name} if name exists, else Unknown({code})
    """
    return {
        "target": None,
        "modifications": {
            "nameToApply": {
                "value": rail.result('extract_client_info')['client_name']
            },
            "codeToApply": {
...
def put_eligible_project_team_member(dag_run):
    ...
    Version 1.7: Enhanced with Cost Center based assignment restrictions
    """
    ...
    # Version 1.7: Cost Center restriction scope
    assign_from_cost_center_uris = team_deps.get('assign_from_cost_center_uris', [])
    assign_from_employee_type_uris = team_deps.get('assign_from_employee_type_uris', [])

    if assign_from_cost_center_uris and assign_from_employee_type_uris:
        data_access_scopes.append({
            "locations": [],
            "divisions": [],
            "costCenters": [],
            "serviceCenters": [],
            "departmentGroups": [{"uri": uri} for uri in assign_from_cost_center_uris],
    ...
    elif assign_from_cost_center_uris:
        data_access_scopes.append({
            "locations": [],
            "divisions": [],
            "costCenters": [],
    ...
    elif assign_from_employee_type_uris:
        data_access_scopes.append({
            "locations": [],
            "divisions": [],
            "costCenters": [],
