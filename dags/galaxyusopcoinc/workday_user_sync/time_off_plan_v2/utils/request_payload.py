...
def get_create_time_off_type_conf(item, config):
    return {
        ...
        "oef_def_uri": rail.result("get_booking_reference_id_oef_values")[0]['oef_uri'] if rail.result("get_booking_reference_id_oef_values") else None,
        "oef_name": rail.result("get_booking_reference_id_oef_values")[0]['oef_name'] if rail.result("get_booking_reference_id_oef_values") else None,
        "only_admin_can_book_to_uri" : rail.result("get_timeoff_balance_validation_script_uri")['only_admin_can_book_to_uri']
    }
...
def get_minimum_timeoff_increment_policy_uri():
...
def get_put_time_off_type_data(dag_run):
    DEFAULT_URIS = {
        ...
        "time_specification_requirement_uri": "urn:replicon:policy:time-off:start-end-time-specification-requirement:allow-start-end-time-for-partial-days",
        "default_pay_code": f"urn:replicon-tenant:{rail.get_tenant_slug()}:pay-code:3"
    }
    timeoff_measurement_unit_display_format_uri=f"urn:replicon:time-off-measurement-unit:hours"
    return {
        "timeOffType": {
            "target": {
                "uri": null,
            ...
            "minimumTimeOffIncrementPolicyUri": get_minimum_timeoff_increment_policy_uri(),
