...
from galaxyusopcoinc.workday_user_sync.user_schedule_v4.task.run_report_task import run_schedule_base_report
from galaxyusopcoinc.workday_user_sync.user_schedule_v4.utils.python_callable_method import get_work_duration_for_validation
...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        send_bad_file_format_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon User Schedule Sync - Incorrect File Format - {{ current_time_in_specified_tz() }}',
        ...
        def can_decrypt_file_callable():
            return Variable.get(config.can_decrypt_file, default_var='true').lower() == 'true'

        if_decrypt_file = rail.IfOperator(
            task_id='if_decrypt_file',
            test=can_decrypt_file_callable,
        ...
        dummy_load_data = rail.PythonOperator(
            ...
            python_callable= lambda: rail.result('decrypt_file') if can_decrypt_file_callable() else  rail.result('download_file'),
            show_return_value_in_logs= False
        )

        has_decrypted_file = rail.IfOperator(
            task_id='has_decrypted_file',
            test=lambda:rail.result('decrypt_file') if can_decrypt_file_callable() else True,
        ...
        send_blank_payload_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | Replicon User Schedule Sync - Blank File - {{ current_time_in_specified_tz() }}',
        ...
        def get_update_schedule_names():
            schedules = rail.result('get_all_schedule_details_for_update')
            _schedue_data_size = len(schedules)
            if _schedue_data_size == 0:
                return ()
            if _schedue_data_size > 1:
                return tuple(set(map(lambda schedule_data: schedule_data["displayText"], schedules)))
            if _schedue_data_size == 1:
                return f"('{schedules[0]['displayText']}')"

        get_all_the_shcedule_names_for_update = rail.PythonOperator(
            task_id="get_all_the_shcedule_names_for_update",
            python_callable=get_update_schedule_names,
        )

        query_users_with_shcedule_names_for_update = rail.QueryCollectionOperator(
            task_id="query_users_with_shcedule_names_for_update",
            query="""SELECT * FROM final_data_to_process
                     WHERE replicon_schedule_name IN {{result("get_all_the_shcedule_names_for_update")}}"""
        )

        write_update_schedule_log = rail.WriteLogOperator(
            task_id="write_update_schedule_log",
            severity='Exception',
            message='Schedule pattern mismatch',
            items="{{ result('query_users_with_shcedule_names_for_update') }}",
            properties={
                'schedulename': "{{item.replicon_schedule_name}}",
                'employeeid': "{{ item.EmployeeId}}",
                'status': 'Exception',
        ...
        query_changed_records_to_process = rail.QueryCollectionOperator(
            task_id="query_changed_records_to_process",
            query="""SELECT * FROM final_data_to_process
                     WHERE replicon_schedule_name NOT IN {{result("get_all_the_shcedule_names_for_update")}}""",
        )

        if_changed_records_to_process = rail.IfOperator(
            task_id="if_changed_records_to_process",
            test="{{ result('query_changed_records_to_process', 'length') > 0 }}",
            yes_task="dummy_process_user_schedule_records",
            no_task="generate_output_log"
        )  
        ...
        process_each_user_schedule_records = rail.trigger_parallel_dagrun(
            ...
            parallel_count=25,
            items="{{ result('query_changed_records_to_process') }}",
            execution_timeout=timedelta(days=config.execution_timeout_days),
        ...
        if_decrypt_file >> rail.Label("No") >> dummy_load_data
        download_file >> archive_file >> if_decrypt_file >> rail.Label("Yes") >> decrypt_file >> dummy_load_data >> has_decrypted_file >> rail.Label("Yes") >> has_file_content
        ...
        wait_for_process_new_schedules >> get_all_schedule_details_for_update >>\
        get_all_the_shcedule_names_for_update >> query_users_with_shcedule_names_for_update >> write_update_schedule_log>>\
        query_changed_records_to_process >>\
        if_changed_records_to_process >> rail.Label("No") >> generate_output_log
        if_changed_records_to_process >> rail.Label("Yes") >>\
        dummy_process_user_schedule_records >>\
        process_each_user_schedule_records >> generate_output_log
