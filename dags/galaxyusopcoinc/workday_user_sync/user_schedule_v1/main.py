...
from galaxyusopcoinc.workday_user_sync.user_schedule_v1.task.run_report_task import run_schedule_base_report
from galaxyusopcoinc.workday_user_sync.user_schedule_v1.utils.python_callable_method import get_work_duration_for_validation
...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'vialtopartners_user_schedule_master_{config.dag_id_postfix}',
        ...
        process_new_schedules = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'vialtopartners_new_schedule_child_dag_{config.dag_id_postfix}',
            conf={
        ...
        def get_all_schedule_details_for_update_data_handler(response, service_call_input):
            ...
            for item in service_call_input:
                ...
                generate_pattern_by_name = get_pattern_for_schedule(item['displayText'])
        ...
        process_schedule_correction = rail.trigger_parallel_dagrun(
            ...
            execution_timeout=timedelta(days=config.execution_timeout_days),
            trigger_dag_id=f'vialtopartners_schedule_correction_child_dag_{config.dag_id_postfix}',
            conf={
        ...
        process_each_user_schedule_records = rail.trigger_parallel_dagrun(
            ...
            execution_timeout=timedelta(days=config.execution_timeout_days),
            trigger_dag_id=f'vialtopartners_user_schedule_child_dag_{config.dag_id_postfix}',
