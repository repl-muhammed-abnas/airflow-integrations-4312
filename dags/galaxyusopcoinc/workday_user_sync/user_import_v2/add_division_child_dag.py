...
import rail
from galaxyusopcoinc.workday_user_sync.user_import_v2.utils.response_filter import get_all_division_from_replicon_filter


def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.division_dag_id,
        description=f'vialto_partners_user_import_add_division_{config.instance}',
        ...
        get_parent_department_details = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response, dag_run: list(filter(lambda item: item['full_path'] == dag_run.conf['parent_division_full_path'],
                                                               get_all_division_from_replicon_filter(response)))
        )

        create_new_division = rail.RepliconServiceOperator(
        ...
        get_parent_department_details >> create_new_division
