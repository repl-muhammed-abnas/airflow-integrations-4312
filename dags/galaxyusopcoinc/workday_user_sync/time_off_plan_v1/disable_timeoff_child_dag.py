...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        remove_timeoff_from_mapper = rail.RepliconServiceOperator(
            task_id = "remove_timeoff_from_mapper",
            ...
            data= lambda dag_run:
                ...
                "keyValue": {
                        "key": dag_run.conf['country'],
                        "jsonValue": get_updated_timeoff_mapper_callable(dag_run, "remove")
                    }
                }
        )
        ...
        get_country_mapper >> can_update_timeoff_name >> rail.Label("Yes") >> update_timeoff_name >> disable_timeoff_in_replicon
        can_update_timeoff_name >> rail.Label("No") >> disable_timeoff_in_replicon \
            >> remove_timeoff_from_mapper >> log_timeoff_disabled >> rail.Label("On Error") >> catch_and_log_error
