...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        process_enable_time_off_types = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.enable_timeoff_type_dag_id,
        ...
        process_each_country_time_off = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            items="{{result('get_distinct_country_from_created_time_off_type')}}",
            trigger_dag_id=config.update_generic_key_value_mapper_dag_id,
            conf={
                "country": "{{ item.Country }}"
            }
        )
        ...
        wait_for_process_enable_time_off_types >> query_timeoff_to_update >> process_update_timeoff_name >> wait_for_process_timeoff_update\
