...
def get_users_assigned_timeoff_filter(response):
    """
    Filter time off types to determine which need zero line policy.
    Only apply zero line policy to TOIL types that:
    1. Are currently assigned to the user (have a policy)
    2. Contain 'TOIL' in the name (managed by this integration)
    3. Are NOT in the new mapper's time_off_types (employee no longer eligible)

    Also stores existing non-TOIL time off URIs for retention during updates.
    """
    timeoff_list = rail.result("get_timeoff_toassign", "timeoff_names")

    # Get all currently assigned time off types
    all_assigned_timeoffs = list(map(get_timeoff_type_fields, response['policiesByTimeOffType']))
    all_assigned_timeoffs_names = [t['name'] for t in all_assigned_timeoffs if t['timeoff_allowed']]
    def is_managed_toil_type(timeoff):
        """Check if this is a TOIL type managed by the integration"""
        return 'TOIL' in timeoff['name'].upper()

    def is_mapper_managed_type(timeoff):
        """Check if this time off type is managed by the mapper"""
        return timeoff['name'] in timeoff_list or is_managed_toil_type(timeoff)

    def should_apply_zero_line_policy(timeoff):
        """
        Apply zero line policy only to TOIL types that:
        - Have an existing policy
        - Are managed by this integration (contain TOIL)
        - Are not in the new mapper's eligible time off types
        """
        return (bool(timeoff['policy']) and
                is_managed_toil_type(timeoff) and
                timeoff['name'] not in timeoff_list
                and timeoff['timeoff_allowed']) # consider time off allowed flag to avoid unnecessary zero line policy assignment to disabled timeoffs

    rail.set_result(key='timeoff_to_remove_with_zero_line_policy',
                    val=list(filter(should_apply_zero_line_policy, all_assigned_timeoffs)))

    alL_timeoffs_already_assigned = all(map(lambda timeoff:  timeoff in all_assigned_timeoffs_names, timeoff_list))
    rail.set_result(key='all_timeoffs_already_assigned', val=alL_timeoffs_already_assigned)

    # Store existing non-mapper time off URIs to retain during updates
    # These are time offs NOT managed by this integration (non-TOIL and not in mapper)
    existing_non_mapper_timeoff_uris = [
        t['uri'] for t in all_assigned_timeoffs
        if not is_mapper_managed_type(t)
    ]
    rail.set_result(key='existing_non_mapper_timeoff_uris', val=existing_non_mapper_timeoff_uris)
