...
def get_project_with_tasks_payload(dag_run):
    payload = dag_run.conf.get('time_entry_data', {})
...
def build_time_entry_or_punch_payload(dag_run):
    ...
    project_result = rail.result("get_project_with_tasks")
    project_uri = project_result["project_uri"]
    client_uri = project_result["client_uri"]
    task_uri = project_result["task_uri"]
...
def get_project_level_assignee_tags_payload(dag_run):
    project_uri = rail.result("get_project_with_tasks")["project_uri"]
    assignee_name_oef_uri = rail.result("get_oef_definition_types", "assignee_name_oef_uri")
...
def apply_assignee_to_project_payload():
    return {
        "project": {
            "uri": rail.result("get_project_with_tasks")["project_uri"],
            "name": None,
            "code": None,
