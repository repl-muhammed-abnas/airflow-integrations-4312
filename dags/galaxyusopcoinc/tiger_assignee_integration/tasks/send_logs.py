...
def get_send_logs(config):
    with rail.TaskGroup(group_id='send_logs', prefix_group_id=False):
        ...
        render_logs_csv = rail.WriteCSVFileOperator(
            ...
            header=['Project Name', 'Client Name', 'Client Shortname', 'Assignee ID', 'Assignee Status', 'Status', 'Details', 'JobID',
                    '{{ current_time("%d/%m/%YT%H:%M:%S") }}',
                    'Number of Rows:' + '{{- result("get_logged_errors", key="length") +\
                result("get_logged_success", key="length") + result("get_logged_exceptions", key="length") + result("get_logged_skipped", key="length") }}'],
            row=['{{ item.properties | attr_or_default("projectname", "") }}', '{{  item.properties | attr_or_default("clientname", "") }}',
                 '{{ item.properties | attr_or_default("clientshortname", "") }}', '{{ item.properties | attr_or_default("assigneeid", "") }}',
                 '{{ item.properties | attr_or_default("assigneestatus", "") }}', '{{ item.properties | attr_or_default("status", "") }}', '{{ item.message }}', '{{ item.ecid }}'],
        )
