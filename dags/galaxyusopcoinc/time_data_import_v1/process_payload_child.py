...
from galaxyusopcoinc.time_data_import_v1.utils import request_payload, response_filter, custom_methods
...
def _create_single_child_dag(config, postfix="", replicon_conn='replicon_conn_id'):
    ...
    with rail.create_airflow_dag(
        ...
        get_project_details = rail.RepliconServiceOperator(
            ...
            data_handler=response_filter.validate_and_extract_project_details
        )

        project_validation_passed = rail.IfOperator(
            task_id="project_validation_passed",
            test="{{ result('get_project_details')['all_valid'] | is_truthy }}",
            yes_task="needs_client_validation",
            no_task="prepare_log_for_customer"
        )

        needs_client_validation = rail.IfOperator(
            task_id="needs_client_validation",
            test="{{ result('get_project_details')['needs_client_validation'] | is_truthy }}",
            yes_task="get_client_details",
            no_task="get_task_list"
        )
        ...
        get_client_details = rail.RepliconServiceOperator(
            ...
            data_handler=response_filter.validate_client_details
        )

        client_validation_passed = rail.IfOperator(
            task_id="client_validation_passed",
            test="{{ result('get_client_details')['all_valid'] | is_truthy }}",
            yes_task="get_task_list",
            no_task="prepare_log_for_customer"
        )

        get_task_list = rail.RepliconServicePageOperator(
            task_id="get_task_list",
            endpoint="/services/TaskListService1.svc/GetData",
            data=request_payload.get_task_list_payload,
            page_handler=request_payload.page_handler,
            all_result_data_handler=response_filter.validate_and_extract_task_details_paged
        )

        task_validation_passed = rail.IfOperator(
            task_id="task_validation_passed",
            test="{{ result('get_task_list')['all_valid'] | is_truthy }}",
            yes_task="get_task_team",
            no_task="prepare_log_for_customer"
        )

        get_task_team = rail.RepliconServiceOperator(
            task_id="get_task_team",
            endpoint="/services/TaskService1.svc/BulkGetResourceAssignments",
            data=request_payload.get_task_team_payload,
            data_handler=response_filter.validate_user_in_task_team
        )

        team_validation_passed = rail.IfOperator(
            task_id="team_validation_passed",
            test="{{ result('get_task_team')['is_team_member'] | is_truthy }}",
        ...
        can_post_log_to_customer = rail.IfOperator(
            task_id='can_post_log_to_customer',
            test=lambda: Variable.get(
                config.can_post_log_to_customer_var_name, default_var='true').lower() == 'true',
            yes_task='post_log_to_customer',
            no_task='prepare_error_log_for_customer'
        )
        ...
        can_post_error_to_customer = rail.IfOperator(
            task_id='can_post_error_to_customer',
            test=lambda: Variable.get(
                config.can_post_log_to_customer_var_name, default_var='true').lower() == 'true',
            yes_task='post_error_to_customer',
            no_task='fail_dag'
        )
        ...
        timesheet_validation_passed >> rail.Label("Yes") >> get_project_details >> project_validation_passed
        timesheet_validation_passed >> rail.Label("No") >> prepare_log_for_customer

        project_validation_passed >> rail.Label("Yes") >> needs_client_validation
        project_validation_passed >> rail.Label("No") >> prepare_log_for_customer

        needs_client_validation >> rail.Label("Yes") >> get_client_details >> client_validation_passed
        needs_client_validation >> rail.Label("No") >> get_task_list >> task_validation_passed

        client_validation_passed >> rail.Label("Yes") >> get_task_list
        client_validation_passed >> rail.Label("No") >> prepare_log_for_customer

        task_validation_passed >> rail.Label("Yes") >> get_task_team >> team_validation_passed
        task_validation_passed >> rail.Label("No") >> prepare_log_for_customer

        team_validation_passed >> rail.Label("Yes") >> get_oef_definition_types >> has_skipped_oefs
        team_validation_passed >> rail.Label("No") >> prepare_log_for_customer
        ...
        prepare_log_for_customer >> can_post_log_to_customer
        can_post_log_to_customer >> rail.Label("Yes") >> post_log_to_customer >> prepare_error_log_for_customer
        can_post_log_to_customer >> rail.Label("No") >> prepare_error_log_for_customer

        prepare_error_log_for_customer >> can_post_error_to_customer
        can_post_error_to_customer >> rail.Label("Yes") >> post_error_to_customer >> fail_dag
        can_post_error_to_customer >> rail.Label("No") >> fail_dag
