...
def get_send_logs(config):
    with rail.TaskGroup(group_id='send_logs', prefix_group_id=False):
        ...
        render_logs_csv = rail.WriteCSVFileOperator(
            ...
            header=['EmployeeID', 'ReferenceID', 'TimeOffEntryID', 'Status', 'Details', 'JobID', '{{ current_time("%d/%m/%YT%H:%M:%S") }}', 'Number of Rows:' + '{{- result("get_logged_errors", key="length") + result("get_logged_success", key="length") + result("get_logged_exceptions", key="length") }}',
                    'Function: LOA & RFL Bookings Import', '', '', ''],
            row=['{{ item.properties | attr_or_default("employeeid", "") }}', '{{  item.properties | attr_or_default("referenceid", "") }}', '{{  item.properties | attr_or_default("timeoffentryid", "") }}',
