...
def get_send_logs(config):
    with rail.TaskGroup(group_id='send_logs', prefix_group_id=False):
        ...
        render_logs_csv = rail.WriteCSVFileOperator(
            ...
            header=['BatchID', 'EmployeeID', 'ReferenceID', 'Status', 'Details', 'JobID', '{{ current_time("%d/%m/%YT%H:%M:%S") }}', 'Number of Rows:' + '{{- result("get_logged_errors", key="length") + result("get_logged_success", key="length") + result("get_logged_exceptions", key="length") + result("get_logged_skipped", key="length") }}',
                    'Function: Vialto Partners TO Balance Import', '', '', ''],
            row=['{{ item.properties | attr_or_default("batchid", "") }}', '{{  item.properties | attr_or_default("employeeid", "") }}',
                 '{{ item.properties | attr_or_default("referenceid", "") }}', '{{ item.properties | attr_or_default("status", "") }}', '{{ item.message }}', '{{ item.ecid }}'],
            footer=['Number of Records Errored: {{ result("get_logged_errors", "length") }}',
                    ...
                    'Number of Records Skipped: {{ result("get_logged_skipped", "length") }}', '', ''],
        )
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('get_logged_errors', 'length') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Replicon TimeOff balance import - " }} \
