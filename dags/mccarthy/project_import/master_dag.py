...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'mccarthy_project_import_in_replicon_master_{config.instance}',
        description=f'Mccarthy - Creating the project in Replicon Master {config.instance}',
        ...
        query_existing_projects=rail.QueryCollectionOperator(
            task_id='query_existing_projects',
            query="""Select  inputfile.RegionName, inputfile.ProjectName, inputfile.ProjectCode, inputfile.ProjectDescription, inputfile.ProjectStartDate,
                    inputfile.ProjectEndDate, inputfile.TaskName, inputfile.TaskCode, inputfile.TaskStartDate, inputfile.TaskEndDate,
                    projectstatus.projecturi, projectstatus.Projectstatus from  projectstatus,
        ...
        process_projects_with_blank_name = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f"mccarthy_project_import_process_new_projects_child_dag_{config.instance}",
        ...
        trigger_child_to_update_project=rail.TriggerDagRunForEachItemOperator(
            ...
            retries = 0,
            items= "{{result('query_existing_projects')}}",
            trigger_dag_id=f'mccarthy_project_import_update_projects_in_replicon_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
        ...
        compose_csv_logs=rail.WriteCSVFileOperator(
            ...
            source="{{ result('search_all_entries')}}",
            header=['Project Name',
                'Project Code',
                'Task Name',
                'Task Code',
        ...
        run_all_project_report >> parse_csv >> create_collection_from_report >> query_new_projects >> query_existing_projects
        query_existing_projects >> get_project_import_logs_lookup_table >> query_distinct_new_projects >> query_projects_with_name_blank
