...
def report_batch(config):
    with rail.TaskGroup(group_id='generate_report_batch', prefix_group_id=False):
        ...
        expected_report_columns = 'Login Name,User Status,UserURI,Time Off Type,TimeOffTypeURI,Time Off Balance'
        ...
        check_report_column_order = rail.IfOperator(
            ...
            test='{{ result("run_timeoff_balance_report.get_report_result").reportGenerationResults[0].payload | starts_with("%s") }}' % expected_report_columns,
    ...
    return get_report_details, create_collection_from_report_data, fail_no_report_data, fail_column_order_mismatch
