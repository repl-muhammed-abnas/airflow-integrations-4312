...
def get_send_logs(config):
    with rail.TaskGroup(group_id='send_logs', prefix_group_id=False):
        ...
        render_logs_csv = rail.WriteCSVFileOperator(
            ...
            header=['User Name', 'Permission Name', 'Status', 'Details', 'ECID'],
            row=['{{ item.properties | attr_or_default("username", "") }}', '{{  item.properties | attr_or_default("permissionname", "") }}',
                 '{{ item.properties | attr_or_default("status", "") }}', '{{ item.properties | attr_or_default("details", "") }}',
                 '{{ item.ecid }}'],
        )
        ...
        generate_download_link = rail.GeneratePresignedDownloadUrlOperator(
            ...
            expires_in_seconds=config.download_link_expiry,
        )
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc = "{%- if result('get_logged_errors', 'length') > 0  -%}\
                ...
                {%- endif -%}",
            subject='{{ get_company_key() }} | Permission Change Audit - {{" "}} \
