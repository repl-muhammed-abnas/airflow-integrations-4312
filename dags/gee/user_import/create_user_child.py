...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.create_user_child,
        description=f'GEE create user child {config.instance}',
        ...
        if_employeeid_present = rail.IfOperator(
            ...
            yes_task='if_loginname_present',
            no_task='add_to_lookup_table'
        )

        if_loginname_present = rail.IfOperator(
            task_id='if_loginname_present',
            test=lambda dag_run: bool(dag_run.conf['LoginName']),
            yes_task='trigger_user_supervisor_dag',
            no_task='add_to_lookup_table'
        )

        trigger_user_supervisor_dag = rail.TriggerDagRunOperator(
            task_id='trigger_user_supervisor_dag',
            trigger_dag_id=config.create_supervisor_child,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=request_payload.trigger_user_supervisor_dag
        )

        wait_for_trigger_user_supervisor_dag = rail.WaitForDagRunsSensor(
            task_id='wait_for_trigger_user_supervisor_dag',
            dag_runs='{{ result("trigger_user_supervisor_dag") }}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
        )
        ...
        get_user_details >> split_start_date >> create_user_in_replicon >> get_all_products_available_for_user_assignment >> \
        ...
        if_workweekhours_and_workweekdropdownuri_present >> rail.Label(
            ...
            "Yes") >> get_user_details_with_supervisorid_63 >> if_get_user_details_with_supervisorid_63_present >> rail.Label(
        ...
        if_supervisor_permissionsets_present >> rail.Label(
            ...
            "Yes") >> get_all_permissionsets_71 >> add_supervisor_permission >> put_supervisors_assigment_schedule_74 >> if_employeeid_present
        ...
        if_get_user_details_with_supervisorid_63_present >> rail.Label(
            "No") >> if_employeeid_present >> rail.Label(
            "Yes") >> if_loginname_present >> rail.Label(
            "Yes") >> trigger_user_supervisor_dag >> wait_for_trigger_user_supervisor_dag >> add_to_lookup_table
        if_loginname_present >> rail.Label(
            "No") >> add_to_lookup_table
