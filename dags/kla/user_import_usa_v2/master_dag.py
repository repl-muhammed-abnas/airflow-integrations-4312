...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'kla_user_import_usa_master_v2_{config.instance}',
        ...
        process_cost_center_department_check = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=['process'],
            trigger_dag_id=f'kla_user_import_usa_cost_center_department_check_v2_{config.instance}',
        ...
        process_users = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'kla_user_import_usa_process_users_v2_{config.instance}',
            conf=lambda item: {
        ...
        process_supervisor_assignment = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'kla_user_import_usa_supervisor_assignment_v2_{config.instance}',
            execution_timeout=timedelta(days=14),
            conf=lambda item: {
        ...
        def do_format_logs():
            logs = list(list(itertools.chain(
                *list(map(rail.load_all_records, rail.result('gather_child_logs') + [rail.result(create_log.task_id)])))))
            users = [(k, list(g)) for k, g in itertools.groupby(
                logs, lambda x:x['properties']['loginname'])]
            return list(map(lambda item: {
                "ecid": item[1][-1]['ecid'],
                "properties": {
                    "loginname": item[1][-1]['properties']['loginname'],
                    "action": item[1][-1]['properties']['action'],
                    "status": item[1][-1]['properties']['status'],
                    "details": item[1][-1]['properties']['message'],
                }
            }, users))
        ...
        upload_logfile_s3 = rail.S3UploadFileOperator(
            ...
            key_name=config.s3_key_name +
            '/{{ result("log_message_filenamefullpath") }}',
