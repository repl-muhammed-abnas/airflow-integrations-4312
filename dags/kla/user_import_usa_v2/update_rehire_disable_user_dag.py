...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'kla_user_import_usa_update_rehire_disable_user_v2_{config.instance}',
        description=f'KLATencor Update / Rehire / Disable User v2 {config.instance}',
        ...
        can_update_wfn_id_udf = rail.IfOperator(
            task_id='can_update_wfn_id_udf',
            test="{{ dag_run.conf.wfn_id | is_truthy and dag_run.conf.wfn_id != result('parse_csv_user_data').get('WFN ID') and result('get_custom_fielduri') | find_first_by_attr_and_get_attr('displayText', 'WFN ID', 'uri') | is_truthy }}",
        ...
        update_wfn_id_udf = rail.RepliconServiceOperator(
            ...
            data={
                "objectUri": "{{ dag_run.conf.useruri }}",
                "customFieldUri": "{{ result('get_custom_fielduri') | find_first_by_attr_and_get_attr('displayText', 'WFN ID', 'uri') }}",
                "value": "{{ dag_run.conf.wfn_id }}"
            }
        )
        ...
        getlocation_data_final = rail.RepliconServiceOperator(
            ...
            data_handler=lambda data: next(iter(filter(lambda x: x['code'] == rail.result('log_message_final_locationvaluenew'),
                                                       map(lambda x: {
                                                           "uri": x['cells'][0].get('uri'),
        ...
        process_update_user_timeoff = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[1],
            trigger_dag_id=f'kla_user_import_usa_update_user_timeoff_v2_{config.instance}',
        ...
        has_firstdayofleave >> rail.Label(
            'No') >> can_update_wfn_id_udf

        can_update_first_dayleave_udf >> rail.Label(
            'No') >> can_update_wfn_id_udf
        can_update_first_dayleave_udf >> rail.Label(
            'Yes') >> update_first_dayof_leave_udf >> can_update_wfn_id_udf

        can_update_wfn_id_udf >> rail.Label(
            'No') >> has_change_in_lasthiredate2
        can_update_wfn_id_udf >> rail.Label(
            'Yes') >> update_wfn_id_udf >> has_change_in_lasthiredate2
        ...
        has_pdr_country_value >> rail.Label(
            'Yes') >> add_success_log >> has_empl_status_terminated
