...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'kla_user_import_usa_process_users_{config.instance}',
        description=f'KLATencor User Import USA process Users {config.instance}',
        ...
        process_update_user = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[1],
            trigger_dag_id=f'kla_user_import_usa_update_rehire_disable_user_{config.instance}',
        ...
        process_add_user = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[1],
            trigger_dag_id=f'kla_user_import_usa_add_user_{config.instance}',
            execution_timeout=timedelta(days=14),
            conf={
                "log": "{{ result('create_log') }}",
                "firstname": "{{ dag_run.conf.firstname }}",
