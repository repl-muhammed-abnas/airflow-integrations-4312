...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'ge_netherlands_child_add_foreign_supervisor_v1_0_{config.instance}',
        description=f'GE_netherlands_Child_Add Foreign Supervisor V1.0 {config.instance}',
        ...
        netherlands_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9 = rail.PythonOperator(
            task_id='netherlands_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9',
            python_callable=lambda: list(
                filter(lambda x: x['legal_entity'] == 'Foreign Supervisors', netherlands_master_mapper))
        )

        def get_entity_from_mapper(entity, typeval):
            emp_types = list(filter(
                lambda x: x['type'] == entity
                and x['legacy_payroll_id'] == typeval, netherlands_master_mapper))
        ...
        def get_mapper_licenses(entity, typeval):
            employee_licenses = list(filter(
                ...
                and x['type'] == typeval, netherlands_master_mapper))
        ...
        def get_mapper_permissions(entity, mapper_type, identifier_1):
            employee_permissions = list(filter(
                lambda x: x['type'] == entity
                and x['legacy_payroll_id'] == mapper_type
        ...
        if_log_getsupervisor_uri_4_blank_5 >> rail.Label(
            ...
            netherlands_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9 >> \
