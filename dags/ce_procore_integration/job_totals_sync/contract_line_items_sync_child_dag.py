...
def create_dag_instance(config): # pylint: disable=too-many-statements
    with rail.create_airflow_dag(
        ...
        fetch_procore_project = rail.ProcoreApiOperator(
            ...
            data_handler=lambda projects, dag_run: rail.find_first_by_attr_and_get_attr(
                projects, 'origin_id', f'CE_{dag_run.conf["job_code"]}', 'id', None)
        )
        ...
        def parse_contract_data_for_sync(dag_run):
            ...
            contract_line_items = list(
                filter(lambda item: item.get('level') == contract_by, line_items))
            ...
            for item in contract_line_items:
                if contract_by == ContractLevel.CATEGORY:
                    ...
                    if float(contract_amount) == 0:
                        continue
                    flat_code = f"{(phase_code + '-') if phase_code else ''}{category_code}.{config.revenue_cost_type}"
                    parsed_data.append({
                        'job_code': job_code,
                    ...
                    if float(contract_amount) == 0:
                        continue
                    flat_code = f"{phase_code}.{config.revenue_cost_type}"
                    parsed_data.append({
                        'job_code': job_code,
                        'phase_code': phase_code,
                        'contract_amount': contract_amount,
        ...
        fetch_existing_wbs_codes = rail.ProcoreApiOperator(
            ...
            endpoint=lambda dag_run: f'/projects/{rail.result("fetch_procore_project")}/work_breakdown_structure/wbs_codes',
            method='GET',
            query_params=lambda dag_run: {
                'company_id': dag_run.conf['procore_company_id']
            }
        ...
        if_wbs_codes_missing = rail.IfOperator(
            ...
            no_task='identify_line_item_actions'
        )
        ...
        def get_line_item_actions():
            parsed_contract_data = rail.result('parse_contract_data')
            existing_line_items = rail.result('fetch_existing_line_items')
            wbs_codes_lookup = rail.result('identify_missing_wbs_codes')[
                'existing_flat_codes_lookup'].copy()

            # Update WBS codes lookup with creation results
            try:
                created_wbs_results = rail.result(
                    'gather_wbs_creation_results')
        ...
        def get_sync_error_message(error_object):
            try:
                existing_wbs_codes = rail.result('fetch_existing_wbs_codes')
                wbs_codes_reverse_map = {wbs.get('id'): wbs.get(
                    'flat_code') for wbs in existing_wbs_codes if wbs.get('id') and wbs.get('flat_code')}
        ...
        log_sync_failure = rail.WriteLogOperator(
            ...
            items=lambda dag_run: [
                {
                    'entity_code': dag_run.conf.get('job_code', ''),
                    'error_message': get_sync_error_message(err),
                    'sync_type': SyncType.CONTRACT,
                    'reset_retry_count': dag_run.conf.get('reset_retry_count', False)
                }
        ...
        check_prime_contract_exists >> rail.Label(
            'Yes') >> fetch_job_details >> parse_contract_data >> fetch_existing_line_items >> fetch_existing_wbs_codes >> identify_missing_wbs_codes >> if_wbs_codes_missing
        check_prime_contract_exists >> rail.Label(
            'No') >> log_prime_contract_not_found >> catch_error

        if_wbs_codes_missing >> rail.Label(
            'Yes') >> trigger_wbs_code_creation >> wait_for_wbs_code_creation >> gather_wbs_creation_results >> identify_line_item_actions >> if_wbs_creation_errors
        if_wbs_codes_missing >> rail.Label(
            'No') >> identify_line_item_actions >> if_wbs_creation_errors
