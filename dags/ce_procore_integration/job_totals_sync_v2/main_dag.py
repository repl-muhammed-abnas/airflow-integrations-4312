...
def create_dag_instance(config):
    ...
    with rail.create_airflow_dag(
        ...
        def build_direct_cost_flat_code(phase_code, category_code, costtype_reference):
            phase_code = (phase_code or '').strip()
            category_code = (category_code or '').strip()
            costtype_reference = (costtype_reference or '').strip()
            if phase_code and category_code:
                return f"{phase_code}-{category_code}.{costtype_reference}"
            return f"{phase_code or category_code}.{costtype_reference}"
                ...
                for category in phase.get('categories', []):
                    ...
                    for costtype in category.get('costtypes', []):
                        ...
                        items.append({
                            ...
                            'flat_code': build_direct_cost_flat_code(
                                phase_code, category_code, costtype_reference),
                            'amount': str(round(direct_cost, 2))
                        })
            return items

        def build_budget_flat_code(phase_code, category_code, costtype_reference):
            phase_code = (phase_code or '').strip()
            category_code = (category_code or '').strip()
            costtype_reference = (costtype_reference or '').strip()
            if phase_code:
                return f"{phase_code}-{category_code}.{costtype_reference}"
            return f"{category_code}.{costtype_reference}"
                ...
                for category in phase.get('categories', []):
                    ...
                    for costtype in category.get('costtypes', []):
                        ...
                        budget_line_items.append({
                            ...
                            'costtype_code': costtype_code,
                            'costtype_reference': costtype_reference,
                            'flat_code': build_budget_flat_code(
                                phase_code, category_code, costtype_reference),
        ...
        def build_contract_flat_code(phase_code, category_code, level):
            phase_code = (phase_code or '').strip()
            category_code = (category_code or '').strip()
            if level == ContractLevel.CATEGORY:
                if phase_code:
                    return f"{phase_code}-{category_code}.{config.revenue_cost_type}"
                return f"{category_code}.{config.revenue_cost_type}"
            return f"{phase_code}.{config.revenue_cost_type}"
            ...
            for phase in job.get('phases', []):
                ...
                if float(phase_contract_amount) != 0:
                    contract_line_items.append({
                        ...
                        'costtype_reference': config.revenue_cost_type,
                        'flat_code': build_contract_flat_code(
                            phase_code, '', ContractLevel.PHASE)
                    })
                ...
                for category in phase.get('categories', []):
                    ...
                    if float(category_contract_amount) != 0:
                        contract_line_items.append({
                            ...
                            'costtype_reference': config.revenue_cost_type,
                            'flat_code': build_contract_flat_code(
                                phase_code, category_code, ContractLevel.CATEGORY)
                        })
        ...
        def parse_required_data():
            ...
            for job in jobs_data:
                ...
                budget_line_items = parse_budget_data(
                    job, ce_cost_type_mapper) if config.sync_budget else []
                contract_line_items = parse_contract_data(
                    job) if config.sync_contract else []
                direct_cost_line_items = parse_direct_cost_data(
                    job, ce_cost_type_mapper) if config.sync_direct_cost else []
        ...
        def extract_failure_count(fingerprint):
            if '_FAILED_' in fingerprint:
                try:
                    return int(fingerprint.split('_FAILED_')[1].split('_RETRYAFTER_')[0])
                    ...
                    if previous_fp and current_fp != previous_fp:
                        ...
                        if '_FAILED_' in previous_fp:
                            if data_changed_or_retry_requested:
                                ...
                                failure_count = extract_failure_count(previous_fp)
                                # Stop retrying once every configured delay has been used.
                                # Retries beyond len(retry_delays_hours) previously reused the
                                # last delay (168h) forever. A data change / force-retry resets
                                # the count and resumes retries.
                                if failure_count is not None and failure_count > len(config.retry_delays_hours):
                                    continue
        ...
        def get_all_delta_jobs(dag_run):
            # Second enforcement point for the per-domain toggles (see parse_required_data).
            # A disabled domain produces no delta at all, so it never enters the sync path.
            # This is essential for budget: without it, get_delta_jobs_by_type runs
            # _add_removed_budget_items, which re-emits vanished items as 0.00 and would
            # zero out the already-synced budget in Procore.
            budget_deltas = get_delta_jobs_by_type(
                SyncType.BUDGET, 'fingerprint', dag_run) if config.sync_budget else []
            contract_deltas = get_delta_jobs_by_type(
                SyncType.CONTRACT, 'contract_fingerprint', dag_run) if config.sync_contract else []
            direct_cost_deltas = get_delta_jobs_by_type(
                SyncType.DIRECT_COST, 'direct_cost_fingerprint', dag_run) if config.sync_direct_cost else []
        ...
        fetch_all_procore_projects = rail.ProcoreApiOperator(
            ...
            data_handler=lambda projects: {
                project.get('origin_id'): project.get('id')
                for project in (projects or [])
                if project.get('origin_id', '') and project.get('origin_id', '').startswith('CE_') and project.get('id')
            }
        )

        # The REVENUE WBS segment item is consumed only by contract sync
        # (parse_contract_data / contract_line_items_sync_child_dag). Budget and
        # direct-cost use the CE cost-type mapper and never touch it, so only create
        # or update it when contract sync is enabled.
        if_revenue_cost_type_required = rail.IfOperator(
            task_id='if_revenue_cost_type_required',
            test=lambda: config.sync_contract,
            yes_task='check_if_revenue_cost_type_exists',
            no_task='group_data_by_job'
        )
        ...
        def group_delta_data_by_job():
            delta_jobs = rail.result('get_delta_jobs')
            budget_by_job = {j['job_code']: j for j in delta_jobs['budget_delta_jobs']}
            contract_by_job = {j['job_code']: j for j in delta_jobs['contract_delta_jobs']}
            direct_cost_by_job = {j['job_code']: j for j in delta_jobs['direct_cost_delta_jobs']}

            affected_job_codes = set(budget_by_job) | set(contract_by_job) | set(direct_cost_by_job)

            grouped_jobs = []
            for job_code in affected_job_codes:
                job_entry = {'job_code': job_code}
                budget = budget_by_job.get(job_code)
                if budget and budget.get('budget_line_items'):
                    job_entry['budget_data'] = {
                        'line_items': budget['budget_line_items'],
                        'reset_retry_count': budget.get('reset_retry_count', False)
                    }
                contract = contract_by_job.get(job_code)
                if contract and contract.get('contract_line_items'):
                    job_entry['contract_data'] = {
                        'line_items': contract['contract_line_items'],
                        'reset_retry_count': contract.get('reset_retry_count', False)
                    }
                direct_cost = direct_cost_by_job.get(job_code)
                if direct_cost and direct_cost.get('direct_cost_line_items'):
                    job_entry['direct_cost_data'] = {
                        'line_items': direct_cost['direct_cost_line_items'],
                        'fingerprint': direct_cost.get('direct_cost_fingerprint', ''),
                        'reset_retry_count': direct_cost.get('reset_retry_count', False)
                    }
                if 'budget_data' in job_entry or 'contract_data' in job_entry or 'direct_cost_data' in job_entry:
                    grouped_jobs.append(job_entry)
            return grouped_jobs

        group_data_by_job = rail.PythonOperator(
            task_id='group_data_by_job',
            python_callable=group_delta_data_by_job
        )

        trigger_per_job_dag = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_per_job_dag',
            items=lambda: rail.result('group_data_by_job'),
            trigger_dag_id=config.job_totals_per_job_child_dag_id,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: {
                **item,
                'procore_company_id': rail.render_template(procore_company_id_template),
                'procore_project_id': rail.result('fetch_all_procore_projects').get(f"CE_{item['job_code']}"),
        ...
        wait_for_per_job_completion = rail.WaitForDagRunsSensor(
            task_id='wait_for_per_job_completion',
            dag_runs='{{ result("trigger_per_job_dag") }}',
            execution_timeout=timedelta(days=config.execution_timeout_days)
        )
        ...
        def filter_notification_logs():
            # Notify on:
            #  - first-time failures (count == 1)
            #  - give-up (count > len(retry_delays_hours)): all configured retries exhausted;
            #    the job is abandoned until its data changes. A marker is appended to the reason.
            # Middle retries (2..len) stay suppressed to avoid re-notifying while retries continue.
            if rail.result('search_logs', 'length') == 0:
                return []
            error_logs = rail.load_all_records(rail.result('search_logs'))
            adjusted_by_job = {fp['job_code']: fp for fp in rail.result('adjust_fingerprints')}
            fingerprint_key_map = {
                SyncType.BUDGET: 'fingerprint',
                SyncType.CONTRACT: 'contract_fingerprint',
                SyncType.DIRECT_COST: 'direct_cost_fingerprint'
            ...
            for log_entry in error_logs:
                job_code = log_entry.get('properties', {}).get('entity_code')
                sync_type = log_entry.get('properties', {}).get('sync_type') or SyncType.BUDGET
                fp = adjusted_by_job.get(job_code, {})
                count = extract_failure_count(fp.get(fingerprint_key_map.get(sync_type, 'fingerprint'), ''))
                if not count:
                    continue
                if count > max_retries:
                    properties = log_entry.setdefault('properties', {})
                    properties['error_message'] = (properties.get('error_message', '') or '') + ' [RETRIES EXHAUSTED - SYNC STOPPED]'
                    notification_logs.append(log_entry)
                elif count == 1:
                    notification_logs.append(log_entry)
            return notification_logs

        filter_notification_logs_task = rail.PythonOperator(
            task_id='filter_notification_logs',
            python_callable=filter_notification_logs
        )
        ...
        if_logs_present = rail.IfOperator(
            ...
            test=lambda: len(rail.result("filter_notification_logs")) > 0,
        ...
        write_logs_into_csv = rail.WriteCSVFileOperator(
            ...
            source="{{ result('filter_notification_logs') | to_json }}",
        ...
        check_if_delta_jobs_found >> rail.Label('Yes') >> fetch_wbs_segments >> get_cost_code_segment_id >> get_cost_type_segment_id >> fetch_cost_type_segment_items >> fetch_all_procore_projects >> if_revenue_cost_type_required

        if_revenue_cost_type_required >> rail.Label('Yes') >> check_if_revenue_cost_type_exists
        if_revenue_cost_type_required >> rail.Label('No') >> group_data_by_job

        check_if_revenue_cost_type_exists >> rail.Label('Yes') >> update_revenue_cost_type >> group_data_by_job
        check_if_revenue_cost_type_exists >> rail.Label('No') >> create_revenue_cost_type >> group_data_by_job

        group_data_by_job >> trigger_per_job_dag >> wait_for_per_job_completion >> search_logs >> adjust_fingerprints >> write_current_fingerprints_csv
        write_current_fingerprints_csv >> upload_current_fingerprints_to_s3 >> filter_notification_logs_task >> if_logs_present
