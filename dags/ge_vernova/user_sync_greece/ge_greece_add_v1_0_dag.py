...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'ge_greece_add_v1_0_{config.instance}',
        description=f'GE_Greece Add V1.0 {config.instance}',
        ...
        if_log_20_present_conditiontocheckifrequiredfieldsexistin_inputfile_5 = rail.IfOperator(
            ...
            no_task="ge_greece_user_sync_master_mapper_search_entries_8",
        )
        ...
        ge_greece_user_sync_master_mapper_search_entries_8 = rail.PythonOperator(
            task_id='ge_greece_user_sync_master_mapper_search_entries_8',
        ...
        if_first_id_blank_9 = rail.IfOperator(
            ...
            test='''{{ result('ge_greece_user_sync_master_mapper_search_entries_8') | is_falsy }}''',
        ...
        def get_employe_type_from_mapper(dag_run):
            emp_types = list(filter(
                lambda x: x['legal_entity'] == dag_run.conf['LegalEntity'] and x['type'] == "Employee Type", greece_master_mapper))
        ...
        def get_entity_from_mapper(dag_run, entity_type):
            emp_types = list(filter(
                lambda x: x['legal_entity'] == dag_run.conf['LegalEntity'] and x['type'] == entity_type, greece_master_mapper))
            return emp_types[0]['value'] if emp_types else None
        ...
        log_required_timesheet_approval_path_name_22 = rail.PythonOperator(
            task_id='log_required_timesheet_approval_path_name_22',
            python_callable=lambda dag_run: get_entity_from_mapper(
        ...
        def get_value_mapper(dag_run, entity_type1):
            emp_types = list(filter(
                lambda x: x['legal_entity'] == dag_run.conf['LegalEntity']
                and x['type'] == entity_type1, greece_master_mapper))
            return emp_types[0]['value'] if emp_types else None

        log_required_mapper_payrule_name_23 = rail.PythonOperator(
            task_id='log_required_mapper_payrule_name_23',
            python_callable=lambda dag_run: get_value_mapper(
                dag_run, "Payrule")
        )
            ...
            data=lambda dag_run: {
                "user": {
                    ...
                    "timesheetApprovalPath": {
                        "uri": null,
                        "name": rail.result('log_required_timesheet_approval_path_name_22')
                    },
                    ...
                    "payRuleScriptSchedule": [
                        {
                            "payRuleScript": {
                                "uri": null,
                                "name": rail.result('log_required_mapper_payrule_name_23')
                            },
                            "effectiveDate": null
                        }
                    ]
        ...
        def get_timesheet_template(dag_run):
            ...
            identifier_1 = "All" if legalentity == 'AL7931' else specialworkschedule
            ts_template = get_value_from_mapper(
                dag_run, 'Timesheet Template', identifier_1)
            return ts_template if ts_template else "GR_InOut - 9AM"

        log_required_timesheet_template_38 = rail.PythonOperator(
            task_id='log_required_timesheet_template_38',
            python_callable=get_timesheet_template
        )

        if_log_required_timesheet_template_38_blank_39 = rail.IfOperator(
            task_id='if_log_required_timesheet_template_38_blank_39',
            test='''{{ result('log_required_timesheet_template_38') | is_falsy }}''',
            yes_task="insert_to_list_40",
            no_task="_adhoc_http_action_42",
        )
        ...
        def get_tstemplate_uri():
            current_template = list(filter(lambda x: x['name'] and x['name'].lower() == rail.result(
                'log_required_timesheet_template_38').lower(), rail.result('_adhoc_http_action_42')))
            return current_template[0]['uri'] if current_template else None

        log_required_timesheet_template_uri_43 = rail.PythonOperator(
            task_id='log_required_timesheet_template_uri_43',
            python_callable=get_tstemplate_uri
        )

        if_log_required_timesheet_template_uri_43_present_44 = rail.IfOperator(
            task_id='if_log_required_timesheet_template_uri_43_present_44',
            test='''{{ result('log_required_timesheet_template_uri_43') | is_truthy }}''',
            yes_task="assign_policy_set_to_user_45",
            no_task="insert_to_list_47",
        )

        assign_policy_set_to_user_45 = rail.RepliconServiceOperator(
            task_id='assign_policy_set_to_user_45',
            endpoint="/services/PolicySetService1.svc/AssignPolicySetToUser",
            data={
                "userUri": "{{ result('create_user_32').uri }}",
                "policySetUri": "{{ result('log_required_timesheet_template_uri_43') }}"
            }
        )
        ...
        insert_to_list_47 = rail.SetVariableOperator(
            ...
            name='{{ result("declare_list_3").name }}',
            value={
                "log": '''Timesheet template "{{result('log_required_timesheet_template_38')}}" not available in Replicon'''
            }
        )
        ...
        if_request_industryfocusgroup_present_102 = rail.IfOperator(
            ...
            no_task="trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109",
        )
        ...
        trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109 = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109',
            retries=0,
            items=[1],
            trigger_dag_id=f'ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0_{config.instance}',
            execution_timeout=timedelta(days=14),
        ...
        wait_for_completion_trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109 = rail.WaitForDagRunsSensor(
            task_id='wait_for_completion_trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109',
            execution_timeout=timedelta(days=14),
            dag_runs='{{ result("trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109") }}'
        )

        if_reply_output_present_110 = rail.IfOperator(
            # Todo
            task_id='if_reply_output_present_110',
            test='''{{ result('trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109') | is_truthy }}''',
        ...
        insert_to_list_111 = rail.SetVariableOperator(
            ...
            name='{{ result("declare_list_3").name }}',
            value={
                "log": "{{ result('trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109') }}"
            }
        )
        ...
        ge_greece_user_sync_master_mapper_search_entries_8
        if_log_20_present_conditiontocheckifrequiredfieldsexistin_inputfile_5 >> rail.Label(
            'No') >> ge_greece_user_sync_master_mapper_search_entries_8 >> if_first_id_blank_9
        ...
        if_request_departmenturi_blank_16 >> rail.Label('No') >> log_required_timesheet_period_name_19 >> \
            log_required_time_off_template_name_20 >> log_required_time_off_approval_path_name_21 >> log_required_timesheet_approval_path_name_22 >> \
            log_required_mapper_payrule_name_23 >> log_required_holiday_calendar_name_24 >> log_required_workweek_name_25 >> \
            ...
            log_required_timesheet_template_38 >> if_log_required_timesheet_template_38_blank_39
        if_log_required_timesheet_template_38_blank_39 >> rail.Label(
            'Yes') >> insert_to_list_40 >> if_request_supervisorssoid_present_48
        if_log_required_timesheet_template_38_blank_39 >> rail.Label(
            'No') >> _adhoc_http_action_42 >> log_required_timesheet_template_uri_43 >> if_log_required_timesheet_template_uri_43_present_44
        if_log_required_timesheet_template_uri_43_present_44 >> rail.Label(
            'Yes') >> assign_policy_set_to_user_45 >> if_request_supervisorssoid_present_48
        if_log_required_timesheet_template_uri_43_present_44 >> rail.Label(
            'No') >> insert_to_list_47 >> if_request_supervisorssoid_present_48
        ...
        if_log_gettherequired_industry_focus_groupvalue_uri_104_present_105 >> rail.Label(
            ...
            trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109
        if_log_gettherequired_industry_focus_groupvalue_uri_104_present_105 >> rail.Label(
            'No') >> insert_to_list_108 >> trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109
        if_request_industryfocusgroup_present_102 >> rail.Label(
            'No') >> trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109 >> \
            wait_for_completion_trigger_dag_run_live_ge_greece_child_workflow_to_add_timeoff_type_for_new_user_v1_0109 >> if_reply_output_present_110
