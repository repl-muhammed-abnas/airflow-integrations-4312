...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'ge_greece_child_add_foreign_supervisor_v1_0_{config.instance}',
        description=f'GE_Greece_Child_Add Foreign Supervisor V1.0 {config.instance}',
        ...
        greece_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9 = rail.PythonOperator(
            task_id='greece_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9',
            python_callable=lambda: list(
                filter(lambda x: x['legal_entity'] == 'Foreign Supervisors', greece_master_mapper))
        )

        def get_entity_from_mapper(entity, typeval):
            emp_types = list(filter(
                lambda x: x['legal_entity'] == entity
                and x['type'] == typeval, greece_master_mapper))
        ...
        def get_mapper_permissions(entity, mapper_type, identifier_1):
            employee_permissions = list(filter(
                ...
                and x['identifier__1__(_legal_entity_code/_type/_timeoff_type)'] == identifier_1, greece_master_mapper))
        ...
        if_log_getsupervisor_uri_4_blank_5 >> rail.Label(
            ...
            greece_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9 >> \
