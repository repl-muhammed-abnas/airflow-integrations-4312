...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.expense_report_master_dag_id,
        description=f'LEAD3R LLC Expense Import Reports Master {config.instance}',
        ...
        new_file_sensor = rail.SFTPAnyFileSensor(
            ...
            path=config.sftp_input_filepath_report,
        ...
        send_incorrect_file_format_mail = rail.EmailOperator(
            ...
            subject='''{{ get_company_key() }} | Expense Report Import - Incorrect file format received - {{ current_time_in_specified_tz("US/Pacific", "%Y-%m-%dT%H:%M:%S") }}''',
            html_content="templates/incorrect_file_format_report.html"
        )
        ...
        archive_file = rail.SFTPMoveFileOperator(
            ...
            new_filename=config.sftp_archive_filepath_report +
        ...
        create_collection_from_csv = rail.CreateCollectionOperator(
            ...
            columns={
                'Username': 'Username',
                'Concur Username': 'Concur_Username',
                'Expense Type': 'Expense_Type',
                'Business Purpose': 'Business_Purpose',
                'Amount': 'Amount',
                'Expense Report Number': 'Expense_Report_Number',
        ...
        if_csv_has_data = rail.IfOperator(
            ...
            yes_task='create_log_expense_report_import',
        ...
        send_blank_payload_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | Expense Report Import - Blank File - {{ current_time_in_specified_tz("US/Pacific", "%Y-%m-%dT%H:%M:%S") }} ',
            html_content="templates/email_blank_payload_report.html"
        )

        create_log_expense_report_import = rail.CreateLogOperator(
            task_id='create_log_expense_report_import'
        )

        query_list_expense_report_missing_required_fields = rail.QueryCollectionOperator(
            task_id='query_list_expense_report_missing_required_fields',
            query="""SELECT * FROM  inputfile 
                WHERE (NULLIF(Concur_Username,'') IS NULL 
                OR NULLIF(Transaction_Date,'') IS NULL
        ...
        if_records_with_missing_required_fields_list_has_data = rail.IfOperator(
            ...
            test=lambda: rail.result(
                'query_list_expense_report_missing_required_fields', 'length') > 0,
        ...
        log_invalid_records = rail.WriteLogOperator(
            ...
            log="{{result('create_log_expense_report_import')}}",
            items="{{result('query_list_expense_report_missing_required_fields')}}",
            message='One or more mandatory field is missing',
            severity='Exception',
            properties=lambda item: {
                "concur_username": item['Concur_Username'],
                ...
                "action": "Validation",
                "status": "Exception",
                "details": custom_methods.get_missing_field_message_report(item)
            }
        )
        ...
        query_list_valid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM inputfile 
                WHERE (NULLIF(Concur_Username,'') IS NOT NULL 
        ...
        get_owner_uri_for_expense_report_import = rail.RepliconServiceOperator(
            task_id='get_owner_uri_for_expense_report_import',
            endpoint="/services/UserService1.svc/GetUser2",
            data={
                "user": {
                    "loginName": config.user_loginname_for_report
                }
            },
        ...
        query_distinct_concur_username_and_report_date_from_valid_records = rail.QueryCollectionOperator(
            task_id='query_distinct_concur_username_and_report_date_from_valid_records',
            query="""SELECT DISTINCT Concur_Username, Report_Date FROM valid_records""",
            name="distinct_concur_username_report_date"
        )

        trigger_dag_create_expense_sheet = rail.trigger_parallel_dagrun(
            task_id='trigger_dag_create_expense_sheet',
            items="{{result('query_distinct_concur_username_and_report_date_from_valid_records')}}",
            trigger_dag_id=config.child_create_expense_sheet_for_report_dag_id,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            parallel_count=config.create_expense_sheet_trigger_parallel_count,
            conf=lambda item: {
                'concur_username': item['Concur_Username'],
                'report_date': item['Report_Date'],
                'owner_uri': rail.result('get_owner_uri_for_expense_report_import'),
                ...
                'expense_report_import_logs': rail.result('create_log_expense_report_import')
            }
        )
        ...
        process_log_generation = rail.TriggerDagRunOperator(
            ...
            retries=0,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            trigger_dag_id=config.process_log_generation_for_report_dag_id,
            conf=lambda: {
                'expense_import_log': rail.result('create_log_expense_report_import'),
        ...
        if_csv_has_data >> rail.Label('Yes') >> create_log_expense_report_import >> query_list_expense_report_missing_required_fields \
        ...
        create_collection_replicon_expense_codes_uri >> get_required_project_column_uri >> get_replicon_projects_list >> create_replicon_project_collection \
            >> query_valid_records_to_join_expense_code_uris >> get_owner_uri_for_expense_report_import >> get_required_expense_sheet_customfield_uri \
            >> get_default_currency_uri >> query_distinct_concur_username_and_report_date_from_valid_records

        query_distinct_concur_username_and_report_date_from_valid_records >> trigger_dag_create_expense_sheet \
