...
import rail
import uuid
from lead3rllc.expense_import.utils.request_payload import put_expense_sheet_payload_report


def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.child_create_expense_sheet_for_report_dag_id,
        description=f'LEAD3R LLC Expense Import Report - Create Expense Sheet {config.instance}',
        ...
        query_get_matching_records_from_valid_records_for_expense_sheet = rail.QueryCollectionOperator(
            ...
            query_params={
                'concur_username': '{{ dag_run.conf.concur_username }}',
        ...
        put_expense_sheet_entries = rail.RepliconServiceOperator(
            ...
            data=put_expense_sheet_payload_report
        )
        ...
        add_expense_sheet_success_log_entry = rail.WriteLogOperator(
            ...
            log="{{dag_run.conf.expense_report_import_logs}}",
            message='na',
            severity='Success',
