...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_log_generation_for_invoice_dag_id,
        description=f'LEAD3R LLC Expense Import Invoice Process Log Generation {config.instance}',
        ...
        generate_downloadlink_logs = rail.GeneratePresignedDownloadUrlOperator(
            ...
            output_file_name="expense_invoice_import_log_{{ dag_run.conf.input_filename}}_{{ current_time_in_specified_tz('US/Pacific', '%Y-%m-%dT%H:%M:%S') }}.csv",
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_logs', 'error_record_count') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() }} | Expense Invoice Import {{" "}} \
            ...
            html_content="/templates/import_complete_invoice.html"
        )
