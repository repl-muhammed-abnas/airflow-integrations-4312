...
from deltek_vantagepoint.user_sync.utils.python_callable_method import get_exception_logs
null = None


def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'deltek_vantagepoint_process_each_user_child_{config.instance}',
        description='Handles Sync of One Vantagepoint User to Replicon',
        ...
        if_logs_tobe_added = rail.IfOperator(
            task_id = 'if_logs_tobe_added',
            test = lambda dag_run: rail.result('check_exceptions_for_logging') or ((not dag_run.conf['loginenabled']) and dag_run.conf[
                    'currentdetails'] and dag_run.conf['currentdetails']['status'] == 'True'),
            yes_task = 'add_logs_and_exceptions',
            no_task = 'catch_error'
        )

        add_logs_and_exceptions = rail.WriteLogOperator(
            task_id='add_logs_and_exceptions',
            message="na",
            severity="Error/Exception",
            properties=lambda dag_run: {
                "loginname": dag_run.conf['loginname'],
                "action": "Add/Update",
                "status": "Exception" if rail.result('check_exceptions_for_logging') else 'Info',
                "reason":
                    ( "User disabled in Replicon and please take necessary actions." if ((not dag_run.conf['loginenabled']) and dag_run.conf[
                    'currentdetails'] and dag_run.conf['currentdetails']['status'] == 'True') else "User created/updated in Replicon." ) +
                    ((" Exceptions: " + rail.result('check_exceptions_for_logging')) if rail.result('check_exceptions_for_logging') else "")
            }
        )
        ...
        can_run_batch_task >> rail.Label(
            'No') >> check_exceptions_for_logging >> if_supervisor_to_assign
        if_supervisor_to_assign >> rail.Label(
            'Yes') >> search_supervisor_permission >> if_supervisor_permission_not_present
        if_supervisor_permission_not_present >> rail.Label(
            'Yes') >> assign_supervisor_permission >> create_or_update_user
        if_supervisor_permission_not_present >> rail.Label(
            'No') >> create_or_update_user
        if_supervisor_to_assign >> rail.Label(
            'No') >> create_or_update_user >> if_supervisor_to_be_removed
        if_supervisor_to_be_removed >> rail.Label(
            'Yes') >> remove_supervisor >> if_supervisor_sent_but_not_found_in_replicon
        if_supervisor_to_be_removed >> rail.Label(
            'No') >> if_supervisor_sent_but_not_found_in_replicon
        if_supervisor_sent_but_not_found_in_replicon >> rail.Label(
            'Yes') >> add_supervisor_assignment_log >> if_logs_tobe_added
        if_supervisor_sent_but_not_found_in_replicon >> rail.Label(
            'No') >> if_logs_tobe_added
        if_logs_tobe_added >> rail.Label('Yes') >> add_logs_and_exceptions >> catch_error
        if_logs_tobe_added >> rail.Label('No') >> catch_error >> log_to_sumo
