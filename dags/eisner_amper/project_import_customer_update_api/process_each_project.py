...
from eisner_amper.project_import_customer_update_api.utils import request_payload, response_filter, python_callable_methods
...
def create_child_dag_wbs(config):
    with rail.create_airflow_dag(
        ...
        log_project_exception = rail.WriteLogOperator(
            ...
            log='{{ result("create_project_log") }}',
            message= 'Project does not exist in replicon, project update skipped via customer update API',
            severity='Exception',
            properties=lambda dag_run: {
                'clientcode': dag_run.conf['clientcode'],
                'projectcode': dag_run.conf['item']['ProjectCode'],
