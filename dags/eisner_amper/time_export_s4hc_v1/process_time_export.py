...
from eisner_amper.time_export_s4hc_v1.utils.custom_methods import logging_details
from eisner_amper.time_export_s4hc_v1.utils import request_payload
from eisner_amper.time_export_s4hc_v1.custom_http_operator.CustomSimpleHttpOperator import CustomSimpleHttpOperator
...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f"eisner_amper_time_export_child_s4hc_v1_{config.instance}",
        description=f"Eisner Amper Time Export Child S4HC V1 {config.instance}",
        ...
        send_data_client_endpoint = CustomSimpleHttpOperator(
            ...
            data=lambda: rail.result('compose_json_payload').encode("utf-8"),
            extra_options={
                'verify': False
            },
            retries=0
