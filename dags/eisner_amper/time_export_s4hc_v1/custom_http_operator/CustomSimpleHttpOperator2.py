...
from rail import SimpleHttpOperator

if TYPE_CHECKING:
    from airflow.utils.context import Context
 
class CustomSimpleHttpOperator2(SimpleHttpOperator):
    """
    ...
    def execute(self, context: Context) -> Any:
        ...
        if callable(self.data):
            kwargs_callable = make_kwargs_callable(self.data)
            self.data = kwargs_callable(**context)
        http = HttpHook(
            self.method,
        ...
        should_check_response = self.extra_options.pop("check_response", True)
        response = http.run(self.endpoint, self.data, self.headers, {**self.extra_options, "check_response": False})
        if self.log_response:
            self.log.info(response.text)
            self.log.info("Response status: %s", response.status_code)
            self.log.info("Response headers: %s", dict(response.headers))
        if response.status_code >= 400:
            correlation_id = response.headers.get('correlationid', '')
            if correlation_id:
                context['ti'].xcom_push(key='sap_correlation_id', value=correlation_id)
