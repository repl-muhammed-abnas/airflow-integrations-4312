...
from eisner_amper.time_export_s4hc_v1.utils.custom_methods import (
    safe_parse_date,
    build_lookup_dict,
...
from eisner_amper.time_export_s4hc_v1.mappers.location_mapper import location_code_map
from eisner_amper.time_export_s4hc_v1.mappers.service_line_mapper import service_line_map
null = None
...
def get_formated_row(item):
    ...
    cost_center_code = item.get("Cost_Center_Code", '') or item.get("Cost Center Code", '')
...
def create_json_payload_for_s4hc_from_csv():
    ...
    if not csv_content:
        return json.dumps({"timedata": [], "log": ""}, ensure_ascii=False)
    ...
    json_output = json.dumps({"timedata": json_records, "log": ""}, separators=(',', ':'), ensure_ascii=False)
    return json_output
