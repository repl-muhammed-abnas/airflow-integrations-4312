...
import json
import uuid
from eisner_amper.user_import.mapper.user_import_mapper import EISNER_AMPER_USER_SYNC_MAPPER
import rail
...
def get_current_office_schedule():
    ...
    for item in rail.result('bulk_get_user')[0]['schedulePolicies']:
        ...
        effective_date = (
            str(item['effectiveDate'].get('day', None)) + "/" +
            str(item['effectiveDate'].get('month', None)) + "/" +
            str(item['effectiveDate'].get('year', None))
        ) if item.get('effectiveDate') else (
            str(rail.result('bulk_get_user')[0]['userDetails']['employmentDateRange']['startDate']['day']) + "/" +
            str(rail.result('bulk_get_user')[0]['userDetails']['employmentDateRange']['startDate']['month']) + "/" +
            str(rail.result('bulk_get_user')[
