...
from eisner_amper.project_import_internal_update_api.utils import request_payload, response_filter, python_callable_methods
...
def create_child_dag_wbs(config):
    with rail.create_airflow_dag(
        ...
        is_project_available = rail.IfOperator(
            ...
            yes_task="create_projectorapply_modifications",
        ...
        log_project_exception = rail.WriteLogOperator(
            ...
            log='{{ result("create_project_log") }}',
            message='Project does not exist in replicon, project update skipped via internal update API',
            severity='Exception',
            properties=lambda dag_run: {
                'clientcode': dag_run.conf['clientcode'],
                'projectcode': dag_run.conf['item']['ProjectCode'],
        ...
        is_project_available >> rail.Label('No') >> log_project_exception >> catch_and_log_errors
        is_project_available >> rail.Label('Yes') >> create_projectorapply_modifications >> is_cost_center_present_in_payload
        ...
        is_task_available_in_payload >> rail.Label('Yes') >> get_children_task_details >> get_tasks_to_add_update >> has_task_validation_errors
        ...
        has_tasks_to_update >> rail.Label('Yes') >> update_tasks >> log_tasks_updated >> check_project_type
        has_tasks_to_update >> rail.Label('No') >> check_project_type
