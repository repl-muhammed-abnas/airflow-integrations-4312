...
from eisner_amper.project_import_customer_v1.utils import python_callable_methods

def create_child_dag_wbs(config):
    with rail.create_airflow_dag(
        dag_id=f'eisner_amper_project_import_customer_records_log_generation_{config.instance}',
        ...
        upload_log_to_sftp = rail.SFTPUploadFileOperator(
            ...
            '/log_{{ dag_run_ecid() | replace(":", "-") }}_customer.csv'
        )
        ...
        send_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_logs', key='error_record_count') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Replicon Eisner Amper Project Import Customer sync -  " }} \
