...
import uuid
import rail
from eisner_amper.project_import_internal_update_api_v1.mapper.time_entry_codes_mapper import time_entry_code_mapper
...
def get_modified_task_name(dag_run):
    return dag_run.conf["task_code"] + " " + dag_run.conf["task_name"]
...
def get_put_task_data(dag_run):
    return {
        ...
        "task": {
            "target": {
                "uri": null,
                "name": get_modified_task_name(dag_run),
                "parent": null,
                "parameterCorrelationId": null
            },
            "name": get_modified_task_name(dag_run),
...
def get_update_task_payload(dag_run):
    return {
        "target": {
            "uri": dag_run.conf['task_details'][0]['uri'],
        },
        "project": {
            "uri": dag_run.conf['project_uri'],
        },
...
def get_task_details(item, children_task_details):
    return list(filter(lambda x: x['code'] == item['TaskCode'], children_task_details))

def get_add_tasks_payload():
    """Get payload for adding new tasks - using Mammoet API structure with Internal mappings"""
    ...
    # Use Mammoet structure: project + taskHierarchy
    task_hierarchy = []
    for task in tasks_to_add:
        task_data = {
            "target": null,  # null for new tasks
            "taskModificationToApply": {
                "name": task['TaskCode'] + " " + task['TaskName'],  # Internal format: code + name
                "codeToApply": {
                    "value": task['TaskCode']  # Internal task code
                },
                "isClosed": "false",
                "timeEntryStartDateToApply": null,
                "timeEntryEndDateToApply": null,
...
def get_update_tasks_payload():
    """Get payload for updating existing tasks - using Mammoet API structure with Internal mappings"""
    ...
    for task in tasks_to_update:
        task_data = {
            "target": {"uri": task['existing_task_uri']},  # existing task URI for updates
            "taskModificationToApply": {
                "name": task['TaskCode'] + " " + task['TaskName'],  # Internal format: code + name
                "codeToApply": null,  # Don't update code for existing tasks
                "isClosed": "false",
                "timeEntryStartDateToApply": null,
                "timeEntryEndDateToApply": null,
                "timeAndExpenseEntryTypeToApply": {
                ...
                "isTimeEntryAllowed": "true",
                "resourceAssignmentModifications": null  # Don't modify resources for updates
            }
        }
        task_hierarchy.append(task_data)
