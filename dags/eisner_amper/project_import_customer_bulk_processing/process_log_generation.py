...
from eisner_amper.project_import_customer_bulk_processing.utils import python_callable_methods
        ...
        send_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_logs', key='error_record_count') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Replicon EisnerAmper Project Import (BULK Processing) for customer sync -  " }} \
