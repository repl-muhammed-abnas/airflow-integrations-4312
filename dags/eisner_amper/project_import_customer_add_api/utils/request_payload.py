...
def get_data_to_process(dag_run):
    ...
    # Validate mandatory task fields and separate valid from invalid tasks
    valid_tasks = []
    invalid_tasks = []
    ...
    # Store invalid tasks in rail context for logging
    if invalid_tasks:
        rail.set_result(key="task_validation_errors", val=invalid_tasks)
