...
from seaspanshipyards.auto_shift_assignment.daily_update.utils import request_payload, python_callable_method


def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'seaspanshipyards_default_shift_assignment_daily_update_V3.0_{config.instance}',
        description=f'Seaspanshipyards_Default shift assignment_daily update  V3.0 {config.instance}',
        ...
        def validate_shift_dates_name():
            dag_conf = get_dag_run_conf()
            start_date_str = dag_conf['Startdate']
            start_date_time = datetime.strptime(start_date_str, "%Y-%m-%d")
            end_date_str = dag_conf['Enddate']
            end_date_time = datetime.strptime(end_date_str, "%Y-%m-%d")
            return not ((start_date_time > end_date_time) or (not dag_conf['Shiftname']))

        has_shift_name_start_end_date = rail.IfOperator(
            task_id="has_shift_name_start_end_date",
            test=validate_shift_dates_name,
        ...
        any_shifts_to_be_updated = rail.IfOperator(
            task_id ='any_shifts_to_be_updated',
            test=python_callable_method.check_any_shifts_to_be_deleted,
            yes_task = "has_shift_assigned_data",
            no_task = "finish"
        )
        ...
        has_shift_name_start_end_date
        has_shift_name_start_end_date >> rail.Label(
            "No") >> finish
        has_shift_name_start_end_date >> rail.Label(
            "Yes") >> create_date_range_seq >> get_shift_schedule_summary >> get_assigned_shift_dates >> any_shifts_to_be_updated
        any_shifts_to_be_updated >> rail.Label(
            "Yes") >> has_shift_assigned_data
        any_shifts_to_be_updated >> rail.Label(
            "No") >> finish
