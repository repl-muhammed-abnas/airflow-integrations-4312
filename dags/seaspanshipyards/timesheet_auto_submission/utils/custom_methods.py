...
def is_task_mark_timesheet_out_of_date_failed():
    if get_task_state('mark_timesheet_out_of_date') == 'failed':
        reason  = rail.result('mark_timesheet_out_of_date', key="error").get('response').get('json').get('error').get('reason')
        if "URI not found" in reason:
            return True
        return False
    return False