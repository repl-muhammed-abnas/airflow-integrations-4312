...
from quickbooks.invoice_export.request_payload import get_queuedforsync_invoice


# pylint:disable = too-many-statements
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"standard_quickbooks_online_{config.region.replace('-', '_')}_invoice_export_{config.instance}",
        description=f'QuickBooks Online {config.region} Invoice Export {config.instance}',
        ...
        def get_tax_type_details(tax_rate):
            tax_type = rail.find_first_by_attr_and_get_attr(tax_rate, 'text', 'Type', 'collection')
            rate = rail.find_first_by_attr_and_get_attr(tax_rate, 'text', 'Rate', 'collection')
            is_enabled = rail.find_first_by_attr_and_get_attr(tax_rate, 'text', 'Enabled', 'collection')
            tax_type_details = {
                'type': tax_type and tax_type[1] and tax_type[1].get('text'),
                'rate': rate and rate[1] and rate[1].get('number'),
                'enabled': is_enabled and is_enabled[1] and is_enabled[1].get('bool')
            }
            return tax_type_details
        ...
        def handle_updated_invoices(response, item):
            def compare_datetime_value(datetime_value):
                ...
                last_synctime = datetime.strptime(
                    rail.result('get_lastsync_time_and_current_time')['last_synctime'], '%Y-%m-%dT%H:%M:%SZ')
            ...
            if is_valid_invoice:
                tax_profile = rail.find_first_by_attr_and_get_attr(response['customMetadata'], 'keyUri', 'urn:replicon:invoice-metadata-key:tax-profile')
                if ( #pylint: disable=too-many-boolean-expressions
                    tax_profile and
                    'value' in tax_profile and
                    'collection' in tax_profile['value'] and
                    isinstance(tax_profile['value']['collection'], list) and
                    len(tax_profile['value']['collection']) > 0 and
                    'collection' in tax_profile['value']['collection'][0]
                ):
                    item = {
                        **item,
                        'tax_profile': tax_profile['value']['uri'],
                        'tax_type_details': get_tax_type_details(tax_profile['value']['collection'][0]['collection'])
                    }
                return item
            return ''
        ...
        trigger_invoice_child_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f"standard_quickbooks_online_{config.region.replace('-', '_')}_invoice_export_child_dag_{config.instance}",
            conf=lambda dag_run, item: {
                **dict(item.items()),
                **({
                    'object_extension_binding': rail.result('get_required_object_extension_binding'),
                    'synced_dropdown_uri': rail.result('get_synced_status_required_dropdowns')
                } if config.instance == 'production' else {}),
                **{
        ...
        should_log_history = rail.IfOperator(
            ...
            test="{{ not((get_task_state('has_list_data') == 'success' and \
                ...
                (get_task_state('has_quickbooks_invoice_data') == 'success' and \
        ...
        can_run_batch_task >> rail.Label(
            'No') >> get_lastsync_time_and_current_time >> get_sync_status_filter_definition >> \
            get_queued_for_sync_invoice >> update_lastsync_time >> has_list_data

        has_list_data >> rail.Label(
            'Yes') >> get_required_invoices >> \
            has_quickbooks_invoice_data
