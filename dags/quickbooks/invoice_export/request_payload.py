...
def get_item_ref_test_override_id():
    return Variable.get('ff_qb_item_ref_test_override_id_trial', default_var=None)


def is_rounding_fix_enabled_for_company(company_key):
    allowed_companies_str = Variable.get('ff_rp_qb_fix_rounding_issue', default_var=' ')
    allowed_companies = [key.strip() for key in allowed_companies_str.split(',') if key.strip()]
    is_enabled = company_key in allowed_companies
    return is_enabled


def get_sales_item(invoice_qbo_items, tax_code_id, dag_run):
    billing_type_mapping = {
        'timesheet': 'Services',
        'fixed-bid': 'Services',
        'expense': 'Reimbursable Expense Item'
    }
    invoice_items = result('get_invoice_items')
    is_rounding_fix_enabled = is_rounding_fix_enabled_for_company(dag_run.conf.get('company_key', ''))
    sales_line_items = []
    for invoice in invoice_items:
        ...
        rate_amount = (
                invoice['unroundedRate'].get('amount') if is_rounding_fix_enabled and invoice.get('unroundedRate') is not None
                else (invoice['rate']['amount'] if invoice['rate'] else total_amount)
            )
        quantity = invoice['quantity'] if invoice.get(
            'quantity') != None else 1
        bill_type = billing_type_mapping.get(billing_type) or 'Sales'
        sales_items = [x for x in invoice_qbo_items if x['Name'] == bill_type]
        if sales_items:
            resolved_item = sales_items[0]
        else:
            fallback_items = [x for x in invoice_qbo_items if x['Name'] == 'Services']
            if fallback_items:
                resolved_item = fallback_items[0]
            else:
                raise ValueError(
                    f"No QBO item found for billing type '{bill_type}' and no 'Services' "
                    f"fallback item present for invoice {dag_run.conf.get('invoice')}"
                )
        test_override_id = get_item_ref_test_override_id()
        if test_override_id and resolved_item['Id'] == '1':
            logging.warning(
                f"[TEST OVERRIDE] ItemRef Id overridden from '1' to '{test_override_id}' "
                f"for invoice {dag_run.conf.get('invoice')} (ff_qb_item_ref_test_override_id_trial is set)"
            )
            resolved_item = {**resolved_item, 'Id': test_override_id}
...
def get_txn_tax_detail(tax_code_id):
    return {
        'TxnTaxDetail': {
            'TxnTaxCodeRef': {
                'value': tax_code_id
...
def determine_tax_code_ids(dag_run, should_sync_invoice_with_tax_code):
    line_item_tax_code_id = 'NON'
    invoice_level_tax_code_id = None
    
    if should_sync_invoice_with_tax_code:
        tax_codes = result('search_taxcodes_qbo')['QueryResponse'].get('TaxCode', [])
        invoice_level_tax_code_id = find_first_by_attr_and_get_attr(tax_codes, 'Name', dag_run.conf['tax_type_details']['type'], 'Id')

        # To Handle US customers, try 'TAX' first, then fall back to invoice level
        tax_code = find_first_by_attr_and_get_attr(tax_codes, 'Name', 'TAX', 'Id')
        line_item_tax_code_id = tax_code if tax_code else (invoice_level_tax_code_id if invoice_level_tax_code_id else 'NON') # If 'TAX' not found, use invoice level tax code or 'NON' as last resort
    
    return line_item_tax_code_id, invoice_level_tax_code_id
...
def create_invoice_with_multiline_item_request(dag_run):
    ...
    should_sync_invoice_with_tax_code = dag_run.conf.get('tax_type_details', False) and dag_run.conf['customSettings'].get('syncInvoiceWithTaxCode', False)
    line_item_tax_code_id, invoice_level_tax_code_id = determine_tax_code_ids(dag_run, should_sync_invoice_with_tax_code)# These 2 var can be different for US Customers, hence calculated separately
    return {
        'CustomerRef': {
            'value': customer_id
        },
        ...
        'Line': get_sales_item(invoice_qbo_items, line_item_tax_code_id, dag_run),
        **(get_txn_tax_detail(invoice_level_tax_code_id) if should_sync_invoice_with_tax_code else {}),
        'DueDate': due_date
    }
