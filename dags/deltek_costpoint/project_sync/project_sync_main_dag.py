...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'deltek_costpoint_project_sync_main_{config.instance}',
        description=f'deltek_costpoint_project_sync_main{config.instance}',
        ...
        get_modified_workforce_plc_costpoint = rail.DeltekCostPointServiceOperator(
            ...
            data=lambda: {
                "filter": {
                    "id": "replicon_exp_project_wfc_plc",
                    "where": [
                        {
                            "rsWhere": {
                                "rsId": "PJM_PROJEMPL_HDR",
        ...
        process_each_root_project = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'deltek_costpoint_project_sync_child_{config.instance}',
            execution_timeout=timedelta(days=14),
            conf=lambda item: {'item': {**item},
                               'billing_rates':  rail.result('get_updated_billing_rates_from_replicon'),
        ...
        can_load_data_in_chunks >> rail.Label('yes') >> get_modified_projects_in_chunks >> has_project_data
        can_load_data_in_chunks >> rail.Label('no') >> get_modified_projects >> has_project_data
        has_project_data >> rail.Label('yes') >> group_data_by_root_project
        has_project_data >> rail.Label('no') >> delete_this_dagrun >> update_last_run_date >> finish
        group_data_by_root_project >> get_all_clients_from_replicon >> get_billing_rates_from_replicon >> get_costpoint_plcs >> \
            process_new_billingrates >> get_updated_billing_rates_from_replicon >> get_replicon_divisions >> \
        ...
        has_error_logs >> rail.Label('yes') >> create_csv_lines >> log_filename >> generate_download_link >> send_mail_error >> update_last_run_date >> finish
