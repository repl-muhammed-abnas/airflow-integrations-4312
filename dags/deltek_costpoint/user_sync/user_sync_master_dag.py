...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
            ...
            if emplabinfo_childs:
                for emp_salary_info in emplabinfo_childs:
                    ...
                    emp_history.append({
                        ...
                        "supervisor": emp_salary_info['row']['data'].get('SPVSR_EMPL_ID') if emp_salary_info and emp_salary_info['row']['data'].get('SPVSR_EMPL_ID') != employee_id else None
                    })
            return emp_history
            ...
            for companyUsers in cost_point_user_obj:
                ...
                if companyUsers['document']['rows'] and\
                    ...
                    for cost_point_usr in companyUsers['document']['rows']:
                        employee_sub_trees = cost_point_usr['row'].get('children', [])
                        ...
                        cost_point_user.append({
                            ...
                            "supervisor": emplabinfo_child['row']['data'].get('SPVSR_EMPL_ID') if emplabinfo_child else None,
        ...
        process_user = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=lambda: rail.result('cost_point_user'),
            execution_timeout=timedelta(days=config.execution_timeout_days),
        ...
        gather_new_user_dag_run = rail.GatherResultsFromDagRunsOperator(
            task_id='gather_new_user_dag_run',
            dag_runs="{{result('process_user')}}",
            dagrun_task_id="new_user_dag_run",
            flatten=True
        )
        ...
        def get_superuser(super_assignments):
            super_assignment_users = []
            new_user_creations = rail.result('gather_new_user_dag_run')
            for super_user in super_assignments:
                if super_user['superuseruri'] is None:
                    missing_super_user_uri = rail.find_first_by_attr_and_get_attr(
                        new_user_creations, 'loginName', super_user['supervisor'], 'uri') if new_user_creations else None
                    if missing_super_user_uri:
                        super_assignment_users.append({
                            "supervisor": super_user['supervisor'],
                            "effectivedate": super_user['effectivedate'],
                            "superuseruri": missing_super_user_uri
                        })
                else:
                    super_assignment_users.append({
                        "supervisor": super_user['supervisor'],
                        "effectivedate": super_user['effectivedate'],
                        "superuseruri": super_user['superuseruri']
                    })

            return super_assignment_users
            ...
            for supervisor_info in supervisor_log_informations:
                if supervisor_info['properties']:
                    supervisor_details.append({
                        ...
                        "supervisorassignment": get_superuser(supervisor_info['properties'].get('supervisorassignment')),
        ...
        if_costpoint_user_present >> rail.Label('Yes') >> supervisor_processing_log >> get_alltimeoff_types >> \
            ...
            get_oef_tags_for_idtype >> cost_point_user >> search_modified_cp_users_from_rep >> \
            process_user >> wait_for_completion_trigger_process_user >> \
            gather_new_user_dag_run >> trigger_dag_run_deltek_costpoint_supervisor_assignment >> \
        ...
        has_error_logs >> rail.Label('Yes') >> create_csv_lines >> log_filename >> generate_download_link >> \
            send_mail_error >> update_last_run_date >> catch_error
        has_error_logs >> rail.Label(
            'no') >> update_last_run_date >> catch_error >> send_configuration_error >> log_to_sumo
