...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        def get_policy_sets(dag_run):
            policy_sets = []
            if dag_run.conf.get('time_off_template'):
                policy_sets.append({"uri": null, "name": dag_run.conf['time_off_template']})
            if dag_run.conf.get('timesheet_template'):
                policy_sets.append({"uri": null, "name": dag_run.conf['timesheet_template']})
            if dag_run.conf.get('punchentrypolicy'):
                policy_sets.append({"uri": null, "name": dag_run.conf['punchentrypolicy']})
            return policy_sets
        ...
        foreach_supervisor_assignment >> foreach_supervisor_assignment_end >> \
            sync_users >> assign_required_timeofftypes >> if_timeoff_present_skip_timeoff_assignment
