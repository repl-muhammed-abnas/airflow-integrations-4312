...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'deltek_costpoint_process_each_user_dag_{config.instance}',
        ...
        def is_missing_supervisor_assignment(dag_run):
            for usersuper in dag_run.conf['employeehistory']:
                super_user_history = rail.result('search_supervisor_users')
                if usersuper['supervisor']:
                    supervisor_uri = rail.find_first_by_attr_and_get_attr(
                        super_user_history, 'employeeid', usersuper['supervisor'], 'useruri')
                    if supervisor_uri is None:
                        return True
            return False
            ...
            for employee in dag_run.conf['employeehistory']:
                if employee['supervisor']:
                    supervisor_uri = rail.find_first_by_attr_and_get_attr(super_user_history, 'employeeid', employee['supervisor'], 'useruri') \
                        if employee['supervisor'] and super_user_history else None
        ...
        if_user_uri_present_update_user = rail.IfOperator(
            ...
            yes_task="trigger_dag_run_user_update",
        ...
        def get_mapper_configurations(dag_run):
            ...
            return {
                ...
                "punchentrypolicy": mapper_configurations.get('punchentrypolicy')
            }
        ...
        trigger_dag_run_user_update = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[1],
            trigger_dag_id=f'deltek_costpoint_update_user_sync_{config.instance}',
        ...
        trigger_dag_run_add_user = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[1],
            trigger_dag_id=f'deltek_costpoint_add_user_sync_{config.instance}',
        ...
        new_user_dag_run = rail.GatherResultsFromDagRunsOperator(
            task_id='new_user_dag_run',
            dag_runs="{{result('trigger_dag_run_add_user')}}",
            dagrun_task_id="sync_users",
            flatten=True
        )
        ...
        if_superuser_not_present_skip_processing >> rail.Label(
            'No') >> search_users
        if_superuser_not_present_skip_processing >> rail.Label(
            'Yes') >> search_supervisor_users >> if_supervisor_assignment_missing
        if_supervisor_assignment_missing >> rail.Label(
            'Yes') >> add_supervisor_assignment_table >> search_users
        if_supervisor_assignment_missing >> rail.Label(
            'No') >> search_users >> if_user_uri_present_update_user
        if_user_uri_present_update_user >> rail.Label('No') >> \
            trigger_dag_run_add_user >> wait_for_completion_trigger_dag_run_add_user >> new_user_dag_run >> process_users
        if_user_uri_present_update_user >> rail.Label('Yes') >> \
            trigger_dag_run_user_update >> wait_for_completion_trigger_dag_run_user_update >> process_users >> catch_error
