...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.time_punch_master_6_35_am_dagid,
        description=f"victoriashipyards Timesheet Auto Submission Time Punch Master 6:35 AM {config.instance}",
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=datetime(2022, 12, 7, tz=config.time_zone),
        schedule_interval=config.schedule_interval_6_35_am,
