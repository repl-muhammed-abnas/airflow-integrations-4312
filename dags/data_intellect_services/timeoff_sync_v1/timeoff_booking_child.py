...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'Data Intellect Timeoff Sync Booking Child {config.instance}',
        ...
        log_user_not_present = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "employee_id": dag_run.conf["actual_employee_id"],
                "unique_id": dag_run.conf["booking_data"]["employeeId"],
        ...
        get_specfic_time_off_type = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response, dag_run: rail.find_first_by_attr_and_get_attr(
                response, 'displayText', dag_run.conf["booking_data"]["policyTypeDisplayName"], 'uri')
        )
        ...
        log_timeoff_type_not_present = rail.WriteLogOperator(
            ...
            log='{{ dag_run.conf.log_artifact}}',
            message='Time Off type {{ dag_run.conf.booking_data.policyTypeDisplayName }} not available in Replicon',
            severity='Skipped',
            properties=lambda dag_run: {
                ...
                "booking_end_date": dag_run.conf["booking_data"].get("endDate") or dag_run.conf["booking_data"].get("date"),
                "status": "Skipped",
                "comments": 'Time Off type "'+ dag_run.conf["booking_data"]["policyTypeDisplayName"] + '" not available in Replicon',
                "dag_run_id": get_dagrun_ecid(dag_run)
            }
        )
        ...
        log_timeoff_type_not_assigned_to_user = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "booking_end_date": dag_run.conf["booking_data"].get("endDate") or dag_run.conf["booking_data"].get("date"),
                "status": "Skipped",
                "comments": 'Time Off type "'+ dag_run.conf["booking_data"]["policyTypeDisplayName"] + '" is not assigned to user in Replicon',
                "dag_run_id": get_dagrun_ecid(dag_run)
            }
        )

        get_timeoff_desc = rail.SimpleHttpOperator(
            task_id='get_timeoff_desc',
            method='GET',
            http_conn_id=config.http_conn_id,
            endpoint='timeoff/employees/{{ dag_run.conf.booking_data.employeeId }}/requests/{{ dag_run.conf.booking_data.requestId }}',
            headers={
                'accept': 'application/json'
            },
            response_filter=response_filters.get_timeoff_desc
        )
        ...
        is_timeoff_type_assigned_to_user >> rail.Label("Yes") >> get_timeoff_desc >> put_and_submit_timeoff_booking_for_user >> \
