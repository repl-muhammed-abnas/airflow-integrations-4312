import json
from ipipeline.user_import.utils import request_payload, custom_methods
...
def create_add_user_child_dag(config):
    with rail.create_airflow_dag(
        ...
        if_orgrole_code_exists = rail.IfOperator(
            ...
            no_task='if_timeoff_types_exist_before_create'
        )
        ...
        get_resource_pool_from_replicon = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response, dag_run: rail.find_first_by_attr_and_get_attr(response, "displayText", dag_run.conf["calculated_orgrole_code"])
        )
        ...
        # Timeoff assignment tasks - moved before create_new_user
        if_timeoff_types_exist_before_create = rail.IfOperator(
            task_id='if_timeoff_types_exist_before_create',
            test='{{ dag_run.conf.calculated_time_off_types | is_truthy }}',
            yes_task='get_default_timeoff_policies_for_create',
            no_task='process_user_logging'
        )

        # Get default policies for all timeoff types before user creation
        get_default_timeoff_policies_for_create = rail.RepliconServiceCallForEachItemOperator(
            task_id="get_default_timeoff_policies_for_create",
            items=lambda dag_run: [timeoff_config.get("uri") for timeoff_config in dag_run.conf.get("calculated_time_off_types", {}).values()] if dag_run.conf.get("calculated_time_off_types") else [],
        ...
        calculate_timeoff_policy_accruals_for_create = rail.PythonOperator(
            task_id="calculate_timeoff_policy_accruals_for_create",
            python_callable=lambda dag_run: custom_methods.build_timeoff_types_for_user_creation(dag_run, config)
        )

        if_timeoff_policies_calculated = rail.IfOperator(
            task_id="if_timeoff_policies_calculated",
            test=lambda: bool(rail.result("calculate_timeoff_policy_accruals_for_create", {})),
            yes_task="assign_timeoff_policies_to_user",
            no_task="process_user_logging"
        )

        assign_timeoff_policies_to_user = rail.RepliconServiceCallForEachItemOperator(
            task_id="assign_timeoff_policies_to_user",
            items=lambda: rail.result("calculate_timeoff_policy_accruals_for_create", []),
        ...
        if_pending_supervisor_flag = rail.IfOperator(
            ...
            test=lambda: rail.result("get_pending_supervisor_flag")["value"] == 'true',
        ...
        write_added_user_logs = rail.WriteLogOperator(
            ...
            message=lambda dag_run: "User created successfully" if not request_payload.get_exception_logs(dag_run, config) else
                ("User partially created - " + " | ".join(request_payload.get_exception_logs(dag_run, config))),
            severity=lambda dag_run: "Success" if not request_payload.get_exception_logs(dag_run, config) else "Exception",
            properties=lambda dag_run: {
                "employeeid": dag_run.conf["employee_id"],
                "action": "Add",
        ...
        is_supervisor_present >> rail.Label("Yes") >> if_user_and_supervisor_same
        ...
        if_supervisor_present_as_user_in_feed >> rail.Label("Yes") >> pending_supervisor_flag >> create_new_user
        if_supervisor_present_as_user_in_feed >> rail.Label("No") >> create_new_user
        ...
        if_orgrole_code_exists >> rail.Label("No") >> if_timeoff_types_exist_before_create
        
        is_resource_pool_present_in_replicon >> rail.Label("Yes") >> assign_resource_pool_to_user >> if_timeoff_types_exist_before_create
        is_resource_pool_present_in_replicon >> rail.Label("No") >> if_timeoff_types_exist_before_create
        
        # Timeoff processing after user creation and resource pool assignment
        if_timeoff_types_exist_before_create >> rail.Label("Yes") >> get_default_timeoff_policies_for_create >> calculate_timeoff_policy_accruals_for_create >> if_timeoff_policies_calculated
        if_timeoff_types_exist_before_create >> rail.Label("No") >> process_user_logging
        
        if_timeoff_policies_calculated >> rail.Label("Yes") >> assign_timeoff_policies_to_user >> get_pending_supervisor_flag >> if_pending_supervisor_flag
        if_pending_supervisor_flag >> rail.Label("Yes") >> write_supervisor_pending_logs >> process_user_logging
        if_pending_supervisor_flag >> rail.Label("No") >> process_user_logging
        if_timeoff_policies_calculated >> rail.Label("No") >> process_user_logging
        
        process_user_logging >> if_user_created_with_errors
        
        # Time off assignment flow (COMMENTED OUT - now handled in create_new_user payload)
        # if_timeoff_types_exist >> rail.Label("Yes") >> calculate_timeoff_policy_accruals >> if_timeoff_policies_calculated
        # if_timeoff_types_exist >> rail.Label("No") >> process_user_logging >> if_user_created_with_errors
        
        # if_timeoff_policies_calculated >> rail.Label("Yes") >> prepare_timeoff_policy_assignments >> if_policy_assignments_ready
        # if_timeoff_policies_calculated >> rail.Label("No") >> process_user_logging >> if_user_created_with_errors
        
        # if_policy_assignments_ready >> rail.Label("Yes") >> assign_timeoff_policies_to_user >> process_user_logging >> if_user_created_with_errors
        # if_policy_assignments_ready >> rail.Label("No") >> process_user_logging >> if_user_created_with_errors
        
        # User creation logging flow
        if_user_created_with_errors >> rail.Label("Yes") >> write_added_user_with_exceptions_logs >> finish_user_creation
        if_user_created_with_errors >> rail.Label("No") >> write_added_user_logs >> finish_user_creation
