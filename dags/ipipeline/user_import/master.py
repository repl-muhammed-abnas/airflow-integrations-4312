...
from ipipeline.user_import.tasks import process_user_groups_data, process_user_prerequisites
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        archive_reference_file = rail.SFTPMoveFileOperator(
            ...
            new_filename=config.archive_reference_filepath + 'Archive_{{ dag_run_ecid() }}_' + config.reference_filename,
        ...
        create_mapped_collection = rail.CreateCollectionOperator(
            ...
            columns=custom_methods.get_input_columns(config.input_fields_mapper_data),
        ...
        create_csv_with_hash = rail.WriteCSVFileOperator(
            ...
            header=[
                ...
                'elt', 'uksick', 'transfer_date', 'employee_type', 'paygroup', 'project',
        ...
        identify_changed_records = rail.QueryCollectionOperator(
            ...
            query=request_payload.get_changed_records_query(config.can_use_reference_file_var_name),
        ...
        get_all_groups_data = process_user_groups_data.get_all_groups_data("all")

        get_all_prerequisites_data = process_user_prerequisites.get_all_prerequisites_data(config)
        ...
        analyze_departments_to_create = rail.PythonOperator(
            ...
            python_callable=lambda: custom_methods.analyze_departments_to_create(config.defaults_mapper_data["root_department"])
        )
        ...
        def gather_all_the_run_ids_callable():
            ...
            for result in results:
                if result:  # Skip None and empty lists
                    run_ids.extend(result)  # result should always be a list here
        ...
        get_updated_groups_data = process_user_groups_data.get_all_groups_data("updated")
        ...
        process_user_record = rail.trigger_parallel_dagrun(
            ...
            conf=lambda item: {
                **custom_methods.create_users_payload_from_variable(item, config),
                ...
                "oef_data": {
                    oef["oef_name"]: rail.result("get_all_user_oefs").get(f"{oef['field_name']}_oef_uri")
        ...
        is_csv >> rail.Label("Yes") >> download_input_file >> was_new_file_found
        ...
        parse_csv >> can_use_reference_file >> rail.Label("Yes") >> download_reference_file
        ...
        check_for_changes >> rail.Label("Yes") >> start_batch_task >> can_run_batch_task
        check_for_changes >> rail.Label("No") >> upload_reference_file
        
        can_run_batch_task >> rail.Label("Yes") >> batch_task >> start_user_processing
        ...
        check_if_departments_need_creation >> rail.Label("No") >> check_if_locations_need_creation
        ...
        check_if_locations_need_creation >> rail.Label("No") >> check_if_employee_types_need_creation
        ...
        check_if_employee_types_need_creation >> rail.Label("No") >> check_if_project_roles_need_creation
        ...
        check_if_project_roles_need_creation >> rail.Label("No") >> gather_all_the_run_ids
        
        gather_all_the_run_ids >> wait_for_groups_creation >> process_updated_groups_data >> get_updated_groups_data >> start_user_processing
        ...
        start_user_processing >> process_user_record
        ...
        if_filtered_pending_supervisor_records >> rail.Label("No") >> format_logs
