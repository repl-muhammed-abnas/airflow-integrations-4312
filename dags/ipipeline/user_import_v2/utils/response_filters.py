...
def get_required_policy_set_uris_data(response, assignment_rules_mapper_data):
    """
    Extracts required timesheet templates, overtime request templates from the response based on the iPipeline mapper data.
    Returns name:uri format.
    """
    # Get all unique timesheet,overtime request templates from mapper
    unique_templates = set()

    for rule in assignment_rules_mapper_data:
        timesheet_template_name = rule.get('timesheet_template')
        if timesheet_template_name:  # Skip empty templates
            unique_templates.add(timesheet_template_name)
        overtime_request_template_name = rule.get('overtime_request_template')
        if overtime_request_template_name:  # Skip empty templates
            unique_templates.add(overtime_request_template_name)

    # Get only those timesheet, overtime request templates from response in name:uri format
    final_templates = {}
    for template in response:
        if template['displayText'] in unique_templates:
            final_templates[template['displayText']] = template['uri']
