...
def create_process_each_user_child(config):
    with rail.create_airflow_dag(
        ...
        get_invalid_update_user_input_details = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: custom_methods.get_invalid_user_input_details(
                dag_run, true, config.input_fields_mapper_data)
        )
        ...
        process_update_user = rail.TriggerDagRunOperator(
            ...
            conf=lambda dag_run: request_payload.get_add_update_user_conf(
                dag_run, config, "update")
        )
        ...
        get_invalid_new_user_input_details = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: custom_methods.get_invalid_user_input_details(
                dag_run, false, config.input_fields_mapper_data)
        )
        ...
        catch_and_log_errors = rail.WriteLogOperator(
            ...
            log='{{ result("user_child_log") }}',
            message=lambda: "User not processed for the following reason/s " +
                custom_methods.get_error_message(),
            severity="Error",
        ...
        if_employee_id_present >> rail.Label(
            "Yes") >> get_user_details >> if_user_exists
        if_employee_id_present >> rail.Label(
            "No") >> log_employee_id_missing >> catch_and_log_errors
        if_user_exists >> rail.Label(
            "Yes") >> get_invalid_update_user_input_details >> if_missing_update_mandatory_fields
        if_missing_update_mandatory_fields >> rail.Label(
            "Yes") >> log_missing_update_mandatory_fields >> catch_and_log_errors
        if_missing_update_mandatory_fields >> rail.Label(
            "No") >> process_update_user >> wait_for_process_update_user >> catch_and_log_errors
        if_user_exists >> rail.Label(
            "No") >> get_invalid_new_user_input_details >> if_missing_add_user_mandatory_fields
        if_missing_add_user_mandatory_fields >> rail.Label(
            "Yes") >> log_missing_add_user_mandatory_fields >> catch_and_log_errors
        if_missing_add_user_mandatory_fields >> rail.Label(
