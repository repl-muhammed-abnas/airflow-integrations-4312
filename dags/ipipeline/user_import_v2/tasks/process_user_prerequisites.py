from ipipeline.user_import_v2.utils import response_filters
import rail
null = None
...
def get_all_prerequisites_data(config):
    with rail.TaskGroup(
        ...
        get_required_policy_set_uris = rail.RepliconServiceOperator(
            task_id="get_required_policy_set_uris",
            endpoint="/services/PolicySetService1.svc/GetAllPolicySets",
            data_handler=lambda response: response_filters.get_required_policy_set_uris_data(response, config.assignment_rules_mapper_data)
        )

        get_all_ot_request_approval_paths = rail.RepliconServiceOperator(
            task_id="get_all_ot_request_approval_paths",
        ...
        start_prerequisites >> get_required_holiday_calendars >> get_required_time_off_types >> get_required_permission_sets \
            >> get_required_policy_set_uris >> get_all_ot_request_approval_paths >> get_required_activities >> get_required_payrules \
