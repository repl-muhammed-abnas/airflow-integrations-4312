...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.annual_run_master,
        description=f'iPipeline | YEAR END POLICY LINE - CUSTOM SCRIPT - Annual Master {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=dt(2024, 1, 1, tz=config.time_zone),
        schedule_interval=config.schedule_interval_annual,
        ...
        def can_trigger_run(dag_run):
            ...
            #     (DATE_IN_REQUIRED_TIMEZONE.strftime("%Y/%m/%d") == DATE_IN_REQUIRED_TIMEZONE.strftime("%Y") + "/12/15"))
        ...
        dag_run_log_time_info = rail.PythonOperator(
            ...
            python_callable=lambda: {
                ...
                'dag_run_date': str(int(DATE_IN_REQUIRED_TIMEZONE.strftime("%Y")) + 1) + "/01/01",
        ...
        get_report_details = rail.RepliconReportDetailsOperator(
            ...
            report_name=config.annual_timeoff_report
        )
        ...
        #     python_callable=lambda: rail.load_all_records(rail.result('query_uniq_timeoff_uris'))
        # )
        ...
        process_log_generation = rail.TriggerDagRunOperator(
            ...
            conf={
                ...
                'log_filename': "{{get_company_key()}}_year_end_policy_line_{{result('dag_run_log_time_info').current_date_time }}.csv",
            }
        )
