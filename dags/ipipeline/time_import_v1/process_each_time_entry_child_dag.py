...
from ipipeline.time_import_v1.utils import request_payload, response_filters, custom_methods
import rail
    ...
    with rail.create_airflow_dag(
       ...
       # Task: Check if batch processing mode is enabled
        ...
        check_user_resource_at_task_level = rail.IfOperator(
            ...
            yes_task="get_task_and_project_billable_details",
        ...
        get_task_and_project_billable_details = rail.RepliconServiceOperator(
            task_id='get_task_and_project_billable_details',
            endpoint='/services/TaskService1.svc/GetTaskDetails',
            data=lambda dag_run: {
                "taskUri": rail.find_first_by_attr_and_get_attr(rail.result(
                        'get_required_tasks_for_project'), 'full_task_name', dag_run.conf['task_type'], 'uri', '')
            },
            data_handler=custom_methods.get_is_billable_status
        )
       ...
       # DAG dependencies
        ...
        check_user_resource_at_task_level >> rail.Label(
            'Yes') >> get_task_and_project_billable_details

        get_task_and_project_billable_details >> add_time_entry
