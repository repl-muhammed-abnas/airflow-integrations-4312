...
from ipipeline.project_client_import.utils import custom_methods
...
def create_log_generation_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        description=f'iPipeline Project Client Import Process Log Generation - {config.dag_id_suffix}',
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_logs').total_error_logs_count == 0 -%}\
                    "+config.internal_logs_email+"\
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Replicon Project and Client Import from Salesforce - " }} \
                {%- if result("format_logs").total_error_logs_count > 0 -%} \
                    completed with errors  \
                {%- else -%} \
                    {%- if result("format_logs").total_exception_logs_count > 0 -%} \
