    ...
    with rail.create_airflow_dag(
        ...
        log_no_parent_found_for_task = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                'details': f"Parent (Story) not found for Task issue id {dag_run.conf.get('issue_id')}"
            }
        )
        ...
        log_no_epic = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                'action': 'Task Metadata Retrieval',
                'task_processing_status': 'Exception',
                'details': f"GrandParent (Epic) not found for Task issue id {dag_run.conf.get('issue_id')}"
            }
        )
