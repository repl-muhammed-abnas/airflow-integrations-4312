...
from strayeruniversity.user_sync_v4.utils.python_callable import get_ref_file_name, get_inp_file_name, get_inp_file_name_no_changed_records
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        description=f'strayeruniversity_usersync_master',
        ...
        process_each_managementlevel = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=config.child_managementlevel_customfield_check_dag_id,
        ...
        get_inp_filepath_name_for_no_changed_records = rail.PythonOperator(
            ...
            python_callable=lambda: get_inp_file_name_no_changed_records(
        ...
        process_each_user = rail.trigger_parallel_dagrun(
            ...
            trigger_dag_id=config.child_proecss_each_user_dag_id,
        ...
        write_supervisor_assignment_log_file = rail.WriteCSVFileOperator(
            ...
            header=['username', 'managername',
                    'useruri', 'emplid', 'user_log'],
            row=lambda item: [
        ...
        process_each_mapper_data = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.child_update_supervisor_dag_id,
        ...
        write_supervisor_assignment_log_file >> load_master_log >> check_supervisor_mapper_csv_has_data

        check_supervisor_mapper_csv_has_data >> rail.Label(
            'Yes') >> process_each_mapper_data >> wait_for_mapper_data_process >> format_logs
        check_supervisor_mapper_csv_has_data >> rail.Label(
            'No') >> format_logs

        format_logs >> archive_reference_file >> list_input_files >> get_inp_filepath_name >> is_inp_csv

        is_inp_csv >> rail.Label(
            'Yes') >> set_inputfile_as_new_reference >> write_userimportlog_file
        is_inp_csv >> rail.Label('No') >> write_userimportlog_file

        write_userimportlog_file >> check_userimportlog_csv_has_data

        check_userimportlog_csv_has_data >> rail.Label(
            'Yes') >> get_log_file_name >> generate_downloadlink
        check_userimportlog_csv_has_data >> rail.Label('No') >> finish

        generate_downloadlink >> send_import_complete_email >> finish
