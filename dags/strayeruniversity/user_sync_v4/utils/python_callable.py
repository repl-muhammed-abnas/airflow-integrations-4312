...
def get_schedulename(dag_run, config):
    return list(filter(lambda x: x['employeetype'] == dag_run.conf['employeetype'] and
                       x['scheduledhours'] == dag_run.conf['scheduledhours'], config.SCHEDULE_MAPPER))


def get_timeoff_tobe_assigned(dag_run, config):
    ...
    return list(filter(lambda x: x['division'] == dag_run.conf['division'] and
                       ...
                       x['scheduledhours'] == rail.result('get_scheduledhours_var_val'), config.DYNAMIC_TIMEOFF_MAPPER))


def get_timeoff_tobe_assigned_for30(dag_run, config):
    return list(filter(lambda x: x['division'] == dag_run.conf['division'] and
                       x['scheduledhours'] == rail.result('get_scheduledhours_var_val_for30'), config.STATIC_TIMEOFF_MAPPER))


def get_statictimeoff_with_scheduled_all(dag_run, config):
    return list(filter(lambda x: x['division'] == dag_run.conf['division'] and
                       x['scheduledhours'] == 'All', config.STATIC_TIMEOFF_MAPPER))
...
def update_user_log():
    ...
    user_logs_from_supervisor_dag = rail.load_all_records(
        rail.result('create_temporary_user_logs_for_supervisor_assignment'))

    for item in user_logs_from_supervisor_dag:
        for entry in formatted_logs:
            if item['properties']['username'] == entry['username']:
                if "Exception" in item['properties']['status']:
                    entry['status'] = "Error" if "Error" in entry['status'] else "Exception"
                    entry['details'] = rail.smartjoin_by_delim(
                        (str(entry['details']) + ',' + item['properties']['details']).split(','), ";")
                    break
                if "Error" in item['properties']['status']:
                    entry['status'] = "Error"
                    entry['details'] = entry['details'] + \
                        ";" + item['properties']['details']
                    break
                if "Success" in item['properties']['status']:
    ...
    return {
        "get_record_summary": get_record_summary(formatted_logs),
        "final_logs": json.dumps(formatted_logs, ensure_ascii=False)
    }
