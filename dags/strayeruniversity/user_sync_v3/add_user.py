...
import rail
from strayeruniversity.user_sync_v3.utils import request_payload, python_callable
from strayeruniversity.user_sync_v3.utils.python_callable import get_today, get_substitueUserUris, get_substitueUser_fromsearch, get_policyschedule_entries
from strayeruniversity.user_sync_v3.mappers.strayer_payrule_mapper import payrule_mapper
from strayeruniversity.user_sync_v3.mappers.strayer_timezone_mapper import timezone_mapper

def create_dag(config):
    # pylint: disable=too-many-statements
    with rail.create_airflow_dag(
        dag_id=f'strayeruniversity_usersync_add_user_child_v3_{config.instance}',
        description=f'strayeruniversity_usersync_add_user_child_v3_{config.instance}',
        ...
        put_payrule_script_assignment_schedule = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "scheduleEntries": [{
                    ...
                    "effectiveDate": request_payload.effective_dateformat_payload(datetime.strptime(dag_run.conf['hiredate'], '%d-%b-%Y'))
                }]
            }
        )
        ...
        get_schedulename_from_mapper = rail.PythonOperator(
            ...
            python_callable=python_callable.get_schedulename
        )
        ...
        trigger_customfield_dropdown_update_emptype = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=f'strayeruniversity_process_customfield_for_dropdown_child_v3_{config.instance}',
        ...
        trigger_createsubtituteuser_strayeruniversity = rail.TriggerDagRunOperator(
            ...
            trigger_dag_id=f'strayeruniversity_usersync_assign_substitute_user_child_v3_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        get_timeoff_to_be_assigned_from_mapper = rail.PythonOperator(
            ...
            python_callable=python_callable.get_timeoff_tobe_assigned
        )
        ...
        get_timeoff_to_be_assigned_from_mapper_for30 = rail.PythonOperator(
            ...
            python_callable=python_callable.get_timeoff_tobe_assigned_for30
        )
        ...
        get_statictimeoff_with_scheduled_as_all = rail.PythonOperator(
            ...
            python_callable=python_callable.get_statictimeoff_with_scheduled_all
        )
