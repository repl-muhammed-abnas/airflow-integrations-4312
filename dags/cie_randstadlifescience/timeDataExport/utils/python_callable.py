...
def get_filtered_data(config,  currentDateTimeStr):
    ...
    to_excluded_TS = rail.load_all_records(
        rail.result("load_timedata_csv"))

    list_to_df = pd.DataFrame(to_excluded_TS)
    ...
    if excludedTS_details:
        ...
        if is_future_TS_available_in_file:
            ...
            if not excludedTS_details1.empty:
                updated_dataframe = excludedTS_details1
    new_list = rail.result('generate_final_report_data').get('excluded_data')
...
def create_timesheet_uri_str():
    ...
    processed_data = json.loads(rail.result(
        "generate_final_report_data").get('processed_data'))
...
def getmin_max():
    ...
    # datetime(datetime_now.year-1, datetime_now.month, datetime_now.day)
...
def get_user_list():
    ...
    filter_details.append({
        "reportFilterUri": rail.find_first_by_attr_and_get_attr(
            rail.result('get_timesheetaudit_report_details')['filterConfiguration']['enabledFilters'], 'displayText', "TimesheetPeriodFilter", 'uri'),
        "value": (datetime.strptime(rail.result('min_max_date')["min_date"], '%m/%d/%Y') - timedelta(days=10)).strftime('%m/%d/%Y'),
    })
    filter_details.append({
        "reportFilterUri": rail.find_first_by_attr_and_get_attr(
            rail.result('get_timesheetaudit_report_details')['filterConfiguration']['enabledFilters'], 'displayText', "TimesheetPeriodFilter", 'uri'),
        "value": (datetime.strptime(rail.result('min_max_date')["max_date"], '%m/%d/%Y') + timedelta(days=10)).strftime('%m/%d/%Y'),
    })
    user_list = rail.load_all_records(
        rail.result("query_unique_users"))
    user_filter = rail.find_first_by_attr_and_get_attr(
        rail.result('get_timesheetaudit_report_details')['filterConfiguration']['enabledFilters'], 'displayText', "UserFilter", 'uri')
    new_list = [filter_details.append({"reportFilterUri": user_filter,
                "value": entry.get('UserUri').split(':')[-1], }) for entry in user_list]
    return filter_details


def get_report_data_to_csv():
    ...
    if artifact_futureTS and artifact_pastTS:
        ...
        final_artifact = artifact_futureTS if artifact_futureTS else artifact_pastTS
        report_output = rail.load_json_artifact(final_artifact)
        csv_string = report_output.get('reportGenerationResults')[
            0].get('payload')
