...
import pendulum
import rail
from cie_randstadlifescience.timeDataExport.utils import upload_to_s3, download_from_s3, python_callable
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'CIE_{config.company_key}_timeDataExport_master_{config.instance}',
        ...
        future_TS_from_report1 = rail.QueryCollectionOperator(
            task_id='future_TS_from_report1',
            query="""SELECT DISTINCT TimesheetUri, Timesheet_End_Date, Approval_Date_Time FROM timedata""",
        )
        ...
        get_user_filter_details = rail.PythonOperator(
            task_id='get_user_filter_details',
            python_callable=python_callable.get_user_list
        )
                ...
                "reportParameters": [
                    {
                        ...
                        "filterValues": json.loads(json.dumps(rail.result('get_user_filter_details'))),
                        "outputFormatUri": "urn:replicon:report-output-format-option:csv"
                    }
                ]
            },
                ...
                "reportParameters": [
                    {
                        ...
                        "filterValues": [
                            ...
                            },
                            {
                                "reportFilterUri": rail.find_first_by_attr_and_get_attr(
                                    rail.result('get_timeentry_report_details')['filterConfiguration']['enabledFilters'], 'displayText', "EntryDateFilter", 'uri'),
                                "value": rail.result('min_max_date')["min_date"],
                            },
                            {
                                "reportFilterUri": rail.find_first_by_attr_and_get_attr(
                                    rail.result('get_timeentry_report_details')['filterConfiguration']['enabledFilters'], 'displayText', "EntryDateFilter", 'uri'),
                                "value": rail.result('min_max_date')["max_date"],
                            },
                        ],
                        "outputFormatUri": "urn:replicon:report-output-format-option:csv"
                    }
        ...
        generate_final_report_data = rail.PythonOperator(
            ...
            python_callable=python_callable.get_filtered_data,
            op_args=[config, currentDateTimeStr]
        )

        final_report_has_data = rail.IfOperator(
            task_id='final_report_has_data',
            test="{{ result('generate_final_report_data').get('processed_data') != '' }}",
        ...
        write_data_to_csv = rail.WriteCSVFileOperator(
            ...
            source="{{ result('generate_final_report_data').get('processed_data') }}",
            delimiter="|",
            header=col_names,
            ...
            message=log_timestamp+'{{ "Process started." }}\
                {%- if result("get_all_report") | is_truthy -%} \
                    ...
                    {%- if result("get_all_report") | is_truthy -%} \
                        ...
                        '+log_timestamp+'{{ "Report - ' + config.audit_report_name + ' Data processed" if result("generate_report_2_in_batch.get_report_result","has_data")  else "No Data available for Report - ' + config.audit_report_name + '" }} \
                        {%- if result("get_all_report") | is_truthy -%} \
                            '+log_timestamp+'{{ "Report - ' + config.entrydata_report_name + '" }}\
                            '+log_timestamp+'{{ "Report - ' + config.entrydata_report_name + ' Data processed" if result("generate_base_time_entry_data_report_in_batch.get_report_result","has_data")  else "No Data available for Report - ' + config.entrydata_report_name + '" }} \
        ...
        report_and_file_has_data >> rail.Label(
            'Yes') >> load_timedata_csv >> create_timedata_collection >> query_unique_dates >> query_unique_users >> future_TS_from_report1 >> min_max_date >> get_timesheetaudit_report_details >> get_user_filter_details >> generate_report_2_in_batch >> report_has_timesheet_audit_data

        report_and_file_has_data >> rail.Label('Yes') >> finish

        future_ts_file_has_data >> rail.Label('No') >> report_and_file_has_data >> rail.Label(
            'Yes') >> load_timedata_csv >> create_timedata_collection >> query_unique_dates >> query_unique_users >> future_TS_from_report1 >> min_max_date >> get_timesheetaudit_report_details >> get_user_filter_details >> generate_report_2_in_batch >> report_has_timesheet_audit_data

        report_has_timesheet_audit_data >> rail.Label(
            'No') >> finish
        report_has_timesheet_audit_data >> rail.Label(
            'Yes') >> get_timeentry_report_details >> generate_base_time_entry_data_report_in_batch >> report_has_entrydata

        report_has_entrydata >> rail.Label(
            'No') >> finish
        report_has_entrydata >> rail.Label(
            'Yes') >> get_processed_TimesheetUris >> generate_final_report_data >> final_report_has_data

        final_report_has_data >> rail.Label(
            'No') >> remove_processed_excludedTs >> update_excludedTS_details_file >> finish
        final_report_has_data >> rail.Label(
            'Yes') >> write_data_to_csv >> upload_csv_to_sftp >> create_timesheet_uri_content >> update_timesheet_uris_file >> remove_processed_excludedTs >> update_excludedTS_details_file >> finish
