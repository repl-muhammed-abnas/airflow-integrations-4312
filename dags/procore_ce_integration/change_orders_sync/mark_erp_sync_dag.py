...
def create_dag_instance(config):  # pylint: disable=too-many-statements
    with rail.create_airflow_dag(
        dag_id=f'procore_computerease_budget_revision_mark_erp_sync_{config.instance}',
        description='Procore Change Order Sync - Mark ERP Sync (set origin_id after CE import acceptance)',
        ...
        def resolve_pending():
            ...
            for row in pending:
                ...
                if import_uuid in accepted:
                    ...
                    # Scope delete to this exact import so a newer re-queued row (same change
                    # event, different import_uuid) upserted after query_pending survives.
                    delete_params.append([change_event_id, import_uuid])
                elif import_uuid in rejected:
                    print(f"Origin ID not set: ComputerEase import '{import_uuid}' for "
                          f"change event '{change_event_id}' was rejected; link abandoned.")
                    delete_params.append([change_event_id, import_uuid])
            return {
                'updates': updates,
        ...
        # Change order writes origin_id per change event, so PATCH each accepted row.
        ...
        patch_origin_id = rail.ProcoreApiOperator(
            ...
            endpoint="/change_events/{{ result('for_each_set_origin_id').change_event_id }}",
            method='PATCH',
            query_params={
                'project_id': "{{ result('for_each_set_origin_id').project_id }}"
            },
            data=lambda: {
                'project_id': rail.result('for_each_set_origin_id')['project_id'],
                'change_event': {
        ...
        delete_resolved_rows = rail.S3UpdateCollectionOperator(
            ...
            query=f'DELETE FROM {collection_name} WHERE change_event_id = ? AND import_uuid = ?',
