...
from procore_ce_integration.ap_invoice_sync_v2.utils.constants import RESOURCE_DRAW_REQUESTS

def create_dag_instance(config):
    with rail.create_airflow_dag(
        dag_id=config.webhook_dag_id,
        description='Procore AP Invoice Webhook Processing - Store Events to S3',
        ...
        def extract_webhook_data(dag_run):
            ...
            for key, value in payload.items():
                ...
                if key == 'resource_name' and value != RESOURCE_DRAW_REQUESTS:
        ...
        def add_new_event(existing_events, webhook_data):
            if webhook_data['project_id'] and webhook_data['resource_id']:
                key = f"{webhook_data['project_id']}.{webhook_data['resource_id']}.{RESOURCE_DRAW_REQUESTS}"
            else:
        ...
        def prepare_events_list():
            ...
            return {
                ...
                'events_json': json.dumps(updated_events, indent=1),
