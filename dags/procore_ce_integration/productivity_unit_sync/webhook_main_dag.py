...
def create_dag_instance(config):  # pylint: disable=too-many-statements
    with rail.create_airflow_dag(
        ...
        description='Procore Productivity Logs Webhook Processing - Store Events to S3',
        ...
        def extract_webhook_data(dag_run):
            ...
            if extracted_data['resource_name'] != RESOURCE_PRODUCTIVITY_LOG:
        ...
        def add_new_event(events_dict, webhook_data):
            if webhook_data['project_id'] and webhook_data['resource_id']:
                key = f"{webhook_data['project_id']}.{webhook_data['resource_id']}.{RESOURCE_PRODUCTIVITY_LOG}"
            else:
