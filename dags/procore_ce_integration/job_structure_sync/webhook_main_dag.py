...
def create_dag_instance(config):  # pylint: disable=too-many-statements
    with rail.create_airflow_dag(
        ...
        description='Procore Job Structure Webhook Processing - Store Events to S3',
        ...
        def extract_webhook_data(dag_run):
            ...
            if extracted_data['resource_name'] == 'Cost Codes' and not extracted_data['project_id']:
                raise ValueError(
                    "Missing required field: project_id for Cost Codes")
            if extracted_data['resource_name'] == 'Projects' and not extracted_data['project_id']:
                extracted_data['project_id'] = extracted_data['resource_id']
            if extracted_data['resource_name'] not in ['Projects', 'Cost Codes', 'Budget Line Items', 'Prime Contracts']:
                extracted_data['skip_processing'] = True
            else:
        ...
        def add_new_event(events_dict, webhook_data):
            if webhook_data['resource_name'] == 'Projects' and webhook_data['project_id']:
                key = str(webhook_data['project_id'])
            elif webhook_data['resource_name'] in ['Cost Codes', 'Budget Line Items', 'Prime Contracts'] and webhook_data['project_id'] and webhook_data['resource_id']:
                key = f"{webhook_data['project_id']}.{webhook_data['resource_id']}.{webhook_data['resource_name']}"
            else:
