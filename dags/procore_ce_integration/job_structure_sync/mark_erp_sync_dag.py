...
from procore_ce_integration.job_structure_sync.utils.constants import RESOURCE_JOBS


def create_dag_instance(config):  # pylint: disable=too-many-statements
    with rail.create_airflow_dag(
        dag_id=f'procore_ce_job_structure_mark_erp_sync_dag_{config.instance}',
        description='Procore Job Structure - Mark ERP Sync (set origin_id after CE import acceptance)',
        ...
        def resolve_pending():
            ...
            for row in pending:
                ...
                if import_uuid in accepted:
                    updates.append({
                        'id': int(row['procore_project_id']),
                        'origin_id': row.get('origin_id'),
                        'project_number': project_number
                    })
                    delete_params.append([project_number])
                elif import_uuid in rejected:
                    print(f"Origin ID not set: ComputerEase import '{import_uuid}' for "
                          f"project number '{project_number}' was rejected; link abandoned.")
                    delete_params.append([project_number])
            return {
                'patch_payload': {
        ...
        delete_resolved_rows = rail.S3UpdateCollectionOperator(
            ...
            query=f'DELETE FROM {collection_name} WHERE project_number = ?',
