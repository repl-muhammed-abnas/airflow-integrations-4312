import rail
from bccsstechnologyservices.report_to_sftp.utils.custom_methods import logging_details
...
def process_report_to_sftp(config,report_name,file_name):
    with rail.TaskGroup(group_id='process_report_to_sftp', prefix_group_id=False):
        ...
        report_has_data = rail.IfOperator(
            ...
            yes_task='move_existing_report_archive',
            no_task='send_no_data_mail',
        )

        move_existing_report_archive = rail.SFTPMoveFileOperator(
            task_id='move_existing_report_archive',
            new_filename=config.extract_report_archive_file_path + file_name + '_{{ result("get_logging_details")["dag_start_time"] }}.csv',
            existing_filename=config.extract_report_file_path + file_name + '.csv',
        )
        ...
        upload_report_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.extract_report_file_path + file_name + '.csv',
        )
        ...
        is_report_failed >> rail.Label("No") >> report_has_data >> rail.Label(
            "Yes") >> move_existing_report_archive >> load_report_data >> upload_report_to_sftp >> log_to_sumo
