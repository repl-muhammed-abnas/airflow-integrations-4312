...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        query_invalid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM report_data_collection WHERE NULLIF(login_name, '') IS NULL or (timeoff_balance == '0.00' AND NULLIF(login_name, '') IS NOT NULL)""",
        ...
        log_invalid_records = rail.WriteLogOperator(
            ...
            properties= lambda item: {
                ...
                "details": "Annual Leave Balance Transfer Not Processed as Login Name is not present in record" if not item['login_name'] else \
                    "Annual Leave Balance Transfer Not Processed as time off balance is 0 for the time off type- {} as of date {}".format\
        ...
        def get_balace_to_transfer(balance):
            return[
                {
                    'name': config.ANNUAL_LEAVE_ADDITIONAL,
                    'balance': "5.0" if balance >=5 else balance
                },
                {
                    'name': config.ANNUAL_LEAVE_CARRIED_OVER,
                    'balance': str(float(balance-5)) if balance > 5 else "0.0"
                }
            ]
        ...
        trigger_dag_run_transfer_timeoff_balance = rail.trigger_parallel_dagrun(
            ...
            conf=lambda item: {
                ...
                "balance_to_transfer": get_balace_to_transfer(float(item['timeoff_balance'])),
