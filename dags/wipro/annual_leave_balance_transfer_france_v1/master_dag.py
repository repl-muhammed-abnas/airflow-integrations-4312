...
from wipro.annual_leave_balance_transfer_france_v1.utils import request_payload, response_filters
import rail
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        trigger_whit_monday = rail.TriggerDagRunOperator(
            task_id='trigger_whit_monday',
            trigger_dag_id=config.whit_monday_master_dag,
            conf={'skip_rundate_validation': True},
            execution_timeout=timedelta(days=config.execution_timeout_days),
        )
        ...
        create_balance_transfer_logs >> get_required_timeoff_type_uris >> get_country_servicecenter_uri \
        ...
        query_distinct_loginname_records >> trigger_dag_run_transfer_timeoff_balance >> compose_logs_csv \
         >> generate_download_link >> check_for_error_log >> check_for_exception_log >> send_import_complete_email >> can_fail_dag

        can_fail_dag >> rail.Label("Yes") >> fail_dagrun
        can_fail_dag >> rail.Label("No") >> trigger_whit_monday

        [fail_dagrun, trigger_whit_monday] >> log_to_sumo
