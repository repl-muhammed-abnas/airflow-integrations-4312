...
def get_put_table_view_setting_supervisor(user_uri, caller='supervisor', project_manager_flag="N"):
    with rail.TaskGroup(group_id=f'put_table_view_setting_{caller}', prefix_group_id=False) as put_table_view_setting_supervisor:
        ...
        put_column_settings_for_pm_timesheet_tab = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run:request_payload.get_put_column_settings_for_pm_timesheets_data(
                rail.render_template(user_uri),"myTeamTimeSheet_list", dag_run),
        ...
        put_column_settings_for_supervisor_timesheet_tab = rail.RepliconServiceOperator(
            ...
            endpoint='/services/ListSettingsService1.svc/PutColumnSettingsForUser',
            data=lambda dag_run:request_payload.get_put_column_settings_for_user_timesheets_data(
                rail.render_template(user_uri), "myTeamTimeSheet_list", dag_run),
            headers=lambda: rail.result(
                f'impersonate_and_create_interactive_session_{caller}'),
        )

        put_columnsettings_for_supervisor_approvals = rail.RepliconServiceOperator(
            task_id=f'put_columnsettings_for_supervisor_approvals_{caller}',
            endpoint='/services/ListSettingsService1.svc/PutColumnSettingsForUser',
            data=lambda dag_run:request_payload.get_put_column_settings_for_user_approvals_data(rail.render_template(user_uri)),
            headers=lambda: rail.result(
                f'impersonate_and_create_interactive_session_{caller}'),
        )
        ...
        put_columnsettings_for_user_approvals = rail.RepliconServiceOperator(
            ...
            endpoint='/services/ListSettingsService1.svc/PutColumnSettingsForUser',
            data=lambda dag_run:request_payload.get_put_column_settings_for_pm_timesheets_data(
                rail.render_template(user_uri),"timeSheetForApproval_list",dag_run),
            headers=lambda: rail.result(
                f'impersonate_and_create_interactive_session_{caller}'),
        )
        ...
        if_supervisor_type_pm >> rail.Label("No") >>\
            ...
            put_columnsettings_for_supervisor_approvals >> supervisor_process_complete
