...
from wipro.user_import_united_kingdom_v2.task import process_user_group_data
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        query_valid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM united_kingdom_users WHERE NULLIF("employee_id","") IS NOT NULL AND
        ...
        query_invalid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM united_kingdom_users WHERE NULLIF("employee_id","") IS NULL OR
        ...
        if_valid_user_records = rail.IfOperator(
            ...
            no_task="end_united_kingdom_user"
        )
        ...
        query_to_identify_new_entity = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT *,
                    ...
                    FROM valid_users""",
            name="united_kingdom_valid_users"
        )
        ...
        process_user_for_united_kingdom = rail.trigger_parallel_dagrun(
            task_id="process_user_for_united_kingdom",
            ...
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item, config=config: custom_methods.get_united_kingdom_user_conf(
                item, config)
        )
        ...
        process_supervisor_for_united_kingdom = rail.trigger_parallel_dagrun(
            task_id="process_supervisor_for_united_kingdom",
            items='{{result("load_supervisor_data")|to_json}}',
            trigger_dag_id=config.create_supervisor_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item, config=config: custom_methods.get_united_kingdom_supervisor_conf(
                item, config)
        )

        end_united_kingdom_user = rail.EmptyOperator(
            task_id="end_united_kingdom_user"
        )

        process_logs_for_united_kingdom = rail.TriggerDagRunOperator(
            task_id="process_logs_for_united_kingdom",
        ...
        if_invalid_users >> rail.Label("No") >>\
            if_valid_user_records >> rail.Label("Yes") >> query_to_identify_new_entity >> groups_data >>\
            get_all_prerequisite_data >> \
            create_location >>\
            get_all_location_hierarchy >>\
            process_user_start >>\
            process_user_for_united_kingdom >> get_supervisor_pending_logs >>\
            load_supervisor_data >> process_supervisor_for_united_kingdom >>\
            end_united_kingdom_user >> process_logs_for_united_kingdom
        if_valid_user_records >> rail.Label(
            "No") >> end_united_kingdom_user
        return dag
