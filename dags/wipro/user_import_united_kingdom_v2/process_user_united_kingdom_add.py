...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        create_united_kingdom_user = rail.RepliconServiceOperator(
            ...
            data=request_payload.get_united_kingdom_create_payload,
        ...
        if_supervisor_details_in_feed = rail.IfOperator(
            ...
            test=lambda dag_run: bool(dag_run.conf["primary_supervisor_id"] or dag_run.conf["project_supervisor_id"]),
        ...
        if_primary_manager_permission = rail.IfOperator(
            ...
            no_task="if_project_manager_permission"
        )
        ...
        if_project_manager_permission = rail.IfOperator(
            task_id="if_project_manager_permission",
            test=lambda dag_run: dag_run.conf["project_manager_flg"] and dag_run.conf["project_manager_flg"] == "Y",
            yes_task="start_pm_table_setting",
            no_task="end_table_setting"
        )

        start_pm_table_setting = rail.EmptyOperator(task_id="start_pm_table_setting")

        put_table_view_setting_for_create_pm = put_supervisor_table_settings.get_put_table_view_setting_supervisor(
            '{{result("create_united_kingdom_user")}}', "supervisor", "Y"
        )
        ...
        assign_all_time_off_types = rail.RepliconServiceOperator(
            ...
            endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_united_kingdom_user")}}'),
        ...
        get_annual_leave_policy = rail.RepliconServiceOperator(
            task_id='get_annual_leave_policy',
            endpoint="/services/TimeOffPolicyService2.svc/GetDefaultTimeOffTypePolicyScheduleForUser",
            data=lambda dag_run:{
                "timeOffAccount": {
                    "userUri": rail.result("create_united_kingdom_user"),
        ...
        put_annual_acquisition_accrued_balance = rail.RepliconServiceOperator(
            ...
            endpoint="/services/TimeOffPolicyService2.svc/PutUserTimeOffAccountPolicySetSchedule",
            data=request_payload.get_uk_annual_acquistion_payload
        )
        ...
        if_user_is_assignee >> rail.Label("No") >>\
            assign_default_timeoff_policy >> get_annual_leave_policy >> put_annual_acquisition_accrued_balance >>\
            time_off_end >>\
            write_added_user_logs >> catch_and_log_errors
        if_primary_manager_permission >> rail.Label("No") >>\
        if_project_manager_permission >> rail.Label("Yes") >> start_pm_table_setting >>\
        put_table_view_setting_for_create_pm >> end_table_setting
        if_project_manager_permission >> rail.Label("No") >> end_table_setting
        return dag
