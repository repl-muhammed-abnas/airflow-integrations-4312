...
from wipro.user_import_saudi_arabia_v2.task import put_user_and_table_settings, put_supervisor_table_settings
from wipro.user_import_saudi_arabia_v2.utils import request_payload, custom_methods, response_filter
import rail
null = None
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task="batch_task",
            no_task="create_saudi_arabia_user"
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id="batch_task",
            start_task="create_saudi_arabia_user",
            end_task="catch_and_log_errors"
        )

        create_saudi_arabia_user = rail.RepliconServiceOperator(
            task_id="create_saudi_arabia_user",
            endpoint="/services/ImportService1.svc/PutUser3",
            data=request_payload.get_saudi_arabia_create_payload,
        ...
        unassign_products_for_user = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "user": {
                    "uri": rail.result("create_saudi_arabia_user"),
                    "loginName": null,
        ...
        put_view_settings_for_user = put_user_and_table_settings.get_put_table_view_setting(
            '{{result("create_saudi_arabia_user")}}', "saudi_arabia", 'user')
        ...
        put_manager_table_view = put_supervisor_table_settings.get_put_table_view_setting_supervisor(
            '{{result("create_saudi_arabia_user")}}', 'supervisor_user', '{{dag_run.conf.project_manager_flg}}')
        ...
        remove_all_time_off_types = rail.RepliconServiceOperator(
            ...
            data={
                    "userUri": '{{result("create_saudi_arabia_user")}}',
                    "timeOffTypeUris": []
            }
        )
        ...
        assign_all_time_off_types = rail.RepliconServiceOperator(
            ...
            endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_saudi_arabia_user")}}'),
        ...
        create_saudi_arabia_user >> unassign_products_for_user >>\
