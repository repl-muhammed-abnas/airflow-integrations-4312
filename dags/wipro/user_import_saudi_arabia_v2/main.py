...
from wipro.user_import_saudi_arabia_v2.task import process_user_group_data
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        query_valid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM saudi_arabia_users WHERE NULLIF("employee_id","") IS NOT NULL AND
            ...
            name="saudi_arabia_valid_users"
        )

        query_invalid_user_records = rail.QueryCollectionOperator(
            task_id="query_invalid_user_records",
            query="""SELECT * FROM saudi_arabia_users WHERE NULLIF("employee_id","") IS NULL OR
        ...
        process_user_for_saudi_arabia = rail.trigger_parallel_dagrun(
            task_id="process_user_for_saudi_arabia",
            items='{{result("query_valid_user_records")}}',
            trigger_dag_id=config.valid_user_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item,config=config:custom_methods.get_saudi_arabia_user_conf(item,config)
        )

        process_user_end = rail.EmptyOperator(task_id="process_user_end")

        process_logs_for_saudi_arabia = rail.TriggerDagRunOperator(
            task_id="process_logs_for_saudi_arabia",
        ...
        if_invalid_users >> rail.Label("No") >>\
            ...
            process_user_start >>\
            process_user_for_saudi_arabia >>\
        process_user_end >> process_logs_for_saudi_arabia >> end_user
