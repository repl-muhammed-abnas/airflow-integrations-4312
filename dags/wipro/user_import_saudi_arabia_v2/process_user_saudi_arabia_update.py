from wipro.user_import_saudi_arabia_v2.task import update_roles_for_user
from wipro.user_import_saudi_arabia_v2.utils import custom_methods, request_payload
    ...
    with rail.create_airflow_dag(
        ...
        get_extension_field_values = rail.PythonOperator(
            ...
            python_callable=lambda: list(map(lambda i: {
                ...
                "textValue": i["textValue"] or i["numericValue"]
        ...
        assign_supervisor_to_employee = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "userUri":rail.result('get_update_user_details')['userDetails']['uri'],
                "supervisorUri": dag_run.conf["supervisor_uri"],
                "dateRange": {
        ...
        if_reversal_date_update = rail.IfOperator(
            ...
            test=lambda dag_run: bool(custom_methods.check_if_custom_field_date_udapte(
                dag_run.conf["reversal_date"], "Reversal Date") or dag_run.conf["enddateupdate_flag"]),
        ...
        update_onsite_end_date = rail.RepliconServiceOperator(
            task_id="update_onsite_end_date",
            endpoint="/services/CustomFieldService1.svc/UpdateDateValue",
            data=lambda dag_run: {
                    "objectUri": rail.result('get_update_user_details')["userDetails"]["uri"],
                    "customFieldUri": dag_run.conf["onsite_end_dateuri"],
                    "value": null
            }
        )
        ...
        get_extension_field_values >> get_current_location_for_the_user >>\
        ...
        check_supervisor_update >> rail.Label("Yes") >> assign_supervisor_to_employee >> if_reversal_date_update
        check_supervisor_update >> rail.Label("No") >>\
        ...
        remove_end_date >> update_onsite_end_date >> update_the_reversal_date >> if_end_date_update
        if_reversal_date_update >> rail.Label("No") >>\
        if_end_date_update >> rail.Label("No") >> update_user_details >> user_role_update_start
        if_end_date_update >> rail.Label("Yes") >>\
        update_end_date >> update_user_details>>\
