...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        if_project_supervisor_in_replicon = rail.IfOperator(
            ...
            test=lambda:not rail.result("get_project_supervisor_details") or\
                not rail.result("get_project_supervisor_details")["securityConfiguration"]["isLoginEnabled"],
            yes_task="write_log_project_supervisor_details",
            no_task="if_project_supervisor_with_permissions"
        )

        write_log_project_supervisor_details = rail.WriteLogOperator(
            task_id="write_log_project_supervisor_details",
            ...
            properties=lambda dag_run: {
                ...
                "status": "Exception",
                "details": "Project supervisor not present/disabled in replicon",
            },
        )
        ...
        if_project_supervisor_with_permissions = rail.IfOperator(
            ...
            test=lambda: rail.find_first_by_attr_and_get_attr(
                rail.result("get_project_supervisor_details")["permissionSets"],
                "displayText",
                config.GENERAL_MAPPER["project_supervisor_permission"],
        ...
        if_country_changed = rail.IfOperator(
            ...
            test=lambda dag_run:dag_run.conf["country"] != rail.result("get_current_group_membership").get("existingcountryname",""),
        ...
        if_reversal_date_present = rail.IfOperator(
            ...
            test=lambda dag_run: rail.result("get_user_details")["securityConfiguration"]["isLoginEnabled"] or\
                (not rail.result("get_user_details")["securityConfiguration"]["isLoginEnabled"] and
                                dag_run.conf["reversal_date"] and dag_run.conf["reversal_date"] not in INVALID_DATES)or\
                (
                    not rail.result("get_user_details")["securityConfiguration"]["isLoginEnabled"] and\
                    rail.result('get_user_details')[
                    "userDetails"]["employmentDateRange"]["endDate"] and 
                    dag_run.conf["ard_lrd"] and dag_run.conf["ard_lrd"] not in INVALID_DATES and
                datetime(**rail.result('get_user_details')[
            "userDetails"]["employmentDateRange"]["endDate"]) < datetime.strptime(
            dag_run.conf["ard_lrd"], "%Y-%m-%d")),
        ...
        process_update_user = rail.TriggerDagRunOperator(
            ...
            conf=lambda dag_run:
            {
                **dag_run.conf,
                ...
                "supervisor_uri": rail.result("get_project_supervisor_details")["userDetails"]["uri"] if rail.result("get_project_supervisor_details") else "",
        ...
        get_user_details >>\
        if_project_supervisor_details_in_feed >> rail.Label("No")>> if_user_not_exists
        if_project_supervisor_details_in_feed >> rail.Label("Yes")>>\
        get_project_supervisor_details >>\
        if_project_supervisor_in_replicon >> rail.Label("No")>> \
        write_log_project_supervisor_details >> if_user_not_exists
        ...
        if_project_supervisor_with_permissions >> rail.Label("Yes") >> if_user_not_exists
        if_user_not_exists >> rail.Label("Yes") >> start_process_add_user >> process_add_user >> wait_for_add_user >> catch_and_log_errors
