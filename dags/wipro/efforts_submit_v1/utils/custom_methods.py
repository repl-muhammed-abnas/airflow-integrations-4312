...
def get_oncall_projects(instance):

    if instance in ['production']:
        distinct_projects = rail.load_all_records(rail.result('query_oef_oncall'))
        proj_entry_data = {}
    ...
    # Group by employee, date, and project to consolidate hours
    consolidated_data = {}
    for data in distinct_projects:
        key = (data['employee_id'], data["entry_date"], data["project_code"])
        ...
        # Sum hours for the same employee, date, and project
        consolidated_data[key]["Hours"] += float(data["hours_current"])
    
    # Convert back to string format for hours
    for entry in consolidated_data.values():
        entry["Hours"] = str(entry["Hours"])
    
    proj_entry_data["IT_ONCALL1"] = list(consolidated_data.values())
    return proj_entry_data

def get_callout_projects(instance):

    if instance in ['production']:
        distinct_projects = rail.load_all_records(rail.result('query_oef_callout'))
        proj_entry_data = {}
    ...
    proj_entry_data["IT_CALLOUT1"] = list(consolidated_data.values())
    return proj_entry_data
...
def get_overtime_projects(dag_run, instance):
    ...
    proj_entry_data["IT_OVERTIME1"] = list(consolidated_data.values())
    return proj_entry_data
...
def map_ot_oncall_callout_time_data_per_day():
    oncall_data = rail.result('map_oncall_projects') if rail.result('map_oncall_projects') else {"IT_ONCALL1":[DEFAULT_PAYLOAD]}
    callout_data = rail.result('map_callout_projects') if rail.result('map_callout_projects') else {"IT_CALLOUT1":[DEFAULT_PAYLOAD]}
    overtime_data = rail.result('map_overtime_projects') if rail.result('map_overtime_projects') else {"IT_OVERTIME1":[DEFAULT_PAYLOAD]}
