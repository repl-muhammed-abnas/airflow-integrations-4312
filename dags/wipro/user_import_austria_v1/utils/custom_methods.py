...
import functools
from wipro.user_import_austria_v1.utils import request_payload
...
custom_fields_update = {"acquired_doj": "Acquired DOJ",
...
def get_location_hierarchy_data(response):
    ...
    parent_uri = rail.result("get_austria_parent_location_details")
...
def get_austria_object_extension_fields():
...
def get_austria_time_off_types(item, config):
...
@functools.lru_cache(maxsize=128)
def get_payrule(companycode, config):
    payrule = None
    if companycode in config.PAYRULE_MAPPER:
        payrule = config.PAYRULE_MAPPER[companycode]["payrule"]

    return {
        "payrule_uri": rail.find_first_by_attr_and_get_attr(
            rail.result("get_all_payrules"), "name", payrule, "uri") if payrule else "",
    }
...
def get_approval_path_uri(country, config):
    return {
        ...
        "countryuri": rail.result("get_austria_country_uri"),
        ...
        "punch_policyuri": rail.find_first_by_attr_and_get_attr(
            rail.result("get_austria_punch_policy"), "displayText", config.GENERAL_MAPPER["punch_policy"], "uri"),
        "schedule_uri": rail.find_first_by_attr_and_get_attr(rail.result("get_all_office_schedules"),
            "displayText", config.GENERAL_MAPPER["schedule"], "uri"),
...
def get_austria_supervisor_conf(item, config):
    return {
        **item,
...
def get_austria_user_conf(dag_run,item, config):
    item["employee_last_name"] = item["employee_last_name"] or "."
    item["hr_manager_id"] = "20436461"
    item["hr_manager_mailid"] = "STEFANIE.BROECKL@WIPRO.COM"
    item["hr_adid"] = "ST20436461@wipro.com"
    ...
    return {
        **item,
        "gpo_adid": "G112400@wipro.com",
        "gpo_email_id": "austria.hrss@wipro.com",
        "gpo_id": "G112400",
        **get_austria_object_extension_fields(),
        **get_austria_custom_fields(),
        "locationcountryuri": rail.result("get_austria_parent_location_details"),
        ...
        **get_payrule(item["company_code"], config),
        "holidaycalendaruri":  rail.find_first_by_attr_and_get_attr(
            rail.result("get_all_holiday_calendars"), "displayText",
            config.GENERAL_MAPPER["holiday_calendar"], "uri"),
        "timeoff_type_uris": get_austria_time_off_types(item, config),
        ...
        "schedule_policyuri": rail.result("get_austria_schedule_policy"),
        "activity_uri": rail.result("get_all_activities"),
        "educationdrop_down_uri": rail.find_first_by_attr_and_get_attr(
            rail.result("get_education_custom_field_drop_down"), "displayText", item["education"], "uri") if item.get("education") else null,
    }
...
def check_if_austria_user_location_update(dag_run):
...
def get_austria_user_create_permissions(dag_run):
    permission_sets = []
...
def get_austria_user_create_policy_data_access(dag_run):
    policy_data_access = []
    if dag_run.conf["hr_manager_flg"] == "Y":
...
def get_austria_user_create_policy_sets(dag_run):
    policy_sets = []
    if dag_run.conf["timeoff_uri"]:
        policy_sets.append(
            {
...
def get_austria_user_create_custom_fields(dag_run):
    custom_field_values = []
    ...
    custom_fields_text = ["_1st_language",
                        "_2nd_language","education",
                        "experience", "gpo_adid", "hr_adid"]
    for i in custom_fields_text:
        if dag_run.conf[i]:
            if i == "experience":
                ...
                continue
            if i == "education":
                if dag_run.conf[i+"drop_down_uri"]:
                    custom_field_values.append({
                        "customField": {
                            "uri": dag_run.conf[i+"uri"],
                            "name": null,
                        ...
                        "date": null,
                        "dropDownOption": {
                            "uri": dag_run.conf[i+"drop_down_uri"],
                            "name": null
                        },
                        "number": null
                    })
    ...
    for i in custom_fields:
        if dag_run.conf[i] and dag_run.conf[i] not in INVALID_DATES:
            if i in ["onsite_start_date","tavel_start_date"] and\
                dag_run.conf["onsite_direct_recruit"].lower() == "assignee" and\
                dag_run.conf["travel_start_date"] != dag_run.conf["onsite_start_date"]:
                continue
            custom_field_values.append({
                "customField": {
                    "uri": dag_run.conf[i+"uri"],
...
def get_austria_user_create_location(dag_run):
    location = []
...
def get_austria_user_payrule_script(dag_run):
    payrules = []
...
def get_austria_user_create_oefs(dag_run):
...
def get_updated_logs(dag_run):
    ...
    logs.append(get_austria_user_update_custom_fields(dag_run)[1])
    logs.append(request_payload.get_basic_user_details_update(dag_run)[1])
    logs.append(check_if_austria_user_location_update(dag_run)[1])
    logs.append(request_payload.get_policy_set_update(dag_run)[1])
    logs.append(request_payload.get_employee_type_update(dag_run)[1])
    logs.append(request_payload.get_payrule_update(dag_run)[1])
...
def get_austria_user_update_custom_fields(dag_run):
    custom_field_values = []
    custom_field_logs = ""
    custom_fields_text = {
    ...
    for i, v in custom_fields_text.items():
        if dag_run.conf[i] and rail.find_first_by_attr_and_get_attr(
            ...
            if i == "experience" and float(rail.find_first_by_attr_and_get_attr(
                rail.result("get_user_custom_field_values"),"displayText",
                v, "textValue")) != dag_run.conf[i]:
                custom_field_values.append({"value":{
                    "customField": {
                        "uri": dag_run.conf[i+"uri"],
                        "name": null,
                        "groupUri": null
    ...
    for i, v in custom_fields_update.items():
