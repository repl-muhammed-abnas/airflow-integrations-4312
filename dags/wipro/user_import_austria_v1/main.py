...
from wipro.user_import_austria_v1.task import process_user_group_data
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        query_valid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM austria_users WHERE NULLIF("employee_id","") IS NOT NULL AND
            ...
            name="austria_valid_users"
        )

        query_invalid_user_records = rail.QueryCollectionOperator(
            task_id="query_invalid_user_records",
            query="""SELECT * FROM austria_users WHERE NULLIF("employee_id","") IS NULL OR
        ...
        get_all_austria_users = rail.RepliconServiceOperator(
            task_id="get_all_austria_users",
            endpoint="/services/UserListService1.svc/GetData",
            data=request_payload.get_austria_user_list,
            data_handler=lambda response:list(map(lambda user:{
                "employee_id": user["cells"][1].get("textValue",""),
                "email": user["cells"][0].get("textValue",""),
                "login_name": user["cells"][2].get("textValue","")
            }, response["rows"]))
        )

        process_user_for_austria = rail.trigger_parallel_dagrun(
            task_id="process_user_for_austria",
            items='{{result("query_valid_user_records")}}',
            trigger_dag_id=config.valid_user_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda dag_run,item, config=config: custom_methods.get_austria_user_conf(
                dag_run,item, config)
        )
        ...
        load_supervisor_data = rail.PythonOperator(
            ...
            python_callable=lambda: list(map(lambda i: {**i["properties"]},
                rail.load_all_records(rail.result("get_supervisor_pending_logs")))))

        process_supervisor_for_austria = rail.trigger_parallel_dagrun(
            task_id="process_supervisor_for_austria",
            items='{{result("load_supervisor_data")|to_json}}',
            trigger_dag_id=config.create_supervisor_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item, config=config: custom_methods.get_austria_supervisor_conf(
                item, config)
        )

        end_austria_user = rail.EmptyOperator(
            task_id="end_austria_user"
        )

        process_logs_for_austria = rail.TriggerDagRunOperator(
            task_id="process_logs_for_austria",
        ...
        if_invalid_users >> rail.Label("No") >>\
            ...
            process_user_start >>\
            get_all_austria_users>>\
            process_user_for_austria >> get_supervisor_pending_logs >>\
            load_supervisor_data >> process_supervisor_for_austria >>\
            end_austria_user >> process_logs_for_austria
