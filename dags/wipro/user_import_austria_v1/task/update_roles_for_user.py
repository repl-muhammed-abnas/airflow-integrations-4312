...
from wipro.user_import_austria_v1.task import put_supervisor_table_settings
import rail
...
def update_user_roles(config):
    with rail.TaskGroup(
        group_id="update_austria_user_roles",
        ...
        assign_primary_manager_permission = rail.SetVariableOperator(
            ...
            value=lambda dag_run: [
                dag_run.conf["l1_manager_uri"], dag_run.conf["default_user_permission_uri"]],
            append=True
        )
        ...
        assign_project_manager_permission = rail.SetVariableOperator(
            ...
            name='{{result("create_manager_permission_variable").name}}',
            value='{{dag_run.conf.project_manager_uri}}',
            append=True
        )
        ...
        if_project_manager_permission_update >> rail.Label("Yes") >>\
            assign_project_manager_permission >> put_table_view_setting_for_user_update_pm >> get_manager_permission >>\
