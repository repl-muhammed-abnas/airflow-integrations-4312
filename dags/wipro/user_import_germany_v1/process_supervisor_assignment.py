from wipro.user_import_germany_v1.task import put_supervisor_table_settings
...
def create_aiflow_child(config):
    with rail.create_airflow_dag(
        ...
        if_supervisor_has_permissions = rail.IfOperator(
            ...
            test=lambda: (rail.find_first_by_attr_and_get_attr(
                rail.result("get_supervisor_user_details")[
                    0]["permissionSets"],
                "displayText",
                config.GENERAL_MAPPER["l1_manager_wbs"],
                "uri") and rail.find_first_by_attr_and_get_attr(
                rail.result("get_supervisor_user_details")[
                    0]["permissionSets"],
                "displayText",
                config.GENERAL_MAPPER["l1_manager_non_wbs"],
                "uri")),
            yes_task="if_initial_supervisor",
            no_task="if_valid_legal_entity_manager_uri"
        )

        if_valid_legal_entity_manager_uri = rail.IfOperator(
            task_id="if_valid_legal_entity_manager_uri",
            test=lambda dag_run: dag_run.conf["l1_manager_uri"],
            yes_task="update_supervisor_permission",
        ...
        if_supervisor_in_replicon >> rail.Label("Yes") >> if_supervisor_has_permissions
        if_supervisor_has_permissions >> rail.Label("Yes") >> if_initial_supervisor
        if_supervisor_has_permissions >> rail.Label("No") >> if_valid_legal_entity_manager_uri
        if_valid_legal_entity_manager_uri >> rail.Label("Yes") >> update_supervisor_permission
        if_valid_legal_entity_manager_uri >> rail.Label("No") >> catch_and_log_errors
        update_supervisor_permission >> write_log_supervisor_updated >>\
