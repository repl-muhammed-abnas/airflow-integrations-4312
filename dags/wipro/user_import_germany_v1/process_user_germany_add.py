import json
from wipro.user_import_germany_v1.task import put_user_and_table_settings, put_supervisor_table_settings
from wipro.user_import_germany_v1.utils import request_payload, custom_methods
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task="batch_task",
            no_task="create_germany_user"
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id="batch_task",
            start_task="create_germany_user",
            end_task="catch_and_log_errors"
        )

        create_germany_user = rail.RepliconServiceOperator(
            task_id="create_germany_user",
            endpoint="/services/ImportService1.svc/PutUser3",
            data=lambda dag_run, config=config: request_payload.get_germany_create_payload(dag_run, config),
        ...
        write_supervisor_pending_logs = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "Add_Update":"Add",
                "useruri": rail.result("create_germany_user")
            }
        )
        ...
        unassign_products_for_user = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "user": {
                    "uri": rail.result("create_germany_user"),
                    "loginName": null,
        ...
        put_view_settings_for_user = put_user_and_table_settings.get_put_table_view_setting(
            '{{result("create_germany_user")}}', "germany", 'user')
        ...
        put_manager_table_view = put_supervisor_table_settings.get_put_table_view_setting_supervisor(
            '{{result("create_germany_user")}}', 'supervisor_user', '{{dag_run.conf.project_manager_flg}}')
        ...
        remove_all_time_off_types = rail.RepliconServiceOperator(
            ...
            data={
                    "userUri": '{{result("create_germany_user")}}',
                    "timeOffTypeUris": []
            }
        )
        ...
        assign_all_time_off_types = rail.RepliconServiceOperator(
            ...
            endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_germany_user")}}'),
        ...
        if_no_gender >> rail.Label("No") >>\
            assign_all_time_off_types >> get_default_time_off_type_policy_schedule_for_user >> if_user_is_assignee
