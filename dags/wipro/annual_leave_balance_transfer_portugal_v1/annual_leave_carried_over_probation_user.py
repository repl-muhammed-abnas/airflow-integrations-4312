...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.annual_leaves_carried_over_to_lapsed_probation_users_dag_id,
        description=f'WIPRO | Annual leave Balance Transfer for Portugal | Annual Leaves Carried Over To Lapsed For Probation Users {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=dt(2024, 1, 1, tz=config.time_zone),
        schedule_interval=config.schedule_interval_annual_leave_carried_over_probation_users,
        ...
        def can_trigger_run(dag_run):
            return bool(dag_run.conf.get('skip_rundate_validation', False) or
                        (DATE_IN_REQUIRED_TIMEZONE.strftime("%Y/%m/%d") == DATE_IN_REQUIRED_TIMEZONE.strftime("%Y") + "/07/01"))

        if_run_date_is_1st_jul = rail.IfOperator(
            task_id='if_run_date_is_1st_jul',
        ...
        log_dag_run_report_run_and_probation_cutoff_date = rail.PythonOperator(
            ...
            python_callable=lambda: {
                ...
                'dag_run_date': DATE_IN_REQUIRED_TIMEZONE.strftime("%Y") + "/07/01",
                'report_run_date': DATE_IN_REQUIRED_TIMEZONE.strftime("%Y") + "/06/30",
        ...
        if_non_probation_users_greater_than_0 = rail.IfOperator(
            task_id='if_non_probation_users_greater_than_0',
            test=lambda: rail.result(
                "query_non_probation_users", "length") > 0,
            yes_task='log_non_probation_users_skipped',
            no_task='query_probation_users'
        )

        log_non_probation_users_skipped = rail.WriteLogOperator(
            task_id='log_non_probation_users_skipped',
            log="{{result('create_timeoff_balance_transfer_logs')}}",
            items="{{result('query_non_probation_users')}}",
            message='na',
            severity='Exception',
            properties=lambda item: {
                ...
                "status": "Skipped",
                "details": "Annual Leave Balance Transfer Not Processed in current run as user start date is before 1st JULY of previous year (User is not on Probation)"
            }
        )
        ...
        query_probation_users_where_timeoff_balance_is_0 = rail.QueryCollectionOperator(
            task_id='query_probation_users_where_timeoff_balance_is_0',
            query="""SELECT * FROM probation_users WHERE timeoff_balance == '0.00' """,
            name='skipped_records'
        )

        if_probation_users_with_timeoff_balance_0_greater_than_0 = rail.IfOperator(
            task_id='if_probation_users_with_timeoff_balance_0_greater_than_0',
            test=lambda: rail.result(
                "query_probation_users_where_timeoff_balance_is_0", "length") > 0,
            yes_task='log_skipped_records',
            no_task='query_probation_users_where_timeoff_balance_is_not_0'
        )

        log_skipped_records = rail.WriteLogOperator(
            task_id='log_skipped_records',
            log="{{result('create_timeoff_balance_transfer_logs')}}",
            items="{{result('query_probation_users_where_timeoff_balance_is_0')}}",
            message='na',
            severity='Exception',
            properties=lambda item: {
                'jobid': rail.render_template("{{dag_run_ecid()}}"),
        ...
        query_probation_users_where_timeoff_balance_is_not_0 = rail.QueryCollectionOperator(
            task_id='query_probation_users_where_timeoff_balance_is_not_0',
            query="""SELECT * FROM probation_users WHERE timeoff_balance != '0.00' """,
        ...
        trigger_dag_run_transfer_timeoff_balance = rail.trigger_parallel_dagrun(
            ...
            items="{{result('query_probation_users_where_timeoff_balance_is_not_0')}}",
        ...
        if_run_date_is_1st_jul >> rail.Label(
        ...
        if_invalid_records_greater_than_0 >> rail.Label(
            "Yes") >> log_invalid_records >> query_non_probation_users
        if_invalid_records_greater_than_0 >> rail.Label(
            "No") >> query_non_probation_users

        query_non_probation_users >> if_non_probation_users_greater_than_0

        if_non_probation_users_greater_than_0 >> rail.Label(
            "Yes") >> log_non_probation_users_skipped >> query_probation_users
        if_non_probation_users_greater_than_0 >> rail.Label(
            "No") >> query_probation_users

        query_probation_users >> query_probation_users_where_timeoff_balance_is_0 >> if_probation_users_with_timeoff_balance_0_greater_than_0

        if_probation_users_with_timeoff_balance_0_greater_than_0 >> rail.Label(
            "Yes") >> log_skipped_records >> query_probation_users_where_timeoff_balance_is_not_0
        if_probation_users_with_timeoff_balance_0_greater_than_0 >> rail.Label(
            "No") >> query_probation_users_where_timeoff_balance_is_not_0

        query_probation_users_where_timeoff_balance_is_not_0 >> trigger_dag_run_transfer_timeoff_balance
