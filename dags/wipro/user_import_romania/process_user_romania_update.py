from wipro.user_import_romania.task import update_roles_for_user
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        get_assigned_policy_set_for_user = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response: {
                "timesheet_template": rail.find_first_by_attr_and_get_attr(
                    response, "policyUri", "urn:replicon:policy:timesheet", "user"
                ),
                "punch_policy": rail.find_first_by_attr_and_get_attr(
                    response, "policyUri", "urn:replicon:policy:time-punch", "user"
                )
            }
        )
        ...
        get_future_time_off_bookings = rail.RepliconServiceOperator(
            task_id="get_future_time_off_bookings",
            endpoint="/services/TimeOffService1.svc/GetTimeOffDetailsForUserAndDateRange2",
            data=request_payload.get_future_time_off_bookings_payload,
            data_handler=lambda response: [
                b["uri"] for b in (response or [])
                if b.get("uri")
            ]
        )

        delete_future_time_off_bookings = rail.RepliconServiceCallForEachItemOperator(
            task_id="delete_future_time_off_bookings",
            endpoint="/services/TimeOffService1.svc/DeleteTimeOff",
            items='{{result("get_future_time_off_bookings")|to_json}}',
            data=lambda dag_run, item: request_payload.get_delete_time_off_booking_payload(item)
        )
        ...
        if_should_check_hr_manager >> rail.Label("No") >> get_update_user_details
        if_hr_manager_found >> rail.Label("Yes") >> check_hr_manager_country >> get_update_user_details
        if_hr_manager_found >> rail.Label("No") >> get_update_user_details
        ...
        if_end_date_update >> rail.Label("Yes") >>\
            update_end_date >> get_future_time_off_bookings >>\
            delete_future_time_off_bookings >>\
