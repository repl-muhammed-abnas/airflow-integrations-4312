...
from wipro.user_import_romania.task import put_supervisor_table_settings
import rail
...
def update_user_roles(config):
    with rail.TaskGroup(
        group_id="update_romania_user_roles",
        ...
        assign_primary_manager_permission = rail.SetVariableOperator(
            ...
            value=lambda dag_run: [
                dag_run.conf["l1_manager_uri"], dag_run.conf["project_manager_uri"], dag_run.conf["end_user_manager_uri"]],
            append=True
        )
