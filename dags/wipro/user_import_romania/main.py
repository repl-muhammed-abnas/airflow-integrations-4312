...
from wipro.user_import_romania.task import process_user_group_data
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        query_valid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM romania_users WHERE NULLIF("employee_id","") IS NOT NULL AND
            ...
            name="romania_valid_users"
        )

        query_invalid_user_records = rail.QueryCollectionOperator(
            task_id="query_invalid_user_records",
            query="""SELECT * FROM romania_users WHERE NULLIF("employee_id","") IS NULL OR
        ...
        process_user_for_romania = rail.trigger_parallel_dagrun(
            task_id="process_user_for_romania",
            items='{{result("query_valid_user_records")}}',
            trigger_dag_id=config.valid_user_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item, config=config: custom_methods.get_romania_user_conf(
                item, config)
        )
        ...
        process_supervisor_for_romania = rail.trigger_parallel_dagrun(
            task_id="process_supervisor_for_romania",
            items='{{result("load_supervisor_data")|to_json}}',
            trigger_dag_id=config.create_supervisor_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item, config=config: custom_methods.get_romania_supervisor_conf(
                item, config)
        )

        end_romania_user = rail.EmptyOperator(
            task_id="end_romania_user"
        )

        process_logs_for_romania = rail.TriggerDagRunOperator(
            task_id="process_logs_for_romania",
        ...
        if_invalid_users >> rail.Label("No") >>\
            ...
            process_user_start >>\
            process_user_for_romania >> get_supervisor_pending_logs >>\
            load_supervisor_data >> process_supervisor_for_romania >>\
            end_romania_user >> process_logs_for_romania
