...
def create_airflow_child_dag(config):
    for country in config.time_export_for_country:
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.submit_ot_data_child}_{cnt}_{config.instance}_v1",
            ...
            efforts_submission_to_wipro = CustomSimpleHttpOperator(
                ...
                endpoint="h2r/my-time/1.0.0/oncallset",
                method="POST",
                auth_type=None,
                headers={
            ...
            upload_log_to_sftp = rail.SFTPUploadFileOperator(
                ...
                remote_filepath=config.log_filepath + '/Log_{{ dag_run.conf.data.Pernr if dag_run.conf.data | is_truthy else "empty" }}_' + str(uuid.uuid4()) + '.json',
            )
