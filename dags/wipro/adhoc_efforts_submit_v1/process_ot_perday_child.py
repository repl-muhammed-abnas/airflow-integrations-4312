...
from wipro.adhoc_efforts_submit_v1.utils import custom_methods
import rail
...
def create_airflow_child_dag(config):
    for country in config.time_export_for_country:
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.process_ot_perday_child}_{cnt}_{config.instance}_v1",
            ...
            if_oef_oncall = rail.IfOperator(
                ...
                yes_task="query_distinct_oef_oncall",
            ...
            map_oncall_projects = rail.PythonOperator(
                ...
                python_callable=custom_methods.get_oncall_projects
            )
            ...
            if_oef_callout = rail.IfOperator(
                ...
                yes_task="query_distinct_oef_callout",
            ...
            map_callout_projects = rail.PythonOperator(
                ...
                python_callable=custom_methods.get_callout_projects
            )
            ...
            if_oef_overtime = rail.IfOperator(
                ...
                yes_task="query_distinct_oef_overtime",
            ...
            submit_data_to_wipro = rail.TriggerDagRunOperator(
                ...
                trigger_dag_id=f"{config.submit_ot_data_child}_{cnt}_{config.instance}_v1",
                conf=lambda: {
            ...
            get_distinct_querys_options >> if_oef_oncall >> rail.Label("Yes") >> query_distinct_oef_oncall >> map_oncall_projects >> if_oef_callout
            if_oef_oncall >> rail.Label("No") >> if_oef_callout
            if_oef_callout >> rail.Label("Yes") >> query_distinct_oef_callout >> map_callout_projects >> if_oef_overtime
            if_oef_callout >> rail.Label("No") >> if_oef_overtime
            if_oef_overtime >> rail.Label("Yes") >> query_distinct_oef_overtime >> map_overtime_projects >> combine_perday_data
