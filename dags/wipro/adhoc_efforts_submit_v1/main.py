...
from wipro.adhoc_efforts_submit_v1.tasks.project_time_data import project_time_export
    ...
    for country in config.time_export_for_country:
        ...
        with rail.create_airflow_dag(
            ...
            time_export_run = project_time_export(
                group_id=group_id, country_code="adhoc", country=country)
            ...
            create_time_export_collection = rail.CreateCollectionOperator(
                ...
                columns={'Employee ID': 'employee_id', 'Hours (Current)': 'hours_current',
                         ...
                         'AT Select Activity': 'at_select_activity', 'CH Select Activity': 'ch_select_activity',
                         'UK Select Type': 'uk_select_type'
                        }
            )
            ...
            data=lambda dag_run,country=country: {
                        "target": {
                            "uri": dag_run.conf["time_export_uri"],
                            "name": null
                        },
                    "name": f"Time_export_adhoc_no_data_" + rail.result("process_start_time")
                }
            )
            ...
            log_to_sumo_no_export_data = rail.SendToSumoOperator(
                ...
                data={
                    ...
                    "TimeExportName": f"Time_export_adhoc_no_data_" + '{{result("process_start_time")}}',
            ...
            query_valid_time_export_collection = rail.QueryCollectionOperator(
                ...
                query=f"""SELECT * FROM time_export_data_{cnt} WHERE
                        ...
                        NULLIF("at_select_activity", "") IS NULL AND
                        NULLIF("ch_select_activity", "") IS NULL AND
                        NULLIF("uk_select_type", "") IS NULL
                    """
            )
            ...
            log_to_sumo_export_data = rail.SendToSumoOperator(
                ...
                data={
                    "Country": country,
                    "JobStartTime": '{{result("process_start_time")}}',
                    "TimeExportName": f"Time_export_adhoc_" + '{{ result("process_start_time") }}',
                    "TimeStamp": '{{ current_time_in_specified_tz("Etc/UTC", "%Y-%m-%dT%H:%M:%S") }}',
            ...
            query_valid_ot_oncall_callout_time_export_collection = rail.QueryCollectionOperator(
                ...
                query=f"""SELECT * FROM time_export_data_{cnt} WHERE
                        ...
                        NULLIF("at_select_activity", "") IS NOT NULL OR
                        NULLIF("ch_select_activity", "") IS NOT NULL OR
                        NULLIF("uk_select_type", "") IS NOT NULL
                    """
            )
            ...
            if_valid_time_export_has_data = rail.IfOperator(
                ...
                no_task=["get_export_name","query_unique_empid_time_sheet_period_for_ot_oncall_callout"]
            )
            ...
            if_valid_time_export_has_data >> rail.Label("No") >>[query_unique_empid_time_sheet_period_for_ot_oncall_callout, get_export_name]
            get_export_name >>\
