...
from wipro.user_import_poland_v1.task import put_user_and_table_settings, put_supervisor_table_settings
from wipro.user_import_poland_v1.utils import request_payload, custom_methods
import rail
null = None
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task="batch_task",
            no_task="create_poland_user"
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id="batch_task",
            start_task="create_poland_user",
            end_task="catch_and_log_errors"
        )

        create_poland_user = rail.RepliconServiceOperator(
            task_id="create_poland_user",
            endpoint="/services/ImportService1.svc/PutUser3",
            data=request_payload.get_poland_create_payload,
        ...
        if_supervisor_details_in_feed = rail.IfOperator(
            ...
            test=lambda dag_run:bool(dag_run.conf["primary_supervisor_id"]) or bool(dag_run.conf["project_supervisor_id"]),
        ...
        write_supervisor_pending_logs = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "primary_supervisor_id": dag_run.conf["primary_supervisor_id"] if dag_run.conf['personnel_subarea_text'] in ['D&OP', 'D&OP Poland'] else dag_run.conf["project_supervisor_id"],
                "primary_supervisor_adid":dag_run.conf["primary_supervisor_adid"] if dag_run.conf['personnel_subarea_text'] in ['D&OP', 'D&OP Poland'] else None,
                "primary_supervisor_mailid":dag_run.conf["primary_supervisor_mailid"]  if dag_run.conf['personnel_subarea_text'] in ['D&OP', 'D&OP Poland'] else dag_run.conf["project_supervisor_mailid"],
                "Add_Update":"Add",
                "useruri": rail.result("create_poland_user")
            }
        )
        ...
        unassign_products_for_user = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "user": {
                    "uri": rail.result("create_poland_user"),
                    "loginName": null,
        ...
        put_view_settings_for_user = put_user_and_table_settings.get_put_table_view_setting(
            '{{result("create_poland_user")}}', "poland", 'user')
        ...
        put_manager_table_view = put_supervisor_table_settings.get_put_table_view_setting_supervisor(
            '{{result("create_poland_user")}}', 'supervisor_user', '{{dag_run.conf.project_manager_flg}}')
        ...
        assign_all_time_off_types = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_poland_user")}}'),
                    "timeOffTypeUris": dag_run.conf["timeoff_type_uris"] + dag_run.conf["disabled_timeoff_type_uris"]
            }
        )
        ...
        get_default_time_off_type_policy_schedule_for_user = rail.RepliconServiceCallForEachItemOperator(
            ...
            items='{{(dag_run.conf.timeoff_type_uris + dag_run.conf.disabled_timeoff_type_uris)|to_json}}',
            data={
                "timeOffAccount": {
                    "userUri": '{{result("create_poland_user")}}',
                    "timeOffTypeUri": "{{ item }}"
                }
            }
        )
        ...
        assign_default_timeoff_policy_for_assignee = rail.RepliconServiceCallForEachItemOperator(
            ...
            data=lambda dag_run, item: {
                ...
                "policySetScheduleEntries": json.loads(json.dumps(rail.result('replace_effective_date')
                                                                  [list(dag_run.conf["timeoff_type_uris"]+dag_run.conf["disabled_timeoff_type_uris"]).index(item)])
        ...
        enable_only_time_off_types = rail.RepliconServiceOperator(
            task_id="enable_only_time_off_types",
            endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_poland_user")}}'),
        ...
        end_table_setting >> assign_all_time_off_types >> get_default_time_off_type_policy_schedule_for_user >>\
            if_user_is_assignee
        if_user_is_assignee >> rail.Label("Yes") >> replace_effective_date >> assign_default_timeoff_policy_for_assignee\
        >> enable_only_time_off_types
        if_user_is_assignee >> rail.Label("No") >> assign_default_timeoff_policy >>\
            enable_only_time_off_types >> time_off_end >>\
