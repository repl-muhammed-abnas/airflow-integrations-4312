from wipro.user_import_poland_v1.task import update_roles_for_user
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        get_assigned_policy_sets = rail.RepliconServiceOperator(
            task_id="get_assigned_policy_sets",
        ...
        if_get_timesheet_for_date2_uri_present = rail.IfOperator(
            ...
            test="{{ result('get_timesheet_for_date2') | is_truthy }}",
            yes_task="get_timesheet_details",
            no_task="get_current_location_for_the_user",
        )
        ...
        if_end_date_update = rail.IfOperator(
            ...
            test=lambda dag_run: bool(custom_methods.check_if_end_date(dag_run) and
                                      not rail.result('get_update_user_details')["userDetails"]["employmentDateRange"]["endDate"]),
        ...
        can_run_batch_task >> rail.Label("No") >>\
            ...
            get_extension_field_values >> get_assigned_policy_sets >> get_timesheet_for_date2 >>\
            if_get_timesheet_for_date2_uri_present
        if_get_timesheet_for_date2_uri_present >> rail.Label("Yes") >> get_timesheet_details >>\
            get_current_location_for_the_user
        if_get_timesheet_for_date2_uri_present >> rail.Label("No") >> get_current_location_for_the_user
        get_current_location_for_the_user >> update_user_details >>\
        if_supervisor_details_in_feed >> rail.Label("No") >> if_reversal_date_update
