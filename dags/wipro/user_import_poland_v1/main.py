...
from wipro.user_import_poland_v1.task import process_user_group_data
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        query_valid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM poland_users WHERE NULLIF("employee_id","") IS NOT NULL AND
            NULLIF("employee_first_name","") IS NOT NULL AND
            NULLIF("date_of_joining","") IS NOT NULL AND ("date_of_joining" NOT LIKE "9999-12-31"
            AND "date_of_joining" NOT LIKE "0000-00-00") AND NULLIF("country","") IS NOT NULL AND NULLIF("employment_status","") IS NOT NULL AND
            NULLIF("company_code","") IS NOT NULL AND
            NULLIF("adid","") IS NOT NULL AND NULLIF("primary_supervisor_id","") IS NOT NULL AND
            NULLIF("primary_supervisor_mailid","") IS NOT NULL AND NULLIF("primary_supervisor_adid","") IS NOT NULL AND
            NULLIF("billability_status","") IS NOT NULL AND
                    NULLIF("employment_percentage","") IS NOT NULL""",
            name="poland_valid_users"
        )

        query_invalid_user_records = rail.QueryCollectionOperator(
            task_id="query_invalid_user_records",
            query="""SELECT * FROM poland_users WHERE NULLIF("employee_id","") IS NULL OR
            NULLIF("employee_first_name","") IS NULL OR
            NULLIF("date_of_joining","") IS NULL OR NULLIF("country","") IS NULL OR
            NULLIF("employment_status","") IS NULL OR
            NULLIF("company_code","") IS NULL OR
            NULLIF("adid","") IS NULL OR "date_of_joining" LIKE "9999-12-31"
            or "date_of_joining" LIKE 0000-00-00 OR
              NULLIF("primary_supervisor_id","") IS NULL OR
                NULLIF("primary_supervisor_mailid","") IS NULL OR NULLIF("primary_supervisor_adid","") IS NULL OR
                    NULLIF("billability_status","") IS NULL OR
                        NULLIF("employment_percentage","") IS NULL"""
        )
        ...
        process_user_for_poland = rail.trigger_parallel_dagrun(
            task_id="process_user_for_poland",
            items='{{result("query_valid_user_records")}}',
            trigger_dag_id=config.valid_user_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item, config=config: custom_methods.get_poland_user_conf(
                item, config)
        )
        ...
        process_supervisor_for_poland = rail.trigger_parallel_dagrun(
            task_id="process_supervisor_for_poland",
            items='{{result("load_supervisor_data")|to_json}}',
            trigger_dag_id=config.create_supervisor_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item,config=config:custom_methods.get_poland_supervisor_conf(item,config)
        )

        process_user_end = rail.EmptyOperator(task_id="process_user_end")

        process_logs_for_poland = rail.TriggerDagRunOperator(
            task_id="process_logs_for_poland",
        ...
        if_invalid_users >> rail.Label("No") >>\
            ...
            process_user_start >>\
            process_user_for_poland >> get_supervisor_pending_logs >>\
            load_supervisor_data >> process_supervisor_for_poland >>\
            process_user_end >> process_logs_for_poland
