import rail
from wipro.user_import_spain_v2.mapper.default_settings_mapper import user_default_settings
...
def get_put_table_view_setting(user_uri, country, caller='supervisor'):
    with rail.TaskGroup(group_id=f'put_table_view_setting_{caller}', prefix_group_id=False) as put_table_view_setting:
        ...
        has_language_and_local_hire = rail.IfOperator(
            task_id=f"has_language_and_local_hire_{caller}",
            test=lambda dag_run: ("spanish_api_value" in user_default_settings[country] \
                                  and dag_run.conf["onsite_direct_recruit"].lower() == "local_hire"),
            yes_task=f"update_spanish_{caller}",
            no_task=f"has_language_and_assignee_{caller}"
        )

        update_spanish = rail.RepliconServiceOperator(
            task_id=f"update_spanish_{caller}",
            endpoint='/services/InternationalizationService1.svc/UpdateLanguageForUser',
            data={
                "userUri": user_uri,
                "languageUri": user_default_settings[country]["spanish_api_value"]
            },
            headers=lambda: rail.result(
                f'impersonate_and_create_interactive_session_{caller}')
        )

        has_language_and_assignee = rail.IfOperator(
            task_id=f"has_language_and_assignee_{caller}",
            test=lambda dag_run: ("language_api_value" in user_default_settings[country]\
                        and dag_run.conf["onsite_direct_recruit"].lower() != "local_hire"),
        ...
        has_language_and_local_hire >> rail.Label("No") >>\
        has_language_and_assignee
        has_language_and_local_hire >> rail.Label("Yes") >>\
        update_spanish >>\
            has_language_and_assignee >> rail.Label(
                "Yes") >> update_language >> has_date_format
        has_language_and_assignee >> rail.Label("No") >>\
