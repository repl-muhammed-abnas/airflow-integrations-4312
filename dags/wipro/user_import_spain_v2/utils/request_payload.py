...
from wipro.user_import_spain_v2.utils import custom_methods
import rail
...
def get_spain_annual_acquistion_terminated_user_payload(dag_run):
    ...
    annual_accrual_uri = dag_run.conf["annual_leave_uri"]
    ...
    if dag_run.conf["onsite_direct_recruit"] == "ASSIGNEE":
            start_date = dag_run.conf["onsite_end_date"]
    else:
            start_date = dag_run.conf["ard_lrd"]
    effective_date = get_end_date(start_date)
