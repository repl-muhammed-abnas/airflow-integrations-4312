from wipro.user_import_spain_v2.task import put_supervisor_table_settings
from wipro.user_import_spain_v2.utils import request_payload, custom_methods
...
def create_aiflow_child(config):
    with rail.create_airflow_dag(
        ...
        validate_supervisor_employee_id_unequal = rail.IfOperator(
            ...
            test=lambda dag_run:(
                dag_run.conf["project_supervisor_id"] and\
                  dag_run.conf["project_supervisor_id"] != dag_run.conf["employee_id"]) or \
                  (
        ...
        put_supervisor_table_view = put_supervisor_table_settings.get_put_table_view_setting_supervisor(
            '{{result("get_user_uri")}}', "supervisor", "N")
        ...
        write_log_supervisor_data_not_available = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "company_code": dag_run.conf["company_code"],
                "status": "Exception",
                "details": "Supervisor not created as required adid or mailid not present",
        ...
        write_supervisor_employee_id_match_log = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "company_code": dag_run.conf["company_code"],
                "status": "Exception",
                "details": "Supervisor not assigned as employee id same as supervisor id",
        ...
        write_supervisor_employee_id_match_log >> end_supervisor
        ...
        if_initial_supervisor >> rail.Label("Yes") >> assign_initial_supervisor_to_employee >> end_supervisor
