...
from wipro.user_import_spain_v2.task import put_user_and_table_settings, put_supervisor_table_settings
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task="batch_task",
            no_task="create_spain_user"
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id="batch_task",
            start_task="create_spain_user",
            end_task="catch_and_log_errors"
        )

        create_spain_user = rail.RepliconServiceOperator(
            task_id="create_spain_user",
            endpoint="/services/ImportService1.svc/PutUser3",
            data=request_payload.get_spain_create_payload,
        ...
        write_supervisor_pending_logs = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "Add_Update":"Add",
                "useruri": rail.result("create_spain_user"),
        ...
        unassign_products_for_user = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "user": {
                    "uri": rail.result("create_spain_user"),
                    "loginName": null,
        ...
        put_view_settings_for_user = put_user_and_table_settings.get_put_table_view_setting(
            '{{result("create_spain_user")}}', "spain", 'user')
        ...
        put_manager_table_view = put_supervisor_table_settings.get_put_table_view_setting_supervisor(
            '{{result("create_spain_user")}}', 'supervisor_user', '{{dag_run.conf.project_manager_flg}}')
        ...
        remove_all_time_off_types = rail.RepliconServiceOperator(
            ...
            data={
                    "userUri": '{{result("create_spain_user")}}',
                    "timeOffTypeUris": []
            }
        )
        ...
        assign_all_time_off_types = rail.RepliconServiceOperator(
            ...
            endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_spain_user")}}'),
        ...
        assign_default_timeoff_policy_for_assignee = rail.RepliconServiceCallForEachItemOperator(
            ...
            data=lambda dag_run, item: {
                ...
                "policySetScheduleEntries": custom_methods.get_spain_starting_balance(item, dag_run)
            }
        )
        ...
        disable_time_off_types = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_spain_user")}}'),
                    "timeOffTypeUris": dag_run.conf["disable_timeoff_uri"] 
        ...
        if_user_is_assignee >> rail.Label("No") >>\
            ...
            time_off_end >> disable_time_off_types >>\
