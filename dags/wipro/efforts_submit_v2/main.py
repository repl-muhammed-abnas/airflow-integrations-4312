...
from wipro.efforts_submit_v2.tasks.project_time_data import project_time_export
from wipro.efforts_submit_v2.tasks.approval_data_report import approval_data_report_task
...
def create_airflow_master_dag(config):
    for country in config.time_export_for_country:
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.master_dag}_{cnt}_{config.instance}_v2",
            ...
            def get_currnt_country_callable(dag_run):
                ...
                _country = _country.split(f"_{config.instance}_v2")[0]
            ...
            create_time_export_collection = rail.CreateCollectionOperator(
                ...
                columns={'Employee ID': 'employee_id', 'Hours (Current)': 'hours_current',
                         ...
                         'Select an Option': 'select_an_option', "Login Name": "login_name",
                         'Type Selection': 'type_selection','RO_Select_Type': 'ro_select_type',
                         ...
                         'AT_Select_Activity_All_IN': 'at_select_activity_all_in', 'CH Activity': 'ch_activity',
            ...
            query_valid_time_export_collection = rail.QueryCollectionOperator(
                ...
                query=f"""SELECT * FROM time_export_data_{cnt} WHERE
                        ...
                        NULLIF("ch_activity", "") IS NULL AND
            ...
            process_each_time_period_record = rail.trigger_parallel_dagrun(
                ...
                trigger_dag_id=f"{config.process_time_period_child}_{cnt}_{config.instance}_v2",
                conf=lambda item:{
            ...
            can_trigger_ot_oncall = rail.IfOperator(
                ...
                test=lambda:Variable.get(
                    ...
                    "true").lower() == "true",
                yes_task="query_valid_oef_time_export_collection",
                no_task="send_regular_time_entry_submission_complete"
            )

            query_valid_oef_time_export_collection = rail.QueryCollectionOperator(
                task_id="query_valid_oef_time_export_collection",
                query=f"""SELECT * FROM time_export_data_{cnt} WHERE
                        (NULLIF("project_code","") IS NOT NULL AND
                            (
                                ...
                                NULLIF("ch_activity", "") IS NOT NULL OR
            ...
            if_valid_ot_oncall_callout_time_export_data = rail.IfOperator(
                ...
                test='{{result("query_valid_oef_time_export_collection", "length")>0}}',
                yes_task="get_export_name",
                no_task="send_time_entry_submission_complete"
            )

            start_time_export_report = rail.EmptyOperator(task_id="start_time_export_report")

            run_time_export_report = approval_data_report_task(
                "approval_data_report", config.report_name, country
            )
            ...
            query_valid_ot_oncall_callout_time_export_collection = rail.QueryCollectionOperator(
               ...
               query="""SELECT ot.*, r.hours AS report_hours
                FROM ot_on_call ot
                LEFT JOIN dedup_oef_report r ON
                ot.employee_id = r.employee_id AND
                ot.entry_date = r.entry_date AND
                ot.project_name = r.project_name AND
                ot.task_name = r.task_name AND (
                NULLIF(ot.select_a_type,'') = NULLIF(r.select_a_type,'') OR
                NULLIF(ot.select_type,'') = NULLIF(r.select_type,'') OR
                NULLIF(ot.type_selection,'') = NULLIF(r.type_selection,'') OR
                NULLIF(ot.ro_select_type,'') = NULLIF(r.ro_select_type,'') OR
                NULLIF(ot.select_clm,'') = NULLIF(r.select_clm,'') OR
                NULLIF(ot.select_an_option,'') = NULLIF(r.select_an_option,'') OR
                NULLIF(ot.ksa_select_type,'') = NULLIF(r.ksa_select_type,'') OR
                NULLIF(ot.nl_select_activity,'') = NULLIF(r.nl_select_activity,'') OR
                NULLIF(ot.select_spain,'') = NULLIF(r.select_spain,'') OR
                NULLIF(ot.belgium_activity_selection,'') = NULLIF(r.belgium_activity_selection,'') OR
                NULLIF(ot.ireland_activity,'') = NULLIF(r.ireland_activity,'') OR
                NULLIF(ot.at_select_activity_all_in,'') = NULLIF(r.at_select_activity_all_in,'') OR
                NULLIF(ot.ch_activity,'') = NULLIF(r.ch_activity,'') OR
                NULLIF(ot.uk_select_type,'') = NULLIF(r.uk_select_type,'') OR
                NULLIF(ot.uk_overtime,'') = NULLIF(r.uk_overtime,''))"""
           )
            ...
            process_each_time_period_record_ot_oncall_callout = rail.trigger_parallel_dagrun(
                ...
                trigger_dag_id=f"{config.process_ot_time_period_child}_{cnt}_{config.instance}_v2",
                conf=lambda item:{
            ...
            log_to_sumo_oef_export_data = rail.SendToSumoOperator(
                ...
                data={
                    ...
                    "TimeStamp": '{{ current_time_in_specified_tz("Etc/UTC", "%Y-%m-%dT%H:%M:%S") }}',
                    "Details":"Export contains OEF entries (oncall/callout/overtime)."
                },
                sumo_conn_id=config.sumo_conn_id
            )
            ...
            can_trigger_ot_oncall >> rail.Label("Yes") >> query_valid_oef_time_export_collection
            can_trigger_ot_oncall >> rail.Label("No") >> send_regular_time_entry_submission_complete >> log_to_sumo
            query_valid_oef_time_export_collection >>\
            if_valid_ot_oncall_callout_time_export_data >> rail.Label("Yes") >> get_export_name >> \
            update_export_name_to_oef_data >> log_to_sumo_oef_export_data >> start_time_export_report >>\
            run_time_export_report >>\
            query_valid_ot_oncall_callout_time_export_collection >>\
            ...
            if_valid_ot_oncall_callout_time_export_data >> rail.Label("No") >> send_time_entry_submission_complete >> log_to_sumo >> can_fail_dag >> fail_dagrun
