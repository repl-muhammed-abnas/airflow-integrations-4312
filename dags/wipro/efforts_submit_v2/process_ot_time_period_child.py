...
def create_airflow_child_dag(config):
    for country in config.time_export_for_country:
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.process_ot_time_period_child}_{cnt}_{config.instance}_v2",
            ...
            create_collection_time_export = rail.CreateCollectionOperator(
                task_id="create_collection_time_export",
            ...
            query_per_entry_and_timeperiod = rail.QueryCollectionOperator(
                ...
                query="""SELECT * FROM  ot_time_export WHERE
                        ot_time_export.employee_id='{{dag_run.conf.employee_id}}'
                        AND ot_time_export.timesheet_period='{{dag_run.conf.timesheet_period}}' """
            )
