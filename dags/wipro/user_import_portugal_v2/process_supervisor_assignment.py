from wipro.user_import_portugal_v2.task import put_supervisor_table_settings
...
def create_aiflow_child(config):
    with rail.create_airflow_dag(
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task="batch_task",
            no_task="start_supervisor"
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id="batch_task",
            start_task="start_supervisor",
            end_task="catch_and_log_errors"
        )

        start_supervisor = rail.EmptyOperator(task_id="start_supervisor")
        ...
        validate_supervisor_employee_id_unequal = rail.IfOperator(
            ...
            yes_task="if_project_supervisor",
        ...
        if_project_supervisor = rail.IfOperator(
            task_id="if_project_supervisor",
            test=lambda dag_run: dag_run.conf["project_supervisor_id"],
            yes_task="get_project_supervisor_details",
            no_task="get_supervisor_user_details"
        )
        ...
        if_project_supervisor_in_replicon = rail.IfOperator(
            ...
            test='{{result("get_project_supervisor_details")| is_truthy}}',
            yes_task="if_project_supervisor_has_permissions",
            no_task="write_supervisor_not_present_log"
        )

        write_supervisor_not_present_log = rail.WriteLogOperator(
            task_id="write_supervisor_not_present_log",
            log='{{dag_run.conf.lookuptable}}',
            message="Supervisor not assigned",
            severity="Exception",
            properties=lambda dag_run: {
                ...
                "company_code": dag_run.conf["company_code"],
                "status": "Exception",
                "details": "Supervisor not assigned as" + dag_run.conf["project_supervisor_id"]+"not available in replicon.",


            }
        )

        if_project_supervisor_has_permissions = rail.IfOperator(
            task_id="if_project_supervisor_has_permissions",
            ...
            no_task="update_project_supervisor_permission"
        )

        update_project_supervisor_permission = rail.RepliconServiceOperator(
            task_id="update_project_supervisor_permission",
        ...
        get_user_uri = rail.PythonOperator(
            ...
            python_callable=custom_methods.get_supervisor_or_pm_uri
        )

        project_manager_flg = '{{"Y" if result("get_project_supervisor_details") else "N"}}'
        put_supervisor_table_view = put_supervisor_table_settings.get_put_table_view_setting_supervisor(
            '{{result("get_user_uri")}}', "supervisor", project_manager_flg)

        get_supervisor_uri = rail.EmptyOperator(task_id="get_supervisor_uri")

        if_initial_supervisor = rail.IfOperator(
            task_id="if_initial_supervisor",
            test=lambda dag_run: (
                dag_run.conf["Add_Update"] == "Add" or not dag_run.conf.get("supervisor_uri")),
        ...
        assign_initial_supervisor_to_employee = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "supervisorUri": custom_methods.get_supervisor_or_pm_uri(),
                "dateRange": {}
            }
        )
        ...
        validate_supervisor_employee_id_unequal >> rail.Label("Yes") >>\
            if_project_supervisor >> rail.Label("Yes") >>\
            get_project_supervisor_details >>\
            if_project_supervisor_in_replicon >> \
            rail.Label(
                "No") >> write_supervisor_not_present_log >> catch_and_log_errors
        if_project_supervisor_in_replicon >> \
            rail.Label("Yes") >> if_project_supervisor_has_permissions >>\
            rail.Label("Yes") >> get_supervisor_uri
        if_project_supervisor_has_permissions >> rail.Label("No") >>\
            update_project_supervisor_permission >> start_supervisor_table_settings
        if_project_supervisor >> rail.Label("No") >>\
        ...
        if_mailid_and_loginname_in_feed >> rail.Label("Yes") >>\
            ...
            start_supervisor_table_settings
        ...
        if_supervisor_has_permissions >> rail.Label("No") >>\
            ...
            start_supervisor_table_settings >> get_user_uri >> put_supervisor_table_view >> get_supervisor_uri >> if_initial_supervisor
        if_user_is_enabled >> rail.Label("No") >>\
            write_log_supervisor_disabled >> get_supervisor_uri
        if_initial_supervisor >> rail.Label(
            "Yes") >> assign_initial_supervisor_to_employee >> end_supervisor
