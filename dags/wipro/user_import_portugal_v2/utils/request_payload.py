...
from wipro.user_import_portugal_v2.utils import custom_methods
import rail
null = None
true = "true"
false = "false"
...
def get_location_hierarchy_payload():
    return {
        ...
        "parentUri": rail.result("get_portugal_parent_location_details")
    }
...
def get_new_entity_user_create_payload(dag_run, user_template_name):
    ...
    return {
        ...
        "modifications": {
            ...
            "extensionFields":  list(map(lambda i: {"value": i}, custom_methods.get_portugal_user_create_oefs(dag_run,))),
            "customFields": list(map(lambda i: {"value": i}, custom_methods.get_portugal_user_create_custom_fields(dag_run))),
            "permissionSets": [
                {
                    "modificationOptionUri": "urn:replicon:collection-modification-option:add",
                    "items": list(map(lambda i: {"permissionSetPolicy": i}, custom_methods.get_portugal_user_create_permissions(dag_run))),
                }],
...
def get_portugal_create_payload(dag_run):
    loginname = dag_run.conf["adid"] + "@wipro.com"
    employeetypeuri = rail.find_first_by_attr_and_get_attr(
        dag_run.conf["employeetypeuris"], "displayText", dag_run.conf['exempt_non_exempt'], "uri", "")
    return {
        "user": {
            ...
            "lastname": dag_run.conf["employee_last_name"] if dag_run.conf["employee_last_name"] else '.',
            ...
            "holidayCalendarAssignmentSchedule": null,
            "timeOffPolicy": null,
            "permissionSets": custom_methods.get_portugal_user_create_permissions(dag_run),
            "policySets": custom_methods.get_portugal_user_create_policy_sets(dag_run),
            "employeeType": null,
            ...
            "timesheetApprovalPath": {
                "uri": dag_run.conf["timesheet_system_approval_path_uri"] if dag_run.conf['employee_band']
                in ['GROUP D1', 'GROUP D2', 'GROUP E'] else dag_run.conf["timesheet_approval_pathuri"],
                "name": null
            },
            "expenseApprovalPath": null,
            "timeOffApprovalPath": {
                "uri": dag_run.conf["timeoff_approval_path_group_uri"] if dag_run.conf['employee_band']
                in ['GROUP D1', 'GROUP D2', 'GROUP E'] else dag_run.conf["timeoff_approval_path_uri"],
                "name": null
            },
            "workAuthorizationApprovalPath": {
                "uri": dag_run.conf["ot_request_approval_path_dop_uri"]} if dag_run.conf["personnel_subarea_text"] == "DOP Portugal" else ({
                    "uri": dag_run.conf["ot_request_approval_path_uri"]} if dag_run.conf["ot_request_approval_path_uri"] else null),
            "timeOffBalancePayoutApprovalPath": null,
            "customFieldValues": custom_methods.get_portugal_user_create_custom_fields(dag_run),
            "assignedActivities": [],
            "timeZone":  {
            ...
            "locationSchedule": custom_methods.get_portugal_user_create_location(dag_run),
            "divisionSchedule": [
                {
                    "division": {
                        "uri": dag_run.conf["legalentityuri"],
            ...
            "timesheetPeriodSchedule": [
                {
                    ...
                    (rail.parse_date(dag_run.conf["onsite_start_date"], "%Y-%m-%d") if dag_run.conf["onsite_start_date"]
                     and dag_run.conf["onsite_start_date"] not in custom_methods.INVALID_DATES else null)
                }
            ],
            "policyDataAccessScopes": custom_methods.get_portugal_user_create_policy_data_access(dag_run),
            "payRuleScriptSchedule": custom_methods.get_portugal_user_payrule_script(dag_run),
            "displayNameParameter": {
                "displayName": dag_run.conf["employee_first_name"] + " " +
                (dag_run.conf["employee_last_name"]
                 if dag_run.conf["employee_last_name"] else ".") + " " + dag_run.conf["employee_id"]
            },
            "decimalSeparatorUri": null,
            "numberGroupSeparatorUri": "urn:replicon:number-group-separator:language-default",
            "extensionFieldValues": custom_methods.get_portugal_user_create_oefs(dag_run)
        }
    }
...
def get_basic_user_details_update(dag_run):
    ...
    if dag_run.conf["employee_last_name"] != rail.result("get_update_user_details")["userDetails"]["lastName"]:
        last_name = dag_run.conf["employee_last_name"] if dag_run.conf["employee_last_name"] else "."
        basic_details_logs += "Last name updated"
    if first_name or last_name:
        display_name = dag_run.conf["employee_first_name"] + " " + \
            (dag_run.conf["employee_last_name"] if dag_run.conf["employee_last_name"] else ".") + \
    ...
    return {
        ...
        "modifications": {
            **get_basic_user_details_update(dag_run)[0],
            "locationSchedule": custom_methods.check_if_portugal_user_location_update(dag_run)[0],
            "extensionFields": custom_methods.get_extension_field_values_updates(dag_run)[0],
            "customFields": custom_methods.get_portugal_user_update_custom_fields(dag_run)[0],
            ...
            "holidayCalendar": custom_methods.get_holiday_calendar(dag_run)[0] if not dag_run.conf["new_entity_flag"] else null,
            "workAuthorizationApprovalPath": custom_methods.get_ot_request_approval_path(dag_run)[0] if not dag_run.conf["new_entity_flag"] else null,
