...
from wipro.user_import_portugal_v2.task import put_user_and_table_settings, put_supervisor_table_settings
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task="batch_task",
            no_task="get_hr_manager_belongs_to_portugal"
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id="batch_task",
            start_task="get_hr_manager_belongs_to_portugal",
            end_task="catch_and_log_errors"
        )

        get_hr_manager_belongs_to_portugal = rail.EmptyOperator(
            task_id="get_hr_manager_belongs_to_portugal"
        )

        create_portugal_user = rail.RepliconServiceOperator(
            task_id="create_portugal_user",
            endpoint="/services/ImportService1.svc/PutUser3",
            data=request_payload.get_portugal_create_payload,
        ...
        write_supervisor_pending_logs = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "Add_Update": "Add",
                "useruri": rail.result("create_portugal_user"),
                "project_supervisor_id": None
            }
        )
        ...
        unassign_products_for_user = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "user": {
                    "uri": rail.result("create_portugal_user"),
                    "loginName": null,
        ...
        put_view_settings_for_user = put_user_and_table_settings.get_put_table_view_setting(
            '{{result("create_portugal_user")}}', "portugal", 'user')
        ...
        put_manager_table_view = put_supervisor_table_settings.get_put_table_view_setting_supervisor(
            '{{result("create_portugal_user")}}', 'supervisor_user', '{{dag_run.conf.project_manager_flg}}')
        ...
        remove_all_time_off_types = rail.RepliconServiceOperator(
            ...
            data={
                    "userUri": '{{result("create_portugal_user")}}',
                    "timeOffTypeUris": []
            }
        )
        ...
        assign_all_time_off_types = rail.RepliconServiceOperator(
            ...
            endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_portugal_user")}}'),
        ...
        if_travelport_user = rail.IfOperator(
            task_id="if_travelport_user",
            test=lambda dag_run: dag_run.conf["acquired_company"].lower(
            ) == 'travelport',
            yes_task="time_off_end",
            no_task="get_annual_leave_type_policy_schedule_for_user"
        )

        get_annual_leave_type_policy_schedule_for_user = rail.RepliconServiceOperator(
            task_id='get_annual_leave_type_policy_schedule_for_user',
            endpoint="/services/TimeOffPolicyService2.svc/GetDefaultTimeOffTypePolicyScheduleForUser",
            data={
                "timeOffAccount": {
                    "userUri": '{{result("create_portugal_user")}}',
        ...
        update_annual_timeoff_policy_line = rail.RepliconServiceOperator(
            task_id='update_annual_timeoff_policy_line',
            endpoint="/services/TimeOffPolicyService2.svc/PutUserTimeOffAccountPolicySetSchedule",
            data=lambda dag_run: {
                "timeOffAccount": {
                    "userUri": rail.render_template('{{result("create_portugal_user")}}'),
                ...
                "policySetScheduleEntries": custom_methods.get_annual_policy_set_schedule_entries(dag_run, config)
            }
        )
        ...
        can_run_batch_task >> rail.Label("No") >>\
            get_hr_manager_belongs_to_portugal >>\
            create_portugal_user >>\
        ...
        if_no_gender >> rail.Label("No") >>\
            ...
            replace_effective_date >> assign_default_timeoff_policy_for_assignee >> if_travelport_user
        if_user_is_assignee >> rail.Label("No") >>\
            assign_default_timeoff_policy >> if_travelport_user
        if_travelport_user >> rail.Label("Yes") >> time_off_end
        if_travelport_user >> rail.Label("No") >> get_annual_leave_type_policy_schedule_for_user >> update_annual_timeoff_policy_line >>\
            time_off_end >>\
