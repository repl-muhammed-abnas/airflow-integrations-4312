...
from wipro.user_import_portugal_v2.task import process_user_group_data
from wipro.user_import_portugal_v2.utils import custom_methods, request_payload
import rail
null = None
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        query_valid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM portugal_users WHERE NULLIF("employee_id","") IS NOT NULL AND
            ...
            NULLIF("adid","") IS NOT NULL AND NULLIF("location","") IS NOT NULL AND NULLIF("personnel_area_text","") IS NOT NULL AND NULLIF("personnel_subarea_text","") IS NOT NULL""",
            name="valid_users"
        )

        query_invalid_user_records = rail.QueryCollectionOperator(
            task_id="query_invalid_user_records",
            query="""SELECT * FROM portugal_users WHERE NULLIF("employee_id","") IS NULL OR
            ...
            NULLIF("location","") IS NULL OR NULLIF("personnel_area_text","") IS NULL OR
            NULLIF("personnel_subarea_text","") IS NULL"""
        )
        ...
        process_user_for_portugal = rail.trigger_parallel_dagrun(
            task_id="process_user_for_portugal",
            ...
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item, config=config: custom_methods.get_portugal_user_conf(
                item, config)
        )
        ...
        process_supervisor_for_portugal = rail.trigger_parallel_dagrun(
            task_id="process_supervisor_for_portugal",
            items='{{result("load_supervisor_data")|to_json}}',
            trigger_dag_id=config.create_supervisor_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item, config=config: custom_methods.get_portugal_supervisor_conf(
                item, config)
        )

        process_user_end = rail.EmptyOperator(task_id="process_user_end")

        process_logs_for_portugal = rail.TriggerDagRunOperator(
            task_id="process_logs_for_portugal",
        ...
        if_invalid_users >> rail.Label("No") >>\
            ...
            process_user_start >>\
            process_user_for_portugal >> get_supervisor_pending_logs >>\
            load_supervisor_data >> process_supervisor_for_portugal >>\
            process_user_end >> process_logs_for_portugal
