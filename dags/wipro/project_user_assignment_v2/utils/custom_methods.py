...
from wipro.project_user_assignment_v2.utils import custom_methods
...
def merge_task_data(task_data):
    result = []
    for task in task_data:
        employee_uri = None
        for item in task:
            if 'employee_uri' in item:
                employee_uri = item['employee_uri']
        for item in task:
            if 'task_uri' in item:
                task_copy = item.copy()
                if employee_uri:
                    task_copy['employee_uri'] = employee_uri
                result.append(task_copy)

        if employee_uri and not any('task_uri' in item for item in task):
            result.append({'employee_uri': employee_uri})
...
def get_task_to_add_update_skip():
    ...
    all_digq = bool(task_to_process) and all(t.get('taskclassification') == 'DIGQ' for t in task_to_process)

    if custom_methods.is_classification_changed() and not all_digq:
        task_to_disable = [t for t in current_task_in_project if t.get('task_classification') != 'DIGQ' and not t.get('closed')]
        current_task_in_project = [t for t in current_task_in_project if t.get('task_classification') == 'DIGQ' and not t.get('closed')]
    ...
    for task in task_to_process:
        ...
        if task_details:
            ...
            assignment_start_date = list(filter(
                lambda item: item.get('task_uri', '') == task_details['uri'] and item.get('employee_uri', '') == task['user_uri'],
                merge_task_data(rail.result("get_resource_start_dates"))
            ))
            task['assignment_start_date'] = assignment_start_date[0]['assignment_start_date'] if assignment_start_date else None
        ...
        if task_counter[task['taskcode']] == 1:
            ...
            task_mapper_for_add[task['taskcode']].append(task)
    ...
    return {
        ...
        'tasks_to_disable': task_to_disable,
        'action': 'update'
    }
...
def map_task_success_error(task_id, action):
        ...
        for idx, task_res in enumerate(task_add_update_result):
            ...
            for item in task_data.values():
                for data in item:
                    ...
                    data['status'] = status
                    data['details'] = msg if not checker else "Task and the User Assignment were skipped since the specified Task name already exists with the different task code."
                    res.append(data)
        return res
    ...
    for idx, task_res in enumerate(task_add_update_result):
        ...
        if action == 'add':
            ...
            msg = "Task and the User Assignment Updated Successfully"
        else:
        ...
        task_detail['details'] = msg if not checker else "Task and the User Assignment were skipped since the specified Task name already exists with the different task code."
        res.append(task_list[idx])
    return res

def load_project_data_from_query_callable():
    all_records = rail.load_all_records(rail.result("get_project_data_from_query"))
    first_record = all_records[0]

    # If first record is a DIGQ task row, project classification will be empty.
    # Find the first non-DIGQ row and use its classification for the project.
    if not first_record.get('classification') and first_record.get('taskclassification') == 'DIGQ':
        non_digq_record = next(
            (r for r in all_records if r.get('taskclassification') != 'DIGQ' and r.get('classification')),
            None
        )
        if non_digq_record:
            first_record = {**first_record, 'classification': non_digq_record['classification']}

    return first_record

def is_classification_changed():
    project_details = rail.result('get_project_details')
    classification = rail.result('load_project_data_from_query')['classification']
    return False if not project_details else rail.find_first_by_attr_and_get_attr(
        project_details['customFields'], 'customField.displayText', 'Classification', 'text') != classification

def is_all_digq_tasks():
    task_to_process = rail.load_all_records(rail.result("get_project_data_from_query"))
    return bool(task_to_process) and all(t.get('taskclassification') == 'DIGQ' for t in task_to_process)
