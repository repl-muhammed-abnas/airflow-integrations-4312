...
def get_log_message():
    msg =''
    if get_task_state("log_user_skipped") and get_task_state("log_user_skipped").lower() == "success":
...
def create_projectorapply_modifications(dag_run):
    ...
    # Build customFieldsToApply list
    custom_fields = [
        {
            "customField": {
    ...
    if not custom_methods.is_all_digq_tasks():
        custom_fields.append({
            "customField": {
                "uri": dag_run.conf['classification_project_udf_uri']
            },
            "text": get_project_data['classification']
        })
...
def get_add_task_payload(dag_run,data):
    ...
    for item in data:
        ...
        "taskModificationToApply": {
                "name": task_details['taskname'] + '_' + task_details['taskcode'],
                "codeToApply": {
                ...
                "customFieldsToApply": [
                    {
                        "customField": {
                            "uri": dag_run.conf["classification_task_udf_uri"],
                        },
                        "text": task_details['taskclassification'],
                    }
                ] if task_details['taskclassification'] == 'DIGQ' else [],
                "resourceTaskAssignmentModifications": {
                    "resourceAllocationsToAdd": list(map( lambda res :
                        ...
                        "dateRange": {
                            "startDate": rail.parse_date(res['taskstartdate'],'%Y%m%d') if res['taskclassification'] == 'DIGQ' else rail.parse_date(res['userstartdate'],'%Y-%m-%d'),
                            "endDate": rail.parse_date(res['taskenddate'],'%Y%m%d') if res['taskclassification'] == 'DIGQ' else rail.parse_date(res['userenddate'],'%Y-%m-%d')
                        }
                        }
                    , all_records[0]))
                }
...
def get_task_payload(dag_run,action,data):
    return list(map(lambda task: {
        ...
        "taskModificationToApply": {
                "name": task['taskname'] + '_' + task['taskcode'],
                "codeToApply": {
                    "value": task['taskcode']
                },
                "isClosed": 0,
                ...
                ] if task['taskclassification'] == 'DIGQ' else [],
                "resourceTaskAssignmentModifications": {
                    "resourceAllocationsToAdd": [
                        ...
                        "dateRange": {
                           "startDate": rail.parse_date(task['taskstartdate'],'%Y%m%d') if task['taskclassification'] == 'DIGQ' else rail.parse_date(task['userstartdate'],'%Y-%m-%d'),
                           "endDate": rail.parse_date(task['taskenddate'],'%Y%m%d') if task['taskclassification'] == 'DIGQ' else rail.parse_date(task['userenddate'],'%Y-%m-%d')
                        }
                        }
                    ]
                }
...
def get_disable_task_payload(data):
    return list(map(lambda task: {
        "target": {"uri": task['uri']},
        "taskModificationToApply": {
                "name": task['task_name'] + '_' + task['task_code'],
                "codeToApply": {
                    "value": task['task_name'] + '_' + task['task_code']
                },
                "isClosed": 1,
                "timeAndExpenseEntryTypeToApply": {
                    "value": "urn:replicon:time-and-expense-entry-type:billable"
...
def get_batch_put_task_payload(dag_run):
    return {
        ...
        "taskHierarchy": get_add_task_payload(dag_run,rail.result("get_all_task_to_add_update")['tasks_to_add']),
        "taskModificationOptionUri": "urn:replicon:task-modification-option:save",
        "unitOfWorkId": str(uuid.uuid4())
    }

def get_batch_disable_task_payload():
    return {
        "project": {
            "uri": rail.result('create_project')['uri'],
        },
        "taskHierarchy": get_disable_task_payload(rail.result("get_all_task_to_add_update")['tasks_to_disable']),
        "taskModificationOptionUri": "urn:replicon:task-modification-option:save",
        "unitOfWorkId": str(uuid.uuid4())
    }
...
def get_update_oef_payload(dag_run):
    return {
        "target": {
            "uri": rail.result('create_project')['uri'],
        },
            ...
            "customFieldsToApply": [
                {
                    ...
                    "text": rail.result('load_project_data_from_query')['classification']
                }
            ]
        },
        "projectModificationOptionUri": "urn:replicon:project-modification-option:save",
