def format_project_task_details(response):
    def get_task_classification(custom_fields):
        for cf in custom_fields:
            if cf.get('customField', {}).get('displayText') == 'Task_Classification':
                return cf.get('text', '')
        return ''
    ...
    return list(map(lambda task: {
        ...
        "task_classification": get_task_classification(task.get('customFields', []))
    }, response))
