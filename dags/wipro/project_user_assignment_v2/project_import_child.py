...
import rail
from wipro.project_user_assignment_v2.utils import custom_methods,response_filter
    ...
    with rail.create_airflow_dag(
        ...
        create_collection_input_data = rail.CreateCollectionOperator(
            ...
            columns= {
                ...
                "CLASSIFICATION": "classification",          # Changed from "Classification"
                "TASK_CLASSIFICATION": "taskclassification", # Changed from "Task_classification"
            }
        )
        ...
        query_any_blankmandatory_check = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM inputdata WHERE (NULLIF(empid,'') IS NULL OR NULLIF(projectcode,'') IS NULL OR
                NULLIF(projectname,'') IS NULL OR NULLIF(taskname,'') IS NULL)"""
        )
        ...
        query_valid_data_from_rawdata = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM inputdata WHERE (NULLIF(empid,'') IS NOT NULL AND NULLIF(projectcode,'') IS NOT NULL AND
                NULLIF(projectname,'') IS NOT NULL AND NULLIF(taskname,'') IS NOT NULL)""",
            name= 'final_data'
        )
        ...
        get_classification_project_udf = rail.RepliconServiceOperator(
            task_id="get_classification_project_udf",
            endpoint="/services/CustomFieldService1.svc/GetAllCustomFields",
            data={
                "objectUri": "urn:replicon:object-type:project"
            },
            data_handler=lambda response: rail.find_first_by_attr_and_get_attr(response, 'displayText', 'Classification', 'uri')
        )

        get_classification_task_udf = rail.RepliconServiceOperator(
            task_id="get_classification_task_udf",
            endpoint="/services/CustomFieldService1.svc/GetAllCustomFields",
            data={
                "objectUri": "urn:replicon:object-type:task"
            },
            data_handler=lambda response: rail.find_first_by_attr_and_get_attr(response, 'displayText', 'Task_Classification', 'uri')
        )
        ...
        process_each_projects = rail.trigger_parallel_dagrun(
            ...
            conf=lambda item: {
                ...
                'classification_project_udf_uri': rail.result("get_classification_project_udf"),
                'classification_task_udf_uri': rail.result("get_classification_task_udf"),
        ...
        has_any_valid_users >> rail.Label(
            "Yes") >> map_user_details_with_feed >> create_final_valid_data_collection >> query_distict_projects >>\
                get_project_last_modified_date_udf >> get_classification_project_udf >> get_classification_task_udf >> get_project_export_type_oef_details >> \
                    get_oef_drop_down_project_export_type_values >> get_all_employeetypes >> process_each_projects >> log_project_details
