...
import functools
from wipro.user_import_ireland_v1.utils import request_payload
...
def get_location_hierarchy_data(response):
    ...
    parent_uri=rail.result("get_ireland_parent_location_details")
...
def get_ireland_object_extension_fields():
...
def get_ireland_time_off_types(item,config):
    onsite_recruit = item["onsite_direct_recruit"]
    acquired_company = item["acquired_company"].lower().replace(" ","_")
    if acquired_company not in ['appirio_ltd', "rizing_consulting_ireland_limited"]:
         acquired_company = "other_acquired"
    get_all_timeoff_types = []
    if item["gender"].lower() in ["female", "male"]:
        get_all_timeoff_types.extend(
            config.TIME_OFF_TYPES_MAPPER["local_hire_or_assignee"][acquired_company]["all"])
        get_all_timeoff_types.extend(
            config.TIME_OFF_TYPES_MAPPER["local_hire_or_assignee"][acquired_company][item["gender"].lower()])
        if onsite_recruit.lower() == "local_hire":
            get_all_timeoff_types.extend(
                config.TIME_OFF_TYPES_MAPPER["local_hire"][acquired_company]["all"])
            get_all_timeoff_types.extend(
                config.TIME_OFF_TYPES_MAPPER["local_hire"][acquired_company][item["gender"].lower()])
        elif onsite_recruit.lower() == "assignee":
            get_all_timeoff_types.extend(
                config.TIME_OFF_TYPES_MAPPER["assignee"][acquired_company]["all"])
            get_all_timeoff_types.extend(
                config.TIME_OFF_TYPES_MAPPER["assignee"][acquired_company][item["gender"].lower()])
...
def get_approval_path_uri(country,config):
    return {
        ...
        "timesheet_full_time_uri": rail.find_first_by_attr_and_get_attr(
            rail.result("get_all_policy_sets"), "displayText", config.GENERAL_MAPPER["timesheet_template_for_full_time"], "uri"),
        ...
        "schedule_policyuri": rail.result("get_ireland_schedule_policy")
    }


def get_ireland_supervisor_conf(item,config):
    return {
        **item,
...
def get_ireland_annual_acquistion_uri():
    return{
        ...
        "yearly_carry_over_expiry_uri": rail.find_first_by_attr_and_get_attr(
            rail.result("get_all_timeoff_event_scripts"),
            "displayText",
...
def get_ireland_user_conf(item,config):
    ...
    return {
        **item,
        **get_ireland_object_extension_fields(),
        **get_ireland_custom_fields(),
        "locationcountryuri": rail.result("get_ireland_parent_location_details"),
        ...
        **get_approval_path_uri(item["country"],config),
        "timeoff_type_uris": get_ireland_time_off_types(item,config),
        ...
        **get_ireland_annual_acquistion_uri(),
...
def check_if_ireland_user_location_update(dag_run):
...
def get_ireland_user_create_permissions(dag_run):
    permission_sets = []
...
def get_ireland_user_create_policy_data_access(dag_run):
    policy_data_access = []
    if dag_run.conf["hr_manager_flg"] == "Y":
...
def get_ireland_user_create_policy_sets(dag_run):
    policy_sets = []
    policy_sets.append(
        {
    ...
    policy_sets.append({
        "uri": dag_run.conf["timesheet_full_time_uri"],
        "name": null
    }
    )
...
def get_ireland_user_create_custom_fields(dag_run):
    custom_field_values = []
    ...
    for i in custom_fields:
        if dag_run.conf[i] and dag_run.conf[i] not in INVALID_DATES:
            if i in ['onsite_start_date', 'travel_start_date']:
                if (dag_run.conf['onsite_start_date'] == dag_run.conf['travel_start_date'] and\
                     dag_run.conf["onsite_direct_recruit"].lower() == "assignee") or\
                         dag_run.conf["onsite_direct_recruit"].lower() != "assignee" :
...
def get_ireland_user_create_location(dag_run):
    location = []
...
def get_ireland_user_payrule_script(dag_run):
    return [
        {
            "payRuleScript": {
...
def get_ireland_user_create_oefs(dag_run):
    ...
    extension_fields = ["project_supervisor_id", "project_supervisor_mailid",
                        ...
                        "personnel_area_text", "personnel_subarea_text","religion",
                        "project_supervisor_adid"]
...
def get_updated_logs(dag_run):
    ...
    custom_field_logs = get_ireland_user_update_custom_fields(dag_run)[1]
    basic_details = request_payload.get_basic_user_details_update(dag_run)[1]
    location_logs = check_if_ireland_user_location_update(dag_run)[1]
    if oef_logs:
...
def check_if_end_date(dag_run):
    ...
    if end_date:
        ...
        if start_date:
            start_date = str(
                start_date["year"]) + "/" + str(start_date["month"]) + "/" + str(start_date["day"])
        ...
        if current_end_date:
            current_end_date = str(current_end_date["year"]) + "/" + str(
                current_end_date["month"]) + "/" + str(current_end_date["day"])
            current_end_date = datetime.strftime(
                datetime.strptime(current_end_date,  "%Y/%m/%d"), "%Y-%m-%d")
...
def get_extension_field_values_updates(dag_run):
    ...
    extension_fields = {"project_supervisor_id":"Project Supervisor ID",
                        ...
                        "employment_percentage":"FTE", "forfait_emp_identifier":"Forfait jour employment identifier",
...
def get_ireland_user_update_custom_fields(dag_run):
    custom_field_values = []
