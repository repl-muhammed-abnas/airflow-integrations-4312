...
from wipro.user_import_ireland_v1.utils import custom_methods
import rail
import calendar
null = None
true = "true"
...
def get_location_hierarchy_payload():
    return {
        ...
        "parentUri": rail.result("get_ireland_parent_location_details")
    }
...
def get_ireland_create_payload(dag_run):
    ...
    return {
        "user": {
            ...
            "holidayCalendarAssignmentSchedule": null,
            "timeOffPolicy": null,
            "permissionSets": custom_methods.get_ireland_user_create_permissions(dag_run),
            "policySets": custom_methods.get_ireland_user_create_policy_sets(dag_run),
            "employeeType": null,
            ...
            "customFieldValues": custom_methods.get_ireland_user_create_custom_fields(dag_run),
            "assignedActivities": [],
            "timeZone":  {
            ...
            "locationSchedule": custom_methods.get_ireland_user_create_location(dag_run),
            "divisionSchedule": [
                {
                    "division": {
                        "uri": dag_run.conf["legalentityuri"],
            ...
            "policyDataAccessScopes": custom_methods.get_ireland_user_create_policy_data_access(dag_run),
            "payRuleScriptSchedule": custom_methods.get_ireland_user_payrule_script(dag_run) if dag_run.conf["payrule_uri"] else null,
            "displayNameParameter": {
            ...
            "extensionFieldValues": custom_methods.get_ireland_user_create_oefs(dag_run)
        }
    }
...
def get_update_user_req(dag_run):
  ...
  "modifications": {
    **get_basic_user_details_update(dag_run)[0],
    "locationSchedule":custom_methods.check_if_ireland_user_location_update(dag_run)[0],
    "extensionFields": custom_methods.get_extension_field_values_updates(dag_run)[0],
    "customFields": custom_methods.get_ireland_user_update_custom_fields(dag_run)[0],
    ...
    payload = {
        ...
        "filterExpression": {
            "leftExpression": {
                ...
                "rightExpression": {
                    "value": {
                        ...
                        "money": null,
                        "number": null,
                        "text": dag_run.conf["hr_manager_id"],
                    },
                    "filterDefinitionUri": null
                },
                "value": null,
            ...
            "rightExpression": {
                ...
                "rightExpression": {
                    "value": {
                        # the country URI will always be of Ireland
                        "uri":  dag_run.conf["countryuri"],
                        "uris": [],
                    },
                    "filterDefinitionUri": null
...
def get_timeoff_balance(**effective_date):
    year = effective_date.get('year')
    month = effective_date.get('month')
    day = effective_date.get('day')

    joining_date, year_enddate = date(year, month, day), date(year, 12, 31)
    total_days = 366 if calendar.isleap(year) and joining_date <= date(year, 2, 29) else 365
    days_remaining = (year_enddate - joining_date).days + 1 # adding 1 as we are calculating the joining date as well
    balance = (days_remaining * 20) / total_days

    return f"{balance:.2f}"

def get_ireland_annual_acquistion_payload(dag_run):
    effective_date = rail.parse_date(
        dag_run.conf["date_of_joining"], "%Y-%m-%d")
    balance = get_timeoff_balance(**effective_date)
    return {
        "timeOffAccount": {
            "userUri": rail.result("create_ireland_user"),
        ...
        "policySetScheduleEntries": [
            {
                ...
                "policySet": {
                    ...
                    {
                        "scriptTarget": {
                        "uri": dag_run.conf['yearly_carry_over_expiry_uri']
                        },
                        "additionalParameters": [
                        {
                        ...
                        },
                        {
                            ...
                            "value": {
                            "uri": "urn:replicon:time-off-upon-expiry-option:pay-out"
                            }
                        },
                        {
                            "keyUri": "urn:replicon:script-key:parameter:precedence",
