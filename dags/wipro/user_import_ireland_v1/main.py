...
from wipro.user_import_ireland_v1.task import process_user_group_data
from wipro.user_import_ireland_v1.utils import custom_methods, request_payload
import rail
null = None
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        query_valid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM ireland_users WHERE NULLIF("employee_id","") IS NOT NULL AND
            ...
            name="ireland_valid_users"
        )

        query_invalid_user_records = rail.QueryCollectionOperator(
            task_id="query_invalid_user_records",
            query="""SELECT * FROM ireland_users WHERE NULLIF("employee_id","") IS NULL OR
        ...
        process_user_for_ireland = rail.trigger_parallel_dagrun(
            task_id="process_user_for_ireland",
            items='{{result("query_valid_user_records")}}',
            trigger_dag_id=config.valid_user_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item,config=config:custom_methods.get_ireland_user_conf(item,config)
        )
        ...
        process_supervisor_for_ireland = rail.trigger_parallel_dagrun(
            task_id="process_supervisor_for_ireland",
            items='{{result("load_supervisor_data")|to_json}}',
            trigger_dag_id=config.create_supervisor_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item,config=config:custom_methods.get_ireland_supervisor_conf(item,config)
        )

        process_user_end = rail.EmptyOperator(task_id="process_user_end")

        process_logs_for_ireland = rail.TriggerDagRunOperator(
            task_id="process_logs_for_ireland",
        ...
        if_invalid_users >> rail.Label("No") >>\
            ...
            process_user_start >>\
            process_user_for_ireland >> get_supervisor_pending_logs>>\
            load_supervisor_data >> process_supervisor_for_ireland>>\
        process_user_end >> process_logs_for_ireland >> end_user
