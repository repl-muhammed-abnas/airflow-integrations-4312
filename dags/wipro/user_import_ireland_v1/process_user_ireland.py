...
from wipro.user_import_ireland_v1.utils import request_payload, custom_methods
import rail
null = None
    ...
    with rail.create_airflow_dag(
        ...
        get_current_group_membership = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response: {
                ...
                "existingdepartment":  response.get('departments', [{}])[0].get('department', {}).get('department', {}).get('displayText') if response.get('departments') else None,
        ...
        process_update_user = rail.TriggerDagRunOperator(
            ...
            conf=lambda dag_run:
            {
                **dag_run.conf,
                ...
                "get_current_group_membership").get("existingdepartment", "")
            }
        )

        start_process_user = rail.EmptyOperator(
            task_id="start_process_user"
        )
        ...
        if_user_not_exists >> rail.Label("Yes") >> start_process_user >> start_process_add_user
        ...
        write_disable_tranferred_user_log>> start_process_add_user >>\
