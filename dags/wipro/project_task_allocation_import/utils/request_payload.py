    ...
    for item in data:
        ...
        "taskModificationToApply": {
                ...
                "timeEntryStartDateToApply": {
                    "date": rail.parse_date(task_details['taskstartdate'],'%Y%m%d')
                    },
                    "timeEntryEndDateToApply": {
                    "date": rail.parse_date(task_details['taskenddate'],'%Y%m%d')
                },
                ...
                "resourceTaskAssignmentModifications": {
                    "resourceAllocationsToAdd": list(map( lambda res :
                        ...
                        "dateRange": {
                            "startDate": rail.parse_date(res['userstartdate'],'%Y-%m-%d'),
                            "endDate": rail.parse_date(res['userenddate'],'%Y-%m-%d')
                        }
                        }
                    , all_records[0]))
                }
...
def get_task_payload(action,data):
    return list(map(lambda task: {
        ...
        "taskModificationToApply": {
                ...
                "isClosed": 0,
                "timeEntryStartDateToApply": {
                    "date": rail.parse_date(task['taskstartdate'],'%Y%m%d')
                    },
                    "timeEntryEndDateToApply": {
                    "date": rail.parse_date(task['taskenddate'],'%Y%m%d')
                },
                "timeAndExpenseEntryTypeToApply": {
                    "value": "urn:replicon:time-and-expense-entry-type:billable"
                },
                ...
                "resourceTaskAssignmentModifications": {
                    "resourceAllocationsToAdd": [
                        ...
                        "dateRange": {
                            "startDate": rail.parse_date(task['userstartdate'],'%Y-%m-%d'),
                            "endDate": rail.parse_date(task['userenddate'],'%Y-%m-%d')
                        }
                        }
                    ]
                }
