...
from wipro.user_import_switzerland.task import put_supervisor_table_settings
import rail
...
def update_user_roles(config):
    with rail.TaskGroup(
        group_id="update_switzerland_user_roles",
        ...
        assign_project_manager_permission = rail.SetVariableOperator(
            ...
            name='{{result("create_manager_permission_variable").name}}',
            value=lambda dag_run: [
                dag_run.conf["project_manager_uri"], dag_run.conf["l1_manager_uri"], dag_run.conf["end_user_manager_uri"]],
            append=True
        )
