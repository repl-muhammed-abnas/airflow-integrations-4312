import json
from wipro.user_import_switzerland.task import put_user_and_table_settings, put_supervisor_table_settings
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task="batch_task",
            no_task="create_switzerland_user"
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id="batch_task",
            start_task="create_switzerland_user",
            end_task="catch_and_log_errors"
        )

        create_switzerland_user = rail.RepliconServiceOperator(
            task_id="create_switzerland_user",
            endpoint="/services/ImportService1.svc/PutUser3",
            data=request_payload.get_switzerland_create_payload,
        ...
        write_supervisor_pending_logs = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "Add_Update": "Add",
                "useruri": rail.result("create_switzerland_user")
            }
        )
        ...
        unassign_products_for_user = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "user": {
                    "uri": rail.result("create_switzerland_user"),
                    "loginName": null,
        ...
        put_view_settings_for_user = put_user_and_table_settings.get_put_table_view_setting(
            '{{result("create_switzerland_user")}}', "switzerland", 'user')
        ...
        put_manager_table_view = put_supervisor_table_settings.get_put_table_view_setting_supervisor(
            '{{result("create_switzerland_user")}}', 'supervisor_user')
        ...
        remove_all_time_off_types = rail.RepliconServiceOperator(
            ...
            data={
                    "userUri": '{{result("create_switzerland_user")}}',
                    "timeOffTypeUris": []
            }
        )
        ...
        assign_all_time_off_types = rail.RepliconServiceOperator(
            ...
            endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_switzerland_user")}}'),
                    "timeOffTypeUris": list(chain.from_iterable(dag_run.conf["timeoff_type_uris"].values()))
            }
        )
        ...
        get_default_time_off_type_policy_schedule_for_user = rail.RepliconServiceCallForEachItemOperator(
            ...
            items=lambda dag_run: list(chain.from_iterable(dag_run.conf["timeoff_type_uris"].values())),
            data={
                "timeOffAccount": {
                    "userUri": '{{result("create_switzerland_user")}}',
                    "timeOffTypeUri": "{{ item }}"
        ...
        assign_default_timeoff_policy_for_assignee = rail.RepliconServiceCallForEachItemOperator(
            ...
            data=lambda dag_run, item: {
                ...
                "policySetScheduleEntries": json.loads(json.dumps(rail.result('replace_effective_date')
                    [list(chain.from_iterable(dag_run.conf["timeoff_type_uris"].values())).index(item)])
        ...
        disable_time_off_types = rail.RepliconServiceOperator(
            task_id="disable_time_off_types",
            endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_switzerland_user")}}'),
                    "timeOffTypeUris": dag_run.conf["timeoff_type_uris"]["enabled"]
            }
        )
        ...
        if_no_gender >> rail.Label("No") >>\
            ...
            replace_effective_date >> assign_default_timeoff_policy_for_assignee >> disable_time_off_types >> time_off_end
        if_user_is_assignee >> rail.Label("No") >>\
            assign_default_timeoff_policy >>\
            disable_time_off_types  >> time_off_end >>\
