from wipro.user_import_switzerland.task import update_roles_for_user
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        update_the_onsite_end_date = rail.RepliconServiceOperator(
            task_id="update_the_onsite_end_date",
            endpoint="/services/CustomFieldService1.svc/UpdateDateValue",
            data=lambda dag_run: {
                    "objectUri": rail.result('get_update_user_details')["userDetails"]["uri"],
        ...
        if_end_date_update = rail.IfOperator(
            ...
            test=lambda dag_run: custom_methods.check_if_end_date(dag_run),
        ...
        if_user_has_end_date >> rail.Label("Yes") >>\
            remove_end_date >> update_the_reversal_date >>\
                update_the_onsite_end_date >> if_end_date_update
