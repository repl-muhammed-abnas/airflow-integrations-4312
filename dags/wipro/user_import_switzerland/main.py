...
from wipro.user_import_switzerland.task import process_user_group_data
from wipro.user_import_switzerland.utils import custom_methods, request_payload
import rail
null = None
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        query_valid_user_records = rail.QueryCollectionOperator(
            ...
            query="SELECT * FROM switzerland_users WHERE NULLIF('employee_id','') IS NOT NULL AND\
            NULLIF('employee_first_name','') IS NOT NULL AND\
            NULLIF('date_of_joining','') IS NOT NULL AND date_of_joining NOT IN "+str(invalid_dates)+"\
            AND NULLIF('country','') IS NOT NULL AND\
            ...
            name="switzerland_valid_users"
        )

        query_invalid_user_records = rail.QueryCollectionOperator(
            task_id="query_invalid_user_records",
            query="SELECT * FROM switzerland_users WHERE NULLIF('employee_id','') IS NULL OR\
            NULLIF('employee_first_name','') IS NULL OR\
            NULLIF('date_of_joining','') IS NULL OR date_of_joining IN "+str(invalid_dates)+"\
            AND NULLIF('country','') IS NULL OR\
            NULLIF('location','') IS NULL OR NULLIF('employment_status','') IS NULL OR NULLIF('company_code','') IS  NULL OR\
            NULLIF('adid','') IS NULL "
        )
        ...
        process_user_for_switzerland = rail.trigger_parallel_dagrun(
            task_id="process_user_for_switzerland",
            items='{{result("query_valid_user_records")}}',
            trigger_dag_id=config.valid_user_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item, config=config: custom_methods.get_switzerland_user_conf(
                item, config)
        )
        ...
        process_supervisor_for_switzerland = rail.trigger_parallel_dagrun(
            task_id="process_supervisor_for_switzerland",
            items='{{result("load_supervisor_data")|to_json}}',
            trigger_dag_id=config.create_supervisor_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item, config=config: custom_methods.get_switzerland_supervisor_conf(
                item, config)
        )

        end_switzerland_user = rail.EmptyOperator(
            task_id="end_switzerland_user"
        )

        process_logs_for_switzerland = rail.TriggerDagRunOperator(
            task_id="process_logs_for_switzerland",
        ...
        if_invalid_users >> rail.Label("No") >>\
            ...
            process_user_start >>\
            process_user_for_switzerland >> get_supervisor_pending_logs >>\
            load_supervisor_data >> process_supervisor_for_switzerland >>\
            end_switzerland_user >> process_logs_for_switzerland
        if_valid_user_records >> rail.Label(
            "No") >> end_switzerland_user
        return dag
