...
def get_task_to_add_update_skip():
    ...
    for task in task_to_process:
        ...
        if task_details:
            ...
            assignment_start_date = list(filter(lambda item: item.get('task_uri','') == task_details['uri'] and item.get(
                'employee_uri','') == task['user_uri'],merge_task_data(rail.result("get_resource_start_dates"))))
