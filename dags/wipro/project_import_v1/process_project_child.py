import rail
from wipro.project_import_v1.utils import request_payload,response_filter,custom_methods
...
def create_child_dag_wbs(config):
    with rail.create_airflow_dag(
        ...
        is_project_manager_is_present = rail.IfOperator(
            ...
            no_task= 'allocate_users_to_project'
        )
        ...
        is_project_manager_available >> rail.Label(
            "Yes") >> get_project_manager_permission_set >> assign_permission_set >> assign_project_manager_to_project >> \
                allocate_users_to_project >> log_project_success

        is_project_manager_available >> rail.Label(
            "No") >> check_user_details_from_payload

        check_user_details_from_payload >> rail.Label(
            "Yes") >> log_user_skipped >> allocate_users_to_project

        check_user_details_from_payload >> rail.Label(
            "No") >> create_user >> put_product_assignments_for_user >> assign_project_manager_to_project

        log_project_success >> is_new_project >> rail.Label(
            "Yes") >> get_distinct_users >> get_resource_start_dates >> get_all_task_to_add_update >> has_tasks_to_add
