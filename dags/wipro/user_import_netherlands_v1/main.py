...
from wipro.user_import_netherlands_v1.task import process_user_group_data
from wipro.user_import_netherlands_v1.utils import custom_methods, request_payload
import rail
null = None
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        query_valid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM netherlands_users WHERE NULLIF("employee_id","") IS NOT NULL AND
        ...
        query_invalid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM netherlands_users WHERE NULLIF("employee_id","") IS NULL OR
        ...
        process_user_for_netherlands = rail.trigger_parallel_dagrun(
            task_id="process_user_for_netherlands",
            ...
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item,config=config:custom_methods.get_netherlands_user_conf(item,config)
        )
        ...
        process_supervisor_for_netherlands = rail.trigger_parallel_dagrun(
            task_id="process_supervisor_for_netherlands",
            items='{{result("load_supervisor_data")|to_json}}',
            trigger_dag_id=config.create_supervisor_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item,config=config:custom_methods.get_netherlands_supervisor_conf(item,config)
        )

        end_netherlands_user = rail.EmptyOperator(
            task_id="end_netherlands_user"
        )

        process_logs_for_netherlands = rail.TriggerDagRunOperator(
            task_id="process_logs_for_netherlands",
        ...
        if_invalid_users >> rail.Label("No") >>\
            ...
            process_user_start >>\
            process_user_for_netherlands >>\
            get_supervisor_pending_logs >> load_supervisor_data >>\
            process_supervisor_for_netherlands >>\
            end_netherlands_user
        if_valid_user_records >> rail.Label(
            "No") >> end_netherlands_user >> process_logs_for_netherlands
        return dag
