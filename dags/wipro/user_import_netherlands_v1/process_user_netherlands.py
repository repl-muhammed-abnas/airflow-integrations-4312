    ...
    with rail.create_airflow_dag(
        ...
        if_project_supervisor_with_permissions = rail.IfOperator(
            ...
            test=lambda: rail.find_first_by_attr_and_get_attr(
                ...
                rail.result("get_project_supervisor_details")["permissionSets"],
                "displayText",
                config.GENERAL_MAPPER["schedule_supervisor_permission"],
                "uri") and rail.find_first_by_attr_and_get_attr(
                rail.result("get_project_supervisor_details")["permissionSets"],
                "displayText",
                config.GENERAL_MAPPER["resource_supervisor_permission"],
                "uri") and rail.find_first_by_attr_and_get_attr(
                rail.result("get_project_supervisor_details")["permissionSets"],
                "displayText",
        ...
        if_reversal_date_present_or_active_user = rail.IfOperator(
            ...
            test=lambda dag_run: rail.result("get_user_details")["securityConfiguration"]["isLoginEnabled"] or\
                (not rail.result("get_user_details")["securityConfiguration"]["isLoginEnabled"] and
                                ...
                                datetime.strptime(dag_run.conf["ard_lrd"],"%Y-%m-%d") < datetime.today())),
        ...
        if_project_supervisor_in_replicon >> rail.Label("No")>> if_user_not_exists
        ...
        if_user_not_exists >> rail.Label("Yes") >> start_process_add_user >>\
        if_new_entity_user >> rail.Label("Yes") >> process_new_entity_add_user >> catch_and_log_errors
        if_new_entity_user >> rail.Label("No") >>\
        process_add_user >> catch_and_log_errors
        ...
        write_disable_tranferred_user_log >> if_new_entity_user
        if_company_code_changed >> rail.Label("No") >>\
        if_reversal_date_present_or_active_user >> rail.Label("Yes") >>\
        process_update_user >> catch_and_log_errors
