...
import rail
from wipro.annual_leave_balance_transfer_netherlands_v1.utils import python_callable
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        log_dag_run_and_report_run_date = rail.PythonOperator(
            task_id='log_dag_run_and_report_run_date',
            python_callable=lambda: {
                'current_date_time': DATE_IN_REQUIRED_TIMEZONE.strftime("%Y_%m_%d_%H_%M_%S"),
        ...
        def get_required_timeoff_type_uris(response):
            return {
                'timeoff_uris_to_pick_balance_from': rail.find_first_by_attr_and_get_attr(response, 'displayText', config.ANNUAL_LEAVE, 'uri'),
                'timeoff_uri_to_transfer_balance_into': [{
                    'name': config.ANNUAL_LEAVE_CARRIED_OVER,
                    'uri':rail.find_first_by_attr_and_get_attr(response, 'displayText', config.ANNUAL_LEAVE_CARRIED_OVER, 'uri')},
                    {
                    'name': config.ANNUAL_LEAVE_ADDITIONAL,
                    'uri':rail.find_first_by_attr_and_get_attr(response, 'displayText', config.ANNUAL_LEAVE_ADDITIONAL, 'uri')}
                ]
            }
        ...
        log_get_required_timeoff_type_uris = rail.RepliconServiceOperator(
            ...
            data_handler=get_required_timeoff_type_uris
        )
        ...
        def get_report_parameters():
            ...
            filter_values.append({
                "reportFilterUri": rail.find_first_by_attr_and_get_attr(enabled_filters, 'displayText', 'AsOfDateFilter', 'uri'),
                "value": "DateRange"
            })
            filter_values.append({
                "reportFilterUri": rail.find_first_by_attr_and_get_attr(enabled_filters, 'displayText', 'AsOfDateFilter', 'uri'),
                "value": rail.result('log_dag_run_and_report_run_date')['report_run_date']
            })
            filter_values.append({
                "reportFilterUri": rail.find_first_by_attr_and_get_attr(enabled_filters, 'displayText', 'AsOfDateFilter', 'uri'),
                "value": rail.result('log_dag_run_and_report_run_date')['report_run_date']
            ...
            filter_values.append({
                "reportFilterUri": rail.find_first_by_attr_and_get_attr(enabled_filters, 'displayText', 'TimeOffTypeFilter', 'uri'),
                "value": rail.result("log_get_required_timeoff_type_uris")['timeoff_uris_to_pick_balance_from'].split(":")[-1]
            })

            filter_values.append({
                "reportFilterUri": rail.find_first_by_attr_and_get_attr(enabled_filters, 'displayText', 'CurrentServiceCenterFilter', 'uri'),
        ...
        query_invalid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM report_data_collection WHERE NULLIF(login_name, '') IS NULL \
                or CAST(timeoff_balance AS REAL) <= 0 \
                or (CAST(timeoff_balance AS REAL) > 0 \
                    AND UPPER(TRIM(COALESCE(onsite_direct_recruit, ''))) NOT IN ('LOCAL_HIRE', 'ASSIGNEE'))""",
        ...
        def get_invalid_record_details(item):
            if not item['login_name']:
                return "Annual Leave Balance Transfer Not Processed as Login Name is not present in record"
            try:
                balance = float(item['timeoff_balance'])
            except (TypeError, ValueError):
                balance = 0.0
            if balance <= 0:
                return "Annual Leave Balance Transfer Not Processed as time off balance is 0 or less for the time off type- {} as of date {}".format(
                    item['timeoff_type'], rail.result('log_dag_run_and_report_run_date')['report_run_date'])
            return "Annual Leave Balance Transfer Not Processed as 'Onsite Direct Recruit' value '{}' is not {} or {}".format(
                item['onsite_direct_recruit'], config.ONSITE_DIRECT_RECRUIT_LOCAL_HIRE, config.ONSITE_DIRECT_RECRUIT_ASSIGNEE)
        ...
        log_invalid_records = rail.WriteLogOperator(
            ...
            properties= lambda item: {
                ...
                "details": get_invalid_record_details(item)
            }
        )

        query_valid_records = rail.QueryCollectionOperator(
            task_id='query_valid_records',
            query="""SELECT * FROM report_data_collection WHERE (CAST(timeoff_balance AS REAL) > 0 AND NULLIF(login_name, '') IS NOT NULL \
                AND UPPER(TRIM(COALESCE(onsite_direct_recruit, ''))) IN ('LOCAL_HIRE', 'ASSIGNEE'))""",
        ...
        trigger_dag_run_transfer_timeoff_balance = rail.trigger_parallel_dagrun(
            ...
            conf=lambda item: {
                ...
                "timeoff_type_uri_for_transferring_balance_into": rail.result("log_get_required_timeoff_type_uris")['timeoff_uri_to_transfer_balance_into'],
                "balance_to_transfer": python_callable.get_balance_to_transfer(
                    float(item['timeoff_balance']),
                    item['onsite_direct_recruit'],
                    item['user_start_date'],
                    rail.result('log_dag_run_and_report_run_date')['report_run_date']),
                "user_start_date": item['user_start_date'],
                "efective_date_for_new_policyset": rail.result('log_dag_run_and_report_run_date')['dag_run_date'],
        ...
        compose_logs_csv = rail.WriteCSVFileOperator(
            ...
            header=['login_name', 'status', 'details',"ECID | Run ID"],
            row=lambda item: [
        ...
        upload_logs_to_sftp = rail.SFTPUploadFileOperator(
            ...
            '''/{{get_company_key()}}_netherlands_annual_leave_balance_transfer_log_{{result("log_dag_run_and_report_run_date").current_date_time }}.csv''',
        )

        generate_download_link = rail.GeneratePresignedDownloadUrlOperator(
            task_id='generate_download_link',
            artifact_name="{{ result('compose_logs_csv')}}",
            output_file_name='{{get_company_key()}}_netherlands_annual_leave_balance_transfer_log_{{result("log_dag_run_and_report_run_date").current_date_time }}.csv',
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('check_for_error_log', key='length') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Netherlands Annual leave balance transfer - " }} \
                ...
                {%- else -%} \
                    ...
                    {%- endif -%} \
                {%- endif -%} \
                {{ " - " + result("log_dag_run_and_report_run_date").current_date_time  }}',
        ...
        log_dag_run_and_report_run_date >> create_timeoff_balance_transfer_logs >> log_get_required_timeoff_type_uris >> get_required_country_service_center_uri \
        ...
        query_valid_records >> trigger_dag_run_transfer_timeoff_balance >> compose_logs_csv \
