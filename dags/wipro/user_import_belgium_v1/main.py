...
from wipro.user_import_belgium_v1.task import process_user_group_data
from wipro.user_import_belgium_v1.utils import custom_methods, request_payload
import rail
null = None
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        query_valid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM belgium_users WHERE NULLIF("employee_id","") IS NOT NULL AND
            ...
            NULLIF("adid","") IS NOT NULL AND NULLIF("hr_adid","") IS NOT NULL""",
            name="belgium_valid_users"
        )

        query_invalid_user_records = rail.QueryCollectionOperator(
            task_id="query_invalid_user_records",
            query="""SELECT * FROM belgium_users WHERE NULLIF("employee_id","") IS NULL OR
            ...
            or "date_of_joining" LIKE 0000-00-00 OR NULLIF("hr_adid","") IS NULL"""
        )
        ...
        process_user_for_belgium = rail.trigger_parallel_dagrun(
            task_id="process_user_for_belgium",
            items='{{result("query_valid_user_records")}}',
            trigger_dag_id=config.valid_user_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item,config=config:custom_methods.get_belgium_user_conf(item,config)
        )
        ...
        process_supervisor_for_belgium = rail.trigger_parallel_dagrun(
            task_id="process_supervisor_for_belgium",
            items='{{result("load_supervisor_data")|to_json}}',
            trigger_dag_id=config.create_supervisor_dag_id,
            parallel_count=config.max_active_run_child,
            execution_timeout=timedelta(days=config.execution_timeout),
            conf=lambda item,config=config:custom_methods.get_belgium_supervisor_conf(item,config)
        )

        end_belgium_user = rail.EmptyOperator(
            task_id="end_belgium_user"
        )

        process_logs_for_belgium = rail.TriggerDagRunOperator(
            task_id="process_logs_for_belgium",
        ...
        if_invalid_users >> rail.Label("No") >>\
            ...
            process_user_start >>\
            process_user_for_belgium >>\
            get_supervisor_pending_logs >> load_supervisor_data >>\
            process_supervisor_for_belgium >>\
            end_belgium_user
        if_valid_user_records >> rail.Label(
            "No") >> end_belgium_user >> process_logs_for_belgium
        return dag
