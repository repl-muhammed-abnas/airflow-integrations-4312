...
def create_aiflow_child(config):
    with rail.create_airflow_dag(
        ...
        if_add_user = rail.IfOperator(
            task_id="if_add_user",
            test=lambda dag_run:dag_run.conf["Add_Update"] == "Add",
        ...
        start_supervisor_table_settings >> put_supervisor_table_view >> if_add_user
        if_user_is_enabled >> rail.Label("No") >>\
        write_log_supervisor_disabled >>\
        if_add_user >> rail.Label("Yes") >> assign_initial_supervisor_to_employee >> catch_and_log_errors
        if_add_user >> rail.Label("No") >> check_supervisor_update >>\
