...
def create_prerequisite_data():
    with rail.TaskGroup(
        ...
        get_belgium_parent_location_details = rail.RepliconServiceOperator(
            task_id="get_belgium_parent_location_details",
            endpoint="/services/LocationListService1.svc/GetData",
            data=request_payload.get_parent_location_payload,
            data_handler=lambda response: list(filter(lambda i: i["cells"][0]["textValue"] == "Belgium", response["rows"]))
        )
        ...
        start_groups >>\
        get_belgium_parent_location_details>>\
            [
