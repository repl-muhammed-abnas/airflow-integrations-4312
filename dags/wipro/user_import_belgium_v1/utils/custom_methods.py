...
import functools
from wipro.user_import_belgium_v1.utils import request_payload
...
def get_location_hierarchy_data(response):
    ...
    parent_uri=rail.result("get_belgium_parent_location_details")[0]["cells"][0]["uri"]
...
def get_belgium_object_extension_fields():
...
def excluded_timeoff_list(country, gender):
    time_off_list = []
    time_off_type_mapper = rail.result("get_time_off_mapper")
    for i in time_off_type_mapper[country][gender]:
        ...
        if timeoff:
            time_off_list.append(timeoff)
    return time_off_list


def get_belgium_time_off_types(item,config):
    ...
    if item["gender"].lower() in ["female", "male"]:
        if acquired_company.lower() == "at&t":
            get_all_timeoff_types.extend(
                config.TIME_OFF_TYPES_MAPPER["at&t"]["all"])
            get_all_timeoff_types.extend(
                config.TIME_OFF_TYPES_MAPPER["at&t"][item["gender"].lower()])
...
def get_approval_path_uri(country,config):
    return {
        ...
        "netherlands_countryuri": rail.find_first_by_attr_and_get_attr(
            rail.result("get_all_countries"), "displayText", "Netherlands", "uri"),
...
def get_belgium_supervisor_conf(item,config):
    return {
        **item,
        "lookuptable": rail.result("create_log_for_user_import_global"),
        "l1_manager_uri": rail.find_first_by_attr_and_get_attr(
...
def get_belgium_user_conf(item,config):
    ...
    return {
        **item,
        **get_belgium_object_extension_fields(),
        **get_belgium_custom_fields(),
       "locationcountryuri": rail.result("get_belgium_parent_location_details")[0]["cells"][0]["uri"],
        ...
        **get_approval_path_uri(item["country"],config),
        "timeoff_type_uris": get_belgium_time_off_types(item,config),
...
def check_if_belgium_user_location_update(dag_run):
...
def get_belgium_user_create_permissions(dag_run):
    permission_sets = []
...
def get_belgium_user_create_policy_data_access(dag_run):
    policy_data_access = []
    if dag_run.conf["hr_manager_flg"] == "Y":
...
def get_belgium_user_create_policy_sets(dag_run):
    policy_sets = []
    policy_sets.append(
        {
...
def get_belgium_user_create_custom_fields(dag_run):
    custom_field_values = []
    ...
    custom_fields_text = ["hr_adid", "gpo_adid", ]
    for i in custom_fields_text:
        if dag_run.conf[i]:
            if i == 'hr_adid':
                if dag_run.conf["hr_manager_flg"] != "Y" and rail.result('get_hr_adid_belongs_to_belgium')['rows']:
...
def get_belgium_user_create_location(dag_run):
    location = []
...
def get_belgium_user_payrule_script(dag_run):
    payrules = []
    payrules.append({
        "payRuleScript": {
...
def get_belgium_user_create_oefs(dag_run):
    ...
    for i in extension_fields:
        if dag_run.conf.get(i,""):
            if i in ['hr_manager_id', 'hr_manager_mailid']:
                if dag_run.conf["hr_manager_flg"] != "Y" and rail.result('get_hr_manager_belongs_to_belgium')['rows']:
...
def get_updated_logs(dag_run):
    ...
    custom_field_logs = get_belgium_user_update_custom_fields(dag_run)[1]
    basic_details = request_payload.get_basic_user_details_update(dag_run)[1]
    location_logs = check_if_belgium_user_location_update(dag_run)[1]
...
def get_belgium_user_update_custom_fields(dag_run):
    custom_field_values = []
