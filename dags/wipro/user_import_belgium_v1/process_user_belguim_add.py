...
from wipro.user_import_belgium_v1.task import put_user_and_table_settings, put_supervisor_table_settings
from wipro.user_import_belgium_v1.utils import request_payload, custom_methods
import rail
null = None
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task="batch_task",
            no_task="get_hr_manager_belongs_to_belgium"
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id="batch_task",
            start_task="get_hr_manager_belongs_to_belgium",
            end_task="catch_and_log_errors"
        )

        get_hr_manager_belongs_to_belgium = rail.RepliconServiceOperator(
            task_id="get_hr_manager_belongs_to_belgium",
            endpoint="/services/UserListService1.svc/GetData",
            data=request_payload.get_hr_manager_with_location_payload
        )

        get_hr_adid_belongs_to_belgium = rail.RepliconServiceOperator(
            task_id="get_hr_adid_belongs_to_belgium",
            endpoint="/services/UserListService1.svc/GetData",
            data=request_payload.get_hr_adid_with_location_payload
        )

        create_belgium_user = rail.RepliconServiceOperator(
            task_id="create_belgium_user",
            endpoint="/services/ImportService1.svc/PutUser3",
            data=request_payload.get_belgium_create_payload,
        ...
        write_supervisor_pending_logs = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "Add_Update":"Add",
                "useruri": rail.result("create_belgium_user")
            }
        )
        ...
        unassign_products_for_user = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "user": {
                    "uri": rail.result("create_belgium_user"),
                    "loginName": null,
        ...
        put_view_settings_for_user = put_user_and_table_settings.get_put_table_view_setting(
            '{{result("create_belgium_user")}}', "belgium", 'user')
        ...
        put_manager_table_view = put_supervisor_table_settings.get_put_table_view_setting_supervisor(
            '{{result("create_belgium_user")}}', 'supervisor_user', '{{dag_run.conf.project_manager_flg}}')
        ...
        remove_all_time_off_types = rail.RepliconServiceOperator(
            ...
            data={
                    "userUri": '{{result("create_belgium_user")}}',
                    "timeOffTypeUris": []
            }
        )
        ...
        assign_all_time_off_types = rail.RepliconServiceOperator(
            ...
            endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_belgium_user")}}'),
        ...
        get_hr_manager_belongs_to_belgium >> get_hr_adid_belongs_to_belgium >>\
        create_belgium_user >> write_supervisor_pending_logs >> unassign_products_for_user >>\
