...
from wipro.user_import_france_v2.task import set_balance_timeoff_for_user
    ...
    with rail.create_airflow_dag(
        ...
        get_user_details >>\
            if_user_not_exists >> rail.Label(
                "Yes") >> start_process_add_user >> process_add_user >> catch_and_log_errors
        if_user_not_exists >> rail.Label("No") >>\
        get_current_group_membership >> is_country_changed >> rail.Label("Yes") >>\
        write_country_mismatch_log >> catch_and_log_errors
        is_country_changed >> rail.Label("No") >>\
        ...
        if_reversal_date_present_or_active_user >> rail.Label("No") >> write_log_user_not_processed >> catch_and_log_errors
