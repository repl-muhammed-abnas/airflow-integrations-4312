...
from wipro.user_import_france_v2.task import put_user_and_table_settings, put_supervisor_table_settings
...
def create_airflow_child(config):
    with rail.create_airflow_dag(
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task="batch_task",
            no_task="get_hr_manager_belongs_to_france"
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id="batch_task",
            start_task="get_hr_manager_belongs_to_france",
            end_task="catch_and_log_errors"
        )

        get_hr_manager_belongs_to_france = rail.RepliconServiceOperator(
            task_id="get_hr_manager_belongs_to_france",
            endpoint="/services/UserListService1.svc/GetData",
            data=request_payload.get_hr_manager_with_location_payload
        )

        create_france_user = rail.RepliconServiceOperator(
            task_id="create_france_user",
            endpoint="/services/ImportService1.svc/PutUser3",
            data=request_payload.get_france_create_payload,
        ...
        write_supervisor_pending_logs = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "Add_Update": "Add",
                "useruri": rail.result("create_france_user")
            }
        )
        ...
        unassign_products_for_user = rail.RepliconServiceOperator(
            ...
            data=lambda: {
                "user": {
                    "uri": rail.result("create_france_user"),
                    "loginName": null,
        ...
        put_view_settings_for_user = put_user_and_table_settings.get_put_table_view_setting(
            '{{result("create_france_user")}}', "france", 'user')
        ...
        put_manager_table_view = put_supervisor_table_settings.get_put_table_view_setting_supervisor(
            '{{result("create_france_user")}}', 'supervisor_user', '{{dag_run.conf.project_manager_flg}}')
        ...
        remove_all_time_off_types = rail.RepliconServiceOperator(
            ...
            data={
                    "userUri": '{{result("create_france_user")}}',
                    "timeOffTypeUris": []
            }
        )
        ...
        assign_all_time_off_types = rail.RepliconServiceOperator(
            ...
            endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
            data=lambda dag_run: {
                    "userUri": rail.render_template('{{result("create_france_user")}}'),
        ...
        get_annual_leave_sr_time_off_type_policy_schedule_for_user = rail.RepliconServiceCallForEachItemOperator(
            task_id='get_annual_leave_sr_time_off_type_policy_schedule_for_user',
            endpoint="/services/TimeOffPolicyService2.svc/GetDefaultTimeOffTypePolicyScheduleForUser",
            items='{{dag_run.conf.annual_leave_sr_timeoff_type_uris|to_json}}',
            data={
                "timeOffAccount": {
                    "userUri": '{{result("create_france_user")}}',
                    "timeOffTypeUri": "{{ item }}"
        ...
        if_user_is_assignee = rail.IfOperator(
            ...
            no_task='assign_default_timeoff_policy_for_local_hire'
        )
        ...
        replace_onsite_recruit_date = rail.PythonOperator(
            task_id="replace_onsite_recruit_date",
            python_callable=custom_methods.get_assignee_annual_policies
        )

        assign_default_annual_timeoff_policy_for_assignee = rail.RepliconServiceCallForEachItemOperator(
            task_id='assign_default_annual_timeoff_policy_for_assignee',
            endpoint="/services/TimeOffPolicyService2.svc/PutUserTimeOffAccountPolicySetSchedule",
            items='{{dag_run.conf.annual_leave_sr_timeoff_type_uris|to_json}}',
            data=lambda dag_run, item: {
                "timeOffAccount": {
                ...
                "policySetScheduleEntries": json.loads(json.dumps(rail.result('replace_onsite_recruit_date')
                                                                  [list(dag_run.conf["annual_leave_sr_timeoff_type_uris"]).index(item)])
                                                       .replace('"script"', '"scriptTarget"')
                                                       .replace('"description": null', '"description": "effective"'))
            }
        )

        assign_default_timeoff_policy_for_local_hire = rail.RepliconServiceCallForEachItemOperator(
            task_id='assign_default_timeoff_policy_for_local_hire',
            endpoint="/services/TimeOffPolicyService2.svc/PutUserTimeOffAccountPolicySetSchedule",
            items='{{dag_run.conf.timeoff_type_uris|to_json}}',
            data=lambda dag_run, item: {
                "timeOffAccount": {
        ...
        replace_acquired_date = rail.PythonOperator(
            task_id="replace_acquired_date",
            python_callable=custom_methods.get_local_hire_annual_policies
        )

        assign_default_annual_timeoff_policy_for_local_hire = rail.RepliconServiceCallForEachItemOperator(
            task_id='assign_default_annual_timeoff_policy_for_local_hire',
            endpoint="/services/TimeOffPolicyService2.svc/PutUserTimeOffAccountPolicySetSchedule",
            items='{{dag_run.conf.annual_leave_sr_timeoff_type_uris|to_json}}',
            data=lambda dag_run, item: {
                "timeOffAccount": {
                ...
                "policySetScheduleEntries": json.loads(json.dumps(rail.result('replace_acquired_date')
                                                                  [list(dag_run.conf["annual_leave_sr_timeoff_type_uris"]).index(item)])
                                                       .replace('"script"', '"scriptTarget"')
                                                       .replace('"description": null', '"description": "effective"'))
            }
        ...
        can_run_batch_task >> rail.Label("No") >>\
            get_hr_manager_belongs_to_france >>\
            create_france_user >> write_supervisor_pending_logs >> unassign_products_for_user >>\
        ...
        if_no_gender >> rail.Label("No") >>\
            ...
            replace_effective_date >> assign_default_timeoff_policy_for_assignee >>\
            replace_onsite_recruit_date >> assign_default_annual_timeoff_policy_for_assignee >>\
            time_off_end
        if_user_is_assignee >> rail.Label("No") >>\
            assign_default_timeoff_policy_for_local_hire >> replace_acquired_date >>\
            assign_default_annual_timeoff_policy_for_local_hire >>\
            time_off_end >>\
