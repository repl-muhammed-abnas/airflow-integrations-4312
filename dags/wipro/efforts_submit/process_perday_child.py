...
from wipro.efforts_submit.utils.custom_methods import get_it_proj_efforts, get_it_training_efforts, get_non_proj_efforts, map_time_data_per_day
import rail
null = None
dag_created = []
...
def create_airflow_child_dag(config):
    for country in config.time_export_for_country:
        ...
        with rail.create_airflow_dag(
            dag_id=f"wipro_efforts_submission_process_project_time_master_child_process_perday_{cnt}_{config.instance}",
            ...
            submit_data_to_wipro = rail.TriggerDagRunOperator(
                ...
                trigger_dag_id=f"wipro_efforts_submission_submit_project_time_child_{cnt}_{config.instance}",
                conf=lambda: {
            ...
            query_it_projects >> if_it_projects >> rail.Label("Yes") >> query_distinct_it_projects >>\
                it_project_list >>\
            ...
            if_it_non_proj >> rail.Label("No") >> combine_perday_data >>\
                ...
                submit_data_to_wipro >> log_to_sumo
            if_data_perday >> rail.Label("No") >> log_to_sumo
