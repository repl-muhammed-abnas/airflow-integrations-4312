...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'kmc_timeoff_transaction_details_report_extract_master_{config.instance}',
        description=f'kmc_timeoff_transaction_details_report_extract_master {config.instance}',
        ...
        send_success_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Time off Transaction Details Report Extract Completed - {{ current_time("%d%m%Y%H%M%S") }}',
            html_content="templates/emails/success_email.html",
        )
        send_nodata_email = rail.EmailOperator(
            task_id='send_nodata_email',
            to=config.tenant_email,
            subject='{{ get_company_key() }} | Time off Transaction Details Report Extract skipped - {{ current_time("%d%m%Y%H%M%S") }}',
