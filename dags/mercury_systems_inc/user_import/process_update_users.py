...
from mercury_systems_inc.user_import.task_groups.process_supervisor import process_supervisor_assignment_task_group
...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        get_new_eligible_timeoffs_and_existing_non_eligible_timeoffs_for_user = rail.PythonOperator(
            task_id='get_new_eligible_timeoffs_and_existing_non_eligible_timeoffs_for_user',
            python_callable=custom_methods.get_new_eligible_timeoffs_for_update_user
        )

        get_update_user_payload = rail.PythonOperator(
            task_id='get_update_user_payload',
            python_callable=lambda dag_run: request_payload.get_update_user_payload(
                dag_run, config)
        )
        ...
        update_user = rail.RepliconServiceOperator(
            ...
            data=lambda: rail.result(
                'get_update_user_payload')['payload']
        )

        if_remaining_modifications_to_apply = rail.IfOperator(
            task_id='if_remaining_modifications_to_apply',
            test=lambda: rail.result(
                'get_update_user_payload')['applyusermodifications3_payload_for_modifications'],
        ...
        apply_remaining_modifications = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "modifications": rail.result('get_update_user_payload')['applyusermodifications3_payload_for_modifications'],
        ...
        if_non_eligible_timeoff_types = rail.IfOperator(
            ...
            test=lambda: rail.result('get_new_eligible_timeoffs_and_existing_non_eligible_timeoffs_for_user')[
        ...
        stop_accruals_for_non_eligible_timeoff_types = rail.TriggerDagRunForEachItemOperator(
            ...
            items=lambda: rail.result('get_new_eligible_timeoffs_and_existing_non_eligible_timeoffs_for_user')[
                'timeoff_for_stopping_accrual'],
        ...
        get_effective_user_groupmembership >> log_existing_timeoff_policies_for_user >>\
            get_new_eligible_timeoffs_and_existing_non_eligible_timeoffs_for_user

        get_new_eligible_timeoffs_and_existing_non_eligible_timeoffs_for_user >> get_update_user_payload

        get_update_user_payload >> update_user >> if_remaining_modifications_to_apply
