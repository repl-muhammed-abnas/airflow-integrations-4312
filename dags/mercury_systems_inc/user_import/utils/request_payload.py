...
def update_end_date_payload(dag_run, config):
    return {
        "user": {
...
def get_create_user_payload(dag_run, config):
    ...
    add_user_payload = {
        ...
        "modifications": {
            ...
            ] if dag_run.conf["Manager_Type"] == "Yes" else [],
            "bankedTimePolicies": [],
            "timeOffTypes": [
                {
                    "modificationOptionUri": "urn:replicon:collection-modification-option:add",
...
def get_update_user_timeoff_types_with_default_policies(dag_run, config):
    final_timeoff_policies_payload = [{
        ...
        "policySchedule": []
    } for timeoff in rail.result('get_new_eligible_timeoffs_and_existing_non_eligible_timeoffs_for_user')['new_eligible_timeoffs']]
        ...
        'payload': {
            ...
            "modifications": {
                **basic_details,
                ...
                ] if rail.result('get_new_eligible_timeoffs_and_existing_non_eligible_timeoffs_for_user')['new_eligible_timeoffs'] else [],
