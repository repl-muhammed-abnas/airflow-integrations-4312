...
from mercury_systems_inc.user_import_v1.utils import response_filter
...
def pre_requisites_task_group(config):
    ...
    with rail.TaskGroup(group_id='get_user_pre_requisites', prefix_group_id=False) as get_user_pre_requisites:
        ...
        get_all_permission_sets = rail.RepliconServiceOperator(
            ...
            data_handler=lambda res: list(map(lambda permission_set: {
                'permission_set_name': permission_set['cells'][0].get('textValue', ''),
                'permission_set_uri': permission_set['cells'][0].get('uri', ''),
                'permission_policy_uri': permission_set['cells'][1].get('uri', '')
            }, res['rows'])) if res['rows'] else []
        )
        ...
        dummy_get_user_prereqs >> [get_updated_location_grps, get_updated_department_grps, get_all_permission_sets, get_all_employee_types_grp,
