...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.process_rehire_user_dagid,
        description='MercurySystemsInc User Import Process Rehire Users',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.max_active_runs_process_disable_rehire_users,
    ) as dag:
        ...
        get_rehire_type_and_tenure = rail.PythonOperator(
            task_id='get_rehire_type_and_tenure',
            python_callable=custom_methods.get_type_of_rehire_and_tenure,
        )

        if_rehire_type_not_found_or_tenure_negative = rail.IfOperator(
            task_id='if_rehire_type_not_found_or_tenure_negative',
            test=lambda: not (rail.result(
                "get_rehire_type_and_tenure")["rehire_type"]) or rail.result(
                "get_rehire_type_and_tenure")["tenure"] < 0,
            yes_task='log_rehire_type_not_found_or_tenure_negative',
            no_task='dummmy_process_supervisor'
        )

        log_rehire_type_not_found_or_tenure_negative = rail.WriteLogOperator(
            task_id='log_rehire_type_not_found_or_tenure_negative',
            log='{{ dag_run.conf.user_log }}',
            message=lambda: "Rehire type not found or negative tenure",
            severity='Exception',
            properties=lambda dag_run: {
                ...
                'action': 'Rehire',
                'status': 'Exception',
                'details': "User not rehired ; " + ("Rehire type not found" if not (rail.result(
                    "get_rehire_type_and_tenure")["rehire_type"]) else "Effective date is prior to start date in feed file")
            }
        )

        dummmy_process_supervisor = rail.EmptyOperator(
            task_id='dummmy_process_supervisor'
        )
        ...
        log_validation_errors = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                'action': 'Validation',
                'status': 'Exception',
                'details': "User not rehired ; " + ' ; '.join(rail.result('validate_replicon_fields')['missing_fields'])
            }
        )
        ...
        apply_remaining_modifications = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "modifications": rail.result('log_rehire_user_payload')['applyusermodifications3_payload_for_modifications'],
        ...
        if_non_eligible_timeoff_types = rail.IfOperator(
            ...
            no_task='dummy_check_if_existing_eligible_timeoffs'
        )
        ...
        dummy_check_if_existing_eligible_timeoffs = rail.EmptyOperator(
            task_id="dummy_check_if_existing_eligible_timeoffs"
        )

        if_existing_eligible_timeoffs = rail.IfOperator(
            task_id='if_existing_eligible_timeoffs',
            test=lambda: rail.result('get_new_and_existing_timeoff_types_for_user')[
                'existing_eligible_timeoffs'],
            yes_task='dummy_process_stop_accruals_for_existing_eligible_timeoffs',
            no_task='log_user_completion'
        )

        dummy_process_stop_accruals_for_existing_eligible_timeoffs = rail.EmptyOperator(
            task_id='dummy_process_stop_accruals_for_existing_eligible_timeoffs'
        )

        process_existing_eligible_timeoffs = rail.TriggerDagRunForEachItemOperator(
            task_id='process_existing_eligible_timeoffs',
            items=lambda: rail.result('get_new_and_existing_timeoff_types_for_user')[
                'existing_eligible_timeoffs'],
            trigger_dag_id=config.process_existing_eligible_timeoff_types_for_rehire_users_dagid,
            conf=lambda item, dag_run: {
                'user_uri': dag_run.conf["user_uri"],
                'timeoff_uri_for_updating': item,
                'existing_policyset_schedule_for_timeoff': rail.find_first_by_attr_and_get_attr(rail.result(
                    'log_existing_timeoff_policies_for_user'), 'timeOffType.uri', item, 'policySetSchedule'),
                "starting_balance_set_to_script_uri": dag_run.conf["starting_balance_set_to_script_uri"],
                'tenure_of_user': rail.result('get_rehire_type_and_tenure')["tenure"],
                'rehire_type': rail.result('get_rehire_type_and_tenure')["rehire_type"],
                'effective_date': dag_run.conf["Effective_Date"],
                'new_hire_date': dag_run.conf["Hire_Date"]
            },
            execution_timeout=timedelta(days=config.execution_timeout_days),
        )

        wait_for_process_existing_eligible_timeoffs = rail.WaitForDagRunsSensor(
            task_id='wait_for_process_existing_eligible_timeoffs',
            dag_runs="{{ result('process_existing_eligible_timeoffs') }}",
            execution_timeout=timedelta(
                days=config.execution_timeout_days)
        )

        gather_errors_from_child_runs = rail.GatherResultsFromDagRunsOperator(
            task_id='gather_errors_from_child_runs',
            dag_runs='{{ result("process_existing_eligible_timeoffs") }}',
            dagrun_task_id='catch_errors',
            execution_timeout=timedelta(
                hours=config.gather_errors_from_child_timeout_hours),
        ...
        if_error_in_child_runs = rail.IfOperator(
            task_id='if_error_in_child_runs',
            test=lambda: 'Error' in json.dumps(
                rail.result('gather_errors_from_child_runs')),
            yes_task='failure_while_updating_existing_eligible_timeoffs',
            no_task='log_user_completion'
        )

        failure_while_updating_existing_eligible_timeoffs = rail.FailOperator(
            task_id='failure_while_updating_existing_eligible_timeoffs',
            message="Failed while updating existing eligible time off types",
        )

        log_user_completion = rail.WriteLogOperator(
            task_id='log_user_completion',
            log='{{ dag_run.conf.user_log }}',
            message="User Rehired Successfully",
            severity="Success",
            properties=lambda dag_run: {
                'employee_id': dag_run.conf['Employee_ID'],
                'first_name': dag_run.conf['First_Name'],
                ...
                'action': 'Rehire',
                "status": "Success",
                'details': "User Rehired Successfully",
            }
        )
        ...
        can_run_batch_task >> rail.Label(
            'No') >> get_user_details >> get_rehire_type_and_tenure

        get_rehire_type_and_tenure >> if_rehire_type_not_found_or_tenure_negative

        if_rehire_type_not_found_or_tenure_negative >> rail.Label(
            'Yes') >> log_rehire_type_not_found_or_tenure_negative >> catch_and_log_errors
        if_rehire_type_not_found_or_tenure_negative >> rail.Label(
            'No') >> dummmy_process_supervisor

        dummmy_process_supervisor >> process_supervisor_entry
        ...
        get_new_and_existing_timeoff_types_for_user >> log_rehire_user_payload

        log_rehire_user_payload >> rehire_user >> if_remaining_modifications_to_apply
        ...
        if_non_eligible_timeoff_types >> rail.Label(
            'No') >> dummy_check_if_existing_eligible_timeoffs
        ...
        if_error_in_child >> rail.Label(
            'No') >> dummy_check_if_existing_eligible_timeoffs
        if_error_in_child >> rail.Label(
            'Yes') >> failure_while_stoppping_accrual_for_timeoffs >> dummy_check_if_existing_eligible_timeoffs

        dummy_check_if_existing_eligible_timeoffs >> if_existing_eligible_timeoffs

        if_existing_eligible_timeoffs >> rail.Label(
            'No') >> log_user_completion
        if_existing_eligible_timeoffs >> rail.Label(
            'Yes') >> dummy_process_stop_accruals_for_existing_eligible_timeoffs

        dummy_process_stop_accruals_for_existing_eligible_timeoffs >> process_existing_eligible_timeoffs >>\
            wait_for_process_existing_eligible_timeoffs >> gather_errors_from_child_runs >> if_error_in_child_runs

        if_error_in_child_runs >> rail.Label('No') >> log_user_completion
        if_error_in_child_runs >> rail.Label(
            'Yes') >> failure_while_updating_existing_eligible_timeoffs >> log_user_completion
