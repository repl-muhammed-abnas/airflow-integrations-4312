...
import rail
from mercury_systems_inc.project_import_v1.utils import request_payload, custom_method
    ...
    with rail.create_airflow_dag(
        ...
        query_any_blankmandatory_check = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM inputdatacollection WHERE
                ...
                NULLIF(task_allow_time_entry,'') IS NULL OR
                (NULLIF(assign_team,'') IS NOT NULL AND
                 assign_team NOT IN ('Assign them to all tasks', 'Do not assign them to all tasks'))"""
        )
        ...
        query_valid_data_from_rawdata = rail.QueryCollectionOperator(
            ...
            query="""SELECT ROW_NUMBER() OVER(ORDER BY ROWID) AS record_id,* FROM inputdatacollection WHERE
                ...
                NULLIF(task_allow_time_entry,'') IS NOT NULL AND
                (NULLIF(assign_team,'') IS NULL OR assign_team IN ('Assign them to all tasks', 'Do not assign them to all tasks'))"""
        )
