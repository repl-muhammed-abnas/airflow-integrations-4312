...
import rail
from mercury_systems_inc.gl_labor_cost_export.tasks.run_approval_date_report import run_approval_date_report_for_labor_cost
from mercury_systems_inc.gl_labor_cost_export.tasks.run_entry_date_report import run_entry_date_labor_cost_report
from mercury_systems_inc.gl_labor_cost_export.utils.python_callable import get_csv_filename, get_date_range, format_csv_row


def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.master_dag_id,
        description=f'Mercury Systems Inc GL Labor Cost Export {config.instance}',
        ...
        run_approval_date_report = run_approval_date_report_for_labor_cost(config)

        run_entry_date_report = run_entry_date_labor_cost_report(config)


        query_data_within_range = rail.QueryCollectionOperator(
            task_id="query_data_within_range",
            query="""SELECT * FROM cost_data WHERE NULLIF(week_ending,'') IS NOT NULL
                    UNION SELECT * FROM cost_approval_data WHERE NULLIF(week_ending,'') IS NOT NULL"""
        )
        ...
        send_empty_export_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | GL Labor Cost Export - No records to export - {{ result("process_start_time") }}',
        ...
        send_valid_export_complete_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | GL Labor Cost Export completed - {{ result("process_start_time") }}',
        ...
        fail_dag = rail.FailOperator(
            task_id="fail_dag",
            message="GL Labour Cost Export failed"
        )

        # Define task dependencies
        process_start_time >>  generate_filename >>\
        get_date_range_values >> run_approval_date_report >> run_entry_date_report >>\
