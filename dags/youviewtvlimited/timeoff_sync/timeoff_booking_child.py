...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        if_timeoff_type_in_nonsync = rail.IfOperator(
            ...
            test=lambda dag_run: dag_run.conf["booking_data"]["policyTypeDisplayName"] in config.TIMEOFF_TYPE_NO_SYNC,
        ...
        log_timeoff_type_not_present = rail.WriteLogOperator(
            ...
            log='{{ dag_run.conf.log_artifact}}',
            message='"Absence" Time Off type  not available in Replicon',
            severity='Skipped',
            properties=lambda dag_run: {
                ...
                "booking_end_date": dag_run.conf["booking_data"].get("endDate") or dag_run.conf["booking_data"].get("date"),
                "status": "Skipped",
                "comments": '"Absence" Time Off type  not available in Replicon',
                "dag_run_id": get_dagrun_ecid(dag_run)
            }
        )
        ...
        log_timeoff_type_not_assigned_to_user = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "booking_end_date": dag_run.conf["booking_data"].get("endDate") or dag_run.conf["booking_data"].get("date"),
                "status": "Skipped",
                "comments": 'Time Off type Absence is not assigned to user in Replicon',
                "dag_run_id": get_dagrun_ecid(dag_run)
            }
        )
