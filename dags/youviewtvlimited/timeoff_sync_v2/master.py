...
from youviewtvlimited.timeoff_sync_v2.tasks.send_logs import get_send_logs
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'Youview TV Timeoff Sync Master {config.instance}',
        ...
        get_timeoff_bookings_from_hibob = rail.SimpleHttpOperator(
            ...
            endpoint='timeoff/requests/changes?since={{ result("logging_details").prev_3_hours_encoded }}',
            headers={
                'accept': 'application/json',
                # 'Authorization': f'{open_bracket}var.value.{config.access_token}{close_bracket}'
            },
            response_filter=lambda response, dag_run: response_filters.filter_timeoff_data(response, dag_run)
        )
        ...
        get_absense_time_off_type = rail.RepliconServiceOperator(
            task_id='get_absense_time_off_type',
            endpoint='/services/TimeOffService1.svc/GetAllTimeOffTypes',
            data_handler=lambda response, dag_run: {
                type_name: rail.find_first_by_attr_and_get_attr(
                    response, 'displayText', type_name, 'uri')
                for type_name in set(config.HIBOB_TO_REPLICON_TIMEOFF_TYPES_MAPPER.values())
            }
        )

        timeoff_types_mapper = {
            hibob_timeoff_type.lower(): replicon_timeoff_type
            for hibob_timeoff_type, replicon_timeoff_type in config.HIBOB_TO_REPLICON_TIMEOFF_TYPES_MAPPER.items()
        }
        ...
        book_timeoff = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda item: request_payload.get_book_timeoff_conf(timeoff_types_mapper, item),
        ...
        if_timeoff_bookings_exists >> rail.Label("Yes") >> \
            get_booking_id_oef_value >> get_absense_time_off_type >> book_timeoff >> wait_for_bookings \
            >> delete_timeoff >> wait_for_delete_bookings >> process_logs >> send_logs_enter
        send_logs_exit >> finish
        if_timeoff_bookings_exists >> rail.Label("No") >> send_no_new_timeoff_bookings_email >> finish
