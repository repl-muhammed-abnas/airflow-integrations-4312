...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        # resolve_plc_from_task_user_pairs defaults to False — opt-in per instance
        is_resolve_plc_from_task_user_pairs = rail.IfOperator(
            task_id='is_resolve_plc_from_task_user_pairs',
            test=lambda: getattr(config, 'resolve_plc_from_task_user_pairs', False) == True,
            yes_task='get_task_user_plc_assignments',
            no_task='get_time_entries_to_sync'
        )
        ...
        def project_role_list_input(response):
            ...
            result = {}
            for row in (rows or []):
                if not row['cells'][2].get('boolValue'):
                    continue
                name = row['cells'][0].get('textValue')
                result[name] = {
                    "name": name,
                    "description": row['cells'][1].get('textValue'),
                    "enabled": row['cells'][2].get('boolValue'),
        ...
        get_all_project_roles = rail.RepliconServiceOperator(
            ...
            data_handler=project_role_list_input
        )
        ...
        get_task_user_plc_assignments = rail.RepliconServiceOperator(
            task_id='get_task_user_plc_assignments',
            endpoint='/services/TaskService1.svc/BulkGetTaskResourceEstimateDetailsForTaskResourceUserAssignmentPairs',
            data=lambda: get_task_resource_pairs_for_plc()
        )
        ...
        # TODO: When resolve_plc_from_task_user_pairs is True, PLC verification still uses the old
        # task-allocation chain. Confirm both sources agree, or switch verification to the new
            ...
            if polarisTimeEntries and polarisTimeEntries["document"] and polarisTimeEntries["document"]["rows"] and \
                ...
                for child in polarisTimeEntries["document"]["rows"][0]["row"]["children"]:
                    if child and child["row"] and child["row"]["data"]:
                        proj_id = child["row"]["data"]["PROJ_ID"] or ''
                        parent_project_id = proj_id.split(".")[0]
        ...
        def get_current_costpoint_timesheet_period(costpoint_periods, timesheet):
            ...
            if costpoint_periods and costpoint_periods.get('document') and costpoint_periods['document'].get('rows'):
                header_row = costpoint_periods['document']['rows'][0]['row']
                matching = None
                for child in (header_row.get('children') or []):
                    ...
                    if child_row.get('rsId') == 'LDMTSPD_TSPDSCH_DTL':
                        ...
                        if end_dt and end_dt >= timesheet_end_date and data.get('OPEN_FL') == 'Y':
                            if matching is None or end_dt < matching['END_DT']:
                                matching = data
                if matching:
                    return matching
        ...
        def get_children(entries, taskDetails, payCodes, roles, accountDetails, projects,
            ...
            plc_lookup = build_plc_lookup() if getattr(config, 'resolve_plc_from_task_user_pairs', False) else {}
                ...
                for entry in entries:
                    if is_project_allocation_type(entry):
                        ...
                        plc = get_billing_labor_category(
                            entry, taskDetails, roles, userUri, taskAllocations, send_blank_plc_account_org, plc_lookup)
                    ...
                    childRows.append({
                        "row": {
                            ...
                            "data": {
                                ...
                                "BILL_LAB_CAT_CD": get_billing_labor_category(projectEntry[0], taskDetails, roles, userUri, taskAllocations, send_blank_plc_account_org, plc_lookup),
                            ...
                            childRows.append(
                                {
                                    "row": {
                                        ...
                                        "data": {
                                            "ACCT_ID": get_account_id(accountDetails, send_blank_plc_account_org),
                                            "BILL_LAB_CAT_CD": get_billing_labor_category(entry, taskDetails, roles, userUri, taskAllocations, send_blank_plc_account_org, plc_lookup),
        ...
        def get_billing_labor_category(entry, tasks, roles, userUri, taskAllocations, send_blank_plc_account_org, plc_lookup=None):
            if getattr(config, 'resolve_plc_from_task_user_pairs', False):
                task_uri = get_task_uri(entry)
                if task_uri:
                    employee_id = rail.result('get_replicon_user_details')[0]['userDetails']['employeeId']
                    # Default to GENRL when no PLC is assigned (e.g. indirect projects without Workforce Assignments)
                    return (plc_lookup or {}).get((task_uri, employee_id), 'GENRL')
                return 'GENRL'  # direct project entry — no PLC assignable, use default
        ...
        def get_task_resource_pairs_for_plc():
            employee_id = rail.result('get_replicon_user_details')[0]['userDetails']['employeeId']
            entries = rail.result('get_replicon_time_entries')
            seen = set()
            pairs = []
            for entry in entries:
                task_uri = get_task_uri(entry)
                if task_uri and task_uri not in seen:
                    seen.add(task_uri)
                    pairs.append({
                        "task": {"uri": task_uri, "name": null, "parent": null, "project": null, "parameterCorrelationId": null},
                        "user": {"uri": null, "loginName": null, "employeeId": employee_id, "parameterCorrelationId": null},
                        "parameterCorrelationId": null
                    })
            return {"taskResourceUserAssignmentPairs": pairs}

        def build_plc_lookup():
            employee_id = rail.result('get_replicon_user_details')[0]['userDetails']['employeeId']
            entries = rail.result('get_replicon_time_entries')
            seen = []
            ordered_task_uris = []
            for entry in entries:
                task_uri = get_task_uri(entry)
                if task_uri and task_uri not in seen:
                    seen.append(task_uri)
                    ordered_task_uris.append(task_uri)
            raw_response = rail.result('get_task_user_plc_assignments') or []
            lookup = {}
            for i, item in enumerate(raw_response):
                if i >= len(ordered_task_uris):
                    break
                task_uri = ordered_task_uris[i]
                if item.get('error') is not None:
                    print(f"[WARNING] PLC lookup error for task={task_uri}, employee={employee_id}: {item['error']}")
                    continue
                estimate_details = item.get('estimateDetails')
                if not estimate_details:
                    print(f"[WARNING] PLC lookup: no estimateDetails for task={task_uri}, employee={employee_id}")
                    continue
                project_role = estimate_details.get('projectRole')
                if not project_role:
                    print(f"[WARNING] PLC lookup: no projectRole for task={task_uri}, employee={employee_id}")
                    continue
                display_text = project_role.get('name')
                if not display_text:
                    print(f"[WARNING] PLC lookup: empty displayText for task={task_uri}, employee={employee_id}")
                    continue
                project_roles_dict = rail.result('get_all_project_roles') or {}
                role_info = project_roles_dict.get(display_text)
                plc_description = role_info.get('description') if role_info else display_text
                lookup[(task_uri, employee_id)] = plc_description
            return lookup
        ...
        def get_duration_hours(hoursObject):
            raw = hoursObject['hours'] + hoursObject['minutes']/60.00 + hoursObject['seconds']/3600.00 + hoursObject['milliseconds']/3600000.00 + hoursObject['microseconds']/3600000000.00
            return round(raw, 2)
        ...
        get_replicon_timesheet >> get_all_project_roles >> get_replicon_user_details >> get_replicon_timesheet_period_details >> is_costpoint_user
        ...
        is_sync_to_open_period >> rail.Label(
            'yes') >> get_open_subperiods >> get_costpoint_timesheet_periods >> get_current_timesheet_period >> get_existing_deltek_timesheet_open_period >> get_existing_deltek_timesheet
        ...
        get_replicon_role_details >> is_resolve_plc_from_task_user_pairs

        is_resolve_plc_from_task_user_pairs >> rail.Label(
            'yes') >> get_task_user_plc_assignments >> get_time_entries_to_sync

        is_resolve_plc_from_task_user_pairs >> rail.Label(
            'no') >> get_time_entries_to_sync

        get_time_entries_to_sync >> get_costpoint_project_plcs >> verify_costpoint_plc_assignments >> is_missing_plc_assignment
