"""
Guidehouse Resource Assignment - Request Payload Utilities
...
def get_project_not_found_log_properties(item):
    ...
    return {
        ...
        "action": "Validation",
        "status": "Exception",
        "details": f"Project {item.get('project_id')} does not exist in Replicon"
    }
...
def get_project_inactive_log_properties(item):
    ...
    return {
        ...
        "action": "Validation",
        "status": "Exception",
        "details": f"Project {item.get('project_id')} is not active in Replicon"
    }
...
def update_resource_allocation_mutation(item, DATE_FORMAT_INPUT):
    ...
    CRITICAL: This prevents creating duplicate allocations by reusing the existing allocation ID.
    Both start and end dates from the feed always overwrite the existing Replicon dates.
    ...
    # if assignment dates in feed always overwrite existing Replicon dates
    start_date_iso = csv_start_date.strftime('%Y-%m-%dT00:00:00.000Z')
    end_date_iso = csv_end_date.strftime('%Y-%m-%dT00:00:00.000Z')
