...
from guidehouse.resource_assignment.utils import custom_method


def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.main_dag_id,
        description='Guidehouse Resource Assignment - Pipe Delimited File Processing',
        ...
        is_txt_pgp = rail.IfOperator(
            task_id='is_txt_pgp',
            test="{{result('new_file_sensor') | lower | ends_with('.txt.pgp') }}",
            yes_task='validate_file_name',
            no_task='archive_bad_format_file'
        )
        ...
        check_file_type = rail.IfOperator(
            ...
            test=lambda: rail.result('validate_file_name')['is_project_file'],
            yes_task='delete_this_dagrun_non_resource_file',  # project file -> leave it, no archive
            no_task='archive_invalid_name_file'               # unknown name -> archive + email
        )
        ...
        delete_this_dagrun_invalid_name = rail.DeleteCurrentDagRunOperator(
            task_id='delete_this_dagrun_invalid_name'
        )

        archive_bad_format_file = rail.SFTPMoveFileOperator(
            task_id='archive_bad_format_file',
            existing_filename='{{ result("new_file_sensor") }}',
            new_filename=config.archive_filepath +
                "/INVALID_FORMAT_{{ dag_run_ecid() | replace(':', '-') }}_{{ result('new_file_sensor') | file_name }}"
        )
        ...
        archive_invalid_name_file = rail.SFTPMoveFileOperator(
            task_id='archive_invalid_name_file',
            existing_filename='{{ result("new_file_sensor") }}',
            new_filename=config.archive_filepath +
                "/INVALID_NAME_{{ dag_run_ecid() | replace(':', '-') }}_{{ result('new_file_sensor') | file_name }}"
        )
        ...
        send_invalid_file_name_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject="{{ get_company_key() }} | Replicon Resource Assignment - File processing skipped (invalid file name) on {{ current_time_in_specified_tz() }}",
        ...
        create_collection_from_csv = rail.CreateCollectionOperator(
            ...
            columns={
                ...
                "ASSIGNED_START_DATE": "assignmentstartdate",
                "ASSIGNED_END_DATE": "assignmentenddate"
            }
        )
        ...
        def convert_report_dates(item):
            if not item:
                return item
            result = dict(item)
            for col in ['User Start Date', 'User End Date']:
                if result.get(col):
                    result[col] = result[col].replace('/', '-')
            return result

        transform_report_dates = rail.DataAdaptorOperator(
            task_id="transform_report_dates",
            source="{{ result('load_user_report_data') }}",
            data=convert_report_dates
        )

        create_user_report_collection = rail.CreateCollectionOperator(
            task_id="create_user_report_collection",
            source="{{ result('transform_report_dates') }}",
        ...
        query_invalid_users = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM validdata WHERE employee_id NOT IN
        ...
        query_valid_user_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT v.*, u.UserUri as user_uri, u.User_Start_Date as userstartdate, u.User_End_Date as userenddate FROM validdata v
                INNER JOIN user_report_collection u ON v.employee_id = u.Employee_ID""",
        ...
        query_distinct_assignments = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT project_id, record_id FROM valid_user_records GROUP BY project_id"""
        )
        ...
        # ========== Task Dependencies ==========
        new_file_sensor >> is_txt_pgp
        is_txt_pgp >> rail.Label("Yes") >> validate_file_name >> is_valid_file_name
        is_txt_pgp >> rail.Label("No") >> archive_bad_format_file >> send_bad_file_format_email

        is_valid_file_name >> rail.Label("Yes") >> download_file
        is_valid_file_name >> rail.Label("No") >> check_file_type
        check_file_type >> rail.Label("Yes") >> delete_this_dagrun_non_resource_file
        check_file_type >> rail.Label("No") >> archive_invalid_name_file >> send_invalid_file_name_email >> delete_this_dagrun_invalid_name
        ...
        generate_user_report >> load_user_report_data >> transform_report_dates >> create_user_report_collection >> query_invalid_users >> has_invalid_users
