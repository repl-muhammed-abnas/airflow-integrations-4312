...
from guidehouse.peoplesoft_project_import.utils import custom_method, request_payload, response_filter
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        has_valid_data = rail.IfOperator(
            ...
            yes_task='query_multi_client_exceptions',
            no_task='format_logs'
        )

        query_multi_client_exceptions = rail.QueryCollectionOperator(
            task_id='query_multi_client_exceptions',
            name='multiclient_exceptions',
            query="""SELECT ROW_NUMBER() OVER(ORDER BY ROWID) AS record_id,*,
                            'Multiple clients not supported in Polaris. Only one client per project allowed.' AS error_message,
                            CASE
                                WHEN bill_to_cust_id LIKE '%,%' AND customer_name LIKE '%,%' THEN 'Both BILL_TO_CUST_ID and CUSTOMER_NAME have multiple values'
                                WHEN bill_to_cust_id LIKE '%,%' THEN 'BILL_TO_CUST_ID has multiple values'
                                WHEN customer_name LIKE '%,%' THEN 'CUSTOMER_NAME has multiple values'
                                ELSE 'Multiple client values detected'
                            END AS details
                     FROM validdata
                     WHERE bill_to_cust_id LIKE '%,%' OR customer_name LIKE '%,%'"""
        )

        has_multi_client_exceptions = rail.IfOperator(
            task_id='has_multi_client_exceptions',
            test="{{ result('query_multi_client_exceptions', 'length') > 0 }}",
            yes_task='write_multi_client_exceptions',
            no_task='query_clean_client_data'
        )

        query_clean_client_data = rail.QueryCollectionOperator(
            task_id='query_clean_client_data',
            name='clean_client_data',
            query="""SELECT ROW_NUMBER() OVER(ORDER BY ROWID) AS record_id,*
                     FROM validdata
                     WHERE (bill_to_cust_id NOT LIKE '%,%' OR bill_to_cust_id IS NULL OR bill_to_cust_id = '')
                       AND (customer_name NOT LIKE '%,%' OR customer_name IS NULL OR customer_name = '')"""
        )

        write_multi_client_exceptions = rail.WriteLogOperator(
            task_id="write_multi_client_exceptions",
            items="{{ result('query_multi_client_exceptions') }}",
            log="{{ result('create_exception_log') }}",
            severity="Exception",
            message="Multiple clients per project not allowed",
        ...
        has_valid_data >> rail.Label("Yes") >> query_multi_client_exceptions >> has_multi_client_exceptions
        has_valid_data >> rail.Label("No") >> format_logs

        has_multi_client_exceptions >> rail.Label("Yes") >> write_multi_client_exceptions >> query_clean_client_data
        has_multi_client_exceptions >> rail.Label("No") >> query_clean_client_data >> query_distinct_clients
        ...
        query_distinct_projects >> get_project_custom_fields >> get_task_custom_fields >> get_all_permission_sets >> process_projects >> format_logs
