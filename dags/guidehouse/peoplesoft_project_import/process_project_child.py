...
from guidehouse.peoplesoft_project_import.utils import request_payload, response_filter, custom_method
    ...
    for idx in range(0, config.PROJECT_BATCH_COUNT):
        ...
        with rail.create_airflow_dag(
            ...
            should_create_project_link = rail.IfOperator(
                ...
                no_task='log_iwo_link_already_exists'
            )
            ...
            log_iwo_link_already_exists = rail.WriteLogOperator(
                task_id="log_iwo_link_already_exists",
                log="{{ dag_run.conf.log }}",
                severity="Info",
                message=lambda: f"IWO link already exists to parent project '{rail.result('load_project_data_from_query').get('cp_project', '')}'",
                properties=lambda: custom_method.get_guidehouse_task_log_properties(
                    rail.result("load_project_data_from_query"),
                    {'activity': '', 'activity_descr': ''},
                    "Skip (IWO Parent Project Linking)",
                    "Exception",
                    f"IWO link already exists to parent project '{rail.result('load_project_data_from_query').get('cp_project', '')}'"
                )
            )
            ...
            get_project_data_from_query >> load_project_data_from_query >> validate_optional_fields >> validate_dates >> are_project_dates_valid
            ...
            should_create_project_link >> rail.Label('No') >> log_iwo_link_already_exists >> get_parent_project_leader
