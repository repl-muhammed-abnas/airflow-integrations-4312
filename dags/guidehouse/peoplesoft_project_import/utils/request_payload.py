...
def get_log_message():
    ...
    if get_task_state("assign_project_manager_to_project") and get_task_state("assign_project_manager_to_project").lower() != "success":
        if project_manager:
            ...
            issues.append("project manager assignment skipped (empty employee ID in CSV)")
    ...
    if has_csv_co_managers or has_parent_co_managers:
        ...
        if has_csv_co_managers:
            co_manager_description_parts.append(f"CSV co-managers '{co_managers}'")
        ...
        if has_csv_co_managers:
            ...
            if disabled_users and enabled_count > 0:
                ...
                issues.append(f"co-manager assignment partially failed for CSV co-managers (some users disabled in Replicon: {', '.join(disabled_names)})")
                csv_specific_issue_added = True

            elif total_requested == 0:
                # Users not found in Replicon - highest priority
                issues.append(f"co-manager assignment skipped for CSV co-managers (employee IDs '{co_managers}' not found in Replicon)")
                csv_specific_issue_added = True
            elif disabled_users and enabled_count == 0:
                # All users disabled in Replicon
                disabled_names = [f"{u['display_name']} ({u['employee_id']})" for u in disabled_users]
                issues.append(f"co-manager assignment skipped for CSV co-managers (employee IDs '{co_managers}' - all users disabled in Replicon: {', '.join(disabled_names)})")
                csv_specific_issue_added = True
    ...
    if cp_project:
        ...
        if validation and not validation.get('should_continue'):
            ...
            elif get_task_state("should_create_project_link") and get_task_state("should_create_project_link").lower() == "skipped":
                issues.append(f"IWO linking skipped (link already exists to parent project '{cp_project}')")
...
def create_project_rest_api(dag_run):
    ...
    # Only assign client if single client (no comma-separated values)
    bill_to_cust_id = project_data.get('bill_to_cust_id', '')
    customer_name = project_data.get('customer_name', '')

    has_single_client = (
        (bill_to_cust_id or customer_name) and
        ',' not in bill_to_cust_id and
...
def build_create_cost_center_payload(dag_run):
    ...
    # Only add description if exactly 3 characters
    if cost_center_code and len(cost_center_code.strip()) == 3:
        modifications["descriptionToApply"] = {"value": cost_center_code}
    # If not 3 characters, simply don't include descriptionToApply
