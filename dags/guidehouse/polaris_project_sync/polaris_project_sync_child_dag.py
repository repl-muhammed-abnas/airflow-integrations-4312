...
def _normalize_client_name(name):
    """Strip whitespace and coerce None to ''. Must match the normalization
    used by master DAG `process_clients`, otherwise the master may create a
    client under one spelling and the child may ask Replicon to resolve another."""
    if name is None:
        return ''
...
def _flatten_task_list_response(data, project_flag_map=None,
                                org_wf_required_map=None, org_id_map=None,
                                eligible_map=None):
    ...
    eligible_map carries a pre-computed _is_eligible() bool per PROJ_ID so
    callers can gate group/service-center resource assignment without
    re-fetching Costpoint data. Defaults to False for unknown task codes
    (conservative — unknown tasks don't get group resource assigned).
    """
    project_flag_map = project_flag_map or {}
    org_wf_required_map = org_wf_required_map or {}
    ...
    def _walk(nodes, parent_hierarchy):
        for node in (nodes or []):
            ...
            result.append({
                ...
                'taskName': task.get('name'),
                'org_wf_required': org_wf_required_map.get(task_code),
                'org_id': org_id_map.get(task_code),
                'is_eligible': eligible_map.get(task_code, False),
            })
...
def _extract_udef(row, udef_label):
    """Extract the UDEF_TXT value from a CostPoint row's CPMUDINF_UDEFLBL_CHLD children
    by matching UDEF_LBL. Returns None when the label is absent."""
    for child in (row.get('children') or []):
        child_data = (child.get('row') or {}).get('data') or {}
        if child_data.get('UDEF_LBL') == udef_label:
            return child_data.get('UDEF_TXT')
    return None
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        def _is_eligible(project_info):
            ...
            Eligibility now uses an *exclusion* list of TC_PROJ_FL codes sourced
            from the instance config (``config.excluded_project_type_flags``) so
            tenants can adjust the disallowed codes without changing this DAG.
            """
            if not project_info:
                return False
            return (
        ...
        def _resolve_service_center_uri(service_center_uri, project_row):
            """When ORG WF REQUIRED='Y', use the matching division URI for the
            project's ORG_ID (Replicon divisions are the cost-center groups for
            this integration). Falls back to the CostPoint service center when
            no matching division is found."""
            if (project_row or {}).get('org_wf_required') == 'Y':
                org_id = (project_row or {}).get('org_id')
                div_uri = rail.find_first_by_attr_and_get_attr(
                    rail.get_dag_run_conf().get('divisions', []), 'code', org_id, 'uri'
                ) if org_id else None
                if div_uri:
                    return div_uri
            return service_center_uri

        def _costpoint_project_maps():
            """Build project_flag_map, org_wf_required_map, org_id_map, and
            eligible_map in a single pass over get_costpoint_projects_data().
            eligible_map carries _is_eligible() per PROJ_ID so sync-time
            functions can gate group/service-center assignment without
            re-fetching Costpoint data. Memoised so the data is fetched only
            once per DAG run regardless of how many callers use it."""
            if 'value' not in _costpoint_maps_memo:
                flag_map = {}
                wf_required_map = {}
                org_id_map = {}
                eligible_map = {}
                for row in (get_costpoint_projects_data() or []):
                    proj_id = (row.get('row') or {}).get('data', {}).get('PROJ_ID')
                    if not proj_id:
                        continue
                    row_data = (row.get('row') or {}).get('data', {})
                    flag_map[proj_id] = row_data.get('PROJ_WORK_FRC_FL')
                    wf_required_map[proj_id] = _extract_udef(row.get('row') or {}, 'ORG WF REQUIRED')
                    org_id_map[proj_id] = row_data.get('ORG_ID')
                    eligible_map[proj_id] = _is_eligible(row_data)
                _costpoint_maps_memo['value'] = (flag_map, wf_required_map, org_id_map, eligible_map)
            return _costpoint_maps_memo['value']
                ...
                for child in (row.get('row') or {}).get('children', []):
                    ...
                    if child_rs_id == 'PJM_PROJEMPL_LABCAT_PLCWKFRCE':
                        for entry in (child.get('row') or {}).get('children', []):
                            ...
                            role = (data.get(
                                'PJM_PROJEMPLLABCAT_PLCWK_BILL_LAB_CAT_CD') or '').strip() or None
        ...
        def do_get_pm_comanager_candidates():
            pm_map = getattr(config, 'cp_role_to_pm_map', None)
            if not pm_map:
                return {'candidates': [], 'missing_empl_id': []}
        ...
        def do_pm_comanager_users_handler(data):
            # BulkGetUsers3 + omit-data-if-insufficient-access-permission does NOT
            # preserve request order when some users are omitted — slots shift.
            # Align by userDetails.employeeId instead of positional index.
        ...
        assign_pm_comanager_permission = rail.RepliconServiceCallForEachItemOperator(
            ...
            items=lambda: [
                uri for uri in (
                    [(rail.result('resolve_pm_comanagers') or {}).get('pm_user_uri')]
                    + ((rail.result('resolve_pm_comanagers') or {}).get('co_manager_user_uris') or [])
                ) if uri
            ],
        ...
        def get_assigned_resources(service_center_uri, project_workforce_flag, proj_id, plc=None, project_row=None):
            result = []
            if project_workforce_flag == 'N':
                if (project_row or {}).get('org_wf_required') == 'Y':
                    org_id = (project_row or {}).get('org_id')
                    div_uri = rail.find_first_by_attr_and_get_attr(
                        rail.get_dag_run_conf().get('divisions', []), 'code', org_id, 'uri'
                    ) if org_id else None
                    group_entry = {"division": {"uri": div_uri}} if div_uri else {"serviceCenter": {"uri": service_center_uri}}
                else:
                    group_entry = {"serviceCenter": {"uri": service_center_uri}}
                result.extend([group_entry])
        ...
        def _get_task_type_field_value(proj_type_dc):
            if not getattr(config, 'task_type_custom_field', None) or not proj_type_dc:
                return []
            task_udfs = rail.get_dag_run_conf().get('task_udfs') or []
            uri = rail.find_first_by_attr_and_get_attr(task_udfs, 'textValue', config.task_type_custom_field, 'uri')
            if not uri:
                return []
            valid_options = rail.get_dag_run_conf().get('task_type_dropdown_options') or []
            if valid_options and proj_type_dc not in valid_options:
                return []
            return [{
                "customField": {"uri": uri},
                "text": null,
                "dropDownOption": {"uri": null, "name": proj_type_dc}
            }]

        def get_tasks_param(data, parent_id, service_center_uri, level_no, root_task = None):
            tasks = list(map(lambda x: get_task_details(data, x, service_center_uri),
                        filter(
            ...
            if config.multi_plc_subtask_mode:
                ...
                if root_task:
                    ...
                    get_plc_tasks_create(root_task, plc_assignments, service_center_uri, data)
                for task in tasks:
                    plc_assignments =  list(filter( lambda x: x['projectid'] == task['task']['code'], plc_mapping))
                    get_plc_tasks_create(task, plc_assignments, service_center_uri, data)
            return tasks
        
        def get_plc_tasks_create(task, plc_assignments, service_center_uri, data=None):
            if plc_assignments and len(plc_assignments):
                ...
                for assignment in plc_assignments:
                    if assignment['role'] is not None and assignment['role'] not in added_role:
                        plc_task = get_plc_task(task, assignment, service_center_uri, data)
        ...
        def get_plc_task(task, assignment, service_center_uri, data=None):
            active_status_uri = _active_status_uri()
            plc_task_name =  get_plc_task_name(assignment['role'], task['task']['name'])
            proj_type_dc = rail.find_first_by_attr_and_get_attr(data or [], 'PROJ_ID', task['task']['code'], 'PROJ_TYPE_DC', None)
            resources = get_assigned_resources(service_center_uri, 'Y', task['task']['code'], assignment['role'])
            task_resources = []
            user_uris = []
            ...
            replTask = {
                    "task": {
                        ...
                        "customFieldValues": _get_task_type_field_value(proj_type_dc) + ([
                            {
        ...
        def get_task_details(data, x, service_center_uri):
            active_status_uri = _active_status_uri()
            return {
                        "task": {
                            "target": {
                            ...
                            "customFieldValues": _get_task_type_field_value(x.get('PROJ_TYPE_DC')) + ([
                                {
                                    "customField": {"uri": active_status_uri},
                                    "text": null,
                                    "date": null,
                            ...
                            "estimatedCost": null,
                            "costTypeUri": null,
                            "assignedResources": get_assigned_resources(service_center_uri, x.get('PROJ_WORK_FRC_FL'), x.get('PROJ_ID'), project_row=x),
                            "timeAndMaterials": null,
                            "keyValues": [],                                "historicalKeyValues": []
                        },
                        "childTasks": get_tasks_param(data, x['PROJ_ID'], service_center_uri, x['LVL_NO'])
                    }
        ...
        def get_task_params():
            ...
            if config.multi_plc_subtask_mode:
                #UPDATE SCENARIO
                plc_mapping = rail.result('get_billing_rates_costpoint')
                # Pre-build lookup dicts to avoid O(n²) linear scans inside the loop
                plc_by_project = {}
                for p in plc_mapping:
                    plc_by_project.setdefault(p['projectid'], []).append(p)
                data_by_proj_id = {}
                for p in data:
                    if p['PROJ_ID'] not in data_by_proj_id:
                        data_by_proj_id[p['PROJ_ID']] = p
                plc_tasks = []
                for task in task_hierarchy:
                    ...
                    plc_assignments = plc_by_project.get(parent_task_code, [])
                    ...
                    if plc_assignments:
                        for assignment in plc_assignments:
                            if assignment['role'] is not None and assignment['role'] not in added_role:
                                ...
                                parent_proj_type_dc = (data_by_proj_id.get(parent_task_code) or {}).get('PROJ_TYPE_DC', None)
                                ...
                                plc_task = get_plc_task_update(plc_task_uri, parent_task_uri, parent_task_name, parent_task_code, root_project_id, plc_task_name, \
                                    ...
                                    task['taskModificationToApply']['isTimeEntryAllowed'], code_parent_mapping, parent_proj_type_dc, costpoint_active_status_label,
        ...
        def get_plc_task_update(target_task_uri, parent_task_uri, parent_task_name, parent_task_code, root_project_id, plc_task_name, plc_task_code, task_start_date, task_end_date, isTimeEntryAllowed, code_parent_mapping, proj_type_dc=None, costpoint_active_status_label='Active', parent_proj_id=None):
            active_status_uri = _active_status_uri()
            ...
            return {
                    ...
                    "taskModificationToApply": {
                        ...
                        "customFieldsToApply": _get_task_type_field_value(proj_type_dc) + ([
                            {
                                "customField": {"uri": active_status_uri},
                                "text": null,
                                "date": null,
        ...
        def get_task_uri(data, task_code, parent_hierarchy, plc_code = ''):
            ...
            by_hierarchy = _task_info_memo.get('by_hierarchy', {})

            if plc_code:
                #multiple plc tasks will have same hierarchy and code .. so check for name
                candidates = by_hierarchy.get(hierarchy, [])
                filtered_tasks = [p for p in candidates if p['name'].startswith(plc_code + ' - ')]
                if filtered_tasks and len(filtered_tasks) == 1:
                    ...
                    candidates = by_hierarchy.get(hierarchy, [])
                    return candidates[0].get('uri') if candidates else None
            else :
                #nothing matching with name and code
                candidates = by_hierarchy.get(hierarchy, [])
                task_uri = candidates[0].get('uri') if candidates else None
                if task_uri:
                    return task_uri
            return None
        ...
        def get_apply_tasks_param(task_hierarchy, root_project_id, level_no, parent_req, data, code_parent_mapping, parent_task_code = None, num_records = 1, parent_hierarchy ='', parent_task_name = ''):
            active_status_uri = _active_status_uri()
            ...
            if 'task_info' not in _task_info_memo:
                _task_info = rail.result('get_task_info_from_replicon')
                _by_hierarchy = {}
                for _t in _task_info:
                    _by_hierarchy.setdefault(_t['taskCodeHierarchy'], []).append(_t)
                _task_info_memo['task_info'] = _task_info
                _task_info_memo['by_hierarchy'] = _by_hierarchy
            if 'proj_name_count' not in _task_info_memo:
                _proj_name_count = {}
                for _p in data:
                    _proj_name_count[_p['PROJ_NAME']] = _proj_name_count.get(_p['PROJ_NAME'], 0) + 1
                _task_info_memo['proj_name_count'] = _proj_name_count
            task_data = _task_info_memo['task_info']
            ...
            for prime_level_info in prime_level:
                ...
                task_name = prime_level_info['PROJ_NAME'] if _task_info_memo['proj_name_count'].get(prime_level_info['PROJ_NAME'], 0) == 1 else f"{prime_level_info['PROJ_NAME']}_{prime_level_info['PROJ_ID']}"
                ...
                task_hierarchy.append({
                    ...
                    "taskModificationToApply": {
                        ...
                        "isClosed": _closed_flag(prime_level_info['PROJ_ID']),
                        "timeEntryStartDateToApply": {
                        ...
                        "isTimeEntryAllowed": "true" if prime_level_info['ALLOW_CHARGES_FL'] == 'Y' and prime_level_info['ACTIVE_FL'] == 'Y' and prime_level_info.get('TC_PROJ_FL') not in ('E', 'N') else "false",
                        "customFieldsToApply": _get_task_type_field_value(prime_level_info.get('PROJ_TYPE_DC')) + ([
                            {
                                "customField": {"uri": active_status_uri},
                                "text": null,
                                "date": null,
                                "dropDownOption": {
                                    "uri": null,
                                    "name": "Active" if _is_eligible(prime_level_info) else "Inactive"
                                },
                                "number": null
                            }
                        ] if active_status_uri else []),
                ...
                get_apply_tasks_param(
                    task_hierarchy, root_project_id, prime_level_info['LVL_NO'], next_parent_req, data, code_parent_mapping, task_code, num_records, parent_hierarchy_code, task_name)

        def _resolved_pm_emp_id_or_none():
            """Returns the gated PM employeeId for Guidehouse-style instances, else None.
            When cp_role_to_pm_map is set, the PM is taken from CP role assignments
            (LEM) and only kept if the user already has the Project Manager permission."""
        ...
        def get_add_project_and_task_param():
            service_center_uri = rail.get_dag_run_conf().get('service_center_uri')
            root_project_id, data, root_project_info = get_project_data()
            #root_task = get_root_task(root_project_info, service_center_uri)
            # child_tasks = get_tasks_param(
            #                     data, root_project_id, service_center_uri,
                ...
                "project": {
                    ...
                    "projectInfo": {
                        ...
                        "clients": [
                            {
                                "client": {
                                    "uri": null,
                                    "name": _normalize_client_name(root_project_info.get('CUST_NAME')),
                                    "code": null,
                                    "parameterCorrelationId": null
                                },
                                "costAllocationPercentage": 100
                            }
                        ] if _normalize_client_name(root_project_info.get('CUST_NAME')) else [],
                        "program": null,
                        "projectLeader": {
                            "uri": null,
                            "loginName": null,
                        ...
                        "customFieldValues": [
                            ...
                            *([{
                                "customField": {
                                    "uri":   rail.find_first_by_attr_and_get_attr(rail.get_dag_run_conf()['project_udfs'], 'textValue', getattr(config, 'proj_project_type', None), 'uri'),
                                },
                                "text": null,
                                "dropDownOption": {
                                    "uri": null,
                                    "name": root_project_info.get('PROJ_TYPE_DC')
                                }
                            }] if getattr(config, 'proj_project_type', None) else []),
                            *([{
                                "customField": {
                                    "uri":   rail.find_first_by_attr_and_get_attr(rail.get_dag_run_conf()['project_udfs'], 'textValue', getattr(config, 'proj_source_system', None), 'uri'),
                                },
                                "text": null,
                                "dropDownOption": {
                                    "uri": null,
                            ...
                            }] if getattr(config, 'proj_source_system', None) else []),
                            *([{
                                "customField": {
                    ...
                    "tasks": [],
                    "team": {
                        "teamMembers": get_project_resource_param_project(service_center_uri, root_project_info)
                    },
                    "expenses": null,
                    "timeAndMaterials": null,
                    "fixedBid": null
        ...
        def get_root_task(root_project_info, service_center_uri):
            active_status_uri = _active_status_uri()
            return {
                "task": {
                    "target": {
                    ...
                    "estimatedCost": null,
                    "costTypeUri": null,
                    "assignedResources": get_assigned_resources(service_center_uri, root_project_info['PROJ_WORK_FRC_FL'], root_project_info['PROJ_ID'], project_row=root_project_info),
                    "timeAndMaterials": null,
                    "keyValues": [],
                    "historicalKeyValues": []
                }
        ...
        def get_project_data():
            if 'value' not in _project_data_memo:
                root_project_id = rail.get_dag_run_conf()['item']['root_project_id']
                data = [
                    {**x['row']['data'],
                     'org_wf_required': _extract_udef(x['row'], 'ORG WF REQUIRED'),
                     'org_id': x['row']['data'].get('ORG_ID')}
                    for x in get_costpoint_projects_data()
                ]
                root_project_info = next(
                    (x for x in data if x['PROJ_ID'] == root_project_id), None)
                _project_data_memo['value'] = (root_project_id, data, root_project_info)
            return _project_data_memo['value']

        assign_project_leader_permission = rail.RepliconServiceCallForEachItemOperator(
            task_id='assign_project_leader_permission',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            items=lambda: [] if getattr(config, 'cp_role_to_pm_map', None) else (
                [1] if rail.result('get_project_leader_info_from_replicon') else []),
            endpoint="/services/PermissionSetService1.svc/AssignPermissionSetToUser",
            data=lambda: {
        ...
        check_should_create = rail.IfOperator(
            ...
            test=lambda: any(_is_eligible(row) for row in (get_project_data()[1] or [])),
        ...
        def update_project_create_or_modifiy():
            ...
            # Decide project status symmetrically with the task-level close rule:
            # root closable (self + every descendant ACTIVE_FL='N') -> Archived,
            # otherwise -> In Progress. Sent by canonical status name; Polaris
            # resolves it server-side, so no separate status-URI fetch is needed.
            root_closable = bool(root_project_info) and (root_project_info['PROJ_ID'] in _closable_ids())
            project_status_name = 'Archived' if root_closable else 'In Progress'
            ...
            return {
                ...
                "modifications": {
                    ...
                    "clientAssignmentsSchedulesToApply": {
                        ...
                        "effectiveDate": null
                    } if _normalize_client_name(root_project_info.get('CUST_NAME')) else null,
                    ...
                    "serviceCenterToApply": {
                        "serviceCenter": {
                            "uri": rail.get_dag_run_conf().get('service_center_uri')
                        }
                    } if rail.get_dag_run_conf().get('service_center_uri') else null,
        ...
        def get_task_hierarchy_chunks_callable():
            task_hierarchy = get_task_params()['taskHierarchy']
            chunk_size = 1000
            return [{'items': task_hierarchy[i:i + chunk_size]} for i in range(0, len(task_hierarchy), chunk_size)]

        get_task_hierarchy_chunks = rail.PythonOperator(
            execution_timeout=timedelta(days=config.execution_timeout_days),
            task_id='get_task_hierarchy_chunks',
            python_callable=get_task_hierarchy_chunks_callable
        )

        foreach_update_task = rail.ForEachOperator(
            task_id='foreach_update_task',
            items="{{ result('get_task_hierarchy_chunks') | to_json }}",
            start_task='update_task_chunk',
            end_task='foreach_update_task_end'
        )

        update_task_chunk = rail.RepliconServiceCallForEachItemOperator(
            task_id='update_task_chunk',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            items=lambda: rail.result('foreach_update_task')['items'],
        ...
        foreach_update_task_end = rail.EmptyOperator(
            task_id='foreach_update_task_end'
        )
        ...
        def update_managed_project():
            ...
            return {
                ...
                "modifications": {
                    ...
                    } if service_center_uri else null,
                },
                "projectModificationOptionUri": "urn:replicon:project-modification-option:save",
                "unitOfWorkId": str(uuid.uuid4())
            }
        ...
        def get_project_resource_param_project(service_center_uri, root_project_info):
            if root_project_info['PROJ_WORK_FRC_FL'] == 'N':
                if not _is_eligible(root_project_info):
                    return []
                return [
                    {
                        "resource": {
                            "uri": _resolve_service_center_uri(service_center_uri, root_project_info)
                        },
                        "timeAndMaterials": {
        ...
        def get_project_resources(root_project_info, service_center_uri):
            if root_project_info and root_project_info.get('PROJ_WORK_FRC_FL') == 'N':
                if not _is_eligible(root_project_info):
                    user_uris = []
                else:
                    user_uris = [_resolve_service_center_uri(service_center_uri, root_project_info)]
        ...
        assign_project_team_members = rail.RepliconServiceOperator(
            ...
            data=lambda: get_project_resources(
                get_project_data()[2],
                rail.get_dag_run_conf().get('service_center_uri')
            )
        )
        ...
        def get_resource_uris_for_task(task_uri, project_work_force_flag, service_center_uri, task_item=None):
            if project_work_force_flag == 'N':
                if not (task_item or {}).get('is_eligible'):
                    return []
                return [_resolve_service_center_uri(service_center_uri, task_item)]
            return [
                u['userUri']
        ...
        def get_task_resources(item):
            ...
            return {
                ...
                "resourceUris": get_resource_uris_for_task(item['taskUri'], item.get('project_work_force_flag'), service_center_uri, task_item=item),
                "isAssigned": "1"
            }
        ...
        def get_tasks_with_resources():
            ...
            return [
                ...
                if get_resource_uris_for_task(t['taskUri'], t.get('project_work_force_flag'), service_center_uri, task_item=t)
            ]
        ...
        trigger_assign_task_resources = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_assign_task_resources',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            items=get_tasks_with_resources,
            trigger_dag_id=config.replicon_endpoint_caller_dag_id,
            conf=lambda item: {
                "endpoint": "/services/TaskService1.svc/BulkUpdateResourceAssignments",
                "data": get_task_resources(item)
            },
            reset_count=10000,
            thread_pool_size=getattr(config, 'parallel_count', 15)
        )

        wait_for_assign_task_resources = rail.WaitForDagRunsSensor(
            task_id='wait_for_assign_task_resources',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs="{{ result('trigger_assign_task_resources') }}",
            poke_interval=30,
        )

        # For small workloads run the resource assignments inline; for large ones
        # fan them out to child dag runs (trigger_assign_task_resources), mirroring
        # pick_bulk_update_estimates_method below.
        pick_assign_task_resources_method = rail.IfOperator(
            task_id='pick_assign_task_resources_method',
            test=lambda: len(get_tasks_with_resources())
            < int(Variable.get(config.assign_task_resources_inline_threshold_var_name, default_var='1000')),
            yes_task='assign_task_resources',
            no_task='trigger_assign_task_resources',
        )

        def get_all_project_resources(service_center_uri, root_project_info=None):
            return {
                "projectUri": (
                    (rail.result('get_project_basic_details') or {}).get('uri')
                    or (rail.result('add_project_and_task') or {}).get('uri')
                ),
                "resourceUri": [_resolve_service_center_uri(service_center_uri, root_project_info)] if _is_eligible(root_project_info) else [],
                "projectTeamMemberAssignmentOptionUri": "urn:replicon:project-team-member-assignment-option:assign"
            }
        ...
        def get_all_task_resources(item):
            ...
            return {
                "taskUri": item['taskUri'],
                "resourceUris": [_resolve_service_center_uri(service_center_uri, item)],
                "isAssigned": "1"
            }

        def get_tasks_needing_all_resources():
            return [
                t for t in (rail.result('get_project_task_list') or [])
        ...
        assign_task_all_resources = rail.RepliconServiceCallForEachItemOperator(
            ...
            execution_timeout=timedelta(days=config.execution_timeout_days),
            items=get_tasks_needing_all_resources,
            endpoint='/services/TaskService1.svc/BulkUpdateResourceAssignments',
            data=get_all_task_resources
        )

        trigger_assign_task_all_resources = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_assign_task_all_resources',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            items=get_tasks_needing_all_resources,
            trigger_dag_id=config.replicon_endpoint_caller_dag_id,
            conf=lambda item: {
                "endpoint": "/services/TaskService1.svc/BulkUpdateResourceAssignments",
                "data": get_all_task_resources(item)
            },
            reset_count=10000,
            thread_pool_size=getattr(config, 'parallel_count', 15)
        )

        wait_for_assign_task_all_resources = rail.WaitForDagRunsSensor(
            task_id='wait_for_assign_task_all_resources',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs="{{ result('trigger_assign_task_all_resources') }}",
            poke_interval=30,
        )

        pick_assign_task_all_resources_method = rail.IfOperator(
            task_id='pick_assign_task_all_resources_method',
            test=lambda: len(get_tasks_needing_all_resources())
            < int(Variable.get(config.assign_task_resources_inline_threshold_var_name, default_var='1000')),
            yes_task='assign_task_all_resources',
            no_task='trigger_assign_task_all_resources',
        )
        ...
        def get_bulk_update_estimate_conf(item):
            return {
        ...
        trigger_bulk_update_estimates = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_bulk_update_estimates',
            execution_timeout=timedelta(days=14),
            items=get_tasks_with_estimates,
            trigger_dag_id=config.replicon_endpoint_caller_dag_id,
            conf=get_bulk_update_estimate_conf,
            reset_count=10000,
            thread_pool_size=getattr(config, 'parallel_count', 15)
        )

        wait_for_bulk_update_estimates = rail.WaitForDagRunsSensor(
            task_id='wait_for_bulk_update_estimates',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs="{{ result('trigger_bulk_update_estimates') }}",
            poke_interval=30,
        )
        ...
        # For small workloads run the estimate updates inline; for large ones
        # fan them out to child dag runs (trigger_bulk_update_estimates) so a
        # single task doesn't have to process everything sequentially.
        pick_bulk_update_estimates_method = rail.IfOperator(
            task_id='pick_bulk_update_estimates_method',
            test=lambda: len(rail.result('get_missing_task_resource_estimates') or [])
            < int(Variable.get(config.bulk_update_inline_threshold_var_name, default_var='1000')),
            yes_task='bulk_update_task_resource_estimates',
            no_task='trigger_bulk_update_estimates',
        )
        ...
        catch_and_log_error = rail.WriteLogOperator(
            ...
            message="na",
            severity="Error",
            # A single API failure represents one root-project sync failure, so
            # emit one Error row per failed root, using the first leaf row's
            # PROJ_ID and PROJ_NAME so proj_id and proj_name describe the same
            # entity (the leaf that represents this root's batch).
            items=lambda: ((rail.get_dag_run_conf().get('item') or {}).get('data') or [])[:1],
            properties={
                "proj_id": "{{ item.row.data.PROJ_ID }}",
                "proj_name":  "{{ item.row.data.PROJ_NAME }}",
                "action": "{{ 'Add'  if result('get_project_basic_details') | is_falsy else 'Update' }}",
        ...
        is_project_role_assigment_enabled >> rail.Label('Yes') >> get_project_roles_costpoint >> \
            ...
            log_pm_comanager_skips >> assign_pm_comanager_permission >> get_task_list_info
        ...
        if_project_present >> rail.Label(
            'Yes') >> update_project >> get_task_hierarchy_chunks >> foreach_update_task >> update_task_chunk >> foreach_update_task_end
        if_project_present >> rail.Label('No') >> check_should_create
        check_should_create >> rail.Label('Yes') >> add_project_and_task >> \
            update_manage_project >> update_division >> get_task_hierarchy_chunks >> foreach_update_task >> update_task_chunk >> foreach_update_task_end
        foreach_update_task >> foreach_update_task_end >> project_setup_complete
        check_should_create >> rail.Label('No') >> catch_and_log_error
        ...
        has_missing_task_resource_estimates >> rail.Label(
            'Yes') >> assign_project_team_members >> pick_assign_task_resources_method
        pick_assign_task_resources_method >> rail.Label(
            'Yes') >> assign_task_resources >> pick_bulk_update_estimates_method
        pick_assign_task_resources_method >> rail.Label(
            'No') >> trigger_assign_task_resources >> wait_for_assign_task_resources >> pick_bulk_update_estimates_method
        pick_bulk_update_estimates_method >> rail.Label(
            'Yes') >> bulk_update_task_resource_estimates >> add_log_entry >> catch_and_log_error
        pick_bulk_update_estimates_method >> rail.Label(
            'No') >> trigger_bulk_update_estimates >> wait_for_bulk_update_estimates >> add_log_entry >> catch_and_log_error
        has_missing_task_resource_estimates >> rail.Label(
            'No') >> need_to_assign_all_users
        need_to_assign_all_users >> rail.Label(
            'Yes') >> assign_project_all_members >> pick_assign_task_all_resources_method
        pick_assign_task_all_resources_method >> rail.Label(
            'Yes') >> assign_task_all_resources >> add_log_entry
        pick_assign_task_all_resources_method >> rail.Label(
            'No') >> trigger_assign_task_all_resources >> wait_for_assign_task_all_resources >> add_log_entry
        need_to_assign_all_users >> rail.Label(
            'No') >> add_log_entry
