...
def create_child_dag_wbs(config):
    with rail.create_airflow_dag(
        ...
        description='Guidehouse Workday User Import - Process Supervisor',
        ...
        log_supervisor_disabled_with_past_enddate = rail.WriteLogOperator(
            ...
            properties=lambda item, dag_run: {
                ...
                'status': 'Exception',
                'details': f"Supervisor {item['properties']['manager']} disabled with past enddate"
            }
        )
        ...
        is_supervisor_disabled_with_past_enddate >> rail.Label('Yes') >> log_supervisor_disabled_with_past_enddate >> dummy_filter_user_logs
        is_supervisor_disabled_with_past_enddate >> rail.Label('No') >> if_supervisor_disabled
        ...
        get_missing_supervisor_permission >> should_add_missing_permissions >> rail.Label(
        ...
        is_supervisor_changed >> rail.Label('Yes') >> update_supervisor_schedule_for_user >> dummy_filter_user_logs
        is_supervisor_changed >> rail.Label('No') >> same_supervisor_already_assigned >> dummy_filter_user_logs
