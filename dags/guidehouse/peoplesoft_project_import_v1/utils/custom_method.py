...
def validate_project_file_name(file_path, expected_prefix):
    ...
    project_pattern = rf'^{expected_prefix}_Project_\d{{8}}_\d{{6}}\.txt\.pgp$'
    resource_pattern = rf'^{expected_prefix}_Project_team_\d{{8}}_\d{{6}}\.txt\.pgp$'
    ...
    if re.match(project_pattern, file_name):
        ...
        return {
            ...
            'error_message': f"Invalid file name: {file_name}. Expected PeopleSoft project format: {expected_prefix}_Project_YYYYMMDD_HHMMSS.txt.pgp or resource format: {expected_prefix}_Project_team_YYYYMMDD_HHMMSS.txt.pgp"
        }
...
def get_invalid_logs_property_conf(item):
    def get_missing_field():
        ...
        if missing_fields:
            ...
            return f"Project processing is skipped due to required fields missing in input file: {fields_str}"
...
def _load_log_records_safely(log_artifact):
    """Load all records from a log artifact, returning [] if it cannot be read."""
    try:
        records = rail.load_all_records(log_artifact)
        return records or []
    except Exception:
        logging.warning("Could not load log records from artifact %s; skipping.", log_artifact, exc_info=True)
        return []

def format_logs_callable():
    """
    Format and aggregate logs from all DAG runs
    Merges the master log (client/cost-center/main-level logs) with the per-project
    child logs gathered from each project child dagrun, then calculates
    success, error, and exception counts for email reporting.
    """
    final_log_records = []

    # Merge per-project child logs (each child writes to its own log to avoid
    # lock contention on the master log). None/absent when there is no valid data.
    child_log_artifacts = rail.result("gather_child_project_logs")
    if child_log_artifacts:
        if not isinstance(child_log_artifacts, list):
            child_log_artifacts = [child_log_artifacts]
        for log_artifact in child_log_artifacts:
            final_log_records.extend(_load_log_records_safely(log_artifact))

    # Merge the master log (client, cost center, and main-level records)
    final_log_records.extend(_load_log_records_safely(rail.result("create_exception_log")))
...
def get_all_project_task_uris_for_enforce():
    ...
    try:
        ...
        logging.warning("Could not extract newly added task URIs for enforce.", exc_info=True)
    try:
        ...
        logging.warning("Could not extract existing task URIs for enforce.", exc_info=True)
