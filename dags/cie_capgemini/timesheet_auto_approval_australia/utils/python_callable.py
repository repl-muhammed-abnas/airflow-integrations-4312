...
def getmin_max(config):
    ...
    start_date = min(unique_dates, key=lambda d: datetime.strptime(
        d["Timesheet_Start_Date"], config.dateFormat))
    end_date = max(unique_dates, key=lambda d: datetime.strptime(
        d["Timesheet_End_Date"], config.dateFormat))
    min_dt = datetime.strptime(
        start_date["Timesheet_Start_Date"], config.dateFormat).strftime(config.dateFormatTimeOffReport)
    max_dt = datetime.strptime(
        end_date["Timesheet_End_Date"], config.dateFormat).strftime(config.dateFormatTimeOffReport)
            ...
            if timedata.get('TimesheetURI') not in time_export_data:
                if not timeoffEntryDataReport.empty and timedata.get('UserUri') in timeoffEntryDataReport_groupby_user.groups.keys():
                    ...
                    any_timeoff = len([entry for entry in timeoff_entries.to_dict('records') if datetime.strptime(
                        timedata['Timesheet Start Date'], config.dateFormat) <= datetime.strptime(entry['Time Off Date'], config.dateFormat) and datetime.strptime(entry['Time Off Date'], config.dateFormat) <= datetime.strptime(timedata['Timesheet End Date'], config.dateFormat)])
                    if any_timeoff == 0:
...
def filter_error_uris(config):
    batch_res = rail.result('get_bulk_validation')
    final_uris, error_uris = [], []
    recalc_ts_uris = rail.result('get_latest_recalc_ts_uris')
    print("recalc_ts_uris", recalc_ts_uris)
    ...
    print("recalc_ts_uris", recalc_ts_uris)
    final_uris = list(set(recalc_ts_uris) - set(error_uris))
    return final_uris
