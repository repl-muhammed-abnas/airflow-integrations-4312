...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        get_shifts_to_assign = rail.PythonOperator(
            task_id = 'get_shifts_to_assign',
            python_callable=lambda: custom_methods.get_shifts_to_assign(rail.result("get_query_data"))
        )
        ...
        log_success = rail.WriteLogOperator(
            ...
            items= '{{ result("get_shifts_to_assign","extra_records") | to_json  }}',
        ...
        if_exception_logs_present = rail.IfOperator(
            ...
            test= '{{ result("get_shifts_to_assign", "skipped_records") | is_truthy }}',
        ...
        log_exception = rail.WriteLogOperator(
            ...
            message="Regular shift is not assigned to the user since the 'CTR:SHIFT' is not received in the feed file.",
            items= '{{ result("get_shifts_to_assign", "skipped_records") | to_json }}',
            log= "{{ dag_run.conf.log }}",
            severity="Exception",
        ...
        has_shift_to_delete >> rail.Label(
            "Yes") >> bulk_delete_shifts >> get_shifts_to_assign >> bulk_put_shift_assignments

        has_shift_to_delete >> rail.Label(
            "No") >> get_shifts_to_assign >> bulk_put_shift_assignments >> log_success >> if_exception_logs_present

        if_exception_logs_present >> rail.Label(
