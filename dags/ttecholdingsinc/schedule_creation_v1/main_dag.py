...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'ttec_schedule_creation_master_{config.instance}_v1',
        ...
        query_distinct_shifts = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT schedulename FROM inputdatacollection WHERE NULLIF(schedulename,'') IS NOT NULL AND schedulename == 'CTR:SHIFT' """
        )
        ...
        download_file >> load_user_data >> create_log >> create_rawdata_collection >> has_collection_data
        ...
        query_unique_users_for_pto >> process_pto_schedules_to_assign >> wait_for_process_pto_schedules_to_assign >> format_logs

        format_logs >> render_logs_csv >> get_errored_logs >> get_log_file_name >> upload_logs_to_sftp >>\
