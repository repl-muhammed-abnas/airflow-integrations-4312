...
def get_process_users_conf(item, mapper, punch_policy_name, payrule_mapper):
    ...
    def get_servicecenter_fullpath():
        return item['client_name']+"|"+item['program_name']+"|"+item['project_name']
    ...
    return {
        ...
        **{
            ...
            'servicecenter_uri': rail.find_first_by_attr_and_get_attr(
                rail.result('get_all_updated_servicecenter_grps'), 'full_path', get_servicecenter_fullpath(), 'uri')
                if item['client_name'] and item['client_code'] and item['program_code'] and item['program_name'] and item['project_name'] else null,
...
def get_put_user_payload(dag_run):
    ...
    put_user_payload = {
        "user": {
            ...
            "serviceCenterSchedule": [
                ...
                ] if dag_run.conf['servicecenter_uri'] else null,
    ...
    if not dag_run.conf['servicecenter_uri']:
        log.append("Service Center group assignment skipped, all required fields for servicecenter not available in feed file")
...
def update_servicecenter_grp(servicecenter_uri, current_servicecenter_uri, dag_run):
    if not dag_run.conf['servicecenter_uri']:
        return null
    return {
    ...
    } if servicecenter_uri != current_servicecenter_uri else null
...
def apply_user_modifications_payload(dag_run):
    update_user_payload =  {
        ...
        "modifications": {
            ...
            "serviceCenterScheduleToApply": update_servicecenter_grp(dag_run.conf['servicecenter_uri'],
...
def get_update_user_message(dag_run):
    ...
    if not dag_run.conf['servicecenter_uri']:
        exception_msg.append("Service Center group assignment skipped, all required fields for servicecenter not available in feed file")
...
def get_add_servicecenter_payload(dag_run):
    return {
        "serviceCenter": {
            "parent": {
                "uri": rail.result("get_parent_servicecenter_details")[0]['uri']
            },
        }if rail.result("get_parent_servicecenter_details") else null,
        "modifications": {
            "name": dag_run.conf['servicecenter_name'],
            "codeToApply": {
                "value": dag_run.conf['servicenter_code']
                },
            "isEnabled": 1
        },
        "unitOfWorkId": str(uuid4())
