...
from ttecholdingsinc.user_sync_v1.utils.request_payload import get_today_date, get_replicon_date
from ttecholdingsinc.user_sync_v1.utils.response_filter import get_missing_permissions
    ...
    with rail.TaskGroup(group_id='process_supervisor_assignment_task', prefix_group_id=False):
        ...
        log_supervisor_not_present = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                'supervisor_id': dag_run.conf['supervisor_id'],
                'action': 'Add' if status == 'new_user' else 'Update',
                'effectivedate': dag_run.conf['effective_date'] if status != 'new_user' else None,
