...
from ttecholdingsinc.user_sync.utils import request_payload
from ttecholdingsinc.user_sync.tasks.process_supervisor import process_supervisor_assignment_task_group


def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_new_users_dagid,
        description='TTEC HOLDINGS INC - User Sync Process New Users',
        ...
        is_enddate_available >> rail.Label(
            'Yes') >> log_endate_exception >> catch_and_log_errors
        is_enddate_available >> rail.Label('No') >> add_new_user

        add_new_user >> remove_timeoff_assignments >> process_supervisor_entry
        process_supervisor_exit >> process_time_off_type_assignment_new_user >> wait_for_process_time_off_type_assignment_new_user
        wait_for_process_time_off_type_assignment_new_user >> gather_time_off_type_error_logs >> gather_time_off_type_exception_logs
        gather_time_off_type_exception_logs >> has_any_error_or_exceptions_present
        has_any_error_or_exceptions_present >> rail.Label(
            'No') >> log_user_completion
        has_any_error_or_exceptions_present >> rail.Label(
            'No') >> log_error_or_exceptions_present >> catch_and_log_errors
