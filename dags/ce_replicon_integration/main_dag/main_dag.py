...
# config :
# https://github.com/replicon/airflow-integrations/blob/main/dags/ce_replicon_integration/main_dag/config.py


def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"standard_computerease_{config.region.replace('-', '_')}_main_trigger_{config.instance}",
        description=f'ComputerEase {config.region} Master {config.instance}',
        ...
        get_computerease_company_conn_ids = rail.SimpleHttpOperator(
            task_id='get_computerease_company_conn_ids',
            method='POST',
        ...
        parse_computerease_clientids = rail.PythonOperator(
            task_id='parse_computerease_clientids',
            python_callable=lambda: rail.get_connector_clientids_by_integration(rail.result(
                'get_computerease_company_conn_ids'), config.workflows)
        )

        def get_overdue_interval_clients(workflow_key, variable_name_prefix, interval_config_name):
            client_ids = rail.result('parse_computerease_clientids').get(workflow_key, [])
            if not client_ids:
                return []
        ...
        trigger_initial_setup = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.initial_setup_dag,
            retries=0,
        ...
        is_user_sync = rail.IfOperator(
            ...
            test=lambda: len(rail.result('parse_computerease_clientids')[
                             config.user_sync]) > 0,
        ...
        trigger_user_sync = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.user_sync_dag,
            retries=0,
            items=lambda: rail.result('parse_computerease_clientids')[
                config.user_sync],
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: dict(item.items())
        )
        ...
        is_time_sync = rail.IfOperator(
            task_id='is_time_sync',
            test=lambda: len(rail.result('parse_computerease_clientids')[
                             config.time_sync]) > 0,
            yes_task='trigger_time_sync',
            no_task='is_custom_integrations_present'
        )

        trigger_time_sync = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_time_sync',
            trigger_dag_id=config.time_sync_dag,
            retries=0,
            items=lambda: rail.result('parse_computerease_clientids')[
                config.time_sync],
            execution_timeout=timedelta(days=config.execution_timeout_days),
        ...
        should_delete_dagrun = rail.IfOperator(
            ...
            test="{{ get_task_state('trigger_initial_setup') == 'skipped' and \
                    ...
                    get_task_state('trigger_time_sync') == 'skipped' and \
        ...
        create_hmac_signature_and_get_request_body >> get_computerease_company_conn_ids >> parse_computerease_clientids >> check_initial_setup_schedule
        check_initial_setup_schedule >> rail.Label('Yes') >> trigger_initial_setup >> is_user_sync
        check_initial_setup_schedule >> rail.Label('No') >> is_user_sync

        is_user_sync >> rail.Label(
            'Yes') >> trigger_user_sync >> is_project_sync
        is_user_sync >> rail.Label(
            'No') >> is_project_sync
        is_project_sync >> rail.Label(
            'Yes') >> trigger_project_sync >> is_time_sync
        is_project_sync >> rail.Label(
            'No') >> is_time_sync
        is_time_sync >> rail.Label(
            'Yes') >> trigger_time_sync >> is_custom_integrations_present
        is_time_sync >> rail.Label(
