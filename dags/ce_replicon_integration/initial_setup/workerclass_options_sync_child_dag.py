...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.worker_class_child_dag_id,
        description=f'{config.company_key} Syncs Worker Class options from ComputerEase to Replicon',
        ...
        batch_task = rail.BatchTaskRunOperator(
            task_id='batch_task',
            start_task='get_worker_classes_from_ce',
            end_task='catch_error',
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
        )

        get_worker_classes_from_ce = rail.ComputereaseAPIOperator(
            task_id='get_worker_classes_from_ce',
            endpoint='/catalog/worker-class',
            computerease_conn_id='{{ dag_run.conf.computerease_conn_id }}',
            request_method='GET',
            data_handler=lambda response: response.get('data', [])
        )

        def extract_worker_class_options():
            worker_classes = rail.result('get_worker_classes_from_ce')
            unique_combinations = set()

            for wc in worker_classes:
                union = wc.get('union', '').strip()
                code = wc.get('code', '').strip()

                if code:
                    if not union:
                        union = 'NON-UNION'

                    combined_key = f"{union} : {code}"
                    unique_combinations.add(combined_key)
        ...
        extract_workerclass_options = rail.PythonOperator(
            task_id='extract_workerclass_options',
            python_callable=extract_worker_class_options
        )

        get_workerclass_oef = rail.RepliconServiceOperator(
            task_id='get_workerclass_oef',
            ...
            data_handler=lambda response: next(
                (oef for oef in response if oef.get('name') == 'Worker Class'),
                None
            )
        )
        ...
        get_existing_tags = rail.RepliconServicePageOperator(
            ...
            data=lambda: {
                ...
                "filterExpression": {
                    ...
                    "rightExpression": {
                        "value": {
                            "uri": rail.result('get_workerclass_oef')['uri']
                        }
                    }
                }
            },
        ...
        def get_tags_to_create_and_enable():
            existing_tags = rail.result('get_existing_tags')
            required_tags = rail.result('extract_workerclass_options')
            tags_to_enable = []
            tags_to_create = []
            tags_to_update = []
        ...
        create_tag_draft = rail.RepliconServiceOperator(
            ...
            replicon_conn_id='{{ dag_run.conf.replicon_conn_id }}',
            data=lambda: {
                "objectExtensionTagDefinitionUri": rail.result('get_workerclass_oef')['uri']
            }
        )
        ...
        if_tags_to_create = rail.IfOperator(
            ...
            test=lambda: len(rail.result('get_tags_to_create_and_enabled')['tags_to_create']) > 0,
        ...
        update_tag_names = rail.RepliconServiceCallForEachItemOperator(
            ...
            items=lambda: rail.result('get_tags_to_create_and_enabled')['tags_to_update'],
            endpoint='services/ObjectExtensionTagService1.svc/UpdateName',
            replicon_conn_id='{{ dag_run.conf.replicon_conn_id }}',
            data=lambda item: {
        ...
        enable_tags = rail.RepliconServiceCallForEachItemOperator(
            task_id='enable_tags',
            items=lambda: rail.result('get_tags_to_create_and_enabled')['tags_to_enable'],
        ...
        disable_tags = rail.RepliconServiceCallForEachItemOperator(
            ...
            items=lambda: rail.result('get_tags_to_create_and_enabled')['tags_to_disable'],
        ...
        def get_downstreamtasks_error(error_message):
            return {
                'error': f'Error in worker class sync - {error_message}'
            }
        ...
        batch_task >> get_worker_classes_from_ce >> extract_workerclass_options >> get_workerclass_oef >> get_existing_tags >> get_tags_to_create_and_enabled >> created_tags_list >> foreach_tag_to_create >> create_tag_draft
