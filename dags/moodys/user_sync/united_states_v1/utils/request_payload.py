...
from moodys.user_sync.united_states_v1.mapper.user_sync_mapper import user_sync_mapper
...
def get_process_users_conf(item, config):
    ...
    return {
        ...
        **{
            ...
            'usalocationudfdropdownuri': rail.find_first_by_attr_and_get_attr(rail.result(
                'get_location_udf_dropdown_values'), 'name', 'USA', 'uri')
...
def get_udfs(userstatus, dag_run):
    ...
    if userstatus == 'adduser':
        ...
        add_udf_field_values(
            definitionuri=dag_run.conf['locationudfdefinitionuri'], dropdownuri=dag_run.conf['usalocationudfdropdownuri'])
...
def get_invalid_fields_message_add(dag_run):
    ...
    if not dag_run.conf['payrulescripturi'] and not dag_run.conf['payrulename']:
        exception.append(
            f'Payrule is not available for location "{dag_run.conf["locationname"]}" in mapper')
