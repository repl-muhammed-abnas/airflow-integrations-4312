...
from moodys.user_sync.united_states_v1.utils import request_payload
from moodys.user_sync.united_states_v1.tasks.process_supervisor import process_supervisor_assignment_task_group
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        add_new_user = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: request_payload.get_put_user_payload(
                dag_run, config)
        )
        ...
        has_valid_data >> rail.Label('Yes') >> is_enddate_available >> rail.Label(
            'Yes') >> log_endate_exception >> catch_and_log_errors
        is_enddate_available >> rail.Label('No') >> add_new_user

        add_new_user >> remove_timeoff_assignments >> is_supervisor_in_feed_file >> rail.Label(
            'No') >> put_timeoff_assignment_for_user >> log_user_completion
