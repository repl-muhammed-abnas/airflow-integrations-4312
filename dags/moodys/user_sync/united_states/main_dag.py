...
from moodys.user_sync.united_states.utils import request_payload
from moodys.user_sync.united_states.tasks.get_user_prereqs import get_user_prereqs_task_group
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        send_blank_payload_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | User Sync - United States no records in file - {{ current_time_in_specified_tz() }}',
        ...
        query_valid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM inputdatacollection WHERE NULLIF(countryid, '') IS NOT NULL and
                    ...
                    NULLIF(companyname, '') IS NOT NULL and NULLIF(companycode, '') IS NOT NULL"""
        )
        ...
        query_invalid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM inputdatacollection WHERE NULLIF(countryid, '') IS NULL or
                    ...
                    NULLIF(companyname, '') IS NULL or NULLIF(companycode, '') IS NULL"""
        )
        ...
        query_invalid_records >> has_invalid_records >> rail.Label('No') >> no_invalid_records_present
