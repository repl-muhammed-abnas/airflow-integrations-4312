...
from moodys.user_sync.united_states.utils import request_payload, response_filter
from moodys.user_sync.united_states.tasks.process_supervisor import process_supervisor_assignment_task_group
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        has_valid_data >> rail.Label('Yes') >> is_enddate_available >> rail.Label('No') >> is_user_disabled

        is_enddate_available >> rail.Label('Yes') >> is_enddate_greater_than_start_date

        is_enddate_greater_than_start_date >> rail.Label('Yes') >> update_employee_endate >> is_enddate_in_future
        is_enddate_greater_than_start_date >> rail.Label('No') >> log_endate_exception >> catch_and_log_errors

        is_enddate_in_future >> rail.Label('Yes') >> log_end_date_future >> catch_and_log_errors
        is_enddate_in_future >> rail.Label('No') >> disable_login >> log_disabled_success >> catch_and_log_errors

        is_user_disabled >> rail.Label('Yes') >> enable_login >> get_user_info
        is_user_disabled >> rail.Label('No') >> get_user_info >> get_current_udf_values
