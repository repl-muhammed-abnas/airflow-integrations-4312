...
from moodys.user_sync.united_states.mapper.user_sync_mapper import user_sync_mapper
    ...
    return {
        ...
        **{
            ...
            'timesheetapprovalpathuri': rail.find_first_by_attr_and_get_attr(rail.result('get_timesheet_approval_paths'),
                'displayText', config.TIMESHEET_APPROVAL_PATH, 'uri'),
            ...
            'usalocationudfdropdownuri': rail.find_first_by_attr_and_get_attr(rail.result('get_location_udf_dropdown_values'),
                'name', 'USA', 'uri')
...
def get_udfs(userstatus, dag_run):
    ...
    if userstatus =='adduser':
        ...
        add_udf_field_values(definitionuri = dag_run.conf['locationudfdefinitionuri'],dropdownuri = dag_run.conf['usalocationudfdropdownuri'])
    ...
    if userstatus == 'updateuser':
        ...
        if current_employeecategory:
            add_udf_field_values(definitionuri = dag_run.conf['employeecategorydefinitionuri'])
...
def get_schedule_name(dag_run):
    if dag_run.conf['actualworkinghrs']:
        if int(float(dag_run.conf['actualworkinghrs'])) == 30:
            return "USA - Standard hours 30 - 6 hours/day, Su, Sa off"
        if int(float(dag_run.conf['actualworkinghrs'])) == 35:
            return "USA - Standard hours 35 - 7 hours/day, Su, Sa off"
        if int(float(dag_run.conf['actualworkinghrs'])) == 40:
            return "USA - Standard hours 40 - 8 hours/day, Su, Sa off"
    return "USA - Standard hours 40 - 8 hours/day, Su, Sa off"
...
def get_invalid_fields_message_add(dag_run):
    ...
    if not dag_run.conf['payrulescripturi'] and dag_run.conf['payrulename']:
        exception.append(f'Payrule "{dag_run.conf["payrulename"]}" not available in replicon')
    if not dag_run.conf['payrulescripturi'] and not dag_run.conf['payrulename']:
        exception.append(f'Payrule is not available for location "{dag_run.conf["locationname"]}" in mapper')
