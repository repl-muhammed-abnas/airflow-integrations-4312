...
def get_required_time_off_types(response):
    assignable_timeoffs_details = list(
        filter(lambda x: x['type'] == 'timeofftype', user_sync_mapper))
    assignable_timeoffs_names = list(
        map(lambda item: item['timeofftypename'], assignable_timeoffs_details))
...
def get_missing_permissions(response, dag_run):
    ...
    if response:
        ...
        end_user_permission = len(
            [x for x in response if x['permissionSet']['name'] == 'End user with reports']) > 0
