...
from moodys.user_sync.france.mapper.user_sync_mapper import user_sync_mapper
...
def get_process_users_conf(item, config):
    def get_location_uri(item):
        location_details = list(filter(lambda x: x['name']==item['locationname'] and x['code']==item['locationcode'],rail.result('get_all_locations')))
        if location_details:
            return location_details[0]['uri']
        return null

    def get_department_uri(item):
        dept_details = list(filter(lambda x: x['name']==item['companyname'] and x['code']==item['companycode'],rail.result('get_all_departments')))
        if dept_details:
            return dept_details[0]['uri']
        return null

    def get_timezone_uri(item):
        timezone_details = list(filter(lambda x: x['type']=='timezone' and x['timezone_ww']==item['timezone'],user_sync_mapper))
        if timezone_details:
    ...
    def get_timesheettemplate_name(item):
        if item['employeetypename'] == "CA":
            return "France - Standard Timesheet - Forfait Jours"
        if item['employeetypename'] != "NA":
            if get_ftepercenttype(item) == "Full Time":
                return "France - In/Out Timesheet - Full Time"
            if get_ftepercenttype(item) == "Part Time":
                return "France - In/Out Timesheet - Part Time"
        return null

    def get_timesheetapprovalpathname(item):
        if item['employeetypename'] == "CA":
            return "Forfait jours"
        return "France Custom Approval"

    def get_payrule_name(item):
        if item['employeetypename'] == "CA":
            return "Moodys - France - Forfait Jours"
        if get_ftepercenttype(item) == "Full Time":
            return "Moodys France others Full time"
        if get_ftepercenttype(item) == "Part Time":
            return "Moodys France others Part time"
        return null
    ...
    return {
        ...
        **{
            ...
            'employeecategorydropdownuri' : rail.find_first_by_attr_and_get_attr(rail.result('get_employeecategory_udf_dropdown_values'),
                'name', item['employeecategory'], 'uri') if item['employeecategory'] else null,
            ...
            'timesheetapprovalpathuri': rail.find_first_by_attr_and_get_attr(rail.result('get_timesheet_approval_paths'),
                'displayText', get_timesheetapprovalpathname(item), 'uri'),
            'timesheetperiod': "Forfait jours (Monthly)" if item['employeetypename'] == "CA" else "France others Weekly",
            ...
            'timeoffapprovalpathuri': rail.find_first_by_attr_and_get_attr(rail.result('get_timeoff_approval_paths'),
                'displayText', config.TIMEOFF_APPROVAL_PATH, 'uri'),
            ...
            'supervisorpermissionuri': rail.find_first_by_attr_and_get_attr(rail.result('get_all_permission_set'),
                'displayText', 'Supervisor', 'uri'),
            'enduserwithreportspermissionuri': rail.find_first_by_attr_and_get_attr(rail.result('get_all_permission_set'),
                'displayText', 'End user with reports', 'uri'),
...
def get_udfs(userstatus, dag_run):
    ...
    if userstatus =='adduser':
        if dag_run.conf['employeecategory']:
            add_udf_field_values(definitionuri = dag_run.conf['employeecategorydefinitionuri'], dropdownuri= dag_run.conf['employeecategorydropdownuri'])
            add_udf_field_values(definitionuri = dag_run.conf['employeecategory2definitionuri'],textvalue = dag_run.conf['employeecategory'])
        if dag_run.conf['ftepercent'] and dag_run.conf['ftepercent'] != 'None':
            add_udf_field_values(definitionuri = dag_run.conf['ftepercentdefinitionuri'], number = dag_run.conf['ftepercent'])
        if dag_run.conf['adpfile']:
            add_udf_field_values(definitionuri = dag_run.conf['adpfiledefinitionuri'],textvalue = dag_run.conf['adpfile'])
        if dag_run.conf['rehire']:
            add_udf_field_values(definitionuri = dag_run.conf['rehiredefinitionuri'],textvalue = dag_run.conf['rehire'])
            add_udf_field_values(definitionuri = dag_run.conf['rehiredefinitionuri'],textvalue = dag_run.conf['rehire'])
        if dag_run.conf['actualworkinghrs']:
            add_udf_field_values(definitionuri = dag_run.conf['actualworkinghrsdefinitionuri'],textvalue = dag_run.conf['actualworkinghrs'])

    if userstatus == 'updateuser':
        current_rehire = rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
            'customField.displayText', 'Rehire', 'text')

        current_adpfile = rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
            'customField.displayText', 'ADP File#', 'text')

        current_ftppercent = rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
            'customField.displayText', 'FTE%', 'text')

        current_employeecategory = rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
            'customField.displayText', 'Employee Category', 'text')

        current_actualworkinghrs = rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
            'customField.displayText', 'Actual Working Hours', 'text')

        current_locationudf = rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
            'customField.displayText', 'Location', 'text')
        ...
        if current_locationudf == 'USA':
            add_udf_field_values(definitionuri = dag_run.conf['locationudfdefinitionuri'])
...
def get_schedule_name(dag_run):
    ...
    if dag_run.conf['actualworkinghrs']:
        if float(dag_run.conf['actualworkinghrs']) == 38.30 and dag_run.conf['employeetypename'] == "CR":
            return "France - 38H30/WEEK"
        if int(float(dag_run.conf['actualworkinghrs'])) == 35 and (dag_run.conf['employeetypename'] == "CA"
            or dag_run.conf['employeetypename'] == "CA213" or dag_run.conf['employeetypename'] == "CA217"):
            return "France - 7H/day Mon to Fri"
        if dag_run.conf['employeetypename'] == "ETAM":
            if int(float(dag_run.conf['actualworkinghrs'])) == 35:
                return "France - 7H/day Mon to Fri"
            if float(dag_run.conf['actualworkinghrs']) == 37.13:
                return "France - 37 h 13 nim - 37 H13/WEEK"
            if int(float(dag_run.conf['actualworkinghrs'])) == 37:
                return "France - 37 hours per week - 37 H/WEEK"
        if int(float(dag_run.conf['actualworkinghrs'])) == 35 and dag_run.conf['employeetypename'] == "NA":
            return "France - 7H/day Mon to Fri"
    return "France - 7H/day Mon to Fri"
...
def put_timeoff_assignment_for_user(dag_run):
    timeofftype_uris = list(map(lambda x: x['timeofftypeuri'] , rail.load_all_records(dag_run.conf['get_required_time_off_types'])))
    return {
...
def is_employeetype_available_in_mapper(dag_run):
    employeetype_in_mapper = list(filter(lambda x:x['type']=='employeetype' and  x['employeetypename']== dag_run.conf['employeetypename']
        ,user_sync_mapper))
...
def test_valid_fields_add(dag_run):
    return bool(is_location_available_in_mapper(dag_run) and dag_run.conf['locationuri'] and
        is_department_available_in_mapper(dag_run) and dag_run.conf['departmenturi']
