...
from moodys.user_sync.canada.mapper.user_sync_mapper import user_sync_mapper
...
def get_invalid_fields_message_add(dag_run):
    ...
    if not dag_run.conf['payrulescripturi'] and not dag_run.conf['payrulename']:
        if dag_run.conf['actualworkinghrs']:
            exception.append(f'Payrule is not available for location "{dag_run.conf["locationname"]}" and Actual Working hours "{dag_run.conf["actualworkinghrs"]}" in mapper')
        else:
            exception.append(f'Payrule is not available for location "{dag_run.conf["locationname"]}" in mapper as Actual Working hours is blank in payload')
