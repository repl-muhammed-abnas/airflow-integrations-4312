...
from moodys.user_sync.japan_v1.mapper.user_sync_mapper import user_sync_mapper
...
def get_process_users_conf(item, config):
    ...
    return {
        ...
        **{
            ...
            'regularshiftuserdropdownuri': rail.find_first_by_attr_and_get_attr(rail.result(
                'get_regularshiftuser_udf_dropdown_values'), 'name', "9 - 5 with lunch", 'uri'),
...
def get_udfs(userstatus, dag_run):
    ...
    if userstatus == 'adduser':
        ...
        if dag_run.conf['employeetypename'] in ['A', 'B', 'C', 'D']:
            add_udf_field_values(
                definitionuri=dag_run.conf['regularshiftuserdefinitionuri'], dropdownuri=dag_run.conf['regularshiftuserdropdownuri'])
    ...
    if userstatus == 'updateuser':
        ...
        if dag_run.conf['employeetypename'] in ['A', 'B', 'C', 'D']:
            ...
            add_udf_field_values(
                definitionuri=dag_run.conf['regularshiftuserdefinitionuri'])
