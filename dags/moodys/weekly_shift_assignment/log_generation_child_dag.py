...
import rail
from moodys.weekly_shift_assignment.utils import python_callable_method


# config :
# https://github.com/replicon/airflow-integrations/blob/main/dags/moodys/weekly_shift_assignment/config.py


def create_log_weekly_shift_update_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'moodysemea_weeklyshiftassignment_loggeneration_{config.instance}',
        description=f'Moodysemea_Weekly shift assignment_log generation V1.0 {config.instance}',
        ...
        create_weekly_shift_log_csv = rail.WriteCSVFileOperator(
            task_id='create_weekly_shift_log_csv',
        ...
        def file_upload_failed(context):
            ...
            subject = '{{ get_company_key() }} | Weekly Shift Assignment - Uploading Logs to SFTP failed {{ current_time_in_specified_tz("America/Los_Angeles") }}'
        ...
        upload_log_to_sftp = rail.SFTPUploadFileOperator(
            ...
            content='{{ result("create_weekly_shift_log_csv") }}',
        ...
        generate_download_link = rail.GeneratePresignedDownloadUrlOperator(
            ...
            artifact_name='{{ result("create_weekly_shift_log_csv")}}',
        ...
        send_weekly_shift_update_email = rail.EmailOperator(
            task_id='send_weekly_shift_update_email',
            to=config.tenant_email,
            bcc="{%- if result('get_errored_logs', key='length') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Weekly Shift Assignment - "  }} \
            ...
            html_content="templates/email/weekly_shift_update.html"
        )
        ...
        format_logs >> create_weekly_shift_log_csv >> upload_log_to_sftp >> generate_download_link \
            >> [get_errored_logs, get_exception_logs, get_skipped_logs, get_success_logs] >> \
            send_weekly_shift_update_email >> log_to_sumo

        return dag


rail.for_each_instance(create_log_weekly_shift_update_child_dag)
