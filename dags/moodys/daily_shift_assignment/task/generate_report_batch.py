...
def report_batch(config):
    ...
    get_users_to_be_processed = rail.QueryCollectionOperator(
        ...
        AND loginname NOT IN (SELECT loginname FROM referencefile)""",
    ...
    run_report_group_exit >> report_has_data >> rail.Label(
        "Yes") >> load_report_data >> create_report_collection >> download_reference_file >> load_reference_file >> create_reference_collection \
        >> get_users_to_be_processed >> get_users_to_be_ignored
