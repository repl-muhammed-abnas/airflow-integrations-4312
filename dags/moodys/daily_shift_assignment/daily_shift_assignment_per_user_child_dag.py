...
from moodys.daily_shift_assignment.utils import response_filter
...
def create_moodys_daily_shift_per_existing_user_dag(config):
    # pylint: disable=too-many-statements
    with rail.create_airflow_dag(
        dag_id=f'moodysemea_dailyshiftassignment_perexistinguser_{config.instance}',
        description=f'Moodys_daily shift assignment_per existing user V1.0 {config.instance}',
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task='batch_task',
            no_task='create_daily_shift_per_user_log'
        )
        ...
        batch_task = rail.BatchTaskRunOperator(
            ...
            start_task='create_daily_shift_per_user_log',
            end_task='catch_and_log_errors',
        )

        create_daily_shift_per_user_log = rail.CreateLogOperator(
            task_id='create_daily_shift_per_user_log'
        )
        ...
        log_schedule_not_populated = rail.WriteLogOperator(
            ...
            log='{{ result("create_daily_shift_per_user_log") }}',
        ...
        can_run_batch_task >> rail.Label(
            'No') >> create_daily_shift_per_user_log

        create_daily_shift_per_user_log >> get_data_shift_details >> has_shift_name
...
rail.for_each_instance(create_moodys_daily_shift_per_existing_user_dag)
