...
import uuid
import rail
from moodys.timeoff_import_v1.utils import request_payload, response_filter
    ...
    with rail.create_airflow_dag(
        ...
        log_madatory_fields_not_present = rail.WriteLogOperator(
            ...
            message='\
                ...
                {%- endif -%}\
                {%- set duration = dag_run.conf.Duration -%}\
                {%- set no_of_days = dag_run.conf.No_of_Days_Booked -%}\
                {%- if duration and no_of_days -%} \
                    Both Duration and No of Days Booked are provided - only one field should be present (Duration for hourly bookings OR No of Days Booked for day-based bookings), \
                {%- elif not duration and not no_of_days -%} \
                    Neither Duration nor No of Days Booked is provided - one field is required (Duration for hourly bookings OR No of Days Booked for day-based bookings), \
                {%- endif -%}',
            severity='Exception',
            properties={
        ...
        log_time_off_delete = rail.WriteLogOperator(
            task_id='log_time_off_delete',
            message="Time off was successfully deleted from replicon",
            severity='Success',
            properties={
                'countryid': "{{dag_run.conf.Country_ID}}",
                'employeeid': "{{dag_run.conf.Employee_ID}}",
        ...
        time_off_success = rail.WriteLogOperator(
            ...
            message="Time off was successfully booked in replicon",
            severity='Success',
            properties={
                'countryid': "{{dag_run.conf.Country_ID}}",
                'employeeid': "{{dag_run.conf.Employee_ID}}",
        ...
        is_status_approved >> rail.Label(
            "No") >> is_timeoff_entryid_available >> rail.Label("Yes") >> delete_timeoff_booking >> log_time_off_delete >> catch_and_log_errors
