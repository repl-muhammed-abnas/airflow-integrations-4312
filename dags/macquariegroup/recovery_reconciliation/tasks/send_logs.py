...
def get_send_logs(config):
    with rail.TaskGroup(group_id='send_logs', prefix_group_id=False):
        ...
        render_logs_csv = rail.WriteCSVFileOperator(
            ...
            header=['LoginName', "Employee Type", "Department",
                    "Cost Centre", "Action", 'Status', "Details"],
            row=[
                '{{ item.properties.login_name }}',
                '{{ item.properties.employee_type }}',
                '{{ item.properties.department }}',
                '{{ item.properties.cost_centre }}',
        ...
        generate_download_link = rail.GeneratePresignedDownloadUrlOperator(
            ...
            output_file_name='log_{{ dag_run_ecid()}}_{{ result("new_file_sensor") | file_base }}.csv',
        ...
        upload_log_to_sftp = rail.SFTPUploadFileOperator(
            ...
            '/log_{{ dag_run_ecid()}}_{{ result("new_file_sensor") | file_base }}.csv',
        )
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('get_logged_errors', 'length') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | User Recovery Reconciliation is - " }} \
        ...
        has_any_entries_in_log >> rail.Label("Yes") >> [
            get_logged_errors, get_logged_skipped,
            get_logged_exceptions,
            get_logged_success] >> render_logs_csv >> generate_download_link >> upload_log_to_sftp >> send_import_complete_email
