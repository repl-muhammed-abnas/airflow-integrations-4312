...
def get_org_cost_uris():
    for row_data in rail.result("custom_fields")["rows"]:
...
def create_projects_tasks_list():
    task_details = read_collection(rail.result("query_task_level"))
    project_details = read_collection(rail.result("query_distinct_projects"))

    return [{
        "project_name": project_record["projectname"],
        "project_uri": project_record["projecturi"],
        "task_details": [{
            "project_task_fullpath": project_record['projectname']+"-"+task_record["tasknamefullpath"],
            ...
            "currency_uri": rail.find_first_by_attr_and_get_attr(rail.result('get_all_currencies'), 'displayText', 'USD$', 'uri'),
            ...
            "resource_assignment": rail.find_first_by_attr_and_get_attr(
                rail.result("resource_assignments"), 'task_uri', task_record['taskuri'], 'resource_uri'),
            ...
            "is_time_entry_allowed": rail.find_first_by_attr_and_get_attr(
                rail.result("model_task_details"), 'uri', task_record['taskuri'], 'isTimeEntryAllowed')
        } for task_record in task_details]
    } for project_record in project_details]
