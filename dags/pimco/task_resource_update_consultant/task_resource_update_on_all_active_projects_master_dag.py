...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'pimco_task_resource_update_consultant_on_all_active_consultant_projects_master_{config.instance}',
        description=f'PIMCO Task Resource Update on All Active consultant Projects_master {config.instance}',
        ...
        send_mail = rail.EmailOperator(
            ...
            subject= config.company_key + " | " + " Updating Task resource from base project to all in-progress consultant projects completed successfully at " + "{{current_time() }}",
        ...
        send_failure_mail = rail.EmailOperator(
            ...
            bcc=config.internal_logs_email,
            # pylint: disable=line-too-long
            subject= config.company_key + " | " + "Updating Task resource from base project to all in-progress consultant projects Failed at " + "{{current_time() }}",
