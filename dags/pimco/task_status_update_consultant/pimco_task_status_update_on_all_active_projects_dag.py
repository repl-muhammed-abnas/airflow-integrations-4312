...
from pimco.task_status_update_consultant.utils import request_payload, python_callable_method
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'pimco_task_status_update_consultant_on_all_active_consultant_projects_master_{config.instance}',
        description=f'PIMCO Task Status Update on All Active consultant Projects master dag {config.instance}',
        ...
        send_mail=rail.EmailOperator(
            ...
            subject= '{{ get_company_key() }}' + " | " + "Tasks status update from base project to all in-progress consultant projects completed successfully at " + "{{ current_time()}}",
        ...
        send_failure_mail = rail.EmailOperator(
            ...
            bcc=config.internal_logs_email,
            subject=  '{{ get_company_key() }}' + " | " + "  Tasks status update to all consultant project master recipe Failed at" + "{{current_time() }}",
            html_content="templates/failure_mail.html",
            params={
                'dag_id': f'pimco_consultant_task_status_update_consultant_child_{config.instance}'
            }
        )
