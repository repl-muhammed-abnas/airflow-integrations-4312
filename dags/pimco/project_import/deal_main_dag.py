...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"pimco_deal_project_import_master_{config.instance}",
        description=f"PIMCO Deal Project Import master {config.instance}",
        ...
        new_file_sensor = rail.SFTPAnyFileSensor(
            ...
            path=config.deal_input_filepath,
        ...
        move_input_file_to_processing = rail.SFTPMoveFileOperator(
            ...
            new_filename=config.deal_processing_filepath +
        ...
        create_time_data_collection = rail.CreateCollectionOperator(
            ...
            name="input_data",
            columns={
                'Deal/IO Number': 'Projectcode',
                'Deal Description': 'Projectname',
                'Active/Inactive Flag': 'flag'
            }
        )
        ...
        has_any_records = rail.IfOperator(
            ...
            yes_task='create_consultant_deal_data',
        ...
        create_consultant_deal_data = rail.WriteCSVFileOperator(
            task_id='create_consultant_deal_data',
            source="{{ result('create_time_data_collection') }}",
            header=['Deal/IO Number','Deal Description','Active/Inactive Flag'],
            row=lambda item: [
                item['Projectcode'],
                item['Projectname'],
                item['flag']
        ...
        upload_file_to_consultant_sftp = rail.SFTPUploadFileOperator(
            task_id = 'upload_file_to_consultant_sftp',
            content="{{ result('create_consultant_deal_data') }}",
            remote_filepath=config.consultant_deal_input_filepath +
            "{{ result('new_file_sensor') | file_name }}"
        )
        ...
        process_projects = rail.trigger_parallel_dagrun(
            ...
            parallel_count=20,
            trigger_dag_id=f"pimco_project_import_process_each_record_{config.instance}",
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            conf=lambda item: request_payload.get_trigger_parallel_dagrun_conf(item, "Deal")
        )
        ...
        upload_logs_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.deal_log_filepath +
        ...
        send_completion_mail = rail.EmailOperator(
            ...
            bcc="{%- if result('get_errored_logs', key='length') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Project Import for Deal is - " }} \
            ...
            params={
                'log_filepath': config.deal_log_filepath
            }
        )

        archieve_input_file = rail.SFTPMoveFileOperator(
            task_id='archieve_input_file',
            existing_filename=config.deal_processing_filepath + '{{ result("new_file_sensor") | file_name }}',
        ...
        has_any_records >> rail.Label(
            "Yes") >> create_consultant_deal_data >> upload_file_to_consultant_sftp >> \
                get_base_project_details >> process_projects >> render_logs_csv
