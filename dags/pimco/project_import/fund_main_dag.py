...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"pimco_fund_project_import_master_{config.instance}",
        description=f"PIMCO Fund Project Import master {config.instance}",
        ...
        new_file_sensor = rail.SFTPAnyFileSensor(
            ...
            path=config.fund_input_filepath,
        ...
        move_input_file_to_processing = rail.SFTPMoveFileOperator(
            ...
            new_filename=config.fund_processing_filepath +
        ...
        declare_dag_runs_list = rail.SetVariableOperator(
            task_id='declare_dag_runs_list',
            name='process_dag_runs',
            value=[]
        )
        ...
        process_projects = rail.trigger_parallel_dagrun(
            ...
            conf=lambda item: request_payload.get_trigger_parallel_dagrun_conf(item, "Fund")
        )
        ...
        upload_logs_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.fund_log_filepath +
        ...
        send_completion_mail = rail.EmailOperator(
            ...
            bcc="{%- if result('get_errored_logs', key='length') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Project Import for Fund is - " }} \
            ...
            params={
                'log_filepath': config.fund_log_filepath
            }
        )

        archieve_input_file = rail.SFTPMoveFileOperator(
            task_id='archieve_input_file',
            existing_filename=config.fund_processing_filepath + '{{ result("new_file_sensor") | file_name }}',
        ...
        has_any_records >> rail.Label(
            "Yes") >> get_base_project_details >> declare_dag_runs_list >> process_projects >> render_logs_csv
