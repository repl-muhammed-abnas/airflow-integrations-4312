...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"pimco_consultant_project_import_process_each_record_{config.instance}",
        ...
        def get_project_name(dag_run):
            return f"{dag_run.conf['Projectcode']}_"
        ...
        process_update_project = rail.TriggerDagRunOperator(
            ...
            conf=lambda dag_run: request_payload.get_process_update_project_conf(
                dag_run, dag_run.conf['caller']),
            retries=0,
            execution_timeout=timedelta(
                days=config.execution_timeout_days)
        )
        ...
        process_create_project = rail.TriggerDagRunOperator(
            ...
            conf=lambda dag_run: request_payload.get_process_create_project_conf(
                dag_run, dag_run.conf['caller']),
            retries=0,
            execution_timeout=timedelta(
                days=config.execution_timeout_days)
        ...
        can_run_batch_task >> rail.Label("No") >> load_project >> does_project_exist >> rail.Label(
            "Yes") >> process_update_project >> wait_for_update_project
        does_project_exist >> rail.Label(
            "No") >> process_create_project >> wait_for_process_create_project

        [wait_for_process_create_project, wait_for_update_project] >> rail.Label(
            "on_error") >> catch_and_log_error >> log_to_sumo
