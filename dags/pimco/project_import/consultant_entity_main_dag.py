...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"pimco_consultant_entity_project_import_master_{config.instance}",
        ...
        new_file_sensor = rail.SFTPAnyFileSensor(
            ...
            path=config.consultant_entity_input_filepath,
        ...
        move_input_file_to_processing = rail.SFTPMoveFileOperator(
            ...
            new_filename=config.consultant_entity_processing_filepath +
        ...
        create_time_data_collection = rail.CreateCollectionOperator(
            ...
            name="input_data",
            columns={
                'Entity ID': 'Projectcode',
                'Entity Legal Name': 'Projectname',
        ...
        process_projects = rail.trigger_parallel_dagrun(
            ...
            conf=lambda item: request_payload.get_trigger_parallel_dagrun_conf(item, "Entity", "Consultant")
        )
        ...
        upload_logs_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.consultant_entity_log_filepath +
        ...
        send_completion_mail = rail.EmailOperator(
            ...
            bcc="{%- if result('get_errored_logs', key='length') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | Project Import for Consultant Entity is - " }} \
            ...
            params={
                'log_filepath': config.consultant_entity_log_filepath
            }
        )

        archieve_input_file = rail.SFTPMoveFileOperator(
            task_id='archieve_input_file',
            existing_filename=config.consultant_entity_processing_filepath + 'Consultant_{{ result("new_file_sensor") | file_name }}',
