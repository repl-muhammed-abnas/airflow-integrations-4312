...
def get_bulk_tasks_payload():
    return {
        ...
        "pageSize": "10000",
        "projectUris": [
            get_dag_run_conf()["project_uri"]
        ],
        "taskDataInclusionOptionUris": []
    }

def get_parent_uri(parent_task, task_code):
    if get_dag_run_conf()["task_level"] == 2:
        return " ".join(list(map(lambda all_tasks: all_tasks["uri"], filter(lambda all_tasks: parent_task == all_tasks["name"],
                rail.result("get_task_details")))))
    return " ".join(list(map(lambda all_tasks: all_tasks["uri"], filter(lambda all_tasks: parent_task == all_tasks["name"] and
                    task_code == all_tasks["code"], rail.result("get_task_details")))))
        ...
        "taskHierarchy": [
            {
                "target": {
                    "parent": {
                        "uri": get_parent_uri(task_details["parent_task"], task_details["task_code"]),
                    }
                } if task_details["parent_task"] and get_parent_uri(task_details["parent_task"], task_details["task_code"]) is not null
                and get_parent_uri(task_details["parent_task"], task_details["task_code"]) != "" else null,
            ...
            } for task_details in rail.result("project_task_data")["task_details"]
        ]
    }
...
def get_add_task_payload():
    return {
        "project": {
            "uri": get_dag_run_conf()["project_uri"],
        },
