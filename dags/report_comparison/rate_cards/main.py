...
from report_comparison.rate_cards.utils.custom_methods import *
...
def create_airflow_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f"maconomy_workbook rate cards report comparison {config.instance}",
        ...
        maconomy_data = rail.SimpleHttpOperator(
            ...
            data=json.dumps(
                {
                    "fields": "jobpricelistname,currency,description",
                    "limit": 0,
                }
            ),
            response_filter=lambda response: json.loads(response.text)
        ...
        maconomy_rate_card_data = rail.PythonOperator(
            task_id="maconomy_rate_card_data",
            python_callable=process_maconomy_rate_card_data
        )
        ...
        generate_download_link = rail.GeneratePresignedDownloadUrlOperator(
            ...
            output_file_name="Rate_Cards_Comparison_report_{{ecid()|replace(':','_')}}.csv",
        ...
        send_report_complete_mail = rail.EmailOperator(
            ...
            subject="Rate Cards Report Comparison is completed {{current_time_in_specified_tz()}}",
            html_content="templates/send_completion_mail.html",
        )
        
        workbook_logout >> workbook_data_api >> workbook_data_python
        workbook_data_python >> maconomy_data >> maconomy_rate_card_data
        maconomy_rate_card_data >> comparison_report
