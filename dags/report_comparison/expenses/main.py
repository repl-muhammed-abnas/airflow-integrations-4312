...
import rail
from report_comparison.expenses.utils.custom_methods import *


def create_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.master_dag_id,
        description=f"maconomy_workbook_expenses_comparison {config.instance}",
        ...
        workato_purchases_vat_api = rail.SimpleHttpOperator(
            task_id="workato_purchases_vat_api",
            http_conn_id="workato_endpoint",
            endpoint=config.workato_purchases_vat,
            headers={
        ...
        get_purchases_vat_mapper = rail.PythonOperator(
            task_id="get_purchases_vat_mapper",
            python_callable=process_purchases_vat_data
        )
        ...
        workbook_data_api = rail.SimpleHttpOperator(
            ...
            data=json.dumps({"DataboardId": 10052, "Parameters": {"1": "1"}}),
        ...
        maconomy_data = rail.SimpleHttpOperator(
            ...
            data=json.dumps(
                {
                    "fields": "remark2,employeenumber,entrydate,jobnumber,taskname,currency,financevatcode,amountcurrency,linenumber",
                    "restriction": "",
                    "limit": 0
                }
            ),
        ...
        maconomy_expenses_data = rail.PythonOperator(
            task_id="maconomy_expenses_data",
            python_callable=process_maconomy_expenses_data
        )
        
        maconomy_emp_data = rail.SimpleHttpOperator(
            task_id="maconomy_emp_data",
            method="POST",
            http_conn_id="maconomy_http_connid",
            endpoint=config.maconomy_emp_api,
            headers={
                "Maconomy-Authentication": "X-Reconnect",
                "Accept": "application/vnd.deltek.maconomy.containers+json; version=5.0",
                "Content-Type": "application/vnd.deltek.maconomy.containers+json; version=5.0",
        ...
        maconomy_combined_data = rail.PythonOperator(
            task_id="maconomy_combined_data",
            python_callable=combine_maconomy_data
        )
        ...
        generate_download_link = rail.GeneratePresignedDownloadUrlOperator(
            ...
            output_file_name="Employee_Expenses_report_{{ecid()|replace(':','_')}}.csv",
        ...
        send_report_complete_mail = rail.EmailOperator(
            ...
            subject="Expenses Report Comparison is completed {{current_time_in_specified_tz()}}",
            html_content="templates/completion_email.html",
        )

        workato_purchases_vat_api >> get_purchases_vat_mapper >> workbook_data_api
        workbook_data_api >> workbook_data_python
        workbook_data_python >> maconomy_data >> maconomy_expenses_data
        maconomy_expenses_data >> maconomy_emp_data >> maconomy_employee_data
        maconomy_employee_data >> maconomy_combined_data
        maconomy_combined_data >> comparison_report >> generate_csv_report
