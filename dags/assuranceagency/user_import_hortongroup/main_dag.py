...
from assuranceagency.user_import_hortongroup.utils.python_callable import get_ref_file_name
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'assuranceagency_user_import_hortongroup_master_{config.instance}',
        description=f'assuranceagency_user_importhortongroup__master_{config.instance}',
        ...
        send_incorrect_file_format_mail = rail.EmailOperator(
            ...
            subject=f'{config.company_key} | User import for Horton Group has been skipped - {datetime.now().strftime("%d-%m-%Y")}',
                     ...
                     "enabled","employeeid","startdate","enddate","emailaddress","initialsupervisorloginname", \
                        "permissionsets","timesheettemplate","timesheetperiodtype","timesheetapprovalpath","timezone", \
                            "workweek","holidaycalendar","initialschedulename","timeofftemplate","timeoffapprovalpath", \
                                "initialpayrulename","workdayid","position","workercategory","manager","businessunit","encoded"],
        ...
        query_blank_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM input_csv_data WHERE
                            ...
                            OR NULLIF(businessunit, '') IS NULL)   
                    """,
            name="blank_records"
        )
        ...
        query_input_on_loginame_employeeid = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM input_csv_data WHERE
                            ...
                            AND NULLIF(businessunit, '') IS NOT NULL)   
                    """,
        ...
        process_each_data_to_disable_user = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id = f'assuranceagency_user_import_hortongroup_disable_user_child_{config.instance}',
        ...
        process_each_data_to_add_user = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id = f'assuranceagency_user_import_hortongroup_add_user_child_{config.instance}',
            execution_timeout = timedelta(config.execution_timeout_days),
        ...
        process_each_data_to_update_user = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id = f'assuranceagency_user_import_hortongroup_update_user_child_{config.instance}',
            execution_timeout = timedelta(config.execution_timeout_days),
        ...
        process_each_supervisor_log_data = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'assuranceagency_user_import_hortongroup_update_supervisor_from_logs_child_{config.instance}',
        ...
        send_error_in_upload_mail = rail.EmailOperator(
            ...
            to=config.to_email,
            bcc=config.alert_email,
            subject=f'{config.company_key} | User Import for Horton Group - Uploading Logs to SFTP failed - {datetime.now().strftime("%d-%m-%Y")}',
        ...
        send_cshare_mail = rail.EmailOperator(
            ...
            {%- endif -%}",
            subject=f'{config.company_key} | User import for Horton Group - ' + "{{ result('email_subject_line') }} -" + datetime.now().strftime("%d-%m-%Y %H:%M:%S"),
