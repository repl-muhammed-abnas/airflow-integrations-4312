...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'nttdata_ot_hours_source_file_last3months_report_to_sftp_master_{config.instance}',
        description=f'nttdata_ot_hours_source_file_last3months_report_to_sftp_master {config.instance}',
        ...
        upload_reportdata_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.filepath + "OT Hours Source File_"
            '{{ current_time_in_specified_tz("America/New_York","%Y_%m_%d_%H_%M") }}' + ".csv"
        )
        upload_reportdata_to_sftp1 = rail.SFTPUploadFileOperator(
            task_id='upload_reportdata_to_sftp1',
            content="{{ result('load_report_data') }}",
            sftp_conn_id=config.sftp_conn_id,
            remote_filepath=config.filepath1 + "OT Hours Source File_"
            '{{ current_time_in_specified_tz("America/New_York","%Y_%m_%d_%H_%M") }}' + ".csv"
        )
        ...
        log_to_sumo = rail.DagRunLogToSumoOperator(
            ...
            extra_info={
                "exportfile_name": config.filepath + "OT Hours Source File_"
                '{{ current_time_in_specified_tz("America/New_York","%Y_%m_%d_%H_%M") }}' + ".csv"
            }
        )
