...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'nttdata_replicon_pto_for_envision_report_to_sftp_master_{config.instance}',
        description=f'nttdata_replicon_pto_for_envision_report_to_sftp_master {config.instance}',
        company_key=config.company_key,
        start_date=datetime(2023, 1, 1, tz=config.central_timezone),
        ...
        upload_reportdata_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.filepath + "Replicon_Time_SAP_"
            '{{ current_time_in_specified_tz("America/Chicago","%Y_%m_%d_%H_%M") }}' + ".csv"
        )
        upload_reportdata_to_sftp1 = rail.SFTPUploadFileOperator(
            task_id='upload_reportdata_to_sftp1',
            content="{{ result('load_report_data') }}",
            sftp_conn_id=config.sftp_conn_id,
            remote_filepath=config.filepath1 + "Replicon_Time_SAP_"
            '{{ current_time_in_specified_tz("America/Chicago","%Y_%m_%d_%H_%M") }}' + ".csv"
        )
        ...
        log_to_sumo = rail.DagRunLogToSumoOperator(
            ...
            extra_info={
                "exportfile_name": config.filepath + "Replicon_Time_SAP_"
                '{{ current_time_in_specified_tz("America/Chicago","%Y_%m_%d_%H_%M") }}' + ".csv"
            }
        )
        ...
        run_report_group_exit >> is_report_failed >> rail.Label('No') >> report_has_data >> rail.Label(
            'Yes') >> load_report_data >> upload_reportdata_to_sftp >> upload_reportdata_to_sftp1 >> log_to_sumo
