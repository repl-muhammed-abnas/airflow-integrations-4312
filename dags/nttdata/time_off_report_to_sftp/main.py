...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'nttdata_time_off_report_to_sftp_master_{config.instance}',
        description=f'nttdata_time_off_report_to_sftpp_master {config.instance}',
        ...
        upload_reportdata_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.filepath + "REPLICON_PTO_CLARITY"
            '{{ current_time_in_specified_tz("America/New_York","%m%d%Y%H%M") }}' + ".csv"
        )
        ...
        log_to_sumo = rail.DagRunLogToSumoOperator(
            ...
            extra_info={
                "exportfile_name": config.filepath + "Payroll_Hours_This_Month"
