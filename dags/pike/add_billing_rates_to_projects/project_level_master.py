...
from pike.add_billing_rates_to_projects.utils import custom_methods
import rail
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'pike_adding_billing_rates_to_projects_master_{config.instance}',
        description=f'Pike Adding Billing Rates to Projects Master {config.instance}',
        ...
        has_billing_rates_data = rail.IfOperator(
            ...
            yes_task='get_enabled_billing_rates',
            no_task='finish'
        )

        get_enabled_billing_rates = rail.RepliconServiceOperator(
            task_id="get_enabled_billing_rates",
        ...
        process_billing_rate = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'pike_adding_billing_rates_to_project_child_{config.instance}',
        ...
        render_logs_csv = rail.WriteCSVFileOperator(
            ...
            header=["ProjectName", "BillingRateName", "Results"],
            row=['{{ item.properties.project_name }}', '{{ item.properties.billing_rate_name }}', '{{ item.properties.status }}']
        )
        ...
        send_complete_email = rail.EmailOperator(
            ...
            to='{{ result("get_email_from_file") }}',
            bcc=config.internal_logs_email,
            subject="{{ get_company_key() }} | Request to add billing rate to Project - Completed",
        ...
        has_billing_rates_data >> rail.Label("Yes") >> get_enabled_billing_rates >> process_billing_rate \
