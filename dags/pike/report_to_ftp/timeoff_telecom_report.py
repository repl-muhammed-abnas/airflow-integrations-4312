...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"pike_timeoff_telecom_report_export_master_dag_{config.instance}",
        description=f"PIKE Timeoff Telecom Report Export Master Dag {config.instance}",
        ...
        upload_csv_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.timeoff_export_path +
                "Timeoff For Telecom - Telecom-{{current_time_in_specified_tz('Canada/Eastern', '%m_%d_%YT%H_%M')}}.csv",
        )
