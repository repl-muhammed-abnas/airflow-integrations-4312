...
from evenicacorp.invoice_export.request_payload import get_queuedforsync_invoice
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"{config.company_key}_quickbooks_online_invoice_export_{config.instance}",
        ...
        get_queued_for_sync_invoice = rail.RepliconServicePageOperator(
            ...
            all_result_data_handler=filter_data_new if config.instance == 'trial' else filter_data
        )
        ...
        trigger_invoice_child_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f"{config.company_key}_quickbooks_online_invoice_export_child_dag_{config.instance}",
        ...
        log_dagrun_details_to_table = rail.PostDagRunDetailsToRepliconOperator(
            ...
            integration_type=f'invoice{config.company_key}_quickbooks_online_invoice_export_{config.instance}_export'
        )
