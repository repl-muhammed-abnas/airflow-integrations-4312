...
from evenicacorp.invoice_export import request_payload
null = None
    ...
    with rail.create_airflow_dag(
        ...
        search_invoice_by_invoicenumber = rail.InternalQuickbooksAPIOperator(
            ...
            query_params=lambda dag_run: {
                'query': "SELECT * FROM Invoice WHERE DocNumber = '" + escape_single_quotes(dag_run.conf['invoice_number']) + "'"
            }
        )
        ...
        search_custom_feilds = rail.InternalQuickbooksAPIOperator(
            task_id='search_custom_feilds',
            request_method='GET',
            endpoint="/query",
            intuit_conn_id='{{ dag_run.conf.quickbooks_conn_id }}',
            query_params=lambda: {
        ...
        get_customfield_data_values_qbo = rail.PythonOperator(
            task_id = "get_customfield_data_values_qbo",
            python_callable=request_payload.get_customefield_data_val_qbo
        )

        def get_invoice_items(response):
            if not response:
                return []
            return list(filter(
                lambda item: item.get('billingType') != 'urn:replicon:invoice-item-type:tax',
                response
            ))
        ...
        get_invoice_items = rail.RepliconServiceOperator(
            ...
            replicon_conn_id='{{ dag_run.conf.replicon_conn_id }}',
            data_handler= get_invoice_items,
        )
        ...
        get_project_details = rail.RepliconServiceOperator(
            ...
            data={
                "projects": [
                    {
                        "name": "{{ result('get_invoice_items')[0].project.name }}",
                    }
                ]
            },
        ...
        get_project_oef_feild_values = rail.PythonOperator(
            task_id = "get_project_oef_feild_values",
            python_callable=request_payload.get_project_oef_values
        )

        get_product_service_qbo_and_polaris = rail.PythonOperator(
            task_id = "get_product_service_qbo_and_polaris",
            python_callable=request_payload.check_product_service_present_in_qbo
        )

        check_product_service_present_qbo = rail.IfOperator(
            task_id='check_product_service_present_qbo',
            test="{{ result('get_product_service_qbo_and_polaris') | is_truthy}}",
            yes_task='create_invoice_with_multiline_item',
            no_task='send_product_service_absent_exception_mail'
        )

        send_product_service_absent_exception_mail = rail.EmailOperator(
            task_id='send_product_service_absent_exception_mail',
            to='{{ dag_run.conf.notification_email }}',
            subject='{{ dag_run.conf.company_key }} | Replicon Invoice not moved to QBO. Invoice: {{ dag_run.conf.invoice_number }}',
            html_content="invoice_present.html"
        )
        ...
        search_items_qbo >> search_taxcodes_qbo >> search_custom_feilds >> get_customfield_data_values_qbo >> get_invoice_items >> \
            get_project_details >> get_project_oef_feild_values >> get_product_service_qbo_and_polaris >> check_product_service_present_qbo
        
        check_product_service_present_qbo >> rail.Label(
            'Yes') >> create_invoice_with_multiline_item >> update_invoice_sync_status >> update_invoice_external_system_number >> update_invoice_sync_note
        check_product_service_present_qbo >> rail.Label(
            'No') >>  send_product_service_absent_exception_mail >> rail.Label('On Error') >> catch_invoice_error
