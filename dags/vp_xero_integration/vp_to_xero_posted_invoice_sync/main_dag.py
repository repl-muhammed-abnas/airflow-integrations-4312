"""
Main (scheduler) DAG for VP PSA -> Xero Posted Invoices Sync.
...
trigger recipe's tenant fan-out stage.
"""
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'vp_xero_posted_invoice_sync_main_{config.instance}',
        description='Scheduler DAG for VP PSA -> Xero Posted Invoices Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=Variable.get(
            f'vp_xero_posted_invoice_sync_schedule_interval_{config.instance}',
            default_var='*/5 * * * *'
        ),
        max_active_runs=config.max_active_runs,
        tags=['vantagepoint_xero', 'posted_invoices', 'main'],
        default_args={
        ...
        fetch_customers_by_integration = rail.SimpleHttpOperator(
            ...
            data={
                ...
                'integration_type': 'posted_invoice_sync',
                'status': 'enabled',
            },
        ...
        process_customers = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'vp_xero_posted_invoice_sync_dispatcher_{config.instance}',
            conf=lambda item: {
                **item,
