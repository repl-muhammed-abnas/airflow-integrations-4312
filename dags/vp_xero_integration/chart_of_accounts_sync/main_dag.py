"""
Main DAG for Xero -> VP Chart of Accounts Sync.

Scheduled cron entry point. Fetches enabled tenants from the middleware and
triggers the dispatcher DAG per tenant. Mirrors the sibling
`vp_xero_mapping_sync` / QBO `chart_of_accounts_sync` main DAGs, targeting the
Xero-source / VP-sink direction for the chart of accounts (GL accounts),
...
def create_dag(config):
    """Create main scheduler DAG for Xero -> VP Chart of Accounts Sync."""
    with rail.create_airflow_dag(
        dag_id=f'vp_xero_chart_of_accounts_sync_main_{config.instance}',
        description='Scheduler DAG for Xero -> VP Chart of Accounts Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=Variable.get(
            f'vp_xero_chart_of_accounts_sync_schedule_interval_{config.instance}',
        ...
        fetch_customers_by_integration = rail.SimpleHttpOperator(
            ...
            data={
                'dag_id': (
                    f'vp_xero_chart_of_accounts_sync_dispatcher_{config.instance}'
                ),
