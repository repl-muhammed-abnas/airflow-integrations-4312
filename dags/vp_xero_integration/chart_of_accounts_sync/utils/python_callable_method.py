...
def _extract_xero_records(rail_result):
    ...
    Handles the three shapes the Xero operators emit:
      ...
      - {'<Resource>': [...]} (raw XeroAPIOperator envelope, e.g. {'Accounts': [...]})
    Mirrors mapping_sync's `_extract_xero_records`.
    """
    ...
    if isinstance(rail_result, dict):
        if rail_result.get('success') is False:
            logger.warning("Xero query failed: %s", rail_result.get('error'))
            return []
        ...
        accounts = rail_result.get('Accounts')
...
def _account_insert(values, context=None):
    ...
    collection_update(
        MAP_CHART_OF_ACCOUNTS_TABLE_NAME,
        f"INSERT INTO {MAP_CHART_OF_ACCOUNTS_TABLE_NAME} ({columns}) "
...
def decide_action_method():
    ...
    if not mapped_vp_code:
        if code_match:
            ...
            if vp_account == xero_code and vp_name == xero_name:
                decision.update(
                    action='link',
                    vp_code=vp_account,
                    vp_name=vp_name,
    ...
    if not mapped_xero_id:
        ...
        if code_match and _s(code_match.get('Name')) == xero_name:
            decision.update(
                action='backfill',
