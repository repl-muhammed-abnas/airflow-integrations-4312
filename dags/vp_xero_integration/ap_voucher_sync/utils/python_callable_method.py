# dags/vp_xero_integration/ap_voucher_sync/utils/python_callable_method.py
...
Ports the Workato AP Voucher for Xero bundle (014_501 PSA recipes):
  ...
  - dispatcher:       voucher identity grouping, disabled-flag check
...
Deviations from Workato:
  - No VP /firm search step: map_firm is keyed by FirmID and
    PSALedger.Vendor is assumed to equal the VP FirmID directly.
...
def _compute_due_date(date_str):
    ...
    from vp_xero_integration.ap_voucher_sync.config import default_payment_period_days
    if not date_str:
        return ''
    try:
...
# ---------------------------------------------------------------------------
# Dispatcher: disabled-flag check
# ---------------------------------------------------------------------------
def is_integration_enabled_method(instance):
    """IfOperator test: is the AP Voucher Xero integration enabled?

    Reads Airflow Variable CFG_DisableApVoucherXeroIntegration_{instance}.
    Returns True (enabled) when the Variable is absent or not 'true'.
    """
    from airflow.models import Variable
    value = Variable.get(
        f'CFG_DisableApVoucherXeroIntegration_{instance}',
        default_var='false',
    )
...
def load_lookup_tables_method():
    ...
    Returns:
        ...
        firm_map:    {FirmID -> ContactID (Xero contact UUID string)}
...
def enrich_lines_method():
    ...
    for line in lines:
        ...
        xero_tax_type = tax_map.get(tax_code, 'NONE') if tax_code else 'NONE'
        enriched.append({
            **line,
...
def validate_enriched_lines_method():
    ...
    for line in enriched:
        ...
        if tax_code and line.get('_XeroTaxType') == 'NONE':
            errors.append(
