# dags/vp_xero_integration/ap_voucher_sync/main_dag.py
"""Main DAG for VP -> Xero AP Voucher Sync.

Scheduled cron entry point. Fetches enabled tenants from the middleware
(integration_type='ap_voucher_xero_sync') and triggers one dispatcher run
per tenant. Mirrors vp_quickbooks_integration/ap_voucher/main_dag.py.
"""
...
def create_dag(config):
    """Create main scheduler DAG for VP -> Xero AP Voucher Sync."""
    with rail.create_airflow_dag(
        dag_id=f'vp_xero_ap_voucher_sync_main_{config.instance}',
        description='Scheduler DAG for VP -> Xero AP Voucher Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=Variable.get(
            f'vp_xero_ap_voucher_sync_schedule_interval_{config.instance}',
            '*/5 * * * *',
        ),
        ...
        fetch_customers_by_integration = rail.SimpleHttpOperator(
            ...
            data={
                'dag_id': (
                    f'vp_xero_ap_voucher_sync_dispatcher_{config.instance}'
                ),
                'integration_type': 'ap_voucher_xero_sync',
                'status': 'enabled',
            },
