...
def update_last_sync_time_method(instance):
    """Advance the watermark Variable; guard against disabled flag."""
    try:
...
def is_integration_enabled_method(instance):
    """True when CFG_DisablePollContactUpdatesXeroIntegration_{instance} != 'true'."""
    flag = Variable.get(
        f'CFG_DisablePollContactUpdatesXeroIntegration_{instance}',
        default_var='false',
    )
    enabled = str(flag).strip().lower() != 'true'
    if not enabled:
        logger.info(
            "Poll contact updates integration disabled for instance '%s'", instance
        )
    return enabled
...
def sync_single_xero_firm_to_vp(instance):  # pylint: disable=too-many-locals,unused-argument
    ...
    from vp_xero_integration.mapping_sync.utils._firm_sync import (  # pylint: disable=import-outside-toplevel
    ...
    from vp_xero_integration.mapping_sync.utils._shared import (  # pylint: disable=import-outside-toplevel
        _extract_xero_records,
        _extract_vp_client_id,
    )
    from vp_xero_integration.common.python_callable_method import unwrap_vp_response  # pylint: disable=import-outside-toplevel
    from vp_xero_integration.mapping_sync.config import IntegrationConfig  # pylint: disable=import-outside-toplevel
