"""
Main DAG for VP Xero Mapping Sync.

Scheduled per-instance. Fetches the list of enabled mapping_sync customers from
the middleware, then triggers the dispatcher DAG once per customer. Mirrors the
QuickBooks mapping_sync/main_dag.py exactly so the operational shape of the two
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        description='Processor DAG for VP Xero Mapping Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=Variable.get(
            f'vp_xero_mapping_sync_schedule_interval_{config.instance}',
            config.mapping_population_schedule,  # per-instance default
        ),
        max_active_runs=config.max_active_runs_master,
        tags=['vantagepoint_xero', 'mapping_sync', 'main'],
        default_args={
        ...
        get_middleware_auth_token = rail.SimpleHttpOperator(
            ...
            data={
                ...
                # environments where these Variables aren't seeded. Real
                # deployments seed both vantagepoint_client_id and
                # vantagepoint_client_secret at provisioning time, so the
                # default never gets used in production. Without default_var,
                # parse-time Variable.get raises KeyError and the whole DAG file
                # fails to import.
