...
from vp_xero_integration.common.python_callable_method import unwrap_vp_response
...
def _load_vp_firms_by_name(vp_conn_id, context):
    ...
    for record in records:
        ...
        if existing is None or str(client_id) < str(existing['ClientID']):
            index[name] = {'ClientID': str(client_id), 'Name': name}
    return index
...
def sync_xero_firms_to_vp(instance):  # pylint: disable=too-many-locals,too-many-statements
    ...
    if not contacts:
        raise RuntimeError(
            "Xero returned 0 contacts. Verify the Xero connection points to "
            "the correct tenant. map_firm step left incomplete — will retry on "
            "next run."
        )
