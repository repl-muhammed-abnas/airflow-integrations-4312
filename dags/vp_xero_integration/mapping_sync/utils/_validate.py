...
def _live_id_set(task_id, key):
    """Build a set of str(record[key]) from a fetched source-task result."""
    return {
        str(r.get(key))
        for r in _extract_xero_records(rail.result(task_id))
...
def run_all_mapping_validations():
    ...
    live_vp_firm_ids = _live_id_set('validate_fetch_vp_firms', 'ClientID')
    live_vp_account_codes = _live_id_set('validate_fetch_vp_accounts', 'Account')
    live_xero_account_ids = _live_id_set('validate_fetch_xero_accounts', 'AccountID')
    live_vp_tax_codes = _live_id_set('validate_fetch_vp_tax_codes', 'Code')
    live_xero_components = {
