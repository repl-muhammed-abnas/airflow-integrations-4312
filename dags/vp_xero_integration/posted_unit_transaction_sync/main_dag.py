"""
Main (scheduler) DAG for VP -> Xero Posted Unit Transaction Sync.

Cron-driven. Pulls enabled tenants from the integrations-platform-api
middleware and triggers one dispatcher DAG run per tenant. Mirrors
vp_quickbooks_integration/unit_transaction_sync/main_dag.py.
"""
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'vp_xero_unit_transaction_sync_main_{config.instance}',
        description='Scheduler DAG for VP -> Xero Posted Unit Transaction Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=Variable.get(
            f'vp_xero_unit_transaction_sync_schedule_interval_'
            f'{config.instance}',
            default_schedule_interval,
        ),
        max_active_runs=config.max_active_runs,
        tags=[
            'vantagepoint_xero', 'unit_transaction_sync', 'main'
        ],
        default_args={
        ...
        fetch_customers_by_integration = rail.SimpleHttpOperator(
            ...
            data={
                'dag_id': (
                    f'vp_xero_unit_transaction_sync_main_{config.instance}'
                ),
                'integration_type': 'unit_transaction_xero_sync',
                'status': 'enabled'
            },
