...
import rail
from horizonmedia.timesheet_autosubmission.util import data_formatting, request_payload
from horizonmedia.timesheet_autosubmission.send_logs import get_send_logs
from pendulum import datetime as dt

def create_dag(config):
    # pylint: disable=too-many-statements,unnecessary-lambda
    with rail.create_airflow_dag(
        dag_id=f'horizonmedia_timesheet_autosubmission_{config.instance}',
        ...
        get_tsauto_submission_report_columns = 'Timesheet Period,User Name,First Day of Leave,Actual last day of Leave,Approval Status,TimesheetPeriodUri,Worker Status,Timesheet End Date,Total Hrs (In Period),validation message,Duedate,DayDiff'
        ...
        get_tsauto_submission_report_data_collection = rail.CreateCollectionOperator(
            ...
            columns={
                ...
                "Total Hrs (In Period)":"Total_Hrs_In_Period",
        ...
        query_valid_ts = rail.QueryCollectionOperator(
            ...
            query='SELECT * FROM "timesheet_report" WHERE NULLIF("First_Day_of_Leave","") is NOT NULL AND "DayDiff" < 0 AND ("Duedate" > "First_Day_of_Leave" OR "Duedate"="First_Day_of_Leave") AND ("Duedate" < "Actual_last_day_of_Leave" OR "Duedate" = "Actual_last_day_of_Leave")'
        )
        ...
        submit_ts = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f"horizonmedia_timesheet_autosubmission_submit_timesheets_child_dag_{config.instance}",
