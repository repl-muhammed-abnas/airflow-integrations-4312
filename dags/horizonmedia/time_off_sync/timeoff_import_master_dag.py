...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'horizonmediatimeoff_importmaster{dag_id_postfix}',
        ...
        default_args={
            'rep_sftp_conn_id': config.rep_sftp_conn_id,
        ...
        process_entry_child = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'horizonmedia_timeoff_importchild{dag_id_postfix}',
        ...
        upload_to_cshare = rail.SFTPUploadFileOperator(
            task_id='upload_to_cshare',
            content="{{ result('create_log_csv') }}",
            sftp_conn_id=config.rep_sftp_conn_id,
            remote_filepath=config.replicon_logs_file_path +
            "/Log_{{ dag_run_ecid() }}_Time Off Sync.csv",
        )
        ...
        create_log_csv >> write_updated_reference_file >> upload_to_sftp >> upload_to_cshare >> gather_child_error_data >> if_any_failure
