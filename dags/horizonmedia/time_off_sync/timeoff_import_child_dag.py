...
import uuid
import rail
from horizonmedia.time_off_sync.utils import request_payload
from horizonmedia.time_off_sync.utils import formatted_data
...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'horizonmedia_timeoff_importchild{dag_id_postfix}',
        ...
        Put_TimeOff_for_user = rail.RepliconServiceOperator(
            ...
            data={
                "timeOff": {
                    "target": {
                        "uri": "{{ result('CreateDraft_timeoffbooking_for_user') }}",
                    },
                    "owner": {
        ...
        timeoff_booking_successful_logs_entry = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                "md5": "{{ dag_run.conf.md5 }}",
                "status": "success",
                "details": "The time off is added successfully."
            }
        )
        ...
        timeoff_uri_present_for_add_update >> rail.Label(
            'Yes') >> get_timeoff_hours_list >> timeoff_hours_sum_equals_or_greater_then_8
        timeoff_uri_present_for_add_update >> rail.Label(
        ...
        timeoff_hours_sum_equals_or_greater_then_8 >> rail.Label(
            'No') >> CreateDraft_timeoffbooking_for_user
        CreateDraft_timeoffbooking_for_user >> Put_TimeOff_for_user >> Publish_TimeOffDraft_for_user >> Put_TimeOff_ExtensionFieldValues_for_user >> Approve_timeoffbooking_for_user >> timeoff_booking_successful_logs_entry >> catch_and_log_errors >> finish
