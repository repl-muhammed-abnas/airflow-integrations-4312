...
from solaris.report_to_sftp.utils.custom_methods import logging_details


def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'solaris_report_to_sftp_master_{config.instance}',
        description='SolarisMCI Report To Sftp',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=datetime(2022, 4, 1, tz=config.pst_time_zone),
        ...
        get_logging_details = rail.PythonOperator(
            ...
            op_args=[config.pst_time_zone]
        )

        get_raw_data_report_details = rail.RepliconReportDetailsOperator(
            task_id='get_raw_data_report_details',
            report_name=config.extract_report_name_raw_data,
        )
        ...
        report_group_entry, report_group_exit = rail.run_report(
            ...
            report_params={
                "reportParameters": [
                    {
                        "reportUri": "{{ result('get_raw_data_report_details').uri }}",
                        "filterValues": [],
        ...
        upload_report_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath='/Raw Data Export_{{ result("get_logging_details")["dag_start_time"] }}.csv',
        )
        ...
        log_to_sumo = rail.DagRunLogToSumoOperator(
            ...
            extra_info={
                'filename': '/Raw Data Export_{{ result("get_logging_details")["dag_start_time"] }}.csv'
            }
        )

        get_logging_details >> get_raw_data_report_details >> report_group_entry
