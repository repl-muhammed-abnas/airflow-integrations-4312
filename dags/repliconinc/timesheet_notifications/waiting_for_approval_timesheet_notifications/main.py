...
from repliconinc.timesheet_notifications.waiting_for_approval_timesheet_notifications.utils.custom_methods import get_timesheet_report_payload

def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.main_dag_id,
        description='Inhouse - Waiting On Approval Timesheet Notifications',
        ...
        generate_timesheet_report_details = rail.RepliconServiceOperator(
            ...
            data=lambda: get_timesheet_report_payload(
                config.waiting_for_approval_timesheets_report_uri,
        ...
        load_timesheet_report_data = rail.LoadCSVFileOperator(
            ...
            headers=['timesheet_period', 'user', 'waiting_on_approver']
        )
        ...
        def send_standard_response_callable():
            subject = """Action Required: """ + config.company_identifier + """ | Waiting On Approver Timesheets - {{ current_time_in_specified_tz() }}"""
