...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"standard_jira_{config.region.replace('-', '_')}_create_task_{config.instance}",
        description=f'Jira {config.region} Create Task {config.instance}',
        ...
        def get_issues_details(response):
            issues = list(filter(lambda x: x['status'].lower() != "done", list(map(lambda item: {
                ...
                "created": item['fields']['created'].split('T')[0],
        ...
        def get_request_body():
            if rail.result('get_lastsync_time')['last_synctime'] == '1970-01-01 00:00':
                return {
                    ...
                    "fieldsByKeys": "false",
                    "jql": "status != Done AND created >= -60m ORDER BY updated DESC",
                    "maxResults": 15,
                    "startAt": 0
                }
            datetime_str = rail.result("get_server_time")
            base_jql = "status != Done AND created >= {} ORDER BY updated DESC"
        ...
        jira_new_issues = rail.JiraAPIOperator(
            task_id='jira_new_issues',
            request_method='POST',
        ...
        get_is_legacy_department_enabled = rail.RepliconServiceOperator(
            task_id='get_is_legacy_department_enabled',
            replicon_conn_id='{{ dag_run.conf.replicon_conn_id }}',
            endpoint="/services/DepartmentService1.svc/GetIsLegacyDepartmentEnabled",
            data=None
        )
        ...
        trigger_create_task_child_dag = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_create_task_child_dag',
            items=lambda: rail.result('jira_new_issues'),
            retries=0,
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f"standard_jira_{config.region.replace('-', '_')}_create_task_child_dag_{config.instance}",
            conf=lambda item, dag_run: {
                **{
                    'legacy_department': rail.result('get_is_legacy_department_enabled'),
        ...
        wait_for_create_task_child_dag = rail.WaitForDagRunsSensor(
            task_id='wait_for_create_task_child_dag',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs='{{ result("trigger_create_task_child_dag") }}'
        )

        gather_create_task_error = rail.GatherResultsFromDagRunsOperator(
            task_id='gather_create_task_error',
            dag_runs="{{ result('trigger_create_task_child_dag') }}",
            dagrun_task_id='catch_create_task_error',
            flatten=True
        )

        is_create_task_error = rail.IfOperator(
            task_id='is_create_task_error',
            # pylint: disable=line-too-long
            test="{{ (get_task_state('gather_create_task_error') == 'success' and result('gather_create_task_error') | length > 0)}}",
            yes_task='fail_create_task_error',
            no_task='should_log_history'
        )

        fail_create_task_error = rail.FailOperator(
            task_id='fail_create_task_error',
            message="{{ result('gather_create_task_error') | map_to_attr('error') | join('|') }}"
        )

        should_log_history = rail.IfOperator(
            task_id='should_log_history',
            test="{{ not(get_task_state('jira_new_issues') == 'success' and \
                result('jira_new_issues') | length == 0 )}}",
        ...
        can_run_batch_task >> rail.Label(
            'No') >> get_lastsync_time >> get_current_user >> get_server_time >> jira_new_issues
        jira_new_issues >> update_lastsync_time >> get_is_legacy_department_enabled \
            >> get_my_actual_useridentity >> is_polaris_permissions_present \
            >> trigger_create_task_child_dag >> wait_for_create_task_child_dag \
            >> gather_create_task_error >> is_create_task_error
        is_create_task_error >> rail.Label(
            'Yes') >> fail_create_task_error >> should_log_history
        is_create_task_error >> rail.Label(
