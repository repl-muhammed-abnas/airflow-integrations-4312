...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.child_user_sick_pay_h_policy_update_dag_id,
        description=f'Assured Partners User Import User Sick Pay-H policy update Child {config.instance}',
        ...
        log_default_policy_set_modified = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: get_default_policy_set_modified(rail.result(
        ...
        def get_final_policy_set(relevant_historical_policies, modified_default_policyset, starting_balance_script_uri):
            ...
            gsub_to_remove_starting_balance_script = json.dumps({"additionalParameters": [{"keyUri": "urn:replicon:script-key:parameter:amount", "value": {"number": 0.0}}, {"keyUri": "urn:replicon:script-key:parameter:precedence", "value": {
                                                                "number": 20.0}}], "scriptTarget": {"description": "Set initial balance for the first day of a policy", "name": "Starting Balance Set To", "uri": starting_balance_script_uri}})

            final_policyset = json.loads(json.dumps(modified_default_policyset, ensure_ascii=False).replace(
                gsub_to_remove_starting_balance_script, '').replace(", ]", "]").replace("[,", "[").replace(", ,", ","))
        ...
        log_final_policy_set = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: get_final_policy_set(rail.result(
                "log_relevant_historical_policies"), rail.result("log_default_policy_set_modified"), dag_run.conf['starting_balance_script_uri'])
        )

        assign_time_offpolicy_50 = rail.RepliconServiceOperator(
            task_id='assign_time_offpolicy_50',
        ...
        catch_and_log_error = rail.PythonOperator(
            ...
            python_callable=lambda: rail.render_template(
                "Error in Sick Pay-H policy update : {{get_error_message()}}")
        )
        ...
        if_request_timeoffuri_present_3 >> rail.Label('Yes') >> log_effective_date_derived >> get_defaultpolicyfromgloballevel_9 \
            >> get_user_time_off_type_policy_summary_10 >> log_relevant_historical_policies >> log_default_policy_set_modified \
            >> if_request_startdate_equals_to_previousstartdate_36
        ...
        log_final_policy_set >> assign_time_offpolicy_50 >> catch_and_log_error >> final_response_from_dag
