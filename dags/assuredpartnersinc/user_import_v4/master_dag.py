...
import itertools
import rail
from assuredpartnersinc.user_import_v4.utils import request_payload, python_callable
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        description=f'Assured Partners User Import Master {config.instance}',
        ...
        if_delta_greater_than_deltavariance_38 = rail.IfOperator(
            task_id='if_delta_greater_than_deltavariance_38',
            test=lambda: rail.result(
                "log_delta_list_size_to_percentage") > int(Variable.get(
                    config.deltavariance_percentage, default_var=11)),
            yes_task="upload_uploadchangefile_40",
            no_task="trigger_dag_run_assured_partners_child_groups_update_044",
        )

        upload_uploadchangefile_40 = rail.SFTPUploadFileOperator(
            task_id='upload_uploadchangefile_40',
            content='''{{ result('query_list_changed_items_27') }}''',
            remote_filepath=config.sftp_change_filepath +
            "/changefile_{{ dag_run_ecid() }}_{{ result('new_file_sensor') | file_name }}",
        )

        send_mail_too_many_deltas = rail.EmailOperator(
            task_id='send_mail_too_many_deltas',
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | User Import - URGENT- Census File Not Processed - '
            + '{{ current_time_in_specified_tz("' + config.time_zone + '", "%m/%d/%YT%H:%M:%S") }}',
            html_content='''templates/too_many_delta.html''',
            params={
                "changefile_path": config.sftp_change_filepath
            }
            
        )

        trigger_dag_run_assured_partners_child_groups_update_044 = rail.TriggerDagRunOperator(
            task_id='trigger_dag_run_assured_partners_child_groups_update_044',
            retries=0,
        ...
        wait_for_dag_run_assured_partners_child_groups_update = rail.WaitForDagRunsSensor(
            ...
            dag_runs='{{ result("trigger_dag_run_assured_partners_child_groups_update_044") }}'
        )
        ...
        log_skipped_records = rail.WriteLogOperator(
            ...
            items="{{ result('query_list_unchangeditems_28') }}",
            ...
            properties=lambda item: {
                ...
                "details": "No change found from previous input file",
        ...
        get_process_each_user_dag_ids = rail.PythonOperator(
            ...
            python_callable=lambda: list(itertools.chain(
                *list(map(lambda x: rail.result(
                    f'process_each_user_parallel_dagrun_{x+1}'), range(config.process_each_user_trigger_parallel_count_master))))),
        ...
        render_logs_csv = rail.WriteCSVFileOperator(
            ...
            footer=lambda: [
                ...
                'Number of skipped records: ' +
                    str(rail.result(
                        "update_user_logs_after_supervisor_assignment", "skipped_record_count")),
        ...
        download_reference_file >> parse_reference_file >> create_referencefile_collection \
            >> query_list_changed_items_27 >> query_list_invalid_delta_records >> query_list_validated_delta_records >> query_list_unchangeditems_28 >> if_delta_records_less_than_1

        if_delta_records_less_than_1 >> rail.Label(
            'No') >> log_delta_list_size_to_percentage >> if_delta_greater_than_deltavariance_38
        if_delta_records_less_than_1 >> rail.Label(
        ...
        if_delta_greater_than_deltavariance_38 >> rail.Label(
            'No') >> trigger_dag_run_assured_partners_child_groups_update_044 >> wait_for_dag_run_assured_partners_child_groups_update \
            >> get_departmentdata_50
        if_delta_greater_than_deltavariance_38 >> rail.Label(
            'Yes') >> upload_uploadchangefile_40 >> send_mail_too_many_deltas >> finish
        ...
        if_unchanged_records_greater_than_0_63 >> rail.Label(
            'Yes') >> log_skipped_records >> wait_for_dag_completion_variable
        ...
        if_entries_in_supervisor_assignment_logs >> rail.Label(
            'Yes') >> create_supervisor_user_temp_logs >> trigger_dag_run_assured_partners_child_add_supervisor_86 >> update_user_logs_after_supervisor_assignment >> foreach_dir_user_importreferencefolder_19_96
        if_entries_in_supervisor_assignment_logs >> rail.Label(
            'No') >> foreach_dir_user_importreferencefolder_19_96
