...
def create_modified_allocation_child_dags(config):
    _api_headers = {"Content-Type": "application/json", **({"X-RP-Database": config.rp_api_db_env} if getattr(config, 'rp_api_db_env', None) else {})}
    _dags = []
    for child_index in range(1, config.modified_child_count + 1):
        suffix = f"_{child_index}" if child_index > 1 else ""
        with create_airflow_dag(
            dag_id=f"{config.modified_allocation_child_dag_id}{suffix}",
            description=f"Processes modified task allocations (partition {child_index})",
            start_date=config.start_date,
            company_key=config.company_key,
            replicon_conn_id=config.replicon_conn_id,
            max_active_runs=config.modified_child_max_active_runs,
        ) as dag:
            ...
            can_run_batch_task = IfOperator(
                ...
                test=lambda: Variable.get(
                    "resource_planner_task_alloc_webhook_modified_enable_batch_task", "true"
                ).lower() == "true",
                yes_task="batch_task",
            ...
            # Payloads are static — compute at DAG parse time.
            ...
            # =================================================================
            # Phase 5: Expand, replace via API (atomic delete + insert)
            # =================================================================
            ...
            def prepare_write_payload(dag_run):
...
for_each_instance(create_modified_allocation_child_dags)
