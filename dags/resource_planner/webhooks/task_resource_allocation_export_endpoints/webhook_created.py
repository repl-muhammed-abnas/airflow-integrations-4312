...
def create_created_webhook_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.webhook_created_dag_id,
        description="Webhook receiver for ProjectPolarisTaskAllocationCreated events",
        start_date=datetime(2023, 9, 1),
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.master_max_active_run,
        webhook_conf=rail.WebhookConf(
            hmac_secret_var=config.webhook_created_bearer_token,
        ),
    ) as dag:
        ...
        trigger_new_allocation_dag = rail.TriggerDagRunOperator(
            task_id="trigger_new_allocation_dag",
            trigger_dag_id=config.new_allocation_dag_id,
            retries=0,
            conf=_build_processing_conf
        )

        trigger_new_allocation_dag

    return dag


rail.for_each_instance(create_created_webhook_dag)
