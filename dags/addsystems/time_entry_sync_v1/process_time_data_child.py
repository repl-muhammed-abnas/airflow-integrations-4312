import uuid
import rail
from addsystems.time_entry_sync_v1.utils import request_payload, response_filter
null = None
...
def create_child_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f"addsystems_time_data_process_each_time_record_child_{config.instance}_v1",
        ...
        is_timesheet_submitted_update = rail.IfOperator(
            task_id="is_timesheet_submitted_update",
            ...
            yes_task="log_timesheet_is_approved_submitted_update",
            no_task="check_nonbillable_and_billable_time_is_not_zero"
        )

        is_timesheet_submitted_create = rail.IfOperator(
            task_id="is_timesheet_submitted_create",
            test='''{{result('get_timsheet_for_date').timesheet.statusUri | ends_with('approved') or result('get_timsheet_for_date').timesheet.statusUri | ends_with('waiting') }}''',
            yes_task="log_timesheet_is_approved_submitted_create",
            no_task="check_nonbillable_and_billable_time"
        )

        log_timesheet_is_approved_submitted_create = rail.WriteLogOperator(
            task_id='log_timesheet_is_approved_submitted_create',
            message="Time entry is skipped since, the timesheet is Approved or Submitted.",
            severity='Exception',
            properties={
                'employeeid': "{{dag_run.conf.item.UserInitials}}",
                'entrydate': "{{dag_run.conf.item.EventAddDate}}",
        ...
        log_timesheet_is_approved_submitted_update = rail.WriteLogOperator(
            task_id='log_timesheet_is_approved_submitted_update',
            message="Time entry is skipped since, the timesheet is Approved or Submitted.",
            severity='Exception',
            properties={
                'employeeid': "{{dag_run.conf.item.UserInitials}}",
        ...
        is_time_entry_not_updated_manually_billable = rail.IfOperator(
            ...
            yes_task="update_time_entry_billable",
            no_task="time_entry_updated_success"
        )
        ...
        has_task_data >> rail.Label("Yes") >> get_timeentry_oef >> get_timsheet_for_date >> get_timeentry_revision_filters\
            ...
            "No") >> is_timesheet_submitted_create >> rail.Label("Yes") >> log_timesheet_is_approved_submitted_create >> catch_and_log_errors 

        is_timesheet_submitted_create >> rail.Label("No") >> check_nonbillable_and_billable_time >> rail.Label("No") >> create_time_entry >> [check_nonbillabletime, check_billabletime]


        is_timeentry_present >> rail.Label("Yes") >> get_time_entry_details >> get_time_entries_for_user_and_user\
            >> is_timesheet_submitted_update >> rail.Label("No") >> check_nonbillable_and_billable_time_is_not_zero >> rail.Label("No")\
        ...
        is_timesheet_submitted_update >> rail.Label("Yes") >> log_timesheet_is_approved_submitted_update >> catch_and_log_errors
        ...
        check_not_zero_nonbillabletime >> rail.Label("Yes") >> is_time_entry_not_updated_manually_non_billable\
            >> rail.Label("yes") >>  update_time_entry_nonbillable >> time_entry_updated_success >> catch_and_log_errors
        ...
        check_not_zero_billabletime >> rail.Label("Yes") >> is_time_entry_not_updated_manually_billable\
            >> rail.Label("Yes") >> update_time_entry_billable >> time_entry_updated_success >> catch_and_log_errors
