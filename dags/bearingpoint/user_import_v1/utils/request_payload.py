...
def get_timesheet_template_name(dag_run, location_wise_data_mapper):
    """
    Get the appropriate timesheet template name based on location and employee type.
    Version 1.6+ logic: Uses employee type code to determine internal vs external template.
    """
    location_code = dag_run.conf["location_code"]

    if location_code not in location_wise_data_mapper:
        return null

    location_data = location_wise_data_mapper[location_code]

    # V1.6 logic: Use employee type code to determine template
    if "timesheet_template_internal" in location_data and "timesheet_template_external" in location_data:
        employee_type_code = dag_run.conf.get("employee_type_code", "")
        external_codes = ["C", "8", "7", "9"]

        if employee_type_code in external_codes:
            return location_data["timesheet_template_external"]
        else:
            return location_data["timesheet_template_internal"]
    else:
        # Fallback for old mapper structure
        return location_data.get("timesheet_template", null)
    ...
    return {
        ...
        "modifications": {
            ...
            "timesheetTemplate": {
                "value": {
                    "uri": null,
                    "name": get_timesheet_template_name(dag_run, config.location_wise_data_mapper)
                }
            } if dag_run.conf["location_code"] in (config.location_wise_data_mapper).keys() and get_timesheet_template_name(dag_run, config.location_wise_data_mapper) else null,
            "timeoffTemplate": {
                "value": {
                    "uri": null,
                    "name": "Time Off"
...
def get_exception_logs(dag_run, location_wise_data_mapper):
    ...
    if dag_run.conf["location_code"] in location_wise_data_mapper.keys():
        location_template_name = get_timesheet_template_name(dag_run, location_wise_data_mapper)
...
def get_updated_timesheet_template(dag_run, location_wise_data_mapper):
    if not dag_run.conf.get("timesheet_template"):
        return null
    # Get the appropriate template name using the helper function (handles v1.6 logic)
    location_template_name = get_timesheet_template_name(dag_run, location_wise_data_mapper)
