...
import rail
from bearingpoint.project_import_v1.utils import request_payload,response_filter,custom_method
...
def create_child_dag_wbs(config):
    with rail.create_airflow_dag(
        ...
        is_manager_has_not_valid_permission = rail.IfOperator(
            ...
            test=lambda dag_run: dag_run.conf['manager_uri'] and not bool([name for name in dag_run.conf[
                'manager_permission_set'].split(",") if name in dag_run.conf['project_permission_set']]),
        ...
        is_project_exists = rail.IfOperator(
            ...
            test=lambda: request_payload.does_wbs_exist(),
            yes_task='is_restrict_time_posting_changed',
            no_task='is_restrict_time_posting_set_to_yes'
        )

        is_restrict_time_posting_changed = rail.IfOperator(
            task_id='is_restrict_time_posting_changed',
            test=lambda dag_run: dag_run.conf['RestrictTimePosting'] != rail.find_first_by_attr_and_get_attr(
                rail.result("get_project_details").get("customFields", {}),
                'customField.displayText', 'RestrictTimePosting', 'text'),
            yes_task='can_remove_project_team_members',
            no_task='is_restrict_time_posting_set_to_yes'
        )

        can_remove_project_team_members = rail.IfOperator(
            task_id='can_remove_project_team_members',
            test=lambda dag_run: dag_run.conf.get('RestrictTimePosting') == 'Y',
        ...
        get_project_team_members = rail.RepliconServiceOperator(
            ...
            data_handler=response_filter.get_project_team_member_uris
        )

        remove_all_users_from_project = rail.RepliconServiceOperator(
            task_id='remove_all_users_from_project',
            endpoint='/services/ProjectService1.svc/BulkUpdateProjectTeamMembersAssignment',
            data=lambda: {
                "projectUri": rail.result("create_or_update_project")['uri'],
                "resourceUri": [x['uri'] for x in rail.result("get_project_team_members") if x['name']],
        ...
        assign_all_users_in_project_team = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "projectUri": rail.result("create_or_update_project")['uri'],
                "resourceUri": [x['uri'] for x in dag_run.conf['employee_type_uris']],
                "projectTeamMemberAssignmentOptionUri": "urn:replicon:project-team-member-assignment-option:assign"
            }
        )

        assign_billing_rates_to_all_resources = rail.RepliconServiceCallForEachItemOperator(
            task_id="assign_billing_rates_to_all_resources",
            endpoint='/services/TimeAndMaterialsProjectService1.svc/PutProjectTeamMemberBillingRatesAllowedForBillingTime3',
            items= '{{ dag_run.conf.employee_type_uris | to_json }}',
        ...
        has_billing_rates_to_create >> rail.Label(
            "Yes") >> create_resource_billing_rates >> is_project_exists

        has_billing_rates_to_create >> rail.Label(
            "No") >> is_project_exists
        
        is_project_exists >> rail.Label(
            "Yes") >> is_restrict_time_posting_changed
        
        is_project_exists >> rail.Label(
            "No") >> is_restrict_time_posting_set_to_yes
        
        is_restrict_time_posting_changed >> rail.Label(
            "Yes") >> can_remove_project_team_members
        
        is_restrict_time_posting_changed >> rail.Label(
            "No") >> is_restrict_time_posting_set_to_yes
        
        can_remove_project_team_members >> rail.Label(
            "Yes") >> get_project_team_members >> remove_all_users_from_project >> is_restrict_time_posting_set_to_yes
