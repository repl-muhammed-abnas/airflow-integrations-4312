...
import rail
from bearingpoint.project_import_v1.utils import custom_method, request_payload, response_filter
...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        ...
        get_project_custom_field_details = rail.RepliconServiceOperator(
            task_id='get_project_custom_field_details',
            endpoint='/services/CustomFieldService1.svc/GetAllCustomFields',
            data={
                'objectUri': 'urn:replicon:object-type:project'
            },
            data_handler=lambda response: rail.find_first_by_attr_and_get_attr(response, 'displayText', 'RestrictTimePosting', 'uri')
        )
        ...
        get_all_employee_types = rail.RepliconServiceOperator(
            ...
            data_handler=response_filter.get_required_employee_types
        )

        get_project_permission_set = rail.RepliconServiceOperator(
            task_id="get_project_permission_set",
            endpoint="/services/PermissionSetListService1.svc/GetData",
            data= request_payload.get_permission_set_payload,
            data_handler= response_filter.get_permission_set_data_from_list_service
        )
        ...
        get_all_permission_sets >> get_all_project_oef_details >> get_project_custom_field_details >>\
        ...
        get_all_service_center_details >> get_all_cost_center_details >> get_all_billing_rates >> get_all_employee_types >>\
                get_project_permission_set >> process_projects >> wait_for_process_projects >> process_log_generation
