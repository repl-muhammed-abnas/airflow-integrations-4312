...
def create_projectorapply_modifications(dag_run):
    ...
    modifications = {
        ...
        "customFieldsToApply": [
            {
                "customField": {
                    "uri": dag_run.conf['restrict_time_posting_udf_uri'],
                },
                "text": dag_run.conf['RestrictTimePosting'],
            }
        ]
    }
...
def get_efforts_payload(effort_hours):
    hours = int((float(effort_hours)))
    minutes = int((float(effort_hours) - hours) * 60)
    seconds = int((((float(effort_hours) - hours) * 60) - minutes) * 60)
    ...
    return list(map(lambda task: {
        ...
        "taskModificationToApply": {
                ...
                "resourceAssignmentModifications": {
                    ...
                    } for x in dag_run.conf['employee_type_uris']] if (dag_run.conf['RestrictTimePosting'] == 'N' and task['work_package']) else [
                        {
                            "user": {
                                "uri": uri,
                            },
                ...
                "duration": get_efforts_payload(task['efforts'])
...
def get_put_task_data(item,dag_run):
    return {
        ...
        "task": {
            ...
            } for x in dag_run.conf['employee_type_uris']] if (dag_run.conf['RestrictTimePosting'] == 'N' and item['work_package']) else [
                {
                    "user": {
                        "uri": uri,
                    },
            ...
            "estimatedHours": get_efforts_payload(item['efforts']) if item['efforts'] else None,
        }
    }
...
def get_all_users_billing_rates_payload(item, dag_run):
    billing_rates = dag_run.conf.get('billing_rates', [])
...
def get_permission_set_payload():
    return {
        "page": "1",
        "pagesize": "1000",
        "columnUris": [
            "urn:replicon:permission-set-list-column:permission-set",
            "urn:replicon:permission-set-list-column:policy"
        ],
        "sort": [],
        "filterExpression": {
            "leftExpression": {
                "filterDefinitionUri": "urn:replicon:permission-set-list-filter:policy"
            },
            "operatorUri": "urn:replicon:filter-operator:in",
            "rightExpression": {
                "value": {
                    "uris": [
                    "urn:replicon:policy:project-management"
                    ]
                }
            }
        }
