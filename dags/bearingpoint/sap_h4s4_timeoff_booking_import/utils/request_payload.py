...
import rail
from bearingpoint.sap_h4s4_timeoff_booking_import.utils import custom_methods
...
def get_timeoff_bookings_for_min_max_dates(dag_run):
    get_dates_from_query = rail.load_all_records(
        rail.result("get_min_max_dates_from_valid_records"))[0]
    return {
        "page": "1",
        "pagesize": "10000000",
        "columnUris": [
        ...
        "filterExpression": {
            ...
            "rightExpression": {
                "value": {
                    "dateRange": {
                        "startDate": rail.parse_date(get_dates_from_query['start_date'], custom_methods.FEED_ENTRYDATE_DATE_FORMAT),
                        "endDate": rail.parse_date(get_dates_from_query['end_date'], custom_methods.FEED_ENTRYDATE_DATE_FORMAT),
                    }
                }
            }
        }
...
def get_all_timesheet_for_user(item):
    return {
        "page": "1",
        "pagesize": "10000",
        "columnUris": [
        ...
        "filterExpression": {
            "leftExpression": {
                ...
                "rightExpression": {
                    "value": {
                        "dateRange": {
                            "startDate": rail.parse_date(item['startdate'], custom_methods.FEED_ENTRYDATE_DATE_FORMAT),
                            "endDate": rail.parse_date(item['enddate'], custom_methods.FEED_ENTRYDATE_DATE_FORMAT)
                        }
                    }
                }
            },
    ...
    return {
        ...
        "modifications": {
            ...
            "multiDayUsingStartEndDate": {
                "value": {
                    "timeOffStart": {
                        "date": rail.parse_date(item['start_date'], "%d/%m/%Y"),
