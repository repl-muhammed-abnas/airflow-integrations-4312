...
def map_timesheet_with_user_data(user_data_task_id, ts_data_task_id):
    ...
    ts_data = [item for sublist in rail.result(
        ts_data_task_id) for item in sublist]
    ...
    def get_map_data(item):
        timesheet_data = get_timesheet_data(
            item['startdate'], item['enddate'], ts_data)
        return {
            **item,
            **timesheet_data
        }
...
def adjust_timeoff_records(dag_run):
    ...
    for record in rail.result("get_time_off_booking_details"):
