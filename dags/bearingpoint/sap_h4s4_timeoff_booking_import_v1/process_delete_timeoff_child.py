...
import rail
from bearingpoint.sap_h4s4_timeoff_booking_import_v1.utils import request_payload, response_filter
    ...
    with rail.create_airflow_dag(
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            test=lambda: Variable.get(
                config.can_run_batch_task, default_var='true').lower() == 'true1',
            yes_task='batch_task',
            no_task='get_all_timeoff_bookings_within_window'
        )
        ...
        batch_task = rail.BatchTaskRunOperator(
            ...
            start_task='get_all_timeoff_bookings_within_window',
            end_task='catch_and_log_errors',
        )

        get_all_timeoff_bookings_within_window = rail.RepliconServiceOperator(
            task_id='get_all_timeoff_bookings_within_window',
            endpoint='/services/TimeOffListService1.svc/GetData',
            data=request_payload.get_all_timeoffs_within_3month_window,
            data_handler=response_filter.get_timeoff_booking_details
        )

        has_any_timeoff_within_window = rail.IfOperator(
            task_id='has_any_timeoff_within_window',
            test='{{ result("get_all_timeoff_bookings_within_window") | is_truthy }}',
        ...
        create_time_off_collection = rail.CreateCollectionOperator(
            ...
            source='{{ result("get_all_timeoff_bookings_within_window") | to_json }}',
            name='timeoffdata'
        )
        ...
        can_run_batch_task >> rail.Label(
            "No") >> get_all_timeoff_bookings_within_window >> has_any_timeoff_within_window

        has_any_timeoff_within_window >> rail.Label(
