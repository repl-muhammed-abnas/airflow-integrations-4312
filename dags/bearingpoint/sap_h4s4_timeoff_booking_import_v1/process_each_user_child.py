...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        check_if_auto_approval_needed = rail.IfOperator(
            task_id='check_if_auto_approval_needed',
            test=lambda: custom_methods.check_if_any_record_needs_auto_approval('query_valid_date_records'),
            yes_task='get_timesheets_before_window',
            no_task='is_any_timesheet_reopened'
        )

        get_timesheets_before_window = rail.RepliconServiceOperator(
            task_id='get_timesheets_before_window',
            endpoint='/services/TimesheetListService1.svc/GetData',
            data=request_payload.get_timesheets_before_window,
            data_handler=response_filter.get_timesheet_details_to_approve
        )

        has_timesheets_before_window = rail.IfOperator(
            task_id='has_timesheets_before_window',
            test='{{ result("get_timesheets_before_window") | is_truthy }}',
            yes_task='filter_timesheets_to_approve',
            no_task='is_any_timesheet_reopened'
        )

        filter_timesheets_to_approve = rail.PythonOperator(
            task_id='filter_timesheets_to_approve',
            python_callable=lambda: custom_methods.filter_timesheets_needing_approval(
                rail.result('get_timesheets_before_window')
            )
        )

        has_timesheets_to_approve = rail.IfOperator(
            task_id='has_timesheets_to_approve',
            test='{{ result("filter_timesheets_to_approve") | is_truthy }}',
        ...
        force_approve_timesheets = rail.RepliconServiceCallForEachItemOperator(
            ...
            items='{{ result("filter_timesheets_to_approve") | to_json }}',
            endpoint='/services/TimesheetApprovalService1.svc/ForceApprove',
            data={
                "timesheetUri": "{{ item.ts_uri }}",
                "unitOfWorkId": "{{ item.unit_of_work_id }}",
                "comments": "Forced Approved by Integration (Time Off Booking Import - Outside 3-Month Window)"
            }
        )
        ...
        check_if_timeoff_template_assigned >> rail.Label(
            "Yes") >> get_user_timesheet_details >> map_timesheet_with_user_data >> reopen_timesheets >> get_all_assigned_time_off_type_for_user >> get_all_timeoff_description_details >> process_each_timeoff_per_user >> wait_for_process_each_timeoff >> check_if_auto_approval_needed

        check_if_auto_approval_needed >> rail.Label(
            "Yes") >> get_timesheets_before_window >> has_timesheets_before_window

        has_timesheets_before_window >> rail.Label(
            "Yes") >> filter_timesheets_to_approve >> has_timesheets_to_approve

        has_timesheets_to_approve >> rail.Label(
            "Yes") >> force_approve_timesheets >> is_any_timesheet_reopened

        has_timesheets_to_approve >> rail.Label(
            "No") >> is_any_timesheet_reopened
