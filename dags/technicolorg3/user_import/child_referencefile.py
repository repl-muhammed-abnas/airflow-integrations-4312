...
def create_referencefile_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'technicolorg3_user_import_child_referencefile_{config.instance}',
        description=f'Technicolor Child Reference File {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.child_dag_referencefile,
        ...
        archive_reference_file = rail.SFTPMoveFileOperator(
            ...
            "/Old_{{ current_time('%d%m%Y%H%M%S') }}_{{ dag_run.conf.reference_file | file_name }}"
        )
