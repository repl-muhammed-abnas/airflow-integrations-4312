...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'gehealthcare_user_sync_netherlands_suspend_assignment_category_custom_fieldv1_{config.instance}',
        description=f'GE_netherlands Suspend Assignment Category Custom field check V1.0 {config.instance}',
        ...
        if_first_assignmentcategory_present_15 >> rail.Label(
            'Yes') >> put_drop_down_optionsfor_suspend_assignment_category_24 >> log_to_sumo
        if_first_assignmentcategory_present_15 >> rail.Label(
            'No') >> log_to_sumo
