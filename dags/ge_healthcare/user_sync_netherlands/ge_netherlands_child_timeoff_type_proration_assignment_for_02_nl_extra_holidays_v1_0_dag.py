...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'gehealthcare_user_sync_netherlands_child_timeoff_type_proration_assignment_for_02_nl_extra_holidays_v1_0_{config.instance}',
        description=f'GE Netherlands_Child Timeoff type Proration Assignment for 	02. NL_Extra Holidays v1.0 {config.instance}',
        ...
        def get_script_policy():
            timeOffBalanceEventScripts = rail.result('get_default_time_off_type_policy_schedule_for_user_4')[
                0]['policySet']['timeOffBalanceEventScripts'] if rail.result('get_default_time_off_type_policy_schedule_for_user_4') else []
            return rail.find_first_by_attr_and_get_attr(timeOffBalanceEventScripts, 'script.name', 'Yearly Accrual with Expiry', 'additionalParameters', [])

        log_gettheaccrualbalancesetup_7 = rail.PythonOperator(
            task_id='log_gettheaccrualbalancesetup_7',
            python_callable=get_script_policy
        )
        ...
        def get_starting_balance_script_11():
        ...
        log_getthestartingbalancesetup_11 = rail.PythonOperator(
            ...
            python_callable=get_starting_balance_script_11
        )
        ...
        log_required_accrual_17 = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: round(
                (float(dag_run.conf['scheduledweeklyhours']) / 40) * float(dag_run.conf['accrual']))
        )
        ...
        def get_required_starting_balance(dag_run):
            required_starting_balance = 0
            start_date = datetime.strptime(
                dag_run.conf["startdate"], "%d/%m/%Y")
            begining_year = start_date + relativedelta(months=12)
            start_of_year = begining_year.replace(month=1, day=1)
            if start_of_year != start_date:
                end_of_last_year = start_of_year + timedelta(days=-1)
                day_of_year = int(end_of_last_year.strftime('%j'))
                scheduleworkaccrual = float(
                    dag_run.conf['scheduledweeklyhours']) / 40 * float(dag_run.conf['accrual'])
                required_starting_balance = float(scheduleworkaccrual / day_of_year) * float(
                    rail.result('log_required_numberofdaysforprorationcalculation_16'))
            return round(required_starting_balance)
        ...
        log_timeoff_policy_22 = rail.PythonOperator(
            ...
            python_callable=lambda:  json.loads(json.dumps(rail.result(
                ...
                rail.result('log_existing_starting_balance_15'), rail.result('log_required_starting_balance_json_21')).replace(
                rail.result('log_existing_accrual_10'), rail.result('log_required_accrual_json_18')).replace('null', '"effective"').replace(
                '"script"', '"scriptTarget"'))
        )
        ...
        def get_required_accrual(dag_run):
            ...
            begining_year = start_date + relativedelta(months=12)
            start_of_year = begining_year.replace(month=1, day=1)
            end_of_last_year = start_of_year.replace(
                month=1, day=1) + timedelta(days=-1)
            day_of_year = int(end_of_last_year.strftime('%j'))
            scheduleworkaccrual = float(
                dag_run.conf['scheduledweeklyhours']) / 40 * float(dag_run.conf['accrual'])
            required_accrual = float(scheduleworkaccrual / day_of_year) * float(
                rail.result('log_required_numberofdaysforprorationcalculation_16'))
            return round(required_accrual)
        ...
        accumulate_list_items_34 = rail.PythonOperator(
            task_id='accumulate_list_items_34',
        ...
        declare_variable_35 = rail.SetVariableOperator(
            ...
            append=False,
            name='effective date to consider',
            value=None
        )

        def get_effective_date_to_consider(dag_run):
            effective_date_to_consider = rail.result(
                'log_effective_dateto_consider_36')
            start_date = datetime.strptime(
                dag_run.conf["startdate"], "%d/%m/%Y")
            beginining_start_date = start_date.replace(month=1, day=1)
            if rail.result('accumulate_list_items_34'):
                max_date = (max(get_datetime_obj(x['effectiveDate']) for x in rail.result(
                    'accumulate_list_items_34')))
                if max_date < beginining_start_date:
                    effective_date_to_consider = beginining_start_date.strftime(
                        '%d/%m/%Y')
            return effective_date_to_consider

        log_effective_dateto_consider_36 = rail.PythonOperator(
            task_id='log_effective_dateto_consider_36',
            python_callable=lambda: (max(get_datetime_obj(x['effectiveDate']) for x in rail.result(
                'accumulate_list_items_34'))).strftime('%d/%m/%Y') if rail.result('accumulate_list_items_34') else None
        )

        log_effective_dateto_consider_39 = rail.PythonOperator(
            task_id='log_effective_dateto_consider_39',
            python_callable=get_effective_date_to_consider
        )

        def get_effective_policy_summary(dag_run):
            to_policy_list = []
            user_time_off_type_policy_summary = rail.result(
                'get_user_time_off_type_policy_summary_28')
            if user_time_off_type_policy_summary['policiesByTimeOffType']:
                    ...
                    if time_off_type_policy_summary['timeOffType']['displayText'] == dag_run.conf['timeofftype']:
                        for policy in time_off_type_policy_summary['policySetSchedule']:
                            eff_date = get_datetime_obj(
                                policy['effectiveDate'])
                            start_date = datetime.strptime(
                                rail.result('log_effective_dateto_consider_39'), "%d/%m/%Y")
                            if eff_date < start_date:
                                to_policy_list.append({
                                    "description": policy['description'],
                                    "effectiveDate": {
        ...
        accumulate_list_items_43 = rail.PythonOperator(
            task_id='accumulate_list_items_43',
            python_callable=get_effective_policy_summary
        )

        def get_effective_policyto_conssider():
            eff_policy_info = list(filter(
                lambda x: get_datetime_obj(x['effectiveDate']).strftime('%d/%m/%Y') == rail.result('log_effective_dateto_consider_36'), rail.result('accumulate_list_items_43')))
            return [policyinfo['policySet'] for policyinfo in eff_policy_info] if eff_policy_info else []

        log_effective_policyto_consider_44 = rail.PythonOperator(
            task_id='log_effective_policyto_consider_44',
            python_callable=get_effective_policyto_conssider
        )
        ...
        def get_starting_balance_script():
            starting_balance_script = list(filter(lambda x: x['script']['name'] == "Starting Balance Set To", rail.result(
                'parse_json_45')[0]['timeOffBalanceEventScripts']))
            return starting_balance_script[0] if starting_balance_script else []
        ...
        log_starting_balance_46 = rail.PythonOperator(
            task_id='log_starting_balance_46',
            python_callable=lambda:  json.dumps(rail.result(
                'log_gettheexistingstartingbalancesetup_46'), ensure_ascii=False)
        )
        ...
        log_gettheaccrualbalance_49 = rail.PythonOperator(
            ...
            python_callable=lambda:  rail.find_first_by_attr_and_get_attr(rail.result('parse_json_48'),
                                                                          'keyUri',
                                                                          'urn:replicon:script-key:parameter:accrual-annual-amount',
                                                                          'value.number')
        )
        ...
        def get_required_accrual_51(dag_run):
            required_accrual = 0.0
            accrual_balance = rail.result('log_gettheaccrualbalance_49')
            required_accrual = float(accrual_balance) / 365
            start_date = datetime.strptime(
                dag_run.conf["startdate"], "%d/%m/%Y")
            effectivedatetoconsider = datetime.strptime(
                rail.result('log_effective_dateto_consider_39'), "%d/%m/%Y")
            return round(float(required_accrual * ((start_date.timestamp() - effectivedatetoconsider.timestamp()) / 86400)))
        ...
        log_existing_accrual_month_53 = rail.PythonOperator(
            ...
            python_callable=lambda: rail.find_first_by_attr_and_get_attr(rail.result('parse_json_48'),
                                                                         'keyUri',
                                                                         'urn:replicon:script-key:parameter:accrue-on-month',
                                                                         'value.uri')
        )
        ...
        log_required_accrual_month_55 = rail.PythonOperator(
            ...
            datetime.strptime(rail.result('log_effective_dateto_consider_39'), "%d/%m/%Y").strftime(
                '%B').lower()
        )
        ...
        log_existing_accrual_date_57 = rail.PythonOperator(
            ...
            python_callable=lambda: rail.find_first_by_attr_and_get_attr(rail.result(
                'parse_json_48'), 'keyUri', 'urn:replicon:script-key:parameter:accrue-on-day-of-month', 'uri')
        )

        log_existing_accrual_date_58 = rail.PythonOperator(
            task_id='log_existing_accrual_date_58',
            python_callable=lambda:  json.dumps({
                "keyUri": "urn:replicon:script-key:parameter:accrue-on-day-of-month", "value": {"number": rail.result('log_existing_accrual_date_57')}},
                ensure_ascii=False)
        )

        def get_day_option(date_string):
            accrualeffectivedate = datetime.strptime(date_string, "%d/%m/%Y")
        ...
        log_required_accrual_date_59 = rail.PythonOperator(
            ...
            python_callable=lambda: get_day_option(
                rail.result('log_effective_dateto_consider_39'))
        )

        log_required_accrual_date_60 = rail.PythonOperator(
            task_id='log_required_accrual_date_60',
            python_callable=lambda: json.dumps({
                "keyUri": "urn:replicon:script-key:parameter:accrue-on-day-of-month", "value": {"number": rail.result('log_required_accrual_date_59')}},
                ensure_ascii=False)
        )

        def get_policy_set():
            policy_set = list(filter(lambda x: get_datetime_obj(x['effectiveDate']).strftime('%d/%m/%Y') == rail.result(
                'log_effective_dateto_consider_36'), rail.result('accumulate_list_items_34')))
            policytoconsider = policy_set[0]['policySet'] if policy_set else []
            policy_to_consider = json.dumps(policytoconsider, ensure_ascii=False).replace(rail.result('log_existing_accrual_50'), rail.result('log_required_accrual_52')).replace(rail.result('log_existing_accrual_month_54'), rail.result(
                'log_required_accrual_month_56')).replace(rail.result('log_existing_accrual_date_58'), rail.result('log_required_accrual_date_60')).replace(rail.result('log_starting_balance_46')+",", '').replace(", "+rail.result('log_starting_balance_46'), '').replace(rail.result('log_getthestartingbalancesetup_11')+",", '').replace(", "+rail.result('log_getthestartingbalancesetup_11'), '').replace('null', '"effective"').replace('"script"', '"scriptTarget"')
            return policy_to_consider

        log_policy_set_61 = rail.PythonOperator(
            task_id='log_policy_set_61',
            python_callable=get_policy_set
        )

        parse_json_62 = rail.PythonOperator(
            task_id='parse_json_62',
            python_callable=lambda: json.loads(
                rail.result('log_policy_set_61'))
        )

        def add_item_policy_set():
            existing_policy_set = rail.result('accumulate_list_items_43')
            existing_policy_set[0]['policySet'] = json.loads(json.dumps(existing_policy_set[0]['policySet'], ensure_ascii=False).replace(
                'null', '"effective"').replace('"script"', '"scriptTarget"'))
            policy_effective_date = datetime.strptime(rail.result(
                'log_effective_dateto_consider_39'), "%d/%m/%Y")
            existing_policy_set.append({
                "description": "Effective on" + rail.result('log_effective_dateto_consider_39'),
                "effectiveDate": {
                    "day": policy_effective_date.day,
                    "month": policy_effective_date.month,
                    "year": policy_effective_date.year,
                },
        ...
        add_item_policy_set_63 = rail.PythonOperator(
            task_id='add_item_policy_set_63',
            python_callable=add_item_policy_set
        )
        ...
        def get_default_policy_timeoffs(dag_run):
            global_ploicy_consider = rail.result(
                'add_item_policy_set_63') if rail.result('add_item_policy_set_63') else []
            ...
            for policy in globalpolicy:
                ...
                existing_accrual_setup = rail.find_first_by_attr_and_get_attr(
                    balance_event, 'script.name', "Yearly Accrual with Expiry", "additionalParameters")
                ...
                existing_accrual = json.dumps({"keyUri": "urn:replicon:script-key:parameter:accrual-annual-amount", "value": {
                                              "number": accrual_balance}}, ensure_ascii=False)
                existing_accrual_month = rail.find_first_by_attr_and_get_attr(
                    existing_accrual_setup, 'keyUri', 'urn:replicon:script-key:parameter:accrue-on-month', 'uri')
                ...
                existing_accrual_date = rail.find_first_by_attr_and_get_attr(
                    existing_accrual_setup, 'keyUri', 'urn:replicon:script-key:parameter:accrue-on-day-of-month', 'value.uri')
                ...
                derived_policy_set = json.loads(json.dumps(policy['policySet'], ensure_ascii=False).replace(existing_accrual, rail.result('log_required_accrual_26')).replace(existing_accrual_month_obj, required_accrual_month_obj).replace(
                    existing_accrual_date_obj, required_accrual_date_obj).replace(rail.result('log_getthestartingbalancesetup_11')+",", '').replace(", "+rail.result('log_getthestartingbalancesetup_11'), '').replace('null', '"effective"').replace('"script"', '"scriptTarget"'))
            ...
            global_first_policy = json.loads(json.dumps(globalpolicy[0]['policySet'], ensure_ascii=False).replace(rail.result('log_existing_accrual_10'), rail.result(
                'log_required_accrual_json_18')).replace(rail.result('log_getthestartingbalancesetup_11')+",", '').replace(", "+rail.result('log_getthestartingbalancesetup_11'), '').replace('null', '"effective"').replace('"script"', '"scriptTarget"'))
            global_ploicy_consider.append({
        ...
        if_effectivedate_day_present_6 >> rail.Label('Yes') >> log_gettheaccrualbalancesetup_7 >> parse_json_8 >> \
            ...
            log_required_numberofdaysforprorationcalculation_16 >> log_required_accrual_17 >> \
            log_required_accrual_json_18 >> if_request_type_equals_to_add_19
        if_request_type_equals_to_add_19 >> rail.Label('Yes') >> log_required_starting_balance_20 >> \
            log_required_starting_balance_json_21 >> log_timeoff_policy_22 >> put_user_time_off_account_policy_set_schedule_23 >> if_request_type_equals_to_update_24
        ...
        if_request_type_equals_to_update_24 >> rail.Label(
            'No') >> catch_89_89_89
        if_request_type_equals_to_update_24 >> rail.Label('Yes') >> log_required_accrual_25 >> \
            log_required_accrual_26 >> get_user_time_off_type_policy_summary_28 >> accumulate_list_items_34 >> declare_variable_35 >> \
            log_effective_dateto_consider_36 >> log_effective_dateto_consider_39 >> accumulate_list_items_43 >> log_effective_policyto_consider_44 >> \
            parse_json_45 >> log_gettheexistingstartingbalancesetup_46 >> log_starting_balance_46 >> log_gettheexistingaccrualsetup_47 >> parse_json_48 >> \
            log_gettheaccrualbalance_49 >> log_existing_accrual_50 >> log_required_accrual_51 >> log_required_accrual_52 >> \
            log_existing_accrual_month_53 >> log_existing_accrual_month_54 >> log_required_accrual_month_55 >> \
            log_required_accrual_month_56 >> log_existing_accrual_date_57 >> log_existing_accrual_date_58 >> \
            log_required_accrual_date_59 >> log_required_accrual_date_60 >> log_policy_set_61 >> parse_json_62 >> add_item_policy_set_63 >> \
            get_default_time_off_policy_set_schedule_for_time_off_type_64 >> \
            log_effective_dateto_consider_65 >> log_policytoassign_87 >> put_user_time_off_account_policy_set_schedule_88 >> catch_89_89_89
