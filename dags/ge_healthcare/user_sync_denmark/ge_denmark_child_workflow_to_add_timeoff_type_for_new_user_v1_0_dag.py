...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'gehealthcare_denmark_child_workflow_to_add_timeoff_type_for_new_user_v1_0_{config.instance}',
        description=f'GE_denmark_Child Workflow to add timeoff type for new user v1.0 {config.instance}',
        ...
        if_first_displaytext_present_4 = rail.IfOperator(
            ...
            yes_task="ge_denmark_user_sync_master_mapper_search_entries_5",
            no_task="add_timeoff_type_logs",
        )

        def get_entity_from_mapper(LegalEntity, to_type, ot_eligibility):
            ot_eligibility_arrived = ot_eligibility if ot_eligibility else ""
            overtime_eligibility = ot_eligibility_arrived if LegalEntity in [
                'RE1018', 'RE1014'] else ""
            print("overtime_eligibility", overtime_eligibility)
            mapperinfo = list(filter(
                lambda x: x['legal_entity'] == LegalEntity
                and x['type'] == to_type
                and x['overtime_eligibility'] == overtime_eligibility, denmark_master_mapper))
            return [emp_info['value'] for emp_info in mapperinfo]

        ge_denmark_user_sync_master_mapper_search_entries_5 = rail.PythonOperator(
            task_id='ge_denmark_user_sync_master_mapper_search_entries_5',
            python_callable=lambda dag_run: get_entity_from_mapper(
                dag_run.conf['LegalEntity'], 'Timeoff types', dag_run.conf['OvertimeEligibility'])
        )

        if_entry_col1_blank_6 = rail.IfOperator(
            task_id='if_entry_col1_blank_6',
            test='''{{ result('ge_denmark_user_sync_master_mapper_search_entries_5') | is_falsy }}''',
        ...
        def get_final_set_timeoff_info():
            timeoff_uris = []
            missing_timeoffs = []
            for mapper_to_info in rail.result('ge_denmark_user_sync_master_mapper_search_entries_5'):
                ...
                if timeoff_uri:
                    ...
                    missing_timeoffs.append(mapper_to_info)

            return {"timeoff_uris": timeoff_uris, "missing_timeoffs": missing_timeoffs}
        ...
        if_log_12_present_14 = rail.IfOperator(
            ...
            test='''{{ result('log_final_set_timeoff_uris_13').timeoff_uris | is_truthy }}''',
        ...
        put_time_off_type_assignments_for_user_15 = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "timeOffTypeUris": [to['uri'] for to in rail.result('log_final_set_timeoff_uris_13')['timeoff_uris']]
            }
        )
        ...
        log_yearly_reset_script_target_20 = rail.PythonOperator(
            task_id='log_yearly_reset_script_target_20',
            python_callable=lambda: rail.find_first_by_attr_and_get_attr(rail.result(
                'get_all_scriptsfor_time_off_balance_event_script_administration_service'), 'displayText', 'Yearly Reset', 'uri')
        )

        log_startingbalancesetto_script_target_21 = rail.PythonOperator(
            task_id='log_startingbalancesetto_script_target_21',
            python_callable=lambda:  rail.find_first_by_attr_and_get_attr(rail.result(
                'get_all_scriptsfor_time_off_balance_event_script_administration_service'), 'displayText', 'Starting Balance Set To', 'uri')
        )

        log_monthly_accrualscript_22 = rail.PythonOperator(
            task_id='log_monthly_accrualscript_22',
            python_callable=lambda:  rail.find_first_by_attr_and_get_attr(rail.result(
                'get_all_scriptsfor_time_off_balance_event_script_administration_service'), 'displayText', 'Monthly Accrual', 'uri')
        )

        trigger_dag_run_ge_denmark_child_workflow_to_add_timeoff_policy_for_new_user_v1_16 = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_dag_run_ge_denmark_child_workflow_to_add_timeoff_policy_for_new_user_v1_16',
            retries=0,
            items=lambda: rail.result('log_final_set_timeoff_uris_13')[
                'timeoff_uris'],
            trigger_dag_id=f'gehealthcare_denmark_child_add_to_policy_new_user_v1_0_{config.instance}',
            ...
            conf={
                ...
                "yearlyreseturi": "{{ result('log_yearly_reset_script_target_20') }}",
                "startingbalanceuri": "{{ result('log_startingbalancesetto_script_target_21') }}",
                "monthlyaccrualuri": "{{ result('log_monthly_accrualscript_22') }}",
                "numberofworkingdays": "{{ dag_run.conf.numberofworkingdays }}",
                "type": "Add"
            }
        )

        wait_for_completion_trigger_dag_run_ge_denmark_child_workflow_to_add_timeoff_policy_for_new_user_v1_16 = rail.WaitForDagRunsSensor(
            task_id='wait_for_completion_trigger_dag_run_ge_denmark_child_workflow_to_add_timeoff_policy_for_new_user_v1_16',
            execution_timeout=timedelta(days=14),
            dag_runs='{{ result("trigger_dag_run_ge_denmark_child_workflow_to_add_timeoff_policy_for_new_user_v1_16") }}'
        )
        ...
        if_first_displaytext_present_4 >> rail.Label(
            'Yes') >> ge_denmark_user_sync_master_mapper_search_entries_5 >> if_entry_col1_blank_6
        ...
        if_log_12_present_14 >> rail.Label(
            'Yes') >> put_time_off_type_assignments_for_user_15 >> get_all_scriptsfor_time_off_balance_event_script_administration_service >> \
            get_all_scriptsfor_time_off_validation_script_administration_service1_17 >> log_yearly_reset_script_target_20 >> \
            log_startingbalancesetto_script_target_21 >> log_monthly_accrualscript_22 >> \
            trigger_dag_run_ge_denmark_child_workflow_to_add_timeoff_policy_for_new_user_v1_16 >> \
            wait_for_completion_trigger_dag_run_ge_denmark_child_workflow_to_add_timeoff_policy_for_new_user_v1_16 >> add_timeoff_type_logs
        if_log_12_present_14 >> rail.Label(
