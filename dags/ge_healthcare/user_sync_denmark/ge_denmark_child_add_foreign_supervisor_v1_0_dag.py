...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'gehealthcare_denmark_child_add_foreign_supervisor_v1_0_{config.instance}',
        description=f'GE_denmark_Child_Add Foreign Supervisor V1.0 {config.instance}',
        ...
        denmark_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9 = rail.PythonOperator(
            task_id='denmark_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9',
            python_callable=lambda: list(
                filter(lambda x: x['legal_entity'] == 'Foreign Supervisors', denmark_master_mapper))
        )

        def get_entity_from_mapper(entity, typeval):
            emp_types = list(filter(
                lambda x: x['legal_entity'] == entity
                and x['type'] == typeval, denmark_master_mapper))
        ...
        def get_mapper_permissions(entity, mapper_type, identifier_1):
            employee_permissions = list(filter(
                ...
                and x['identifier__1__(_legal_entity_code/_type/_timeoff_type)'] == identifier_1, denmark_master_mapper))
            permissions = []
        ...
        if_log_getsupervisor_uri_4_blank_5 >> rail.Label(
            ...
            denmark_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9 >> \
