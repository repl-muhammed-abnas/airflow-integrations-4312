...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'gehealthcare_denmark_user_update_{config.instance}',
        ...
        def get_cost_center_list(dag_run):
            costcenter_schedule_list = rail.result('log_costcenterschedule_131')[
                'costcenter_schedules']
            current_date = datetime.strptime(
                dag_run.conf['HireEffectiveDate'], '%d/%m/%Y') if dag_run.conf['HireEffectiveDate'] else pendulum.now(config.pacific_timezone)
        ...
        trigger_dag_run_ge_user_sync_denmark_to_add_timeoff_type_for_lm_wind_users_157 = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[-1],
            trigger_dag_id=f'gehealthcare_user_sync_denmark_ge_denmark_child_workflow_to_add_timeoff_type_for_lm_wind_users_v1_0_{config.instance}',
            execution_timeout=timedelta(days=14),
            accumulate_result=True,
            conf={
