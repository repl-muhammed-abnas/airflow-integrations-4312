...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'gehealthcare_czech_child_add_foreign_supervisor_v1_0_{config.instance}',
        description=f'GE_Czech_Child_Add Foreign Supervisor V1.0 {config.instance}',
        ...
        czech_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9 = rail.PythonOperator(
            task_id='czech_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9',
            python_callable=lambda: list(
                filter(lambda x: x['legal_entity'] == 'Foreign Supervisors', czech_master_mapper))
        )
        ...
        def get_mapper_permissions(entity, mapper_type, identifier_1):
            employee_permissions = list(filter(
                ...
                and x['identifier__1__(_legal_entity_code/_type/_timeoff_type)'] == identifier_1, czech_master_mapper))
            permissions = []
        ...
        def get_permission_set_to_assign():
            ...
            permissions = get_mapper_permissions(
                'Foreign Supervisors', 'Permission', 'Supervisor')
        ...
        if_log_getsupervisor_uri_4_blank_5 >> rail.Label(
            ...
            czech_master_mapper_search_entries_getallthevaluestocreatea_foreign_supervisors_9 >> \
