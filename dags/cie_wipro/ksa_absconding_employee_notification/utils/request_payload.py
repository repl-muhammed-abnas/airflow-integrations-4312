...
import json
import rail
from cie_wipro.ksa_absconding_employee_notification import config
...
def get_holiday_report_filter_uris(config, start_date, end_date):
    data = rail.result("get_holiday_report_details")[
        'filterConfiguration']['enabledFilters']
    report_uri = rail.result("get_holiday_report_details")['uri']
    holiday_period_filter = rail.find_first_by_attr_and_get_attr(
        data, 'displayText', config.HOLIDAY_DATE_RANGE_FILTER, 'uri')

    filters = [
        {
            "reportFilterUri": holiday_period_filter,
            "value": null
        },
        {
            "reportFilterUri": holiday_period_filter,
...
def get_employee_payload_sendemail():
    ...
    add_recipients_users("uri", supervisor_uri if reminder_index == "4" else user_uri, emailTo)
    ...
    final_payload = {
        "email": {
            ...
            "bcc": [],
            "replyTo": None,
            "fromDisplayName": "Do-Not-Reply@replicon.com",
