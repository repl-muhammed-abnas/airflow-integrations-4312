...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'{dag_id_prefix}{config.company_key}_send_1st_notification_{config.location}{dag_id_postfix}_child_v1'.lower(),
        description=f'{dag_id_prefix}_send_1st_notification_child{config.location}{dag_id_postfix}',
        ...
        get_email_subject = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: f"Attention: Action required on auto-deducted leave – <{rail.result('foreach_reminder_entry')['Employee ID']}> –Reminder 1,"
        )
