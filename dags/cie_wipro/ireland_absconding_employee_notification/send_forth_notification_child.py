...
import rail
from cie_wipro.ireland_absconding_employee_notification.utils import request_payload
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'{dag_id_prefix}{config.company_key}_send_4th_notification_{config.location}{dag_id_postfix}_child_v1'.lower(),
        description=f'{dag_id_prefix}_send_4th_notification_child{config.location}{dag_id_postfix}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.max_child_run,
    ) as dag:

        html_body = "templates/email/template_for_supervisor_email.html"
        ...
        get_email_subject = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: f"Attention: Defaulter for “Month & Year” – <{rail.result('foreach_reminder_entry')['Employee ID']}>"
        )

        check_for_useremail = rail.IfOperator(
            task_id='check_for_useremail',
            test='''{{ result('foreach_reminder_entry')['Supervisor URI'] | is_truthy }}''',
        ...
        write_log_for_skipped_entries = rail.WriteLogOperator(
            ...
            properties={
                ...
                "Status": "Skipped - Supervisor Email Not Available",
                "Reminder No": "Forth"
            }
        )
        log_success = rail.WriteLogOperator(
            ...
            properties={
                ...
                "Type": "Efforts not entered from {{ result('foreach_reminder_entry')['continousBookingStartDate'] }} to {{ result('foreach_reminder_entry')['continousBookingEndDate'] }}",
                "User Name": "{{ result('foreach_reminder_entry')['User Name'] }}",
                "Status": "{{ dag_run_ecid() }} - Email Sent to Supervisor Successfully",
                "Reminder No": "Forth"
            }
        )
