...
def get_timeoff_report_data(config):
    ...
    for user, value in df_group:
        ...
        for index, row in sorted_df.iterrows():
            ...
            day_diff = abs((current_datetime - previous_datetime).days)
            if (day_diff == 1) or (day_diff == 3 and prevTimeoffStartDate.day_name().lower() == config.weekStartDay and timeoffStartDate.day_name().lower() == config.weekEndDay):
                counter += 1
            else:
                # counter = 1
                # if there is gap other than fri and sat, then continuity get break
                break
...
def update_timeoff_report_data():
    ...
    for entry in notification_data:
        if entry and entry.get("GPO ID") and entry.get("GPO ID") in map_empid_and_uri:
            entry["gpoUserUri"] = map_empid_and_uri[entry["GPO ID"]]

        if entry and entry.get("HR Manager ID") and entry.get("HR Manager ID") in map_empid_and_uri:
            entry["hrManagerUserUri"] = map_empid_and_uri[entry["HR Manager ID"]]
