...
import pendulum
import rail
from cie_wipro.netherlands_ts_auto_approval_utility.utils import python_callable
...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'{dag_id_prefix}{config.company_key}_timesheet_approval_master{dag_id_postfix}'.lower(),
        ...
        startDate = (now - relativedelta(days=config.previous_period_in_days)
                     ).strftime('%b, %d, %Y')
        endDate = (now + relativedelta(days=config.future_period_in_days)
                   ).strftime('%b, %d, %Y')
        ...
        process_timesheet_child = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'{dag_id_prefix}{config.company_key}_process_timesheet_chunk_child{dag_id_postfix}'.lower(
            ),
        ...
        send_task_completion_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon Timesheet Approval - {{ result("get_subject_line") }} - {{ current_time_in_specified_tz() }}',
        ...
        get_current_date >> get_all_report >> has_all_report
