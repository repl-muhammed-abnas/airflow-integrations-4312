...
def get_current_date(config):
    ...
    curr_datetime = str(datetime.strftime(curr_time, config.datetime_format))
    return curr_datetime


def get_filtered_timesheet_data(config):
    curr_datetime_obj = datetime.strptime(
        rail.result("get_current_date"), config.datetime_format)
    ...
    timeentry_csv_string = timesheet_report_artifact.get('reportGenerationResults')[
        0].get('payload')
    timeEntryData = pd.read_csv(
        StringIO(timeentry_csv_string), sep=config.seperator)
    df1 = timeEntryData.replace(np.nan, "")
    if not df1.empty:
        df1['timesheet_end_datetime'] = pd.to_datetime(
            df1['Timesheet End Date'], format=config.datetime_format)
        updated_dataframe = df1[(df1['Match'] == True) & (
            df1['timesheet_end_datetime'] < curr_datetime_obj)]
        updated_dataframe.drop(
            columns=['timesheet_end_datetime'], inplace=True)
...
def getmin_max(config):
    ...
    start_date = min(unique_dates, key=lambda d: datetime.strptime(
        d["Timesheet_Start_Date"], config.datetime_format))
    end_date = max(unique_dates, key=lambda d: datetime.strptime(
        d["Timesheet_End_Date"], config.datetime_format))
    min_dt = datetime.strptime(
        start_date["Timesheet_Start_Date"], config.datetime_format).strftime('%b %d, %Y')
    max_dt = datetime.strptime(
        end_date["Timesheet_End_Date"], config.datetime_format).strftime('%b %d, %Y')
...
def get_timesheet_uri_data(config):
    ...
    timeoffEntryDataReport = pd.read_csv(
        StringIO(timeoff_csv_string), sep=config.seperator)
            ...
            if timedata.get('TimesheetURI') not in time_export_data:
                if not timeoffEntryDataReport.empty and timedata.get('UserUri') in timeoffEntryDataReport_groupby_user.groups.keys():
                    ...
                    any_timeoff = len([entry for entry in timeoff_entries.to_dict('records') if datetime.strptime(
                        timedata['Timesheet Start Date'], config.datetime_format) <= datetime.strptime(entry['Time Off Date'], config.datetime_format) and datetime.strptime(entry['Time Off Date'], config.datetime_format) <= datetime.strptime(timedata['Timesheet End Date'], config.datetime_format)])
                    if any_timeoff == 0:
...
def check_reclac_status(config, item):
    ...
    ldm_utc = item["lastDataModification"]["valueInUtc"]
    ...
    lsa_utc = item["lastSuccessfulAttempt"]["valueInUtc"]
