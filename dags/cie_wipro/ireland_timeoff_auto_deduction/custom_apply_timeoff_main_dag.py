...
import rail
import pendulum
from cie_wipro.ireland_timeoff_auto_deduction.utils import python_callable
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'{dag_id_prefix}{config.company_key}_apply_timeoff_{config.country}{dag_id_postfix}_master_v1'.lower(),
        description=f'Custom Time Off Booking on behalf of user - {dag_id_postfix}',
        ...
        filter_report_data = rail.PythonOperator(
            ...
            op_args=[config, [config.booking_day],
                     start_datetime, current_datetime]
        )
        get_time_off_type_uris = rail.RepliconServiceOperator(
            task_id='get_time_off_type_uris',
            endpoint="/services/TimeOffService1.svc/GetAllTimeOffTypes",
            response_filter=lambda response: python_callable.findTimeoffUris(
                response, config.time_off_name1, config.time_off_name2, config.time_off_name3)
        )

        process_timeoff_On_behalf_of_user_batch = rail.TriggerDagRunForEachItemOperator(
            task_id='process_timeoff_On_behalf_of_user_batch',
            items=lambda: rail.result("filter_report_data"),
            trigger_dag_id=f'{dag_id_prefix}{config.company_key}_apply_timeoff_{config.country}{dag_id_postfix}_child_v1'.lower(),
            conf=lambda item: {
                'Entry Date': item.get("Entry Date"),
                'User Name': item.get("User Name"),
                'Scheduled Hrs': item.get("Scheduled Hrs"),
                'Time Off Hrs': item.get("Time Off Hrs"),
                'User Email': item.get("User Email"),
                'supervisor_email': item.get("supervisor_email"),
                'user_uri': item.get("user_uri"),
                'supervisor_uri': item.get("supervisor_uri"),
                'user_index': item.get("user_index"),
                'timeoff_uris': rail.result("get_time_off_type_uris"),
                'halfday_completed': item.get("halfday_completed")
            },
            execution_timeout=timedelta(days=config.execution_timeout_days),
            retries=0,
        )
        wait_for_process_timeoff_On_behalf_of_user_batch = rail.WaitForDagRunsSensor(
            task_id='wait_for_process_timeoff_On_behalf_of_user_batch',
            dag_runs='{{ result("process_timeoff_On_behalf_of_user_batch") }}',
        ...
        get_all_report >> get_report_details >> get_schedule_report_in_batch >> filter_report_data >> get_time_off_type_uris >> process_timeoff_On_behalf_of_user_batch >> wait_for_process_timeoff_On_behalf_of_user_batch >> gather_child_data >> generate_merged_log_data >> send_task_completion_email >> send_task_failure_email >> final_status
    return dag
