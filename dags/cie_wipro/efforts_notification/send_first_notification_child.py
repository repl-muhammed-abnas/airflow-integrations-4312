...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'{dag_id_prefix}{config.company_key}_send_first_notification_{config.country}{dag_id_postfix}_child_v1'.lower(),
        description=f'{dag_id_prefix}_send_first_notification_child{config.country}{dag_id_postfix}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.max_child_run,
    ) as dag:

        html_body = "templates/emails/email_for_efforts_first_reminder.html"
        ...
        get_email_subject = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: f"First Reminder: Missing Efforts Entry for {rail.result('foreach_reminder_entry')['Entry Date']}"
        )
        ...
        send_reminder_email = rail.RepliconServiceOperator(
            ...
            data=request_payload.get_first_payload_sendemail
        )
