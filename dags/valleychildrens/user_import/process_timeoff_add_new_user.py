...
def _filter_user_timeoff_types(dag_run):
    user_names = _user_timeoff_type_names(dag_run.conf)
    if not user_names:
        return []
    enabled = rail.result('get_enabled_timeoff_types') or []
    out = []
    for t in enabled:
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_timeoff_add_new_user_dagid,
        description='ValleyChildrens User Import - Process Timeoff Add for New User',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.max_active_runs_process_timeoff_add_new_user,
    ) as dag:
        ...
        is_pto_type = rail.IfOperator(
            task_id='is_pto_type',
            test=lambda: (rail.result("process_each_timeoff_type") or {}).get('displayText') == "PTO",
        ...
        get_enabled_timeoff_types >> filter_user_timeoff_types >> assign_user_timeoff_types \
            >> process_each_timeoff_type >> get_default_timeoff_policy_schedule >> is_pto_type
