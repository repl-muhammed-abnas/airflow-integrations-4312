...
def _filter_user_timeoff_types():
    ...
    mapper 'timeofftypes'. Returns full items so the foreach can read .uri
    and .displayText / .name."""
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_update_user_time_off_assign_dagid,
        description='ValleyChildrens User Import - Update User Time Off Assign',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.max_active_runs_process_update_user_time_off_assign,
    ) as dag:
        ...
        is_fte_changed = rail.IfOperator(
            task_id='is_fte_changed',
            test=lambda dag_run: str(dag_run.conf.get('ftetotal') or '') != str(dag_run.conf.get('previous_ftetotal') or ''),
            yes_task='process_fte_change_for_each_policy',
            no_task='process_payout_for_each_policy',
        )
        ...
        process_payout_for_each_policy = rail.EmptyOperator(
            task_id='process_payout_for_each_policy',
        ...
        get_balance_for_account = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "asOfDate": request_payload.to_date_struct(dag_run.conf.get('startdate')),
            }
        )
        ...
        catch_and_log_error = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                'action': 'UpdateUserTimeOffAssign',
                'status': 'Error',
                'details': '{{ get_error_message() }}',
            },
        )
        can_run_batch_task >> rail.Label('Yes') >> batch_task >> catch_and_log_error
        can_run_batch_task >> rail.Label('No') >> get_all_timeoff_types
        get_all_timeoff_types >> filter_user_timeoff_types >> get_user_timeoff_summary >> is_fte_changed
        is_fte_changed >> rail.Label('Yes') >> process_fte_change_for_each_policy >> wait_fte_change >> process_payout_for_each_policy
        is_fte_changed >> rail.Label('No') >> process_payout_for_each_policy
        process_payout_for_each_policy >> get_balance_per_policy >> get_balance_for_account >> trigger_payout_for_policy >> get_balance_per_policy_end
