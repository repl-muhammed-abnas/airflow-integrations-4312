...
"""
Mirrors Workato recipe live_valleychildrens_time_off_policy_update_on_fte_change_v1_0.

When a user's FTE changes:
  1. GetDefaultTimeOffPolicySetScheduleForTimeOffType — tenant default policy
  2. GetUserTimeOffTypePolicySummary — user's existing policies
  3. GetAllScripts (TimeOffBalanceEvent) — balance event scripts
  4. PutUserTimeOffAccountPolicySetSchedule — apply new policy at new FTE

API payload shapes match Workato:
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_time_off_policy_update_on_fte_change_dagid,
        description='ValleyChildrens User Import - Time Off Policy Update on FTE Change',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.max_active_runs_process_time_off_policy_update_on_fte_change,
    ) as dag:
        ...
        put_new_policy_with_fte = rail.RepliconServiceOperator(
            task_id='put_new_policy_with_fte',
        ...
        catch_and_log_error = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                'action': 'TimeOffPolicyUpdateOnFTEChange',
                'status': 'Error',
        ...
        get_default_policy_schedule >> get_user_timeoff_summary >> get_balance_event_scripts \
            >> put_new_policy_with_fte >> catch_and_log_error
    return dag
