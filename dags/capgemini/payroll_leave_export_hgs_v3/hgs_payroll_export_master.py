...
from capgemini.payroll_leave_export_hgs_v3.utils import request_payload, custom_methods
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        approved_timeoffs_report_has_data = rail.IfOperator(
            ...
            no_task='format_timeoff_bookings_records'
        )
        ...
        trigger_process_approved_timeoffs = rail.trigger_parallel_dagrun(
            ...
            parallel_count=config.parallel_runs_approved_bookings,
        ...
        get_process_approved_timeoffs_dag_ids =rail.PythonOperator(
            task_id= 'get_process_approved_timeoffs_dag_ids',
            python_callable= lambda: list(itertools.chain(
                *list(map(lambda x: (rail.result(
                    f'trigger_process_approved_timeoffs_{x+1}') if rail.result(
                    f'trigger_process_approved_timeoffs_{x+1}') else []), range(config.parallel_runs_approved_bookings))))),
            show_return_value_in_logs= False
        )

        gather_process_approved_timeoffs_logs = rail.GatherResultsFromDagRunsOperator(
            task_id='gather_process_approved_timeoffs_logs',
            dag_runs='{{ result("get_process_approved_timeoffs_dag_ids") }}',
        ...
        trigger_process_deleted_timeoffs = rail.trigger_parallel_dagrun(
            ...
            parallel_count=config.parallel_runs_deleted_bookings,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: {
                "timeoff_booking_details": item,
                "logging_details": rail.result("logging_details")
        ...
        get_process_deleted_timeoffs_dag_ids =rail.PythonOperator(
            task_id= 'get_process_deleted_timeoffs_dag_ids',
            python_callable= lambda: list(itertools.chain(
                *list(map(lambda x: (rail.result(
                    f'trigger_process_deleted_timeoffs_{x+1}') if rail.result(
                    f'trigger_process_deleted_timeoffs_{x+1}') else []), range(config.parallel_runs_deleted_bookings))))),
            show_return_value_in_logs= False
        )

        gather_process_deleted_timeoffs_logs = rail.GatherResultsFromDagRunsOperator(
            task_id='gather_process_deleted_timeoffs_logs',
            dag_runs='{{ result("get_process_deleted_timeoffs_dag_ids") }}',
            dagrun_task_id='create_log',
            execution_timeout=timedelta(
                hours=config.gather_logs_timeout_hours),
            flatten=True
        ...
        logging_details >> process_timeoff_reports
        ...
        approved_timeoffs_report_has_data >> rail.Label("No") >> format_timeoff_bookings_records
        ...
        is_approved_timeoffs_report_has_expected_columns >> rail.Label("Yes") >> load_approved_timeoffs_report_data \
            >> create_approved_timeoffs_collection >> query_get_valid_approved_timeoffs_collection \
                >> trigger_process_approved_timeoffs >> get_process_approved_timeoffs_dag_ids \
                    >> gather_process_approved_timeoffs_logs >> format_timeoff_bookings_records
        is_approved_timeoffs_report_has_expected_columns >> rail.Label("No") >> fail_approved_timeoffs_has_no_expected_columns \
            >> dagrun_log_to_sumo

        process_timeoff_reports >> get_deleted_timeoffs_report_details >> run_deleted_timeoffs_report_entry
        ...
        deleted_timeoffs_report_has_data >> rail.Label("No") >> format_timeoff_bookings_records
        ...
        create_deleted_timeoffs_collection >> query_get_valid_deleted_timeoffs_collection >> trigger_process_deleted_timeoffs \
            >> get_process_deleted_timeoffs_dag_ids >> gather_process_deleted_timeoffs_logs >> format_timeoff_bookings_records \
                >> has_data
