...
def get_logging_details(time_zone, export_file_prefix):
    ...
    export_end_date = datetime.strptime(get_conf()["end_date"], CONF_DATE_FORMAT) if get_conf() else today-relativedelta(months=4, day=31)
    export_start_date = datetime.strptime(get_conf()["start_date"], CONF_DATE_FORMAT) if get_conf() else today-relativedelta(months=11, day=1)
