...
import pendulum
from capgemini.time_export_global_v6_pta.tasks.time_data_export import time_data_export
from capgemini.time_export_global_v6_pta.tasks.update_export_status import cancel_time_export
from capgemini.time_export_global_v6_pta.utils import custom_methods
from capgemini.time_export_global_v6_pta.utils import request_payload
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'Capgemini Time export global V6 PTA Master {config.instance}',
        ...
        logging_details = rail.PythonOperator(
            ...
            op_args=[config.time_zone, config.export_file_prefix]
        )
        ...
        log_to_sumo_time_export = rail.SendToSumoOperator(
            ...
            data={
                ...
                'jobendtime': '{{ current_time_in_specified_tz("'+ config.time_zone +'", "%Y-%m-%dT%H:%M:%S.%f%z") }}',
                'exportperiod': '{{ result("logging_details").export_start_date }} - {{ result("logging_details").export_end_date }}',
                'twbexportname': '{{ result("logging_details").time_export_filename }}',
                'exportfilename': '{{ result("logging_details").time_export_filename }}.csv.pgp',
                'exportfilepath': config.input_filepath,
                'country': locations,
