...
import pendulum
from capgemini.morocco_leave_balance_export_to_sopra_disabled_users.utils import custom_methods, request_payload
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'Capgemini Morocco Leave Balances Export for Disabled users to SOPRA Master {config.instance}',
        ...
        get_users_disabled_and_enddate_in_daterange = rail.RepliconServiceOperator(
            task_id='get_users_disabled_and_enddate_in_daterange',
            endpoint='/services/UserListService1.svc/GetData',
            data=lambda dag_run: request_payload.get_users_disabled_and_enddate_in_daterange_payload(dag_run, config.schedules, config.time_zone),
            data_handler=custom_methods.filter_disabled_users_data
        )

        if_disabled_users_exists = rail.IfOperator(
            task_id='if_disabled_users_exists',
            test='{{ result("get_users_disabled_and_enddate_in_daterange") | is_truthy }}',
            yes_task='get_leave_balance_report_details',
        ...
        send_ma01_export_complete_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon leave balances extract of terminated employees to SOPRA for Morocco for MA01 cost center'
        ...
        send_ma02_ma03_export_complete_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | Replicon leave balances extract of terminated employees to SOPRA for Morocco for MA02 and MA03 cost centers'
                + ' is completed - {{ current_time_in_specified_tz("' + config.time_zone + '") }}',
            html_content="/templates/emails/email_valid_export_complete.html",
            params={
                'upload_file_path': config.input_filepath,
        ...
        can_run_batch_task >> rail.Label("Yes") \
            >> batch_task >> finish_leave_balances_report
        can_run_batch_task >> rail.Label("No") >> is_valid_scheduled_run >> rail.Label("Yes") >> logging_details
        logging_details >> get_allowed_location_uris >> get_users_disabled_and_enddate_in_daterange >> if_disabled_users_exists
        
        if_disabled_users_exists >> rail.Label("Yes") >> get_leave_balance_report_details \
            >> run_leaves_balance_report >> is_leave_balances_report_failed
        if_disabled_users_exists >> rail.Label("No") >> finish_leave_balances_report
