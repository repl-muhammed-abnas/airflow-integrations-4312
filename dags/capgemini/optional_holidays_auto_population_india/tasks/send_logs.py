...
def get_send_logs(config):
    with rail.TaskGroup(group_id='send_logs', prefix_group_id=False):
        ...
        get_log_filename = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: f'{config.log_file_prefix}_Optional_Holiday_Booking_' +
            dag_run.conf["properties"]["state_name"].replace(" ", "_")
                    + '_Logs_' +
        ...
        send_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('get_logged_errors', 'length') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() }} | The Auto population of Optional holiday booking for state {{ dag_run.conf.properties.state_name + " is " }} \
                {%- if result("get_logged_errors", key="length") > 0 -%} \
                    completed with errors  \
                {%- else -%} \
                    completed successfully  \
                {%- endif -%} \
                {{ " - " + dag_run.conf.properties.process_start_time }}',
        ...
        has_any_entries_in_log >> rail.Label(
            "Yes") >> get_logged_errors >> get_logged_success >> get_logged_skipped >> render_logs_csv
