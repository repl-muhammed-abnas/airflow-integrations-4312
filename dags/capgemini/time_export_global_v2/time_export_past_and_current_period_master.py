...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'capgemini_time_export_past_and_current_period_global_master_{config.instance}_v2',
        description=f'Capgemini Time export past and current period global Master {config.instance} V2',
        ...
        create_export_for_each_timesheet_period = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f"capgemini_time_export_global_create_export_child_{config.instance}_v2",
            conf=lambda item: {
