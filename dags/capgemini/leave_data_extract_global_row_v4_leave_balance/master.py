...
from capgemini.leave_data_extract_global_row_v4_leave_balance.utils import custom_methods
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.leave_balance_export_master_dag_id,
        description=f'Capgemini Leave Data Export Global Master {config.leave_status} {config.location} {config.instance}',
        ...
        send_empty_export_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon leave data extract of '
                ...
                + ' region is completed - No records to export - {{ result("logging_details")["process_start_time"] }}',
        ...
        get_file_name = rail.PythonOperator(
            ...
            python_callable=custom_methods.get_file_name,
            op_args=[config.filename_prefix, config.location, getattr(config, 'filename_seconds_suffix', null)]
        )

        upload_leave_extract_to_s3 = rail.S3UploadFileOperator(
            task_id='upload_leave_extract_to_s3',
            source="{{ result('write_leave_data_csv') }}",
            key_name=config.s3_upload_filepath + '/{{ result("get_file_name") }}.csv',
        ...
        upload_leave_extract_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.input_filepath + '/{{ result("get_file_name") }}.csv.pgp'
        )
        ...
        send_export_complete_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon leave data extract of '
                + config.leave_status + ' for the ' + config.location
                + ' region is completed - {{ result("logging_details")["process_start_time"] }}',
        ...
        process_report_data >> get_batch_creation_time >> load_csv

        load_csv >> write_leave_data_csv >> get_file_name >> upload_leave_extract_to_s3 >> encrypt_leave_extract_data_csv \
