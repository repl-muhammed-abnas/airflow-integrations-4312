...
from capgemini.france_place_of_work_export_to_sopra.utils import custom_methods, request_payload
    ...
    with rail.create_airflow_dag(
        ...
        query_place_of_work_count_data = rail.QueryCollectionOperator(
            ...
            query="""SELECT
                employee_id,
                place_of_work_fra,
                COUNT(DISTINCT entry_date) AS bucket
                FROM (
                    SELECT
                        employee_id,
                        entry_date,
                        CASE
                            WHEN MAX(CASE WHEN place_of_work_fra = 'Home Medical' THEN 1 ELSE 0 END)
                                OVER (PARTITION BY employee_id, entry_date) = 1 THEN 'Home Medical'
                            WHEN MAX(CASE WHEN place_of_work_fra = 'Flex Abroad' THEN 1 ELSE 0 END)
                                OVER (PARTITION BY employee_id, entry_date) = 1 THEN 'Home Medical'
                            WHEN MAX(CASE WHEN place_of_work_fra = 'Home' THEN 1 ELSE 0 END)
                                OVER (PARTITION BY employee_id, entry_date) = 1 THEN 'Home'
                            ELSE ''
                        END AS place_of_work_fra
                    FROM valid_place_of_work_data
                    GROUP BY employee_id, entry_date, place_of_work_fra
                ) AS PrioritizedEntries
                GROUP BY employee_id, place_of_work_fra
                ORDER BY employee_id, CASE
                    WHEN place_of_work_fra = 'Home Medical' THEN 1
                    WHEN place_of_work_fra = 'Flex Abroad' THEN 2
                    WHEN place_of_work_fra = 'Home' THEN 3
                    ELSE ''
                END
            """
        )
        ...
        get_report_details >> get_time_entry_data >> is_report_failed
        ...
        report_has_data >> rail.Label("No") >> create_place_of_work_blank_data_xml

        is_report_has_expected_columns >> rail.Label("Yes") >> process_report_data >> load_csv

        load_csv >> create_place_of_work_data_collection >> query_valid_users >> query_place_of_work_count_data \
