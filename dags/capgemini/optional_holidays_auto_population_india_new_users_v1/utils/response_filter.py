...
import itertools
import json
from capgemini.optional_holidays_auto_population_india_new_users_v1.utils.custom_methods import get_task_state
...
def get_optional_holiday_for_user(response):
    ...
    user_optional_holiday_cal = rail.result("get_user_holiday_calendar")
    return list(map(lambda calendar_data: {
        "user_uri": user_info["userDetails"]["uri"],
        "holiday_calendar_name": user_optional_holiday_cal.replace("_Optional", ""),
        "optional_holiday_calendar_name": user_optional_holiday_cal,
        "optional_holiday_calendar_uri": calendar_data["uri"]
    }, filter(lambda data: user_optional_holiday_cal == data["name"], response)))
