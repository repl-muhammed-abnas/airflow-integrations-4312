...
from capgemini.uk_payroll_export_v2.utils import custom_methods, request_payload
import rail
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f"Capgemini UK Overtime Payroll Export Create Export Child {config.instance} V2",
        ...
        get_cost_center_uri = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response, dag_run: custom_methods.get_costcenter_uri(
                response, dag_run)
        )
        ...
        update_payrun_name = rail.RepliconServiceOperator(
            ...
            data={
                ...
                "name": "{{ dag_run.conf.cost_center_code }}_{{ dag_run.conf.exportdetails.payroll_name_suffix }}"
            }
        )
        ...
        trigger_overtime_export = rail.TriggerDagRunOperator(
            ...
            conf=lambda dag_run: {
                ...
                "export_filename": f'{dag_run.conf["cost_center_code"]}_{dag_run.conf["exportdetails"]["overtime_export_filename_suffix"]}',
                "payrun_export_name": f'{dag_run.conf["cost_center_code"]}_{dag_run.conf["exportdetails"]["payroll_name_suffix"]}',
        ...
        trigger_oncall_export = rail.TriggerDagRunOperator(
            ...
            conf=lambda dag_run: {
                "exportdetails": dag_run.conf["exportdetails"],
                "payrunuri": rail.result("get_payrun_batch_result")["payRunUri"],
                "export_filename": f'{dag_run.conf["cost_center_code"]}_{dag_run.conf["exportdetails"]["oncall_export_filename_suffix"]}',
                "payrun_export_name": f'{dag_run.conf["cost_center_code"]}_{dag_run.conf["exportdetails"]["payroll_name_suffix"]}',
                "cost_center_name": dag_run.conf["cost_center_name"]
            }
        )
        ...
        fail_blank_empid_payrun = rail.FailOperator(
            ...
            message="Local Employee Number not present for some users. Users available to validate in payrun '{{ dag_run.conf.cost_center_code }}_{{ dag_run.conf.exportdetails.payroll_name_suffix }}'"
        )
        ...
        is_costcenter_present >> rail.Label("Yes") >> create_payrun_batch >> execute_payrun_batch \
