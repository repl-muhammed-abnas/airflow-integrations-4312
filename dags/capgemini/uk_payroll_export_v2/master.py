...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f"Capgemini UK Overtime Payroll Export Master {config.instance} V2",
        ...
        create_export_for_costcenters = rail.TriggerDagRunForEachItemOperator(
            ...
            items=list(config.cost_centers_list.keys()),
            conf=lambda item: {
                "exportdetails": rail.result("logging_details"),
                "cost_center_name": item,
                "cost_center_code": item.split(" | ")[-1],
                "cost_center_hierarchy_level": config.cost_centers_list[item],
