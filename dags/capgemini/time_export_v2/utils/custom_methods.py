...
def get_logging_details(config):
    current_time = pendulum.now(config.time_zone).strftime('%Y%m%d%H%M%S')
    return {
        ...
        "export_start_date": config.export_start_date,
        "export_end_date": str((pendulum.now(config.time_zone)+relativedelta(day=31)).strftime("%Y/%m/%d")),
