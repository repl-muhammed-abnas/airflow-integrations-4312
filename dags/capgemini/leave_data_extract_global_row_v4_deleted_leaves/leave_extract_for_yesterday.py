...
import pendulum
from capgemini.leave_data_extract_global_row_v4_deleted_leaves.utils import custom_methods
from capgemini.leave_data_extract_global_row_v4_deleted_leaves.tasks.get_tenant_wide_logs import get_tenant_wide_logs
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.previous_day_leave_export_master_dag_id,
        description=f'Capgemini Leave Data Export Global Master for Previous Day {config.leave_status} {config.location} {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=datetime(2025, 3, 24),
        schedule_interval=config.schedule_interval_1am,
        ...
        run_report_group_entry, run_report_group_exit = rail.run_report(
            ...
            report_params=lambda: custom_methods.get_report_parameters(
                (pendulum.now(tz=config.time_zone) - timedelta(days=1)).strftime("%m/%d/%Y")),
            target='artifact'
        )
