...
import pendulum
from capgemini.france_leave_data_export_to_sopra.sick_leaves_export_v1.utils import custom_methods, request_payload
...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'Capgemini France Sick Leaves Data Export to SOPRA Master {config.instance}',
        ...
        create_approved_leave_data_collection = rail.CreateCollectionOperator(
            ...
            columns={
                ...
                "Initial Or Extension ?": "initial_or_extension",
                "Have you worked on the start date of the leave ?": "worked_on_start_date"
            },
        ...
        can_run_deleted_leaves_batch_task = rail.IfOperator(
            task_id='can_run_deleted_leaves_batch_task',
            test=lambda: Variable.get(
                config.can_run_batch_task_var_name, default_var='true').lower() == 'true',
            yes_task='deleted_leaves_batch_task',
            no_task='get_deleted_report_details'
        )

        deleted_leaves_batch_task = rail.BatchTaskRunOperator(
            task_id='deleted_leaves_batch_task',
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            start_task='get_deleted_report_details',
            end_task='finish_deleted_leaves_process',
        )
        ...
        write_deleted_leave_data_csv = rail.WriteCSVFileOperator(
            ...
            source="{{ result('query_valid_users_deleted_leaves_data') }}",
        ...
        create_leave_data_xml = rail.RenderTemplateOperator(
            ...
            target='artifact',
            template_file='xml_schema/sopra_sick_leaves.xml',
        ...
        send_export_complete_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon Sick leaves data extract to SOPRA for France'
        ...
        logging_details >> process_leave_reports
        ...
        load_approved_leaves_csv >> create_approved_leave_data_collection \
            >> query_valid_users_approved_leaves_data >> write_approved_leave_data_csv \
                >> final_approved_leaves >> finish_approved_leaves_process >> process_leave_data

        is_approved_report_has_expected_columns >> rail.Label("No") >> fail_approved_no_expected_columns >> finish_approved_leaves_process

        process_leave_reports >> can_run_deleted_leaves_batch_task >> rail.Label("Yes") \
            >> deleted_leaves_batch_task >> finish_deleted_leaves_process
        can_run_deleted_leaves_batch_task >> rail.Label("No") >> get_deleted_report_details \
            >> run_deleted_leaves_report >> is_deleted_report_failed
