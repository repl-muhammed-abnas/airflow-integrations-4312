...
def get_scheduled_hours_report_filters(dag_run):
    date_filter = rail.find_first_by_attr_and_get_attr(rail.result('get_scheduled_hours_report_details')['filterConfiguration']['enabledFilters'],
                    'displayText', "DateFilter", 'uri')
    return [
        {
...
def get_scheduled_hours_report_parameters(dag_run):
    return {
        "reportParameters": [
            {
                "reportUri": rail.result('get_scheduled_hours_report_details')["uri"],
                "filterValues": get_scheduled_hours_report_filters(dag_run),
                "outputFormatUri": "urn:replicon:report-output-format-option:csv"
            }
        ]
    }
