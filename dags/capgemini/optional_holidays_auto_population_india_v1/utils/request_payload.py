...
def get_user_holiday_cal_payload(dag_run, config):
    date_range = (datetime.strptime(config.e1_schedule, "%m/%d")
        if dag_run.conf["properties"]["schedule"] == "E1"
            else datetime.strptime(config.e2_schedule, "%m/%d"))
    effective_date = {
        "year": pendulum.now(config.time_zone).year,
