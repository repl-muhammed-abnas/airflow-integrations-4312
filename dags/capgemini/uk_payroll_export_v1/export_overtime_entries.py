...
from capgemini.uk_payroll_export_v1.utils import custom_methods
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.overtime_export_child_dag_id,
        description=f"Capgemini UK Payroll Export Overtime Entries Child {config.instance} V1",
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.export_overtime_max_active_runs,
        default_args={
        ...
        query_entries_on_paycodes = rail.QueryCollectionOperator(
            ...
            query=f"SELECT * FROM payrunpayrolldata WHERE Pay_Code_Name IN {config.overtime_paycodes_list}",
            name='overtime_paycode_entries'
        )
        ...
        write_payroll_data_csv = rail.WriteCSVFileOperator(
            ...
            row=custom_methods.get_overtime_payroll_data_rows,
            header=config.overtime_export_headers,
            execution_timeout=timedelta(
        ...
        upload_payroll_extract_to_s3 = rail.S3UploadFileOperator(
            ...
            key_name=config.s3_overtime_upload_filepath +
        ...
        upload_payroll_extract_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.overtime_input_filepath +
        ...
        send_export_complete_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon Payroll Export of overtime entries of {{ dag_run.conf.cost_center_name }} cost center for United Kingdom'
            + ' is completed - {{ current_time_in_specified_tz("' + config.time_zone + '") }}',
            html_content="/templates/emails/email_valid_export_complete.html",
            params={
                'upload_file_path': config.overtime_input_filepath,
                'location': config.location,
                'time_zone': config.time_zone,
                'entries_for': "overtime"
            }
        )
