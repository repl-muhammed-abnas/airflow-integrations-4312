...
from capgemini.uk_payroll_export_v1.utils import custom_methods, request_payload
import rail
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f"Capgemini UK Overtime Payroll Export Create Export Child {config.instance} V1",
        ...
        fail_blank_empid_payrun = rail.FailOperator(
            ...
            message="Employee ID not present for some users. Users available to validate in payrun '{{ dag_run.conf.cost_center_code }}_{{ dag_run.conf.exportdetails.payroll_name_suffix }}'"
        )
