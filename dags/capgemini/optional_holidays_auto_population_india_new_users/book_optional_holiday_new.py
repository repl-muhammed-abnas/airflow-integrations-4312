...
from capgemini.optional_holidays_auto_population_india_new_users.utils import request_payload, custom_methods
from capgemini.optional_holidays_auto_population_india_new_users.utils.response_filter import get_user_timeoff_bookings, get_timeoff_booking_uri
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'capgemini_auto_population_of_optional_holidays_india_book_optional_holiday_child_new_user_{config.instance}_v0',
        description=f'Capgemini Auto Population of Optional Holidays India Child V0 {config.instance}',
        ...
        is_status_not_submitted >> rail.Label(
            "Yes") >> delete_timeoff_booking >> empty_task
        ...
        log_booking_successful >> empty_task
        empty_task >> batch_end >> can_fail_dag

        is_bookings_available_for_given_timeoff_types >> rail.Label(
            "Yes") >> log_booking_already_present >> batch_end >> can_fail_dag

        can_fail_dag >> rail.Label("Yes") >> fail_dagrun >> catch_and_log_errors
