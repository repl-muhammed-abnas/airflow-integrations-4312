...
import rail
from mci.time_sync.utils.python_callable import do_format_logs

def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_log_generation,
        description=f'{config.company_key} MCI Time Sync - Process Log Generation',
        ...
        send_sftp_upload_fail_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Time sync  to paycom - Uploading Logs to SFTP failed ' +
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            {%- else -%}\
                "+config.alert_email+"\
            {%- endif -%}",
            subject='{{ get_company_key() + " | Time sync from replicon to paycoms " }} \
