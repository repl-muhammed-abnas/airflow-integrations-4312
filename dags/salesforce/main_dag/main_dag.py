...
# config :
# https://github.com/replicon/airflow-integrations/blob/main/dags/salesforce/main_dag/config.py


def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"standard_salesforce_{config.region.replace('-', '_')}_main_trigger_{config.instance}",
        description=f'Salesforce {config.region} Master {config.instance}',
        ...
        get_salesforce_company_conn_ids = rail.SimpleHttpOperator(
            task_id='get_salesforce_company_conn_ids',
            method='POST',
        ...
        def get_sf_clientids_by_integration():
            null = None
            client_import = []
            project_import = []
            custom_integrations = []

            # Load salesforce company connections
            salesforce_company_connids = json.loads(
                rail.result('get_salesforce_company_conn_ids'))

            # Define workflow IDs
            client = config.client
            project = config.project

            # Iterate through salesforce company connections
            for each_client in salesforce_company_connids:
                ...
                if dag_settings:
                    ...
                    project_worflow = next(
                        iter(filter(lambda x: not (x.get('isCustom')) and x['workflowId'] == project and x['enabled'].lower() == 'yes', dag_settings)), null)
                    if project_worflow:
                        project_import.append(
                            {**each_client, 'customSettings': project_worflow['customSettings']})
            ...
            return {
                ...
                f'{project}': project_import,
                'custom_integrations': custom_integrations
            }

        parse_sf_clientids = rail.PythonOperator(
            task_id='parse_sf_clientids',
            python_callable=lambda: rail.get_connector_clientids_by_integration(rail.result(
                'get_salesforce_company_conn_ids'), config.workflows)
        )

        is_client_import = rail.IfOperator(
            task_id='is_client_import',
            test=lambda: len(rail.result('parse_sf_clientids')[
                             config.client]) > 0,
        ...
        trigger_client_import = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=lambda: rail.result('parse_sf_clientids')[config.client],
        ...
        trigger_project_import = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=lambda: rail.result('parse_sf_clientids')[config.project],
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: dict(item.items())
        )
        ...
        trigger_custom_integrations = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=lambda: rail.result('parse_sf_clientids')[
                'custom_integrations'],
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: dict(item.items())
        )
        ...
        create_hmac_signature_and_get_request_body >> get_salesforce_company_conn_ids >> \
            parse_sf_clientids >> is_client_import
