from datetime import timedelta
import rail
from viaplus.timeoff_sync_v1.utils import custom_methods
from viaplus.timeoff_sync_v1.utils import request_payload
from viaplus.timeoff_sync_v1.utils import response_filters
from rail.lib.ecid import get_dagrun_ecid
from airflow.models import Variable

null = None


def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.timeoff_booking_child,
        description=f'ViaPlus Timeoff Sync Booking Child {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.max_active_booking_child
    ) as dag:

        rail.ViewDagRunConfOperator(
            task_id="view_dagrun_config")

        can_run_batch_task = rail.IfOperator(
            task_id='can_run_batch_task',
            test=lambda: Variable.get(
                config.can_run_batch_task_booking_child_var_name, default_var='true').lower() == 'true',
            yes_task='batch_task',
            no_task='get_user_info'
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id='batch_task',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            start_task='get_user_info',
            end_task='catch_and_log_errors',
        )

        get_user_info = rail.RepliconServiceOperator(
            task_id='get_user_info',
            endpoint='/services/ImportService1.svc/BulkGetUsers3',
            data=request_payload.get_bulk_users_payload,
            data_handler=lambda res: res[0] if len(res) > 0 and res[0]["userDetails"]["uri"] else null
        )

        is_user_present = rail.IfOperator(
            task_id='is_user_present',
            test='{{ result("get_user_info") | is_truthy }}',
            yes_task='is_user_in_legal_entity',
            no_task='log_user_not_present'
        )

        log_user_not_present = rail.WriteLogOperator(
            task_id='log_user_not_present',
            log='{{ dag_run.conf.log_artifact }}',
            message="User not present in Replicon",
            severity='Skipped',
            properties=lambda dag_run: {
                "username": dag_run.conf["booking_data"].get("employeeDisplayName", ""),
                "employee_id": dag_run.conf["booking_data"]["employeeNumber"],
                "unique_id": dag_run.conf["booking_data"]["id"],
                "leave_type": dag_run.conf["booking_data"]["leaveTypeName"],
                "booking_start_date": dag_run.conf["booking_data"]["fromDate"],
                "booking_end_date": dag_run.conf["booking_data"]["toDate"],
                "status": "Skipped",
                "comments": "User not present in Replicon",
                "dag_run_id": get_dagrun_ecid(dag_run)
            }
        )

        is_user_in_legal_entity = rail.IfOperator(
            task_id='is_user_in_legal_entity',
            test=custom_methods.check_user_legal_entity,
            yes_task='get_specific_time_off_type',
            no_task='log_user_not_in_legal_entity'
        )

        log_user_not_in_legal_entity = rail.WriteLogOperator(
            task_id='log_user_not_in_legal_entity',
            log='{{ dag_run.conf.log_artifact }}',
            message="User not in legal entity {{ dag_run.conf.legal_entity_filter }}",
            severity='Skipped',
            properties=lambda dag_run: {
                "username": rail.result("get_user_info")["userDetails"]["displayText"],
                "employee_id": rail.result("get_user_info")["userDetails"]["employeeId"],
                "unique_id": dag_run.conf["booking_data"]["id"],
                "leave_type": dag_run.conf["booking_data"]["leaveTypeName"],
                "booking_start_date": dag_run.conf["booking_data"]["fromDate"],
                "booking_end_date": dag_run.conf["booking_data"]["toDate"],
                "status": "Skipped",
                "comments": f"User not in legal entity {dag_run.conf['legal_entity_filter']}",
                "dag_run_id": get_dagrun_ecid(dag_run)
            }
        )

        get_specific_time_off_type = rail.RepliconServiceOperator(
            task_id='get_specific_time_off_type',
            endpoint='/services/TimeOffService1.svc/GetAllTimeOffTypes',
            data_handler=lambda response, dag_run: rail.find_first_by_attr_and_get_attr(
                response, 'displayText', dag_run.conf["booking_data"]["leaveTypeName"], 'uri')
        )

        is_timeoff_type_present = rail.IfOperator(
            task_id='is_timeoff_type_present',
            test='{{ result("get_specific_time_off_type") | is_truthy }}',
            yes_task='is_timeoff_type_assigned_to_user',
            no_task='log_timeoff_type_not_present'
        )

        log_timeoff_type_not_present = rail.WriteLogOperator(
            task_id='log_timeoff_type_not_present',
            log='{{ dag_run.conf.log_artifact }}',
            message='Time Off type {{ dag_run.conf.booking_data.leaveTypeName }} not available in Replicon',
            severity='Skipped',
            properties=lambda dag_run: {
                "username": rail.result("get_user_info")["userDetails"]["displayText"],
                "employee_id": rail.result("get_user_info")["userDetails"]["employeeId"],
                "unique_id": dag_run.conf["booking_data"]["id"],
                "leave_type": dag_run.conf["booking_data"]["leaveTypeName"],
                "booking_start_date": dag_run.conf["booking_data"]["fromDate"],
                "booking_end_date": dag_run.conf["booking_data"]["toDate"],
                "status": "Skipped",
                "comments": f'Time Off type "{dag_run.conf["booking_data"]["leaveTypeName"]}" not available in Replicon',
                "dag_run_id": get_dagrun_ecid(dag_run)
            }
        )

        is_timeoff_type_assigned_to_user = rail.IfOperator(
            task_id='is_timeoff_type_assigned_to_user',
            test=custom_methods.check_timeoff_type_assigned_to_user,
            yes_task='check_existing_booking',
            no_task='log_timeoff_type_not_assigned_to_user'
        )

        log_timeoff_type_not_assigned_to_user = rail.WriteLogOperator(
            task_id='log_timeoff_type_not_assigned_to_user',
            log='{{ dag_run.conf.log_artifact }}',
            message='Time Off type {{ dag_run.conf.booking_data.leaveTypeName }} is not assigned to user in Replicon',
            severity='Skipped',
            properties=lambda dag_run: {
                "username": rail.result("get_user_info")["userDetails"]["displayText"],
                "employee_id": rail.result("get_user_info")["userDetails"]["employeeId"],
                "unique_id": dag_run.conf["booking_data"]["id"],
                "leave_type": dag_run.conf["booking_data"]["leaveTypeName"],
                "booking_start_date": dag_run.conf["booking_data"]["fromDate"],
                "booking_end_date": dag_run.conf["booking_data"]["toDate"],
                "status": "Skipped",
                "comments": f'Time Off type "{dag_run.conf["booking_data"]["leaveTypeName"]}" is not assigned to user in Replicon',
                "dag_run_id": get_dagrun_ecid(dag_run)
            }
        )

        # Check if booking already exists in Replicon (by Keka Booking ID)
        check_existing_booking = rail.RepliconServiceOperator(
            task_id="check_existing_booking",
            endpoint="/services/TimeOffListService1.svc/GetData",
            data=request_payload.get_time_off_details_by_keka_booking_id,
            data_handler=response_filters.get_existing_booking_details
        )

        is_existing_booking = rail.IfOperator(
            task_id='is_existing_booking',
            test='{{ result("check_existing_booking") | is_truthy }}',
            yes_task='delete_existing_timeoff',
            no_task='put_and_submit_timeoff_booking_for_user'
        )

        # Delete existing booking before creating updated one
        delete_existing_timeoff = rail.RepliconServiceOperator(
            task_id='delete_existing_timeoff',
            endpoint="/services/TimeOffService1.svc/DeleteTimeOff",
            data={
                "timeOffUri": "{{ result('check_existing_booking')[0].timeoff_uri }}"
            }
        )

        put_and_submit_timeoff_booking_for_user = rail.RepliconServiceOperator(
            task_id='put_and_submit_timeoff_booking_for_user',
            endpoint="/services/TimeOffApprovalService1.svc/PutAndSubmitTimeOff",
            data=request_payload.get_put_and_submit_timeoff_payload
        )

        get_time_off_approval_status = rail.RepliconServiceOperator(
            task_id="get_time_off_approval_status",
            endpoint="/services/TimeOffApprovalService1.svc/GetApprovalHistoryDetails",
            data={
                "timeOffUri": "{{ result('put_and_submit_timeoff_booking_for_user').uri }}"
            },
            data_handler=lambda response: response['approvalStatus']['displayText']
        )

        is_timeoff_approved = rail.IfOperator(
            task_id="is_timeoff_approved",
            test=lambda: rail.result('get_time_off_approval_status') == 'Approved',
            yes_task='log_booking_successful',
            no_task='approve_timeoff_booking_for_user'
        )

        approve_timeoff_booking_for_user = rail.RepliconServiceOperator(
            task_id='approve_timeoff_booking_for_user',
            endpoint="/services/TimeOffApprovalService1.svc/ForceApprove",
            data=request_payload.get_approve_holiday_booking_payload
        )

        log_booking_successful = rail.WriteLogOperator(
            task_id='log_booking_successful',
            log='{{ dag_run.conf.log_artifact }}',
            message='Timeoff Synced Successfully',
            severity='Success',
            properties=lambda dag_run: {
                "username": rail.result("get_user_info")["userDetails"]["displayText"],
                "employee_id": rail.result("get_user_info")["userDetails"]["employeeId"],
                "unique_id": dag_run.conf["booking_data"]["id"],
                "leave_type": dag_run.conf["booking_data"]["leaveTypeName"],
                "booking_start_date": dag_run.conf["booking_data"]["fromDate"],
                "booking_end_date": dag_run.conf["booking_data"]["toDate"],
                "status": "Success",
                "comments": f'Timeoff "{dag_run.conf["booking_data"]["leaveTypeName"]}" Synced Successfully',
                "dag_run_id": get_dagrun_ecid(dag_run)
            }
        )

        catch_and_log_errors = rail.WriteLogOperator(
            task_id='catch_and_log_errors',
            log='{{ dag_run.conf.log_artifact }}',
            trigger_rule='one_failed',
            message='{{ get_error_message() }}',
            severity='Error',
            properties={
                "username": '{{ result("get_user_info").userDetails.displayText if result("get_user_info") else dag_run.conf.booking_data.employeeNumber }}',
                "employee_id": '{{ result("get_user_info").userDetails.employeeId if result("get_user_info") else dag_run.conf.booking_data.employeeNumber }}',
                "unique_id": '{{ dag_run.conf.booking_data.id }}',
                "booking_start_date": '{{ dag_run.conf.booking_data.fromDate }}',
                "booking_end_date": '{{ dag_run.conf.booking_data.toDate }}',
                "status": "Error",
                "comments": '{{ get_error_message() }}',
                "dag_run_id": '{{ dag_run_ecid() }}'
            }
        )

        dagrun_log_to_sumo = rail.DagRunLogToSumoOperator(
            task_id='dagrun_log_to_sumo',
            trigger_rule='all_done',
            sumo_conn_id=config.sumo_conn_id
        )

        # DAG Flow
        can_run_batch_task >> rail.Label("Yes") >> batch_task >> catch_and_log_errors
        can_run_batch_task >> rail.Label("No") >> get_user_info

        get_user_info >> is_user_present
        is_user_present >> rail.Label("Yes") >> is_user_in_legal_entity
        is_user_present >> rail.Label("No") >> log_user_not_present >> catch_and_log_errors

        is_user_in_legal_entity >> rail.Label("Yes") >> get_specific_time_off_type
        is_user_in_legal_entity >> rail.Label("No") >> log_user_not_in_legal_entity >> catch_and_log_errors

        get_specific_time_off_type >> is_timeoff_type_present
        is_timeoff_type_present >> rail.Label("Yes") >> is_timeoff_type_assigned_to_user
        is_timeoff_type_present >> rail.Label("No") >> log_timeoff_type_not_present >> catch_and_log_errors

        is_timeoff_type_assigned_to_user >> rail.Label("Yes") >> check_existing_booking
        is_timeoff_type_assigned_to_user >> rail.Label("No") >> log_timeoff_type_not_assigned_to_user >> catch_and_log_errors

        check_existing_booking >> is_existing_booking
        is_existing_booking >> rail.Label("Yes") >> delete_existing_timeoff >> put_and_submit_timeoff_booking_for_user
        is_existing_booking >> rail.Label("No") >> put_and_submit_timeoff_booking_for_user

        put_and_submit_timeoff_booking_for_user >> get_time_off_approval_status >> is_timeoff_approved
        is_timeoff_approved >> rail.Label("Yes") >> log_booking_successful
        is_timeoff_approved >> rail.Label("No") >> approve_timeoff_booking_for_user >> log_booking_successful

        log_booking_successful >> catch_and_log_errors
        catch_and_log_errors >> dagrun_log_to_sumo

    return dag


rail.for_each_instance(create_child_dag)