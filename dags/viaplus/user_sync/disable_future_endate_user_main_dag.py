...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.disable_future_enddate_user_master_dagid,
        description='ViaPlus User Sync -  Disable User Master',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=datetime(2022, 4, 1, tz=config.indian_timezone),
