"""
ViaPlus User Sync - Process Supervisor Assignment TaskGroup

This TaskGroup handles supervisor assignment logic matching CRL user_import_ireland_v1 pattern:
1. Search for supervisor in Replicon
2. Check if supervisor end date is in past
3. Enable supervisor if disabled
4. Add missing permissions (Supervisor, Report User)
5. Update supervisor schedule for user

Matches CRL user_import_ireland_v1 pattern exactly.
"""
from datetime import timedelta, datetime
from dateutil.parser import parse as date_parser
import rail
from viaplus.user_sync.utils.request_payload import get_today_date, get_replicon_date, get_date_from_replicon_date
from viaplus.user_sync.utils.response_filter import get_missing_permissions
...
def process_supervisor_assignment_task_group(user_uri, status):
    """
    Create TaskGroup for supervisor assignment.

    Args:
        user_uri: Task ID or conf key for user URI
        status: Type of user ('new_user', 'update_user', or 'rehire_user')

    Returns:
        Tuple of (entry_task, exit_task) for TaskGroup
    """
    with rail.TaskGroup(group_id='process_supervisor_assignment_task', prefix_group_id=False):
                ...
                "users": [
                    {
                        "uri": null,
                        "loginName": null,
                        "employeeId": "{{ dag_run.conf.sup_employee_number }}",
        ...
        log_supervisor_not_present = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                'sup_emp_id': dag_run.conf['sup_employee_number'],
                'action': 'Add' if status == 'new_user' else ('Update' if status != 'rehire_user' else 'Rehire'),
                'effectivedate': dag_run.conf['todays_date'] if status != 'new_user' else None,
                'exception_logs': rail.result('apply_user_modifications', 'exception_logs') if status != 'new_user'
                    else rail.result('add_new_user', 'exception_logs'),
        ...
        # ================================================================
        # Task Dependencies (matching CRL pattern exactly)
        # ================================================================
