...
import rail
from vp_ukgpro_integration.journal_sync.utils.python_callable_method import (
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'vp_ukgpro_journal_sync_main_{config.instance}',
        description=(
        ...
        def prepare_sync_timestamps():
            ...
            variable_key = (
                f'vp_ukgpro_journal_sync_last_run_'
        ...
        trigger_cash_disbursement = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=(
                f'vp_ukgpro_journal_sync_cash_disbursement_{config.instance}'
            ),
            conf=lambda item: {
                'artifact_name': item,
                'connections': (
        ...
        prepare_timestamps >> fetch_ukgpro_payroll >> has_payroll_data
        (
