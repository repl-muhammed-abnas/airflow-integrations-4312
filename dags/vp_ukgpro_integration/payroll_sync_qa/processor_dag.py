...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        def extract_timesheet_data_from_webhook(dag_run):
            ...
            elif isinstance(webhook_data, dict) and 'entries' in webhook_data:
                timesheet_entries = webhook_data['entries']
            ...
            return {
                ...
                'received_at': (
                    dag_conf.get('received_at')
                    or dag_conf.get('webhook', {}).get('received_at', '')
                )
            }
        ...
        if_data_present = rail.IfOperator(
            ...
            test=lambda **kwargs: rail.result(
        ...
        if_validation_passed = rail.IfOperator(
            ...
            test=lambda **kwargs: rail.result('validate_and_transform').get(
                'valid_count', 0) > 0,
        ...
        def validate_employees_in_ukgpro():
            if not config.validate_employees_in_ukgpro:
        ...
        def create_batches_of_earnings():
            ...
            batches = []
            for i in range(0, len(all_earnings), config.batch_size):
                batch = all_earnings[i:i + config.batch_size]
                batches.append({
        ...
        # Task dependencies
        extract_timesheet_data >> if_data_present
        ...
        [extract_timesheet_data, if_data_present, validate_and_transform, if_validation_passed,
