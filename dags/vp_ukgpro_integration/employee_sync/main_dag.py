"""
Processor DAG for VP UKG Pro Employee Sync.
"""
...
def create_dag(config):
    """
    Create processor DAG for VP UKG Pro Employee Sync.

    Args:
        config: Configuration object with instance settings
    """
    with rail.create_airflow_dag(
        dag_id=f'vp_ukgpro_employee_sync_main_{config.instance}',
        description='Processor DAG for VP UKG Pro Employee Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=Variable.get(
            f'vp_ukgpro_employee_sync_schedule_interval_{config.instance}',
            '0 * * * *'  # By default, it runs every 1 hour
        ),
        max_active_runs=config.max_active_runs,
        tags=['vantagepoint_ukgpro', 'employee_sync', 'main'],
        default_args={
        ...
        fetch_customers_by_integration = rail.SimpleHttpOperator(
            ...
            data={
                'integration_type': 'employee_sync',
                'status': 'enabled'
            },
        ...
        process_customers = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=(
                f'vp_ukgpro_employee_sync_dispatcher_{config.instance}'
            ),
            conf=lambda item: {
                **item,
