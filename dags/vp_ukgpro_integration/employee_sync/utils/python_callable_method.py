...
def warn_supervisor_not_found_for_create():
    ...
    conf = rail.get_current_context()['dag_run'].conf
    print(
        f"Warning: Supervisor not assigned for employee "
        f"{conf.get('employeeNumber')} - supervisor "
        f"{conf.get('supervisorEmployeeNumber')} not found in "
        f"Vantagepoint. Employee will be created without supervisor."
    )
...
def warn_supervisor_not_found_for_update():
    ...
    print(
        ...
        f"{conf.get('employeeNumber')} - supervisor "
        f"{conf.get('supervisorEmployeeNumber')} not found in "
        f"Vantagepoint. Employee will be updated without supervisor."
    )
