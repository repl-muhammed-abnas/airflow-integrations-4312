...
tenant_email = "bartosz.polawski@pwc.com, damian.korpas@pwc.com"
...
toil_extract_file_name = 'TOIL_Hours_Extract_{0}_JPN'
timestamp = '%Y%m%d%H%M%S'
s3_output_filepath = "PwC/Toil_Extract/Output/"

reference_file_name = "Toil_Reference_Japan"
...
location = 'Japan'
dag_id_post_fix = "_".join('Japan'.split()) # location name to display in dag id(without spaces and special characters)
toil_to_types = ['[JPN] Leave in Lieu (Paid) 振替休日（有給）']

prev_period_in_months = 3
report_config = {
    "timesheet_day_template_report_name": "***TOIL Extract TS Day Report - JPN",
    "timeoff_transaction_report_name": "***TOIL Extract TO Transaction Report - JPN",
    "user_template_report_name": "***TOIL Extract User Template Report - JPN",
}
report_sep = ","
