...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task='batch_task',
            no_task='if_disable_profile'
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id='batch_task',
            start_task='if_disable_profile',
            end_task='log_to_sumo',
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
        )

        if_disable_profile = rail.IfOperator(
            task_id='if_disable_profile',
            test=lambda dag_run: bool(dag_run.conf.get('disable', False)),
            yes_task='update_user_loginname',
            no_task='is_non_primary_user',
        )
        ...
        update_user_end_date = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "modifications": {
                    "userDetailsToApply": {
                        "employmentEndDate": {
                            "date": rail.get_replicon_date(datetime.now(timezone.utc)),
                        }
                    }
                }
            }
        ...
        is_non_primary_user = rail.IfOperator(
            task_id='is_non_primary_user',
            test=lambda dag_run: not dag_run.conf.get('is_primary', True),
            yes_task='remove_email_for_non_primary_user',
            no_task='is_primary_user',
        )

        is_primary_user = rail.IfOperator(
            task_id='is_primary_user',
            test=lambda dag_run: dag_run.conf.get('is_primary', False),
            yes_task='add_email_for_primary_user',
            no_task='get_custom_fieldsforuser_3',
        )

        add_email_for_primary_user = rail.RepliconServiceOperator(
            task_id='add_email_for_primary_user',
            endpoint="/services/UserService1.svc/UpdateEmail",
            data={
                "userUri": "{{ dag_run.conf.useruri }}",
        ...
        update_primary_user_login_name = rail.RepliconServiceOperator(
            task_id='update_primary_user_login_name',
        ...
        remove_email_for_non_primary_user = rail.RepliconServiceOperator(
            task_id='remove_email_for_non_primary_user',
            endpoint="/services/UserService1.svc/UpdateEmail",
            data={
                "userUri": "{{ dag_run.conf.useruri }}",
                "email": null
        ...
        update_user_replicon_authentication = rail.RepliconServiceOperator(
            task_id='update_user_replicon_authentication',
            endpoint="/services/securityservice1.svc/SetRepliconAuthenticationForUser",
            data={
                "userUri": "{{ dag_run.conf.useruri }}",
                "loginName": "{{ dag_run.conf.loginname }}",
        ...
        if_pluckvalue_smart_join_not_equals_to_dataworkato_service3cd9c331requestempnumber_26 = rail.IfOperator(
            ...
            test=lambda dag_run: not is_employee_data_matching(
        ...
        log_forlookuplogs_64 = rail.PythonOperator(
            ...
            python_callable=lambda: 'User profile re-enabled' if rail.result('enable_user_63') else ''
        )

        if_request_accountstatus_equals_to_disabled_65 = rail.IfOperator(
            task_id='if_request_accountstatus_equals_to_disabled_65',
            test='''{{ dag_run.conf.accountstatus | lower == 'disabled' and dag_run.conf.leaveofabsence != 'LOA' }}''',
        ...
        log_forlookuplogs_70 = rail.PythonOperator(
            ...
            python_callable=lambda: 'User profile disabled' if rail.result('disable_userprofile_67', default=False) else ''
        )
        ...
        log_forlookuplogs_72 = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: 'User is already disabled' if dag_run.conf.get('accountstatus', '').lower() == 'disabled' and not rail.result('disable_userprofile_67', default=False) else ''
        )

        def get_log_meesage(dag_run):
            message_logs = []
            # Only collect logs from tasks that were actually executed
            if dag_run.conf['leaveofabsence'] == 'LOA':
                if rail.result('log_forlookuplogs_loa_disabled'):
                    message_logs.append(rail.result('log_forlookuplogs_loa_disabled'))
                if rail.result('log_forlookuplogs_loa_enabled'):
                    message_logs.append(rail.result('log_forlookuplogs_loa_enabled'))
            else:
                message_logs.append("User profile updated successfully")
                if rail.result('log_forlookuplogs_21'):
                    message_logs.append(rail.result('log_forlookuplogs_21'))
                if rail.result('log_forlookuplogs_25'):
                    message_logs.append(rail.result('log_forlookuplogs_25'))
                if rail.result('log_forlookuplogs_28'):
                    message_logs.append(rail.result('log_forlookuplogs_28'))
                if rail.result('log_forlookuplogs_31'):
                    message_logs.append(rail.result('log_forlookuplogs_31'))
                if rail.result('log_forlookuplogs_34'):
                    message_logs.append(rail.result('log_forlookuplogs_34'))
                if rail.result('log_forlookuplogs_41'):
                    message_logs.append(rail.result('log_forlookuplogs_41'))
                if rail.result('log_forlookuplogs_59'):
                    message_logs.append(rail.result('log_forlookuplogs_59'))
                if rail.result('log_forlookuplogs_64'):
                    message_logs.append(rail.result('log_forlookuplogs_64'))
                if rail.result('log_forlookuplogs_70'):
                    message_logs.append(rail.result('log_forlookuplogs_70'))
                if rail.result('log_forlookuplogs_72'):
                    message_logs.append(rail.result('log_forlookuplogs_72'))

            return rail.smartjoin_by_delim(message_logs, '|')

        log_forlogs_73 = rail.EmptyOperator(
            task_id='log_forlogs_73'
        )

        log_removingblankspaces_74 = rail.PythonOperator(
            task_id='log_removingblankspaces_74',
            python_callable=get_log_meesage
        )

        if_log_removingblankspaces_74_present_75 = rail.IfOperator(
            task_id='if_log_removingblankspaces_74_present_75',
            test=lambda: bool(rail.result("log_removingblankspaces_74")),
        ...
        nrdc_user_import_logs_add_entry_76 = rail.WriteLogOperator(
            ...
            message=lambda: rail.result('log_removingblankspaces_74'),
            severity="Success",
            properties={
                "user": "{{ dag_run.conf.firstname }} {{ dag_run.conf.lastname }} | {{ dag_run.conf.emailaddress }}",
                "action": "Update User",
                "status": "Success",
                "details": "{{ result('log_removingblankspaces_74') }}",
                "jobId": "{{ dag_run_ecid() }}"
            }
        )
        ...
        nrdc_user_import_logs_add_entry_78 = rail.WriteLogOperator(
            ...
            properties={
                ...
                "action": "Update User",
                "status": "Error",
                "details": "User Not Updated {{ get_error_message() }}",
                "jobId": "{{ dag_run_ecid() }}"
            }
        )
        ...
        can_run_batch_task >> rail.Label('No') >> if_disable_profile
        if_disable_profile >> rail.Label('Yes') >> update_user_loginname >> update_email >> disable_login >>\
            update_user_end_date >> log_user_disabled >> log_to_sumo
        if_disable_profile >> rail.Label('No') >> is_non_primary_user
        is_non_primary_user >> rail.Label('Yes') >> remove_email_for_non_primary_user >> update_user_replicon_authentication >> get_custom_fieldsforuser_3
        is_non_primary_user >> rail.Label('No') >> is_primary_user
        is_primary_user >> rail.Label('Yes') >> add_email_for_primary_user >> update_primary_user_login_name >> get_custom_fieldsforuser_3
        is_primary_user >> rail.Label('No') >> \
        ...
        if_to_date_to_time_greater_than_todayto_time_68 >> rail.Label(
            'No') >> log_forlookuplogs_70 >> log_forlogs_73
        if_userdetails_isenabled_is_true_enabled_66 >> rail.Label(
            'No') >> else_71 >> log_forlookuplogs_72 >> log_forlogs_73
        if_request_accountstatus_equals_to_disabled_65 >> rail.Label(
