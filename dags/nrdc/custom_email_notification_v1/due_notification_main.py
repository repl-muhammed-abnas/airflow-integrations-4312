...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.due_notification_master_dagid,
        description=f'NRDC Custom Email Notification Due Notification Master {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        schedule_interval=config.schedule_interval_due_notification,
        ...
        is_valid_schedule = rail.IfOperator(
            ...
            test= custom_methods.check_schedule(config),
            yes_task= 'get_required_due_date',
        ...
        get_reminder_number = rail.PythonOperator(
            task_id='get_reminder_number',
            python_callable=lambda: {'25': 1, '28': 2, '30': 3}.get(datetime.now(pytz.timezone("America/New_York")).strftime('%d'), 5)
        )
        ...
        trigger_send_due_notification_c4 = rail.trigger_parallel_dagrun(
            task_id = 'trigger_send_due_notification_c4',
            trigger_dag_id=config.due_notification_send_mail_c4_dagid,
            ...
            conf=lambda item: {
                ...
                'reminder_number': rail.result('get_reminder_number')
            }
        ...
        trigger_send_due_notification_c3 = rail.trigger_parallel_dagrun(
            task_id = 'trigger_send_due_notification_c3',
            trigger_dag_id=config.due_notification_send_mail_c3_dagid,
        ...
        is_valid_schedule >> rail.Label('Yes') >>\
        get_required_due_date >> get_reminder_number >> get_data_for_not_submitted_timesheets >> create_notsubmittedtimesheets_collection >> if_notsubmittedtimesheets_has_data
        if_notsubmittedtimesheets_has_data >> rail.Label('Yes') >> get_userlist_report_details >> generate_userlist_report >> parse_csv >>\
        create_collection_user_list_replicon >> query_get_unique_list_of_emails_c4 >> trigger_send_due_notification_c4 >>\
        query_get_unique_list_of_emails_c3 >> trigger_send_due_notification_c3 >> finish

        if_notsubmittedtimesheets_has_data >> rail.Label('No') >> finish

        is_valid_schedule >> rail.Label('No') >> delete_this_dagrun
    return dag
