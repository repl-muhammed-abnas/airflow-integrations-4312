import rail
from nrdc.custom_email_notification_v1.utils import request_payload, custom_methods
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.overdue_7_send_mail_c4_dagid,
        description=f'NRDC Custom Email Notification Overdue 7 Send mail c4{config.instance}',
        ...
        get_email_subject = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: "Reminder: Your " + datetime.strptime(dag_run.conf["timesheetperiod"].replace(" ", "").split("-")[0], "%m/%d/%Y").strftime("%B") + " Action Fund timesheet is now OVERDUE"
        )
        ...
        get_email_body = rail.RenderTemplateOperator(
            ...
            template_file='templates/overdue_7_notification/c4_notification_email.html',
            target='result',
        )
