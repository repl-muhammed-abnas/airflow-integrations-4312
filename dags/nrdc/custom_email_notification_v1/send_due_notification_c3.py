...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.due_notification_send_mail_c3_dagid,
        ...
        get_email_subject = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: "Reminder: Your " + datetime.strptime(dag_run.conf["timesheetperiod"].replace(" ", "").split("-")[0], "%m/%d/%Y").strftime("%B") + " lobbying timesheet is due on " + dag_run.conf["duedate"]
        )
        ...
        get_template_name = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: f"templates/due_notification/c3_notification_email_{dag_run.conf.get('reminder_number', 1)}.html"
        )
