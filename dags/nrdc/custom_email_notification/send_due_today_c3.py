import rail
from nrdc.custom_email_notification.utils import custom_methods
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.due_today_send_mail_c3_dagid,
        ...
        get_email_subject = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: "Reminder: Your " + datetime.strptime(dag_run.conf["timesheetperiod"].replace(" ", "").split("-")[0], "%m/%d/%Y").strftime("%B") + " lobbying timesheet is due TODAY."
        )
        ...
        get_email_body = rail.RenderTemplateOperator(
            ...
            template_file="templates/due_today/c3_notification_email.html",
            target='result',
        )
