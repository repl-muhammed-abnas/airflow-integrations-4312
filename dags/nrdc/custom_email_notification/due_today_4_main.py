...
import rail
from nrdc.custom_email_notification.utils import custom_methods, request_payload
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'nrdc_custom_email_notification_due_today_4pm_master_{config.instance}',
        description=f'NRDC Custom Email Notification Due Today 4PM Master {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        schedule_interval=config.schedule_interval_due_today_4,
        ...
        get_required_due_date = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: rail.get_replicon_date(datetime.now()) if not dag_run.conf.get("req_date") else dag_run.conf["req_date"]
        )
        ...
        trigger_send_due_today_notification_c4 = rail.trigger_parallel_dagrun(
            task_id = 'trigger_send_due_today_notification_c4',
            trigger_dag_id=config.due_today_send_mail_c4_dagid,
        ...
        trigger_send_due_today_notification_c3 = rail.trigger_parallel_dagrun(
            task_id = 'trigger_send_due_today_notification_c3',
            trigger_dag_id=config.due_today_send_mail_c3_dagid,
        ...
        create_collection_user_list_replicon >> query_get_unique_list_of_emails_c4 >> trigger_send_due_today_notification_c4 >>\
        query_get_unique_list_of_emails_c3 >> trigger_send_due_today_notification_c3 >> finish
