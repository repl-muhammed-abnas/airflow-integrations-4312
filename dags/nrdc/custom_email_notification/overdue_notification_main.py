...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'nrdc_custom_email_notification_overdue_master_{config.instance}',
        description=f'NRDC Custom Email Notification Overdue Master {config.instance}',
        ...
        get_required_due_date = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: rail.get_replicon_date(datetime.now() - timedelta(days=1)) if not dag_run.conf.get("req_date") else dag_run.conf["req_date"]
        )
