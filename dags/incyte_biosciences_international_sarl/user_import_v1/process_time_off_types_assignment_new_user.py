...
from incyte_biosciences_international_sarl.user_import_v1.utils.response_filter import get_filtered_time_off_types, get_policy_to_assign
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        for_each_time_off_assign_default_policy = rail.ForEachOperator(
            ...
            items=lambda: rail.result('get_enabled_time_off_types'),
            start_task='is_compensation_day_timeoff_type_and_country_germany_austria',
            end_task='for_each_time_off_assign_default_policy_end'
        )

        is_compensation_day_timeoff_type_and_country_germany_austria = rail.IfOperator(
            task_id='is_compensation_day_timeoff_type_and_country_germany_austria',
            test=lambda dag_run: rail.result('for_each_time_off_assign_default_policy')['timeoff_type_name'] == "Compensation Day" and
                dag_run.conf['country_name'] in ['Austria','Germany'],
            yes_task='get_custom_time_off_policy_schedule_compensation_day',
            no_task='get_default_time_off_policy_schedule'
        )

        get_custom_time_off_policy_schedule_compensation_day= rail.RepliconServiceOperator(
            task_id="get_custom_time_off_policy_schedule_compensation_day",
        ...
        put_user_timeoff_policy_germany_austria = rail.RepliconServiceOperator(
            task_id="put_user_timeoff_policy_germany_austria",
            ...
            data=lambda dag_run: {
                ...
                "policySetScheduleEntries": json.loads(rail.result('get_custom_time_off_policy_schedule_compensation_day'))
            }
        )
        ...
        get_default_time_off_policy_schedule = rail.RepliconServiceOperator(
            ...
            endpoint="/services/TimeOffPolicyService2.svc/GetDefaultTimeOffTypePolicyScheduleForUser",
            data=lambda dag_run:request_payload.get_default_timeoff_policy_schedule_payload(dag_run,"for_each_time_off_assign_default_policy"),
            data_handler=get_policy_to_assign
        )
        ...
        get_all_time_off_types >> get_enabled_time_off_types >> rail.Label('Yes') >> put_timeoff_assignment_for_user
        put_timeoff_assignment_for_user >> for_each_time_off_assign_default_policy
        for_each_time_off_assign_default_policy >> for_each_time_off_assign_default_policy_end

        for_each_time_off_assign_default_policy >> is_compensation_day_timeoff_type_and_country_germany_austria
        is_compensation_day_timeoff_type_and_country_germany_austria >> rail.Label('No') >> get_default_time_off_policy_schedule
        is_compensation_day_timeoff_type_and_country_germany_austria >> rail.Label('Yes') >> get_custom_time_off_policy_schedule_compensation_day
        get_custom_time_off_policy_schedule_compensation_day >> put_user_timeoff_policy_germany_austria >> for_each_time_off_assign_default_policy_end
