...
from intercontinentalxchange.user_import_v2.intercontinentalexchange_timezone_mapper import intercontinentalexchange_timezone_mapper
from intercontinentalxchange.user_import_v2.intercontinentalexchange_holiday_calendar_mapper import intercontinentalexchange_holiday_calendar_mapper
import rail
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'intercontinentalexchange_userimportmaster_v2_{config.instance}',
        description=f'IntercontinentalExchange_User import - Master V2.0 {config.instance}',
        ...
        if_query_list_identify_changedrecords_16_rows_greater_than_400_17 = rail.IfOperator(
            ...
            test=lambda: rail.result(
                "query_list_identify_changedrecords_16", "length") > config.threshold,
        ...
        trigger_dag_run_live_intercontinentalexchange_groups_check_location53 = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[-1],
            trigger_dag_id=f'intercontinentalexchange_groups_check_location_v2_{config.instance}',
        ...
        trigger_dag_run_live_intercontinentalexchange_process_groups_child_v1_055 = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[-1],
            trigger_dag_id=f'intercontinentalexchange_process_groups_child_v2_{config.instance}',
            execution_timeout=timedelta(days=14),
            conf=lambda: {
        ...
        def _already_updated():
            person_number = rail.result(
                'foreach_query_list_changedrecordswith_mandatoryfields_31_72')['person_number']
            triggered = Variable.get(
                config.triggered_var, default_var=[], deserialize_json=True)
            if person_number in triggered:
                return True
            return person_number in rail.result('processed_users')

        def _load_processed_users():
            if rail.get_current_context()['ti'].try_number == 1:
                Variable.set(config.triggered_var, [], serialize_json=True)
            try:
                ...
                return set()
            return set(
                r.get('properties', {}).get('Empid')
                for r in records
                if r.get('properties', {}).get('Status') == 'Success'
                and (r.get('properties', {}).get('Action') or '').lower() in ('update', 'add')
            )

        processed_users = rail.PythonOperator(
            task_id='processed_users',
            python_callable=_load_processed_users
        )
        ...
        if_output_useruri_blank_74 = rail.IfOperator(
            ...
            no_task="check_already_updated_idempotency_79",
        )

        check_already_updated_idempotency_79 = rail.PythonOperator(
            task_id='check_already_updated_idempotency_79',
            python_callable=_already_updated
        )

        if_already_updated_idempotency_79 = rail.IfOperator(
            task_id='if_already_updated_idempotency_79',
            test='''{{ result('check_already_updated_idempotency_79') | is_truthy }}''',
            yes_task="skip_already_processed_79",
            no_task="mark_empid_triggered_79",
        )

        skip_already_processed_79 = rail.EmptyOperator(
            task_id='skip_already_processed_79',
        )

        mark_empid_triggered_79 = rail.SetVariableOperator(
            task_id='mark_empid_triggered_79',
            append=True,
            name=config.triggered_var,
            value="{{ result('foreach_query_list_changedrecordswith_mandatoryfields_31_72').person_number }}"
        )
        ...
        trigger_dag_run_live_intercontinentalexchange_child_add_user_v1_0async_76 = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[-1],
            trigger_dag_id=f'intercontinentalexchange_child_adduser_v2_{config.instance}',
            execution_timeout=timedelta(days=14),
            conf=lambda item: {
                ...
                "timeofftemplateuri": rail.find_first_by_attr_and_get_attr(rail.result(
                    "get_all_policy_sets_get_all_policy_sets_48"), 'displayText', "Time Off", 'uri'),
        ...
        trigger_dag_run_live_intercontinentalexchange_user_update_v1_0async_80 = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[-1],
            trigger_dag_id=f'intercontinentalexchange_userupdate_v2_{config.instance}',
            execution_timeout=timedelta(days=14),
            conf=lambda item: {
                "employeeid": rail.result('foreach_query_list_changedrecordswith_mandatoryfields_31_72')['person_number'],
                "firstname": rail.result('foreach_query_list_changedrecordswith_mandatoryfields_31_72')['first_name']
        ...
        trigger_dag_run_live_intercontinentalexchange_child_supervisor_assignment_v1_0async_87 = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[-1],
            trigger_dag_id=f'intercontinentalexchangechild_supervisorassignment_v2_{config.instance}',
            execution_timeout=timedelta(days=14),
            conf=lambda dag_run: {
        ...
        send_log_mail_94 = rail.EmailOperator(
            ...
            to=config.tenant_email_for_user_import,
            bcc="{%- if result('get_logged_errors_94', key='length') > 0 -%}\
                    "+config.alert_email+"\
                {%- else -%}\
        ...
        if_generate_report_34_payload_not_starts_with_column_37 >> rail.Label('Yes') >> load_report_data_39 >> create_user_collection_40 >> \
            ...
            supervisor_processing_log >> declare_list_dag_runs_72 >> processed_users >> foreach_query_list_changedrecordswith_mandatoryfields_31_72 >> \
            invoke_custom_ruby_code_73 >> if_output_useruri_blank_74
        if_output_useruri_blank_74 >> rail.Label(
            'No') >> check_already_updated_idempotency_79 >> if_already_updated_idempotency_79
        if_already_updated_idempotency_79 >> rail.Label(
            'Yes') >> skip_already_processed_79 >> foreach_query_list_changedrecordswith_mandatoryfields_31_72_end
        if_already_updated_idempotency_79 >> rail.Label(
            'No') >> mark_empid_triggered_79 >> trigger_dag_run_live_intercontinentalexchange_user_update_v1_0async_80 >> insert_to_user_dag_run_list_80 >> \
