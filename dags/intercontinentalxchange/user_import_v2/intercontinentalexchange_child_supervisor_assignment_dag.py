...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'intercontinentalexchangechild_supervisorassignment_v2_{config.instance}',
        description=f'IntercontinentalExchange - Child_Supervisor Assignment V2.0 {config.instance}',
        ...
        if_request_supervisorempid_not_equals_to_dataworkato_service3cd9c331requestuserid_5 >> rail.Label('No') >> \
            log_errorfor_supervisorand_userslogin_nameissame_25 >> finish
        ...
        if_output_urioutput_blank_22 >> rail.Label(
            'No') >> catch_30 >> log_superviosr_process_errror_31 >> finish
