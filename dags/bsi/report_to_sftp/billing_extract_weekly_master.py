...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'bsi_billling_weekly_master_{config.instance}',
        description='BSI Extract Report Billing Weekly master',
        ...
        get_hourly_report_details = rail.RepliconReportDetailsOperator(
            ...
            report_name=config.extract_billing_weekly,
        )
        ...
        upload_report_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath='/'+ config.extract_billing_weekly_file_name +
            '{{ result("get_logging_details")["dag_start_time_weekly_monthly"] }}Weekly Extract.csv',
        )
        ...
        log_to_sumo = rail.DagRunLogToSumoOperator(
            ...
            extra_info={
                'filename': config.extract_billing_weekly_file_name + '{{ result("get_logging_details")["dag_start_time_weekly_monthly"] }}Weekly Extract.csv'
            }
        )
