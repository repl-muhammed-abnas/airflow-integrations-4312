...
from mammoet.time_data_import_v4.utils import custom_methods, response_filter
    ...
    with rail.create_airflow_dag(
        ...
        query_invalid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM raw_input_data
                    ...
                    NULLIF(starttime, '') IS NULL OR NULLIF(endtime, '') IS NULL OR
                    NULLIF(sourcesystem, '') IS NULL OR
                    (abs_att_type != "BRK" AND NULLIF(wbselement, '') IS NULL) OR
                    (abs_att_type == "BRK" AND NULLIF(wbselement, '') IS NOT NULL)""",
        ...
        has_any_valid_records = rail.IfOperator(
            ...
            yes_task="query_non_break_records",
            no_task="format_logs"
        )

        query_non_break_records = rail.QueryCollectionOperator(
            task_id="query_non_break_records",
            query="""SELECT * FROM valid_input_records WHERE abs_att_type != "BRK" """,
            name="non_break_records"
        )

        has_any_non_break_records = rail.IfOperator(
            task_id="has_any_non_break_records",
            test="{{ result('query_non_break_records','length') > 0}}",
            yes_task=["query_unique_users_from_input",
                      "query_unique_projects_from_input"],
            no_task="query_unique_users_from_input"
        )
        ...
        query_unique_projects_from_input = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT wbselement FROM non_break_records""",
        ...
        can_process_records = rail.IfOperator(
            ...
            test=lambda: bool(rail.result('create_project_report_collection')) if rail.result('query_non_break_records','length') > 0 else True,
        ...
        get_entryid_oef_uris = rail.RepliconServiceOperator(
            ...
            data_handler= response_filter.get_all_oef_details
        )
        ...
        get_attendence_type_oef_drop_down_values = rail.RepliconServiceCallForEachItemOperator(
            task_id="get_attendence_type_oef_drop_down_values",
            endpoint="/services/ObjectExtensionTagDefinitionService1.svc/GetObjectExtensionTagDefinitionDetails",
            items= '{{ result("get_entryid_oef_uris").attendence_type_oefs | to_json }}',
            data={"objectExtensionTagDefinitionUri": "{{ item.uri }}"},
            data_handler=lambda resp: {
                'oef_name': resp['name'],
                'dropdown_uri': rail.find_first_by_attr_and_get_attr(resp['tags'],'name','On site','uri')
            }
        )
        ...
        process_each_user_record = rail.trigger_parallel_dagrun(
            ...
            conf=lambda item: {
                ...
                **{
                    ...
                    "attendence_type_oef_uris": rail.result("get_entryid_oef_uris")['attendence_type_oefs'],
                    ...
                    "crane_capacity_oef_uri": rail.result("get_entryid_oef_uris")['crane_capacity'],
                    ...
                    "attendence_type_dropdown_values": rail.result("get_attendence_type_oef_drop_down_values"),
                }
            }
        )
        ...
        has_any_valid_records >> rail.Label("Yes") >> query_non_break_records >> has_any_non_break_records
        has_any_non_break_records >> rail.Label("Yes") >> [query_unique_users_from_input, query_unique_projects_from_input]
        has_any_non_break_records >> rail.Label("No") >> query_unique_users_from_input
        ...
        get_time_entry_all_filter_definitions >> get_attendence_type_oef_drop_down_values >> process_each_user_record
        process_each_user_record >> format_logs >> get_all__child_dag_run_ids >> gather_all_logs >> process_log_generation >> log_to_sumo >> can_fail_dag >> fail_dagrun
