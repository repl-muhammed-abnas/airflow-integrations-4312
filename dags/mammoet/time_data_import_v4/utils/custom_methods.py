...
def get_validation_error_message(item):
    # Check if BRK entry has project data (should not happen)
    if item.get('abs_att_type') == 'BRK' and item.get('wbselement'):
        return "Break entry should not contain project information"

    # Otherwise, return missing mandatory fields message
    return get_missing_field_message(item)
...
def map_user_details_with_feed_callable():
    ...
    project_report_data = rail.load_all_records(
        rail.result("create_project_report_collection")) if rail.result("create_project_report_collection") else []
    final_data = []
    ...
    for record in feed_data:
        ...
        # Skip project data filtering for BRK entries (they don't need project info)
        if record['abs_att_type'] == 'BRK':
            project_data = []
            task_details_via_code = {}
        else:
        ...
        final_data.append({
            ...
            **{
                ...
                "comments": record.get('comments', '').strip() if record.get('comments') else None
            }
        })
