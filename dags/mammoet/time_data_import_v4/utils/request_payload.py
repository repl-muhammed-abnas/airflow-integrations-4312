...
import rail
from mammoet.time_data_import_v4.utils import custom_methods
...
def get_custom_metadata(data):
    ...
    # Comment from the SAP feed takes priority; fall back to the existing comment
    # already on the time entry in Replicon (so updates don't wipe it out).
    source_comments = data.get('comments', '').strip() if data.get('comments') else None
    existing_comments = (rail.result("get_time_entry_details_for_update") and
                         rail.result("get_time_entry_details_for_update").get('comments'))

    comment_to_use = source_comments or existing_comments
    if comment_to_use:
        meta_data_list.append(get_metadata_payload(
            key_uri=f"{meta_data_key_uri}comments",
            text_value=comment_to_use
        ))
...
def get_timeentry_uri():
    ...
    } if rail.result("add_time_entry") else {
        "uri": rail.result("search_replicon_unique_id")['timeentryrevisiongroup'],
    } if rail.result("search_replicon_unique_id") else None
...
def add_entry_id_oefs(conf,action):
    ...
    if conf.get('user_location'):
        for item in conf.get('attendence_type_oef_uris', []):
            if item.get('location') in conf['user_location']:
                dropdown_values = next((x['dropdown_uri'] for x in conf.get('attendence_type_dropdown_values', [])
                    if x.get('oef_name') == item.get('oef_name')),[])
                attendence_type_details.append((item['uri'], dropdown_values))
    ...
    oef_list.append({
            "definition": {
                "uri": conf['crane_capacity_oef_uri']
            },
            "numericValue": conf['cranecapacity']
        }) if conf['cranecapacity'] else None
    ...
    oef_list.append({
            "definition": {
                "uri": attendence_type_details[0][0]
            },
            "tag": {
                "uri": attendence_type_details[0][1]
            }
            ...
            "tag": {
                "uri": rail.result("get_time_entry_details_for_update")['activity_uri'][0] if rail.result(
                    "get_time_entry_details_for_update")['activity_uri'] else attendence_type_details[0][1]
            }
        }) if attendence_type_details and rail.result("get_time_entry_details_for_update") else None
