...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        is_replicon_unique_id_available = rail.IfOperator(
            task_id='is_replicon_unique_id_available',
            test="{{ dag_run.conf.replicon_unique_id | is_truthy }}",
            yes_task='search_replicon_unique_id',
            no_task='is_replicon_id_available'
        )

        search_replicon_unique_id = rail.RepliconServiceOperator(
            task_id='search_replicon_unique_id',
            endpoint='/services/TimeEntryRevisionGroupService1.svc/GetTimeEntryRevisionGroupsDetails',
            data=lambda dag_run: {
                "timeEntryRevisionGroups": [
                    {
                        "uri": 'urn:replicon-tenant:'+ rail.get_tenant_slug() + ':time-entry-revision-group:' + dag_run.conf['replicon_unique_id'],
                    }
                ]
            },
            data_handler= response_filter.get_timeentries_list_replicon_unique_id,
            retries =0
        )

        is_search_replicon_unique_id_failed = rail.IfOperator(
             task_id = 'is_search_replicon_unique_id_failed',
             trigger_rule = "all_done",
             test=lambda: custom_methods.get_task_state("search_replicon_unique_id") in ("failed",'skipped') or rail.result(
                  "search_replicon_unique_id", "failure") is False,
             yes_task= 'is_replicon_id_available',
        ...
        is_timeentry_found = rail.IfOperator(
            ...
            test="{{ result('search_time_entry_by_id') | is_truthy or result('search_time_entry_by_counter_id') | is_truthy or \
                result('search_replicon_unique_id') | is_truthy }}",
        ...
        def check_reopen_timeentry():
            ...
            if rail.result('search_time_entry_by_counter_id'):
                return {
            ...
            return {
                    'status': rail.result('search_replicon_unique_id')['approvalstatus'] in ['Approved', 'Waiting for Approval'],
                    'timeentryrevisiongroup': rail.result('search_replicon_unique_id')['timeentryrevisiongroup']
                }
        ...
        def force_approve_payload():
            time_entry_uri = rail.result(
                ...
                'search_time_entry_by_counter_id')[0]['timeentryrevisiongroup'] if bool(
                    rail.result('search_time_entry_by_counter_id')) else rail.result(
                'search_replicon_unique_id')['timeentryrevisiongroup'] if bool(
                    rail.result('search_replicon_unique_id')) else rail.result('add_time_entry')['uri']
            return {
        ...
        is_timeentry_daterange_valid >> rail.Label(
            "Yes") >> is_timesheet_present >> rail.Label("Yes") >> get_timesheet_details >> is_replicon_unique_id_available
        is_replicon_unique_id_available >> rail.Label("Yes") >> search_replicon_unique_id >> is_search_replicon_unique_id_failed
        is_search_replicon_unique_id_failed >> rail.Label("Yes") >> is_replicon_id_available
        is_search_replicon_unique_id_failed >> rail.Label("No") >> catch_and_log_error
        is_replicon_unique_id_available >> rail.Label("No") >> is_replicon_id_available
        is_timeentry_daterange_valid >> rail.Label(
            "No") >> log_timeentry_date_outside_user_start_end_date >> rail.Label("On Failure") >> catch_and_log_error
        is_timesheet_present >> rail.Label("No") >> create_timesheet_for_period >> is_replicon_unique_id_available
