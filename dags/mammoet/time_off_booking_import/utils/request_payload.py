...
def get_child_conf(item, dag_run):
    ...
    def get_end_date(formatted_start_date, formatted_end_date):
        ...
        if end_date ==  datetime.strptime("9999-12-31",DATE_FORMAT) :
            return datetime.strftime(start_date + timedelta(days=60), DATE_FORMAT)
    ...
    return {
        ...
        'start_time': (item['start_time']).split('.')[0] if item['start_time'] and int(float(item['days'])) < 1 else null,
        'end_time': (item['end_time']).split('.')[0] if item['end_time'] and int(float(item['days'])) < 1 else null,
...
def get_specific_duaration(dag_run):
    if dag_run.conf['start_time'] and dag_run.conf['end_time']:
        start_time = datetime.strptime(dag_run.conf['start_time'], TIME_FORMAT)
        end_time = datetime.strptime(dag_run.conf['end_time'], TIME_FORMAT)
        return {
          "hours": 0,
          "minutes": 0,
