...
def get_activity_type_oef_uri(response):
    ...
    return [item['uri'] for item in oef_list if item] if oef_list else None

def get_entry_id_by_date_range(response,dag_run):
    if not response:
        return []
    return list(filter(lambda item: str(item['integerId']) == dag_run.conf['extdocumentno'],map(lambda res:{
        "timeentryrevisiongroup": res['uri'],
        "approvalstatus": res['approvalStatus']['displayText'],
        "integerId": res['integerId']},response)))
