...
from mammoet.payroll_export_mex.utils.custom_methods import create_json_payload_callable
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        query_records_to_post >> create_json_payload >> create_csv_file >> upload_export_data_to_sftp >> post_to_target\
            >> is_post_to_endpoint_failed >> rail.Label("No") >> is_run_failed >> rail.Label("Yes") >> fail_dag_run >> finish
        is_run_failed >> rail.Label("No") >> finish
        is_post_to_endpoint_failed >> rail.Label(
            "Yes") >> upload_to_backup_path >> send_posting_failed_email >> finish
