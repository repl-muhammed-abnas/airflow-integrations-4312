...
import rail
from mammoet.payroll_export_mex.utils.custom_methods import EXPORT_DATE_FORMAT
from mammoet.payroll_export_mex.utils.request_payload import get_payroll_location_uri_payload
from mammoet.payroll_export_mex.utils.response_filters import get_payroll_location_uri_filter


def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.payroll_export_daily_master_dag_id,
        description="Mammoet Payroll Export Daily Master",
        start_date=datetime(2026, 7, 1, tz=config.time_zone),
        ...
        trigger_daily_export = rail.TriggerDagRunOperator(
            task_id="trigger_daily_export",
            trigger_dag_id=config.payroll_export_process_payroll,
            conf=lambda: {
                    "payroll_export_run_type": "daily",
        ...
        get_payroll_location_uri >> trigger_daily_export
