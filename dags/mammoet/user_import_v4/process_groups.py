...
from mammoet.user_import_v4.utils.custom_methods import PARENT_LEGAL_ENTITY, LOCATION_DELIMITER, get_location_details_from_code
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        query_location = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT pd.location, pd.location_code, pd.legal_entity_code from valid_payload_data pd""",
        ...
        query_cost_center = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT pd.cost_center, pd.cost_center_code FROM valid_payload_data pd WHERE NULLIF(pd.cost_center, '') IS NOT NULL""",
        ...
        def get_cost_center_added():
            return {
                "cost_center_added": "Yes" if rail.result("query_cost_center_to_add", "length") > 0 else "No",
                "cost_center_added_count": rail.result("query_cost_center_to_add", "length"),
                "cost_center_list": rail.load_all_records(rail.result("query_cost_center_to_add"))
            }

        cost_center_added = rail.PythonOperator(
            task_id = "cost_center_added",
            python_callable=get_cost_center_added
        )
        ...
        query_cost_center >> get_replicon_cost_center_details >> create_cost_center_collection >> query_cost_center_to_add\
            >> has_any_cost_center_to_add >> rail.Label("Yes") >> add_cost_center_to_replicon >> cost_center_added
        has_any_cost_center_to_add >> rail.Label("No") >> cost_center_added
