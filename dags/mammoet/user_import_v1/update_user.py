...
import rail
from mammoet.user_import_v1.utils import request_payload, response_filter
from mammoet.user_import_v1.utils.custom_methods import get_timeoff_assignment_log_message
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        update_user = rail.RepliconServiceOperator(
            task_id = "update_user",
            endpoint="/services/ImportService1.svc/ApplyUserModifications2",
            data=request_payload.get_update_user_payload
        )
        ...
        is_location_updated = rail.IfOperator(
            ...
            test=lambda dag_run: request_payload.is_group_changed(dag_run, 'location'),
        ...
        is_status_inactive >> rail.Label("No") >>is_user_rehire >> rail.Label("Yes") >> enable_login >> get_user_details
        ...
        is_user_update_failed >> rail.Label("No") >> log_update_user_success >> is_location_updated\
            >> rail.Label("Yes") >> assign_timeoffs_user >> log_timeoffs_assignment >> log_for_supervisor_assignment
        is_location_updated >> rail.Label("No") >> log_no_timeoffs_to_assign >> log_for_supervisor_assignment >> rail.Label("On Error") >> catch_and_log_error
