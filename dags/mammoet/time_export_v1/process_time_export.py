...
import rail
from mammoet.time_export_v1.utils import custom_methods, request_payload
from mammoet.time_export_v1.tasks.time_export_task import time_data_export
from mammoet.time_export_v1.tasks.update_time_export_status import cancel_time_export
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        has_any_timeexport_data = rail.IfOperator(
            ...
            yes_task="query_blank_replicon_ids",
        ...
        query_blank_replicon_ids = rail.QueryCollectionOperator(
            task_id="query_blank_replicon_ids",
            query="""SELECT * FROM raw_timeexport_data rtd WHERE NULLIF(rtd.employee_id, '') IS NOT NULL
                        AND CAST(rtd.hours AS FLOAT) != 0 AND NULLIF(rtd.replicon_id, '') IS NULL""",
            name="blank_replicon_ids"
        )

        has_any_blank_replicon_ids = rail.IfOperator(
            task_id = "has_any_blank_replicon_ids",
            test = "{{result('query_blank_replicon_ids', 'length') > 0}}",
            yes_task="trigger_process_missing_replicon_ids",
            no_task="query_blank_employee_id_records"
        )

        trigger_process_missing_replicon_ids = rail.TriggerDagRunForEachItemOperator(
            task_id = "trigger_process_missing_replicon_ids",
            items=[0],
            trigger_dag_id=config.time_export_process_missing_replicon_ids_master_dag_id,
            conf=lambda : {
        ...
        has_any_records_to_export = rail.IfOperator(
            ...
            yes_task = "wait_for_trigger_process_missing_replicon_ids",
        ...
        # SAP (Client) sends the data back to Replicon again to update SAP counter ID for the manually done time-entry
        # As the Replicon ID update is done in async manner, it is causing duplication of entries
        # now we will send the data once all the Replicon ID updates are completed
        wait_for_trigger_process_missing_replicon_ids = rail.WaitForDagRunsSensor(
            task_id = "wait_for_trigger_process_missing_replicon_ids",
            dag_runs="{{result('trigger_process_missing_replicon_ids') or []}}",
            retries=0,
        ...
        has_any_timeexport_data >> rail.Label(
            "Yes") >> query_blank_replicon_ids >> has_any_blank_replicon_ids >> rail.Label(
            "No") >> trigger_process_missing_replicon_ids >> query_blank_employee_id_records

        has_any_blank_replicon_ids >> rail.Label(
            "No") >> query_blank_employee_id_records >> has_any_blank_emp_id >> rail.Label("Yes") >>\
        ...
        has_missing_data >> rail.Label("No") >> filter_raw_timeexport_data >> has_any_records_to_export\
            >> rail.Label("Yes") >> wait_for_trigger_process_missing_replicon_ids >> trigger_post_to_api >> wait_for_trigger_post_to_api >> finish
