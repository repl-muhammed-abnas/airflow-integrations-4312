...
def report_batch(config):
    with rail.TaskGroup(group_id='generate_report_batch', prefix_group_id=False):
        get_report_details = rail.RepliconReportDetailsOperator(
            ...
            report_name=config.shift_import_report
        )

        run_shift_import_report = rail.run_report2(
            group_id='run_shift_import_report',
            report_params={
                "reportParameters": [
                    {
        ...
        report_has_data = rail.IfOperator(
            ...
            test='{{ result("run_shift_import_report.get_report_result", "has_data") }}',
        ...
        expected_report_columns = 'User Name,Login Name,Country (Current),Schedule Name (Current),UserUri,User Status'
        ...
        check_report_column_order = rail.IfOperator(
            ...
            test='{{ result("run_shift_import_report.get_report_result").reportGenerationResults[0].payload | starts_with("%s") }}' % expected_report_columns,
        ...
        load_report_data = rail.LoadCSVFileOperator(
            ...
            document='{{ result("run_shift_import_report.get_report_result").reportGenerationResults[0].payload }}',
        )
        ...
        get_report_details >> run_shift_import_report >> report_has_data >> rail.Label(
