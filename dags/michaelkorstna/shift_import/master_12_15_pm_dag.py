...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'mk_shifts_master_recipe_12_15pm_{config.instance}',
        description=f'MK_Shifts_Master_recipe 12:15 PM {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=pendulum.datetime(2023, 1, 1, tz=config.time_zone),
        schedule_interval=config.schedule_interval_12_15_pm,
        max_active_runs=1,
        default_args={
