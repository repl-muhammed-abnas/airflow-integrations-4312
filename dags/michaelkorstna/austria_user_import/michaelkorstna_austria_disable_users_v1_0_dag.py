...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'michaelkorstna_austria_user_import_disable_users_child_{config.instance}',
        description=f'MichaelKorsTnA_Austria_disable users v1.0 {config.instance}',
        ...
        can_run_batch_task = rail.IfOperator(
            ...
            yes_task='batch_task',
            no_task='trigger_child_accrual_stoppage_for_users'
        )

        batch_task = rail.BatchTaskRunOperator(
            task_id='batch_task',
            start_task='trigger_child_accrual_stoppage_for_users',
            end_task='finish',
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
        )

        trigger_child_accrual_stoppage_for_users = rail.TriggerDagRunOperator(
            task_id='trigger_child_accrual_stoppage_for_users',
            retries=0,
            trigger_dag_id=f'michaelkorstna_austria_user_import_accural_stoppage_for_users_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days)
        )

        wait_for_child_accrual_stoppage_for_users = rail.WaitForDagRunsSensor(
            task_id='wait_for_child_accrual_stoppage_for_users',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs='{{ result("trigger_child_accrual_stoppage_for_users") }}'
        )
        ...
        query_list_getalltheuserswhoareenabledandhaveanenddateinpast_22 = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM  enableduserdata WHERE  enableduserdata.daydiff > 0 AND  enableduserdata.country = "Austria" """,
        )
        ...
        trigger_child_annual_leave_timeofftype_termination_proration_assignment = rail.TriggerDagRunOperator(
            task_id='trigger_child_annual_leave_timeofftype_termination_proration_assignment',
            retries=0,
            ...
            conf=lambda: {
                ...
                "userloginname": rail.result('foreach_enabled_user_with_past_enddate')['loginname'],
                "useruri": rail.result('foreach_enabled_user_with_past_enddate')['useruri'],
                "timeoffuri": rail.find_first_by_attr_and_get_attr(rail.result('get_all_time_off_types_7'), 'name', '[AT] Annual leave', 'uri', ''),
                "timeofftype": "[AT] Annual leave",
                "disabledate": rail.result('foreach_enabled_user_with_past_enddate')['enddate']
            }
        )

        insert_to_termination_assignment_list = rail.SetVariableOperator(
            task_id='insert_to_termination_assignment_list',
            name='termination_assignment_list',
            append=True,
            value="{{result('trigger_child_annual_leave_timeofftype_termination_proration_assignment')}}"
        )
        ...
        if_termination_assignment_child_triggered = rail.IfOperator(
            task_id='if_termination_assignment_child_triggered',
            test=lambda: len(rail.get_dag_run_var(
                'termination_assignment_list')),
            yes_task='wait_for_timeoff_termination_assignment_list',
            no_task='if_first_user_present_36'
        )

        wait_for_timeoff_termination_assignment_list = rail.WaitForDagRunsSensor(
            task_id = 'wait_for_timeoff_termination_assignment_list',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs="{{result('insert_to_termination_assignment_list').value | to_json}}"
        )

        check_for_errors_in_timeoff_termination_list = rail.GatherResultsFromDagRunsOperator(
            task_id='check_for_errors_in_timeoff_termination_list',
            dag_runs="{{result('insert_to_termination_assignment_list').value | to_json}}",
        ...
        if_first_user_present_36 = rail.IfOperator(
            ...
            test=lambda: (len(rail.get_dag_run_var('error_while_disabling')) > 0 if rail.result('accumulate_list_items_30') else False) or rail.result(
                'check_for_errors_in_timeoff_termination_list'),
            yes_task="stop_37",
            no_task="finish",
        )
        ...
        can_run_batch_task >> rail.Label(
            'No') >> trigger_child_accrual_stoppage_for_users
        trigger_child_accrual_stoppage_for_users >> wait_for_child_accrual_stoppage_for_users >> log_today_4 >> get_my_actual_user_identity_6
        ...
        if_first_error_present_15 >> rail.Label('Yes') >> stop_16 >> finish
        if_first_error_present_15 >> rail.Label(
            'No') >> if_first_payload_contains_nodata_17
        if_first_payload_contains_nodata_17 >> rail.Label('Yes') >> finish
        ...
        if_first_username_blank_23 >> rail.Label('Yes') >> finish
        ...
        trigger_child_timesheet_recalculation >> trigger_child_annual_leave_timeofftype_termination_proration_assignment
        trigger_child_annual_leave_timeofftype_termination_proration_assignment >> insert_to_termination_assignment_list
        insert_to_termination_assignment_list >> foreach_enabled_user_with_past_enddate_end
        if_user_loginname_equal_actual_user_identity >> rail.Label(
            'No') >> foreach_enabled_user_with_past_enddate_end
        foreach_enabled_user_with_past_enddate >> foreach_enabled_user_with_past_enddate_end >> if_termination_assignment_child_triggered
        if_termination_assignment_child_triggered >> rail.Label(
            'Yes') >> wait_for_timeoff_termination_assignment_list >> check_for_errors_in_timeoff_termination_list
        check_for_errors_in_timeoff_termination_list >> if_first_user_present_36
        if_termination_assignment_child_triggered >> rail.Label(
            'No') >> if_first_user_present_36
        if_first_user_present_36 >> rail.Label('Yes') >> stop_37 >> finish
        if_first_user_present_36 >> rail.Label('No') >> finish >> log_to_sumo
