...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'michaelkorstna_austria_user_import_annual_leave_timeoff_type_termination_proration_assignment_child_{config.instance}',
        description=f'MichaelKorsTnA Austria_Child Annual Leave Timeoff type Termination Proration Assignment v1.0 {config.instance}',
        ...
        log_gettheaccrualbalance_15 = rail.PythonOperator(
            ...
            python_callable=lambda:  float(rail.find_first_by_attr_and_get_attr(json.loads(rail.result(
                'log_gettheaccrualbalancesetup_12')), 'keyUri', 'urn:replicon:script-key:parameter:accrual-annual-amount', 'value.number', ''))
        )
        ...
        invoke_custom_ruby_code_21 = rail.PythonOperator(
            ...
            python_callable=lambda: custom_methods.get_date_object(
        ...
        get_time_off_taken_series_for_user_23 = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response: [{
                ...
                'minutes': int(data['timeTaken']['minutes'])
        ...
        log_requiredtermination_proration_balance_27 = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: round(float((float(rail.result(
                ...
                'log_required_numberofdaysforprorationcalculation_20')))) - float(rail.result('log_numberof_booking_hours_26'))
        )
        ...
        if_timeofftype_name_equals_to_dataworkato_servicereceive_requestrequesttimeofftype_7 >> rail.Label(
            'No') >> foreach_d_6_end
        foreach_d_6 >> foreach_d_6_end >> log_gettheaccrualbalance_15 >> log_gettheaccrual_day_16 >> log_gettheaccrual_month_17 >> log_accrual_day_18
