...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'michaelkorstna_austria_user_import_add_user_child_{config.instance}',
        description=f'MichaelKorsTnA Austria_ Add V1.0 {config.instance}',
        ...
        def get_required_values():
            ...
            punchentrypolicydraft = (list(filter(lambda x: x['type'] == 'Punch Entry Policy' and x['identifier__1'] == 'Default', mapper_entries)))
            timesheettemplatedraft = (list(filter(lambda x: x['type'] == 'Timesheet Template' and x['identifier__1'] == 'Default', mapper_entries)))
            payruledraft = (list(filter(lambda x: x['type'] == 'Payrule' and x['identifier__1'] == 'Default', mapper_entries)))
            officescheduledraft = (list(filter(lambda x: x['type'] == 'Schedule' and x['identifier__1'] == 'Default', mapper_entries)))
            userpermissionsetdraft = list(filter(lambda x: x['type'] == 'Permission' and x['identifier__1'] == 'User',mapper_entries))
            return {
                'timesheetperiod': rail.find_first_by_attr_and_get_attr(mapper_entries, 'type', 'Timesheet Period Type', 'value', ''),
                'timeofftemplate': rail.find_first_by_attr_and_get_attr(mapper_entries, 'type', 'Time Off Template', 'value', ''),
                ...
                'timeoffapprovalpath': rail.find_first_by_attr_and_get_attr(mapper_entries, 'type', 'Time Off Apporval Path', 'value', ''),
                'timesheetapprovalpath': rail.find_first_by_attr_and_get_attr(mapper_entries, 'type', 'Timesheet Approval Path', 'value', ''),
                'holidaycalendar': rail.find_first_by_attr_and_get_attr(mapper_entries, 'type', 'Holiday Calendar', 'value', ''),
                ...
                'licenses': [item['default__uri'] for item in list(filter(lambda x: x['type'] == 'Licenses', mapper_entries))],
                ...
                'supervisorpermission': [entry['value'] for entry in (list(filter(lambda x: x['type'] == 'Permission' and x[
                    'identifier__1'] == 'Supervisor', mapper_entries)))]
            }
        ...
        if_request_managerid_present_40 = rail.IfOperator(
            ...
            yes_task="search_users_search_supervisor_41",
        ...
        trigger_child_add_timeoff_type_for_new_user = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'michaelkorstna_austria_child_workflow_to_add_timeoff_type_for_new_user_child_{config.instance}',
        ...
        if_request_managerid_present_40 >> rail.Label(
            'Yes') >> search_users_search_supervisor_41 >> if_log_supervisor_uri_42_blank_43
