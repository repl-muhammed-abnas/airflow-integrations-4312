...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'michaelkorstna_austria_user_import_add_foreign_supervisor_child_{config.instance}',
        description=f'MichaelKorsTnA Austria_Child_Add Foreign Supervisor V1.0 {config.instance}',
        ...
        search_mappings_for_foreignsupervisors = rail.PythonOperator(
            ...
            python_callable=lambda:  list(filter(
                lambda x: x["country"] == "Foreign Supervisors", michael_kors_gmbh_user_sync_master_mapper_austria))
        )
        ...
        def get_requiredfields_values():
            ...
            return {
                ...
                'licenses': [license['default__uri'] for license in list(filter(lambda entry: entry['type'] == 'Licenses', mapperentries))],
