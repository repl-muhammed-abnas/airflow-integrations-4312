...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'michaelkorstna_austria_child_workflow_to_add_timeoff_type_for_new_user_child_{config.instance}',
        description=f'MichaelKorsTnA Austria_Child Workflow to add timeoff type for new user v1.0 {config.instance}',
        ...
        search_timeofftype_entries_from_mapper = rail.PythonOperator(
            ...
            python_callable=lambda: list(filter(
                lambda entry: entry['country'] == 'Austria' and entry['type'] == 'Timeoff Type', michael_kors_gmbh_user_sync_master_mapper_austria))
        )
        ...
        if_foreach_1_name_not_equals_to_atannualleave_22 = rail.IfOperator(
            task_id='if_foreach_1_name_not_equals_to_atannualleave_22',
            test='''{{ result('foreach_declare_list_5_17').name != '[AT] Annual leave' }}''',
        ...
        trigger_timeoff_type_proration_assignment_child = rail.TriggerDagRunOperator(
            ...
            conf=lambda dag_run: {
                ...
                "scheduledweeklyhours": (38.5 if float(dag_run.conf['scheduledweeklyhours']) >= 38.5 else dag_run.conf['scheduledweeklyhours']) if dag_run.conf[
                    'scheduledweeklyhours'] else 38.5,
                "fullpart": ("Full Time" if float(dag_run.conf['scheduledweeklyhours']) >= 38.5 else "Part Time") if dag_run.conf[
        ...
        get_default_time_off_type_policy_schedule_for_user_20 >> if_foreach_1_name_not_equals_to_atannualleave_22
        if_foreach_1_name_not_equals_to_atannualleave_22 >> rail.Label(
