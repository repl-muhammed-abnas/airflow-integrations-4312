"""
Process delta time-off records from Replicon and prepare them for Workday export.

This DAG extracts delta time-off records that haven't been exported yet,
...
def create_dag(config):
    """Create DAG for processing delta time-off records."""

    with rail.create_airflow_dag(
        dag_id=config.extract_delta_bookings_child_dag_id,
        description=f'Michaelkors extract delta timeoff records {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.max_active_delta_records_runs
    ) as dag:
        ...
        logging_details = rail.PythonOperator(
            ...
            op_args=[config.time_zone, 'Delta', config.export_file_prefix_delta]
        )
        ...
        create_export = rail.RepliconServiceOperator(
            ...
            data=request_payload.create_time_data_export_batch_payload_delta
        )
        ...
        log_invalid_records = rail.WriteLogOperator(
            ...
            properties=lambda item, dag_run:{
                ...
                'transactiontype|childjob': f"Delta|{get_dagrun_ecid(dag_run)}"
            }
        )
