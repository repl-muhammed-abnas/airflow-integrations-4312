...
RETAIL_HOLIDAY_ENTITLEMENT_MAP = {
    40: 264, 38: 250.8, 32: 211.2, 30: 198, 28: 184.8,
    25: 165, 24: 158.4, 22: 145.2, 20: 132, 16: 105.6,
    13: 85.8, 12: 79.2, 10: 66, 8: 52.8, 6: 39.6, 4: 26.4
}
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.holiday_proration_child_dag_id,
        description=f'MichaelKorsTnA UK_Child Timeoff type UK Holiday Proration Assignment (RIT-17599) {config.instance}',
        ...
        def calculate_required_accrual(dag_run):
            scheduled_hours = float(dag_run.conf['scheduledweeklyhours'])
            default_accrual = float(rail.result('log_gettheaccrualbalance_9'))
            is_retail_raw = dag_run.conf.get('is_retail_location', False)
            is_retail = is_retail_raw is True or str(is_retail_raw).lower() == 'true'
            if is_retail and scheduled_hours in RETAIL_HOLIDAY_ENTITLEMENT_MAP:
                return RETAIL_HOLIDAY_ENTITLEMENT_MAP[scheduled_hours]
            return round(float(scheduled_hours / 40) * default_accrual)

        log_required_accrual_16 = rail.PythonOperator(
            task_id='log_required_accrual_16',
            python_callable=calculate_required_accrual
        )
