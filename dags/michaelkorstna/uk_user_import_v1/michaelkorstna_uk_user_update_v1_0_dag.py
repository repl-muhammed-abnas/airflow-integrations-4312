...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.user_update_child_dag_id,
        description=f'MichaelKorsTnA UK User Update (RIT-17599) {config.instance}',
        ...
        trigger_child_holiday_timeofftype_termination_proration_assignment = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=config.holiday_termination_proration_child_dag_id,
        ...
        trigger_child_timesheet_recalculation = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=config.timesheet_recalculation_child_dag_id,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        trigger_child_add_user = rail.TriggerDagRunOperator(
            ...
            conf={
                ...
                "is_retail_location": "{{ dag_run.conf.get('is_retail_location', False) }}"
            }
        )
        ...
        trigger_child_holiday_timeoff_type_proration_assignment = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=config.holiday_proration_child_dag_id,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda dag_run: {
                "callerjobid": dag_run.conf['callerjobid'],
                "userloginname": dag_run.conf['employeeid'],
        ...
        trigger_child_timeoff_type_proration_assignment = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=config.sick_leave_proration_child_dag_id,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda dag_run: {
                "callerjobid": dag_run.conf['callerjobid'],
                "userloginname": dag_run.conf['employeeid'],
