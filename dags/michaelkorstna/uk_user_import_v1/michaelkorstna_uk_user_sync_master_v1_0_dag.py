...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        description=f'MichaelKorsTnA UK User Sync Master (RIT-17599) {config.instance}',
        ...
        def is_location_retail(location_fullpath):
            if not location_fullpath:
                return False
            path_parts = location_fullpath.split('/')
            return any(part.strip() in ('Retail', 'Retail Development') for part in path_parts)
        ...
        def get_add_update_user_payload(action):
            ...
            payload = {
                ...
                "is_retail_location": is_location_retail(rail.result('invoke_custom_ruby_code_47')['requiredlocation']),
        ...
        trigger_child_add_foreign_supervisor = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=config.add_foreign_supervisor_child_dag_id,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf={
        ...
        trigger_child_add_supervisor = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.add_supervisor_child_dag_id,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: {
