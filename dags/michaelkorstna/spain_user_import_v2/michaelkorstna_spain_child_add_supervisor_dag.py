...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'michaelkorstna_spain_user_import_add_supervisor_child_{config.instance}_{config.version}',
        description=f'MichaelKorsTnA Spain_Child_Add Supervisor {config.instance}_{config.version}',
        ...
        get_permissions_from_mapper = rail.PythonOperator(
            ...
            python_callable=lambda dag_run:  list({permission['value'] for permission in list(filter(lambda x: x["type"] == "Permission" and x[
                "identifier___1"] == "Supervisor" and x["country"] == dag_run.conf['country'], michael_kors_gmbh_user_sync_master_mapper_spain))})
        )
