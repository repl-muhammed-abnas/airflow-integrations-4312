...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'michaelkorstna_spain_user_update_child_{config.instance}_{config.version}',
        description=f'MichaelKorsTnA Spain User Update {config.instance}_{config.version}',
        ...
        trigger_child_timesheet_recalculation = rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'michaelkorstna_spain_user_import_timesheet_recalculation_child_{config.instance}_{config.version}',
    ...
    "payRuleScript": {
        ...
        if_scheduled_weekly_hours_or_location_has_changed = rail.IfOperator(
            ...
            yes_task='search_officeschedule_in_mapper_based_on_location',
            no_task='if_timeofftrigger_equal_yes'
        )

        def get_officeschedule_from_mapper_based_on_location(dag_run):
            officescheduleentry = list(filter(lambda entry: entry['location'] == dag_run.conf['location'],michaelkorstna_schedulemapper_spain))
            return officescheduleentry[0]['office_schedule_name'] if officescheduleentry else ''

        search_officeschedule_in_mapper_based_on_location = rail.PythonOperator(
            task_id = 'search_officeschedule_in_mapper_based_on_location',
            python_callable=get_officeschedule_from_mapper_based_on_location
        )

        if_mapper_entry_found = rail.IfOperator(
            task_id = 'if_mapper_entry_found',
            test=lambda: bool(rail.result('search_officeschedule_in_mapper_based_on_location')),
            yes_task='if_scheduledweeklyhours_present',
            no_task = 'update_schedule_assignment_shift'
        )
        ...
        if_scheduledweeklyhours_present = rail.IfOperator(
            ...
            yes_task='search_officeschedule_in_mapper_based_on_location_scheduledweeklyhours',
            no_task='if_timeofftrigger_equal_yes'
        )
        
        def get_officeschedule_from_mapper_based_on_location_scheduledweeklyhours(dag_run):
        ...
        search_officeschedule_in_mapper_based_on_location_scheduledweeklyhours = rail.PythonOperator(
            task_id = 'search_officeschedule_in_mapper_based_on_location_scheduledweeklyhours',
            python_callable=get_officeschedule_from_mapper_based_on_location_scheduledweeklyhours
        )
        
        if_mapper_entry_found_location_scheduledweeklyhours = rail.IfOperator(
            task_id = 'if_mapper_entry_found_location_scheduledweeklyhours',
            test=lambda: bool(rail.result('search_officeschedule_in_mapper_based_on_location_scheduledweeklyhours')),
            yes_task='if_required_schedule_unequal_current',
            no_task = 'update_schedule_assignment_default'
        )

        if_required_schedule_unequal_current = rail.IfOperator(
            task_id = 'if_required_schedule_unequal_current',
            test=lambda dag_run: dag_run.conf['currentschedule'] != rail.result('search_officeschedule_in_mapper_based_on_location_scheduledweeklyhours'),
                ...
                "modifications": {
                    ...
                    "updateScheduleOverDateRange": {
                        "replacementScheduleEntries": [
                        {
                            ...
                            "name": "{{ result('search_officeschedule_in_mapper_based_on_location_scheduledweeklyhours') }}",
                            "officeSchedule": {
                                "officeScheduleUri": null,
                                "name": "{{ result('search_officeschedule_in_mapper_based_on_location_scheduledweeklyhours') }}"
                            },
        ...
        update_schedule_assignment_default = rail.RepliconServiceOperator(
            task_id = 'update_schedule_assignment_default',
            endpoint="/services/ImportService1.svc/ApplyUserModifications2",
            data={
                "user": {
                    "uri": "{{ dag_run.conf.useruri }}",
        ...
        update_schedule_assignment_shift = rail.RepliconServiceOperator(
            task_id = 'update_schedule_assignment_shift',
            endpoint="/services/ImportService1.svc/ApplyUserModifications2",
            data={
                "user": {
                    "uri": "{{ dag_run.conf.useruri }}",
        ...
        log_cost_center_updated >> if_location_present
        if_location_present >> rail.Label('Yes') >> if_scheduled_weekly_hours_or_location_has_changed
        if_scheduled_weekly_hours_or_location_has_changed >> rail.Label('Yes') >> search_officeschedule_in_mapper_based_on_location >> if_mapper_entry_found
        if_mapper_entry_found >> rail.Label('Yes') >> if_scheduledweeklyhours_present
        if_mapper_entry_found >> rail.Label('No') >> update_schedule_assignment_shift >> if_timeofftrigger_equal_yes
        if_scheduledweeklyhours_present >> rail.Label('Yes') >> search_officeschedule_in_mapper_based_on_location_scheduledweeklyhours \
            >> if_mapper_entry_found_location_scheduledweeklyhours
        if_scheduledweeklyhours_present >> rail.Label('No') >> if_timeofftrigger_equal_yes
        if_mapper_entry_found_location_scheduledweeklyhours >> rail.Label('Yes') >> if_required_schedule_unequal_current
        if_mapper_entry_found_location_scheduledweeklyhours >> rail.Label('No') >> update_schedule_assignment_default >> if_timeofftrigger_equal_yes
        ...
        if_location_present >> rail.Label('No') >> if_timeofftrigger_equal_yes
