# dags/vp_xero_integration_v2/payment_sync/dispatcher_dag.py
"""Dispatcher DAG for Xero -> VP Payment Sync (V2 IPA GitSync architecture).
...
V2 changes from V1:
  ...
  - xero_conn_id from get_connections(config) (not Jinja dag_run.conf)
  - connections/customerId in build_payment_processor_conf from config (not dag_run.conf)
...
import rail
from vp_xero_integration_v2.payment_sync.config import (
    watermark_variable_key_template,
)
from vp_xero_integration_v2.common.python_callable_method import get_connections
from vp_xero_integration_v2.payment_sync.utils.python_callable_method import (
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'vp_xero_payment_sync_v2_dispatcher_{config.instance}',
        description=(
        ...
        def _get_processor_dag_id(item):
            if (item or {}).get('PaymentType') == 'ACCRECPAYMENT':
                return f'vp_xero_invoice_payment_v2_processor_{config.instance}'
            return f'vp_xero_bill_payment_v2_processor_{config.instance}'
        ...
        (
            prepare_timestamps >> poll_xero_payments >>
            prepare_payment_items >> trigger_payment_processors >>
            wait_for_payment_processors >> gather_processor_errors >> has_sync_errors
        )
