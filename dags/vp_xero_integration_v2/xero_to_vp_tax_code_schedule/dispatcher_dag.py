# dags/vp_xero_integration_v2/xero_to_vp_tax_code_schedule/dispatcher_dag.py
"""Dispatcher DAG for Xero -> VP Tax Code Schedule (Workato 014-501 variant, V2 IPA GitSync).

Per-tenant: on schedule, triggers one processor DAG with connections and
customerId from the instance config. Mirrors xero_to_vp_tax_code_schedule/dispatcher_dag.py
but uses a distinct dag_id prefix.
...
def create_dag(config):
    """Per-tenant dispatcher: trigger one xero-to-vp tax code processor, gather errors."""
    connections = get_connections(config)
    with rail.create_airflow_dag(
        dag_id=f'vp_xero_xero_to_vp_tax_code_schedule_v2_dispatcher_{config.instance}',
        description='Per-customer Xero -> VP Tax Code sync (014-501 variant) on schedule',
        ...
        tags=['vantagepoint_xero', 'xero_to_vp_tax_code_schedule', 'dispatcher'],
        default_args={
        ...
        trigger_tax_code_sync = rail.TriggerDagRunForEachItemOperator(
            ...
            items=lambda: [{}],
            trigger_dag_id=(
                f'vp_xero_xero_to_vp_tax_code_schedule_v2_processor_{config.instance}'
            ),
        ...
        has_sync_errors = rail.IfOperator(
            ...
            yes_task='fail_xero_to_vp_tax_code_schedule',
            no_task='log_sync_complete',
        )

        fail_xero_to_vp_tax_code_schedule = rail.FailOperator(
            task_id='fail_xero_to_vp_tax_code_schedule',
            message=(
        ...
        has_sync_errors >> rail.Label('Yes') >> fail_xero_to_vp_tax_code_schedule
        has_sync_errors >> rail.Label('No') >> log_sync_complete
        fail_xero_to_vp_tax_code_schedule >> post_dag_run_details
