# dags/vp_xero_integration_v2/posted_unit_transaction_sync/dispatcher_dag.py
"""Dispatcher DAG for VP -> Xero Posted Unit Transaction Sync (V2 IPA GitSync architecture).
...
(no router). TransType='un' (unit transactions).
...
import rail
from vp_xero_integration_v2.posted_unit_transaction_sync.config import (
    TRANS_TYPE,
...
from vp_xero_integration_v2.posted_unit_transaction_sync.utils.python_callable_method import (  # noqa: E501
...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=(
            f'vp_xero_posted_unit_transaction_sync_dispatcher_{config.instance}'
        ),
        description=(
        ...
        log_no_records = rail.PythonOperator(
            ...
            python_callable=lambda: _log.info(
                'No new/updated PSA unit-transaction records in this poll window.'
            )
        )
        ...
        # trigger_rule='all_done': watermark advances even when some unit
        # transaction exports fail. Intentional — a record that fails
        # persistently would otherwise re-enter every subsequent poll window
        # indefinitely. Failures are reported to middleware via
        # PostDagRunDetailsToMiddlewareApiOperator so ops can investigate;
        # the DAG itself is still marked failed by FailOperator.
