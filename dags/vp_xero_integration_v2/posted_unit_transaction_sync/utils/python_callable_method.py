"""
Common utility methods for VP -> Xero Posted Unit Transaction Sync (V2).

Translates the Workato recipes (poll + UN export) into Python callables
...
2. Narration is a deterministic `"UN {Period} {PostSeq}"` instead of the
...
create_manual_journal block ~line 4362). This Workato recipe export is
...
provides the 5 recipe-specific wrappers (narration prefix 'UN', XCom
task 'fetch_unit_transfer_rows', trans_type 'un').
"""
...
# ---------------------------------------------------------------------------
# Recipe-specific wrappers (unit transfer: XCom='fetch_unit_transfer_rows', 'UN')
# ---------------------------------------------------------------------------
def _fetched_rows():
    """All PSA Ledger rows for this (Period, PostSeq)."""
    return _base_fetched_rows('fetch_unit_transfer_rows')
...
def log_no_rows_to_post():
    ...
    return _base_log_no_rows_to_post('un')


def log_missing_account_mapping():
    """Missing account-code mapping for one or more resolved rows."""
    return _base_log_missing_account_mapping('unit transfer')


def build_manual_journal_body():
    """Construct the Xero ManualJournal POST body (Narration: 'UN …')."""
    return _base_build_manual_journal_body('UN')
