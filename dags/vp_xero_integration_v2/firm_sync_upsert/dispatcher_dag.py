"""
Dispatcher DAG for VP -> Xero Firm Sync Upsert (V2 IPA GitSync architecture).
...
V2 changes from V1:
  ...
  - vp_conn_id from config.connections (not Jinja dag_run.conf)
  - connections/customerId in build_process_firm_conf injected from config
...
import rail
from vp_xero_integration_v2.firm_sync_upsert.config import (
...
from vp_xero_integration_v2.firm_sync_upsert.utils.python_callable_method import (
    ...
    build_process_firm_conf as _build_process_firm_conf,
    has_sync_errors,
)
...
def create_dag(config):
    ...
    def build_process_firm_conf(item):
        conf = _build_process_firm_conf(item)
