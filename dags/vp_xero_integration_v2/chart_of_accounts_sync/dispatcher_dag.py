"""
Dispatcher DAG for Xero -> VP Chart of Accounts Sync (V2 IPA GitSync architecture).

Per-tenant: applies the polling watermark, asks Xero for accounts changed since
the last run (`If-Modified-Since`), drops BANK accounts, fetches the full VP
/Accounts list (slimmed to a code/name/type index), and triggers the processor
DAG per Xero account. Replaces the Workato `updated_account` polling trigger on
`014_501_psa_poll_xero_chart_of_accounts` and the data-loading front half of
`014_501_psa_sync_accounts`.

V2 changes from V1:
  - schedule_interval from config.schedule_interval (not None)
  - xero_conn_id / vp_conn_id from config.connections (not Jinja dag_run.conf)
  - connections/customerId in build_processor_dag_conf injected from config
...
import rail
from vp_xero_integration_v2.chart_of_accounts_sync.config import (
...
from vp_xero_integration_v2.chart_of_accounts_sync.utils.python_callable_method import (  # noqa: E501
    extract_account_list_method,
    build_vp_account_index_method,
    check_if_accounts_exist_method,
    build_processor_dag_conf as _build_processor_dag_conf,
)
...
def create_dag(config):
    """Per-tenant dispatcher: poll Xero accounts, fan out, gather, advance watermark."""
    ...
    def build_processor_dag_conf(item):
        conf = _build_processor_dag_conf(item)
        conf['connections'] = connections
        conf['customerId'] = config.customer_id
        return conf

    connections = get_connections(config)
    with rail.create_airflow_dag(
        dag_id=f'vp_xero_chart_of_accounts_sync_dispatcher_{config.instance}',
        description=(
            'Poll Xero for changed accounts and trigger the per-account '
        ...
        # Xero /Accounts has no server-side "changed in window" query (unlike
        # QBO's SQL); the parity match for Workato's poll trigger is the
        # If-Modified-Since header set to the last watermark. First run
        # (watermark = initial_sync_time) backfills the full chart of accounts.
        get_recently_changed_accounts = rail.XeroAccountOperator(
        ...
        log_no_accounts = rail.PythonOperator(
            ...
            python_callable=lambda: logger.info(
                "No changed Xero accounts in this poll window (%s to %s)",
        ...
        process_accounts = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=(
                f'vp_xero_chart_of_accounts_sync_processor_{config.instance}'
            ),
