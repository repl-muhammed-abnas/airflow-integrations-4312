"""Shared Python callable helpers for vp_xero_integration_v2.

Home for methods reused across the integration DAGs (vendor_sync,
journal_entry_sync, chart_of_accounts_sync, ap_voucher, ...). Lives in
`common` — alongside `common.tables` and `common.config`, which already deploy +
import cross-integration — so a fix is made in ONE place instead of being copied
into every integration's utils module.
...
# Every per-customer key across these integrations is
# `vp_xero_<suffix>_<customer>_<instance>`, so the prefix lives here once —
# a format change is a one-line edit, not an edit in every integration's
# config/dispatcher.
_VARIABLE_KEY_PREFIX = 'vp_xero'
...
def build_watermark_variable_key(template, instance, customer_id):
    ...
    `'vp_xero_{customer_id}_<integrationType>_sync_last_run'`) and must contain the
...
def build_customer_variable_key(customer_id, suffix):
    """Canonical per-customer Airflow Variable key: `vp_xero_<customer>_<suffix>`.
...
def watermark_key_template(integration, variable_name='last_run'):
    ...
    `'vp_xero_{customer_id}_vendor_sync_last_run'`. Pass `variable_name` for any
    other key following the `vp_xero_{customer_id}_<integration>_<variable_name>`
    convention, e.g. `watermark_key_template('vendor_sync', 'high_watermark')`
    -> `'vp_xero_{customer_id}_vendor_sync_high_watermark'`.
...
def get_connections(config) -> dict:
    """Resolve the connections dict from an instance config.

    IPA GitSync emits flat vantagepoint_conn_id / xero_conn_id attrs in
    generated instance files. Hand-authored files may use a connections = {}
    dict. This bridges both so dispatcher DAGs work with either format.

    Raises ValueError when neither form is present so a misconfigured instance
    fails at DAG parse time (visible in the Airflow UI) rather than silently
    passing None connection IDs downstream.
    """
    conns = getattr(config, 'connections', None)
    if conns:
        return conns
    vp_conn = getattr(config, 'vantagepoint_conn_id', None)
    xero_conn = getattr(config, 'xero_conn_id', None)
    if not vp_conn and not xero_conn:
        raise ValueError(
            "Instance config exposes neither a 'connections' dict nor flat "
            "'vantagepoint_conn_id' / 'xero_conn_id' attributes; cannot "
            "resolve connection IDs."
        )
    return {'vantagepoint': vp_conn, 'xero': xero_conn}
