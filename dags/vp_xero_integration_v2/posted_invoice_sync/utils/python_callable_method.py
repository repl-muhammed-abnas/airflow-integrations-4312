"""
Python callable methods for VP PSA -> Xero Posted Invoices Sync (V2).
    ...
    for key in invoice_order:
        ...
        for row in detail_by_invoice.get(invoice, []):
            ...
            if not xero_account_code:
                logger.warning(
                    "No Xero account mapping for VP code '%s' (Invoice %s.%s.%s) — skipping line",
...
def check_invoice_exists_method():
    ...
    op = rail.XeroInvoiceOperator(
        ...
        operation='search',
        where='InvoiceNumber="{}"'.format(invoice_number.replace('"', '""')),
        paginate=False,
    )
...
def allocate_credit_note_method():
    ...
    search_op = rail.XeroInvoiceOperator(
        ...
        xero_conn_id=xero_conn_id,
        operation='search',
        where='InvoiceNumber="{}"'.format(original_invoice_number.replace('"', '""')),
        paginate=False,
    )
