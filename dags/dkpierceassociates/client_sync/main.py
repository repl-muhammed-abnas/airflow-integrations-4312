...
def create_main_dag(config):
    ...
    with rail.create_airflow_dag(
        ...
        description='Sync accounts from Salesforce to Replicon - Master DAG',
        ...
        def get_new_or_updated_account_query():
            ...
            Limit {config.salesforce_account_query_limit}"""
        ...
        # Trigger child DAG for each Salesforce account record
        trigger_process_accounts = rail.TriggerDagRunForEachItemOperator(
            task_id='trigger_process_accounts',
            trigger_dag_id=config.process_account_dag_id,
        ...
        collection_task = rail.WriteLogOperator(
            ...
            message="Client Sync is successfully processed",
            severity='Success',
            properties={
                'status': 'Success',
            }
        ...
        ## airflow hierarchy
        get_last_sync_time >> new_or_updated_salesforce_object >> trigger_process_accounts >> wait_for_child_dags >> validate_child_success >> update_lastsync_time >> collection_task >> log_to_sumo
