...
def get_sales_item(invoice_qbo_items,invoice_qbo_taxcodes):
    ...
    for invoice in invoice_items:
        ...
        tax_code_payload = find_first_by_attr_and_get_attr(result('get_invoice_replicon').get('customMetadata'), 'keyUri', 'urn:replicon:invoice-metadata-key:tax-profile', "value")
        tax_code =tax_code_payload.get('collection')[0].get('collection')[0].get('collection')[1].get('text') if tax_code_payload else ''
        if description.replace("-","").strip():
            sales_line_items.append({
                ...
                'SalesItemLineDetail': {
                    'TaxCodeRef': {
                        'value': find_first_by_attr_and_get_attr(
                            invoice_qbo_taxcodes, 'Name', tax_code, "Id"
                        )
                    },
                    'ItemRef': {
...
def create_invoice_with_multiline_item_request(dag_run):
    ...
    return {
        ...
        'Line': get_sales_item(invoice_qbo_items, invoice_qbo_taxcodes),
        'DueDate': due_date
    }
...
def get_invoice_details_request(dag_run):
    return {
  "invoiceUri": dag_run.conf['invoice']
}
