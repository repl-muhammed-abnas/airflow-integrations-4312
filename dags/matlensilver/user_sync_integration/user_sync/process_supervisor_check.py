...
def create_child_dag_wbs(config):
    with rail.create_airflow_dag(
        dag_id=f'matlen_silver_user_sync_child_process_supervisor_check_{config.instance}',
        description='Matlen_Silver User Sync Process Supervisor',
        ...
        is_supervisor_changed = rail.IfOperator(
            ...
            test=lambda dag_run: rail.result('search_supervisor_in_replicon')[0]['loginname'] != dag_run.conf['getuserinfo']
        ...
        write_supervisor_entry_complete=rail.WriteLogOperator(
            ...
            severity='Completed Supervisor Check',
            message= 'Completed',
        ...
        filter_master_logs >> search_supervisor_in_replicon >> is_supervisor_exists >> rail.Label('No') >> log_supervisor_not_present
        ...
        finish_process_supervisor >> has_master_log_entry >> rail.Label('Yes') >> get_message_from_log >> log_to_master_log >> on_error
        has_master_log_entry >> rail.Label('No') >> write_supervisor_entry_complete >> on_error >> log_error_to_master_log
