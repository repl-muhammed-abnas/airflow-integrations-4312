...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'solver_expense_report_extract_send_mail_per_supervisor_friday_child_{config.instance}',
        description=f'SolverInc_Send_Mail_Per_Supervisor_Friday_run_Child  {config.instance}',
        ...
        get_week_start_and_end_date=rail.PythonOperator(
            ...
            python_callable=lambda:  {
                'startdate': (datetime.now()-timedelta(days=5)).strftime("%b %d, %Y"),
