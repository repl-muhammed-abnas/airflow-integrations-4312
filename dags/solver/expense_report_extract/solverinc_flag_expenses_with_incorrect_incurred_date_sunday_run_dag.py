...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'solver_expense_report_extract_sunday_run_master_{config.instance}',
        description=f'SolverInc_Flag_Expenses_with_incorrect_incurred_date_Sunday_run  {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=dt(2023, 1, 1, tz=config.timezone),
        schedule_interval=config.schedule_interval_sunday,
        ...
        trigger_dag_to_send_mail_per_supervisor = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'solver_expense_report_extract_send_mail_per_supervisor_sunday_child_{config.instance}',
