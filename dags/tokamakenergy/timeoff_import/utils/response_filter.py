...
def get_filtered_time_off_details_on_booking_id(response, dag_run):
    booking_id = str(dag_run.conf['id'])
    return list(filter(lambda x: x['booking_id'] == booking_id, map(lambda row: {
...
def get_time_off_details_to_update(response, dag_run):
    result = False
    replicon_dates = {get_date_in_str(row['date']): row['specificDuration'] for row in response['userExplicitEntryDetails']}
    tokamak_dates = dag_run.conf['dates']
    for date, hour in tokamak_dates.items():
        if float(replicon_dates.get(date, 0)) != float(hour):
            result = True
            break
    return result
