...
def get_remaining_page_numbers():
    """Returns list of page numbers to fetch (2 to total_pages) if pagination is needed."""
    initial_response = rail.result("bamboohr_get_employee_fields_first_page")
...
def get_page_fields(task_id):
    """Get fields from the specified task response."""
    response = rail.result(task_id)
    return response.get("fields", [])


def get_flattened_employee_fields():
    """Flatten the employee fields list (handles nested lists from append operations)."""
    result = rail.result("get_employee_fields_var")
    fields = result.get("value", [])
    flattened = list(chain.from_iterable(
        item if isinstance(item, list) else [item] for item in fields
    ))
