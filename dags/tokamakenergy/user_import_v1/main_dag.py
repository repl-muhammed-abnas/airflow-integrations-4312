...
import pendulum
import rail
from tokamakenergy.user_import_v1.utils import request_payload, response_filters, custom_methods
from tokamakenergy.user_import_v1.tasks.send_logs import get_send_logs
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        start_date=pendulum_datetime(2025, 1, 1, tz=config.time_zone),
        ...
        # Paginated BambooHR employee fields workflow (v1.2 API)
        ...
        set_employee_fields_var = rail.SetVariableOperator(
            ...
            value=lambda: custom_methods.get_page_fields(bamboohr_get_employee_fields_first_page.task_id)
        )

        has_more_pages = rail.IfOperator(
            task_id='has_more_pages',
            test=lambda: len(custom_methods.get_remaining_page_numbers()) > 0,
            yes_task='fetch_remaining_pages',
            no_task='get_employee_fields_var'
        )

        fetch_remaining_pages = rail.ForEachOperator(
            task_id='fetch_remaining_pages',
            items=custom_methods.get_remaining_page_numbers,
        ...
        bamboohr_get_employee_fields_page = rail.BambooHROperator(
            ...
            company_domain=config.bamboohr_domain,
            request_method='GET',
            endpoint="/datasets/employee/fields?page={{ result('fetch_remaining_pages') }}&page_size=1000",
            bamboohr_conn_id=config.bamboohr_conn_id
        )

        accumulate_page_fields = rail.SetVariableOperator(
            task_id='accumulate_page_fields',
            name="employee_fields",
            value=lambda: custom_methods.get_page_fields(bamboohr_get_employee_fields_page.task_id),
            append=True
        )
        ...
        filter_required_employee_fields = rail.PythonOperator(
            ...
            python_callable=lambda: response_filters.get_required_employee_datasets_fields(
                custom_methods.get_flattened_employee_fields(), config.required_employee_fields)
        )
        ...
        can_run_batch_task >> rail.Label(
            'No') >> get_lastsync_time_and_current_time >> get_enabled_employee_type_groups \
                >> get_enabled_department_groups >> bamboohr_get_employee_fields_first_page \
                    >> set_employee_fields_var >> has_more_pages
        has_more_pages >> rail.Label('Yes') >> fetch_remaining_pages
        fetch_remaining_pages >> bamboohr_get_employee_fields_page >> accumulate_page_fields >> for_each_end
        has_more_pages >> rail.Label('No') >> get_employee_fields_var
        fetch_remaining_pages >> for_each_end
        for_each_end >> get_employee_fields_var >> filter_required_employee_fields \
            >> bamboohr_all_employees_data >> get_job_table_records >> get_employment_table_records \
                >> get_jobgrade_table_records >> bamboohr_updated_employees_data >> has_bamboohr_updated_employees_data
