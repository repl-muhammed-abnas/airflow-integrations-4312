...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'briarhallllc_timesheet_audit_master_{config.instance}',
        description='Briarhallllc Extract Report Timesheet Audit master',
        ...
        get_timesheet_audit_details = rail.RepliconReportDetailsOperator(
            task_id='get_timesheet_audit_details',
            report_name=config.extract_timesheet_audit,
        )
        ...
        report_group_entry, report_group_exit = rail.run_report(
            ...
            report_params={
                "reportParameters": [
                    {
                        "reportUri": "{{ result('get_timesheet_audit_details').uri }}",
                        "filterValues": [],
        ...
        upload_report_to_sftp = rail.SFTPUploadFileOperator(
            ...
            remote_filepath=config.extract_report_file_path + config.extract_timesheet_audit_file_name +
        ...
        send_success_mail = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Timesheet_Audit_Trail_Extract_Completed_"{{ result("get_logging_details")["dag_start_time"] }}',
            html_content="templates/email/success_mail.html",
            params={
                'report_name': config.extract_timesheet_audit_file_name,
        ...
        log_to_sumo = rail.DagRunLogToSumoOperator(
            ...
            extra_info={
                'filename': config.extract_timesheet_audit_file_name +
                '_{{ result("get_logging_details")["dag_start_time_file_name"] }}.csv'
            }
        )

        get_logging_details >> get_timesheet_audit_details >> report_group_entry
