...
def process_departments_task_group(execution_timeout_days):
    with rail.TaskGroup(group_id='process_deparments_task', prefix_group_id=False):
        ...
        load_valid_departments_data = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT businessunitdescription, businessunitcode FROM validrecords WHERE NULLIF(businessunitdescription, '') IS NOT NULL"""
        )
        ...
        query_departments_to_create = rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT businessunitdescription, businessunitcode FROM feedfiledepartments where LOWER(businessunitdescription) NOT IN
