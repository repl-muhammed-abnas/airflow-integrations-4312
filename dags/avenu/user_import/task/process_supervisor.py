...
def process_supervisor_assignment_task_group(user_uri, status):
    with rail.TaskGroup(group_id='process_supervisor_assignment_task', prefix_group_id=False):
        ...
        is_supervisor_id_present = rail.IfOperator(
            task_id = "is_supervisor_id_present",
            test= lambda dag_run: bool(dag_run.conf['reportstoid']),
            yes_task= "is_supervisor_same_as_user",
            no_task= "log_no_supervisor_id_present"
        )

        log_no_supervisor_id_present = rail.WriteLogOperator(
            task_id = "log_no_supervisor_id_present",
            log="{{result('add_user_exception_log')}}" if status == 'new_user' else "{{result('update_user_exception_log')}}",
            message="Supervisor not updated  - Supervisor id not present in the feed file",
            severity='Exception',
            properties={
        ...
        is_supervisor_same_as_user = rail.IfOperator(
            ...
            test=lambda dag_run: dag_run.conf['reportstoid'] == dag_run.conf['employeeid'],
        ...
        log_supervisor_not_present = rail.WriteLogOperator(
            ...
            properties=lambda dag_run: {
                ...
                'reportstoid': dag_run.conf['reportstoid'],
        ...
        log_supervisor_is_disabled = rail.WriteLogOperator(
            task_id='log_supervisor_is_disabled',
            message="Supervisor is disabled in Replicon",
            severity='Pending',
            properties=lambda dag_run: {
                'employeeid': dag_run.conf['employeeid'],
                'firstname': dag_run.conf['firstname'],
        ...
        update_supervisor_schedule_for_user = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "dateRange":{
                    "startDate": request_payload.get_replicon_date(dag_run.conf['hiredate']) if status == 'new_user' else dag_run.conf['todays_date']
                }
            }
        )
        ...
        is_supervisor_id_present >> rail.Label(
            "Yes") >> is_supervisor_same_as_user
        is_supervisor_id_present >> rail.Label(
            "No") >> log_no_supervisor_id_present >> finish_process_supervisor
        ...
        is_supervisor_disabled >> rail.Label(
            'Yes') >> log_supervisor_is_disabled >> finish_process_supervisor

    return is_supervisor_id_present, finish_process_supervisor
