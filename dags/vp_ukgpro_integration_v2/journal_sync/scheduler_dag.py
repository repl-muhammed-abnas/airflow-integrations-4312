...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'vp_ukgpro_journal_sync_v2_scheduler_{config.instance}',
        description='Scheduler DAG for VP UKG Pro Journal Sync',
        integration_type='generic',
        company_key=config.company_key,
        multi_tenant=True,
        schedule_interval=Variable.get(
            f'vp_ukgpro_journal_sync_v2_schedule_interval_{config.instance}',
        ...
        process_customers = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=(
                f'vp_ukgpro_journal_sync_v2_main_{config.instance}'
            ),
            conf=lambda item: {
                **item,
