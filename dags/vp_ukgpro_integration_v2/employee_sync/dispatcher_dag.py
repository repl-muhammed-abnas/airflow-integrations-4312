...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'vp_ukgpro_employee_sync_v2_dispatcher_{config.instance}',
        description=(
        ...
        def prepare_sync_timestamps():
            ...
            Dual-reads the per-customer Variable: prefers the new
            `vp_ukgpro_employee_sync_v2_last_run_{customer_id}` key and falls
            back to the legacy
            `vp_ukgpro_employee_sync_v2_last_run_{instance}_{customer_id}`
            key during migration. Writes are made only to the new key.
            """
            new_variable_key = (
                f'vp_ukgpro_employee_sync_v2_last_run_{config.customer_id}'
            )
            legacy_variable_key = (
                f'vp_ukgpro_employee_sync_v2_last_run_'
                f'{config.instance}_{config.customer_id}'
            )
            ...
            try:
                last_sync_time = Variable.get(new_variable_key)
                logging.info(
                    "Retrieved last sync time from Variable '%s': %s",
                    new_variable_key, last_sync_time
                )
            except KeyError:
                try:
                    last_sync_time = Variable.get(legacy_variable_key)
                    logging.info(
                        "Retrieved last sync time from legacy Variable "
                        "'%s': %s",
                        legacy_variable_key, last_sync_time
                    )
                except KeyError:
                    last_sync_time = config.initial_sync_time
                    logging.info(
                        "No Variable found (%s / %s), using initial "
                        "sync time: %s",
                        new_variable_key, legacy_variable_key, last_sync_time
                    )
        ...
        def extract_employee_list():
            ...
            if not changes:
                logging.info("No recently changed employees found")
                return []

            if not isinstance(changes, list):
                logging.warning(
                    "Expected list response, got %s", type(changes)
                )
                return []

            extras = getattr(config, 'extras', {}) or {}
            company_code = extras.get('company_code')
            org2_code = extras.get('org_level2_code')
            org3_code = extras.get('org_level3_code')
            ...
            for emp in changes:
                ...
                if employee_id and company_id:
                    ...
                    logging.warning(
                        "Skipping employee with missing data: %s", emp
                    )

            logging.info(
                "Found %s employees to process", len(employee_list)
            )
        ...
        def log_no_employees_found():
            ...
            logging.info(
                ...
                "(query range: %s to %s)",
                timestamps['last_sync_time'],
                timestamps['current_sync_time']
            )
        ...
        process_employees = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'vp_ukgpro_employee_sync_v2_router_{config.instance}',
            conf=lambda item: {
                **item,
                ...
                'connections': {
                    ...
                    'vantagepoint': config.vantagepoint_conn_id,
                },
            },
            execution_timeout=timedelta(days=config.execution_timeout_days)
        )
        ...
        def update_last_sync_time():
            ...
            race conditions. Writes only the new per-customer key.
            """
            variable_key = (
                f'vp_ukgpro_employee_sync_v2_last_run_{config.customer_id}'
            )
        ...
        (
            extract_dynamic_config >> prepare_timestamps >> get_recently_changed_employees >>
