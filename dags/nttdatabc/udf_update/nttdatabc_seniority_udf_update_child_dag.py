...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'nttdatabc_seniority_udf_update_child_{config.instance}',
        ...
        get_sum_for_timeoff_hours=rail.QueryCollectionOperator(
            ...
            query="""SELECT SUM( approvedtimesheetdata.timeoffhours) as sum FROM  approvedtimesheetdata""",
        )
        ...
        get_filter_values = rail.PythonOperator(
            ...
            python_callable = lambda: {
                "loginname": (rail.result('load_records_approved_timesheet_collection')[0]['loginname']
                    if rail.result('load_records_approved_timesheet_collection')[0]['loginname']
                    else rail.result('load_records_employee_pay_code_collection')['loginname']),
                "username": (
                    rail.result('load_records_approved_timesheet_collection')[0]['username']
                    if rail.result('load_records_approved_timesheet_collection')[0]['username']
                    else rail.result('load_records_employee_pay_code_collection')['username']
                    ),
                "timesheetperiod": (
                    rail.result('load_records_approved_timesheet_collection')[0]['timesheetperiod']
                    if rail.result('load_records_approved_timesheet_collection')[0]['timesheetperiod']
                    else rail.result('load_records_employee_pay_code_collection')['timesheetperiod']
                    ),
                "useruri": (
                    rail.result('load_records_approved_timesheet_collection')[0]['useruri']
                    if rail.result('load_records_approved_timesheet_collection')[0]['useruri']
                    else rail.result('load_records_employee_pay_code_collection')['useruri'])
            }
        )
        ...
        def get_subchild_payload(dag_run):
            ...
            return {
                    "timesheeturi": (rail.result('load_records_approved_timesheet_collection')[0]['timesheeturi']
                        if rail.result('load_records_approved_timesheet_collection')[0]['timesheeturi']
                        else rail.result('load_records_employee_pay_code_collection')['timesheeturi']),
                    "approvalstatus": (rail.result('load_records_approved_timesheet_collection')[0]['approvalstatus']
                        if rail.result('load_records_approved_timesheet_collection')[0]['approvalstatus']
                        else rail.result('load_records_employee_pay_code_collection')['approvalstatus']),
        ...
        trigger_subchild_dag=rail.TriggerDagRunOperator(
            ...
            retries=0,
            trigger_dag_id=f'nttdatabc_seniority_udf_update_subchild_{config.instance}',
        ...
        get_final_seniority_value=rail.PythonOperator(
            ...
            python_callable= lambda: ( 0 if rail.result('get_seniority_custom_field_value')['text'] == '' else
        ...
        add_success_logentry_to_lookup=rail.WriteLogOperator(
            ...
            properties=lambda dag_run:{
                ...
                "finalvalue": "{{ result('get_final_seniority_value') }}",
                "approvalstatus": (rail.result('load_records_approved_timesheet_collection')[0]['approvalstatus']
                    if rail.result('load_records_approved_timesheet_collection')[0]['approvalstatus']
                    else rail.result('load_records_employee_pay_code_collection')['approvalstatus']),
                "status": "Success",
        ...
        if_there_are_final_hours >> rail.Label(
            'Yes') >> get_seniority_custom_field_value >> get_final_seniority_value >> update_seniority_custom_field_value
        update_seniority_custom_field_value >> add_success_logentry_to_lookup >> add_entry_to_ntt_mapper >> catch_error_and_log_entry
