...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'nttdatabc_existing_user_default_shift_assignment_master_{config.instance}',
        description=f'NTTData BC Existing user default shift assignment master {config.instance}',
        ...
        end_date = (pendulum.now()+relativedelta(months=+14, day=31)).date()
        ...
        report_has_data >> rail.Label("Yes") >> load_enabled_user_schedule_data >> create_enabled_user_schedule_data \
            >> query_get_all_users_with_shift_schedules >> trigger_shift_assignment_per_user >> wait_for_shift_assignments >> send_logs_enter
