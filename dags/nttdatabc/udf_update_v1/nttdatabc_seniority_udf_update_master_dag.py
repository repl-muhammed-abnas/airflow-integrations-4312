...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'nttdatabc_seniority_udf_update_master_{config.instance}_v1',
        ...
        get_time_tobe_checked=rail.PythonOperator(
            ...
            python_callable= lambda: (datetime.now()-timedelta(days=1)).strftime("%m/%d/%Y")
        )
            ...
            report_params= lambda: {
                "reportParameters": [
                    ...
                    "filterValues": [
                                        ...
                                        {
                                        "reportFilterUri": rail.result('get_approval_status_and_approval_date_filter_uri')['approvaldate'],
                                        "value": rail.result('get_time_tobe_checked')
                                        },
                                        {
                                        "reportFilterUri": rail.result('get_approval_status_and_approval_date_filter_uri')['approvaldate'],
                                        "value": rail.result('get_time_tobe_checked')
        ...
        query_distinct_timesheets=rail.QueryCollectionOperator(
            ...
            query="""SELECT DISTINCT  inputdatatimesheetapproved.timesheeturi, inputdatatimesheetapproved.employeetype FROM  inputdatatimesheetapproved""",
        )
        ...
        process_each_distinct_timesheet = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'nttdatabc_seniority_udf_update_process_distinct_timesheets_{config.instance}_v1',
