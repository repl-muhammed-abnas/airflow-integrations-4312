...
from dipolegroup.invoice_export.request_payload import get_queuedforsync_invoice


def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"dipole_group_invoice_export_master_{config.instance}",
        description=f'Xero Online Invoice Export {config.instance}',
        ...
        trigger_invoice_child_dag = rail.TriggerDagRunForEachItemOperator(
            ...
            execution_timeout=timedelta(
                days=config.execution_timeout_days),
            trigger_dag_id=f"dipole_group_invoice_export_child_dag_{config.instance}",
