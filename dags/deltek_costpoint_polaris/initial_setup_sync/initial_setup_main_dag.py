...
def create_dag(config):
    with rail.create_airflow_dag(
        ...
        def do_get_last_run_date():
            current_time = datetime.now(timezone(config.time_zone)) - timedelta(seconds=2)
        ...
        process_time_offs_types_with_project_code = rail.TriggerDagRunOperator(
            task_id='process_time_offs_types_with_project_code',
            retries=0,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            trigger_dag_id=f'deltek_costpoint_timeoff_type_with_project_code_child_{config.instance}',
            conf=lambda: {
                    "last_modified": get_time()
            }
        )
        ...
        wait_for_completion_trigger_process_timeoffs_with_project_code = rail.WaitForDagRunsSensor(
            task_id='wait_for_completion_trigger_process_timeoffs_with_project_code',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs='{{ result("process_time_offs_types_with_project_code") }}'
        )
        ...
        process_roles_sync = rail.TriggerDagRunOperator(
            task_id='process_roles_sync',
            retries=0,
            execution_timeout=timedelta(days=config.execution_timeout_days),
        ...
        wait_for_completion_trigger_process_roles_sync = rail.WaitForDagRunsSensor(
            task_id='wait_for_completion_trigger_process_roles_sync',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs='{{ result("process_roles_sync") }}'
        )
        ...
        use_project_id_for_time_off_types = rail.IfOperator(
            task_id='use_project_id_for_time_off_types',
            test=lambda: config.use_project_id_for_timeoff_types.lower() == 'true',
            yes_task='process_time_offs_types_with_project_code',
            no_task='process_timeoffs'
        )
        ...
        can_run_batch_task >> rail.Label(
            ...
            use_project_id_for_time_off_types 
        
        use_project_id_for_time_off_types >> rail.Label(
            'Yes') >> process_time_offs_types_with_project_code >> \
            wait_for_completion_trigger_process_timeoffs_with_project_code >> \
            process_tsperiods

        use_project_id_for_time_off_types >> rail.Label(
            'No') >> process_timeoffs >> wait_for_completion_trigger_process_timeoffs >> \
            process_tsperiods 
        ...
        process_tsperiods >> wait_for_completion_trigger_process_tsperiods >> \
            ...
            process_holiday_calendar_sync >> wait_for_completion_trigger_process_holiday_calendars >>\
            process_roles_sync >> wait_for_completion_trigger_process_roles_sync >> \
