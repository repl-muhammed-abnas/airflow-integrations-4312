...
def create_dag(config):
    if config.instance != 'CostpointPolarisProj':
        return None

    with rail.create_airflow_dag(
        dag_id=f'deltek_costpoint_polaris_project_sync_main_manual_test_{config.instance}',
        description=f'deltek_costpoint_polaris_project_sync_main_manual_test_{config.instance}',
        ...
        def get_filters():
            # Restricted to a single root project (BIDPRP) for manual WBS
            # over-sync validation, instead of the production last-modified filter.
            return [
                {
                    "name": "PROJ_ID",
                    "relation": "like%",
                    ...
                    "where": [
                        {
                            "rsWhere": {
                                "rsId": "PJMBASIC_PROJ",
                                # Workforce-change-detection conditions are intentionally
                                # dropped here (unlike production) so this test run stays
                                # scoped to BIDPRP only and can't pull in unrelated
                                # workforce-modified root projects via the OR'd conditions.
                                "conditions": [
                                    {
                                        "joinWithParent": "N",
        ...
        can_run_batch_task >> rail.Label('No') >> get_last_run_date
        ...
        can_load_data_in_chunks >> rail.Label('yes') >> get_modified_projects_in_chunks >> update_last_run_date >> has_project_data
        can_load_data_in_chunks >> rail.Label('no') >> get_modified_projects >> update_last_run_date >> has_project_data
        has_project_data >> rail.Label('yes') >> group_data_by_root_project
        has_project_data >> rail.Label('no') >> delete_this_dagrun >> finish
        group_data_by_root_project >> get_all_clients_from_replicon >> get_costpoint_plcs >> \
            get_all_roles >> get_replicon_divisions >> \
            get_project_udfs >> get_task_udfs >> get_all_permission_sets >> process_clients >> pick_chose_wbs_sync
        pick_chose_wbs_sync >> rail.Label('Yes') >> process_each_root_project_pick_chose
        pick_chose_wbs_sync >> rail.Label('No') >> process_each_root_project
        ...
        has_error_logs >> rail.Label('yes') >> create_csv_lines >> log_filename >> generate_download_link >> send_mail_error >> finish
