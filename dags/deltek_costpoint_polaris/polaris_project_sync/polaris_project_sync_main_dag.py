...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'deltek_costpoint_polaris_project_sync_main_{config.instance}',
        ...
        def get_filters():
            # Eligibility and exclusion filters are intentionally omitted so that
            # projects transitioning out of eligibility or into/out of exclusion are
            # still detected by the incremental poll. Eligibility and exclusion are
            # evaluated downstream in the child DAG (deactivation of excluded projects
            # that were previously synced requires the child DAG to run for them).
            return [
                {
                    ...
                    "where": [
                        {
                            "rsWhere": {
                                ...
                                ] + (get_workforce_proj_id_conditions() if getattr(config, 'workforce_change_detection_enabled', False) else [])
                            }
                        }
                    ]
                }
