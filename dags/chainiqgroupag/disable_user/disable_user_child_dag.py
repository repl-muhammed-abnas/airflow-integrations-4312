...
def create_disable_user_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.disable_user_child_dagid,
        description=f'ChainIQ User Disable Child {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.disable_child_dag_active_runs,
    ) as dag:
        ...
        disable_user >> rail.Label('On Error') >> catch_disable_user_error >> dagrun_log_to_sumo
