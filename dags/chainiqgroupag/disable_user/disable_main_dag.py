...
def create_disable_user_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.disable_user_main_dagid,
        description=f'ChainIQ User Disable Master {config.instance}',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=datetime(2025, 1, 1, tz=config.est_timezone),
        ...
        run_report_group_entry, run_report_group_exit = rail.run_report(
            group_id='disable_user_report_generation',
            report_params={
                'reportParameters': [
                    {
        ...
        is_report_failed = rail.IfOperator(
            ...
            test="{{ result('disable_user_report_generation.get_report_result').reportGenerationResults[0].error | is_truthy }}",
            yes_task='fail_disable_user_report_generation',
            no_task='report_has_data'
        )

        fail_disable_user_report_generation = rail.FailOperator(
            task_id='fail_disable_user_report_generation',
            message="{{ result('disable_user_report_generation.get_report_result').reportGenerationResults[0].error }}"
        )

        report_has_data = rail.IfOperator(
            task_id='report_has_data',
            test="{{ result('disable_user_report_generation.get_report_result', 'has_data') }}",
        ...
        load_report_data = rail.LoadCSVFileOperator(
            ...
            document="{{ result('disable_user_report_generation.get_report_result').reportGenerationResults[0].payload }}"
        )
        ...
        # Filter users where end date has passed (daydiff < 0) and still enabled
        query_users_to_disable = rail.QueryCollectionOperator(
            task_id='query_users_to_disable',
            query="SELECT * FROM userdata WHERE daydiff < 0 AND status = 'Enabled'"
        )
        ...
        disable_user_child = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.disable_user_child_dagid,
        ...
        is_report_failed >> rail.Label('Yes') >> fail_disable_user_report_generation
        ...
        users_to_disable >> rail.Label('Yes') >> disable_user_child >> wait_for_disable_user_child >> gather_disable_user_errors >> is_disable_user_error
