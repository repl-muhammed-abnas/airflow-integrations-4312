...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.imperva_time_type_custom_field_check_child,
        description=f'impervainc time type custom field check child dag {config.instance}',
        ...
        get_time_type_custom_field_uri = rail.RepliconServiceOperator(
            task_id='get_time_type_custom_field_uri',
        ...
        get_custom_dropdowns = rail.RepliconServiceOperator(
            ...
            data={
                'customFieldUri': "{{ result('get_time_type_custom_field_uri') }}"
            }
        )

        create_timetypevalues_collection = rail.CreateCollectionOperator(
            task_id='create_timetypevalues_collection',
            source="{{ result('get_custom_dropdowns') | to_json }}",
            name='timetypevalues'
        )

        get_distinct_timetype = rail.QueryCollectionOperator(
            task_id='get_distinct_timetype',
            query="SELECT DISTINCT Time_Type as timetype FROM workdaydata5 WHERE Time_Type NOT IN \
                (SELECT DISTINCT displayText FROM timetypevalues)"
        )

        if_timetype_present=rail.IfOperator(
            task_id='if_timetype_present',
            test='''{{result('get_distinct_timetype', 'length') > 0}}''',
            yes_task="create_timetype_list",
            no_task="log_to_sumo",
        )

        create_timetype_list = rail.PythonOperator(
            task_id='create_timetype_list',
            python_callable=python_callable.create_imperva_list_schema,
            op_args=["{{result('get_custom_dropdowns')}}",
                     "{{result('get_distinct_timetype')}}",
                     "timetype"]
        )
        ...
        put_dropdown_options = rail.RepliconServiceOperator(
            ...
            data= {
                'customFieldUri': "{{result('get_time_type_custom_field_uri')}}",
                'customFieldDropDownOptionUris': "{{result('create_timetype_list')}}"
            }
        )
        ...
        get_time_type_custom_field_uri >> get_custom_dropdowns >> create_timetypevalues_collection >> \
        get_distinct_timetype >> if_timetype_present >> rail.Label(
            "Yes") >> create_timetype_list >> put_dropdown_options >> log_to_sumo
        if_timetype_present >> rail.Label("No") >> log_to_sumo
