...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.imperva_employee_type_custom_field_check_child,
        description=f'impervainc employee type custom field check child dag {config.instance}',
        ...
        get_employee_type_custom_field_uri = rail.RepliconServiceOperator(
            task_id='get_employee_type_custom_field_uri',
            endpoint='/services/CustomFieldService1.svc/GetAllCustomFields',
            data={
                'objectUri': "urn:replicon:object-type:user"
            },
            data_handler= lambda response: rail.find_first_by_attr_and_get_attr(response, "displayText",
                        "Imperva Employee Type", "uri")
        )
        ...
        get_custom_dropdowns = rail.RepliconServiceOperator(
            ...
            data={
                'customFieldUri': "{{ result('get_employee_type_custom_field_uri') }}"
            }
        )

        create_impervaemployeetypevalues_collection = rail.CreateCollectionOperator(
            task_id='create_impervaemployeetypevalues_collection',
            source="{{ result('get_custom_dropdowns') | to_json }}",
            name='impervaemployeetypevalues'
        )

        get_distinct_impervaemployeetype = rail.QueryCollectionOperator(
            task_id='get_distinct_impervaemployeetype',
            query="SELECT DISTINCT Imperva_Employee_Type as impervaemployeetype FROM workdaydata6 WHERE Imperva_Employee_Type NOT IN \
                (SELECT DISTINCT displayText FROM impervaemployeetypevalues)"
        )

        if_impervaemployeetype_present=rail.IfOperator(
            task_id='if_impervaemployeetype_present',
            test='''{{result('get_distinct_impervaemployeetype', 'length') > 0}}''',
            yes_task="create_impervaemployeetype_list",
            no_task="log_to_sumo",
        )

        create_impervaemployeetype_list = rail.PythonOperator(
            task_id='create_impervaemployeetype_list',
            python_callable=python_callable.create_imperva_list_schema,
            op_args=["{{result('get_custom_dropdowns')}}",
                     "{{result('get_distinct_impervaemployeetype')}}",
                     "impervaemployeetype"]
        )
        ...
        put_dropdown_options = rail.RepliconServiceOperator(
            ...
            data= {
                'customFieldUri': "{{result('get_employee_type_custom_field_uri')}}",
                'customFieldDropDownOptionUris': "{{result('create_impervaemployeetype_list')}}"
            }
        )
        ...
        get_employee_type_custom_field_uri >> get_custom_dropdowns >> create_impervaemployeetypevalues_collection >> \
        get_distinct_impervaemployeetype >> if_impervaemployeetype_present >> rail.Label(
            "Yes") >> create_impervaemployeetype_list >> put_dropdown_options >> log_to_sumo
        if_impervaemployeetype_present >> rail.Label("No") >> log_to_sumo
