...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.imperva_organization_custom_field_check_child,
        description=f'impervainc organization custom field check child dag {config.instance}',
        ...
        get_imperva_organization_custom_field_uri = rail.RepliconServiceOperator(
            task_id='get_imperva_organization_custom_field_uri',
            endpoint='/services/CustomFieldService1.svc/GetAllCustomFields',
            data={
                'objectUri': "urn:replicon:object-type:user"
            },
            data_handler= lambda response: rail.find_first_by_attr_and_get_attr(response, "displayText",
                        "Imperva Organization", "uri")
        )
        ...
        get_custom_dropdowns = rail.RepliconServiceOperator(
            ...
            data={
                'customFieldUri': "{{ result('get_imperva_organization_custom_field_uri') }}"
            }
        )

        create_impervaorganizationvalues_collection = rail.CreateCollectionOperator(
            task_id='create_impervaorganizationvalues_collection',
            source="{{ result('get_custom_dropdowns') | to_json }}",
            name='impervaorganizationvalues'
        )

        get_distinct_imperva_organization = rail.QueryCollectionOperator(
            task_id='get_distinct_imperva_organization',
            query="SELECT DISTINCT Imperva_Organization as impervaorganization FROM workdaydata1 WHERE Imperva_Organization NOT IN \
                (SELECT DISTINCT displayText FROM impervaorganizationvalues)"
        )

        if_impervaorganization_present=rail.IfOperator(
            task_id='if_impervaorganization_present',
            test='''{{result('get_distinct_imperva_organization', 'length') > 0}}''',
            yes_task="create_imperva_employeetype_list",
            no_task="log_to_sumo",
        )

        create_imperva_employeetype_list = rail.PythonOperator(
            task_id='create_imperva_employeetype_list',
            python_callable=python_callable.create_imperva_list_schema,
            op_args=["{{result('get_custom_dropdowns')}}",
                     "{{result('get_distinct_imperva_organization')}}",
                     'impervaorganization']
        )
        ...
        put_dropdown_options = rail.RepliconServiceOperator(
            ...
            data= {
                'customFieldUri': "{{result('get_imperva_organization_custom_field_uri')}}",
                'customFieldDropDownOptionUris': "{{result('create_imperva_employeetype_list')}}"
            }
        )
        ...
        get_imperva_organization_custom_field_uri >> get_custom_dropdowns >> create_impervaorganizationvalues_collection >> \
        get_distinct_imperva_organization >> if_impervaorganization_present >> rail.Label(
            "Yes") >> create_imperva_employeetype_list >> put_dropdown_options >> log_to_sumo
        if_impervaorganization_present >> rail.Label("No") >> log_to_sumo
