...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.imperva_worker_type_custom_field_check_child,
        description=f'impervainc worker type custom field check child dag {config.instance}',
        ...
        get_worker_type_custom_field_uri = rail.RepliconServiceOperator(
            task_id='get_worker_type_custom_field_uri',
            endpoint='/services/CustomFieldService1.svc/GetAllCustomFields',
            data={
                'objectUri': "urn:replicon:object-type:user"
            },
            data_handler= lambda response: rail.find_first_by_attr_and_get_attr(response, "displayText",
                        "Imperva Worker Type", "uri")
        )
        ...
        get_custom_dropdowns = rail.RepliconServiceOperator(
            ...
            data={
                'customFieldUri': "{{ result('get_worker_type_custom_field_uri') }}"
            }
        )

        create_impervaworkertypevalues_collection = rail.CreateCollectionOperator(
            task_id='create_impervaworkertypevalues_collection',
            source="{{ result('get_custom_dropdowns') | to_json }}",
            name='impervaworkertypevalues'
        )

        get_distinct_impervaworkertype = rail.QueryCollectionOperator(
            task_id='get_distinct_impervaworkertype',
            query="SELECT DISTINCT Imperva_Worker_Type as impervaworkertype FROM workdaydata7 WHERE Imperva_Worker_Type NOT IN \
                (SELECT DISTINCT displayText FROM impervaworkertypevalues)"
        )

        if_impervaworkertype_present=rail.IfOperator(
            task_id='if_impervaworkertype_present',
            test='''{{result('get_distinct_impervaworkertype', 'length') > 0}}''',
            yes_task="create_impervaworkertype_list",
            no_task="log_to_sumo",
        )

        create_impervaworkertype_list = rail.PythonOperator(
            task_id='create_impervaworkertype_list',
            python_callable=python_callable.create_imperva_list_schema,
            op_args=["{{result('get_custom_dropdowns')}}",
                     "{{result('get_distinct_impervaworkertype')}}",
                     "impervaworkertype"]
        )
        ...
        put_dropdown_options = rail.RepliconServiceOperator(
            ...
            data= {
                'customFieldUri': "{{result('get_worker_type_custom_field_uri')}}",
                'customFieldDropDownOptionUris': "{{result('create_impervaworkertype_list')}}"
            }
        )
        ...
        get_worker_type_custom_field_uri >> get_custom_dropdowns >> create_impervaworkertypevalues_collection >> \
        get_distinct_impervaworkertype >> if_impervaworkertype_present >> rail.Label(
            "Yes") >> create_impervaworkertype_list >> put_dropdown_options >> log_to_sumo
        if_impervaworkertype_present >> rail.Label("No") >> log_to_sumo
