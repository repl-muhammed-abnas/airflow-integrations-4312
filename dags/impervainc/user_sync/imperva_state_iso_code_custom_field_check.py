...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.imperva_state_iso_code_custom_field_check_child,
        description=f'impervainc state iso code custom field check child dag {config.instance}',
        ...
        get_state_isocode_custom_field_uri = rail.RepliconServiceOperator(
            task_id='get_state_isocode_custom_field_uri',
        ...
        get_custom_dropdowns = rail.RepliconServiceOperator(
            ...
            data={
                'customFieldUri': "{{ result('get_state_isocode_custom_field_uri') }}"
            }
        )

        create_stateisocodevalues_collection = rail.CreateCollectionOperator(
            task_id='create_stateisocodevalues_collection',
            source="{{ result('get_custom_dropdowns') | to_json }}",
            name='stateisocodevalues'
        )

        get_distinct_stateisocode = rail.QueryCollectionOperator(
            task_id='get_distinct_stateisocode',
            query="SELECT DISTINCT State_ISO_Code as stateisocode FROM workdaydata2 WHERE State_ISO_Code NOT IN \
                (SELECT DISTINCT displayText FROM stateisocodevalues)"
        )

        if_stateisocode_present=rail.IfOperator(
            task_id='if_stateisocode_present',
            test='''{{result('get_distinct_stateisocode', 'length') > 0}}''',
            yes_task="create_stateisocode_list",
            no_task="log_to_sumo",
        )

        create_stateisocode_list = rail.PythonOperator(
            task_id='create_stateisocode_list',
            python_callable=python_callable.create_imperva_list_schema,
            op_args=["{{result('get_custom_dropdowns')}}",
                     "{{result('get_distinct_stateisocode')}}",
                     "stateisocode"]
        )
        ...
        put_dropdown_options = rail.RepliconServiceOperator(
            ...
            data= {
                'customFieldUri': "{{result('get_state_isocode_custom_field_uri')}}",
                'customFieldDropDownOptionUris': "{{result('create_stateisocode_list')}}"
            }
        )
        ...
        get_state_isocode_custom_field_uri >> get_custom_dropdowns >> create_stateisocodevalues_collection >> \
        get_distinct_stateisocode >> if_stateisocode_present >> rail.Label(
            "Yes") >> create_stateisocode_list >> put_dropdown_options >> log_to_sumo
        if_stateisocode_present >> rail.Label("No") >> log_to_sumo
