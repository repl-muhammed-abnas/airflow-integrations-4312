...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.imperva_country_iso_custom_field_check_child,
        description=f'impervainc country iso custom field check child dag {config.instance}',
        ...
        get_country_isocode_custom_field_uri = rail.RepliconServiceOperator(
            task_id='get_country_isocode_custom_field_uri',
            endpoint='/services/CustomFieldService1.svc/GetAllCustomFields',
            data={
                'objectUri': "urn:replicon:object-type:user"
            },
            data_handler= lambda response: rail.find_first_by_attr_and_get_attr(response, "displayText",
                        "Country ISO Code", "uri")
        )
        ...
        get_custom_dropdowns = rail.RepliconServiceOperator(
            ...
            data={
                'customFieldUri': "{{ result('get_country_isocode_custom_field_uri') }}"
            }
        )

        create_countryisocodevalues_collection = rail.CreateCollectionOperator(
            task_id='create_countryisocodevalues_collection',
            source="{{ result('get_custom_dropdowns') | to_json }}",
            name='countryisocodevalues'
        )

        get_distinct_countryisocode = rail.QueryCollectionOperator(
            task_id='get_distinct_countryisocode',
            query="SELECT DISTINCT Country_ISO_Code as countryisocode FROM workdaydata3 WHERE Country_ISO_Code NOT IN \
                (SELECT DISTINCT displayText FROM countryisocodevalues)"
        )

        if_countryisocode_present=rail.IfOperator(
            task_id='if_countryisocode_present',
            test='''{{result('get_distinct_countryisocode', 'length') > 0}}''',
            yes_task="create_countryisocode_list",
            no_task="log_to_sumo",
        )

        create_countryisocode_list = rail.PythonOperator(
            task_id='create_countryisocode_list',
            python_callable=python_callable.create_imperva_list_schema,
            op_args=["{{result('get_custom_dropdowns')}}",
                     "{{result('get_distinct_countryisocode')}}",
                     "countryisocode"]
        )
        ...
        put_dropdown_options = rail.RepliconServiceOperator(
            ...
            data= {
                'customFieldUri': "{{result('get_country_isocode_custom_field_uri')}}",
                'customFieldDropDownOptionUris': "{{result('create_countryisocode_list')}}"
            }
        )
        ...
        get_country_isocode_custom_field_uri >> get_custom_dropdowns >> create_countryisocodevalues_collection >> \
        get_distinct_countryisocode >> if_countryisocode_present >> rail.Label(
            "Yes") >> create_countryisocode_list >> put_dropdown_options >> log_to_sumo
        if_countryisocode_present >> rail.Label("No") >> log_to_sumo
