...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.imperva_work_country_custom_field_check_child,
        description=f'impervainc work country custom field check child dag {config.instance}',
        ...
        get_work_country_custom_field_uri = rail.RepliconServiceOperator(
            task_id='get_work_country_custom_field_uri',
        ...
        get_custom_dropdowns = rail.RepliconServiceOperator(
            ...
            data={
                'customFieldUri': "{{ result('get_work_country_custom_field_uri') }}"
            }
        )

        create_workcountryvalues_collection = rail.CreateCollectionOperator(
            task_id='create_workcountryvalues_collection',
            source="{{ result('get_custom_dropdowns') | to_json }}",
            name='workcountryvalues'
        )

        get_distinct_workcountry = rail.QueryCollectionOperator(
            task_id='get_distinct_workcountry',
            query="SELECT DISTINCT Work_Address_Country as workcountry FROM workdaydata4 WHERE Work_Address_Country NOT IN \
                (SELECT DISTINCT displayText FROM workcountryvalues)"
        )

        if_workcountry_present=rail.IfOperator(
            task_id='if_workcountry_present',
            test='''{{result('get_distinct_workcountry', 'length') > 0}}''',
            yes_task="create_workcountry_list",
            no_task="log_to_sumo",
        )

        create_workcountry_list = rail.PythonOperator(
            task_id='create_workcountry_list',
            python_callable=python_callable.create_imperva_list_schema,
            op_args=["{{result('get_custom_dropdowns')}}",
                     "{{result('get_distinct_workcountry')}}",
                     "workcountry"]
        )
        ...
        put_dropdown_options = rail.RepliconServiceOperator(
            ...
            data= {
                'customFieldUri': "{{result('get_work_country_custom_field_uri')}}",
                'customFieldDropDownOptionUris': "{{result('create_workcountry_list')}}"
            }
        )
        ...
        get_work_country_custom_field_uri >> get_custom_dropdowns >> create_workcountryvalues_collection >> \
        get_distinct_workcountry >> if_workcountry_present >> rail.Label(
            "Yes") >> create_workcountry_list >> put_dropdown_options >> log_to_sumo
        if_workcountry_present >> rail.Label("No") >> log_to_sumo
