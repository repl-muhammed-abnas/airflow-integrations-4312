...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.imperva_work_state_custom_field_check_child,
        description=f'impervainc workstate type custom field check child dag {config.instance}',
        ...
        get_work_state_custom_field_uri = rail.RepliconServiceOperator(
            task_id='get_work_state_custom_field_uri',
        ...
        get_custom_dropdowns = rail.RepliconServiceOperator(
            ...
            data={
                'customFieldUri': "{{ result('get_work_state_custom_field_uri') }}"
            }
        )

        create_workstatevalues_collection = rail.CreateCollectionOperator(
            task_id='create_workstatevalues_collection',
            source="{{ result('get_custom_dropdowns') | to_json }}",
            name='workstatevalues'
        )

        get_distinct_workstate = rail.QueryCollectionOperator(
            task_id='get_distinct_workstate',
            query="SELECT DISTINCT Work_Address_State_Province as workstate FROM workdaydata9 WHERE Work_Address_State_Province NOT IN \
                (SELECT DISTINCT displayText FROM workstatevalues)"
        )

        if_workstate_present=rail.IfOperator(
            task_id='if_workstate_present',
            test='''{{result('get_distinct_workstate', 'length') > 0}}''',
            yes_task="create_workstate_list",
            no_task="log_to_sumo",
        )

        create_workstate_list = rail.PythonOperator(
            task_id='create_workstate_list',
            python_callable=python_callable.create_imperva_list_schema,
            op_args=["{{result('get_custom_dropdowns')}}",
                     "{{result('get_distinct_workstate')}}",
                     "workstate"]
        )
        ...
        put_dropdown_options = rail.RepliconServiceOperator(
            ...
            data= {
                'customFieldUri': "{{result('get_work_state_custom_field_uri')}}",
                'customFieldDropDownOptionUris': "{{result('create_workstate_list')}}"
            }
        )
        ...
        get_work_state_custom_field_uri >> get_custom_dropdowns >> create_workstatevalues_collection >> \
        get_distinct_workstate >> if_workstate_present >> rail.Label(
            "Yes") >> create_workstate_list >> put_dropdown_options >> log_to_sumo
        if_workstate_present >> rail.Label("No") >> log_to_sumo
