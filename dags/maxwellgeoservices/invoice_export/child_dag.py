...
import rail
from maxwellgeoservices.invoice_export import request_payload
...
def create_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"{config.company_key}_xero_connector_{config.region.replace('-', '_')}_invoice_export_child_dag_{config.instance}",
        description=f'Xero Connector {config.region} Invoice Export Child Dag {config.instance}',
        ...
        search_invoice_in_xero = rail.XeroAPIOperator(
            ...
            request_method='GET',
            filters='''?where=Reference="{{dag_run.conf.invoice_number}}"'''
        )
        ...
        if_create_contact_in_xero_is_false = rail.IfOperator(
            ...
            test=lambda dag_run: not (
                dag_run.conf['customSettings'].get('createNewContactXero')),
        ...
        get_inventory_items_from_xero = rail.XeroAPIOperator(
            task_id='get_inventory_items_from_xero',
            xero_conn_id='{{dag_run.conf.xero_conn_id}}',
        ...
        can_run_batch_task >> rail.Label(
            'No') >> search_invoice_in_xero >> if_invoice_id_present_and_status_not_voided_deleted
        ...
        if_contact_not_present_in_xero >> rail.Label(
            'No') >> get_all_invoice_items >> get_inventory_items_from_xero
        get_inventory_items_from_xero >> get_invoice_creation_payload >> if_invoice_has_line_items_tobe_added
        if_invoice_has_line_items_tobe_added >> rail.Label(
            'Yes') >> create_invoice_in_xero >> update_invoice_sync_status
        if_invoice_has_line_items_tobe_added >> rail.Label(
            'Yes') >> update_invoice_sync_status
