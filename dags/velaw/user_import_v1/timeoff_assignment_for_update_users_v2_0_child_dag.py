...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=config.timeoff_assignment_for_update_users_child_dag_id,
        ...
        invoke_custom_ruby_code_17 = rail.PythonOperator(
            ...
            python_callable=lambda: list(filter(lambda item:item['enabled'] in [True, 'True', 'true'],map(lambda item: {
        ...
        invoke_custom_ruby_code_19 = rail.PythonOperator(
            ...
            python_callable=lambda: list(filter(lambda item:item['status'] == 'No',map(lambda item: {
        ...
        def get_protected_timeoff_uris():
            return [uri for uri in map(lambda name: rail.find_first_by_attr_and_get_attr(
                rail.result('get_all_time_off_types_4'), 'timeofftypename', name.strip().lower(), 'uri'),
                config.protected_timeoff_type_names) if uri]
        ...
        invoke_custom_ruby_code_20 = rail.PythonOperator(
            ...
            python_callable=lambda: list(filter(lambda item: item['status'] == 'No',map(lambda item: {
                ...
                "uri": item['uri'],
                # UPDATE: in workato this status is getting derived from step 18 which step 13 here
                "status": "Yes" if item['uri'] in rail.result('log_time_off_typeto_assign_13') + get_protected_timeoff_uris() else "No",
                "policy": item['policy']
        ...
        trigger_dag_run_velaw_user_import_velawg3_child_for_timeoff_policy_update_on_each_time_off_type_for_no_accrual_v2_024 = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[0],
            trigger_dag_id=config.timeoff_policy_update_for_no_accrual_child_dag_id,
        ...
        assign_timeoffassignmentsfornewusers_25 = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                "userUri": dag_run.conf['useruri'],
                # keep protected types the user already has, so they are not unassigned
                "timeOffTypeUris": rail.result('log_time_off_typeto_assign_13') + [
                    x['uri'] for x in rail.result('invoke_custom_ruby_code_17')
                    if x['uri'] in get_protected_timeoff_uris() and x['uri'] not in rail.result('log_time_off_typeto_assign_13')]
            }
        )
        ...
        assign_timeoffassignmentsfornewuserstodisabletimeofftypes_35 = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: {
                ...
                "timeOffTypeUris": rail.result('log_time_off_typeto_disable_33') + [
                    x['uri'] for x in rail.result('invoke_custom_ruby_code_17')
                    if x['uri'] in get_protected_timeoff_uris() and x['uri'] not in rail.result('log_time_off_typeto_disable_33')]
            }
        )
