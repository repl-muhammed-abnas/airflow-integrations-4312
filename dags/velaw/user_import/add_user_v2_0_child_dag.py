...
def create_dag(config):
    ...
    with rail.create_airflow_dag(
        dag_id=f'velaw_user_import_velawg3_child_add_user_v2_0_{config.instance}',
        ...
        def get_time_off_approval_path(dag_run):
            ...
            return next(iter(filter(lambda x: x["employee_type"] == (dag_run.conf['department'] if (dag_run.conf['countryisocode'] == "US" and dag_run.conf['department'] in ("Vinson & Elkins|Vinson & Elkins LLP|Global Document Specialists", "Vinson & Elkins|Vinson & Elkins LLP|Billing Coordinators")) else ("All" if dag_run.conf['countryisocode'] == "US" else "All"))
        ...
        def get_timesheet_approval_path(dag_run):
            ...
            def get_employee_type():
                return "All" if dag_run.conf['countryisocode'] == "US" else (
                    "All" if dag_run.conf['department'] == "Vinson & Elkins|Vinson & Elkins LLP|Global Document Specialists" and dag_run.conf['jobfamilies'] == "Paralegal" else (
                        "All" if dag_run.conf['department'] == "Vinson & Elkins|Vinson & Elkins LLP|Billing Coordinators" and dag_run.conf[
                            'jobfamilies'] == "Paralegal" else dag_run.conf['department']
                    )
                )
            ...
            return next(iter(filter(lambda x: x["employee_type"] == get_employee_type()
                                    and x["country_code"] == dag_run.conf['countryisocode']
                                    and x["location"] == get_location()
        ...
        trigger_dag_run_velawg3_child_timeoff_assignment_for_new_users_v2_089 = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=[0],
            trigger_dag_id=f'velaw_user_import_velawg3_child_timeoff_assignment_for_new_users_v2_0_{config.instance}',
