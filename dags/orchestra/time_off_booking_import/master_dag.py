...
from orchestra.time_off_booking_import.utils import request_payload, python_callable_methods

# pylint: disable=too-many-statements
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_timeoff_import_master_dagid,
        description="orchestra Time Off Booking Import Master",
        ...
        write_timeoff_import_csv = rail.WriteCSVFileOperator(
            ...
            header=["booking_id","employee_id","loginname","timeofftype","startdate","enddate","hours","action","md5"],
            row=lambda item: [
                item["Booking ID"],
                item["EE ID"],
                item["Username"],
                ...
                item["End Date"],
                item["Time Off Hours"],
                item["Action (Add, Delete)"],
                sha256("".join([item["Booking ID"], item["EE ID"], item["Username"],
                             item["Time Off Type"], item["Start Date"], item["End Date"],
                             item["Time Off Hours"], item["Action (Add, Delete)"]]).encode()).hexdigest()
            ]
        )
        ...
        query_delta_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM rawdata WHERE md5 NOT IN (SELECT DISTINCT md5 FROM referencefile)""",
            name= 'deltarecords'
        )
        ...
        query_unchanged_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM rawdata WHERE md5 IN (SELECT DISTINCT md5 FROM referencefile)""",
        )
        ...
        query_invalid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM deltarecords WHERE NULLIF(booking_id, '') IS NULL or
                    NULLIF(loginname, '') IS NULL or NULLIF(hours, '') IS NULL
                    or NULLIF(startdate, '') IS NULL or NULLIF(enddate, '') IS NULL
                    or (NULLIF(action, '') IS NULL or action NOT IN ("Add", "Delete", "Taken")) """
        )
        ...
        query_valid_records = rail.QueryCollectionOperator(
            ...
            name='valid_records',
            query="""SELECT * FROM deltarecords WHERE NULLIF(booking_id, '') IS NOT NULL and
                    NULLIF(loginname, '') IS NOT NULL and NULLIF(hours, '') IS NOT NULL
                    and NULLIF(startdate, '') IS NOT NULL and NULLIF(enddate, '') IS NOT NULL
                    and NULLIF(action, '') IS NOT NULL and action IN ("Add", "Delete", "Taken")"""
        )
        ...
        query_distinct_employees = rail.QueryCollectionOperator(
            ...
            query='''SELECT DISTINCT loginname FROM valid_records'''
        )
        ...
        get_time_off_type_uri = rail.RepliconServiceOperator(
            task_id='get_time_off_type_uri',
            endpoint='/services/TimeOffService1.svc/GetAllTimeOffTypes',
            data_handler=lambda res: {
                'timeoff_uri': rail.find_first_by_attr_and_get_attr(res, 'displayText', 'Vacation', 'uri')
            }
        )
        ...
        process_distinct_employees = rail.trigger_parallel_dagrun(
            ...
            conf=lambda item:{
                'loginname': item['loginname'],
                'timeoff_uri': rail.result('get_time_off_type_uri')['timeoff_uri'],
        ...
        get_log_file_name = rail.PythonOperator(
            ...
            python_callable= lambda dag_run: f'logs_{ get_dagrun_ecid(dag_run).replace(":", "-")}' + "_" + rail.render_template(
        ...
        download_file >> load_timeoff_data >> write_timeoff_import_csv >> create_log
        ...
        has_collection_data >> rail.Label(
            "Yes") >> list_reference_file >> get_reference_filename >> download_reference_file >> \
        ...
        has_valid_records >> rail.Label(
            "Yes") >> query_distinct_employees >> get_booking_id_oef_uri >> get_time_off_type_uri >> process_distinct_employees >>\
                finish >> format_logs >> render_logs_csv >> get_log_file_name >> generate_downloadable_link >> upload_logs_to_sftp >>\
