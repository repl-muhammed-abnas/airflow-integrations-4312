...
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'crl_terminationbalance_child_{config.instance}v3',
        ...
        # Resolved at run time so the pay period end date (or a QA-supplied
        # pay_period_end_date from "Trigger DAG w/ config") bounds the export window.
        resolve_termination_window = rail.PythonOperator(
            task_id="resolve_termination_window",
            python_callable=lambda dag_run: request_payload.resolve_userenddate_window_clause(
                dag_run, config)
        )
        ...
        query_disabled_users_data = rail.QueryCollectionOperator(
            ...
            # pylint: disable=line-too-long
            query="SELECT * FROM getalluserdata WHERE {{ result('resolve_termination_window') }} AND exported != 'Yes' "
        )
        ...
        process_child_udf_update = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'crl_ermination_balance_udf_update_child_{config.instance}v3',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: {
        ...
        get_all_user_data_eligible_for_timeoff_template_removal = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM getalluserdata
                        ...
                        AND (
                            exported = 'Yes'
                            OR (
                                {{ result('resolve_termination_window') }}
        ...
        process_timeoff_template_removal = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'crl_terminationbalance_remove_timeoff_template_child_{config.instance}v3',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: {
                'user_uri': item['useruri'],
        ...
        users_report_data_collection >> resolve_termination_window >> query_disabled_users_data >> has_any_users_data >> rail.Label("Yes"
