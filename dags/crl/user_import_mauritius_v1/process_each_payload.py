...
import pendulum
import rail
from crl.user_import_mauritius_v1.utils import request_payload, python_callable_methods
from crl.user_import_mauritius_v1.tasks.get_user_prereqs import get_user_prereqs_task_group

# pylint: disable=too-many-statements
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_user_import_payload_dagid,
        description='CRL - User Import Mauritius',
        ...
        create_input_data_collection = rail.CreateCollectionOperator(
            ...
            columns={
                ...
                "holidayCalendarCode":"holiday_calendar_code",
        ...
        send_blank_payload_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | User Import Mauritius - no records in payload - {{ current_time_in_specified_tz() }}',
        ...
        query_invalid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM input_data WHERE NULLIF(emp_id, '') IS NULL or
                    NULLIF(first_name, '') IS NULL or NULLIF(last_name, '') IS NULL or NULLIF(email, '') IS NULL or NULLIF(login_name, '') IS NULL
                    or NULLIF(emp_status, '') IS NULL or NULLIF(buisness_unit_full_path, '') IS NULL or NULLIF(functional_segment, '') IS NULL
                    or NULLIF(location_full_path, '') IS NULL or NULLIF(job_code, '') IS NULL
                    or NULLIF(pay_type, '') IS NULL or NULLIF(cost_center_full_path, '') IS NULL or NULLIF(activity_type, '') IS NOT NULL
                    or emp_status NOT IN ('Active','Unpaid Leave','Terminated','Suspended','Retired','Paid Leave','Furlough','Dormant','Discarted','Deceased')
                    or NULLIF(staff_category, '') IS NULL or NULLIF(functional_sub_segment, '') IS NULL """
        )
        ...
        query_valid_records = rail.QueryCollectionOperator(
            ...
            name='valid_record',
            query="""SELECT ROW_NUMBER() OVER(ORDER BY ROWID) AS record_id,* FROM input_data WHERE NULLIF(emp_id, '') IS NOT NULL and
                    NULLIF(first_name, '') IS NOT NULL and NULLIF(last_name, '') IS NOT NULL and NULLIF(email, '') IS NOT NULL and NULLIF(login_name, '') IS NOT NULL
                    and NULLIF(emp_status, '') IS NOT NULL and NULLIF(buisness_unit_full_path, '') IS NOT NULL and NULLIF(functional_segment, '') IS NOT NULL
                    and NULLIF(location_full_path, '') IS NOT NULL and NULLIF(job_code, '') IS NOT NULL
                    and NULLIF(pay_type, '') IS NOT NULL and NULLIF(cost_center_full_path, '') IS NOT NULL and NULLIF(activity_type, '') IS NULL
                    and emp_status IN ('Active','Unpaid Leave','Terminated','Suspended','Retired','Paid Leave','Furlough','Dormant','Discarted','Deceased')
                    and NULLIF(staff_category, '') IS NOT NULL and NULLIF(functional_sub_segment, '') IS NOT NULL"""
        )
        ...
        process_start_time >> create_input_data_collection >> has_input_data >> rail.Label('No') >> send_blank_payload_email
