...
from crl.user_import_generic_row.utils import request_payload, response_filter
...
def create_child_dag_wbs(config):
    with rail.create_airflow_dag(
        ...
        description=f'CRL User Import {config.COUNTRY_NAME} - Process Supervisor',
        ...
        search_supervisor_in_replicon = rail.RepliconServiceOperator(
            ...
            data_handler=lambda res: {
                ...
                'end_date': request_payload.get_date_from_replicon_date(res[0]['userDetails']['employmentDateRange']['endDate']).strftime(DATE_FORMAT)
        ...
        update_userlog_entries = rail.WriteLogOperator(
            ...
            properties=lambda item,dag_run: {
                ...
                'status': request_payload.get_supervisor_status(item['properties']['status'], item['properties']['details'],dag_run),
                'details': request_payload.get_supervisor_message(item['properties']['status'],
                        item['properties']['action'], item['properties']['details'],dag_run)
            }
        )
        ...
        is_supervisor_end_date_in_past >> rail.Label('Yes') >> log_supervisor_end_date_in_past >>dummy_filter_user_logs
