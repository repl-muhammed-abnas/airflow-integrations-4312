...
from crl.user_import_generic_row.utils.python_callable_methods import do_format_logs, get_email_details_callable
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'CRL User Import {config.COUNTRY_NAME} - Process Log Generation',
        ...
        get_email_and_log_file_details = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: get_email_details_callable(
                dag_run.conf['job_start_time'], config.TIME_ZONE)
        )
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_logs', 'error_record_count') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | User Import ' + config.COUNTRY_NAME + ' " }} \
        ...
        format_logs >> render_logs_csv >> upload_log_to_sftp >> get_email_and_log_file_details >> generate_downloadable_link >> send_import_complete_email
