...
from crl.user_import_generic_row.utils import request_payload, python_callable_methods
from crl.user_import_generic_row.tasks.get_user_prereqs import get_user_prereqs_task_group

# pylint: disable=too-many-statements
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_user_import_payload_dagid,
        description=f'CRL - User Import {config.COUNTRY_NAME}',
        ...
        set_job_start_time = rail.PythonOperator(
            ...
            python_callable=lambda: python_callable_methods.now(config.TIME_ZONE).isoformat(),
        )
        ...
        create_input_data_collection = rail.CreateCollectionOperator(
            ...
            columns={
                ...
                "Time_Off_Schedule": "office_schedule",
        ...
        send_blank_payload_email = rail.EmailOperator(
            ...
            subject=f'{{{{ get_company_key() }}}} | User Import {config.COUNTRY_NAME} - no records in payload - {{{{ current_time_in_specified_tz() }}}}',
        ...
        query_invalid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM input_data WHERE NULLIF(emp_id, '') IS NULL or
                    ...
                    or (is_contingent ='N' and pay_type NOT IN ('Hourly','Salaried','Exception Hourly'))"""
        )
        ...
        log_invalid_records = rail.WriteLogOperator(
            ...
            message=lambda item: request_payload.get_mandatory_fields_exception_message(item, config),
            severity='Exception',
        ...
        query_valid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT ROW_NUMBER() OVER(ORDER BY ROWID) AS record_id,* FROM input_data WHERE NULLIF(emp_id, '') IS NOT NULL
                    ...
                    (is_contingent ='Y' or pay_type IN ('Hourly','Salaried','Exception Hourly'))"""
        )
        ...
        process_log_generation = rail.TriggerDagRunOperator(
            ...
            conf=lambda dag_run:{
                ...
                'country_name': config.COUNTRY_NAME,
                'job_start_time': rail.result('set_job_start_time'),
            }
        )
        ...
        set_job_start_time >> create_input_data_collection >> has_input_data >> rail.Label('No') >> send_blank_payload_email
        ...
        process_log_generation >> log_to_sumo >> should_fail_dag >> rail.Label('Yes') >> fail_dag
