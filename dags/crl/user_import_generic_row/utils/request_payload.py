...
def get_mandatory_fields_exception_message(item, config):
    missing_fields = []
    for payload_key, log_value in config.MANDATORY_FIELDS.items():
...
def is_mapper_value_found(mapper_data, input_data, config, value_found=False):
    ...
    def bool_value_check(key,input_value,mapper_value, value_type):
        ...
        if value_type=="list":
            return bool(input_value.lower() in [v.lower() if isinstance(v, str) else v for v in mapper_value])
        return bool(input_value.lower() == mapper_value.lower() if isinstance(mapper_value, str) else input_value == mapper_value)

    key_mapping_to_consider = config.KEY_MAPPING_FOR_FEED_FIELDS

    for key in config.MAPPER_KEYS_FOR_DATA_RETRIEVE:

        if key in config.SHOULD_CHECK_FOR_ALL_EXCEPT:
        ...
        if compare(mapper_data[key], 'All'):
            value_found = True
            continue
...
def get_process_users_conf(item, config):
    ...
    def get_mapper_values_for_keys(user_mapper, item):
        for mapper_row in user_mapper:
            if is_mapper_value_found(mapper_row, item, config):
                return mapper_row
        return {}
    ...
    def get_payrule_name_based_on_mapper():
        if user_mapper_values and user_mapper_values['payrule_name']:
            return user_mapper_values['payrule_name']
        return null
    ...
    return {
        ...
        **{
            ...
            "timesheet_template_uri": rail.find_first_by_attr_and_get_attr(
                ...
                if user_mapper_values and user_mapper_values['timesheet_template'] and user_mapper_values['timesheet_template'] !="NA" else null,
            "timesheet_approval_path":user_mapper_values['timesheet_approval_path']
                    if user_mapper_values and user_mapper_values['timesheet_approval_path'] and user_mapper_values['timesheet_approval_path'] != "NA" else null,

            "timeoff_template_name": user_mapper_values['time_off_template']
                    if user_mapper_values and user_mapper_values['time_off_template'] and user_mapper_values['time_off_template'] != "NA" else null,
            ...
            "timeoff_approval_path":user_mapper_values['time_off_approver']
                    if user_mapper_values and user_mapper_values['time_off_approver'] and user_mapper_values['time_off_approver'] != "NA" else null,

            "overtime_template_name": user_mapper_values['overtime_requests_template']
                    if user_mapper_values and user_mapper_values['overtime_requests_template'] and user_mapper_values['overtime_requests_template'] != "NA" else null,
            ...
            "overtime_approval_path":user_mapper_values['overtime_request_approval_path']
                    if user_mapper_values and user_mapper_values['overtime_request_approval_path'] and user_mapper_values['overtime_request_approval_path'] != "NA" else null,
            ...
            "holiday_calendar": config.get_holiday_calendar_hook(item, user_mapper_values) if user_mapper_values else null,
            "holiday_calendar_uri": rail.find_first_by_attr_and_get_attr(rail.result('get_all_holiday_calenders'),
                'displayText', config.get_holiday_calendar_hook(item, user_mapper_values), 'uri') if user_mapper_values else null,
            ...
            'country_location_uri': rail.find_first_by_attr_and_get_attr(rail.result('get_updated_location_grps'), 'full_path', config.COUNTRY_CODE, 'uri'),
...
def get_put_contingent_payload(dag_run, config):
    default_schedule = config.get_default_schedule_hook()
    return {
        "user": {
            "target": {
                "uri": null,
               ...
               [
                {
                    "schedulePolicy": {
                        ...
                        "name": dag_run.conf['office_schedule'] if dag_run.conf['office_schedule'] else default_schedule,
                        "officeSchedule": {
                            "officeScheduleUri":null,
                            "name": dag_run.conf['office_schedule'] if dag_run.conf['office_schedule'] else default_schedule
                        },
...
def get_put_user_payload(dag_run, config):
    ...
    payload_add_user = {
        "user": {
            ...
            } if dag_run.conf['overtime_approval_path'] else null,
            "customFieldValues": get_udfs('adduser', dag_run),
            "assignedActivities": [],
            "timeZone":{
...
def update_permission__hrbp_access_scope(dag_run, config):
    previous_is_hrbp_value = rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
            'customField.displayText', 'is HRBP', 'text')
...
def update_policy_set(dag_run):
    """
    Assigns timesheet template with effective date based on Sunday week start.
...
def apply_user_modifications_payload(dag_run, config):
    ...
    user_update_payload = {
        ...
        "modifications": {
            ...
            "policyDataAccessScopesToApply2": update_permission__hrbp_access_scope(dag_run, config),
            ...
            "workAuthorizationApprovalPathToApply": update_overtime_approval_path(dag_run) if dag_run.conf['overtime_approval_path'] else null,
            'activitiesToApply': [],
...
def assign_policyDataAccessScopes_to_projectmanager(dag_run, config):
    return {
