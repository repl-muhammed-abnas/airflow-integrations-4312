...
def get_time_off_to_be_assigned(dag_run, config):
    return config.get_time_off_types_hook(
        dag_run, config.APPLICABLE_TIME_OFF_TYPES, config.GLOBAL_TIME_OFF_TYPES
    )
...
def get_email_details_callable(job_start_time, time_zone):
    """Compute job timing details for the completion email."""
    _now = now(time_zone)
