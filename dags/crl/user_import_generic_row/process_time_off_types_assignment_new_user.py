...
from crl.user_import_generic_row.utils import request_payload
from crl.user_import_generic_row.utils.response_filter import get_filtered_time_off_types, get_policy_to_assign
from crl.user_import_generic_row.utils.python_callable_methods import get_required_time_off_type_details
...
def create_child_dag(config):
    ...
    for idx in range(0, config.BATCH_COUNT):
        with rail.create_airflow_dag(
            ...
            description=f'CRL User Import {config.COUNTRY_NAME} - Process TIme Off Type Assignment- New User',
            ...
            get_default_time_off_policy_schedule = rail.RepliconServiceOperator(
                ...
                data_handler=lambda response:get_policy_to_assign(
                    response)
            )
