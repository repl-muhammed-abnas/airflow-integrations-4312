...
from crl.user_import_generic_row.tasks.process_supervisor import process_supervisor_assignment_task_group
...
def create_child_dag(config):
    ...
    for idx in range(0, config.BATCH_COUNT):
        with rail.create_airflow_dag(
            ...
            description=f'CRL - User Import {config.COUNTRY_NAME} - Process New Users',
            ...
            add_contingent_user = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run:request_payload.get_put_contingent_payload(dag_run, config)
            )
            ...
            assign_scope_for_hrbp_permissions = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run: request_payload.assign_policyDataAccessScopes_to_projectmanager(dag_run, config)
            )
            ...
            is_hrbp_yes >> rail.Label("No") >> impersonate_and_create_interactive_session
            is_hrbp_yes >> rail.Label("Yes") >> assign_scope_for_hrbp_permissions >> impersonate_and_create_interactive_session
