...
from crl.user_import_usa_v10.utils.response_filter import validate_adjusted_hire_date_updated
    ...
    for timeoff_type in config.APPLICABLE_TIME_OFF_TYPES:
        if timeoff_type in config.REGULAR_USER_TIME_OFF_TYPES:
            if dag_run.conf["reg_temp"]!="Temporary":
                if (timeoff_type !="[USA] Veterans Day") or (timeoff_type =="[USA] Veterans Day" and dag_run.conf['us_veterans_status']=="Y" 
                        and dag_run.conf['buisness_unit_level_2']!= "NA05"):
        ...
        if timeoff_type == "[USA] Sick SAL":
            if validate_sick_sal_eligible_user(dag_run):
                append_timeoff_types_to_assign(timeoff_type,"NA")
            elif (dag_run.conf['buisness_unit_level_2'] == "NA05"
                  and dag_run.conf['pay_type'] in ['Salaried', 'Exception Hourly']
                  and float(dag_run.conf['std_hrs']) >= float(24)):
                append_timeoff_types_to_assign(timeoff_type,"NA")
...
def get_rehire_zero_balance_policy_line(dag_run):
    return {
        "effectiveDate": get_replicon_date(dag_run.conf['start_date']),
        "description": f"Zero balance reset on rehire {dag_run.conf['start_date']}",
        "policySet": {
            "timeOffBalanceEventScripts": [{
                "script": {
                    "description": "Set initial balance for the first day of a policy",
...
def get_all_policy_to_assign_update(dag_run,instance):
    rehire_zero_line = []
    if instance in ["trial","sandbox"]:
        if dag_run.conf.get('rehire') == 'Yes' and \
                rail.result('for_each_time_off_type_policy')['timeoff_type_name'] == '[USA] Vacation' and \
                rail.result('get_default_time_off_policy_schedule'):
            rehire_zero_line = [get_rehire_zero_balance_policy_line(dag_run)]

    if rail.result('for_each_time_off_type_policy')['policy'] and rail.result('get_default_time_off_policy_schedule'):
        data =rail.result('get_historical_policy_to_assign_list') + rehire_zero_line + rail.result('get_default_time_off_policy_schedule')
        return json.dumps(ast.literal_eval(str(data).replace("'script'", "'scriptTarget'")))

    if not rail.result('for_each_time_off_type_policy')['policy'] and rail.result('get_default_time_off_policy_schedule'):
        data = rehire_zero_line + rail.result('get_default_time_off_policy_schedule')
        return json.dumps(ast.literal_eval(str(data).replace("'script'", "'scriptTarget'")))
