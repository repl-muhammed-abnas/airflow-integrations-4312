...
from crl.user_import_usa_v10.utils import request_payload, response_filter, python_callable_methods
from crl.user_import_usa_v10.tasks.process_supervisor import process_supervisor_assignment_task_group
...
def create_child_dag(config):
    ...
    for idx in range(0, config.BATCH_COUNT):
        ...
        with rail.create_airflow_dag(
            ...
            description='CRL User Import USA Process Update Users',
            ...
            process_special_timeoff_type_accruals= rail.TriggerDagRunOperator(
                ...
                conf=lambda dag_run:{
                    "special_timeoff_type_names": config.SPECIAL_ACCRUAL_TO_TYPES,
                    ...
                    'assigned_change_effective_date': rail.find_first_by_attr_and_get_attr(rail.result('get_user_info')['userDetails']['customFieldValues'], 
                        'customField.displayText', 'Change Effective Date', 'date') if rail.result('get_user_info')['userDetails']['customFieldValues'] else null,
                    'action': 'update',
                    ...
                    'assigned_event_reason_code':rail.find_first_by_attr_and_get_attr(rail.result('get_user_info')['userDetails']['customFieldValues'],
                            ...
                            'userDetails']['customFieldValues'] else null,
                    "time_off_types_to_assign": python_callable_methods.get_time_off_to_be_assigned(dag_run, config),
                    "is_contingent": dag_run.conf['is_contingent'],
                    "previous_emp_status":rail.find_first_by_attr_and_get_attr(rail.result('get_user_info')['userDetails']['customFieldValues'],
            ...
            apply_user_modifications = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run: request_payload.apply_user_modifications_payload(
                    dag_run, config.PAI_EMPLOYEES_LOCATION_LVL_3, config.instance,
                    config.SICK_PAYOUT_SALARIED_LOCATIONS if config.ENABLE_SICK_PAYOUT_LEAVE_RETURN else None),
            )
            ...
            process_time_off_type_assignment_update_rehire_user = rail.TriggerDagRunForEachItemOperator(
                ...
                conf=lambda dag_run:{
                    ...
                    "previous_pay_type": rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
                            ...
                            'userDetails']['customFieldValues'] else null,
                    "previous_vacation_to_placeholder": rail.find_first_by_attr_and_get_attr(rail.result('get_current_oef_values'),
                            'definition.displayText', "[USA] Vacation - Placeholder Policy Name", 'textValue') if rail.result('get_user_info')[
                    ...
                    "previous_sick_to_placeholder":rail.find_first_by_attr_and_get_attr(rail.result('get_current_oef_values'),
                            'definition.displayText', "[USA] Sick - Placeholder Policy Name", 'textValue') if rail.result('get_user_info')[
                            'userDetails']['extensionFieldValues'] else null,
                    "previous_emergency_to_placeholder":rail.find_first_by_attr_and_get_attr(rail.result('get_current_oef_values'),
                            'definition.displayText', "[USA] Emergency Leave - Placeholder Policy Name", 'textValue') if rail.result('get_user_info')[
                            'userDetails']['extensionFieldValues'] else null,
                    "previous_floating_to_placeholder":rail.find_first_by_attr_and_get_attr(rail.result('get_current_oef_values'),
                            'definition.displayText', "[USA] Floating Holiday - Placeholder Policy Name", 'textValue') if rail.result('get_user_info')[
                            'userDetails']['extensionFieldValues'] else null,
                    "vacation_to_placeholder_def_uri": dag_run.conf['vacation_to_placeholder_def_uri'],
                    "sick_to_placeholder_def_uri": dag_run.conf['sick_to_placeholder_def_uri'],
                    "floating_to_placeholder_def_uri": dag_run.conf['floating_to_placeholder_def_uri'],
                    'event': dag_run.conf['event'],
                    'event_reason_code': dag_run.conf['event_reason_code'],
                    ...
                    "previous_reg_temp_value": rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
                            ...
                            'userDetails']['customFieldValues'] else null,
                    "is_reg_to_temp_transfer": request_payload.validate_reg_to_temp_transfer(dag_run)
                },
                execution_timeout=timedelta(hours=config.execution_timeout_days),
                retries=0,
            )
            ...
            process_special_timeoff_type_accrual= rail.TriggerDagRunOperator(
                ...
                conf=lambda dag_run:{
                    ...
                    "time_off_types_to_assign": python_callable_methods.get_time_off_to_be_assigned(dag_run, config),
                    "is_contingent": dag_run.conf['is_contingent'],
                    "previous_emp_status":rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
                            'customField.displayText', 'Employee Status', 'text') if rail.result('get_user_info')[
                            'userDetails']['customFieldValues'] else null,
                },
                execution_timeout=timedelta(hours=config.execution_timeout_days),
