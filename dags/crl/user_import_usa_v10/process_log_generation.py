...
from crl.user_import_usa_v10.utils.python_callable_methods import do_format_logs
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        description='CRL User Import USA - Process Log Generation',
        ...
        send_import_complete_email = rail.EmailOperator(
            ...
            bcc="{%- if result('format_logs', 'error_record_count') == 0 -%}\
                ...
                {%- else -%}\
                    "+config.alert_email+"\
                {%- endif -%}",
            subject='{{ get_company_key() + " | User Import USA " }} \
