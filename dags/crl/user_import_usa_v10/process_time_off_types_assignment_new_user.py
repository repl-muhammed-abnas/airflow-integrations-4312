...
from crl.user_import_usa_v10.utils import request_payload
from crl.user_import_usa_v10.utils.response_filter import get_filtered_time_off_types, get_policy_to_assign
from crl.user_import_usa_v10.utils.python_callable_methods import get_required_time_off_type_details, get_placeholder_policy_vacation
...
def create_child_dag(config):
    ...
    for idx in range(0, config.BATCH_COUNT):
        with rail.create_airflow_dag(
            ...
            description='CRL User Import USA- Process TIme Off Type Assignment- New User',
            ...
            for_each_time_off_assign_default_policy = rail.ForEachOperator(
                ...
                start_task='is_vacation_timeoff_type',
                end_task='for_each_time_off_assign_default_policy_end'
            )

            is_vacation_timeoff_type = rail.IfOperator(
                task_id='is_vacation_timeoff_type',
                test=lambda: rail.result("for_each_time_off_assign_default_policy")['timeoff_type_name'] == "[USA] Vacation",
                yes_task='get_placeholder_policy_for_vacation',
                no_task='get_default_time_off_policy_schedule'
            )

            get_placeholder_policy_for_vacation = rail.PythonOperator(
                task_id='get_placeholder_policy_for_vacation',
                python_callable=lambda dag_run: get_placeholder_policy_vacation(dag_run,config)
            )

            process_time_off_types_vacation_new_user = rail.TriggerDagRunOperator(
                task_id='process_time_off_types_vacation_new_user',
                trigger_dag_id=config.process_timeoff_type_assignment_vacation_new_user_dagid,
                conf=lambda dag_run:{
                    ...
                    "placeholder_timeoff_type_uri": rail.result('get_placeholder_policy_for_vacation'),
            ...
            is_vacation_to_wait_required = rail.IfOperator(
                ...
                test=lambda: bool(rail.result("process_time_off_types_vacation_new_user")),
                yes_task='wait_for_process_time_off_types_vacation_new_user',
                no_task='is_any_timeoff_type_to_assign_not_available'
            )

            wait_for_process_time_off_types_vacation_new_user = rail.WaitForDagRunsSensor(
                task_id='wait_for_process_time_off_types_vacation_new_user',
                dag_runs='{{ result("process_time_off_types_vacation_new_user") }}',
                execution_timeout=timedelta(days=config.execution_timeout_days),
            )
            ...
            for_each_time_off_assign_default_policy >> is_vacation_timeoff_type

            is_vacation_timeoff_type >> rail.Label('Yes') >> get_placeholder_policy_for_vacation >> process_time_off_types_vacation_new_user
            process_time_off_types_vacation_new_user >> for_each_time_off_assign_default_policy_end

            is_vacation_timeoff_type >> rail.Label('No') >> get_default_time_off_policy_schedule
            ...
            is_vacation_to_wait_required >> rail.Label( 'Yes') >> wait_for_process_time_off_types_vacation_new_user
            is_vacation_to_wait_required >> rail.Label( 'No') >> is_any_timeoff_type_to_assign_not_available

            wait_for_process_time_off_types_vacation_new_user >> is_any_timeoff_type_to_assign_not_available
