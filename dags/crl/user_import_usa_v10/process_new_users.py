...
from crl.user_import_usa_v10.utils import request_payload, python_callable_methods
    ...
    for idx in range(0, config.BATCH_COUNT):
        with rail.create_airflow_dag(
            ...
            is_remote_worker = rail.IfOperator(
                ...
                no_task="is_place_uri_present"
            )
            
            is_place_uri_present=rail.IfOperator(
                task_id="is_place_uri_present",
                test=lambda dag_run: bool(dag_run.conf['place_uri']),
                yes_task="assign_place",
                no_task="place_uri_not_present"
            )
            
            place_uri_not_present=rail.EmptyOperator(
                task_id="place_uri_not_present"
            )    
            ...
            is_remote_worker >> rail.Label('No') >> is_place_uri_present >> rail.Label('Yes') >> assign_place >> impersonate_and_create_interactive_session
            is_place_uri_present >> rail.Label('No') >> place_uri_not_present >> impersonate_and_create_interactive_session
