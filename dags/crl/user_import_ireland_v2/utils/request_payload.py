...
from crl.user_import_ireland_v2.utils.python_callable_methods import get_placeholder_time_off_to_be_assigned
...
KEY_MAPPING_FOR_FEED_FIELDS = {'location_level_2': 'location_full_path', 'location_level_3': 'location_full_path',
    'reg_temp': 'reg_temp', 'full_part': 'full_part', 'pay_type': 'pay_type'}

MAPPER_KEYS_FOR_DATA_RETRIEVE = ['location_level_2', 'location_level_3', 'reg_temp', 'full_part', 'pay_type']
...
def is_mapper_value_found(mapper_data, input_data, value_found=False):
    ...
    for key in MAPPER_KEYS_FOR_DATA_RETRIEVE:
        ...
        if key in ['location_level_2'] and compare(mapper_data[key], 'All'):
            value_found = True
            continue
...
def get_process_users_conf(item, config):
    ...
    def get_employee_type_name():
        ...
        return user_mapper_values['employee_type'] if user_mapper_values else null

    def get_payrule_name_based_on_mapper():
        return user_mapper_values['payrule_name'] if user_mapper_values else null
    ...
    return {
        ...
        **{
            ...
            "cost_center_label_def_uri": get_user_udfs['cost_center_label_def_uri'],
            ...
            "punch_policy_name": user_mapper_values['punch_policy']
                    if user_mapper_values and user_mapper_values['punch_policy'] and user_mapper_values['punch_policy'] else null,
            ...
            'ireland_location_uri': rail.find_first_by_attr_and_get_attr(rail.result('get_updated_location_grps'), 'full_path', 'IRL', 'uri'),
    
            "annual_to_placeholder_def_uri": rail.find_first_by_attr_and_get_attr(rail.result("get_hidden_to_oef_values"), "hidden_oef_name",
                "[IRE] Annual Leave - Placeholder Policy", "hidden_oef_uri"),
            "sick_to_placeholder_def_uri": rail.find_first_by_attr_and_get_attr(rail.result("get_hidden_to_oef_values"), "hidden_oef_name",
                "[IRE] Sick Leave - Placeholder Policy", "hidden_oef_uri"),
            "personal_to_placeholder_def_uri": rail.find_first_by_attr_and_get_attr(rail.result("get_hidden_to_oef_values"), "hidden_oef_name",
                "[IRE] Personal Flex Day - Placeholder Policy", "hidden_oef_uri"),
        }
    }
               ...
               [
                {
                    "schedulePolicy": {
                        ...
                        "name": dag_run.conf['office_schedule'] if dag_run.conf['office_schedule'] else "IRE_7.5",
                        "officeSchedule": {
                            "officeScheduleUri":null,
                            "name": dag_run.conf['office_schedule'] if dag_run.conf['office_schedule'] else "IRE_7.5"
                        },
...
def get_put_user_payload(dag_run):
    ...
    def get_policy_sets_for_new_user(dag_run):
        ...
        if dag_run.conf['schedule_policy_name']:
            policy_set.append({
                    "uri": null,
                    "name": dag_run.conf['schedule_policy_name']
                })
            ...
            [
                {
                    "schedulePolicy": {
                        "officeScheduleUri": null,
                        "name": (
                            "S537500" if dag_run.conf['location_level_3'] == "Ballina" else
                            "S537509" if dag_run.conf['location_level_3'] == "Dublin" else
                            null
                        ),
                        "officeSchedule": {
                            "officeScheduleUri":null,
    ...
    return {
        ...
        "policyDataAccessScopes": [
            ...
            "locations": [
                 {
                    "location": {
                        "uri": dag_run.conf['ireland_location_uri'],
                        "parentUri": null,
                        "name": null
                    }
                }
...
def get_default_timeoff_policy_schedule_payload(dag_run, config, for_each_loop):
    def get_reference_time_off_type_uri():
        ...
        if rail.result(for_each_loop)['timeoff_type_name'] == "[IRE] Annual Leave":
        ...
        if rail.result(for_each_loop)['timeoff_type_name'] == "[IRE] Sick Leave":
            placeholder_timeoff_type_name = get_placeholder_time_off_to_be_assigned(dag_run, config.SICK_TO_PLACEHOLDER,
            rail.result(for_each_loop)['timeoff_type_name'])

        if rail.result(for_each_loop)['timeoff_type_name'] == "[IRE] Personal Flex Hours":
            placeholder_timeoff_type_name = get_placeholder_time_off_to_be_assigned(dag_run, config.PERSONAL_TO_PLACEHOLDER,
            rail.result(for_each_loop)['timeoff_type_name'])
...
def update_schedule(dag_run, log):
    ...
    if dag_run.conf['office_schedule_uri']:
        if not current_schedule or (dag_run.conf['office_schedule'] !=
    ...
    log.append("Schedule not available in Replicon")
    return null
...
def update_policy_set(dag_run):
    """
    Assigns timesheet template with effective date based on Sunday week start for Ireland.
...
def get_payrule_effective_date(dag_run):
    ...
    new_date = date + timedelta(days=(5 - date.weekday()) % 7)
...
def get_oefs(dag_run, value):
    ...
    annual_placeholder_to_name = rail.find_first_by_attr_and_get_attr(timeoff_policy_to_assign,
        "actual_timeoff_type_name", "[IRE] Annual Leave", "placeholder_timeoff_type_name")
    sick_placeholder_to_name = rail.find_first_by_attr_and_get_attr(timeoff_policy_to_assign,
        "actual_timeoff_type_name", "[IRE] Sick Leave", "placeholder_timeoff_type_name")
    personal_placeholder_to_name = rail.find_first_by_attr_and_get_attr(timeoff_policy_to_assign,
        "actual_timeoff_type_name", "[IRE] Personal Flex Hours", "placeholder_timeoff_type_name")
    ...
    if value == "add":
        ...
        if personal_placeholder_to_name:
            add_text_oef(dag_run.conf['personal_to_placeholder_def_uri'], personal_placeholder_to_name)
    ...
    if value == "update":
        ...
        if dag_run.conf['previous_sick_to_placeholder'] != sick_placeholder_to_name:
            add_text_oef(dag_run.conf['sick_to_placeholder_def_uri'], sick_placeholder_to_name)
        if dag_run.conf['previous_personal_to_placeholder'] != personal_placeholder_to_name:
            add_text_oef(dag_run.conf['personal_to_placeholder_def_uri'], personal_placeholder_to_name)
