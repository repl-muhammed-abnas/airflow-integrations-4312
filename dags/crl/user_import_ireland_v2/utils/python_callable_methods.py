...
def get_placeholder_time_off_to_be_assigned(dag_run, timeoff_type_mapper, timeoff_type):
    key_mapping_for_feed_felids = {'location_level_2': 'location_level_2_to_consider_for_timeoff', 'location_level_3': 'location_level_3_to_consider_for_timeoff',
        'company_code': 'company_code','buisness_unit_level_1': 'buisness_unit_level_1', 'functional_segment': 'functional_segment', 'functional_sub_segment': 'functional_sub_segment',
        'buisness_unit_level_2': 'buisness_unit_level_2','cost_center':'cost_center_full_path', 'department': 'department_name',  'reg_temp': 'reg_temp', 'full_part':'full_part', 'pay_type': 'pay_type',
        'activity_type':'activity_type','std_hrs': 'std_hrs'}
    mapper_keys_for_data_retrieve = ['std_hrs']

    should_check_all = ['location_level_2', 'location_level_3','buisness_unit_level_1', 'functional_segment', 'functional_sub_segment', 'cost_center', 'department',  'reg_temp',
        'full_part', 'pay_type', 'activity_type', 'job_level,''buisness_unit_level_2','company_code']
    ...
    def is_timeoff_type_mapper_value_found(mapper_data,value_found=False):
        ...
        def bool_value_check(key,input_value,mapper_value, value_type):
            if key=="std_hrs":
                return bool(float(input_value) == float(mapper_value))
...
def get_time_off_to_be_assigned(config,dag_run):    
    ...
    for timeoff_type in config.APPLICABLE_TIME_OFF_TYPES:
        if timeoff_type in config.GLOBAL_TIME_OFF_TYPES:
            if timeoff_type=="[IRE] Heritage Day":
                if dag_run.conf['location_level_3'] =="Ballina":
                    append_timeoff_types_to_assign(timeoff_type,"NA")
            else:
                append_timeoff_types_to_assign(timeoff_type,"NA")

        if timeoff_type =="[IRE] Annual Leave":
        ...
        if timeoff_type =="[IRE] Sick Leave":
            placeholder_policy = get_placeholder_time_off_to_be_assigned(dag_run, config.SICK_TO_PLACEHOLDER, timeoff_type)
            if placeholder_policy:
                append_timeoff_types_to_assign(timeoff_type,placeholder_policy)

        if timeoff_type =="[IRE] Personal Flex Hours":
            placeholder_policy = get_placeholder_time_off_to_be_assigned(dag_run, config.PERSONAL_TO_PLACEHOLDER, timeoff_type)
            if placeholder_policy:
                append_timeoff_types_to_assign(timeoff_type,placeholder_policy)
...
def time_off_types_to_be_assigned_update(dag_run):
    ...
    def get_status(item):
        if item['timeoff_type_name'] == '[IRE] Annual Leave':
        ...
        if item['timeoff_type_name'] == '[IRE] Personal Flex Hours':
            current_placeholder = rail.find_first_by_attr_and_get_attr(dag_run.conf['time_off_types_to_assign'],
                "actual_timeoff_type_name","[IRE] Personal Flex Hours","placeholder_timeoff_type_name")
            if dag_run.conf['previous_personal_to_placeholder'] != current_placeholder:
                return 'No'
        
        if item['timeoff_type_name'] == '[IRE] Sick Leave':
            current_placeholder = rail.find_first_by_attr_and_get_attr(dag_run.conf['time_off_types_to_assign'],
