...
from crl.user_import_ireland_v2.tasks.get_user_prereqs import get_user_prereqs_task_group

# pylint: disable=too-many-statements
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_user_import_payload_dagid,
        description='CRL - User Import ireland',
        ...
        send_blank_payload_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | User Import Ireland - no records in payload - {{ current_time_in_specified_tz() }}',
        ...
        query_invalid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM input_data WHERE NULLIF(emp_id, '') IS NULL or
                    ...
                    NULLIF(office_schedule, '') IS NULL """
        )
        ...
        process_log_generation >> log_to_sumo >> should_fail_dag >> rail.Label('No') >> fail_dag
