...
import rail
from crl.termination_balance_export_us.utils import request_payload
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        compose_terminated_employees_with_headers = rail.WriteCSVFileOperator(
            ...
            row=lambda item: [
                ...
                item['Location (Current) (Full Path)'].split(
                    '/')[1].strip() if item['Location (Current) (Full Path)'].split('/')[1].strip() else item['Location (Current) (Full Path)']
            ]
        )
        ...
        query_final_payroll_collection = rail.QueryCollectionOperator(
            ...
            OR (Time_Off_Type = '[USA] Floating Holiday' AND Location IN ('California', 'Colorado', 'Montana', 'Nebraska')))
        ...
        query_valid_termination_balance_data >> has_valid_data >> rail.Label("Yes"
                                                                             ) >> final_termination_balance_data_to_csv >> get_exported_custom_field >> process_child_udf_update >> no_of_records_size_including_header_footer >> create_document >>\
