...
import rail
from crl.termination_balance_export_v1.utils import request_payload


def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'crl_terminationbalance_child_{config.instance}_v1',
        ...
        process_child_udf_update = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'crl_ermination_balance_udf_update_child_{config.instance}_v1',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: {
        ...
        process_timeoff_template_removal = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'crl_terminationbalance_remove_timeoff_template_child_{config.instance}_v1',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: {
                'user_uri': item['useruri'],
