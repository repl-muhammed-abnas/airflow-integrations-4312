...
from crl.user_import_ireland_v1.utils import request_payload, response_filter, python_callable_methods
from crl.user_import_ireland_v1.tasks.process_supervisor import process_supervisor_assignment_task_group
...
def create_child_dag(config):
    ...
    for idx in range(0, config.BATCH_COUNT):
        ...
        with rail.create_airflow_dag(
            ...
            description='CRL User Import Ireland Process Update Users',
            ...
            apply_user_modifications = rail.RepliconServiceOperator(
                ...
                endpoint='/services/ImportService1.svc/ApplyUserModifications3',
                data=lambda dag_run: request_payload.apply_user_modifications_payload(dag_run,config.instance),
            )
            ...
            get_time_off_types_to_assign = rail.PythonOperator(
                ...
                python_callable= lambda dag_run: python_callable_methods.get_time_off_to_be_assigned(config,dag_run)
            )
            ...
            process_time_off_type_assignment_update_rehire_user = rail.TriggerDagRunForEachItemOperator(
                ...
                conf=lambda dag_run:{
                    ...
                    "previous_annual_to_placeholder": rail.find_first_by_attr_and_get_attr(rail.result('get_current_oef_values'),
                            'definition.displayText', "[IRE] Annual Leave - Placeholder Policy", 'textValue') if rail.result('get_user_info')[
                    ...
                    "previous_sick_to_placeholder":rail.find_first_by_attr_and_get_attr(rail.result('get_current_oef_values'),
                            'definition.displayText', "[IRE] Sick Leave - Placeholder Policy", 'textValue') if rail.result('get_user_info')[
                            'userDetails']['extensionFieldValues'] else null,
                    "previous_personal_to_placeholder":rail.find_first_by_attr_and_get_attr(rail.result('get_current_oef_values'),
                            'definition.displayText', "[IRE] Personal Flex Day - Placeholder Policy", 'textValue') if rail.result('get_user_info')[
                            'userDetails']['extensionFieldValues'] else null,
                    ...
                    "personal_to_placeholder_def_uri": dag_run.conf['personal_to_placeholder_def_uri'],
                },
                execution_timeout=timedelta(hours=config.execution_timeout_days),
                retries=0,
            )
            ...
            get_current_udf_values >> get_current_oef_values >> get_effective_user_groupmembership >> get_assigned_policy_to_user >> get_assigned_place_to_user >> apply_user_modifications
