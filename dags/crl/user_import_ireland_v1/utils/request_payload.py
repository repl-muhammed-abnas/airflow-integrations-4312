...
from crl.user_import_ireland_v1.utils.python_callable_methods import get_placeholder_time_off_to_be_assigned
...
def get_default_timeoff_policy_schedule_payload(dag_run, config, for_each_loop):
    def get_reference_time_off_type_uri():
        ...
        if rail.result(for_each_loop)['timeoff_type_name'] == "[IRE] Personal Flex Day":
...
def update_permission_set(dag_run,instance):
    if instance in ["trial","sandbox","prod"]:
...
def apply_user_modifications_payload(dag_run,instance):
    log=[]
    user_update_payload = {
        "user": {
            "uri": dag_run.conf['useruri']
        ...
        "modifications": {
            ...
            "permissionSetsToApply": update_permission_set(dag_run,instance),
