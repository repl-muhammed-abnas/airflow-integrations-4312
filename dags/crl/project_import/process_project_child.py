...
import rail
from crl.project_import.utils import request_payload,response_filter,custom_method
    ...
    for idx in range(0, config.PROJECT_BATCH_COUNT):
        ...
        with rail.create_airflow_dag(
            ...
            get_project_data_from_query = rail.QueryCollectionOperator(
                ...
                query="""SELECT * from validwbsdata WHERE projectcode == :program_code""",
                query_params = {
            ...
            get_service_center_details = rail.RepliconServiceOperator(
                task_id="get_service_center_details",
                endpoint="/services/ServiceCenterListService1.svc/GetData",
                data= request_payload.get_service_center_details,
            ...
            is_groups_available_in_replicon = rail.IfOperator(
                ...
                test=lambda dag_run: not bool(request_payload.check_groups_data(dag_run)),
            ...
            log_project_exception = rail.WriteLogOperator(
                ...
                properties=lambda dag_run: {
                    ...
                    "Status": "Exception",
                    "details": "Project Updated Successfully," + str(request_payload.check_groups_data(dag_run)) if
                    request_payload.does_wbs_exist() else "Project Added Successfully," + str(request_payload.check_groups_data(dag_run)),
                }
            )
            ...
            add_project_team_to_task = rail.RepliconServiceCallForEachItemOperator(
                task_id= 'add_project_team_to_task',
                endpoint= '/services/TaskService1.svc/AssignProjectTeamMembersToTaskResourceAssignments',
                items= '{{ result("add_task") | to_json }}',
                data=lambda item: {
            ...
            has_tasks_to_skip = rail.IfOperator(
                ...
                no_task= 'is_business_area_present'
            )
            ...
            is_business_area_present = rail.IfOperator(
                ...
                test=lambda: bool(request_payload.get_project_data()['businessarea']),
            ...
            bulk_update_project_team_members = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run: {
                    ...
                    'resourceUri': [item['uri'] for item in request_payload.get_cost_center_uris(
                        dag_run)] if request_payload.get_cost_center_uris(dag_run) else [],
            ...
            create_project_log >> log_project_and_exception_log >> get_project_data_from_query >> get_project_details >>\
                get_service_center_details >> create_project >> is_groups_available_in_replicon

            is_groups_available_in_replicon >> rail.Label(
                "Yes") >> log_project_success >> is_new_project

            is_groups_available_in_replicon >> rail.Label(
                "No") >> log_project_exception >> is_new_project
            ...
            has_tasks_to_add >> rail.Label(
                "Yes") >> add_task >> add_project_team_to_task >> log_task_added_success_error >> has_tasks_to_update
            ...
            has_tasks_to_skip >> rail.Label(
                "Yes") >> log_task_skipped >> is_business_area_present

            has_tasks_to_skip >> rail.Label(
                "No") >> is_business_area_present

            is_business_area_present >> rail.Label(
                "Yes") >> update_cost_center >> bulk_update_project_team_members >> catch_and_log_errors
