...
import rail
from crl.timeoff_balance_export.utils import python_callable, response_filter
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        query_report_data = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM sickandbanedtimeoffdetails WHERE (sick_eligible='No' OR banked_eligible='No') AND (NULLIF(empid, '') IS NOT NULL OR empid!="") AND CAST(timeoff_balance AS DECIMAL(10,2)) > 0"""
        )
        ...
        process_child_udf_update = rail.TriggerDagRunForEachItemOperator(
            ...
            conf=lambda item: {
                ...
                'set_banked_payout': rail.result('get_banked_payout_udf_option_uris')["yes"],
        ...
        get_banked_payout_udf_option_uris >> process_child_udf_update >> wait_process_child_udf_update >> \
