...
from crl.timeoff_balance_export.mapper.time_off_balance_mapper import Time_off_mappper, sicked_timeoff_names, banked_timeoff_names
    ...
    for item in data:
        if item['timeoff_balance'] and float(item['timeoff_balance']) > 0:
            ...
            result_list.append({
                ...
                "update_bpo_udf":"yes" if item['timeoff_type'] in banked_timeoff_names else "no",
            })
    return result_list
...
def get_file_name(config):
    file_name = "P" + config.adp_gv_system + config.gv_system_number + "476" + \
            "_" + now(config.time_zone).strftime("%Y%m%d%H%M%S") + "_" + "CATIME_HRMD03_DUT8G2I"
    if if_payroll_processing_date_is_today(config):
        file_name = "P" + config.adp_gv_system + config.gv_system_number + "476" + \
            "_" + now(config.time_zone).strftime("%Y%m%d%H%M%S") + "_" + "CATIME_HRMD04_DUT8G2I"
    return file_name
