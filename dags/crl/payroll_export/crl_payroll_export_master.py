...
import pendulum
import rail
from crl.payroll_export.utils import request_payload
from crl.payroll_export.utils import python_callable
# pylint: disable=no-name-in-module
from crl.payroll_export.mapper.regular_time_employee_types import REGULAR_TIME_EMPLOYEE_TYPES

# pylint: disable=too-many-statements
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"crl_payroll_export_master_{config.instance}",
        description=f"CRL Payroll Export Master {config.instance}",
        ...
        send_email_for_export_started = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon payroll export started for ' + \
        ...
        create_object = rail.TriggerDagRunForEachItemOperator(
            ...
            batch_size=10000,
            trigger_dag_id=f"create_object_canada_child_{config.instance}",
        ...
        get_file_name = rail.PythonOperator(
            ...
            pendulum.now(config.time_zone).strftime(
                "%Y%m%d%H%M%S") + "_" + "CATIME_HRMD01_DUT8G2I"
        )
        ...
        update_payrun_name_to_nodata = rail.RepliconServiceOperator(
            ...
            data={
                ...
                "name":  "{{ result('get_file_name')}}" + "_nodata"
            }
        )
        ...
        get_all_employee_types_from_mapper = rail.PythonOperator(
            ...
            python_callable=lambda: python_callable.get_all_required_employee_types(
                REGULAR_TIME_EMPLOYEE_TYPES)
        )

        query_list_in_final_payroll_collection = rail.QueryCollectionOperator(
            task_id='query_list_in_final_payroll_collection',
            query="SELECT * FROM finalpayrolldata WHERE finalpayrolldata.SUBTY = '2000' AND finalpayrolldata.Employee_Type_Name IN "+
              "('Hourly_Regular_Full-Time_Project','Hourly_Regular_Full-Time','Hourly_Regular_Part-Time','Hourly_Regular_Part-Time_Project',"+
              ...
              "'Temporary PT Hourly') AND finalpayrolldata.SUBTY NOT IN ('" +str(config.paycode)+"')"+
                "UNION ALL "+
                "SELECT * FROM finalpayrolldata WHERE finalpayrolldata.SUBTY <> '2000' AND finalpayrolldata.SUBTY NOT IN ('" +str(config.paycode)+"')"
        )
        ...
        can_process_run >> process_start_time >> send_email_for_export_started >> create_log >> logging_job_start_time >> get_adp_payroll_script >> is_file_format_script_present
