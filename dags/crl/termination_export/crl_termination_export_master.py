...
import pendulum
import rail
from crl.termination_export.utils import request_payload
from crl.termination_export.utils import python_callable

# pylint: disable=too-many-statements
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"crl_termination_export_master_{config.instance}",
        description=f"CRL Termination Export Master {config.instance}",
        ...
        get_all_disabled_users = rail.RepliconServiceOperator(
            task_id="get_all_disabled_users",
        ...
        create_object = rail.TriggerDagRunForEachItemOperator(
            ...
            items=lambda: rail.result('get_all_disabled_users'),
            batch_size=10000,
            trigger_dag_id=f"create_object_termination_child_{config.instance}",
        ...
        get_file_name = rail.PythonOperator(
            ...
            pendulum.now(config.time_zone).strftime(
                "%Y%m%d%H%M%S") + "_" + "CATIME_HRMD" +
            request_payload.get_sequence(
                config.CANADA_PAYROLL_CALENDER_MAPPER_TO_USE, config.time_zone)+"_DUT8G2I"
        )
        ...
        create_payroll_download_batch = rail.RepliconServiceOperator(
            ...
            data=lambda:request_payload.get_create_payroll_download_batch_payload(config.time_zone)
        )
        ...
        send_email_for_no_payroll_data = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Payroll Termination Export Notification',
        ...
        create_payrun_batch = rail.RepliconServiceOperator(
            ...
            data=lambda:request_payload.get_create_payrun_batch_payload(config.time_zone)
        )
        ...
        send_invalid_records_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | Replicon Payroll Termination Export - Invalid records found - {{ result("process_start_time").start_time }}',
        ...
        get_all_employee_types_from_mapper = rail.PythonOperator(
            ...
            python_callable=lambda: python_callable.get_all_required_employee_types(
                config.REGULAR_TIME_EMPLOYEE_TYPES_TO_USE)
        )
        ...
        query_list_in_final_payroll_collection = rail.QueryCollectionOperator(
            ...
            query=f'''SELECT finalpayrolldata.RECTY,finalpayrolldata.CLIID,finalpayrolldata.INTCA,finalpayrolldata.ORDNO,
            ...
            WHERE finalpayrolldata.CLIID = getalluserdata.Employee_ID AND DATE(getalluserdata.User_End_Date) > DATE('{python_callable.get_start_date_begin_of_week()}') AND DATE(getalluserdata.User_End_Date) <= DATE('{python_callable.get_end_date_begin_of_week()}')
            ...
            AND DATE(getalluserdata.User_End_Date) > DATE('{python_callable.get_start_date_begin_of_week()}') AND DATE(getalluserdata.User_End_Date) <= DATE('{python_callable.get_end_date_begin_of_week()}') 
            AND finalpayrolldata.SUBTY <> "2000"'''
        )
        ...
        create_document = rail.RenderTemplateOperator(
            ...
            target='artifact',
            template_file='schema/canada_payroll_export_data.txt',
        ...
        send_email_for_export_copmpletion = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | Replicon Termination export completed for ' + \
        ...
        run_dag_on_payrollcalendar >> rail.Label("Yes") >> can_process_run >> process_start_time >> create_log >> logging_job_start_time >> get_adp_payroll_script >> is_file_format_script_present
        is_file_format_script_present >> rail.Label("Yes") >> get_all_enabled_locations >> get_all_disabled_users >> create_object\
            >> wait_create_object >> create_object_uris >> get_all_employee_type \
            >> get_location_child_hierarchy_data >> get_file_name \
        ...
        has_empty_empid_data >> rail.Label(
            'No') >> get_all_employee_types_from_mapper >> get_user_report_details >> load_user_report\
                  >> users_report_payload_to_csv >> formated_users_data_to_csv\
            >> users_report_data_collection >> query_list_in_final_payroll_collection >> has_item_data >> rail.Label(
