...
def get_service_center_details_for_project_duplicate(dag_run):
    return {
        "page": "1",
        "pagesize": "1000",
        "columnUris": [
        ...
        "filterExpression": {
            ...
            "rightExpression": {
            "value": {
                "text": dag_run.conf['project_data']['companycode']
            }
            }
        }
    }
...
def get_project_by_code():
    return {
        ...
        "filterExpression": {
            ...
            "rightExpression": {
                "value": {
                    "text": get_project_data()['projectcode']
                }
            }
        }
    }
...
def does_wbs_exist():
    return bool(rail.result('get_project_by_name'))

def get_create_project_target_param():
    if does_wbs_exist():
        return {
            "uri": rail.result('get_project_by_name')['uri']
        }
    return None
...
def update_duplicate_project_full_payload(dag_run):
    """Update duplicate projects with same network code with full payload content"""
    status= {'1': 'In Progress', '2': 'Completed', '3': 'Cancelled', '4': 'Tentative'}
    project_data = dag_run.conf['project_data']
    ...
    def get_client_payload():
        return {
            "clients": [
                {
                "client": {
                    "code": project_data['clientcode'],
                },
                "costAllocationPercentage": "100"
                }
            ],
            "effectiveDate": None
        } if project_data['clientcode'] else None

    modifications = {
        "codeToApply":  {
            "value": project_data["projectcode"]
        },
        "descriptionToApply": {
            "value": project_data["projectdescription"]
        } if project_data["projectdescription"] else None,
        "clientAssignmentsSchedulesToApply": get_client_payload(),
        "statusToApply": {
            "name": status[project_data['projectstatus']] if project_data[
                'projectstatus'] and project_data['projectstatus'] in ['1','2','3','4'] else 'In Progress'
        },
        "serviceCenterToApply": {
            "serviceCenter": {
                "uri": rail.result("get_service_center_details_for_project")['uri'],
        ...
        "customFieldsToApply": [
            {
                ...
                "text": project_data['project_business_area']
            }
        ] if project_data['project_business_area'] else []
    }
...
def get_update_task_payload_for_duplicate(dag_run):
    return {
        "project": {
            "uri": dag_run.conf['projecturi'],
        },
...
def get_batch_put_task_payload_for_duplicate(dag_run):
    return {
        "project": {
            "uri": dag_run.conf['projecturi'],
        },
