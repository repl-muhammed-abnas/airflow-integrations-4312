...
def _process_tasks_for_add_update_skip(get_tasks_task_id, get_project_data_task_id='get_project_data_from_query'):
    """Reusable function for processing tasks - works for both primary and duplicate projects"""
    current_task_in_project = rail.result(get_tasks_task_id) if bool(rail.result(get_tasks_task_id)) else []
    task_to_process = rail.load_all_records(rail.result(get_project_data_task_id)) if rail.result(get_project_data_task_id) else []
    ...
    for task in task_to_process:
        ...
        if task_details:
            if can_update_task(task_details, task):
                ...
                task['company_code'] = task_details['company_code']
...
def get_task_to_add_update_skip():
    """For primary project"""
    return _process_tasks_for_add_update_skip('get_all_tasks_for_project')

def get_task_to_add_update_skip_for_duplicate():
    """For duplicate projects"""
    return _process_tasks_for_add_update_skip('get_all_tasks_for_duplicate_project')
...
def get_company_code_details():
    filtered_data =[item for s in rail.result("get_service_center_details_for_task") for item in s]
    seen_uris = set()
    unique_data = []
