...
from crl.user_import_non_live.utils import request_payload, python_callable_methods, response_filter
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        description='CRL User Import- Process TIme Off Type No Accrual',
        ...
        get_historical_policy_to_assign_list_disable_user = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: python_callable_methods.get_historical_policy_to_assign_list(
                dag_run,'for_each_time_off_type_no_accural')
        )

        get_no_accrual_policy_line = rail.PythonOperator(
            task_id='get_no_accrual_policy_line',
            python_callable=python_callable_methods.get_no_accrual_policy_line
        )
        ...
        disable_timeoff_assignment_for_user = rail.RepliconServiceOperator(
            task_id="disable_timeoff_assignment_for_user",
        ...
        can_run_batch_task >> rail.Label('No') >> get_user_time_off_policy_summary

        get_user_time_off_policy_summary >> for_each_time_off_type_no_accural >> get_balance_summary_for_user
        for_each_time_off_type_no_accural >> for_each_time_off_type_no_accural_end
        for_each_time_off_type_no_accural_end
        get_balance_summary_for_user >> get_historical_policy_to_assign_list_disable_user >> get_no_accrual_policy_line
        ...
        for_each_time_off_type_no_accural_end >> disable_timeoff_assignment_for_user >> catch_and_log_errors >> log_to_sumo
