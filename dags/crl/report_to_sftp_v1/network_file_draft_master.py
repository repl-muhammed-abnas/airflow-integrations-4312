...
from crl.report_to_sftp_v1.utlis import python_callable


# Report configuration  
COLUMNS = ['Employee ID', 'User Name', 'Employee Status', 'Employee Type', 'Standard Hours', 
          'Default Activity', 'Company Code', 'Location', 'Business Segment', 'Functional Segment', 
          'Business Unit', 'Cost Center', 'Bus Area', 'Cost Center (Current) (Full Path)', 
          'Profit Center', 'Department', 'Department Code', 'Entry Date', 'Project Name', 
          'Project Code', 'Network Name', 'Network Code', 'Network Activity Name', 
          'Network Activity Code', 'WC Company Code', 'Business Area Network Activity', 
          'Total Hrs', 'Timesheet Period', 'Approval Status', 'Approval Date/Time']
...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.network_file_draft_master_dag_id,
        description=f'CharlesRiverLaboratoriesSandbox Network_File_Draft Report Export',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=datetime(2024, 8, 1, tz=config.time_zone),
        schedule_interval=config.schedule_interval_network_file_draft,
        ...
        get_filename = rail.PythonOperator(
            ...
            op_args=[config.Network_File_Draft_Filename, config.time_zone],
        )

        get_report_details = rail.RepliconReportDetailsOperator(
            task_id='get_report_details',
            report_name=config.Network_File_Draft_Report
        )
