...
import rail
from crl.termination_balance_export.utils import request_payload


def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=f'crl_terminationbalance_child_{config.instance}',
        ...
        process_child_udf_update = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=f'crl_ermination_balance_udf_update_child_{config.instance}',
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: {
