...
from crl.user_import_canada_v6.utils import request_payload, python_callable_methods
from crl.user_import_canada_v6.utils.response_filter import get_filtered_time_off_types, assigned_timeoffs_types_to_user, get_policy_to_assign_for_timeoff
...
def create_child_dag(config):
    ...
    for idx in range(0, config.BATCH_COUNT):
        ...
        with rail.create_airflow_dag(
            dag_id=f"{config.process_timeoff_type_assignment_update_rehire_user_dagid}{get_postfix}",
            description='CRL User Import- Process TIme Off Type Assignment- Update User',
            ...
            time_off_types_to_be_assigned = rail.PythonOperator(
                ...
                python_callable=lambda dag_run: python_callable_methods.time_off_types_to_be_assigned(
                    dag_run,config.REFERENCE_BASED_TIME_OFF_TYPES, config.SPECIAL_TIMEOFF_TYPES_ACCRUALS, config.SPECIAL_ACCRUAL_TIMEOFF_TYPE_NAMES)
            )
            ...
            for_each_time_off_type_policy = rail.ForEachOperator(
                ...
                start_task='get_balance_summary_for_user_update',
                end_task='for_each_time_off_policy_end'
            )

            get_balance_summary_for_user_update = rail.RepliconServiceOperator(
                task_id="get_balance_summary_for_user_update",
                endpoint="/services/TimeOffService2.svc/GetBalanceSummaryForAccount",
                data=lambda dag_run:{
                    "account": {
                        "userUri": dag_run.conf['useruri'],
            ...
            get_default_time_off_policy_schedule = rail.RepliconServiceOperator(
                ...
                data=lambda dag_run: request_payload.get_default_timeoff_policy_set_schedule_for_timeofftype(
                    dag_run, config.REFERENCE_BASED_TIME_OFF_TYPES,'for_each_time_off_type_policy', config.PERSONAL_DAYS_REFERENCE_TIME_OFF_TYPES),
                data_handler=lambda response, dag_run:get_policy_to_assign_for_timeoff(
                    response, config.TIMEOFF_TYPE_POLICY_MODIFY, dag_run, 'for_each_time_off_type_policy', config)
            )

            get_all_policy_to_assign = rail.PythonOperator(
                task_id='get_all_policy_to_assign',
                python_callable=lambda dag_run: python_callable_methods.get_all_policy_to_assign(dag_run,config)
            )
            ...
            for_each_time_off_type_policy >> get_balance_summary_for_user_update
            get_balance_summary_for_user_update >> get_historical_policy_to_assign_list >> get_default_time_off_policy_schedule >> get_all_policy_to_assign
            ...
            for_each_time_off_policy_end >> is_any_timeoff_type_to_assign_not_available >> rail.Label('Yes') >> log_time_off_type_not_available
