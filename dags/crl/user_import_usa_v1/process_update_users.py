...
from crl.user_import_usa_v1.utils import request_payload, response_filter, python_callable_methods
from crl.user_import_usa_v1.tasks.process_supervisor import process_supervisor_assignment_task_group
    ...
    for idx in range(0, config.BATCH_COUNT):
        ...
        with rail.create_airflow_dag(
            ...
            is_status_active = rail.IfOperator(
                ...
                no_task="process_special_timeoff_type_accruals"
            )

            process_special_timeoff_type_accruals= rail.TriggerDagRunForEachItemOperator(
                task_id='process_special_timeoff_type_accruals',
                trigger_dag_id=config.process_timeoff_type_special_accrual_dagid,
                items=config.SPECIAL_ACCRUAL_TO_TYPES,
                conf=lambda dag_run,item:{
                    "timeoff_type_name": item,
            ...
            is_contingent_user = rail.IfOperator(
                ...
                no_task="is_previously_unpaid_leave_or_suspended"
            )
            ...
            is_previously_unpaid_leave_or_suspended = rail.IfOperator(
                task_id="is_previously_unpaid_leave_or_suspended",
                test=request_payload.validate_previous_unpaid_leave_or_suspended,
            ...
            process_special_timeoff_type_accrual= rail.TriggerDagRunForEachItemOperator(
            ...
            is_status_active >> rail.Label("No") >> process_special_timeoff_type_accruals >> wait_for_process_special_timeoff_type_accruals
            wait_for_process_special_timeoff_type_accruals >> process_user_disable >> wait_for_process_user_disable >> catch_and_log_errors
            is_status_active >> rail.Label("Yes") >> is_contingent_user >> rail.Label('No') >> is_previously_unpaid_leave_or_suspended

            is_previously_unpaid_leave_or_suspended >> rail.Label("Yes") >> enable_login
            is_previously_unpaid_leave_or_suspended >> rail.Label("No") >> is_rehire_user
