...
def get_placeholder_time_off_to_be_assigned(dag_run, timeoff_type_mapper, timeoff_type):
    ...
    if timeoff_type =="[USA] Vacation":
        key_mapping_for_feed_felids = {'location_level_2': 'location_level_2', 'location_level_3': 'location_level_3',
...
def time_off_type_to_be_updated(dag_run):
    for item in dag_run.conf['time_off_types_to_assign']:
        if item["actual_timeoff_type_name"] == dag_run.conf["timeoff_type_name"]:
            return {
                "time_off_type_name": dag_run.conf["timeoff_type_name"],
                "time_off_type_uri": rail.find_first_by_attr_and_get_attr(
                    rail.result('get_user_time_off_policy_summary'), 'timeoff_type_name', dag_run.conf["timeoff_type_name"], 'timeoff_type_uri')
            } if rail.find_first_by_attr_and_get_attr(
                    rail.result('get_user_time_off_policy_summary'), 'timeoff_type_name', dag_run.conf["timeoff_type_name"], 'timeoff_type_uri') else null
    return null
