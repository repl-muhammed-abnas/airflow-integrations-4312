...
def get_modified_policy(policy_set, dag_run, config, timeoff_type_name):
    ...
    if timeoff_type_name == "[USA] Floating Holiday":
        ...
        def get_balance_amount():
            ...
            if placeholder_timeoff_type == "[HOLLISTERHC][CCHC][RHC][Oklahomacity]":
                if month ==12:
                    return 16
                return 16 if month <=6 else 8

            if placeholder_timeoff_type == "[CRLHC1][SHC][ROCKHC][MORRHC][COGHC][FrdrkisHC]":
                if month ==12:
                    return 24
                return 24 if month <=6 else (16 if month <=9 else 8)

            if placeholder_timeoff_type == "[MKPHC][ASCHC][SPENHC][MATTHC]":
                if month ==12:
                    return 28
                return 28 if month <=6 else ((16 if dag_run.conf['holiday_calendar'] == "MKPHC" else 20) if month <=9 else 8)

            if placeholder_timeoff_type == "[FrdrkrmsHouHC][DURFRDSKOHC]":
                if month ==12:
                    return 32
                return 32 if month <=6 else ((16 if dag_run.conf['holiday_calendar'] == "DURFRDSKOHC" else 20) if month <=9 else 8)

            if placeholder_timeoff_type == "[RALHC][HHC]":
                if month ==12:
                    return 44
    ...
    if timeoff_type_name == "[USA] Emergency Leave":
        def get_amount():
            if dag_run.conf['location_level_2'] in ["California", "New York", "Colorado"]:
...
def get_policy_to_assign_for_timeoff(response,dag_run,for_each_loop, config):
    ...
    if rail.result(for_each_loop)['timeoff_type_name'] == "[USA] Vacation":
        ...
        tenure = float((change_effective_date - adjusted_hire_date).days//365)
        ...
        if rail.find_first_by_attr_and_get_attr(dag_run.conf['time_off_types_to_assign'],
            ...
            def get_vp_policy_set():
                return {
                        ...
                        "additionalParameters": [{
                            ...
                            "value": {
                            "number": rail.result('get_balance_summary_for_user_vacation_to_type')['timeRemaining']
                            }
                        }]
                        }],
            ...
            return [{
                    "description":"effective on "+ dag_run.conf['change_effective_date'],
                    "effectiveDate": {
                            'year': change_effective_date.year,
                            'month': change_effective_date.month,
                            'day': change_effective_date.day
                        },
    ...
    return list(filter(lambda x: bool(x['effectiveDate']), map(lambda item: {
        ...
        'effectiveDate': get_effective_date(item),
        'policySet': item['policySet'] if rail.result(for_each_loop)['timeoff_type_name'] != "[USA] Emergency Leave"
            else get_modified_policy_update(dag_run,item['policySet'], rail.result(for_each_loop)['timeoff_type_name']),
    }, response)))
...
def get_modified_policy_update(dag_run, policy_set, timeoff_type_name):
    if timeoff_type_name == "[USA] Emergency Leave":
        def get_amount():
            if dag_run.conf['location_level_2'] in ["California", "New York", "Colorado"]:
                return float(dag_run.conf['std_hrs'])*2.0
