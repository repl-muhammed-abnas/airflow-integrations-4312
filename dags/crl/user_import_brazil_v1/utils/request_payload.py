...
# from crl.user_import_brazil_v1.utils.python_callable_methods import get_placeholder_time_off_to_be_assigned
...
def get_process_users_conf(item, config):
    ...
    get_sell_back_approval_path = rail.result("get_sell_back_approval_path_dropdown_values")
    ...
    return {
        ...
        **{
            ...
            "sell_back_approval_path_name": user_mapper_values['sell_back_approval_path']
                    if user_mapper_values and user_mapper_values.get('sell_back_approval_path') else null,
            "sell_back_approval_path_uri": rail.find_first_by_attr_and_get_attr(
                get_sell_back_approval_path, 'displayText', user_mapper_values['sell_back_approval_path'], 'uri')
                    if user_mapper_values and user_mapper_values.get('sell_back_approval_path') else null,
...
def get_disabled_time_off_type_names(dag_run):
    # Names flagged enabled=False in the payload (Req 5, new users only).
    return {item['actual_timeoff_type_name']
            for item in dag_run.conf['time_off_types_to_assign']
            if not item.get('enabled', True)}


def get_time_off_type_uris_to_disable(dag_run):
    # Assigned (available-in-Replicon) types that must end up disabled.
    disabled_names = get_disabled_time_off_type_names(dag_run)
    return [time_off['timeoff_type_uri']
            for time_off in rail.result('get_required_time_off_type_details_to_assign')['result']
            if time_off['timeoff_type_name'] in disabled_names]


def put_timeoff_assignment_enabled_only(dag_run):
    # Re-send the allowed set with the disabled types removed. They keep the
    # policy already assigned but drop out of the allowed set = disabled.
    disabled_names = get_disabled_time_off_type_names(dag_run)
    enabled_uris = [time_off['timeoff_type_uri']
                    for time_off in rail.result('get_required_time_off_type_details_to_assign')['result']
                    if time_off['timeoff_type_name'] not in disabled_names]
    return {
        "userUri": dag_run.conf['useruri'],
        "timeOffTypeUris": enabled_uris
    }


def put_timeoff_assignment_for_user_update(dag_run):
    # Req 6: on a delta update the integration must NOT change the enabled/disabled
    # status of already-assigned time off types. Since PutTimeOffTypeAssignmentsForUser
    # sets the complete allowed set, sending the full mapper set would re-enable types
    # that were intentionally disabled (e.g. new-user disables from Req 5).
    # So the allowed set = types already assigned AND currently enabled (state preserved)
    #                      + required mapper types that are not assigned yet (new).
    required = rail.result('get_required_time_off_type_details_to_assign')['result']
    required_uris = [time_off['timeoff_type_uri'] for time_off in required]

    # Rehire keeps the existing behavior of (re)enabling the full applicable set.
    if dag_run.conf.get('rehire') == 'Yes':
        return {
            "userUri": dag_run.conf['useruri'],
            "timeOffTypeUris": required_uris
        }

    current_assignments = rail.result('get_user_time_off_policy_summary') or []
    currently_assigned_uris = {item['timeoff_type_uri'] for item in current_assignments}
    currently_enabled_uris = [item['timeoff_type_uri'] for item in current_assignments if item['enabled']]

    new_uris = [uri for uri in required_uris if uri not in currently_assigned_uris]

    return {
        "userUri": dag_run.conf['useruri'],
        "timeOffTypeUris": currently_enabled_uris + new_uris
    }


def get_new_disabled_time_off_type_uris(dag_run):
    if dag_run.conf.get('rehire') == 'Yes':
        return []
    disabled_names = get_disabled_time_off_type_names(dag_run)
    current_assignments = rail.result('get_user_time_off_policy_summary') or []
    ...
    return [time_off['timeoff_type_uri']
            for time_off in rail.result('get_required_time_off_type_details_to_assign')['result']
            if time_off['timeoff_type_name'] in disabled_names
            and time_off['timeoff_type_uri'] not in currently_assigned_uris]


def put_timeoff_assignment_for_user_update_enabled_only(dag_run):
    disabled_names = get_disabled_time_off_type_names(dag_run)
    required = rail.result('get_required_time_off_type_details_to_assign')['result']
    ...
    new_enabled_uris = [time_off['timeoff_type_uri'] for time_off in required
                        if time_off['timeoff_type_uri'] not in currently_assigned_uris
                        and time_off['timeoff_type_name'] not in disabled_names]

    return {
        "userUri": dag_run.conf['useruri'],
        "timeOffTypeUris": currently_enabled_uris + new_enabled_uris
    }
