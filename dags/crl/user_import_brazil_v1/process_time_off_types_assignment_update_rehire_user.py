...
from crl.user_import_brazil_v1.utils import request_payload, python_callable_methods
from crl.user_import_brazil_v1.utils.response_filter import get_filtered_time_off_types, assigned_timeoffs_types_to_user, get_policy_to_assign_for_timeoff
...
def create_child_dag(config):
    ...
    for idx in range(0, config.BATCH_COUNT):
        ...
        with rail.create_airflow_dag(
            ...
            has_new_time_off_types_to_disable = rail.IfOperator(
                task_id='has_new_time_off_types_to_disable',
                test=lambda dag_run: bool(request_payload.get_new_disabled_time_off_type_uris(dag_run)),
                yes_task='disable_new_time_off_types',
                no_task='get_hidden_oefs_payload'
            )

            disable_new_time_off_types = rail.RepliconServiceOperator(
                task_id='disable_new_time_off_types',
                endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
                data=request_payload.put_timeoff_assignment_for_user_update_enabled_only
            )
            ...
            for_each_time_off_policy_end >> has_new_time_off_types_to_disable
            has_new_time_off_types_to_disable >> rail.Label('Yes') >> disable_new_time_off_types >> get_hidden_oefs_payload
            has_new_time_off_types_to_disable >> rail.Label('No') >> get_hidden_oefs_payload

            get_hidden_oefs_payload >> is_update_hidden_oefs_needed
