...
from crl.user_import_brazil_v1.tasks.process_supervisor import process_supervisor_assignment_task_group
...
def create_child_dag(config):
    ...
    for idx in range(0, config.BATCH_COUNT):
        ...
        with rail.create_airflow_dag(
            ...
            has_sell_back_approval_path = rail.IfOperator(
                task_id='has_sell_back_approval_path',
                test=lambda dag_run: bool(dag_run.conf['sell_back_approval_path_uri']),
                yes_task='update_sell_back_approval_path',
                no_task='is_remove_hrbp_permission_set'
            )

            update_sell_back_approval_path = rail.RepliconServiceOperator(
                task_id='update_sell_back_approval_path',
                endpoint='/services/TimeOffBalancePayoutApprovalService1.svc/UpdateApprovalPathForUser',
                data=lambda dag_run: {
                    "userUri": dag_run.conf['useruri'],
                    "approvalPathUri": dag_run.conf['sell_back_approval_path_uri']
                }
            )
            ...
            apply_user_modifications >> has_sell_back_approval_path
            has_sell_back_approval_path >> rail.Label('Yes') >> update_sell_back_approval_path >> is_remove_hrbp_permission_set
            has_sell_back_approval_path >> rail.Label('No') >> is_remove_hrbp_permission_set

            is_remove_hrbp_permission_set >> rail.Label('No') >> impersonate_and_create_interactive_session
