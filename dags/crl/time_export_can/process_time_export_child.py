...
import rail
from crl.time_export_can.utils import custom_methods, request_payload
from crl.time_export_can.tasks.time_export_task import time_data_export
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        send_no_data_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon Canada Time Data Export - No records to export - {{ current_time_in_specified_tz() }}',
        ...
        send_invalid_records_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_email,
            subject='{{ get_company_key() }} | Replicon Canada Time Data Export - Invalid records found - {{ current_time_in_specified_tz() }}',
        ...
        filter_raw_timeexport_data = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM mandatory_data WHERE ((project_user == "Yes" AND NULLIF(short_id, '') IS NOT NULL) OR
        ...
        post_to_target = rail.HTTPUploadFileOperator(
            ...
            endpoint="/http/api/sap/v1/Replicon_TimeEntry",
            http_conn_id=config.http_conn_id_dev,
            content="{{result('create_json_payload') }}",
            retries=0,
            headers={
        ...
        send_posting_failed_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_email,
            subject='{{ get_company_key() }} | Replicon Canada Time Data Export - Failed while posting to API endpoint - {{ current_time_in_specified_tz() }}',
        ...
        send_success_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_email,
            subject='{{ get_company_key() }} | Replicon Canada Time Data Export - Completed Successfully - {{ current_time_in_specified_tz() }}',
        ...
        final_export_data >> create_json_payload >> upload_export_data_to_sftp >> \
            process_end_time >> logging_job_end_time >> log_file_data_to_csv >> \
                upload_log_data_to_sftp >> get_access_token >> access_token >> post_to_target >> \
                    is_post_to_endpoint_failed

        is_post_to_endpoint_failed >> rail.Label(
            "No") >> is_run_failed >> rail.Label(
