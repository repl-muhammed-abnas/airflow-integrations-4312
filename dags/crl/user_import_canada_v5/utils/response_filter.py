...
def get_modified_policy(policy_set, time_off_type_name, dag_run, user_value):
    ...
    if time_off_type_name == "[CAN] Jour personnel/Personal Days" and dag_run.conf['reg_temp']=='Regular' \
        ...
        def get_balance_amount():
            compare_month_value = datetime.strptime(dag_run.conf['adjusted_hire_date'],DATE_FORMAT)+relativedelta(months=3)
            if 1 <= compare_month_value.month <=3:
                return 30
            if 3 < compare_month_value.month <=6:
                return 22.50
            return 15
...
def get_updated_service_period_service_annivarsary(old_service_period, hrs):
    new_service_period =[]
    for item in old_service_period.split(","):
        splitted_value = item.split(":")
        splitted_value[-1] = str(hrs)
        item = ":".join(splitted_value)
        new_service_period.append(item)

    return ",".join(new_service_period)

def get_modified_service_anniversary_policy_set(policyset, dag_run, action):
    if action=="add" or dag_run.conf['rehire']=='Yes' or (float(dag_run.conf['std_hrs']) != float(dag_run.conf["previous_std_hrs"])) \
        or (float(dag_run.conf['std_hrs']) != float(37.5)):
            ...
            if item['script']['name'] == 'Accrual Based on Service Period':
                for value in item['additionalParameters']:
                    if value['keyUri']=="urn:replicon:script-key:parameter:tenure-period":
                        value['value']={"text": get_updated_service_period_service_annivarsary(value['value']['text'], float(dag_run.conf['std_hrs']))}
                        break
    return policyset
...
def get_policy_to_assign_for_timeoff(response,modify_default_policy,dag_run,for_each_loop, config):
    ...
    def get_effective_date_service_anniversary_update_rehire(item):
        if dag_run.conf['rehire']=='Yes' or \
            (dag_run.conf["reg_temp"]=="Regular" and dag_run.conf["previous_reg_temp"]=="Temporary") or \
            (dag_run.conf['assigned_location_grp'] == 'STCONSTANT' and dag_run.conf['location_level_3'] != 'STCONSTANT'):
        ...
        _date = to_datetime(dag_run.conf['change_effective_date'], format=DATE_FORMAT)
        return {
                'year': _date.year,
                'month': _date.month,
                'day': _date.day
    ...
    def get_modified_policy_set_for_hrly_to_sal(policy_set_personal_leave):

        default_yearly_entitlement = 0.0
        for item in policy_set_personal_leave['timeOffBalanceEventScripts']:
        ...
        balance_at_effective_date =  rail.result('get_balance_summary_for_user_update')['timeRemaining']
        change_effective_date = datetime.strptime(dag_run.conf['change_effective_date'],DATE_FORMAT)

        if change_effective_date.day<=15:
            per_month_hrs = default_yearly_entitlement/12.0
            remaining_months_for_accrual = 12-(change_effective_date.month-1)
            hrs_to_allocate = float(remaining_months_for_accrual) * per_month_hrs

        if change_effective_date.day>15:
            per_month_hrs = default_yearly_entitlement/12.0
            remaining_months_for_accrual = 12-change_effective_date.month
            hrs_to_allocate = float(remaining_months_for_accrual) * per_month_hrs

        policy_set_personal_leave['timeOffBalanceEventScripts'].append(
            {
             "script": {
            ...
            "additionalParameters": [{
                ...
                "value": {
                "number": hrs_to_allocate + float(balance_at_effective_date)
                }
            }]
            }
        )

        return policy_set_personal_leave
    ...
    def get_effective_date(item):
        if rail.result(for_each_loop)['timeoff_type_name'] == "[CAN] Anniversaire de service/ Service Anniversary":
            return get_effective_date_service_anniversary_update_rehire(item)

        if rail.result(for_each_loop)['timeoff_type_name'] == "[CAN] Vacances/Vacation" and dag_run.conf['location_level_3']!="STCONSTANT":
            return get_effective_date_montreal_vacation(item)
        present = datetime.strptime(dag_run.conf['change_effective_date']
            if dag_run.conf['rehire']!='Yes' else dag_run.conf['start_date'],DATE_FORMAT).strftime(DATE_FORMAT)
        current_date = str(
            (to_datetime(present) + get_offset(item)).strftime(DATE_FORMAT))
        return get_replicon_date(current_date)

    def get_policy_set(item):
        if dag_run.conf['rehire']!='Yes':
            if rail.result(for_each_loop)['timeoff_type_name'] == "[CAN] Jour personnel/Personal Days" and dag_run.conf['reg_temp']=='Regular' \
                and dag_run.conf['location_level_3']!="STCONSTANT" and dag_run.conf['pay_type']=="Hourly" and dag_run.conf['previous_pay_type'] in ["Salaried", "Exception Hourly"]:
                return item['policySet']

            if rail.result(for_each_loop)['timeoff_type_name'] == "[CAN] Jour personnel/Personal Days" and dag_run.conf['reg_temp']=='Regular' \
                and dag_run.conf['location_level_3']!="STCONSTANT" and dag_run.conf['previous_pay_type']=="Hourly" and dag_run.conf['pay_type'] in ["Salaried", "Exception Hourly"]:
                return get_modified_policy_set_for_hrly_to_sal(item['policySet'])

        if rail.result(for_each_loop)['timeoff_type_name'] == "[CAN] Anniversaire de service/ Service Anniversary":
            return get_modified_service_anniversary_policy_set(item['policySet'], dag_run, "update_rehire")

        if rail.result(for_each_loop)['timeoff_type_name'] in modify_default_policy:
            return get_modified_policy(item['policySet'], rail.result(for_each_loop)['timeoff_type_name'],dag_run,'update')

        if dag_run.conf['full_part'] =="Part-Time" and rail.result(for_each_loop)['timeoff_type_name'] == "[CAN] Vacances/Vacation":
            return get_modified_montreal_vacation_policy(item['policySet'], item['startOffset']['offsetValue'],
...
def get_policy_to_assign_for_service_anniversary_add(response,dag_run):
    if not response:
        return None
    ...
    def get_effective_date(item):
        present = datetime.strptime(dag_run.conf['adjusted_hire_date'],DATE_FORMAT).strftime(DATE_FORMAT)
        _date = (to_datetime(present) + get_offset(item))
        return {
                'year': _date.year,
                'month': _date.month,
                'day': _date.day
    ...
    res = list(map(lambda item: {
        'description': 'Added by integration based on adjusted hire date - '+ str(dag_run.conf['adjusted_hire_date']),
        'effectiveDate': get_effective_date(item),
        'policySet': get_modified_service_anniversary_policy_set(item['policySet'],dag_run, "add")
    }, response))
