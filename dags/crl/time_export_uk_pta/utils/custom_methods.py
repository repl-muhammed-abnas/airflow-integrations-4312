...
def get_time_export_name(config):
    ...
    PTA Export uses 'Time Extract_PTA_UK_' prefix
    """
    offset_time = pendulum.now(config.time_zone).strftime(EXPORT_FILE_TIMESTAMP)
    return {
        "time_export_name": f"Time Extract_PTA_UK_{offset_time}",
        "no_data_time_export_name": f"NO_DATA_Time_Extract_PTA_UK_{offset_time}"
    }
