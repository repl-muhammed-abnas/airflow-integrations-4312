...
import pendulum
import rail
from crl.time_export_uk_pta.utils import custom_methods, request_payload
from crl.time_export_uk_pta.tasks.time_export_task import time_data_export
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        description="CRL UK PTA Time Export process time export",
        ...
        create_raw_timeexport_data_collection = rail.CreateCollectionOperator(
            ...
            columns={
                ...
                'Time Type (UK) (Code)': 'time_type_uk_code',
        ...
        send_no_data_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon UK PTA Time Data Export - No records to export - {{ current_time_in_specified_tz() }}',
        ...
        send_invalid_records_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_email,
            subject='{{ get_company_key() }} | Replicon UK PTA Time Data Export - Invalid records found - {{ current_time_in_specified_tz() }}',
        ...
        # UK-specific: Simplified filtering - all users use short_id
        filter_raw_timeexport_data = rail.QueryCollectionOperator(
            task_id="filter_raw_timeexport_data",
            query="""SELECT * FROM mandatory_data WHERE NULLIF(short_id, '') IS NOT NULL""",
            name='final_data'
        )
        ...
        format_raw_export_data = rail.DataAdaptorOperator(
            ...
            columns=['employee_id', 'entry_date', 'hours', 'time_type_uk_code',
        ...
        send_success_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_email,
            subject='{{ get_company_key() }} | Replicon UK PTA Time Data Export - Completed Successfully - {{ current_time_in_specified_tz() }}',
