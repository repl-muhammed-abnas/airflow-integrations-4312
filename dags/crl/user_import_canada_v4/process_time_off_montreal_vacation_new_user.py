...
from crl.user_import_canada_v4.utils import response_filter

null = None

def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_timeoff_type_assignment_montreal_vacation_new_user_dagid,
        description='CRL User Import- Process TIme Off Type Montreal Vacation',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        max_active_runs=config.max_active_runs_process_montreal_vacation_new_user,
    ) as dag:
        ...
        get_default_time_off_policy_schedule = rail.RepliconServiceOperator(
            ...
            data_handler=lambda response,dag_run: response_filter.get_policy_to_assign_for_montreal_vacation_add(response,dag_run,config.FULL_TIME_HRS_VACATION)
        )
