...
from crl.user_import_canada_v4.utils import request_payload, python_callable_methods, response_filter
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        description='CRL User Import- Process TIme Off Type Special Accrual',
        ...
        is_special_timeoff_type_assigned_to_user = rail.IfOperator(
            ...
            test=lambda: bool(list(filter(lambda x: x['timeoff_type_name'] in config.SPECIAL_ACCRUAL_TIMEOFF_TYPE_NAMES,
        ...
        is_reason_available_in_mapper_and_code_10_in_payload = rail.IfOperator(
            ...
            yes_task="is_reason_or_reasoncode_updated",
            no_task="catch_and_log_errors"
        )

        is_reason_or_reasoncode_updated = rail.IfOperator(
            task_id = "is_reason_or_reasoncode_updated",
            test=lambda dag_run:dag_run.conf['event']!=dag_run.conf['assigned_event'] or
                dag_run.conf['event_reason_code']!= dag_run.conf['assigned_event_reason_code'],
        ...
        is_time_off_type_to_be_updated = rail.IfOperator(
            ...
            yes_task="get_balance_summary_for_user",
            no_task="catch_and_log_errors"
        )
        ...
        get_balance_summary_for_user = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run:{
                "account": {
                    "userUri": dag_run.conf['useruri'],
                    "timeOffTypeUri": rail.result('time_off_type_to_be_updated')['time_off_type_uri']
                },
        ...
        get_historical_policy_to_assign_special_accrual = rail.PythonOperator(
            ...
            python_callable=lambda dag_run: python_callable_methods.get_historical_policy_to_assign_special_accrual_list(
                dag_run,rail.result('time_off_type_to_be_updated')['time_off_type_uri'], config)
        )
        ...
        get_all_time_off_types >> get_user_time_off_policy_summary >> is_special_timeoff_type_assigned_to_user
        is_special_timeoff_type_assigned_to_user >> rail.Label('No') >> catch_and_log_errors
        is_special_timeoff_type_assigned_to_user >> rail.Label('Yes') >> is_status_updaid_or_paid_leave >> rail.Label('No') >> catch_and_log_errors
        is_status_updaid_or_paid_leave >> rail.Label('Yes') >> is_reason_available_in_mapper_and_code_10_in_payload >> rail.Label('No') >> catch_and_log_errors
        is_reason_available_in_mapper_and_code_10_in_payload >> rail.Label('Yes') >> is_reason_or_reasoncode_updated >> rail.Label('No') >> catch_and_log_errors
        is_reason_or_reasoncode_updated >> rail.Label('Yes') >> time_off_type_to_be_updated >> is_time_off_type_to_be_updated
        is_time_off_type_to_be_updated >> rail.Label('No') >> catch_and_log_errors
        is_time_off_type_to_be_updated >> rail.Label('Yes') >> get_balance_summary_for_user >> get_historical_policy_to_assign_special_accrual
        ...
        put_user_timeoff_policy_schedule_blank_policy >> catch_and_log_errors >> log_to_sumo
