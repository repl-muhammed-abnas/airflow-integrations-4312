...
import rail
from crl.timeoff_balance_export_us.utils import python_callable, response_filter
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        query_report_data = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM sickandbanedtimeoffdetails WHERE (sick_eligible='No') AND (NULLIF(empid, '') IS NOT NULL OR empid!="") AND CAST(timeoff_balance AS DECIMAL(10,2)) > 0"""
        )
