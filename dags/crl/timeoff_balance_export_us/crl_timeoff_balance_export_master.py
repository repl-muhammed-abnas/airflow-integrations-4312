...
import rail
from crl.timeoff_balance_export_us.utils import request_payload, python_callable, response_filter
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        schedule_interval=config.jan_1st_schedule_interval,
        ...
        query_report_data = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM sickandbanedtimeoffdetails WHERE (sick_eligible='Yes') AND (NULLIF(empid, '') IS NOT NULL OR empid!="") AND CAST(timeoff_balance AS DECIMAL(10,2)) > 0"""
        )
        ...
        compose_item_payout_csv_file = rail.WriteCSVFileOperator(
            ...
            row=request_payload.get_compose_item_payout_data_row,
        )
        ...
        process_start_time >> get_file_name >> get_report_details >> run_report_entry
