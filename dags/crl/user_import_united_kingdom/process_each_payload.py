...
from crl.user_import_united_kingdom.tasks.get_user_prereqs import get_user_prereqs_task_group

# pylint: disable=too-many-statements
def create_child_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.process_user_import_payload_dagid,
        description='CRL - User Import UK',
        ...
        send_blank_payload_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | User Import UK - no records in payload - {{ current_time_in_specified_tz() }}',
        ...
        query_invalid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM input_data WHERE NULLIF(emp_id, '') IS NULL or
                    ...
                    NULLIF(office_schedule, '') IS NULL or NULLIF(pay_scale_group,'') IS NULL or NULLIF(job_level,'') IS NULL"""
        )
        ...
        query_valid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT ROW_NUMBER() OVER(ORDER BY ROWID) AS record_id,* FROM input_data WHERE NULLIF(emp_id, '') IS NOT NULL
                    ...
                    and NULLIF(office_schedule, '') IS NOT NULL and NULLIF(pay_scale_group,'') IS NOT NULL and NULLIF(job_level,'') IS NOT NULL"""
        )
