...
from crl.user_import_united_kingdom.utils import request_payload, python_callable_methods
from crl.user_import_united_kingdom.utils.response_filter import get_filtered_time_off_types, assigned_timeoffs_types_to_user, get_policy_to_assign_for_timeoff, parse_office_schedule_working_days
...
def create_child_dag(config):
    ...
    for idx in range(0, config.BATCH_COUNT):
        ...
        with rail.create_airflow_dag(
            ...
            description='CRL User Import United Kingdom - Process TIme Off Type Assignment- Update User',
            ...
            get_historical_policy_to_assign_list_disable_user = rail.PythonOperator(
                ...
                python_callable=lambda dag_run: python_callable_methods.get_historical_policy_to_assign_list(
                    dag_run,'update', 'for_each_time_off_type_no_accural')
            )
            ...
            is_time_off_types_to_be_assigned = rail.IfOperator(
                ...
                yes_task = "is_schedule_based_proration_applicable",
                no_task = "is_any_timeoff_type_to_assign_not_available"
            )

            # For days-based part-time proration (3000/3080/3300) the working-day
            # count is read from the employee's time-off schedule.
            is_schedule_based_proration_applicable = rail.IfOperator(
                task_id='is_schedule_based_proration_applicable',
                test=lambda dag_run: dag_run.conf.get('full_part') == 'Part-Time'
                    and dag_run.conf.get('company_code') in config.FT_WORKING_DAYS,
                yes_task='get_office_schedule_details',
                no_task='for_each_time_off_type_policy'
            )

            get_office_schedule_details = rail.RepliconServiceOperator(
                task_id='get_office_schedule_details',
                endpoint='/services/OfficeScheduleService1.svc/GetOfficeScheduleDetails',
                data=lambda dag_run: {"officeScheduleUri": dag_run.conf['office_schedule_uri']},
                data_handler=parse_office_schedule_working_days
            )
            ...
            for_each_time_off_type_policy = rail.ForEachOperator(
                ...
                start_task='get_historical_policy_to_assign_list',
            ...
            get_historical_policy_to_assign_list = rail.PythonOperator(
                ...
                python_callable=lambda dag_run: python_callable_methods.get_historical_policy_to_assign_list(
                    dag_run,'update' if dag_run.conf['rehire']!='Yes' else 'rehire','for_each_time_off_type_policy')
            )
            ...
            is_time_off_types_to_be_assigned >> rail.Label('Yes') >> is_schedule_based_proration_applicable
            is_schedule_based_proration_applicable >> rail.Label('Yes') >> get_office_schedule_details >> for_each_time_off_type_policy
            is_schedule_based_proration_applicable >> rail.Label('No') >> for_each_time_off_type_policy
            ...
            for_each_time_off_type_policy >> get_historical_policy_to_assign_list
