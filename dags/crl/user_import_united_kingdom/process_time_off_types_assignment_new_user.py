...
from crl.user_import_united_kingdom.utils import request_payload
from crl.user_import_united_kingdom.utils.response_filter import get_filtered_time_off_types, get_policy_to_assign, parse_office_schedule_working_days
from crl.user_import_united_kingdom.utils.python_callable_methods import get_required_time_off_type_details
...
def create_child_dag(config):
    ...
    for idx in range(0, config.BATCH_COUNT):
        with rail.create_airflow_dag(
            ...
            description='CRL User Import United Kingdom- Process TIme Off Type Assignment- New User',
            ...
            is_schedule_based_proration_applicable = rail.IfOperator(
                ...
                no_task='for_each_time_off_assign_default_policy'
            )
            ...
            get_default_time_off_policy_schedule = rail.RepliconServiceOperator(
                ...
                data_handler=lambda response, dag_run: get_policy_to_assign(
                    response, dag_run, 'for_each_time_off_assign_default_policy')
            )
            ...
            get_all_time_off_types >> get_required_time_off_type_details_to_assign >> put_timeoff_assignment_for_user >> is_schedule_based_proration_applicable
            is_schedule_based_proration_applicable >> rail.Label('Yes') >> get_office_schedule_details >> for_each_time_off_assign_default_policy
            is_schedule_based_proration_applicable >> rail.Label('No') >> for_each_time_off_assign_default_policy
