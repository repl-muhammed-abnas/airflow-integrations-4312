...
from crl.user_import_united_kingdom.utils.python_callable_methods import get_placeholder_time_off_to_be_assigned
...
MANDATORY_FIELDS = {
        ...
        'office_schedule': 'Time_Off_Schedule',
        'pay_scale_group':'Pay_Scale_Group',
        'job_level':'Job_Level'
}

KEY_MAPPING_FOR_FEED_FIELDS = {'location_level_2': 'location_full_path', 'location_level_3': 'location_full_path','company_code':'company_code',
    'buisness_unit_level_2': 'buisness_unit_full_path', 'buisness_unit_level_1': 'buisness_unit_full_path',
    'functional_segment':'functional_segment','functional_sub_segment': 'functional_sub_segment',
    'cost_center': 'cost_center_full_path', 'department' :'department', 'job_code': 'job_code', 'job_level':'job_level',
    'reg_temp': 'reg_temp', 'pay_type': 'pay_type', 'full_part':'full_part','pay_scale_group':'pay_scale_group',
    'activity_type': 'activity_type'}

MAPPER_KEYS_FOR_DATA_RETRIEVE = ['location_level_2', 'location_level_3','company_code',
    'buisness_unit_level_2', 'reg_temp','buisness_unit_level_1','functional_segment','functional_sub_segment','cost_center','department','job_code',
    'job_level','full_part','pay_type','pay_scale_group', 'activity_type']

SHOULD_CHECK_FOR_ALL_EXCEPT = ['location_level_3',
    'buisness_unit_level_2', 'job_code','job_level','pay_scale_group']
...
def is_mapper_value_found(mapper_data, input_data,value_found=False):
    ...
    for key in MAPPER_KEYS_FOR_DATA_RETRIEVE:
        ...
        if key in ['location_level_2', 'location_level_3',
            'buisness_unit_level_2', 'department_grp_lvl_2','buisness_unit_level_1',
            'functional_segment','functional_sub_segment','cost_center','department',
            'pay_scale_group','job_code','job_level'
...
def build_schedule_based_payrule_name(base_payrule_name, std_hrs):
    """v12.1: schedule-based pay rule name = '<base> <weekly hours>hr'
    (e.g. '[UK] 3000 Pay Rule 38hr', '[UK] 3000 Pay Rule PT OT 37.5hr').
    Whole numbers are rendered without a decimal (38hr), fractions kept (37.5hr).
    Returns None when std_hrs is missing/unparseable."""
    if not base_payrule_name or not std_hrs:
        return None
    try:
        return f"{base_payrule_name} {'{:g}'.format(float(std_hrs))}hr"
    except (TypeError, ValueError):
        return None


def is_pre2010_protected_timesheet(current_timesheet_template):
    """v12.2: True if the user's currently assigned timesheet template is a legacy
    Pre-2010 3300 template whose assignment must be left unchanged on update."""
    if not current_timesheet_template:
        return False
    name = current_timesheet_template.get('displayText') or current_timesheet_template.get('name')
    return name in uk_config.PRE2010_PROTECTED_TIMESHEET_TEMPLATES
...
def get_process_users_conf(item, config):
    ...
    def get_payrule_name_based_on_mapper():
        if not (user_mapper_values and user_mapper_values['payrule_name']):
            return null
        base_payrule_name = user_mapper_values['payrule_name']
        # v12.1: for schedule-based companies (3000) prefer "<base> <hours>hr" when a
        # matching pay rule is configured in Replicon; else fall back to the base rule.
        if item['company_code'] in uk_config.SCHEDULE_BASED_PAYRULE_COMPANIES:
            schedule_based_name = build_schedule_based_payrule_name(base_payrule_name, item.get('std_hrs'))
            if schedule_based_name and rail.find_first_by_attr_and_get_attr(
                    rail.result("get_all_payrule_scripts"), 'displayText', schedule_based_name, 'uri'):
                return schedule_based_name
        return base_payrule_name
    ...
    # Resolve once (v12.1 performs a Replicon existence check) and reuse below.
    resolved_payrule_name = get_payrule_name_based_on_mapper()
    ...
    return {
        ...
        **{
            ...
            "overtime_template_name": user_mapper_values['overtime_requests_template']
                    if user_mapper_values and user_mapper_values['overtime_requests_template'] else null,
            "overtime_template_uri": rail.find_first_by_attr_and_get_attr(
                rail.result("get_all_policy_sets"),'displayText',user_mapper_values['overtime_requests_template'],"uri")
                if user_mapper_values and user_mapper_values['overtime_requests_template'] else null,
            "overtime_approval_path":user_mapper_values['overtime_request_approval_path']
                    if user_mapper_values and user_mapper_values['overtime_request_approval_path'] else null,
            ...
            "timezone": user_mapper_values['timezone'] if user_mapper_values and user_mapper_values['timezone'] else null,
            ...
            'payrule_name': resolved_payrule_name,

            'payrule_script_uri': (rail.find_first_by_attr_and_get_attr(
                    rail.result("get_all_payrule_scripts"),'displayText', resolved_payrule_name,"uri")
                if resolved_payrule_name else null),
            ...
            'office_schedule_uri': rail.find_first_by_attr_and_get_attr(rail.result("get_all_office_schedules"),'displayText',item['office_schedule'],"uri")
                if item['office_schedule'] else null,
            ...
            "annual_to_placeholder_def_uri": rail.find_first_by_attr_and_get_attr(rail.result("get_hidden_to_oef_values"), "hidden_oef_name",
                "[UK] Annual leave - Placeholder Policy Name", "hidden_oef_uri"),
            "sick_to_placeholder_def_uri": rail.find_first_by_attr_and_get_attr(rail.result("get_hidden_to_oef_values"), "hidden_oef_name",
                "[UK] Sick leave - Placeholder Policy Name", "hidden_oef_uri"),

            'uk_location_uri': rail.find_first_by_attr_and_get_attr(rail.result('get_updated_location_grps'), 'full_path', 'GBR', 'uri'),
               ...
               [
                {
                    "schedulePolicy": {
                        ...
                        "name": dag_run.conf['office_schedule'] if dag_run.conf['office_schedule'] else " [UK] M-F 8 hours/day",
                        "officeSchedule": {
                            "officeScheduleUri":null,
                            "name": dag_run.conf['office_schedule'] if dag_run.conf['office_schedule'] else " [UK] M-F 8 hours/day"
                        },
...
def get_put_user_payload(dag_run):
    ...
    def get_policy_sets_for_new_user(dag_run):
        ...
        if dag_run.conf['overtime_template_name']:
            policy_set.append({
                    "uri": dag_run.conf['overtime_template_uri'],
                    "name": null
                })
    ...
    payload_add_user = {
        "user": {
            ...
            ] if dag_run.conf['office_schedule'] and dag_run.conf['office_schedule_uri'] else
            [
                {
                    "schedulePolicy": {
                        "officeScheduleUri": null,
                        "name": "[UK] M-F 8 hours/day",
                        "officeSchedule": {
                            "officeScheduleUri":null,
                            "name": "[UK] M-F 8 hours/day"
                        },
...
def update_payrule_script(dag_run):
    ...
    # v9 #2: Preserve legacy "Pre 2010" pay rules for 3300 employees.
    if dag_run.conf.get('company_code') == '3300' and current_payrulescript:
        current_payrule_name = current_payrulescript[-1]['payRuleScript']['displayText']
        if current_payrule_name in uk_config.PROTECTED_3300_PAY_RULES:
            return null
    ...
    if dag_run.conf['is_hrbp']!= previous_is_hrbp_value and dag_run.conf['is_hrbp']=="Y":
        ...
        "policyDataAccessScopes": [
            ...
            "locations": [
                 {
                    "location": {
                        "uri": dag_run.conf['uk_location_uri'],
                        "parentUri": null,
                        "name": null
                    }
                }
...
def update_policy_set(dag_run):
    """
    Assigns timesheet template with effective date based on Sunday week start for UK.
    ...
    # v12.2: leave the timesheet assignment unchanged for legacy Pre-2010 3300 templates.
    skip_timesheet = is_pre2010_protected_timesheet(assigned_timesheet_template)

    # Check if timesheet template needs to be updated
    if (not skip_timesheet and dag_run.conf['timesheet_template_name'] and \
    ...
    if dag_run.conf['schedule_policy_name'] and (not assigned_schedule_policy or (( assigned_schedule_policy and dag_run.conf['schedule_policy_uri']) \
        and (dag_run.conf['schedule_policy_uri'] != assigned_schedule_policy[0]['policySet']['uri']))):
            policy_set_to_assign.append(
                {
...
def get_default_timeoff_policy_schedule_payload(dag_run, config, for_each_loop):
    ...
    def get_reference_time_off_type_uri():
        ...
        if rail.result(for_each_loop)['timeoff_type_name'] == "[UK] Annual leave":
            placeholder_timeoff_type_name = get_placeholder_time_off_to_be_assigned(dag_run, config.ANNUAL_TO_PLACEHOLDER,
            rail.result(for_each_loop)['timeoff_type_name'])

        if rail.result(for_each_loop)['timeoff_type_name'] == "[UK] Sick leave":
...
def get_oefs(dag_run, value):
    ...
    annual_placeholder_to_name = rail.find_first_by_attr_and_get_attr(timeoff_policy_to_assign,
    ...
    if value == "add":
        if annual_placeholder_to_name:
            add_text_oef(dag_run.conf['annual_to_placeholder_def_uri'], annual_placeholder_to_name)
        if sick_placeholder_to_name:
            add_text_oef(dag_run.conf['sick_to_placeholder_def_uri'], sick_placeholder_to_name)

    if value == "update":
        if dag_run.conf['previous_annual_to_placeholder'] != annual_placeholder_to_name:
            add_text_oef(dag_run.conf['annual_to_placeholder_def_uri'], annual_placeholder_to_name)
