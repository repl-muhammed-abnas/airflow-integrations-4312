...
import rail
from crl.time_export_us_pta_v2.utils.custom_methods import EXPORT_DATE_FORMAT, get_time_export_name
from crl.time_export_us_pta_v2.utils import custom_methods, request_payload
from crl.time_export_us_pta_v2.tasks.time_export_task import time_data_export
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        description="CRL Time Export Master",
        ...
        get_processing_time_and_dates = rail.PythonOperator(
            task_id = 'get_processing_time_and_dates',
        ...
        logging_job_start_time = rail.WriteLogOperator(
            ...
            message="{{ result('get_processing_time_and_dates').process_start_time }} - Process started",
            properties={
                "log": "{{ result('get_processing_time_and_dates').process_start_time }} - Process started"
            }
        )
        ...
        get_company_codes_to_ignore = rail.RepliconServiceOperator(
            task_id='get_company_codes_to_ignore',
            endpoint="/services/ServiceCenterService1.svc/GetEnabledServiceCenters",
            data_handler=lambda res: [x['uri'] for x in list(filter(lambda item: item[
                'displayText'] in config.division_to_ignore,res))]
        )
        ...
        time_export_start, time_export_end = time_data_export(
            ...
            get_export_name="{{result('get_processing_time_and_dates').time_export_name}}",
        ...
        create_raw_timeexport_data_collection = rail.CreateCollectionOperator(
            ...
            columns={
                ...
                'Employee Type Name': 'employee_type',
                'Call-in': 'call_in'
            }
        )
        ...
        send_no_data_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon US PTA Time Data Export - No records to export - {{ current_time_in_specified_tz() }}',
        ...
        update_export_name_with_nodata = rail.RepliconServiceOperator(
            ...
            data={
                ...
                "name": "{{ result('get_processing_time_and_dates').no_data_time_export_name}}"
            },
        )
        ...
        log_to_sumo_blank_export = rail.SendToSumoOperator(
            ...
            data={
                'job_start_time': '{{ result("get_processing_time_and_dates").process_start_time }}',
                'job_end_time': f'{OPEN_BRACKETS} current_time_in_specified_tz("{config.time_zone}", "%Y-%m-%dT%H:%M:%S") {CLOSE_BRACKETS}',
                'export_file_name': '{{ result("get_processing_time_and_dates").no_data_time_export_name }}.json',
        ...
        send_invalid_records_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_email,
            subject='{{ get_company_key() }} | Replicon US PTA Time Data Export - Invalid records found - {{ current_time_in_specified_tz() }}',
        ...
        filter_raw_timeexport_data = rail.QueryCollectionOperator(
            ...
            query= f"""SELECT *,
                        CASE
                            WHEN
                                (NULLIF(short_id, '') IS NOT NULL AND call_in NOT IN {config.call_in_dropdowns_to_ignore} AND
                                NULLIF(timeoff_type_description, '') IS NULL AND ((employee_type IN {config.project_employee_types} AND
                                business_unit_name NOT IN {config.business_unit_names}) OR
                                (employee_type IN {config.non_project_employee_types})))
                                THEN 'premium'
                            WHEN
                                (project_user = 'Yes' AND NULLIF(short_id, '') IS NOT NULL AND
                                ((NULLIF(timeoff_type_description, '') IS NULL) OR
                                (NULLIF(timeoff_type_description, '') IS NOT NULL AND business_unit_name NOT IN {config.business_unit_names})) OR
                                ...
                                THEN 'non_premium'
                            ELSE 'not_valid_record'
                        END AS condition_met
                    FROM mandatory_data WHERE condition_met != 'not_valid_record'""",
            name='final_data'
        )
        ...
        logging_file_creation = rail.WriteLogOperator(
            ...
            "log_{{result('get_processing_time_and_dates').time_export_name}}" + ".txt",
            properties={
        ...
        upload_export_data_to_sftp = rail.SFTPUploadFileOperator(
            ...
            '/{{result("get_processing_time_and_dates").time_export_name}}' + '.json'
        )
        ...
        send_success_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_email,
            subject='{{ get_company_key() }} | Replicon US PTA Time Data Export - Completed Successfully - {{ current_time_in_specified_tz() }}',
        ...
        log_to_sumo_valid_export = rail.SendToSumoOperator(
            ...
            data={
                'job_start_time': '{{ result("get_processing_time_and_dates").process_start_time }}',
                'job_end_time': f'{OPEN_BRACKETS} current_time_in_specified_tz("{config.time_zone}", "%Y-%m-%dT%H:%M:%S") {CLOSE_BRACKETS}',
                'export_file_name': '{{ result("get_processing_time_and_dates").time_export_name }}.json',
                'export_filepath': config.timeexport_upload_input_filepath,
                'numberofrecords': "{{ result('format_raw_export_data', 'length')}}",
                'location': 'USA'
            },
        ...
        create_log >> get_processing_time_and_dates >> logging_job_start_time >> get_allowed_location_uris >> \
            get_company_codes_to_ignore >> time_export_download_script
