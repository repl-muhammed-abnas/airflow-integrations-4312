...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.crl_timeoff_balance_export_biweekly_master,
        ...
        get_file_name = rail.PythonOperator(
            ...
            python_callable=lambda: python_callable.get_file_name_biweekly(config)
        )

        get_report_details = rail.RepliconReportDetailsOperator(
            task_id='get_report_details',
            report_name=config.timeoff_report_name_biweekly,
        )

        get_as_of_date = rail.PythonOperator(
            task_id="get_as_of_date",
            python_callable=python_callable.get_last_saturday_of_december,
        )
                        ...
                        "filterValues": [
                            {
                                "reportFilterUri": rail.find_first_by_attr_and_get_attr(rail.result("get_report_details")[
                                    'filterConfiguration']['enabledFilters'], 'displayText', config.as_of_date_filter_name, 'uri'),
                                "value": "DateRange"
                            },
                            {
                                "reportFilterUri": rail.find_first_by_attr_and_get_attr(rail.result("get_report_details")[
                                    'filterConfiguration']['enabledFilters'], 'displayText', config.as_of_date_filter_name, 'uri'),
                                "value": rail.result('get_as_of_date')
                            },
                            {
                                "reportFilterUri": rail.find_first_by_attr_and_get_attr(rail.result("get_report_details")[
                                    'filterConfiguration']['enabledFilters'], 'displayText', config.as_of_date_filter_name, 'uri'),
        ...
        send_email_for_no_payout_data = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Biweekly Sick Payout Export - No Records Found for USA - {{result("process_start_time").start_time}}',
            params={
                'export_type': 'Biweekly'
            },
        ...
        send_email_for_sftp_failure = rail.EmailOperator(
            ...
            bcc=config.internal_logs_email,
            # pylint: disable=line-too-long
            subject='{{ get_company_key() }} | Biweekly Sick Payout Export - Failed while uploading the file to SFTP - {{result("process_start_time").start_time}}',
            params={
        ...
        send_email_for_export_copmpletion = rail.EmailOperator(
            ...
            bcc=config.internal_logs_email,
            # pylint: disable=line-too-long
            subject='{{get_company_key()}} | Biweekly Sick Payout Export completed for '+config.export_location+' at {{result("process_start_time").start_time}}',
            params={
                'output_filepath': config.output_filepath,
                'export_type': 'Biweekly'
            },
        ...
        can_process_run >> process_start_time >> get_file_name >> get_report_details >> get_as_of_date >> run_report_entry
