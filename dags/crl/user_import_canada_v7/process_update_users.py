...
from crl.user_import_canada_v7.tasks.process_supervisor import process_supervisor_assignment_task_group
    ...
    for idx in range(0, config.BATCH_COUNT):
        ...
        with rail.create_airflow_dag(
            ...
            is_status_active = rail.IfOperator(
                ...
                yes_task="is_contingent_or_usa_user",
            ...
            is_status_unpaid_leave = rail.IfOperator(
                ...
                yes_task="update_required_udfs_and_timesheet_period",
                no_task="process_user_disable"
            )

            update_required_udfs_and_timesheet_period = rail.RepliconServiceOperator(
                task_id='update_required_udfs_and_timesheet_period',
                endpoint='/services/ImportService1.svc/ApplyUserModifications3',
                data= request_payload.update_required_udfs_and_timesheet_period_payload,
            )

            process_special_timeoff_type_accruals= rail.TriggerDagRunOperator(
                task_id='process_special_timeoff_type_accruals',
                trigger_dag_id=config.process_timeoff_type_special_accrual_dagid,
                conf=lambda dag_run:{
                    ...
                    'assigned_event': rail.find_first_by_attr_and_get_attr(rail.result('get_user_info')['userDetails']['customFieldValues'],
                            'customField.displayText', 'Event', 'text') if rail.result('get_user_info')[
                    'userDetails']['customFieldValues'] else null,
                    'assigned_event_reason_code':rail.find_first_by_attr_and_get_attr(rail.result('get_user_info')['userDetails']['customFieldValues'],
                            'customField.displayText', 'Event Reason', 'text') if rail.result('get_user_info')[
                    'userDetails']['customFieldValues'] else null,
                    "time_off_types_to_assign": request_payload.get_time_off_to_assign_based_on_mapper(config.TIMEOFF_TYPE_MAPPER,dag_run),
            ...
            wait_for_process_special_timeoff_type_accruals = rail.WaitForDagRunsSensor(
                task_id='wait_for_process_special_timeoff_type_accruals',
                dag_runs='{{ result("process_special_timeoff_type_accruals") }}',
            ...
            is_contingent_or_usa_user = rail.IfOperator(
                task_id = "is_contingent_or_usa_user",
                test=lambda dag_run: dag_run.conf['is_contingent'] == 'Y' or dag_run.conf['location_full_path'].split('|')[0] == 'USA',
            ...
            is_end_date_present = rail.IfOperator(
                ...
                no_task="log_contigent_or_usa_user"
            )

            log_contigent_or_usa_user = rail.WriteLogOperator(
                task_id = 'log_contigent_or_usa_user',
                log = '{{ dag_run.conf.user_log }}',
                message = "User not Updated because Contingent Worker/USA user",
                severity='Exception',
                properties ={
                    "employee_id": "{{ dag_run.conf.emp_id }}",
                    "last_name": "{{ dag_run.conf.last_name }}",
                    ...
                    "action": "Update",
                    "status": "Exception",
                    'details': "User not Updated because Contingent Worker/USA user",
                }
            )
            ...
            get_time_off_types_to_assign = rail.PythonOperator(
                ...
                python_callable= lambda dag_run: request_payload.get_time_off_to_assign_based_on_mapper(config.TIMEOFF_TYPE_MAPPER,dag_run)
            )

            def time_off_type_assignment_update_rehire_user_trigger_id(dag_run):
                if dag_run.conf['modulo'] ==0:
                    return config.process_timeoff_type_assignment_update_rehire_user_dagid
                return f"{config.process_timeoff_type_assignment_update_rehire_user_dagid}_batch_{dag_run.conf['modulo']}"
            ...
            process_time_off_type_assignment_update_rehire_user = rail.TriggerDagRunForEachItemOperator(
                ...
                conf=lambda dag_run:{
                    ...
                    "location_level_3":dag_run.conf['location_full_path'].split('|')[2],
                    "start_date": dag_run.conf['start_date'],
                    "adjusted_hire_date":dag_run.conf['adjusted_start_date'],
                    ...
                    "previous_full_part": rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
                            ...
                            'userDetails']['customFieldValues'] else null,
                    "previous_reg_temp": rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
            ...
            has_any_error_or_exceptions_present_update_rehire_user = rail.IfOperator(
                ...
                no_task='process_special_timeoff_type_accrual'
            )
            ...
            process_special_timeoff_type_accrual= rail.TriggerDagRunOperator(
                task_id='process_special_timeoff_type_accrual',
                trigger_dag_id=config.process_timeoff_type_special_accrual_dagid,
                conf=lambda dag_run:{
                    "emp_id": dag_run.conf['emp_id'],
                    "emp_status": dag_run.conf['emp_status'],
            ...
            wait_for_process_special_timeoff_type_accrual = rail.WaitForDagRunsSensor(
                task_id='wait_for_process_special_timeoff_type_accrual',
                dag_runs='{{ result("process_special_timeoff_type_accrual") }}',
                execution_timeout=timedelta(days=config.execution_timeout_days),
            )

            # V2.7 - Personal Days proration (outbound + return).
            # Only wired in instances that define process_personal_days_proration_dagid.
            # Two trigger points are needed because process_update_users has two
            # branches that never converge:
            #   - Active branch (is_status_unpaid_leave='No' >> is_status_active='Yes')
            #     -> child DAG handles return mode + no-op cases
            #   - Unpaid Leave branch (is_status_unpaid_leave='Yes')
            #     -> child DAG handles outbound mode (Active->Unpaid Leave transition)
            # Both triggers point at the same child DAG; the determine_mode step
            # internally decides what (if anything) to do based on conf.
            personal_days_proration_enabled = bool(getattr(config, 'process_personal_days_proration_dagid', None))
            process_personal_days_proration = None
            wait_for_process_personal_days_proration = None
            process_personal_days_proration_unpaid_leave = None
            wait_for_process_personal_days_proration_unpaid_leave = None

            def _personal_days_proration_conf(dag_run):
                return {
                    'emp_id': dag_run.conf['emp_id'],
                    'first_name': dag_run.conf['first_name'],
                    'last_name': dag_run.conf['last_name'],
                    ...
                    'assigned_change_effective_date': rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
                            'customField.displayText', 'Change Effective Date', 'date') if rail.result('get_user_info')[
                            'userDetails']['customFieldValues'] else null,
                    # V2.7 - dedicated Leave Start / End Date UDFs, written only on status transitions
                    # to/from Unpaid Leave (see request_payload.get_udfs* helpers). These supersede
                    # the general Change Effective Date for duration computation in the proration child DAG.
                    'assigned_leave_start_date': rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
                            'customField.displayText', 'Leave Start Date', 'date') if rail.result('get_user_info')[
                            'userDetails']['customFieldValues'] else null,
                    'assigned_leave_end_date': rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
                            'customField.displayText', 'Leave End Date', 'date') if rail.result('get_user_info')[
                            'userDetails']['customFieldValues'] else null,
                }

            if personal_days_proration_enabled:
                # Triggered from the Active branch (handles return-from-leave + no-op cases).
                process_personal_days_proration = rail.TriggerDagRunOperator(
                    task_id='process_personal_days_proration',
                    trigger_dag_id=config.process_personal_days_proration_dagid,
                    conf=_personal_days_proration_conf,
                    execution_timeout=timedelta(days=config.execution_timeout_days),
                    retries=0,
                )

                wait_for_process_personal_days_proration = rail.WaitForDagRunsSensor(
                    task_id='wait_for_process_personal_days_proration',
                    dag_runs='{{ result("process_personal_days_proration") }}',
                    execution_timeout=timedelta(days=config.execution_timeout_days),
                )

                # Triggered from the Unpaid Leave branch (handles outbound mode -
                # the only branch a user transitioning Active -> Unpaid Leave traverses).
                process_personal_days_proration_unpaid_leave = rail.TriggerDagRunOperator(
                    task_id='process_personal_days_proration_unpaid_leave',
                    trigger_dag_id=config.process_personal_days_proration_dagid,
                    conf=_personal_days_proration_conf,
                    execution_timeout=timedelta(days=config.execution_timeout_days),
                    retries=0,
                ...
                wait_for_process_personal_days_proration_unpaid_leave = rail.WaitForDagRunsSensor(
                    task_id='wait_for_process_personal_days_proration_unpaid_leave',
                    dag_runs='{{ result("process_personal_days_proration_unpaid_leave") }}',
                    execution_timeout=timedelta(days=config.execution_timeout_days),
                )
            ...
            is_status_unpaid_leave >> rail.Label("Yes") >> update_required_udfs_and_timesheet_period >> process_special_timeoff_type_accruals
            if personal_days_proration_enabled:
                process_special_timeoff_type_accruals >> wait_for_process_special_timeoff_type_accruals \
                    >> process_personal_days_proration_unpaid_leave \
                    >> wait_for_process_personal_days_proration_unpaid_leave >> catch_and_log_errors
            else:
                process_special_timeoff_type_accruals >> wait_for_process_special_timeoff_type_accruals >> catch_and_log_errors

            process_user_disable >> wait_for_process_user_disable >> catch_and_log_errors
            is_status_active >> rail.Label("Yes") >> is_contingent_or_usa_user >> rail.Label('No') >> is_previously_suspended_leave_or_non_live_transfer_employee
            ...
            is_contingent_or_usa_user >> rail.Label('Yes') >>  is_end_date_present >> rail.Label("No") >> log_contigent_or_usa_user >> catch_and_log_errors
            ...
            get_current_udf_values >> get_effective_user_groupmembership >> get_assigned_policy_to_user >> get_assigned_place_to_user
            get_assigned_place_to_user >> apply_user_modifications

            apply_user_modifications >> is_remove_hrbp_permossion_set >> rail.Label('No') >> impersonate_and_create_interactive_session

            is_remove_hrbp_permossion_set >> rail.Label('Yes') >> remove_hrbp_permission_set >> impersonate_and_create_interactive_session
            ...
            has_any_error_or_exceptions_present_update_rehire_user >> rail.Label('No') >> process_special_timeoff_type_accrual
            if personal_days_proration_enabled:
                process_special_timeoff_type_accrual >> wait_for_process_special_timeoff_type_accrual >> process_personal_days_proration
                process_personal_days_proration >> wait_for_process_personal_days_proration >> log_user_completion
            else:
                process_special_timeoff_type_accrual >> wait_for_process_special_timeoff_type_accrual >> log_user_completion
