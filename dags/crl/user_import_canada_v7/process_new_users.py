...
from crl.user_import_canada_v7.utils import request_payload
    ...
    for idx in range(0, config.BATCH_COUNT):
        ...
        with rail.create_airflow_dag(
            ...
            is_enddate_available = rail.IfOperator(
                ...
                no_task="is_contingent_or_usa_user"
            )
            ...
            is_contingent_or_usa_user = rail.IfOperator(
                ...
                yes_task="add_contingent_or_usa_user",
                no_task="add_new_user"
            )

            add_contingent_or_usa_user = rail.RepliconServiceOperator(
                task_id="add_contingent_or_usa_user",
                endpoint="/services/importService1.svc/PutUser3",
                data=request_payload.get_put_contingent_or_usa_user_payload
            )

            remove_timeoff_assignment = rail.RepliconServiceOperator(
                task_id="remove_timeoff_assignment",
                endpoint="/services/TimeOffService1.svc/PutTimeOffTypeAssignmentsForUser",
                data=lambda: request_payload.get_remove_timeoff_payload('add_contingent_or_usa_user')
            )

            is_contingent_worker = rail.IfOperator(
                task_id = "is_contingent_worker",
                test=lambda dag_run: dag_run.conf['is_contingent'] == 'Y',
                yes_task="stop_user_notification_preferences",
            ...
            is_canada_user = rail.IfOperator(
                task_id='is_canada_user',
                test=lambda dag_run: bool(dag_run.conf['location_full_path'].split('|')[0] == 'CAN'),
                yes_task="update_language_canada_user",
                no_task="is_supervisor_in_feed_file"
            )

            update_language_canada_user = rail.RepliconServiceOperator(
                task_id='update_language_canada_user',
                endpoint="/services/InternationalizationService1.svc/UpdateLanguageForUser",
                data=lambda: {
                    "userUri": rail.result('add_new_user')['uri'],
                    "languageUri": config.CANADA_LANGUAGE_URI
                }
            )
            ...
            def time_off_type_assignment_new_user_trigger_id(dag_run):
                if dag_run.conf['modulo'] ==0:
                    return config.process_timeoff_type_assignment_new_user_dagid
                return f"{config.process_timeoff_type_assignment_new_user_dagid}_batch_{dag_run.conf['modulo']}"
            ...
            is_enddate_available >> rail.Label('No') >> is_contingent_or_usa_user

            is_contingent_or_usa_user >> rail.Label('No') >> add_new_user
            is_contingent_or_usa_user >> rail.Label('Yes') >> add_contingent_or_usa_user >> remove_timeoff_assignment
            remove_timeoff_assignment >> is_contingent_worker

            is_contingent_worker >> rail.Label('Yes') >> stop_user_notification_preferences >> log_user_completion
            is_contingent_worker >> rail.Label('No') >> log_user_completion
            ...
            update_default_time_off_type_for_bookings >> is_canada_user
            is_canada_user >> rail.Label('Yes') >> update_language_canada_user >> is_supervisor_in_feed_file
            is_canada_user >> rail.Label('No') >> is_supervisor_in_feed_file
