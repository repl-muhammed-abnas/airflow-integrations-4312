...
from crl.user_import_canada_v7.tasks.get_user_prereqs import get_user_prereqs_task_group
...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        send_blank_payload_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | User Import Canada - no records in payload - {{ current_time_in_specified_tz() }}',
        ...
        query_invalid_records = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM input_data_collection WHERE NULLIF(emp_id, '') IS NULL or
                    ...
                    or (is_contingent ='N' and NULLIF(adjusted_start_date, '') IS NULL) or ( is_contingent ='N' and NULLIF(job_code, '') IS NULL)
                    ...
                    or (is_contingent ='N' and pay_type NOT IN ('Hourly','Salaried','Exception Hourly')) or (is_contingent ='N' and full_part ='Per Diem')"""
        )
        ...
        query_valid_records = rail.QueryCollectionOperator(
            ...
            name='valid_records',
            query="""SELECT ROW_NUMBER() OVER(ORDER BY ROWID) AS record_id,* FROM input_data_collection WHERE NULLIF(emp_id, '') IS NOT NULL
                    ...
                    and NULLIF(start_date, '') IS NOT NULL and (is_contingent ='Y' or NULLIF(adjusted_start_date, '') IS NOT NULL)
                    ...
                    (is_contingent ='Y' or pay_type IN ('Hourly','Salaried','Exception Hourly')) and (is_contingent ='Y' or full_part !='Per Diem')"""
        )
        ...
        query_disable_user_records = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT * FROM valid_records WHERE emp_status IN {tuple(config.DISABLE_STATUS)}"""
        )
        ...
        process_disable_users = rail.trigger_parallel_dagrun(
            ...
            conf= lambda item: request_payload.get_process_users_conf(item, config.USER_MAPPER, config.ACTIVE_STATUS, config.BATCH_COUNT, config.instance),
            execution_timeout=timedelta(days=config.execution_timeout_days),
        )
        ...
        query_active_user_records = rail.QueryCollectionOperator(
            ...
            query=f"""SELECT * FROM valid_records WHERE emp_status IN {tuple(config.ACTIVE_STATUS)}"""
        )
