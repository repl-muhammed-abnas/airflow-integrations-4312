...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'CRL payroll balance_weekly_usa {config.instance}',
        ...
        def can_process_run_test():
            ...
            return bool(rail.find_first_by_attr_and_get_attr(
                config.USA_PAYROLL_CALENDER_MAPPER_TO_USE_WEEKLY, "payroll_processing_date", current_date))
        ...
        send_email_for_no_users_data = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon payroll balance export file is skipped for ' + \
            config.location + \
        ...
        final_balance_data_to_csv = rail.WriteCSVFileOperator(
            ...
            row=lambda item: request_payload.get_balance_us_data_row(item,config.USA_PAYROLL_CALENDER_MAPPER_TO_USE_WEEKLY),
        ...
        # pylint: disable=line-too-long
        can_process_run >> rail.Label("Yes") >> get_file_name >> process_start_time >> process_start_time_ymd_format >> process_start_time_hms_format >> create_log >> \
