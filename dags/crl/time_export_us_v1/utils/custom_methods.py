...
def get_time_export_name(config):
    ...
    return {
        "time_export_name": f"Time Extract_US_{offset_time}",
        "no_data_time_export_name": f"NO_DATA_Time_Extract_US_{offset_time}"
    }
...
def format_raw_export_data_callable(item,pai_locations,pay_code_mapper):

    if not item:
        return []

    time_type = list(filter(lambda data: data['pay_code'] == item['distributed_time_type_code'], pay_code_mapper))
    pay_type = ''
    ...
    return {
        ...
        'activity_name': item['activity_name'] if item['network_code'] and item['activity_name'] else item[
            'default_activity'] if item['network_code'] and not item['activity_name'] and item[
                'location_name'] in pai_locations else '',
