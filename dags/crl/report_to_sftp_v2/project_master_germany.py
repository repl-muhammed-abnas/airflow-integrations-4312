...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.project_master_germany,
        description=f'CharlesRiverLaboratoriesSandbox Project Report Export Master Germany',
        ...
        trigger_foreach_report = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=config.PROJECT_REPORTS_GERMANY,
            trigger_dag_id=config.project_child,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item:{
                'report_name': item,
                'filname': get_csv_filename(config.company_key, config.germany_time_zone, item, "Germany_Project")
            }
        )
