...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.user_master_brazil,
        description=f'CharlesRiverLaboratories Brazil User Report Export Master',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=datetime(2024, 8, 1, tz=config.brazil_time_zone),
        schedule_interval=config.schedule_interval_user_brazil,
        ...
        trigger_foreach_report = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=config.USER_REPORT_BRAZIL,
            trigger_dag_id=config.user_child,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item:{
                'report_name': item,
                'filname': get_csv_filename(config.company_key, config.brazil_time_zone, item, "User")
            }
        )
