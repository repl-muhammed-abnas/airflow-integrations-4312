...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.user_master_germany,
        description=f'CharlesRiverLaboratories Germany User Report Export Master',
        company_key=config.company_key,
        replicon_conn_id=config.replicon_conn_id,
        start_date=datetime(2024, 8, 1, tz=config.germany_time_zone),
        schedule_interval=config.schedule_interval_user_germany,
        ...
        trigger_foreach_report = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=config.USER_REPORT_GERMANY,
            trigger_dag_id=config.user_child,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item:{
                'report_name': item,
                'filname': get_csv_filename(config.company_key, config.germany_time_zone, item, "User")
            }
        )
