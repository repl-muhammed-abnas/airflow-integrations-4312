...
def create_main_airflow_dag(config):
    with rail.create_airflow_dag(
        dag_id=config.project_master_uk,
        description=f'CharlesRiverLaboratoriesSandbox Project Report Export Master UK',
        ...
        trigger_foreach_report = rail.TriggerDagRunForEachItemOperator(
            ...
            retries=0,
            items=config.PROJECT_REPORTS_UK,
            trigger_dag_id=config.project_child,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item:{
                'report_name': item,
                'filname': get_csv_filename(config.company_key, config.time_zone, item, "UK_Project")
            }
        )
