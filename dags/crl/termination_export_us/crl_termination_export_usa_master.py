...
import pendulum
import rail
from crl.termination_export_us.utils import request_payload
from crl.termination_export_us.utils import python_callable
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f"CRL termination Export Master USA {config.instance}",
        ...
        def can_process_run_test():
            ...
            return bool(list(filter(lambda calendar_mapper: calendar_mapper["payroll_processing_date"] == current_date and calendar_mapper["processing_time"] == curent_hours, config.USA_PAYROLL_CALENDER_MAPPER_TO_USE)))
        ...
        get_query = rail.PythonOperator(
            task_id='get_query',
            python_callable=python_callable.get_query_data,
        ...
        has_empty_empid_data >> rail.Label(
            ...
            >> users_report_data_collection >> get_query >> query_list_in_final_payroll_collection >> has_item_data >> rail.Label(
