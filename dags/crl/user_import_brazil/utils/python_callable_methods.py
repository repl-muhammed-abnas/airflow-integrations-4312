...
def get_time_off_to_be_assigned(dag_run, config):
    ...
    for timeoff_type in config.APPLICABLE_TIME_OFF_TYPES:
        if timeoff_type in config.GLOBAL_TIME_OFF_TYPES:
            if timeoff_type == "[BRA] Recesso de Estágio":
                if dag_run.conf['employee_type_name'] == "[BRA] Interns":
                    append_timeoff_types_to_assign(timeoff_type,"NA")
            elif timeoff_type == "[BRA] Férias":
                if dag_run.conf['employee_type_name'] in ["[BRA] Regulars OT Eligible", "[BRA] Regulars", "[BRA] Young Apprentice"]:
                    append_timeoff_types_to_assign(timeoff_type,"NA")
            else:
                append_timeoff_types_to_assign(timeoff_type,"NA")
