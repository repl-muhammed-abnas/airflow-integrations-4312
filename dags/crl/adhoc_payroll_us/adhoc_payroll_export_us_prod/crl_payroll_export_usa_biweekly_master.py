...
import pendulum
import rail
from crl.adhoc_payroll_us.adhoc_payroll_export_us_prod.utils import request_payload
from crl.adhoc_payroll_us.adhoc_payroll_export_us_prod.utils import python_callable
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        schedule_interval=None, # will be triggered manually
        ...
        def can_process_run_test():
            # This is adhoc run which should be ran with the start and end date which will be passed in create pay run export
            """
            SAMPLE CONFIG
                {
                    "start_date": {
        ...
        create_payroll_download_batch = rail.RepliconServiceOperator(
            ...
            data=lambda dag_run: request_payload.get_create_payroll_download_batch_payload(
                dag_run, config.time_zone, "BIWEEKLY")
        )
        ...
        execute_payrun_batch, wait_forpayrun_batch = rail.batch_execution(
            'execute_payrun_batch', create_payrun_batch.task_id, retries=0)
