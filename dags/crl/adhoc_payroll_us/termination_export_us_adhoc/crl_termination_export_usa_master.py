...
import pendulum
import rail
from crl.adhoc_payroll_us.termination_export_us_adhoc.utils import request_payload
from crl.adhoc_payroll_us.termination_export_us_adhoc.utils import python_callable
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f"CRL termination Export Master USA Adhoc {config.instance}",
        ...
        get_file_name = rail.PythonOperator(
            ...
            pendulum.now(config.time_zone).strftime(
                "%Y%m%d%H%M%S") + "_" + "USTIME_HRMD" +
            request_payload.get_sequence(
                config.USA_PAYROLL_CALENDER_MAPPER_TO_USE, config.time_zone)+"_DUT8G2I"
        )
