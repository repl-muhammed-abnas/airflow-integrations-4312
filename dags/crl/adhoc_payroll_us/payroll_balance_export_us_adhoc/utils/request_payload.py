...
def get_export_end_date(dag_run):
    return date_parser(dag_run.conf['webhook']['data']['end_date'])

def get_balance_us_data_row(item,dag_run):

    end_date = get_export_end_date(dag_run)

    personnelnumber = item['employeeid']

    timeoff_balance = item['balance']
    ...
    return ["P2010",
            ...
            dt.strptime(str(end_date), "%Y-%m-%d %H:%M:%S").strftime(
                "%Y%m%d"),
            "",
            "",
            "",
