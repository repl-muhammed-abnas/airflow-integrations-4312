...
def get_process_users_conf(item, config):
    ...
    return {
        ...
        **{
            ...
            'ireland_location_uri': rail.find_first_by_attr_and_get_attr(rail.result('get_updated_location_grps'), 'full_path', 'Ireland', 'uri')
        }
    }
...
def get_udfs(user_status, dag_run):
    ...
    if user_status =='adduser':
        ...
        if dag_run.conf['pay_grp']:
            add_udf_field_values(definitionuri = dag_run.conf['pay_grp_def_uri'], textvalue= dag_run.conf['pay_grp_drop_uri'])
...
def can_update_timesheet_payrule_template(dag_run):
    assigned_pay_type = rail.find_first_by_attr_and_get_attr(rail.result('get_current_udf_values'),
        'customField.displayText', 'Pay Type', 'text') if rail.result('get_user_info')[
        'userDetails']['customFieldValues'] else null
    if not assigned_pay_type:
        return True
    if dag_run.conf['pay_type'] == 'Salaried' and assigned_pay_type == "Salaried":
        return False
    return True
...
def update_policy_set(dag_run):
    ...
    assigned_punch_policy = list(filter(lambda item:item['policyUri'] == "urn:replicon:policy:time-punch", rail.result("get_assigned_policy_to_user"))) if rail.result("get_assigned_policy_to_user") else []
    assigned_schedule_policy = list(filter(lambda item:item['policyUri'] == "urn:replicon:policy:shift-schedule", rail.result("get_assigned_policy_to_user"))) if rail.result("get_assigned_policy_to_user") else []

    policy_set_to_assign = []

    # Check if timesheet template needs to be updated
    if can_update_timesheet_payrule_template(dag_run) and (dag_run.conf['timesheet_template_name'] and \
...
def update_payrule_script(dag_run):
    if not can_update_timesheet_payrule_template(dag_run):
        return null
