...
import pendulum
import rail
from crl.payroll_export_us.utils import request_payload
from crl.payroll_export_us.utils import python_callable
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f"CRL Payroll Export Biweekly Master USA {config.instance}",
        ...
        send_email_for_export_started = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon Payroll Data Export started for ' + \
            config.export_location + \
            ' {{ result("process_start_time").start_time }}',
            html_content="/templates/email/export_started_biweekly.html"
        )
        ...
        get_payrun_batch_result >> update_payrun_name >> create_payrun_download_batch >> execute_payrun_download_batch \
            >> wait_for_payrun_download_batch >> get_payrun_download_batch_result >> mark_payrun_as_complete >> execute_payrun_complete_batch \
                >> wait_for_payrun_complete_batch  >> rail.Label(
                "on_success") >> download_final_payload_file_from_url
        mark_payrun_as_complete >> execute_payrun_complete_batch \
                >> wait_for_payrun_complete_batch >> rail.Label(
        ...
        has_payroll_data >> rail.Label("No") >> update_payrun_name_to_nodata >> finish_export_no_payroll_data >> send_email_for_no_payroll_data
