...
from crl.user_import_usa_v2.utils.python_callable_methods import get_placeholder_time_off_to_be_assigned
...
def get_process_users_conf(item, config):
    ...
    def get_custom_payrule_name():
        res= list(filter(lambda x:
            str(x['location_level_2']).lower() == str(item['location_full_path'].split('|')[1]).lower() and
    ...
    return {
        ...
        **{
            ...
            'payrule_name': (user_mapper_values['payrule_name']
                    if user_mapper_values and user_mapper_values['payrule_name'] else null) if not get_custom_payrule_name() else get_custom_payrule_name(),
            ...
            'payrule_script_uri': (rail.find_first_by_attr_and_get_attr(
                ...
                if  user_mapper_values and user_mapper_values['payrule_name'] else null) if not get_custom_payrule_name() else
...
def validate_rehire(dag_run):
    return not rail.result('get_user_info')['userDetails']['isEnabled'] and not dag_run.conf['end_date'] and not validate_previous_suspended_leave()
...
def validate_previous_suspended_leave():
    user_udf_values = rail.result('get_user_info')[
                    'userDetails']['customFieldValues']
    previous_emp_status = rail.find_first_by_attr_and_get_attr(user_udf_values,
        'customField.displayText', 'Employee Status', 'text')
    return bool(previous_emp_status in ["Suspended"])
