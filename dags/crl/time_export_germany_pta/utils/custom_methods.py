...
def get_time_export_name(config):
    ...
    return {
        "time_export_name": f"Time Extract_PTA_Germany_{offset_time}",
        "no_data_time_export_name": f"NO_DATA_Time_Extract_PTA_Germany_{offset_time}"
    }
...
def format_raw_export_data_callable(item, config):
    """
    Germany PTA pay type logic — same rules as regular export:

    1. Time off bookings  → pay_type from timeoff_type_description
    2. Time Type (Germany) code → lookup in pay_code_mapper
    3. Default → '0800' (Regular Hours)
    """
    if not item:
        return []
