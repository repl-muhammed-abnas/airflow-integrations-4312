...
import pendulum
import rail
from crl.time_export_germany_pta.utils import custom_methods, request_payload
from crl.time_export_germany_pta.tasks.time_export_task import time_data_export
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        description="CRL Germany PTA Time Export process time export",
        ...
        get_allowed_location_uris = rail.RepliconServiceOperator(
            ...
            data=request_payload.get_allowed_location_uris_payload(config.export_locations),
            data_handler=lambda response: custom_methods.get_filtered_allowed_location_uris(
                response,
        ...
        time_export_start, time_export_end = time_data_export(
            ...
            generate_request=lambda dag_run: request_payload.get_create_time_data_export_batch_payload(dag_run, config),
        ...
        create_raw_timeexport_data_collection = rail.CreateCollectionOperator(
            ...
            columns={
                ...
                'Time Type (Germany) (Code)': 'time_type_germany_code',
        ...
        send_no_data_email = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon Germany PTA Time Data Export - No records to export - {{ current_time_in_specified_tz() }}',
        ...
        revert_to_draft = rail.RepliconServiceOperator(
            ...
            data=lambda: request_payload.get_revert_draft_payload('time_export')
        )
        ...
        cancel_export = rail.RepliconServiceOperator(
            ...
            data=lambda: request_payload.get_cancel_timeoff_export_payload('time_export')
        )
        ...
        send_invalid_records_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_email,
            subject='{{ get_company_key() }} | Replicon Germany PTA Time Data Export - Invalid records found - {{ current_time_in_specified_tz() }}',
        ...
        filter_raw_timeexport_data = rail.QueryCollectionOperator(
            ...
            query="""SELECT * FROM mandatory_data WHERE
                    ((project_user == "Yes" AND NULLIF(short_id, '') IS NOT NULL) OR
        ...
        format_raw_export_data = rail.DataAdaptorOperator(
            ...
            columns=['employee_id', 'entry_date', 'hours', 'time_type_germany_code',
                     'timeoff_type_description', 'activity_name', 'punch_in_date',
                     'punch_out_date', 'project_code', 'task_code', 'short_id', 'punch_entry_id',
                     'transaction_type', 'network_code', 'default_activity', 'timeoff_type_name'],
            data=lambda item: custom_methods.format_raw_export_data_callable(item, config)
        )
        ...
        process_end_time = rail.PythonOperator(
            ...
            python_callable=lambda: pendulum.now(config.time_zone).strftime("%Y-%m-%dT%H:%M:%S")
        )
        ...
        send_success_email = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_email,
            subject='{{ get_company_key() }} | Replicon Germany PTA Time Data Export - Completed Successfully - {{ current_time_in_specified_tz() }}',
        ...
        has_any_blank_emp_id >> rail.Label("No") >> query_records_where_mandatory_field_missing >> has_missing_data
