...
import rail
import time
from crl.payroll_balance_export_us.utils import request_payload
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'CRL payroll balance_biweekly_usa {config.instance}',
        company_key=config.company_key,
        start_date=datetime(2022, 4, 1, tz=config.time_zone),
        schedule_interval=config.schedule_interval_biweekly,
        ...
        can_process_run = rail.IfOperator(
            ...
            yes_task="delay_filename_generation"
        )

        # Execution sleeps for 2 mins in order to avoid file naming conflict with weekly payroll balance export
        delay_filename_generation = rail.PythonOperator(
            task_id="delay_filename_generation",
        ...
        send_email_for_no_users_data = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon payroll balance export for time off balance file is skipped for ' + \
            config.location + \
        ...
        query_timeoff_balance_data = rail.QueryCollectionOperator(
            ...
            # pylint: disable=line-too-long
            query="SELECT * FROM getallbalancedata WHERE paygroup !='NYW' AND  (" +
            "(timeofftypes='[USA] Floating Holiday' AND employeetype NOT IN "+str(config.employee_types)+")" +
        ...
        final_balance_data_to_csv = rail.WriteCSVFileOperator(
            ...
            row=lambda item: request_payload.get_balance_us_data_row(
                item, config.USA_PAYROLL_CALENDER_MAPPER_TO_USE),
        ...
        send_email_for_export_copmpletion = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | Replicon payroll balance export file is completed  on - {{ current_time_in_specified_tz() }}',
            params={
                'output_filepath': config.output_filepath,
        ...
        send_email_for_log_upload_failure = rail.EmailOperator(
            ...
            to=config.alert_email,
            subject='{{ get_company_key() }} | Replicon payroll balance export file to SFTP failure for ' +
            config.location +
            ' location at {{ current_time_in_specified_tz() }}',
            params={
                'log_filepath': config.log_filepath,
                'location': config.location
            },
        ...
        fail_export = rail.FailOperator(
            task_id="fail_export",
            message="payroll balance file export has failed"
        )
        ...
        # pylint: disable=line-too-long
        can_process_run >> rail.Label("Yes") >> delay_filename_generation >> get_file_name >> process_start_time >> process_start_time_ymd_format >> process_start_time_hms_format >> create_log >> \
