...
def get_time_export_name(config):
    ...
    return {
        "time_export_name": f"Time Extract_PTA_CA_{offset_time}",
        "no_data_time_export_name": f"NO_DATA_Time_Extract_PTA_CA_{offset_time}"
    }
...
def format_raw_export_data_callable(item):
    ...
    return {
        ...
        'hours': item['hours'],
        'pay_type': 'WK60' if (item['timeoff_type_name'] == 'Holiday') else item['timeoff_type_description'] if item['timeoff_type_description'] else item[
