...
import rail
from crl.time_export_can_pta.utils.custom_methods import EXPORT_DATE_FORMAT, get_time_export_name
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        ...
        webhook_conf=[
            rail.WebhookConf(bearer_token_var=config.crl_time_export_bearer_token_variable)
        ]
    ) as dag:
        ...
        rail.TriggerDagRunOperator(
            ...
            conf=lambda dag_run:{
                ...
                **get_time_export_name(config),
                "start_date": dag_run.conf.get('webhook').get('data').get('start_date') if dag_run.conf else None,
                "end_date": dag_run.conf.get('webhook').get('data').get('end_date') if dag_run.conf else None
            }
        )
