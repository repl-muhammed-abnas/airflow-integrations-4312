...
def create_child_dag(config):
    with rail.create_airflow_dag(
        ...
        description=f'CRL terminationbalance_child_usa {config.instance}',
        ...
        send_email_for_no_users_data = rail.EmailOperator(
            ...
            subject='{{ get_company_key() }} | Replicon Termination balance file is skipped for {{ dag_run.conf.location }} location  on - {{ current_time_in_specified_tz() }}',
            params={
        ...
        user_report_expected_report_columns = config.user_column
        ...
        send_email_for_no_termination_balance_data = rail.EmailOperator(
            ...
            bcc=config.internal_logs_email,
            # pylint: disable=line-too-long
            subject='{{ get_company_key() }} | Replicon Termination balance file is skipped for Canada location  on - {{ current_time_in_specified_tz() }}',
            params={
                'start_date': request_payload.get_start_date_begin_of_week(),
                'end_date': request_payload.get_end_date_begin_of_week()
            },
        ...
        # pylint: disable=line-too-long
        termination_balance_expected_report_columns = config.termination_column
        ...
        compose_terminated_employees_with_headers = rail.WriteCSVFileOperator(
            task_id='compose_terminated_employees_with_headers',
            source=lambda: rail.result(
                'termination_balance_report_payload_to_csv'),
            header=[
                'User_Name',
                'Time_Off_Type',
                'Time_Off_Balance',
            ...
            row=lambda item: [
                ...
                item['Location (Current) (Full Path)'].split(
                    '/')[1].strip() if len(item['Location (Current) (Full Path)'].split('/')) > 1 else item['Location (Current) (Full Path)'],
                item['Home Location'].split(
                    '|')[1].strip() if len(item['Home Location'].split('|')) > 1 else item['Home Location']
            ]
        )
        ...
        termination_balance_report_data_collection = rail.CreateCollectionOperator(
            ...
            source='{{result("compose_terminated_employees_with_headers")}}'
        )
        ...
        query_final_payroll_collection = rail.QueryCollectionOperator(
            ...
            AND Employee_Status NOT IN ('Suspended','Unpaid Leave')
            AND Time_Off_Type IN ('[USA] Vacation')
            AND CAST(Time_off_Balance as float) > 0
            UNION ALL
            SELECT * FROM terminationbalance WHERE NULLIF(Employee_ID, '') IS NOT NULL
            AND Employee_Status NOT IN ('Suspended','Unpaid Leave')
            AND Time_Off_Type IN ('[USA] Floating Holiday')
            AND CAST(Time_off_Balance as float) > 0
            AND (Location IN ('California','Colorado','Montana','Nebraska') OR
            Home_Location IN ('California','Colorado','Montana','Nebraska'))"""
        )
        ...
        final_termination_balance_data_to_csv = rail.WriteCSVFileOperator(
            ...
            row=lambda item: request_payload.get_termination_balance_us_data_row(item,config.USA_TIMEOFF_TYPE),
        ...
        process_timeoff_template_removal = rail.TriggerDagRunForEachItemOperator(
            ...
            trigger_dag_id=config.remove_timeoff_templates_child_dag_id,
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda item: {
                'user_uri': item['useruri'],
        ...
        send_email_for_export_copmpletion = rail.EmailOperator(
            ...
            to=config.tenant_email,
            bcc=config.internal_logs_email,
            subject='{{ get_company_key() }} | Replicon Termination balance file completed  on - {{ current_time_in_specified_tz() }}',
            params={
                'output_filepath': config.output_filepath,
        ...
        log_to_sumo_valid_export = rail.SendToSumoOperator(
            ...
            data={
                ...
                'numberofrecords': "{{ result('query_valid_termination_balance_data', 'length')}}",
            },
        ...
        termination_balance_report_has_expected_columns >> rail.Label(
            "Yes") >> termination_balance_report_payload_to_csv >> compose_terminated_employees_with_headers >> termination_balance_report_data_collection
        termination_balance_report_has_expected_columns >> rail.Label(
        ...
        upload_log_data_to_sftp >> rail.Label(
            "on_success") >> finish_export >> log_to_sumo_valid_export
