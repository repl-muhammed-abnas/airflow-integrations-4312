...
def get_termination_balance_us_data_row(items, timeoff_type_mapper):
    ...
    return ["P2010",
            ...
            rail.find_first_by_attr_and_get_attr(
                timeoff_type_mapper, "time_off_type", items['Time_Off_Type'], "time_off_type_paycode"),
...
def terminationbalance_gsap_child_conf(config):
    ...
    return {
        ...
        'file_name': config.file_name_prefix + "_" + dt.utcnow().strftime("%Y%m%d%H%M%S") + "_USTIME_HRMD"+get_sequence(config.USA_PAYROLL_CALENDAR, config.time_zone)+"_DUT8G2I",
