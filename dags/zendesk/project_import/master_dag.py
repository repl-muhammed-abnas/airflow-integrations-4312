...
from zendesk.project_import.utils.custom_method import get_delta_records,get_dag_conf
...
def create_main_dag(config):
    with rail.create_airflow_dag(
        dag_id=f"standard_zendesk_connector_{config.region.replace('-', '_')}_create_updated_projects_import_master_{config.instance}",
        description=f"Zendesk Online {config.region} Create/Update Projects {config.instance}",
        ...
        get_created_tickets = rail.ZendeskAPIOperator2(
            task_id="get_created_tickets",
            zendesk_conn_id="{{ dag_run.conf.zendesk_conn_id }}",
            endpoint="/api/v2/search/export?filter[type]=ticket&query=created>{{result('get_lastsync_time').last_synctime}}Z",
            request_method="GET",
            pagination=True,
        )

        get_updated_tickets = rail.ZendeskAPIOperator2(
            task_id="get_updated_tickets",
            zendesk_conn_id="{{ dag_run.conf.zendesk_conn_id }}",
            endpoint="/api/v2/search/export?filter[type]=ticket&query=updated>{{result('get_lastsync_time').last_synctime}}Z",
            request_method="GET",
            pagination=True,
        )

        get_all_tickets = rail.PythonOperator(
            task_id="get_all_tickets",
            python_callable=lambda: get_delta_records(
                rail.result("get_created_tickets")["results"],
                rail.result("get_updated_tickets")["results"],
            )
        )
        ...
        trigger_dag_run_project_child = rail.TriggerDagRunForEachItemOperator(
            task_id="trigger_dag_run_project_child",
            retries=0,
            items=lambda: rail.result("get_all_tickets"),
            trigger_dag_id=f"standard_zendesk_connector_{config.region.replace('-', '_')}_create_updated_project_import_child{config.instance}",
            execution_timeout=timedelta(days=config.execution_timeout_days),
            conf=lambda dag_run, item:
                {
                    **get_dag_conf(dag_run),
                    "project_items": item,
                    "is_polaris_permissions_present": rail.result(
                    "is_polaris_permissions_present")
                }
        )

        wait_for_process_project_child = rail.WaitForDagRunsSensor(
            task_id="wait_for_process_project_child",
            execution_timeout=timedelta(days=config.execution_timeout_days),
            dag_runs='{{ result("trigger_dag_run_project_child") }}',
        )

        gather_project_error = rail.GatherResultsFromDagRunsOperator(
            task_id="gather_project_error",
            dag_runs="{{ result('trigger_dag_run_project_child') }}",
            dagrun_task_id="catch_project_error",
            flatten=True,
        )
        ...
        should_log_history = rail.IfOperator(
            ...
            test="{{ not(get_task_state('get_all_tickets') == 'success' and result('get_all_tickets') | length == 0)}}",
        ...
        can_run_batch_task >> rail.Label(
            "No") >> get_lastsync_time >> get_created_tickets >> get_updated_tickets
        get_updated_tickets >> get_lastsynced_time >> update_lastsync_time >> get_all_tickets >> get_my_actual_useridentity
        get_my_actual_useridentity >> is_polaris_permissions_present
        is_polaris_permissions_present >> trigger_dag_run_project_child
        trigger_dag_run_project_child >> wait_for_process_project_child
        wait_for_process_project_child >> gather_project_error >> is_project_error
        (
            is_project_error
            >> rail.Label("Yes")
            >> fail_project_error
        ...
        is_project_error >> rail.Label("No") >> should_log_history
        (
            should_log_history
            >> rail.Label("Yes")
